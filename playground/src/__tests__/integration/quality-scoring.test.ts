...
class QualityScorer {
  ...
  calculatePmatScore(metrics: QualityMetrics): PmatScore {
    ...
    const tdg = (
      productivity * this.config.weights.productivity +
  ...
  private calculateAccessibility(metrics: QualityMetrics): number {
    ...
    const parseTimeScore = this.sigmoid(
      10 - metrics.parse_time_ms, // Target: parse in under 10ms
      10
    );
  ...
  validateScore(score: PmatScore): { valid: boolean; issues: string[] } {
    ...
    // Check bounds
    const metrics = [score.productivity, score.maintainability, score.accessibility, score.testability, score.tdg];
    ...
    const expectedTdg = (
      score.productivity * this.config.weights.productivity +
...
describe('Quality Scoring System Integration', () => {
  ...
  describe('Score Validation', () => {
    ...
    it('detects out-of-bounds scores', () => {
      ...
      expect(validation.issues).toContain(expect.stringContaining('productivity score 1.5'));
      expect(validation.issues).toContain(expect.stringContaining('maintainability score -0.1'));
    });
    ...
    it('detects incorrect TDG calculation', () => {
      ...
      expect(validation.valid).toBe(false);
      expect(validation.issues).toContain(expect.stringContaining('TDG score'));
    });
