...
class PerformanceValidator {
  ...
  async validatePerformance(url: string): Promise<{
    ...
    if (scores.performance < this.thresholds.performance.score) {
      violations.push(
        `Performance score ${scores.performance} below threshold ${this.thresholds.performance.score}`,
      );
    }
    ...
    if (scores.fcp > this.thresholds.performance.fcp) {
      violations.push(
        `First Contentful Paint ${scores.fcp}ms exceeds ${this.thresholds.performance.fcp}ms`,
      );
    ...
    if (scores.lcp > this.thresholds.performance.lcp) {
      violations.push(
        `Largest Contentful Paint ${scores.lcp}ms exceeds ${this.thresholds.performance.lcp}ms`,
      );
    ...
    if (scores.cls > this.thresholds.performance.cls) {
      violations.push(
        `Cumulative Layout Shift ${scores.cls} exceeds ${this.thresholds.performance.cls}`,
      );
    ...
    if (scores.tbt > this.thresholds.performance.tbt) {
      violations.push(
        `Total Blocking Time ${scores.tbt}ms exceeds ${this.thresholds.performance.tbt}ms`,
      );
    ...
    if (scores.tti > this.thresholds.performance.tti) {
      violations.push(
        `Time to Interactive ${scores.tti}ms exceeds ${this.thresholds.performance.tti}ms`,
      );
    ...
    if (scores.accessibility < this.thresholds.accessibility.score) {
      violations.push(
        `Accessibility score ${scores.accessibility} below threshold ${this.thresholds.accessibility.score}`,
      );
    ...
    if (scores.bestPractices < this.thresholds.bestPractices.score) {
      violations.push(
        `Best practices score ${scores.bestPractices} below threshold ${this.thresholds.bestPractices.score}`,
      );
  ...
  async validateBundleSize(page: Page): Promise<{
    ...
    if (bundleSizes.totalSize > thresholds.total) {
      issues.push(
        `Total bundle size ${(bundleSizes.totalSize / 1024 / 1024).toFixed(1)}MB exceeds ${
          (thresholds.total / 1024 / 1024).toFixed(1)
        }MB limit`,
      );
    }
    ...
    if (bundleSizes.wasmSize > thresholds.wasm) {
      issues.push(
        `WASM bundle size ${(bundleSizes.wasmSize / 1024 / 1024).toFixed(1)}MB exceeds ${
          (thresholds.wasm / 1024 / 1024).toFixed(1)
        }MB limit`,
      );
    }
    ...
    if (bundleSizes.jsSize > thresholds.js) {
      issues.push(
        `JavaScript bundle size ${(bundleSizes.jsSize / 1024 / 1024).toFixed(1)}MB exceeds ${
          (thresholds.js / 1024 / 1024).toFixed(1)
        }MB limit`,
      );
    }
    ...
    if (bundleSizes.cssSize > thresholds.css) {
      issues.push(
        `CSS bundle size ${(bundleSizes.cssSize / 1024 / 1024).toFixed(1)}MB exceeds ${
          (thresholds.css / 1024 / 1024).toFixed(1)
        }MB limit`,
      );
    }
