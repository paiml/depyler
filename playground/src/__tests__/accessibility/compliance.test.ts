...
class AccessibilityValidator {
  ...
  async validateWcagCompliance(element: Element, level: 'A' | 'AA' | 'AAA' = 'AA'): Promise<{
    ...
    const results = await axe(element, {
      rules: {
        // Enable all rules for the specified level
    ...
    const score = results.violations.length === 0 ? 100 : 
  ...
  validateScreenReaderSupport(element: Element): {
    ...
    for (let i = 1; i < headingLevels.length; i++) {
      if (headingLevels[i] - headingLevels[i - 1] > 1) {
        issues.push(`Heading level skip detected: h${headingLevels[i - 1]} to h${headingLevels[i]}`);
      }
    }
  ...
  validateColorContrast(element: Element): {
    ...
    textElements.forEach((el, index) => {
      ...
      if (mockRatio < requiredRatio) {
        issues.push(`${el.tagName.toLowerCase()} at index ${index} has insufficient contrast ratio: ${mockRatio} (required: ${requiredRatio})`);
      }
    });
...
describe('Accessibility Compliance Testing', () => {
  ...
  describe('Screen Reader Support', () => {
    it('provides comprehensive screen reader support', () => {
      ...
      expect(screenReaderResults.ariaCompliance).toBeGreaterThanOrEqual(90);
  ...
  describe('Motion and Animation Accessibility', () => {
    it('respects reduced motion preferences', () => {
      const { container } = render(<EnergyGauge savings={75} breakdown={mockBreakdown} confidence={0.8} />);
