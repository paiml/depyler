...
export class QualityMonitor {
  ...
  private reportQualityGateViolation(gate: QualityGate) {
    const message = `Quality gate violation: ${gate.name} = ${gate.actual.toFixed(2)}ms (threshold: ${gate.threshold}ms)`;
  ...
  public generateReport(): string {
    ...
    report += `🏆 Quality Score: ${summary.qualityScore.toFixed(1)}% (${summary.gatesPassed}/${summary.totalGates} gates passed)\n\n`;
