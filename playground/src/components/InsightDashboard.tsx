...
export function InsightDashboard() {
  ...
  const energySavings = React.useMemo(() => {
    ...
    return Math.max(0, Math.min(100, (1 - energyData.rust.joules / energyData.python.joules) * 100));
  }, [energyData]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
            ...
            <div className="text-2xl font-bold text-blue-900">
              {(executionResult.python.execution_time_ms / executionResult.rust.execution_time_ms).toFixed(1)}×
            </div>
          ...
          <div className="bg-indigo-50 rounded-lg p-4 border border-indigo-200">
            ...
            <div className="text-2xl font-bold text-indigo-900">
              {Math.round((1 - executionResult.rust.execution_time_ms / executionResult.python.execution_time_ms) * 100)}%
            </div>
...
function EnergyTab({ energyData, savings }: { energyData: { python: EnergyEstimate; rust: EnergyEstimate }, savings: number }) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <ul className="space-y-1 text-sm text-gray-600">
              <li>CO₂ reduced: {((energyData.python.co2Grams - energyData.rust.co2Grams) * 1000).toFixed(2)}mg</li>
              <li>Energy saved: {((energyData.python.joules - energyData.rust.joules) * 1000).toFixed(2)}mJ</li>
              <li>Equivalent to: {energyData.rust.equivalentTo}</li>
            </ul>
          </div>
          <div>
            <h5 className="text-sm font-semibold text-gray-700 mb-2">Scaled Impact (1M executions)</h5>
            <ul className="space-y-1 text-sm text-gray-600">
              <li>CO₂ reduced: {((energyData.python.co2Grams - energyData.rust.co2Grams) * 1000000).toFixed(2)}g</li>
              <li>Energy saved: {((energyData.python.joules - energyData.rust.joules) * 1000000 / 3600).toFixed(2)}Wh</li>
              <li>Tree equivalent: {(((energyData.python.co2Grams - energyData.rust.co2Grams) * 1000000) / 21000).toFixed(3)} trees/year</li>
            </ul>
          </div>
        </div>
      </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">{transpileResult.quality_metrics.pmat_score.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Productivity:</span>
                  <span className="font-mono">{transpileResult.quality_metrics.productivity.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Maintainability:</span>
                  <span className="font-mono">{transpileResult.quality_metrics.maintainability.toFixed(2)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Testability:</span>
                  <span className="font-mono">{transpileResult.quality_metrics.testability.toFixed(2)}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono">{transpileResult.quality_metrics.code_complexity}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Cyclomatic:</span>
                  <span className="font-mono">{transpileResult.quality_metrics.cyclomatic_complexity}</span>
                </div>
                <div className="flex justify-between">
