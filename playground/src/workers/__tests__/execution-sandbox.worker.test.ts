...
class ExecutionSandbox {
  ...
  private measureMemory(): number {
    return 10.5; // Mock memory usage in MB
  }
  
  private estimateEnergy(language: 'python' | 'rust', executionMs: number, memoryMb: number = 0): any {
    const profiles = {
      python: {
...
describe('ExecutionSandbox', () => {
  ...
  describe('Python Execution', () => {
    ...
    it('handles Python execution errors', async () => {
      ...
      expect(result.energyEstimate.joules).toBe(0);
    });
    ...
    it('measures execution time accurately', async () => {
      ...
      expect(result.executionTimeMs).toBeLessThanOrEqual(actualTime + 5); // 5ms tolerance
    });
  });
