...
const ANTI_PATTERNS = [
  {
    ...
    suggestion: 'Consider using ast.literal_eval() for safe evaluation or refactor to avoid dynamic code execution',
  },
  {
    pattern: /exec\s*\(/,
    name: 'exec_usage',
  ...
  analyzeFunction(context: FunctionContext): AnalysisResult {
    ...
    for (const suggestionRule of ANNOTATION_SUGGESTIONS) {
      ...
      if (match) {
        ...
        if (functionDefMatch) {
          const functionLine = context.body.substring(0, context.body.indexOf(functionDefMatch[0])).split('\n').length;
