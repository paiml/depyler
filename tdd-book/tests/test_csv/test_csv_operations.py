"""Test csv module - CSV file reading and writing.

This module tests csv's functions for reading and writing CSV files,
handling different dialects, and working with dictionaries.
"""
...
class TestDictReader:
    ...
    def test_dictreader_extra_fields(self):
        ...
        reader = csv.DictReader(io.StringIO(data))
        rows = list(reader)
        # Extra value goes into None key by default
...
class TestDictWriter:
    ...
    def test_dictwriter_missing_field(self):
        ...
        writer.writerow({'name': 'Alice', 'age': 30})
        result = output.getvalue()
        # Missing city field should result in empty field
        assert 'Alice,30,' in result
...
class TestCSVSniffer:
    ...
    def test_sniffer_no_header(self):
        ...
        sniffer = csv.Sniffer()
        has_header = sniffer.has_header(sample)
        # All numeric data suggests no header
