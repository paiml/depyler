...
pub fn weighted_python_function() -> impl Strategy<Value = PythonFunctionPattern> {
    (
        ...
        prop::collection::vec(
            ("[a-z][a-z0-9_]{1,10}", "int|str|float|bool"),
            0..4
        ),
    ...
    ).prop_map(|(name, params, return_type, complexity, docstring, collections, control_flow)| {
...
impl OptimizedGenerator {
    ...
    pub fn generate_cached(&mut self, pattern: &PythonFunctionPattern) -> String {
        let key = format!("{}-{}-{}", pattern.name, pattern.parameters.len(), pattern.body_complexity);
...
fn generate_function_code(pattern: &PythonFunctionPattern) -> String {
    ...
    // Function signature
    let params: Vec<String> = pattern.parameters.iter()
        .map(|(name, type_hint)| format!("{}: {}", name, type_hint))
        .collect();
    
    let return_annotation = pattern.return_type.as_ref()
        .map(|t| format!(" -> {}", t))
        .unwrap_or_default();
...
mod tests {
    ...
    fn test_custom_python_generators() {
        ...
        for _ in 0..20 {
            ...
            match result {
                ...
                Err(e) => println!("  - Transpilation failed (expected): {}", e.to_string().chars().take(50).collect::<String>()),
            }
        }
    }
    ...
    fn test_compositional_generators() {
        ...
        for (pattern_name, min_funcs, max_funcs) in composition_patterns {
            ...
            assert!(duration.as_millis() <= expected_max_ms,
                "{} took too long: {:?} > {}ms", 
                pattern_name, duration, expected_max_ms);
        }
    }
    ...
    fn test_generator_performance() {
        ...
        for (scenario_name, count) in test_scenarios {
            ...
            println!("{}: {} generations in {:?} ({} μs/generation)", 
                scenario_name, count, duration, per_generation);
            
            // Generators should be fast
            assert!(per_generation < 10000, // Less than 10ms per generation
                "{} generator too slow: {} μs/generation", scenario_name, per_generation);
        }
