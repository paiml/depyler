...
impl MutationTester {
    ...
    fn find_and_replace_mutations(&self, code: &str, original: &str, replacement: &str, operator: MutationOperator) -> Vec<Mutation> {
        let mut mutations = Vec::new();
...
"#;
        ...
        let arithmetic_mutations = mutations.iter().filter(|m| matches!(m.operator, MutationOperator::ArithmeticOperatorReplacement)).count();
        let relational_mutations = mutations.iter().filter(|m| matches!(m.operator, MutationOperator::RelationalOperatorReplacement)).count();
    ...
    fn test_mutation_testing() {
        ...
        let test_cases = vec![
            ...
            ("def logical(p: bool, q: bool) -> bool: return p and q", "Should work with logical ops"),
        ];
    ...
    fn test_comprehensive_mutation_testing() {
        ...
        assert!(results.mutation_score >= 0.0 && results.mutation_score <= 1.0, "Score should be between 0 and 1");
        assert_eq!(
...
"#;
        ...
        let operator_counts: HashMap<_, _> = mutations.iter()
        ...
        println!("Sample mutation kill rate: {}/{} ({:.1}%)", 
            killed_count, 
            sample_size,
    ...
    fn test_mutation_performance_with_caching() {
        ...
        assert!(speedup_ratio >= 0.8, "Caching should not significantly slow down generation");
