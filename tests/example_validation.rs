                ...
                match fs::read_to_string(example_file) {
                    Ok(content) => {
                        ...
                        assert!(result.is_ok() || result.is_err(), 
                            "Example {} should be handled", example_file);
                    }
                    Err(e) => {
...
"#,
    ...
    fn validate_generated_rust_quality() {
        ...
        let quality_test_examples = vec![
            ...
            ("Error Handling", "def div_safe(a: int, b: int) -> int: return a // b if b != 0 else 0"),
        ];
        ...
        for (name, code) in quality_test_examples {
            match pipeline.transpile(code) {
                Ok(rust_code) => {
                    ...
                    assert!(rust_code.len() > code.len(), "Generated code should be longer than source");
