...
"#, Duration::from_millis(500)),
        ...
        for (scenario_name, code, max_duration) in test_scenarios {
            ...
            assert!(duration <= max_duration, 
                ...
                scenario_name, duration, max_duration);
            ...
            assert!(verified_duration <= max_duration * 2, 
                "{} verified scenario took too long: {:?}", 
                scenario_name, verified_duration);
        }
    }
    ...
    fn full_test_suite_benchmark() {
        ...
        let test_categories = vec![
            ("Property Tests", run_simulated_property_tests),
            ("Integration Tests", run_simulated_integration_tests),  
            ("Edge Case Tests", run_simulated_edge_case_tests),
            ("Example Validation", run_simulated_example_tests),
        ];
        ...
        for (category_name, test_function) in test_categories {
            ...
            assert!(category_duration < Duration::from_secs(30), 
                "{} took too long: {:?}", category_name, category_duration);
        }
...
"#;
        ...
        for (config_name, pipeline) in configurations {
            ...
            assert!(duration < Duration::from_secs(1), 
                "{} configuration took too long: {:?}", config_name, duration);
        }
    }
    ...
    fn memory_usage_benchmark() {
        ...
        let code_5_funcs = (0..5).map(|i| format!("def f{}(x: int) -> int: return x + {}", i, i)).collect::<Vec<_>>().join("\n");
        let code_10_funcs = (0..10).map(|i| format!("def f{}(x: int) -> int: return x * {}", i, i)).collect::<Vec<_>>().join("\n");
        ...
        for (function_count, code) in complexity_tests {
            ...
            assert!(duration <= expected_max, 
                ...
                function_count, duration, expected_max);
        }
    }
    ...
    fn error_handling_benchmark() {
        ...
        for (error_type, bad_code) in error_test_cases {
            ...
            assert!(duration < Duration::from_millis(100), 
                "Error handling for '{}' took too long: {:?}", error_type, duration);
        }
    }
    ...
    fn concurrent_access_benchmark() {
        ...
        for thread_count in thread_counts {
            ...
            let handles: Vec<_> = (0..thread_count).map(|_| {
                ...
                thread::spawn(move || {
                    pipeline_clone.transpile(&code_clone)
                })
            }).collect();
            ...
            assert!(duration <= expected_max, 
                ...
                thread_count, duration, expected_max);
        }
    }
}
