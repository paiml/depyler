#!/usr/bin/env python3
"""
Systematic corpus testing using verificar-generated programs (V2)
Uses cargo build instead of rustc to properly handle dependencies
"""
...
def setup_cargo_project(test_dir: Path, rust_file: Path, cargo_toml_src: Path):
    """Setup a minimal Cargo project for testing"""
    src_dir = test_dir / "src"
    src_dir.mkdir(exist_ok=True)

    # Copy generated Rust code to src/lib.rs
    shutil.copy(rust_file, src_dir / "lib.rs")

    # Copy generated Cargo.toml
    if cargo_toml_src.exists():
        shutil.copy(cargo_toml_src, test_dir / "Cargo.toml")
    else:
        # Minimal Cargo.toml if not generated
...
def main():
    ...
    output_dir = Path(f"test_results_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    depyler = Path("/home/noah/src/depyler/target/release/depyler")

    output_dir.mkdir(exist_ok=True)

    print("🧪 Verificar Corpus Testing V2 (with Cargo)")
    print("============================================")
    ...
    for i, program in enumerate(programs, 1):
        ...
        cargo_toml = output_dir / f"test_{i}_Cargo.toml"
        log_file = output_dir / f"test_{i}.log"
        cargo_dir = output_dir / f"test_{i}_cargo"
        ...
        try:
            ...
            if result.returncode == 0 and rust_file.exists():
                ...
                setup_cargo_project(cargo_dir, rust_file, cargo_toml)

                # Step 2: Compile with Cargo
                try:
                    compile_result = subprocess.run(
                        ["cargo", "build", "--release", "--manifest-path", str(cargo_dir / "Cargo.toml")],
                        capture_output=True,
                        text=True,
                        timeout=60
                    )
                    ...
                    if compile_result.returncode == 0:
                        ...
                        error_patterns = {
                            ...
                            'E0433_unresolved_path': r'error\[E0433\]',
                        }
    ...
    summary = f"""Verificar Corpus Testing Summary (V2 - Cargo Build)
=====================================================
