    ...
    fn benchmark_property_test_execution() {
        ...
        for (i, test_case) in simple_test_cases.iter().enumerate() {
            ...
            assert!(duration < Duration::from_secs(1), 
                "Transpilation should complete within 1 second, took {:?}", duration);
        }
    }
...
"#,
    ...
    fn benchmark_property_generators() {
        ...
        quickcheck(|x: i32, y: i32| -> TestResult {
            let pipeline = DepylerPipeline::new();
            let test_code = format!("def test(a: int, b: int) -> int: return {} + {}", x.abs() % 1000, y.abs() % 1000);
        ...
        assert!(generator_duration < Duration::from_secs(30), 
...
"#),
    ...
    fn benchmark_parallel_execution() {
        ...
        let handles: Vec<_> = test_cases.iter().map(|&test_case| {
            ...
            thread::spawn(move || {
                pipeline_clone.transpile(test_case)
            })
        }).collect();
    ...
    fn performance_regression_test() {
        ...
        for (name, code, max_duration) in regression_test_cases {
            ...
            assert!(duration <= max_duration, 
                "{} took {:?}, exceeded maximum {:?}", name, duration, max_duration);
        }
    }
