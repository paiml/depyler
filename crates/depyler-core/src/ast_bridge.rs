...
fn convert_expr(expr: ast::Expr) -> Result<HirExpr> {
    match expr {
        ...
        ast::Expr::UnaryOp(u) => {
            ...
            let operand = Box::new(convert_expr(*u.operand)?);
        ...
        ast::Expr::Subscript(s) => {
            ...
            let index = Box::new(convert_expr(*s.slice)?);
        ...
        ast::Expr::Dict(d) => {
            ...
            for (k, v) in d.keys.into_iter().zip(d.values.into_iter()) {
                if let Some(key) = k {
                    let key_expr = convert_expr(key)?;
                    let val_expr = convert_expr(v)?;
        ...
        ast::Expr::Compare(c) => {
            ...
            let left = Box::new(convert_expr(*c.left)?);
            let right = Box::new(convert_expr(c.comparators.into_iter().next().unwrap())?);
            Ok(HirExpr::Binary { op, left, right })
        }
