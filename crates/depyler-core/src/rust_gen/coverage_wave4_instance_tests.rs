...
fn test_mcr_unknown_var_append_type_inference() {
    let code = transpile("def process(items):\n    items.append(42)\n    return items");
...
fn test_mcr_unknown_var_lower_string_inference() {
    let code = transpile("def process(text):\n    return text.lower()");
...
fn test_mcr_unknown_var_join_string_inference() {
    let code = transpile("def process(sep):\n    return sep.join(['a', 'b', 'c'])");
...
fn test_mcr_unknown_var_keys_dict_inference() {
    let code = transpile("def process(data):\n    return data.keys()");
...
fn test_mcr_unknown_var_intersection_set_inference() {
    let code = transpile("def process(a):\n    b = {1, 2, 3}\n    return a.intersection(b)");
...
fn test_mcr_int_from_bytes_big() {
    let code =
        transpile("def decode_int(data: bytes) -> int:\n    return int.from_bytes(data, 'big')");
...
fn test_mcr_string_method_on_non_module_var() {
    let code = transpile("def process(msg: str) -> str:\n    return msg.title()");
    assert!(!code.is_empty(), "title on str var: {}", code);
}

#[test]
fn test_mcr_string_center() {
    let code = transpile("def format_text(text: str) -> str:\n    return text.center(20)");
...
fn test_mcr_string_zfill() {
    let code = transpile("def pad_num(num: str) -> str:\n    return num.zfill(5)");
...
fn test_lg_lambda_with_captured_list_var() {
    let code = transpile("def make_checker(allowed: list):\n    return lambda x: x in allowed");
...
fn test_lg_fstring_with_collection_debug_format() {
    let code = transpile("def show_items(items: list) -> str:\n    return f'Items: {items}'");
    assert!(!code.is_empty(), "fstring with collection debug: {}", code);
}
...
fn test_lg_fstring_method_call_result() {
    let code = transpile("def show_split(text: str) -> str:\n    return f'Parts: {text.split()}'");
...
fn test_lg_ternary_basic() {
    let code =
        transpile("def choose(cond: bool, a: int, b: int) -> int:\n    return a if cond else b");
...
fn test_lg_ternary_float_int_coercion() {
    let code =
        transpile("def safe_div(a: float, b: float) -> float:\n    return a / b if b != 0 else 0");
...
fn test_lg_ternary_string_truthiness() {
    let code =
        transpile("def get_name(name: str) -> str:\n    return name if name else 'anonymous'");
...
fn test_lg_sorted_identity_key() {
    let code = transpile("def sort_nums(nums: list) -> list:\n    return sorted(nums)");
...
fn test_lg_sorted_with_reverse() {
    let code =
        transpile("def sort_desc(nums: list) -> list:\n    return sorted(nums, reverse=True)");
...
fn test_lg_generator_expression_basic() {
    let code =
        transpile("def gen_squares(n: int) -> list:\n    return list(x * x for x in range(n))");
...
fn test_comp_nested_list_comp() {
    ...
    assert!(code.contains("flat_map"), "nested list comp: {}", code);
}

#[test]
fn test_comp_set_comp_basic() {
    let code =
        transpile("def unique_squares(nums: list) -> set:\n    return {x * x for x in nums}");
...
fn test_comp_set_comp_with_filter() {
    let code =
        transpile("def even_set(nums: list) -> set:\n    return {x for x in nums if x % 2 == 0}");
...
fn test_comp_dict_comp_basic() {
    let code =
        transpile("def make_squares(n: int) -> dict:\n    return {i: i * i for i in range(n)}");
...
fn test_comp_dict_comp_with_filter() {
    ...
    assert!(code.contains("filter"), "dict comp with filter: {}", code);
}

#[test]
fn test_comp_dict_comp_from_items() {
    let code =
        transpile("def swap_dict(d: dict) -> dict:\n    return {v: k for k, v in d.items()}");
...
fn test_cons_list_with_strings() {
    let code = transpile("def make_list() -> list:\n    return ['hello', 'world']");
    assert!(code.contains("to_string"), "list with strings: {}", code);
}

#[test]
fn test_cons_list_with_none_elements() {
    let code = transpile("def make_optional_list() -> list:\n    return [1, None, 3]");
    assert!(code.contains("Some") || code.contains("None"), "list with None: {}", code);
}

#[test]
fn test_cons_tuple_basic() {
    let code = transpile("def make_pair(a: int, b: str):\n    return (a, b)");
...
fn test_cons_set_with_strings() {
    ...
    assert!(code.contains("to_string"), "set with strings: {}", code);
}

#[test]
fn test_cons_frozenset_basic() {
    let code = transpile("def make_frozen():\n    return frozenset([1, 2, 3])");
...
fn test_cons_set_empty() {
    let code = transpile("def make_empty_set() -> set:\n    return set()");
    assert!(!code.is_empty(), "empty set: {}", code);
}

#[test]
fn test_cons_tuple_with_string_literal() {
    let code = transpile("def record() -> tuple:\n    return (1, 'hello', 3.14)");
    assert!(code.contains("to_string"), "tuple with string literal: {}", code);
}

#[test]
fn test_cons_list_with_float_coercion() {
    let code = transpile("def make_floats() -> list:\n    x: list = [0, 1, 2]\n    return x");
...
fn test_dc_dict_nested() {
    let code = transpile("def make_nested() -> dict:\n    return {'outer': {'inner': 1}}");
    assert!(!code.is_empty(), "nested dict: {}", code);
}

#[test]
fn test_dc_dict_homogeneous_int() {
    let code = transpile("def make_int_dict() -> dict:\n    return {'a': 1, 'b': 2, 'c': 3}");
...
fn test_id_file_read_no_args() {
    let code = transpile("def read_file(f) -> str:\n    return f.read()");
    assert!(code.contains("read_to_string"), "file read(): {}", code);
}

#[test]
fn test_id_file_read_with_size() {
    let code = transpile("def read_chunk(f, size: int) -> bytes:\n    return f.read(size)");
    assert!(code.contains("read"), "file read(size): {}", code);
}

#[test]
fn test_id_file_readlines() {
    let code = transpile("def get_lines(f) -> list:\n    return f.readlines()");
    assert!(code.contains("BufReader"), "file readlines: {}", code);
}

#[test]
fn test_id_file_readline() {
    let code = transpile("def get_line(f) -> str:\n    return f.readline()");
    assert!(code.contains("read_line"), "file readline: {}", code);
}

#[test]
fn test_id_file_write() {
    let code = transpile("def write_file(f, content: str):\n    f.write(content)");
    assert!(code.contains("write_all"), "file write: {}", code);
}

#[test]
fn test_id_file_close() {
    let code = transpile("def close_file(f):\n    f.close()");
    assert!(!code.is_empty(), "file close: {}", code);
}

#[test]
fn test_id_path_stat() {
    let code = transpile("def get_stat(path):\n    return path.stat()");
    assert!(code.contains("metadata"), "path.stat(): {}", code);
}

#[test]
fn test_id_path_absolute() {
    let code = transpile("def get_abs(path):\n    return path.absolute()");
    assert!(code.contains("canonicalize"), "path.absolute(): {}", code);
}

#[test]
fn test_id_path_resolve() {
    let code = transpile("def resolve_path(path):\n    return path.resolve()");
    assert!(code.contains("canonicalize"), "path.resolve(): {}", code);
}

#[test]
fn test_id_datetime_isoformat() {
    let code = transpile("def to_iso(dt) -> str:\n    return dt.isoformat()");
    assert!(!code.is_empty(), "datetime isoformat: {}", code);
}

#[test]
fn test_id_datetime_strftime() {
    let code = transpile("def format_date(dt) -> str:\n    return dt.strftime('%Y-%m-%d')");
    assert!(!code.is_empty(), "datetime strftime: {}", code);
}

#[test]
fn test_id_datetime_timestamp() {
    let code = transpile("def to_unix(dt) -> float:\n    return dt.timestamp()");
    assert!(!code.is_empty(), "datetime timestamp: {}", code);
}

#[test]
fn test_id_csv_writerow() {
    let code = transpile("def write_row(writer, row: dict):\n    writer.writerow(row)");
    assert!(code.contains("serialize"), "csv writerow: {}", code);
}

#[test]
fn test_id_csv_writeheader() {
    let code = transpile("def write_header(writer):\n    writer.writeheader()");
...
fn test_id_string_decode() {
    let code = transpile("def from_bytes(data) -> str:\n    return data.decode()");
...
fn test_ac_type_name_pattern() {
    let code = transpile("def get_type(x) -> str:\n    return type(x).__name__");
    assert!(code.contains("type_name"), "type(x).__name__: {}", code);
}
...
fn test_ac_pathlib_name() {
    ...
    assert!(code.contains("file_name"), "pathlib path.name: {}", code);
}
...
fn test_ac_pathlib_suffix() {
    ...
    assert!(code.contains("extension"), "pathlib path.suffix: {}", code);
}
...
fn test_ac_pathlib_stem() {
    ...
    assert!(code.contains("file_stem"), "pathlib path.stem: {}", code);
}
...
fn test_ac_enum_constant_access() {
    ...
    assert!(code.contains("Color::RED"), "enum constant access: {}", code);
}
...
fn test_mcr_collections_defaultdict() {
    let code =
        transpile("import collections\ndef make_dd():\n    return collections.defaultdict(int)");
    assert!(!code.is_empty(), "collections.defaultdict: {}", code);
}
...
fn test_ac_exception_returncode() {
    let code = transpile("def handle_error(e):\n    return e.returncode");
    assert!(!code.is_empty(), "exception returncode: {}", code);
}

#[test]
fn test_ac_tempfile_name() {
    let code = transpile("def get_temp_name(temp):\n    return temp.name");
...
fn test_comp_set_operation_intersection() {
    ...
    assert!(code.contains("intersection"), "set intersection: {}", code);
}
...
fn test_comp_set_operation_difference() {
    ...
    assert!(code.contains("difference"), "set difference: {}", code);
}

#[test]
fn test_comp_set_operation_symmetric_diff() {
    let code = transpile("def sym_diff(a: set, b: set) -> set:\n    return a ^ b");
    assert!(code.contains("symmetric_difference"), "set xor: {}", code);
}
