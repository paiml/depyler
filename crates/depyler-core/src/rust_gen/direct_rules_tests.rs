...
fn test_method_named_match() {
    ...
    assert!(
        code.contains("r#match") || code.contains("fn match") || code.contains("struct Matcher")
    );
}
...
fn test_private_method() {
    ...
    assert!(
        code.contains("fn _private") || code.contains("fn public") || code.contains("struct Foo")
    );
}
...
fn test_multiple_classes() {
    let code = transpile("class Foo:\n    pass\n\nclass Bar:\n    pass\n\nclass Baz:\n    pass");
    assert!(
        code.contains("struct Foo") || code.contains("struct Bar") || code.contains("struct Baz")
    );
}
...
class Factory(Generic[T]):
    ...
    assert!(
        code.contains("struct Factory") || code.contains("PhantomData") || code.contains("count")
    );
}
