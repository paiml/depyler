...
pub(super) fn generate_runtime_type_items(ctx: &CodeGenContext) -> Vec<proc_macro2::TokenStream> {
    ...
    if ctx.needs_python_int_ops || nasa_mode {
        items.push(generate_python_int_ops_trait());
    }

    if ctx.needs_depyler_date || nasa_mode {
        items.push(generate_depyler_date_struct());
    }

    if ctx.needs_depyler_datetime || nasa_mode {
        items.push(generate_depyler_datetime_struct());
    }

    if ctx.needs_depyler_timedelta || nasa_mode {
        items.push(generate_depyler_timedelta_struct());
    }

    if ctx.needs_depyler_regex_match || nasa_mode {
        items.push(generate_depyler_regex_match_struct());
    }

    items
}

/// DEPYLER-1202: Generate PythonIntOps trait providing Python integer method names
/// (bit_length, bit_count) on Rust integer types.
fn generate_python_int_ops_trait() -> proc_macro2::TokenStream {
    quote! {
...
/// DEPYLER-1066: Generate DepylerDate struct wrapping Python datetime.date
/// with .day(), .month(), .year() methods.
fn generate_depyler_date_struct() -> proc_macro2::TokenStream {
    quote! {
...
/// DEPYLER-1067: Generate DepylerDateTime struct wrapping Python datetime.datetime
/// with full date/time component accessors.
fn generate_depyler_datetime_struct() -> proc_macro2::TokenStream {
    quote! {
...
/// DEPYLER-1068: Generate DepylerTimeDelta struct wrapping Python datetime.timedelta
/// with normalized components and arithmetic operators.
fn generate_depyler_timedelta_struct() -> proc_macro2::TokenStream {
    quote! {
...
/// DEPYLER-1070: Generate DepylerRegexMatch struct wrapping Python re.Match
/// with .group(), .groups(), .start(), .end(), .span() methods.
fn generate_depyler_regex_match_struct() -> proc_macro2::TokenStream {
    quote! {
