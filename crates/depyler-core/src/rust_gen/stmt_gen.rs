...
use crate::rust_gen::var_analysis::{
    extract_toplevel_assigned_symbols, extract_walrus_from_condition, find_var_position_in_tuple,
    is_var_reassigned_in_stmt, is_var_used_as_dict_key_in_stmt, is_var_used_in_stmt,
...
fn type_to_vec_annotation(t: &Type) -> Option<proc_macro2::TokenStream> {
    match t {
        ...
        Type::List(inner) => type_to_vec_annotation(inner).map(|inner_tokens| {
...
pub(crate) fn codegen_expr_stmt(
    ...
    if let HirExpr::MethodCall { object, method, args, kwargs } = expr {
        if let Some(result) =
            try_handle_argparser_expr_stmt(object, method, args, kwargs, expr, ctx)?
        {
            return Ok(result);
        }
    }
...
// Note: Unused helper functions were removed in DEPYLER-1167/1168 cleanup
...
fn apply_return_expr_transforms(e: &HirExpr, ctx: &mut CodeGenContext) -> Result<syn::Expr> {
    ...
    if let HirExpr::Var(var_name) = e {
        ...
        let is_vec_return =
            matches!(ctx.current_return_type.as_ref(), Some(Type::List(_)) | Some(Type::Tuple(_)));
...
fn check_is_already_optional(e: &HirExpr, expr_tokens: &syn::Expr, ctx: &CodeGenContext) -> bool {
    if let HirExpr::Var(var_name) = e {
        ...
        if is_get_with_default {
            ...
            matches!(method.as_str(), "get" | "ok" | "cloned" | "copied" | "pop" | "first" | "last")
        }
    } else {
...
fn codegen_return_main_exit(
    ...
    let is_main_entry_point_return =
        matches!(ctx.current_return_type.as_ref(), None | Some(Type::Int) | Some(Type::None));
...
fn extract_raise_message(exc: &HirExpr, ctx: &mut CodeGenContext) -> Result<syn::Expr> {
    match exc {
...
fn apply_truthiness_argparser_field(
    ...
    let is_optional_in_parser =
        ctx.argparser_tracker.parsers.values().any(|pi| pi.arguments.iter().any(&check_optional));
    ...
    let is_vec_field = ctx.argparser_tracker.parsers.values().any(|pi| {
        pi.arguments.iter().any(|arg| {
            arg.rust_field_name() == attr && matches!(arg.nargs.as_deref(), Some("+") | Some("*"))
        })
    });
    if is_vec_field {
        return Some(parse_quote! { !#cond_expr.is_empty() });
...
fn apply_csv_iteration(iter_expr: &mut syn::Expr, ctx: &mut CodeGenContext) -> bool {
    let mut csv_yields_results = false;
    if let Some(pattern) = ctx.stdlib_mappings.get_iteration_pattern("csv", "DictReader") {
        if let crate::stdlib_mappings::RustPattern::IterationPattern { yields_results, .. } =
            pattern
        {
...
pub(crate) fn codegen_assign_stmt(
    ...
    // CB-200 Final: Transform CSE assignments for subcommand comparisons
    if let Some(result) = try_subcommand_assign(target, value, ctx) {
        return Ok(result);
    }
    ...
    // CB-200 Final: Handle dict augmented assignment pattern (dict[key] += value)
    if let Some(result) = try_dict_augassign(target, value, ctx)? {
        return Ok(result);
    }

    // CB-200 Final: Track variable types from annotation and value expression
    if let AssignTarget::Symbol(var_name) = target {
        track_type_from_annotation(var_name, value, type_annotation, ctx);
        track_type_from_value_expr(var_name, value, type_annotation, ctx);
    }

    // CB-200 Final: Set json context when assigning to serde_json::Value dicts
    let prev_json_context = ctx.in_json_context;
    setup_json_context(target, type_annotation, ctx);

    // CB-200 Final: Set assignment target type for dict literal Value wrapping
    let prev_assign_type = ctx.current_assign_type.take();
    setup_current_assign_type(target, type_annotation, ctx);

    let mut value_expr = value.to_rust_expr(ctx)?;

    // CB-200 Final: Clone non-Copy variables in NASA mode to preserve Python semantics
    value_expr = maybe_clone_for_nasa_mode(value, value_expr, ctx);
    ...
    // CB-200 Final: Auto-unwrap Result-returning function calls
    value_expr = maybe_unwrap_result_call(value, value_expr, ctx);
    ...
    // CB-200 Final: Handle class constructor override (early return)
    if let Some(result) =
        try_class_constructor_early_return(target, value, type_annotation, &value_expr, ctx)
    {
        return Ok(result);
    }

    // CB-200 Final: Process type annotation and apply coercions/conversions
    let (type_annotation_tokens, is_final) =
        process_type_annotation(target, value, type_annotation, &mut value_expr, ctx)?;
    // CB-200 Final: Normalize string literals and wrap Box<dyn Write> vars
    value_expr = normalize_string_and_box_wrap(target, value, type_annotation, value_expr, ctx);

    // CB-200 Final: Wrap dict value assignment in NASA mode
    let value_expr = wrap_dict_value_nasa_mode(target, value, value_expr, ctx);

    // CB-200 Final: Cast PyOps results (i64) to i32 when target is i32 in NASA mode
    let value_expr = maybe_cast_pyops_result(target, value, type_annotation, value_expr, ctx);
...
// ============================================================================
// CB-200 Final: Helpers extracted from codegen_assign_stmt
// ============================================================================

/// CB-200 Final: Try transforming CSE assignments for subcommand comparisons
fn try_subcommand_assign(
    target: &AssignTarget,
    value: &HirExpr,
    ctx: &mut CodeGenContext,
) -> Option<proc_macro2::TokenStream> {
...
/// CB-200 Final: Track type from annotation (type_annotation field)
fn track_type_from_annotation(
    var_name: &str,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
    ...
    if let Some(annot_type) = type_annotation {
        match annot_type {
            ...
            | Type::Bool => {
                ...
                if should_insert {
                    ctx.var_types.insert(var_name.to_string(), annot_type.clone());
                }
            }
            _ => {}
        }
    } else {
        // Infer type from literal value expression
        track_type_from_literal(var_name, value, ctx);
    }
}

/// CB-200 Final: Infer type from literal value expression when no annotation
fn track_type_from_literal(var_name: &str, value: &HirExpr, ctx: &mut CodeGenContext) {
    match value {
        HirExpr::Literal(Literal::Int(_)) => {
            ctx.var_types.insert(var_name.to_string(), Type::Int);
        }
        HirExpr::Literal(Literal::Float(_)) => {
            ctx.var_types.insert(var_name.to_string(), Type::Float);
        }
        HirExpr::Literal(Literal::Bool(_)) => {
            ctx.var_types.insert(var_name.to_string(), Type::Bool);
        }
...
/// CB-200 Final: Track type from value expression (match on HirExpr)
fn track_type_from_value_expr(
    var_name: &str,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
    ...
    match value {
        HirExpr::Call { func, .. } => {
            track_type_from_call(var_name, func, type_annotation, ctx);
        }
        HirExpr::Binary { .. } => {
        ...
        HirExpr::Var(source_var) => {
            if let Some(source_type) = ctx.var_types.get(source_var) {
                ctx.var_types.insert(var_name.to_string(), source_type.clone());
            }
        }
        HirExpr::List(elements) => {
            track_type_from_list(var_name, elements, type_annotation, ctx);
        }
        HirExpr::Dict(items) => {
            track_type_from_dict(var_name, items, type_annotation, ctx);
        }
        HirExpr::Set(elements) | HirExpr::FrozenSet(elements) => {
            track_type_from_set(var_name, elements, type_annotation, ctx);
        }
        HirExpr::DictComp { key, value: val, .. } => {
            let key_type = crate::rust_gen::func_gen::infer_expr_type_simple(key);
            let val_type = crate::rust_gen::func_gen::infer_expr_type_simple(val);
            ctx.var_types
                .insert(var_name.to_string(), Type::Dict(Box::new(key_type), Box::new(val_type)));
        }
        HirExpr::ListComp { element, .. } => {
            track_type_from_listcomp(var_name, element, ctx);
        }
        HirExpr::SetComp { element, .. } => {
            let elem_type = crate::rust_gen::func_gen::infer_expr_type_simple(element);
            ctx.var_types.insert(var_name.to_string(), Type::Set(Box::new(elem_type)));
        }
        HirExpr::Tuple(elements) => {
            ...
            ctx.var_types.insert(var_name.to_string(), Type::Tuple(elem_types));
        }
        HirExpr::Slice { base, .. } => {
            track_type_from_slice(var_name, base.as_ref(), ctx);
        }
        HirExpr::MethodCall { object, method, args, .. } => {
            track_type_from_method_call(var_name, object.as_ref(), method, args, ctx);
        }
        HirExpr::Index { base, .. } => {
            track_value_index_type(var_name, base.as_ref(), ctx);
        }
        _ => {
            track_type_from_catchall(var_name, value, ctx);
        }
    }
}

/// CB-200 Final: Track type from function call value
fn track_type_from_call(
    var_name: &str,
    func: &str,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
    ...
    if ctx.class_names.contains(func) {
        ctx.var_types.insert(var_name.to_string(), Type::Custom(func.to_string()));
    } else if func == "set" {
        ...
        if matches!(ret_type, Type::List(_) | Type::Dict(_, _) | Type::Set(_) | Type::Tuple(_)) {
            ctx.var_types.insert(var_name.to_string(), ret_type.clone());
        }
    } else if matches!(func, "search" | "match" | "find") {
        ctx.var_types.insert(var_name.to_string(), Type::Optional(Box::new(Type::Unknown)));
    } else if matches!(ctx.function_return_types.get(func), Some(Type::Float)) {
        ctx.var_types.insert(var_name.to_string(), Type::Float);
    }
}

/// CB-200 Final: Track type from list literal
fn track_type_from_list(
    var_name: &str,
    elements: &[HirExpr],
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
    ...
    if should_update {
        ctx.var_types.insert(var_name.to_string(), Type::List(Box::new(elem_type)));
    }
}

/// CB-200 Final: Track type from dict literal
fn track_type_from_dict(
    var_name: &str,
    ...
    let final_type = if is_optional { Type::Optional(Box::new(dict_type)) } else { dict_type };
    ctx.var_types.insert(var_name.to_string(), final_type);
}

/// CB-200 Final: Track type from set/frozenset literal
fn track_type_from_set(
    var_name: &str,
    elements: &[HirExpr],
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
...
/// CB-200 Final: Track type from list comprehension
fn track_type_from_listcomp(var_name: &str, element: &HirExpr, ctx: &mut CodeGenContext) {
...
/// CB-200 Final: Track type from slice expression
fn track_type_from_slice(var_name: &str, base: &HirExpr, ctx: &mut CodeGenContext) {
    let base_is_string = if let HirExpr::Var(base_var) = base {
    ...
    if base_is_string {
        ...
        let elem_type = if let HirExpr::Var(base_var) = base {
...
/// CB-200 Final: Track type from method call result
fn track_type_from_method_call(
    var_name: &str,
    object: &HirExpr,
    method: &str,
    args: &[HirExpr],
    ...
    if method == "get" {
        track_type_from_get_method(var_name, object, args, ctx);
    } else if matches!(method, "split" | "split_whitespace" | "splitlines") {
        ctx.var_types.insert(var_name.to_string(), Type::List(Box::new(Type::String)));
    } else if matches!(
        method,
        "upper" | "lower" | "strip" | "lstrip" | "rstrip" | "title" | "replace" | "format"
    ) {
        ctx.var_types.insert(var_name.to_string(), Type::String);
    } else if matches!(method, "find" | "search" | "match") {
        ctx.var_types.insert(var_name.to_string(), Type::Optional(Box::new(Type::Unknown)));
    } else if matches!(method, "loads" | "load") {
        track_json_loads_type(var_name, object, method, ctx);
    }
}

/// CB-200 Final: Track type from .get() method call
fn track_type_from_get_method(
    var_name: &str,
    object: &HirExpr,
    args: &[HirExpr],
    ctx: &mut CodeGenContext,
    ...
    if let HirExpr::Var(obj_var) = object {
        if let Some(Type::List(elem_type)) = ctx.var_types.get(obj_var) {
            ctx.var_types.insert(var_name.to_string(), elem_type.as_ref().clone());
        } else if args.len() == 2 {
            if let Some(Type::Dict(_, val_type)) = ctx.var_types.get(obj_var) {
                ctx.var_types.insert(var_name.to_string(), val_type.as_ref().clone());
            } else {
                ctx.var_types.insert(var_name.to_string(), Type::String);
            }
        }
...
/// CB-200 Final: Track type from catch-all (primitive literals and other expressions)
fn track_type_from_catchall(var_name: &str, value: &HirExpr, ctx: &mut CodeGenContext) {
    let should_preserve_value_type = ctx
        .var_types
        .get(var_name)
        .map(|t| matches!(t, Type::Custom(name) if name == "serde_json::Value" || name == "Value"))
        .unwrap_or(false);

    if !should_preserve_value_type {
        let inferred = crate::rust_gen::func_gen::infer_expr_type_simple(value);
        if !matches!(inferred, Type::Unknown) {
            ctx.var_types.insert(var_name.to_string(), inferred);
        }
    }
}

/// CB-200 Final: Set json context for serde_json::Value assignments
fn setup_json_context(
    target: &AssignTarget,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
) {
    ...
    if let AssignTarget::Symbol(var_name) = target {
        setup_json_context_symbol(var_name, type_annotation, ctx);
    }
    if let AssignTarget::Index { base, .. } = target {
        setup_json_context_index(base.as_ref(), ctx);
    }
}

/// CB-200 Final: Set json context for symbol targets
fn setup_json_context_symbol(
    var_name: &str,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
) {
...
/// CB-200 Final: Set json context for index targets
fn setup_json_context_index(base: &HirExpr, ctx: &mut CodeGenContext) {
    if let HirExpr::Var(base_name) = base {
        if let Some(base_type) = ctx.var_types.get(base_name) {
            ...
            if matches!(name_str, "config" | "data" | "value" | "current" | "obj" | "json") {
                ctx.in_json_context = true;
            }
        }
    }
...
/// CB-200 Final: Set current_assign_type for dict literal Value wrapping
fn setup_current_assign_type(
    target: &AssignTarget,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
) {
    setup_assign_type_from_annotation(target, type_annotation, ctx);
    setup_assign_type_from_subscript(target, ctx);
    setup_assign_type_from_var_types(target, ctx);
}

/// CB-200 Final: Set assign type from type annotation
fn setup_assign_type_from_annotation(
    target: &AssignTarget,
    type_annotation: &Option<Type>,
    ctx: &mut CodeGenContext,
) {
...
/// CB-200 Final: Set assign type from subscript chain for nested dicts
fn setup_assign_type_from_subscript(target: &AssignTarget, ctx: &mut CodeGenContext) {
    if ctx.current_assign_type.is_some() {
        return;
    }
    if let AssignTarget::Index { base, .. } = target {
...
/// CB-200 Final: Set assign type from var_types for dict/list assignments
fn setup_assign_type_from_var_types(target: &AssignTarget, ctx: &mut CodeGenContext) {
    if ctx.current_assign_type.is_some() {
        return;
    }
    if let AssignTarget::Symbol(name) = target {
        if let Some(var_type) = ctx.var_types.get(name.as_str()) {
            if let Type::Optional(inner) = var_type {
                if matches!(inner.as_ref(), Type::Dict(_, _) | Type::List(_)) {
                    ctx.current_assign_type = Some(inner.as_ref().clone());
                }
            } else if matches!(var_type, Type::Dict(_, _) | Type::List(_)) {
...
/// CB-200 Final: Clone non-Copy variables in NASA mode
fn maybe_clone_for_nasa_mode(
    value: &HirExpr,
    mut value_expr: syn::Expr,
    ctx: &CodeGenContext,
) -> syn::Expr {
    if !ctx.type_mapper.nasa_mode {
        return value_expr;
...
/// CB-200 Final: Process type annotation and apply coercions/conversions
fn process_type_annotation(
    target: &AssignTarget,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    value_expr: &mut syn::Expr,
    ctx: &mut CodeGenContext,
) -> Result<(Option<proc_macro2::TokenStream>, bool)> {
    if let Some(target_type) = type_annotation {
        process_typed_annotation(target, value, type_annotation, target_type, value_expr, ctx)
    } else {
        Ok(process_untyped_annotation(target, value, ctx))
    }
}

/// CB-200 Final: Process when there IS a type annotation
fn process_typed_annotation(
    target: &AssignTarget,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    target_type: &Type,
    ...
    // Resolve bare tuple from function return type
    let resolved_tuple = resolve_bare_tuple_owned(actual_type, value, ctx);
    let actual_type = resolved_tuple.as_ref().unwrap_or(actual_type);

    // NOTE: Class constructor override is handled in codegen_assign_stmt before this function

    // Infer collection element types for unresolved types
    let inferred_collection_type = infer_unresolved_collection_type(actual_type, value);
    let actual_type = inferred_collection_type.as_ref().unwrap_or(actual_type);

    // Handle None value → Option<T> wrapping
    ...
    if needs_option_wrap {
        if let AssignTarget::Symbol(var_name) = target {
            ctx.var_types.insert(var_name.clone(), Type::Optional(Box::new(actual_type.clone())));
        }
    }

    // Type conversion (e.g., usize to i32)
    if needs_type_conversion(actual_type, value) {
        *value_expr = apply_type_conversion(value_expr.clone(), actual_type);
    }

    // DepylerValue extraction
    apply_depyler_value_extraction(target, value, type_annotation, actual_type, value_expr, ctx);

    // String literal to String conversion
    if matches!(value, HirExpr::Literal(Literal::String(_)))
        && matches!(target_rust_type, crate::type_mapper::RustType::String)
    {
        let ve = value_expr.clone();
        *value_expr = parse_quote! { #ve.to_string() };
    }

    Ok((Some(quote! { : #target_syn_type }), is_const))
}

/// CB-200 Final: Process when there is NO type annotation
fn process_untyped_annotation(
    target: &AssignTarget,
    value: &HirExpr,
    ctx: &CodeGenContext,
...
/// CB-200 Final: Resolve bare tuple annotation from function return type
/// Returns a cloned type if resolution is needed, otherwise None
fn resolve_bare_tuple_owned(
    actual_type: &Type,
    value: &HirExpr,
    ctx: &CodeGenContext,
) -> Option<Type> {
    if matches!(actual_type, Type::Tuple(elems) if elems.is_empty()) {
        if let HirExpr::Call { func, .. } = value {
            if let Some(ret_type) = ctx.function_return_types.get(func) {
...
/// CB-200 Final: Handle class constructor override (early return from codegen_assign_stmt)
fn try_class_constructor_early_return(
    target: &AssignTarget,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    value_expr: &syn::Expr,
    ...
    // Only applies when there IS a type annotation AND value is a class constructor
    type_annotation.as_ref()?;
    let func_name = if let HirExpr::Call { func, .. } = value {
        if ctx.class_names.contains(func) {
            Some(func.as_str())
        } else {
            None
    ...
    let class_ident = syn::Ident::new(func_name, proc_macro2::Span::call_site());
    let class_type_tokens = quote! { : #class_ident };
    ...
    if let AssignTarget::Symbol(name) = target {
        ctx.var_types.insert(name.clone(), Type::Custom(func_name.to_string()));
        ...
        ctx.declare_var(name);
        let ve = value_expr.clone();
        return Some(quote! { let #mutability #ident #class_type_tokens = #ve; });
    }
    None
}

/// CB-200 Final: Infer collection type for unresolved Set/List annotations
fn infer_unresolved_collection_type(actual_type: &Type, value: &HirExpr) -> Option<Type> {
    match actual_type {
        Type::Set(elem) if matches!(**elem, Type::UnificationVar(_) | Type::Unknown) => {
            infer_set_from_call(value)
        }
        Type::List(elem) if matches!(**elem, Type::UnificationVar(_) | Type::Unknown) => {
            infer_list_from_call(value)
        }
        _ => None,
    }
}

/// CB-200 Final: Infer set element type from set() call
fn infer_set_from_call(value: &HirExpr) -> Option<Type> {
    if let HirExpr::Call { func, args, .. } = value {
            ...
            if let HirExpr::List(elems) = &args[0] {
                ...
                match inferred {
                    Type::Int | Type::Float | Type::String | Type::Bool => {
                        return Some(Type::Set(Box::new(inferred)));
                    }
                    _ => {}
                }
            }
...
/// CB-200 Final: Infer list element type from list() call
fn infer_list_from_call(value: &HirExpr) -> Option<Type> {
    if let HirExpr::Call { func, args, .. } = value {
        if func == "list" && args.len() == 1 {
            if let HirExpr::Call { func: inner_func, .. } = &args[0] {
                ...
                match inferred {
                    Type::Int | Type::Float | Type::String | Type::Bool => {
                        return Some(Type::List(Box::new(inferred)));
                    }
                    _ => {}
                }
            }
...
/// CB-200 Final: Apply DepylerValue extraction for typed assignments
fn apply_depyler_value_extraction(
    target: &AssignTarget,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    actual_type: &Type,
    ...
    let is_binary_with_primitive_target =
        is_binary_with_primitive(value, target, type_annotation, ctx);
        ...
        && !is_binary_with_primitive_target
    {
        if let Some(extraction) = get_depyler_extraction_for_type(actual_type) {
            ...
            let ve = value_expr.clone();
            *value_expr = parse_quote! { (#ve) #extraction_tokens };
        }
    }
}

/// CB-200 Final: Check if binary expression has primitive target type
fn is_binary_with_primitive(
    value: &HirExpr,
    target: &AssignTarget,
    type_annotation: &Option<Type>,
    ctx: &CodeGenContext,
) -> bool {
    if let HirExpr::Binary { op, .. } = value {
        let is_arithmetic =
...
/// CB-200 Final: Normalize string literals and wrap Box<dyn Write> vars
fn normalize_string_and_box_wrap(
    target: &AssignTarget,
    value: &HirExpr,
    type_annotation: &Option<Type>,
    mut value_expr: syn::Expr,
...
fn wrap_dict_value_nasa_mode(
    ...
    if let AssignTarget::Index { base, .. } = target {
        if matches!(value, HirExpr::Dict(_)) {
            let skip_format_wrap = should_skip_format_wrap(base.as_ref(), ctx);
            if !skip_format_wrap {
                return parse_quote! { format!("{:?}", #value_expr) };
            }
        }
    }
    value_expr
...
/// CB-200 Final: Check if format wrapping should be skipped for dict assignment
fn should_skip_format_wrap(base: &HirExpr, ctx: &CodeGenContext) -> bool {
    if let Some(val_type) = get_subscript_value_type(base, ctx) {
        return matches!(val_type, Type::Dict(_, _) | Type::Unknown | Type::UnificationVar(_));
    }
    if let HirExpr::Var(base_name) = base {
        return ctx.var_types.get(base_name).is_some_and(|t| match t {
            Type::Dict(_, val_type) if matches!(val_type.as_ref(), Type::Dict(_, _)) => true,
            Type::Dict(_, val_type)
                if matches!(val_type.as_ref(), Type::Unknown | Type::UnificationVar(_)) =>
            {
                true
            }
            _ => false,
...
fn maybe_cast_pyops_result(
    ...
    let is_pyops = matches!(
        value,
        HirExpr::Binary { op: BinOp::Add | BinOp::Sub | BinOp::Mul | BinOp::Div | BinOp::Mod, .. }
    );
...
fn infer_nasa_mode_type_annotation(
    ...
    match value {
        ...
        HirExpr::List(elems) if elems.is_empty() => infer_nasa_empty_list_type(target, ctx),
        HirExpr::List(elems) => infer_nasa_nonempty_list_type(elems, ctx),
        HirExpr::ListComp { generators, .. } => infer_nasa_listcomp_type(generators, ctx),
        HirExpr::Set(elems) => {
        ...
        HirExpr::Call { func, args, .. } if func == "set" => infer_nasa_set_call_type(args, ctx),
        HirExpr::Call { func, args, .. } if func == "list" => infer_nasa_list_call_type(args, ctx),
...
fn infer_nasa_list_call_type(
    ...
    if args.len() == 1 {
        if let HirExpr::Call { func: inner_func, .. } = &args[0] {
            if inner_func == "range" {
                return (Some(quote! { : Vec<i32> }), false);
            }
            return (None, false);
        }
        if let HirExpr::List(elems) = &args[0] {
...
fn normalize_reassign_value(value_expr: syn::Expr, ctx: &CodeGenContext) -> syn::Expr {
...
fn try_mut_option_assign(
    ...
    let is_mut_option =
        ctx.mut_option_dict_params.contains(symbol) || ctx.mut_option_params.contains(symbol);
    if !is_mut_option {
        return None;
    }
    let value_str = quote!(#value_expr).to_string();
...
fn wrap_optional_value(symbol: &str, value_expr: syn::Expr, ctx: &CodeGenContext) -> syn::Expr {
...
fn check_needs_clone(value_expr: &syn::Expr, symbol: &str, ctx: &CodeGenContext) -> bool {
    if let syn::Expr::Path(ref path) = value_expr {
        if path.path.segments.len() == 1 {
...
#[inline]
fn resolve_is_numeric_index(base: &HirExpr, index: &HirExpr, ctx: &CodeGenContext) -> bool {
    if let HirExpr::Var(base_name) = base {
        if let Some(base_type) = ctx.var_types.get(base_name) {
...
#[inline]
fn convert_dict_value_to_string(value_expr: syn::Expr, is_numeric_index: bool) -> syn::Expr {
    if !is_numeric_index {
...
pub(crate) fn codegen_assign_index(
    ...
    let final_value_expr =
        wrap_value_for_json(value_expr, needs_as_object_mut, needs_json_value_wrap, ctx);
...
fn codegen_assign_tuple_symbols(
    ...
    let generator_state_vars: Vec<_> = if ctx.in_generator {
        symbols.iter().filter(|s| ctx.generator_state_vars.contains(&s.to_string())).collect()
    } else {
        vec![]
    };
...
fn codegen_vec_split_unpack(
    ...
    let parts_name = syn::Ident::new("_split_parts", proc_macro2::Span::call_site());
...
fn is_numeric_index_expr(index: &HirExpr, ctx: &CodeGenContext) -> bool {
    match index {
        ...
        HirExpr::Var(name) => {
            ctx.var_types.get(name).map(|ty| matches!(ty, Type::Int)).unwrap_or(false)
        }
        HirExpr::Binary { .. } => true,
        _ => false,
    }
...
mod tests {
    ...
    use crate::rust_gen::expr_analysis::{
        ...
        handler_contains_raise, is_file_creating_expr, is_nested_function_recursive, is_stdio_expr,
    };
    ...
    fn test_find_var_position_in_tuple_nested() {
        let targets = vec![
            AssignTarget::Tuple(vec![AssignTarget::Symbol("inner".to_string())]),
    ...
    // GH-226: Tests updated to match current 1-arg function signature
    ...
    fn test_extract_fields_from_expr_attribute() {
        ...
        let _fields = extract_fields_from_expr(&expr);
    }
    ...
    #[test]
    fn test_extract_fields_recursive_expr() {
        let expr = HirExpr::Attribute {
            value: Box::new(HirExpr::Var("args".to_string())),
            attr: "value".to_string(),
        };
        let _fields = extract_fields_recursive(&expr);
        // Function currently returns empty vec (stub) - just verify it doesn't panic
    }

    #[test]
    fn test_extract_fields_recursive_nested() {
        let expr = HirExpr::Attribute {
            value: Box::new(HirExpr::Var("args".to_string())),
    ...
    fn test_extract_kwarg_string_not_string() {
        let kwargs = vec![("name".to_string(), HirExpr::Literal(Literal::Int(42)))];
        assert_eq!(extract_kwarg_string(&kwargs, "name"), None);
    }
    ...
    fn test_extract_kwarg_bool_not_found() {
        let kwargs = vec![("other".to_string(), HirExpr::Var("True".to_string()))];
    ...
    fn test_extract_kwarg_bool_not_bool() {
        ...
        let kwargs = vec![("flag".to_string(), HirExpr::Literal(Literal::Bool(true)))];
        assert_eq!(extract_kwarg_bool(&kwargs, "flag"), None);
    }
    ...
    fn test_contains_floor_div_in_list() {
        ...
        let expr = HirExpr::List(vec![HirExpr::Literal(Literal::Int(1)), inner]);
        assert!(contains_floor_div(&expr));
    }
    ...
    fn test_contains_floor_div_in_tuple() {
        ...
        let expr = HirExpr::Tuple(vec![HirExpr::Literal(Literal::Int(1)), inner]);
        assert!(contains_floor_div(&expr));
    }
    ...
    fn test_is_dict_with_value_type_string() {
        let t = Type::Dict(Box::new(Type::String), Box::new(Type::String));
        assert!(!is_dict_with_value_type(&t));
    }
    ...
    fn test_extract_string_literal_empty() {
        let expr = HirExpr::Literal(Literal::String(String::new()));
        assert_eq!(extract_string_literal(&expr), "");
    }
    ...
    // ================================================================
    // Session 9: Coverage improvement tests for stmt_gen
    // ================================================================

    // --- Complex assignment patterns ---

    #[test]
    fn test_s9_assign_dict_to_var() {
        let code = r#"
def make_dict() -> dict:
    d = {"key": "value"}
    return d
...
";
    ...
    #[test]
    fn test_s9_assign_tuple_unpacking() {
        let code = r"
def unpack() -> int:
    a, b = 1, 2
    return a + b
";
    ...
    #[test]
    fn test_s9_assign_tuple_unpacking_three() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn unpack_three"), "output: {}", rust);
    }

    #[test]
    fn test_s9_assign_augmented_floordiv() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn floor_assign"), "output: {}", rust);
    }

    #[test]
    fn test_s9_assign_augmented_power() {
        let code = r"
def power_assign(n: int) -> int:
    n **= 2
    return n
";
        let rust = transpile(code);
        assert!(rust.contains("fn power_assign"), "output: {}", rust);
    }

    #[test]
    fn test_s9_assign_augmented_bitand() {
        let code = r"
def bitand_assign(n: int) -> int:
    n &= 0xFF
    return n
";
        let rust = transpile(code);
        assert!(rust.contains("fn bitand_assign"), "output: {}", rust);
    }

    #[test]
    fn test_s9_assign_augmented_bitor() {
        let code = r"
def bitor_assign(n: int) -> int:
    n |= 0x01
    return n
";
        let rust = transpile(code);
        assert!(rust.contains("fn bitor_assign"), "output: {}", rust);
    }

    #[test]
    fn test_s9_assign_augmented_bitxor() {
        let code = r"
def bitxor_assign(n: int) -> int:
    n ^= 0xFF
    return n
";
        let rust = transpile(code);
        assert!(rust.contains("fn bitxor_assign"), "output: {}", rust);
    }

    // --- Return statement patterns ---

    #[test]
    fn test_s9_return_none_explicit() {
        let code = r"
def noop() -> None:
    return None
";
    ...
    #[test]
    fn test_s9_return_empty() {
        let code = r"
def early_exit(x: int) -> None:
    if x < 0:
        return
    print(x)
";
        let rust = transpile(code);
        assert!(rust.contains("fn early_exit"), "output: {}", rust);
        assert!(rust.contains("return"), "Should contain return: {}", rust);
    }

    #[test]
    fn test_s9_return_tuple() {
        let code = r"
def minmax(items: list) -> tuple:
    return (min(items), max(items))
";
        let rust = transpile(code);
        assert!(rust.contains("fn minmax"), "output: {}", rust);
    }

    #[test]
    fn test_s9_return_list_literal() {
        let code = r"
def empty_list() -> list:
    return []
";
        let rust = transpile(code);
        assert!(rust.contains("fn empty_list"), "output: {}", rust);
    }

    #[test]
    fn test_s9_return_dict_literal() {
        let code = r"
def empty_dict() -> dict:
    return {}
";
        let rust = transpile(code);
        assert!(rust.contains("fn empty_dict"), "output: {}", rust);
    }

    #[test]
    fn test_s9_return_conditional() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn sum_range"), "output: {}", rust);
    }

    #[test]
    fn test_s9_for_range_start_end() {
        let code = r"
def sum_range_start(start: int, end: int) -> int:
    total = 0
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn sum_range_start"), "output: {}", rust);
    }

    #[test]
    fn test_s9_for_range_step() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn sum_evens"), "output: {}", rust);
    }

    #[test]
    fn test_s9_for_enumerate() {
        let code = r"
...
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn format_dict"), "output: {}", rust);
    }

    #[test]
    fn test_s9_for_dict_keys() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn sum_dict_values"), "output: {}", rust);
    }

    #[test]
    fn test_s9_for_string_chars() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn find_first_negative"), "output: {}", rust);
    }

    // --- While loop patterns ---

    #[test]
    fn test_s9_while_true_break() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9_while_with_continue() {
        let code = r"
def skip_negatives(items: list) -> int:
    total = 0
    i = 0
    while i < len(items):
        i += 1
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn skip_negatives"), "output: {}", rust);
    }

    // --- If/elif/else patterns ---

    #[test]
    fn test_s9_if_elif_else_chain() {
        let code = r#"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn quadrant"), "output: {}", rust);
    }

    // --- With/context manager patterns ---

    #[test]
    fn test_s9_with_open_read_lines() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9_assign_list_index() {
        let code = r"
def set_first(items: list, val: int) -> None:
    items[0] = val
";
        let rust = transpile(code);
    ...
    #[test]
    fn test_s9_assign_dict_index() {
        let code = r"
def set_key(d: dict, key: str, val: str) -> None:
    d[key] = val
";
        let rust = transpile(code);
        assert!(rust.contains("fn set_key"), "output: {}", rust);
    }
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn del_test"), "output: {}", rust);
    }

    // --- Complex algorithms ---

    #[test]
    fn test_s9_binary_search() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn binary_search"), "output: {}", rust);
    }

    #[test]
    fn test_s9_bubble_sort() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn bubble_sort"), "output: {}", rust);
    }

    #[test]
    fn test_s9_fibonacci() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn fibonacci"), "output: {}", rust);
    }
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn gcd"), "output: {}", rust);
    }

    #[test]
    fn test_s9_factorial() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn factorial"), "output: {}", rust);
    }
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn is_prime"), "output: {}", rust);
    }

    #[test]
    fn test_s9_string_reverse() {
        let code = r#"
...
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn reverse_str"), "output: {}", rust);
    }

    #[test]
    fn test_s9_count_words() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn count_words"), "output: {}", rust);
    }
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn two_sum"), "output: {}", rust);
    }

    // === Session 9 Batch 6: Targeted coverage gaps ===

    #[test]
    fn test_s9b6_nested_list_type_inference() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn make_matrix"), "output: {}", rust);
    }
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn mixed_list"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_multi_level_dict_access() {
        let code = r#"
def deep_access(data: dict) -> int:
    return data["level1"]["level2"]
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn deep_access"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_tuple_unpacking_from_split() {
        let code = r#"
def parse_pair(line: str) -> str:
    parts = line.split(",")
    a = parts[0]
    b = parts[1]
    return a
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn parse_pair"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_walrus_in_while() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn count_lines"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_augmented_assign_float() {
        let code = r"
...
";
        ...
        assert!(rust.contains("total"), "output: {}", rust);
    }
...
";
    ...
    #[test]
    fn test_s9b6_assign_with_type_annotation() {
        let code = r#"
def typed_vars() -> int:
    x: int = 10
    y: float = 3.14
...
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn typed_vars"), "output: {}", rust);
        assert!(rust.contains("i64") || rust.contains("i32"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_assign_empty_dict() {
        let code = r#"
def make_dict() -> dict:
    d = {}
    d["key"] = "value"
...
"#;
    ...
    #[test]
    fn test_s9b6_assign_empty_list() {
        let code = r"
def make_list() -> list:
    items = []
    items.append(1)
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn even_numbers"), "output: {}", rust);
        assert!(rust.contains("step_by") || rust.contains("range"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_nested_for_loops() {
        let code = r"
...
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn classify"), "output: {}", rust);
        assert!(rust.contains("else if"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_del_statement() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_global_constant_assignment() {
        let code = r#"
MAX_SIZE = 100
...
"#;
        let rust = transpile(code);
        assert!(rust.contains("MAX_SIZE"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_string_multiplication() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_list_multiplication() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_power_operator() {
        let code = r"
def square(x: int) -> int:
    return x ** 2
";
        let rust = transpile(code);
        assert!(rust.contains("fn square"), "output: {}", rust);
        assert!(rust.contains("pow") || rust.contains("powi"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_floor_division() {
        let code = r"
def half(x: int) -> int:
    return x // 2
";
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn checked_div"), "output: {}", rust);
        assert!(rust.contains("assert"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_assert_with_message() {
        let code = r#"
...
"#;
        let rust = transpile(code);
        assert!(rust.contains("fn positive"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_with_open_read() {
        let code = r"
...
"#;
    ...
    #[test]
    fn test_s9b6_chained_comparison() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_triple_comparison() {
        let code = r"
def valid_range(x: int, lo: int, hi: int) -> bool:
    return lo <= x <= hi
";
        let rust = transpile(code);
        assert!(rust.contains("fn valid_range"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_string_in_check() {
        let code = r"
def has_substring(text: str, sub: str) -> bool:
    return sub in text
";
        let rust = transpile(code);
        assert!(rust.contains("fn has_substring"), "output: {}", rust);
        assert!(rust.contains("contains"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_list_in_check() {
        let code = r"
def contains_item(items: list, target: int) -> bool:
    return target in items
";
        let rust = transpile(code);
        assert!(rust.contains("fn contains_item"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_dict_in_check() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_not_in_check() {
        let code = r"
def missing_key(d: dict, key: str) -> bool:
    return key not in d
";
        let rust = transpile(code);
        assert!(rust.contains("fn missing_key"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_isinstance_check() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_multiple_assignments() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_string_format_method() {
        let code = r#"
...
";
    ...
    #[test]
    fn test_s9b6_negative_indexing() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_enumerate_with_index() {
        let code = r"
...
";
        let rust = transpile(code);
        assert!(rust.contains("fn indexed_items"), "output: {}", rust);
        assert!(rust.contains("enumerate"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_zip_two_lists() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_sorted_with_key() {
        let code = r"
def sort_by_length(words: list) -> list:
    return sorted(words, key=lambda w: len(w))
";
        let rust = transpile(code);
        assert!(rust.contains("fn sort_by_length"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_list_comprehension_with_condition() {
        let code = r"
def even_squares(n: int) -> list:
    return [x * x for x in range(n) if x % 2 == 0]
";
        let rust = transpile(code);
        assert!(rust.contains("fn even_squares"), "output: {}", rust);
    }
...
";
    ...
    #[test]
    fn test_s9b6_ternary_expression() {
        let code = r"
def abs_val(x: int) -> int:
...
";
    ...
    #[test]
    fn test_s9b6_bool_expression_and() {
        let code = r"
def both_positive(a: int, b: int) -> bool:
    return a > 0 and b > 0
";
        let rust = transpile(code);
        assert!(rust.contains("fn both_positive"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_bool_expression_or() {
        let code = r"
def any_positive(a: int, b: int) -> bool:
    return a > 0 or b > 0
";
        let rust = transpile(code);
        assert!(rust.contains("fn any_positive"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_nested_function_call() {
        let code = r"
...
";
    ...
    #[test]
    fn test_s9b6_class_with_init_and_method() {
        let code = r"
class Point:
...
";
        let rust = transpile(code);
        assert!(rust.contains("struct Point"), "output: {}", rust);
    }

    #[test]
    fn test_s9b6_class_with_str_method() {
        let code = r"
class Named:
