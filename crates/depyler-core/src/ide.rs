...
use crate::hir::{HirModule, HirFunction, HirClass, HirMethod};
...
impl IdeIntegration {
    ...
    fn index_function(&mut self, func: &HirFunction, _source: &str) {
        ...
        self.symbols.entry(func.name.clone()).or_insert_with(Vec::new).push(symbol);
    }
    ...
    fn index_class(&mut self, class: &HirClass, _source: &str) {
        ...
        self.symbols.entry(class.name.clone()).or_insert_with(Vec::new).push(symbol);
        ...
        for field in &class.fields {
            ...
            self.symbols.entry(field.name.clone()).or_insert_with(Vec::new).push(field_symbol);
        }
    }
    ...
    fn index_method(&mut self, method: &HirMethod, class_name: &str, _source: &str) {
        ...
        let detail = format!("{}::{}({}) -> {:?}", class_name, method.name, params.join(", "), method.ret_type);
        ...
        self.symbols.entry(method.name.clone()).or_insert_with(Vec::new).push(symbol);
    }
...
mod tests {
    use super::*;
    use crate::hir::{Type, FunctionProperties};
    ...
    fn test_completions() {
        ...
        ide.symbols.insert("test_func".to_string(), vec![
            Symbol {
                name: "test_func".to_string(),
                kind: SymbolKind::Function,
                range: TextRange::new(TextSize::from(0), TextSize::from(10)),
        ...
        ide.symbols.insert("test_class".to_string(), vec![
            Symbol {
