...
";
    ...
    assert!(
        ...
        "Dict 'in' operator should use contains_key or get: {code}"
    );
}
...
";
    ...
    assert!(
        ...
        "Expected at least 3 contains_key calls for dict patterns, got {contains_key_count}: {code}"
    );
}
...
";
    ...
    if let Ok(code) = result {
        assert!(code.contains("contains_key"), "Dict.contains should become contains_key: {code}");
    }
}
