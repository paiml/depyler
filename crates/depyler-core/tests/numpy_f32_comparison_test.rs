...
";
    ...
    assert!(
        ...
        "Should coerce 0 to float when comparing with norm_a: {code}"
    );
}
...
";
    ...
    assert!(
        code.contains("0f32")
            ...
            || code.contains("0.0_f64")
            || code.contains("0.0f64"),
        "Should coerce 0 to float when comparing with norm_b: {code}"
    );
}
...
";
    ...
    assert!(float_count >= 2, "Should have at least 2 float literals for both comparisons: {code}");
}
...
";
    ...
    assert!(
        ...
        "Should have float literals for comparisons (expected >= 2, got {float_comparison_count}): {code}"
    );
    ...
    assert!(
        !has_else_with_int,
        "Else branch should use float literal (f64 to match body), not integer 0: {code}"
    );
}
...
";
    ...
    assert!(
        code.contains("0f32")
            ...
            || code.contains("0.0_f64")
            || code.contains("0.0f64"),
        "Should coerce 0 to float when comparing with dot: {code}"
    );
}
