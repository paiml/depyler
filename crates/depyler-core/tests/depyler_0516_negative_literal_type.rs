...
fn unique_temp_path() -> (String, String) {
    ...
    let rs_file = format!("/tmp/depyler_0516_{pid}_{id}.rs");
    let rlib_file = format!("/tmp/depyler_0516_{pid}_{id}.rlib");
    (rs_file, rlib_file)
}
...
fn test_DEPYLER_0516_negative_integer_literal() {
    ...
    match check_rust_compiles(&rust_code) {
        Ok(()) => {}
        Err(e) => {
            panic!("DEPYLER-0516: Generated code should compile.\nError:\n{e}\nCode:\n{rust_code}")
        }
    }
}
...
fn test_DEPYLER_0516_various_negatives() {
    ...
    for val in &["-1", "-2", "-10", "-100", "-0"] {
        ...
        assert!(
            rust_code.contains("i32"),
            "DEPYLER-0516: '{val}' should infer i32.\nGenerated:\n{rust_code}"
        );
        assert!(
            !rust_code.contains("serde_json::Value"),
            "DEPYLER-0516: '{val}' should not use serde_json::Value.\nGenerated:\n{rust_code}"
        );

        check_rust_compiles(&rust_code)
            .unwrap_or_else(|e| panic!("DEPYLER-0516: '{val}' should compile.\nError:\n{e}"));
    }
}
...
fn test_DEPYLER_0516_positive_still_works() {
    ...
    for val in &["0", "1", "2", "10", "100"] {
        ...
        assert!(
            rust_code.contains("i32"),
            "DEPYLER-0516: Positive '{val}' should still infer i32.\nGenerated:\n{rust_code}"
        );

        check_rust_compiles(&rust_code)
            .unwrap_or_else(|e| panic!("DEPYLER-0516: '{val}' should still compile.\nError:\n{e}"));
    }
}
