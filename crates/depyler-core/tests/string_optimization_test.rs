...
"#;
    ...
    assert!(
        abc_count >= 2,
        "Should have at least 2 STR_ABC constants (one for 'ABC', one for 'abc' with suffix), found {abc_count}"
    );
...
"#;
        ...
        assert_eq!(
            const_count, 1,
            "MUTATION KILL: Should have exactly 1 constant for 'repeated' (found {const_count})"
        );
    }
