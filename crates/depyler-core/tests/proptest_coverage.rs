...
proptest! {
    ...
    fn prop_valid_param_names(name in "[a-z][a-z0-9_]{0,20}") {
        let code = format!("def f({name}): pass");
        // Should not panic
        let _ = transpile(&code);
    }
}
...
proptest! {
    ...
    fn prop_list_literals(vec in prop::collection::vec(0i32..100, 0..10)) {
        let elements: Vec<String> = vec.iter().map(std::string::ToString::to_string).collect();
...
proptest! {
    ...
    fn prop_default_values(default in 0i32..1000) {
        let code = format!("def f(x: int = {default}) -> int:\n    return x");
        let _ = transpile(&code);
    }
}
