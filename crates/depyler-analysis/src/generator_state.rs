...
impl StateAnalyzer {
    ...
    fn analyze_assign(
        ...
        match target {
            ...
            depyler_hir::hir::AssignTarget::Tuple(targets) => {
                ...
                for (idx, target_elem) in targets.iter().enumerate() {
                    if let depyler_hir::hir::AssignTarget::Symbol(name) = target_elem {
                        let name_str = name.as_str();
                        if !self.declared_vars.contains(name_str) {
                            self.declared_vars.insert(name_str.to_string());
...
mod tests {
    ...
    fn make_assign(name: &str, value: HirExpr, ty: Option<Type>) -> HirStmt {
        HirStmt::Assign {
            target: depyler_hir::hir::AssignTarget::Symbol(name.to_string()),
            value,
            type_annotation: ty,
        }
    }
        ...
        let func = make_func(
            ...
            vec![HirStmt::For {
                ...
                body: vec![HirStmt::For {
                    target: depyler_hir::hir::AssignTarget::Symbol("j".to_string()),
                    iter: HirExpr::List(vec![]),
                    body: vec![HirStmt::Expr(make_yield(None))],
                }],
            }],
    ...
    fn test_tuple_unpacking_simple() {
        let func = make_func(
            ...
            vec![],
            vec![HirStmt::Assign {
                target: depyler_hir::hir::AssignTarget::Tuple(vec![
                    depyler_hir::hir::AssignTarget::Symbol("a".to_string()),
                    depyler_hir::hir::AssignTarget::Symbol("b".to_string()),
                ]),
    ...
    fn test_tuple_unpacking_type_inference() {
        let func = make_func(
            ...
            vec![HirStmt::Assign {
                target: depyler_hir::hir::AssignTarget::Tuple(vec![
                    depyler_hir::hir::AssignTarget::Symbol("x".to_string()),
                    depyler_hir::hir::AssignTarget::Symbol("y".to_string()),
                ]),
                value: HirExpr::Tuple(vec![
                    HirExpr::Literal(Literal::Int(42)),
    ...
    fn test_simple_counter_analysis() {
        ...
        let func = HirFunction {
            ...
            body: vec![
                HirStmt::Assign {
                    target: depyler_hir::hir::AssignTarget::Symbol("current".to_string()),
                    value: HirExpr::Literal(depyler_hir::hir::Literal::Int(0)),
                    type_annotation: Some(Type::Int),
                },
                HirStmt::While {
                    condition: HirExpr::Binary {
                        left: Box::new(HirExpr::Var("current".to_string())),
                        op: depyler_hir::hir::BinOp::Lt,
                        right: Box::new(HirExpr::Var("n".to_string())),
                    },
                    body: vec![
