    ...
    fn analyze_expression_for_use(&mut self, expr: &HirExpr) {
        match expr {
            ...
            HirExpr::FString { parts } => {
                for part in parts {
                    if let depyler_hir::hir::FStringPart::Expr(expr) = part {
                        self.analyze_expression_for_use(expr);
                    }
                }
            }
...
mod tests {
    use super::*;
    use depyler_hir::hir::{BinOp, Literal};
    ...
    fn test_no_use_after_move_simple() {
        let func = HirFunction {
            name: "foo".to_string(),
            params: smallvec![depyler_hir::hir::HirParam {
                name: "x".to_string(),
                ty: Type::Int,
                default: None,
                is_vararg: false,
        ...
        let func = make_function(
            ...
            vec![HirStmt::With {
                context: HirExpr::Call {
                    func: "open".to_string(),
                    args: vec![HirExpr::Literal(depyler_hir::hir::Literal::String(
                        "file.txt".to_string(),
                    ))],
                    kwargs: vec![],
                },
    ...
    fn test_analyze_assert_stmt() {
        let func = make_function(
            ...
            vec![HirStmt::Assert {
                ...
                msg: Some(HirExpr::Literal(depyler_hir::hir::Literal::String(
    ...
    fn test_analyze_nested_function_captures() {
        let func = HirFunction {
            ...
            body: vec![HirStmt::FunctionDef {
                ...
                params: Box::new(smallvec![depyler_hir::hir::HirParam {
                    name: "y".to_string(),
                    ty: Type::Int,
                    default: None,
                    is_vararg: false,
    ...
    fn test_expr_list_comprehension() {
        let func = make_function(
            ...
            vec![HirStmt::Expr(HirExpr::ListComp {
                ...
                generators: vec![depyler_hir::hir::HirComprehension {
                    target: "x".to_string(),
    ...
    fn test_expr_fstring() {
        let func = make_function(
            ...
            vec![HirStmt::Expr(HirExpr::FString {
                parts: vec![
                    depyler_hir::hir::FStringPart::Literal("hello ".to_string()),
                    depyler_hir::hir::FStringPart::Expr(Box::new(make_var("name"))),
                ],
            })],
        );
        let mut analysis = UseAfterMoveAnalysis::new();
    ...
    fn test_analyze_ownership_comprehensive_with_aliasing() {
        let func = make_function(
            ...
            vec![
                HirStmt::Assign {
                    target: AssignTarget::Symbol("a".to_string()),
                    value: HirExpr::Literal(depyler_hir::hir::Literal::String("hello".to_string())),
        ...
        let func = make_function(
            ...
            vec![HirStmt::Expr(HirExpr::Call {
                ...
                kwargs: vec![
                    (
                        "name".to_string(),
                        HirExpr::Literal(depyler_hir::hir::Literal::String("test".to_string())),
                    ),
