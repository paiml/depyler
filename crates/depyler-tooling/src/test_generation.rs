...
mod tests {
    ...
    fn make_param(name: &str, ty: Type) -> depyler_hir::hir::HirParam {
        depyler_hir::hir::HirParam::new(name.to_string(), ty)
    }
    ...
    fn test_is_simple_param_reference_false_not_var() {
        let gen = TestGenerator::new(TestGenConfig::default());
        let expr = HirExpr::Literal(depyler_hir::hir::Literal::Int(1));
        assert!(!gen.is_simple_param_reference(&expr, "x"));
    }
