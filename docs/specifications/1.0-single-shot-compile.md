...
**Version**: 1.14.0 (DEPYLER-1047 Dict Key Type Fix)
**Date**: 2026-01-09
...
*   **Compile Rate**: 74/177 (41.8%) — verified against depyler examples corpus (Jan 9, 2026)
...
*   **DEPYLER-1047**: FIXED — Dict literal key types for HashMap<String, DepylerValue> returns.
    - Bare `dict` return type parses as Dict(Unknown, Unknown) but generates HashMap<String, DepylerValue>
    - Dict literals now use String keys when return/target expects String keys
*   **DEPYLER-1046**: ADDED — IntoIterator trait for DepylerValue (enables `for x in value` syntax).
*   **DEPYLER-1045**: FIXED — Empty dict literal type inference from parameter type annotation.
*   **DEPYLER-1044**: FIXED — Critical CSE numpy detection bugs.
*   **DEPYLER-1043**: FIXED — Reverse trait impls return primitives, not DepylerValue.
...
1. **Type mismatches** (E0308): ~40 files fail with mismatched types (return types, tuple element types)
2. **Lambda type inference** (E0282): Lambda parameters lack type annotations. (DEPYLER-1061 - Documented as limitation)
3. **Scope/binding issues** (E0425): Cannot find value - unresolved imports or missing variables
...
├─────────────────────────────────────────────────────────────┤
│  Compile Rate:  74/177 (41.8%)  ████████░░░░░░░░░░░░  42%   │
│  Target:        80%             ████████████████░░░░  80%   │
│                                                             │
│  ✅ DEPYLER-1047 COMPLETE: Dict key type fix                │
│  ✅ DEPYLER-1046 COMPLETE: IntoIterator for DepylerValue    │
│  ✅ DEPYLER-1045 COMPLETE: Dict type from param annotation  │
│  ✅ CLIPPY CLEAN: Zero warnings in workspace                │
│  ✅ STRATEGIC PIVOT: Hybrid Fallback Strategy ACTIVE        │
│                                                             │
│  🧠 ORACLE STATUS: v3.21.0 (85% Accuracy)                   │
│   Location: crates/depyler/depyler_oracle.apr               │
│   Last Train: Jan 7, 2026 (12,282 samples)                  │
│                                                             │
│  🔴 BLOCKERS: Lambda inference (documented), CSE methods    │
│  Next Action:   Fix assignment coercion for DepylerValue    │
│  Falsification: Point 14 FIXED (Dict integer keys work)     │
└─────────────────────────────────────────────────────────────┘
```
...
| DEPYLER-1050 | Return type→DepylerValue propagation | `expr_gen_instance_methods.rs`, `stmt_gen.rs` | Subscript assignments now wrap |
...
*   **Why 5**: Why wasn't there a separate storage for module-level types?
    *   *Answer:* Architectural oversight — all type tracking was conflated into `var_types`.
...
#### The Static Dictionary Key Vulnerability (DEPYLER-1060) — RESOLVED
**Status**: ✅ FIXED (Jan 9, 2026)
...
6.  [ ] `const` declaration infers `String` for non-string expression. ✅ **SOLVED (DEPYLER-1040b)**
