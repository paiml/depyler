...
**Version**: 1.12.0 (DEPYLER-1042 & DEPYLER-1041 Type Unification)
**Date**: 2026-01-09
...
*   **Compile Rate**: 67/175 (38.3%) — verified against depyler examples corpus (Jan 9, 2026)
...
*   **DEPYLER-1042**: FIXED — Nested dict types now propagate correctly in subscript assignments.
*   **DEPYLER-1041**: PARTIAL — Reverse trait impls (i32 + DepylerValue) added, needs assignment coercion.
...
1. **Lambda type inference** (E0282): Lambda parameters lack type annotations causing `type must be known at this point`. (DEPYLER-1061 - Documented as limitation)
2. **DepylerValue assignment coercion** (E0308): Result of `i32 + DepylerValue` is `DepylerValue`, can't assign back to `i32` variable.
3. **CSE method calls on primitives** (E0599): `.scale()`, `.add()` methods not found on `i32` types (CSE optimization issue).
