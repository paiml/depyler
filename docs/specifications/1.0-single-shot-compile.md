...
**Version**: 1.81.0 (DEPYLER-1133: Restoration of Truth)
**Date**: 2026-01-15
...
*   **Compile Rate**: **33.0% (46/139 example files)** — Example corpus test on depyler/examples.
*   **Verification**: The "Falsification Suite" now compiles successfully.
*   **DEPYLER-1133**: COMPLETE — **Restoration of Truth (Oracle Integration)**. Fixed the "Generator's Rebellion" - the `generate_rust_file_with_overrides` function accepted Oracle-learned type constraints but dropped them on the floor. The root cause identified by Karl Popper: `generate_rust_file_with_overrides` created a context but never used the passed-in `type_overrides`. **Solution**: Refactored `generate_rust_file` into `generate_rust_file_internal` that accepts `initial_var_types`. The Oracle's learned types are now pre-seeded into `ctx.var_types` BEFORE any function/statement generation begins. **Pattern fixed**: When Oracle learns `result: Vec<i32>` from E0308 errors, that constraint now flows through all code generation. `generate_rust_file()` calls internal with empty HashMap (default), `generate_rust_file_with_overrides()` calls internal with Oracle types. **Result: The Generator now obeys the Oracle's constraints.**
*   **DEPYLER-1132**: COMPLETE — **List Repeat Type Inference**. Fixed E0271 type mismatch errors for list repetition patterns. Previously `[0] * 10` inferred as `[i32; 10]` (fixed-size array) but codegen used Vec via PyMul trait. Now both `infer_expr_type_simple()` and `infer_expr_type_with_env()` return `Type::List` for list repetition regardless of size. **Pattern fixed**: `zeros = [0] * 10` return type now `Vec<i32>` not `[i32; 10]`. **Result: E0271 array/Vec mismatches eliminated for list multiplication.**
