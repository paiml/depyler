...
**Version**: 1.13.0 (DEPYLER-1044 CSE/Numpy Detection Fix)
**Date**: 2026-01-09
...
*   **Compile Rate**: 56/141 (39.7%) — verified against depyler examples corpus (Jan 9, 2026)
...
*   **DEPYLER-1044**: FIXED — Critical CSE numpy detection bugs:
    - `numpy_vars` now cleared between functions (prevented pollution from prior functions)
    - `abs(scalar)` correctly returns scalar, not vector
    - Single-letter float heuristics removed (c, r, g, etc. caused false positives)
...
1. **Type mismatches** (E0308): 45 files fail with mismatched types (HashMap key types, return types, etc.)
2. **Lambda type inference** (E0282): Lambda parameters lack type annotations causing `type must be known at this point`. (DEPYLER-1061 - Documented as limitation)
3. **Async edition** (E0670): 4 files use async but target Rust 2015 edition.
...
├─────────────────────────────────────────────────────────────┤
│  Compile Rate:  67/175 (38.3%)  ████████░░░░░░░░░░░░  38%   │
│  Target:        80%             ████████████████░░░░  80%   │
│                                                             │
│  ✅ DEPYLER-1042 COMPLETE: Nested dict type propagation     │
│  ✅ DEPYLER-1041 PARTIAL: Reverse trait impls added         │
