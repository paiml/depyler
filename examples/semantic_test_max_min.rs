...
pub fn find_range(nums: &Vec<i32>) -> i32 {
    (*nums.iter().max().unwrap()).py_sub(*nums.iter().min().unwrap())
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    let data: Vec<i32> = vec![3, 1, 4, 1, 5, 9, 2, 6];
    println!("{}", find_range(&data));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_find_range_examples() {
        assert_eq!(find_range(&vec![]), 0);
        assert_eq!(find_range(&vec![1]), 1);
        assert_eq!(find_range(&vec![1, 2, 3]), 3);
    }
}
