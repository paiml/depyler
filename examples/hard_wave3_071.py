...
def func_b_71(data: List[int]) -> int:
...
def test_71() -> bool:
    ok: bool = True
    h: int = func_a_71(42, 99)
    if h == 0:
        ok = False
    acc: int = func_b_71([1, 2, 3, 4, 5])
    if acc < 0:
        ok = False
    seq: List[int] = func_c_71(7)
    if len(seq) != 10:
        ok = False
    return ok
