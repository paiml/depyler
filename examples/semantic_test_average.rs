...
pub fn average(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        average(&vec![10, 20, 30, 40, 50]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_average_examples() {
        assert_eq!(average(&vec![]), 0);
        assert_eq!(average(&vec![1]), 1);
        assert_eq!(average(&vec![1, 2, 3]), 3);
    }
}