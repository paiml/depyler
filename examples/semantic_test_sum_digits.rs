...
pub fn sum_digits(n: i32) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", sum_digits(12345).expect("operation failed"));
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_sum_digits_examples() {
        assert_eq!(sum_digits(0), 0);
        assert_eq!(sum_digits(1), 1);
        assert_eq!(sum_digits(-1), -1);
    }
}