...
pub fn alternate_sum(nums: &Vec<i32>) -> i32 {
...
pub fn main() {
    println!("{}", alternate_sum(&vec![1, 2, 3, 4, 5]));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_alternate_sum_examples() {
        assert_eq!(alternate_sum(&vec![]), 0);
        assert_eq!(alternate_sum(&vec![1]), 1);
        assert_eq!(alternate_sum(&vec![1, 2, 3]), 6);
    }
}
