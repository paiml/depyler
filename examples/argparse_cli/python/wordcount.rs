use clap::Parser;
    ...
    #[derive(Debug, Clone)] pub struct Stats {
    pub lines: i32, pub words: i32, pub chars: i32, pub filename: String
}
#[doc = "Count statistics for a single file"] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn count_file(filepath: & Path) -> Stats {
    { let content = std::fs::read_to_string(filepath).unwrap();
    let lines = content.lines().map(| s | s.to_string()).collect::<Vec<String>>().len() as i32;
    let words = content.split_whitespace().map(| s | s.to_string()).collect::<Vec<String>>().len() as i32;
    let chars = content.len() as i32;
    Stats::new(lines, words, chars, filepath.to_string()) println!("{}", format!("Error reading {}: {}", filepath, e));
    return Stats::new(0, 0, 0, filepath.to_string());
   
}
} #[doc = "Format statistics for output"] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn format_stats(stats: Stats, show_filename: bool) -> String {
...
let args = Args::parse();
    ...
    println!("{}", format_stats(total_stats));
