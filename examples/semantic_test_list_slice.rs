...
pub fn first_three(nums: &Vec<i32>) -> Vec<i32> {
    {
        let base = &*nums;
        let stop_idx = (3) as isize;
...
pub fn main() {
    ...
    let result = first_three(&data);
    println!("{}", result.len() as i32);
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_first_three_examples() {
        assert_eq!(first_three(vec![]), Vec::<i32>::new());
        assert_eq!(first_three(vec![1]), vec![1]);
    }
}