...
pub struct MarcoPoloGame {
    ...
    pub score: i32,
    pub attempts: i32,
    pub difficulty_ranges: std::collections::HashMap<DepylerValue, DepylerValue>,
}
impl MarcoPoloGame {
    pub fn new(difficulty: impl Into<String>, verbose: bool) -> Self {
        Self {
    ...
    pub fn generate_number(&self) -> i32 {
        let (min_val, max_val) = {
            let _base = &self.difficulty_ranges.clone();
            let _idx = (self.difficulty.clone()) as isize;
            if _idx < 0 {
    ...
    pub fn calculate_performance(&self) -> String {
        ...
        let avg_attempts = self.attempts.clone() / (self.score.clone() as f64).max(1 as f64);
        if avg_attempts <= 5 {
    ...
    if _cse_temp_0 {
        println!(
            ...
            format!(
                ...
                (game.attempts).py_div(game.score)
            )
        );
    }
    println!(
