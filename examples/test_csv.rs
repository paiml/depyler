...
pub fn read_csv_rows(filename: &str) -> Result<Vec<Vec<String>>, std::io::Error> {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_csv_dict(filename: &str) -> Result<Vec<HashMap<String, String>>, std::io::Error> {
    let mut records: Vec<std::collections::HashMap<String, String>> = vec![];
    let mut f = std::fs::File::open(&filename)?;
    let mut reader = std::io::BufReader::new(f);
    for result in reader.deserialize::<HashMap<String, String>>() {
        let row = result?;
        records.push(row.into_iter().collect::<std::collections::HashMap<_, _>>());
    }
    Ok(records)
}
