...
pub fn second_largest(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    let mut second: i32 = Default::default();
    let mut first: i32 = nums
        .get(0usize)
        .cloned()
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        second_largest(&vec![5, 2, 8, 1, 9, 3]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_second_largest_examples() {
        assert_eq!(second_largest(&vec![]), 0);
        assert_eq!(second_largest(&vec![1]), 1);
        assert_eq!(second_largest(&vec![1, 2, 3]), 3);
    }
}