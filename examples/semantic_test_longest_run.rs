...
pub fn longest_run(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    let mut max_run: i32 = Default::default();
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        longest_run(&vec![1, 1, 2, 2, 2, 3, 3]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_longest_run_examples() {
        assert_eq!(longest_run(&vec![]), 0);
        assert_eq!(longest_run(&vec![1]), 1);
        assert_eq!(longest_run(&vec![1, 2, 3]), 3);
    }
}