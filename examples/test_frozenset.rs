...
pub fn test_frozenset() -> (DepylerValue, DepylerValue, DepylerValue) {
    let fs1 = std::sync::Arc::new(std::collections::HashSet::<i32>::new());
    let fs2 = std::sync::Arc::new(
        vec![1, 2, 3]
            .into_iter()
            .collect::<std::collections::HashSet<_>>(),
    );
    let fs3 = std::sync::Arc::new(
        vec![4, 5, 6]
            .into_iter()
            .collect::<std::collections::HashSet<_>>(),
    );
    return (
        DepylerValue::from(fs1),
        DepylerValue::from(fs2),
        DepylerValue::from(fs3),
    );
}
