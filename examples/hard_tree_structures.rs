...
#[doc = "Search for a value in a list-based binary tree via linear scan."]
pub fn bt_search(tree: &Vec<i32>, target: i32) -> Result<bool, Box<dyn std::error::Error>> {
    let mut idx: i32 = 0;
    while idx < tree.len() as i32 {
        if (tree
            .get(idx as usize)
            .cloned()
            .expect("IndexError: list index out of range")
...
#[doc = "Preorder traversal using iterative stack. Root -> Left -> Right."]
pub fn bt_preorder(tree: &Vec<i32>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut result: Vec<i32> = vec![];
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(result);
    }
    ...
    while stack.len() as i32 > 0 {
        ...
        stack = {
            let base = &*stack;
            let stop_idx = ((stack.len() as i32) - (1i32)) as isize;
...
#[doc = "Postorder traversal using two stacks. Left -> Right -> Root."]
pub fn bt_postorder(tree: &Vec<i32>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut result: Vec<i32> = vec![];
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
    ...
    let mut stack1: Vec<i32> = vec![0];
    let mut stack2: Vec<i32> = vec![];
    while stack1.len() as i32 > 0 {
        let idx: i32 = {
            let base = &stack1;
            let idx: i32 = (stack1.len() as i32) - (1i32);
            let actual_idx = if idx < 0 {
                base.len().saturating_sub(idx.abs() as usize)
            } else {
                idx as usize
        ...
        stack1 = {
            let base = &*stack1;
            let stop_idx = ((stack1.len() as i32) - (1i32)) as isize;
            let stop = if stop_idx < 0 {
                (base.len() as isize + stop_idx).max(0) as usize
            } else {
                stop_idx as usize
    ...
    while stack2.len() as i32 > 0 {
        let top: i32 = {
            let base = &stack2;
            let idx: i32 = (stack2.len() as i32) - (1i32);
            let actual_idx = if idx < 0 {
                base.len().saturating_sub(idx.abs() as usize)
            } else {
                idx as usize
        ...
        stack2 = {
            let base = &*stack2;
            let stop_idx = ((stack2.len() as i32) - (1i32)) as isize;
            let stop = if stop_idx < 0 {
                (base.len() as isize + stop_idx).max(0) as usize
            } else {
                stop_idx as usize
...
#[doc = "Insert a value into a BST represented as a list.\n\n    Navigates left/right based on comparison, extending the list as needed.\n    "]
pub fn bst_insert(tree: &Vec<i32>, value: i32) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut result: Vec<i32> = vec![];
    let mut i: i32 = 0;
    while i < tree.len() as i32 {
        result.push(
    ...
    while idx < result.len() as i32 {
        ...
        if value
            < result
                ...
                .expect("IndexError: list index out of range")
        {
            let next_idx: i32 = (((2i32).py_mul(idx) as i32).py_add(1i32)) as i32;
            while next_idx >= result.len() as i32 {
                result.push(-1);
            }
            idx = next_idx;
        } else {
            if value
                > result
                    ...
                    .expect("IndexError: list index out of range")
            {
                let next_idx2: i32 = (((2i32).py_mul(idx) as i32).py_add(2i32)) as i32;
                while next_idx2 >= result.len() as i32 {
                    result.push(-1);
                }
                idx = next_idx2;
            } else {
...
#[doc = "Search for a value in a BST using O(log n) navigation."]
pub fn bst_search(tree: &Vec<i32>, target: i32) -> Result<bool, Box<dyn std::error::Error>> {
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(false);
    ...
    while idx < tree.len() as i32 {
        ...
        if target
            < tree
                ...
                .expect("IndexError: list index out of range")
        {
            idx = (((2i32).py_mul(idx) as i32).py_add(1i32)) as i32;
        } else {
            idx = (((2i32).py_mul(idx) as i32).py_add(2i32)) as i32;
        }
    }
    Ok(false)
}
#[doc = "Check if a list-based binary tree is a valid BST.\n\n    Uses iterative inorder traversal and checks strictly increasing values.\n    "]
pub fn is_valid_bst(tree: &Vec<i32>) -> Result<bool, Box<dyn std::error::Error>> {
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(true);
    ...
    let mut idx: i32 = 0;
    while (idx < tree.len() as i32) || (stack.len() as i32 > 0) {
        while (idx < tree.len() as i32)
            && (tree
                .get(idx as usize)
                .cloned()
            ...
            != -1
        {
            if (values.len() as i32 > 0)
                && (tree
                    ...
                    <= {
                        let base = &values;
                        let idx: i32 = (values.len() as i32) - (1i32);
                        let actual_idx = if idx < 0 {
                            base.len().saturating_sub(idx.abs() as usize)
                        } else {
                            idx as usize
            ...
            values.push(
                tree.get(node as usize)
                    .cloned()
                    .expect("IndexError: list index out of range"),
            );
        }
        let right: i32 = (((2i32).py_mul(node) as i32).py_add(2i32)) as i32;
        if right < tree.len() as i32 {
            idx = right;
        } else {
            idx = tree.len() as i32;
...
#[doc = "Compute the height of a subtree rooted at idx using BFS."]
pub fn tree_height(tree: &Vec<i32>, idx: i32) -> Result<i32, Box<dyn std::error::Error>> {
    let mut max_h: i32 = Default::default();
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = idx >= _cse_temp_0;
    let _cse_temp_2 = _cse_temp_0 == 0;
    let _cse_temp_3 = (_cse_temp_1) || (_cse_temp_2);
    let _cse_temp_4 = tree
        .get(idx as usize)
        .cloned()
        .expect("IndexError: list index out of range")
        == -1;
    ...
    let mut queue: Vec<(i32, i32)> = vec![(idx, 1)];
    max_h = 0;
    let mut front: i32 = 0;
    while front < queue.len() as i32 {
        let current: i32 = queue
            .get(front as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            .0;
        let depth: i32 = queue
            ...
            .expect("IndexError: list index out of range")
            .1;
        front = ((front).py_add(1i32)) as i32;
        if (current >= tree.len() as i32)
            || (tree
        ...
        let left: i32 = (((2i32).py_mul(current) as i32).py_add(1i32)) as i32;
        if (left < tree.len() as i32)
            && (tree
                .get(left as usize)
                .cloned()
                ...
                != -1)
        {
            queue.push((left, (depth).py_add(1i32)));
        }
        let right: i32 = (((2i32).py_mul(current) as i32).py_add(2i32)) as i32;
        if (right < tree.len() as i32)
            && (tree
                .get(right as usize)
                .cloned()
                ...
                != -1)
        {
            queue.push((right, (depth).py_add(1i32)));
        }
    }
    Ok(max_h)
}
#[doc = "Find the depth of a target value using BFS. Returns -1 if not found."]
pub fn tree_depth_of_node(tree: &Vec<i32>, target: i32) -> Result<i32, Box<dyn std::error::Error>> {
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(-1);
    ...
    while front < queue.len() as i32 {
        ...
        if (right < tree.len() as i32)
            && (tree
                ...
                != -1)
        {
            queue.push((right, (d).py_add(1i32)));
        }
    }
    Ok(-1)
}
#[doc = "Compute the balance factor(left height - right height)."]
#[doc = " Depyler: proven to terminate"]
pub fn balance_factor(tree: &Vec<i32>, idx: i32) -> Result<i32, Box<dyn std::error::Error>> {
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = idx >= _cse_temp_0;
    let _cse_temp_2 = tree
        .get(idx as usize)
        .cloned()
        .expect("IndexError: list index out of range")
        == -1;
    ...
    let left_h: i32 = tree_height(&tree, ((2i32).py_mul(idx) as i32).py_add(1i32))?;
    let right_h: i32 = tree_height(&tree, ((2i32).py_mul(idx) as i32).py_add(2i32))?;
    Ok((left_h) - (right_h))
}
#[doc = "Find the Lowest Common Ancestor in a BST array. Returns -1 if not found."]
pub fn find_lca(
    tree: &Vec<i32>,
    val_a: i32,
    val_b: i32,
    ...
    let found_a: bool = bst_search(&tree, val_a)?;
    let found_b: bool = bst_search(&tree, val_b)?;
    let _cse_temp_2 = (!found_a) || (!found_b);
    if _cse_temp_2 {
        return Ok(-1);
    }
    let mut idx: i32 = 0;
    while idx < tree.len() as i32 {
        ...
        let current: i32 = tree
            .get(idx as usize)
            .cloned()
            .expect("IndexError: list index out of range");
        if (val_a < current) && (val_b < current) {
            idx = (((2i32).py_mul(idx) as i32).py_add(1i32)) as i32;
        } else {
            if (val_a > current) && (val_b > current) {
                idx = (((2i32).py_mul(idx) as i32).py_add(2i32)) as i32;
...
#[doc = "Insert a word into a flat trie represented as nested dicts.\n\n    Each key is a prefix path. A special key '#' marks end of word.\n    "]
pub fn trie_insert<'a, 'b>(
    trie: &'a std::collections::HashMap<String, std::collections::HashMap<String, String>>,
    word: &'b str,
    ...
    for k in trie.keys().cloned() {
        let mut inner: std::collections::HashMap<String, String> = {
            let map: HashMap<String, String> = HashMap::new();
            map
        };
        for ik in trie.get(&(k)).cloned().unwrap_or_default().keys().cloned() {
            inner.insert(
                ik.to_string().clone(),
                trie.get(&(k))
                    .cloned()
        ...
        result.insert(k.to_string().clone(), inner.clone());
    }
    prefix = STR_EMPTY.to_string().to_string();
    let mut i: i32 = 0;
    while i < word.len() as i32 {
        ...
        if result.get(&prefix).is_none() {
            result.insert(prefix.to_string().clone(), {
                let map: HashMap<String, String> = HashMap::new();
                map
            });
        }
        result
            ...
            .insert(ch.to_string().clone(), ch.clone());
        prefix = format!("{}{}", prefix, ch);
        i = ((i).py_add(1i32)) as i32;
    }
    let _cse_temp_0 = result.get(&prefix).is_none();
    if _cse_temp_0 {
        result.insert(prefix.to_string().clone(), {
            let map: HashMap<String, String> = HashMap::new();
            map
    ...
    result
        .get_mut(&prefix)
        .expect("key not found in dict")
        .insert(STR__.to_string(), STR__.to_string());
    Ok(result)
}
#[doc = "Search for a complete word in the flat trie."]
pub fn trie_search<'b, 'a>(
    trie: &'a std::collections::HashMap<String, std::collections::HashMap<String, String>>,
    word: &'b str,
    ...
    let _cse_temp_0 = trie.get(&prefix).is_none();
    if _cse_temp_0 {
        return Ok(false);
    }
    let _cse_temp_1 = trie
        .get(&(prefix))
        .cloned()
        .unwrap_or_default()
        .get(STR__)
...
pub fn trie_prefix_search<'a, 'b>(
    ...
    prefix = STR_EMPTY.to_string().to_string();
    let mut i: i32 = 0;
    while i < prefix_query.len() as i32 {
        let ch: String = {
            let base = &prefix_query;
            let idx: i32 = i;
            let actual_idx = if idx < 0 {
                base.chars().count().saturating_sub(idx.abs() as usize)
            } else {
    ...
    let mut stack: Vec<String> = vec![prefix.clone()];
    while stack.len() as i32 > 0 {
        let cur: String = {
            let base = &stack;
            let idx: i32 = (stack.len() as i32) - (1i32);
        ...
        {
            if ch2 != STR__ {
                stack.push(format!("{}{}", cur, ch2));
            }
        }
    }
    Ok(words)
}
#[doc = "Build a segment tree for range sum queries. Iterative bottom-up."]
pub fn segment_tree_build(arr: &Vec<i32>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    ...
    let _cse_temp_2 = ((4i32).py_mul(n)) as i32;
    let size: i32 = _cse_temp_2;
    let mut seg: Vec<i32> = vec![];
    let mut i: i32 = 0;
    while i < size {
        seg.push(0);
        i = ((i).py_add(1i32)) as i32;
    ...
    while k >= 1 {
        ...
        .py_add({
            let base = &seg;
            let idx: i32 = ((2i32).py_mul(k) as i32).py_add(1i32);
            let actual_idx = if idx < 0 {
                base.len().saturating_sub(idx.abs() as usize)
            } else {
                idx as usize
...
#[doc = "Query the sum in range [left, right) on a segment tree."]
pub fn segment_tree_query(
    seg: &Vec<i32>,
    n: i32,
    left: i32,
    right: i32,
    ...
    let mut l: i32 = Default::default();
    let mut r: i32 = Default::default();
    let _cse_temp_0 = n == 0;
    let _cse_temp_1 = left >= right;
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        return Ok(0);
    }
    result = 0;
    l = ((left).py_add(n)) as i32;
    r = ((right).py_add(n)) as i32;
    while l < r {
        if (l).py_mod(2i32) == 1 {
            if l < seg.len() as i32 {
...
#[doc = "Update a position in the segment tree. Returns new list."]
pub fn segment_tree_update(
    seg: &Vec<i32>,
    n: i32,
    pos: i32,
    value: i32,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut left_val: i32 = Default::default();
    let mut right_val: i32 = Default::default();
    let mut result: Vec<i32> = vec![];
    let mut i: i32 = 0;
    while i < seg.len() as i32 {
        result.push(
            seg.get(i as usize)
                .cloned()
                .expect("IndexError: list index out of range"),
    ...
    let mut idx: i32 = ((pos).py_add(n)) as i32;
    let _cse_temp_0 = result.len() as i32;
    let _cse_temp_1 = idx >= _cse_temp_0;
    if _cse_temp_1 {
        return Ok(result);
    ...
    idx = _cse_temp_2;
    while idx >= 1 {
        let left_child: i32 = ((2i32).py_mul(idx)) as i32;
        let right_child: i32 = (((2i32).py_mul(idx) as i32).py_add(1i32)) as i32;
        left_val = 0;
        right_val = 0;
        if left_child < result.len() as i32 {
            left_val = result
                .get(left_child as usize)
                .cloned()
                .expect("IndexError: list index out of range");
        }
        if right_child < result.len() as i32 {
            right_val = result
                .get(right_child as usize)
                .cloned()
                .expect("IndexError: list index out of range");
        }
        result[(idx) as usize] = (left_val).py_add(right_val);
        idx = {
            let a = idx;
            let b = 2;
            let q = a / b;
...
#[doc = "Build a Fenwick(Binary Indexed) Tree. 1-indexed, uses lowbit."]
pub fn fenwick_build(arr: &Vec<i32>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = arr.len() as i32;
    let n: i32 = _cse_temp_0;
    let mut bit: Vec<i32> = vec![];
    let mut i: i32 = 0;
    while i <= n {
        bit.push(0);
        i = ((i).py_add(1i32)) as i32;
    ...
    let mut j: i32 = 0;
    while j < n {
        let idx: i32 = ((j).py_add(1i32)) as i32;
        bit[(idx) as usize] = (bit
            .get(idx as usize)
            .cloned()
            .expect("IndexError: list index out of range"))
        .py_add(
        ...
        let parent: i32 = ((idx).py_add(idx & (-idx))) as i32;
        if parent <= n {
            bit[(parent) as usize] = (bit
                .get(parent as usize)
                .cloned()
                .expect("IndexError: list index out of range"))
            .py_add(
                bit.get(idx as usize)
                    .cloned()
                    .expect("IndexError: list index out of range"),
            );
        }
...
#[doc = "Combined prefix sum query [0, query_idx] and point update at update_idx.\n\n    Returns(prefix_sum, updated_bit). Combines two operations for complexity.\n    "]
pub fn fenwick_query_and_update(
    bit: &Vec<i32>,
    query_idx: i32,
    update_idx: i32,
    delta: i32,
) -> Result<(i32, Vec<i32>), Box<dyn std::error::Error>> {
    let mut result_sum: i32 = Default::default();
    result_sum = 0;
    let mut i: i32 = ((query_idx).py_add(1i32)) as i32;
    while i > 0 {
        if i < bit.len() as i32 {
            result_sum = ((result_sum).py_add(
                bit.get(i as usize)
                    .cloned()
                    .expect("IndexError: list index out of range"),
            )) as i32;
    ...
    let mut new_bit: Vec<i32> = vec![];
    let mut k: i32 = 0;
    while k < bit.len() as i32 {
        new_bit.push(
            bit.get(k as usize)
                .cloned()
                .expect("IndexError: list index out of range"),
    ...
    let _cse_temp_0 = new_bit.len() as i32;
    let n: i32 = ((_cse_temp_0) - (1i32)) as i32;
    let mut j: i32 = ((update_idx).py_add(1i32)) as i32;
    while j <= n {
        new_bit[(j) as usize] = (new_bit
            .get(j as usize)
            .cloned()
            .expect("IndexError: list index out of range"))
        .py_add(delta);
        j = ((j).py_add(j & (-j))) as i32;
    }
    Ok((result_sum, new_bit))
}
#[doc = "Push all values onto a min-heap, then pop all off in sorted order.\n\n    Combines sift-up and sift-down in a single function for stress testing.\n    Returns(sorted_output, empty_heap).\n    "]
pub fn heap_push_pop<'b, 'a>(
    heap: &'a Vec<i32>,
    values: &'b Vec<i32>,
) -> Result<(Vec<i32>, Vec<i32>), Box<dyn std::error::Error>> {
    let mut h: Vec<i32> = Default::default();
    let mut smallest: i32 = Default::default();
    h = vec![];
    let mut i: i32 = 0;
    while i < heap.len() as i32 {
        h.push(
            heap.get(i as usize)
                .cloned()
                .expect("IndexError: list index out of range"),
    ...
    while vi < values.len() as i32 {
        ...
        let mut current: i32 = ((h.len() as i32) - (1i32) as i32) as i32;
        while current > 0 {
            let parent: i32 = {
                let a = (current) - (1i32);
                let b = 2;
            ...
            if h.get(current as usize)
                .cloned()
                .expect("IndexError: list index out of range")
                < h.get(parent as usize)
                    .cloned()
                    .expect("IndexError: list index out of range")
            {
                let temp: i32 = h
                    ...
                    .cloned()
                    .expect("IndexError: list index out of range");
                h[(current) as usize] = h
                    .get(parent as usize)
                    .cloned()
                    .expect("IndexError: list index out of range");
                h[(parent) as usize] = temp;
                current = parent;
            } else {
                break;
            }
    ...
    let mut sorted_out: Vec<i32> = vec![];
    while h.len() as i32 > 0 {
        sorted_out.push(
            h.get(0usize)
                .cloned()
                .expect("IndexError: list index out of range"),
        ...
        if h.len() as i32 == 1 {
            ...
            h = {
                let base = &*h;
                let stop_idx = ((h.len() as i32) - (1i32)) as isize;
                let stop = if stop_idx < 0 {
                    (base.len() as isize + stop_idx).max(0) as usize
                } else {
                    stop_idx as usize
            ...
            let mut current2: i32 = 0;
            loop {
                smallest = current2;
                let left: i32 = (((2i32).py_mul(current2) as i32).py_add(1i32)) as i32;
                let right: i32 = (((2i32).py_mul(current2) as i32).py_add(2i32)) as i32;
                if (left < h.len() as i32)
                    && (h
                        .get(left as usize)
                        .cloned()
                        .expect("IndexError: list index out of range")
                        < h.get(smallest as usize)
                            .cloned()
                            .expect("IndexError: list index out of range"))
                {
                    smallest = left;
                }
                if (right < h.len() as i32)
                    && (h
                        .get(right as usize)
                        .cloned()
                        .expect("IndexError: list index out of range")
                ...
                let temp2: i32 = h
                    .get(current2 as usize)
                    .cloned()
                    .expect("IndexError: list index out of range");
                h[(current2) as usize] = h
                    .get(smallest as usize)
                    .cloned()
                    .expect("IndexError: list index out of range");
                h[(smallest) as usize] = temp2;
                current2 = smallest;
            }
        }
    }
...
#[doc = "Build a min-heap using bottom-up heapification with inline sift-down."]
pub fn heapify(arr: &Vec<i32>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut smallest: i32 = Default::default();
    let mut result: Vec<i32> = vec![];
    let mut i: i32 = 0;
    ...
    let mut start: i32 = ((_cse_temp_1) - (1i32)) as i32;
    while start >= 0 {
        ...
        loop {
            ...
            let temp: i32 = result
                ...
                .cloned()
                .expect("IndexError: list index out of range");
            result[(current) as usize] = result
                .get(smallest as usize)
                .cloned()
                .expect("IndexError: list index out of range");
            result[(smallest) as usize] = temp;
            current = smallest;
        }
        start = ((start) - (1i32)) as i32;
    }
    Ok(result)
}
#[doc = "Serialize a list-based binary tree to a comma-separated string.\n\n    Trims trailing nulls for compact output.\n    "]
pub fn tree_serialize(tree: &Vec<i32>) -> Result<String, Box<dyn std::error::Error>> {
    let mut result: String = Default::default();
    let mut last_valid: i32 = Default::default();
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(STR_EMPTY.to_string());
    }
    last_valid = ((_cse_temp_0) - (1i32)) as i32;
    while (last_valid >= 0)
        && (tree
            .get(last_valid as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            == -1)
    {
        last_valid = ((last_valid) - (1i32)) as i32;
    }
    let _cse_temp_2 = last_valid < 0;
    if _cse_temp_2 {
        return Ok(STR_EMPTY.to_string());
    }
...
#[doc = "Deserialize a comma-separated string into a list-based binary tree.\n\n    Uses manual tokenization with complex conditional logic.\n    "]
pub fn tree_deserialize(data: &str) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    ...
    let mut result: Vec<i32> = vec![];
    current_token = STR_EMPTY.to_string().to_string();
    let mut i: i32 = 0;
    while i < data.len() as i32 {
        ...
        if ch == "," {
            if current_token == "null" {
                ...
                result.push(current_token.parse::<i32>().unwrap_or_default());
            }
            current_token = STR_EMPTY.to_string();
        } else {
            current_token = format!("{}{}", current_token, ch);
        }
        i = ((i).py_add(1i32)) as i32;
    }
    let _cse_temp_2 = current_token.len() as i32;
    let _cse_temp_3 = _cse_temp_2 > 0;
    if _cse_temp_3 {
        let _cse_temp_4 = current_token == "null";
        if _cse_temp_4 {
            result.push(-1);
        } else {
            result.push(current_token.parse::<i32>().unwrap_or_default());
...
#[doc = "Compute the diameter(longest root-to-leaf distance) via BFS from root."]
pub fn tree_diameter(tree: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    while front < queue.len() as i32 {
        ...
        let left: i32 = (((2i32).py_mul(cur) as i32).py_add(1i32)) as i32;
        if (left < tree.len() as i32)
            && (tree
                .get(left as usize)
                .cloned()
                ...
                != -1)
        {
            queue.push((left, (dist).py_add(1i32)));
        }
        let right: i32 = (((2i32).py_mul(cur) as i32).py_add(2i32)) as i32;
        if (right < tree.len() as i32)
            && (tree
                .get(right as usize)
                .cloned()
                ...
                != -1)
        {
            queue.push((right, (dist).py_add(1i32)));
        }
    }
    Ok(max_dist)
}
#[doc = "Check if there exists a root-to-leaf path with the given sum.\n\n    Uses iterative DFS with a stack tracking(index, running_sum).\n    "]
pub fn path_sum_exists(
    tree: &Vec<i32>,
    target_sum: i32,
) -> Result<bool, Box<dyn std::error::Error>> {
    let _cse_temp_0 = tree.len() as i32;
    ...
    while stack.len() as i32 > 0 {
        ...
        let left: i32 = (((2i32).py_mul(idx) as i32).py_add(1i32)) as i32;
        let right: i32 = (((2i32).py_mul(idx) as i32).py_add(2i32)) as i32;
        let has_left: bool = (left < tree.len() as i32)
            && (tree
                .get(left as usize)
                .cloned()
                .expect("IndexError: list index out of range")
                != -1);
        let has_right: bool = (right < tree.len() as i32)
            && (tree
                .get(right as usize)
                .cloned()
                .expect("IndexError: list index out of range")
...
#[doc = "Check if a binary tree is symmetric around its center."]
pub fn is_mirror(tree: &Vec<i32>) -> Result<bool, Box<dyn std::error::Error>> {
    let mut right_val: i32 = Default::default();
    let mut left_val: i32 = Default::default();
    let _cse_temp_0 = tree.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    ...
    let mut queue: Vec<(i32, i32)> = vec![(1, 2)];
    let mut front: i32 = 0;
    while front < queue.len() as i32 {
        let left_idx: i32 = queue
            .get(front as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            .0;
        let right_idx: i32 = queue
            .get(front as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            .1;
        ...
        left_val = -1;
        right_val = -1;
        if left_idx < tree.len() as i32 {
            left_val = tree
                .get(left_idx as usize)
                .cloned()
                .expect("IndexError: list index out of range");
        }
        if right_idx < tree.len() as i32 {
            right_val = tree
                .get(right_idx as usize)
                .cloned()
                .expect("IndexError: list index out of range");
        }
        ...
        queue.push((
            ((2i32).py_mul(left_idx) as i32).py_add(1i32),
            ((2i32).py_mul(right_idx) as i32).py_add(2i32),
        ));
        queue.push((
            ((2i32).py_mul(left_idx) as i32).py_add(2i32),
            ((2i32).py_mul(right_idx) as i32).py_add(1i32),
        ));
    }
    Ok(true)
}
#[doc = "Collect nodes level by level. Returns list of lists per level."]
pub fn level_order_traversal(tree: &Vec<i32>) -> Result<Vec<Vec<i32>>, Box<dyn std::error::Error>> {
    ...
    while front < queue.len() as i32 {
        ...
        if (left < tree.len() as i32)
            && (tree
                ...
                != -1)
        {
            queue.push((left, (level).py_add(1i32)));
        }
        let right: i32 = (((2i32).py_mul(idx) as i32).py_add(2i32)) as i32;
        if (right < tree.len() as i32)
            && (tree
                ...
                != -1)
        {
            queue.push((right, (level).py_add(1i32)));
        }
    }
    Ok(result)
}
#[doc = "Count valid nodes at each depth level. Returns dict depth -> count."]
pub fn count_nodes_per_depth(
    tree: &Vec<i32>,
...
#[doc = "Execute a sequence of priority queue operations on a min-heap.\n\n    Each operation is(op_code, value):\n    - op_code 0: push value onto the queue\n    - op_code 1: pop minimum and append to output(value ignored)\n    Returns the list of popped values in order.\n    "]
pub fn priority_queue_ops(
    operations: &Vec<(i32, i32)>,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut smallest: i32 = Default::default();
    let mut heap: Vec<(i32, i32)> = vec![];
    let mut output: Vec<i32> = vec![];
    let mut op_idx: i32 = 0;
    while op_idx < operations.len() as i32 {
        let op_code: i32 = operations
            .get(op_idx as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            .0;
        let op_val: i32 = operations
            .get(op_idx as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            .1;
        op_idx = ((op_idx).py_add(1i32)) as i32;
        if op_code == 0 {
            heap.push((op_val, op_val));
            let mut current: i32 = ((heap.len() as i32) - (1i32) as i32) as i32;
            while current > 0 {
                let parent: i32 = {
                    let a = (current) - (1i32);
                    let b = 2;
                ...
                if heap
                    ...
                    < heap
                        ...
                        .0
                {
                    let temp: (i32, i32) = heap
                        .get(current as usize)
                        .cloned()
                        .expect("IndexError: list index out of range");
                    heap[(current) as usize] = heap
                        .get(parent as usize)
                        .cloned()
                        .expect("IndexError: list index out of range");
                    heap[(parent) as usize] = temp;
                    current = parent;
                } else {
                    break;
                }
            ...
            if (op_code == 1) && (heap.len() as i32 > 0) {
                output.push(
                    heap.get(0usize)
                        .cloned()
                        .expect("IndexError: list index out of range")
                ...
                if heap.len() as i32 == 1 {
                    heap = vec![];
                } else {
                    heap[(0) as usize] = {
                        let base = &heap;
                        let idx: i32 = (heap.len() as i32) - (1i32);
                        let actual_idx = if idx < 0 {
                            base.len().saturating_sub(idx.abs() as usize)
                        } else {
                            idx as usize
                    ...
                    heap = {
                        let base = &*heap;
                        let stop_idx = ((heap.len() as i32) - (1i32)) as isize;
                        let stop = if stop_idx < 0 {
                            (base.len() as isize + stop_idx).max(0) as usize
                        } else {
                            stop_idx as usize
                    ...
                    loop {
                        ...
                        let temp2: (i32, i32) = heap
                            .get(cur as usize)
                            .cloned()
                            .expect("IndexError: list index out of range");
                        heap[(cur) as usize] = heap
                            .get(smallest as usize)
                            .cloned()
                            .expect("IndexError: list index out of range");
                        heap[(smallest) as usize] = temp2;
                        cur = smallest;
                    }
                }
            }
...
#[doc = "Test all tree/trie/heap functions with concrete data."]
#[doc = " Depyler: proven to terminate"]
pub fn test_all() -> Result<bool, Box<dyn std::error::Error>> {
    let mut all_pass: bool = Default::default();
    all_pass = true;
    let mut t1: Vec<i32> = bt_insert_level_order(&vec![], 10)?;
    t1 = bt_insert_level_order(&t1, 5)?;
    t1 = bt_insert_level_order(&t1, 15)?;
    let _cse_temp_0 = t1.len() as i32;
    ...
    let tree_a: Vec<i32> = vec![10, 5, 15, 3, 7, -1, 20];
    let _cse_temp_4 = (!bt_search(&tree_a, 7)?) || (bt_search(&tree_a, 99)?);
    if _cse_temp_4 {
        all_pass = false;
    }
    let pre: Vec<i32> = bt_preorder(&tree_a)?;
    let _cse_temp_5 = pre.len() as i32;
    let _cse_temp_6 = _cse_temp_5 == 0;
    let _cse_temp_7 = (_cse_temp_6) || (_cse_temp_2);
    if _cse_temp_7 {
        all_pass = false;
    }
    let post: Vec<i32> = bt_postorder(&tree_a)?;
    let _cse_temp_8 = post.len() as i32;
    let _cse_temp_9 = _cse_temp_8 == 0;
    let _cse_temp_10 = (_cse_temp_9) || (_cse_temp_2);
    if _cse_temp_10 {
        all_pass = false;
    }
    let mut bst2: Vec<i32> = bst_insert(&vec![], 50)?;
    bst2 = bst_insert(&bst2, 30)?;
    bst2 = bst_insert(&bst2, 70)?;
    let _cse_temp_11 = (!bst_search(&bst2, 50)?) || (!bst_search(&bst2, 30)?);
    let _cse_temp_12 = (_cse_temp_11) || (bst_search(&bst2, 99)?);
    if _cse_temp_12 {
        all_pass = false;
    }
    if !is_valid_bst(&vec![8, 4, 12, 2, 6, 10, 14])? {
        all_pass = false;
    }
    if is_valid_bst(&vec![8, 12, 4]).unwrap_or(false) {
        all_pass = false;
    }
    if !is_valid_bst(&vec![])? {
        all_pass = false;
    }
    let _cse_temp_13 = tree_height(&tree_a, 0)? < 2;
    if _cse_temp_13 {
        all_pass = false;
    }
    let _cse_temp_14 = tree_height(&vec![], 0)? != 0;
    if _cse_temp_14 {
        all_pass = false;
    }
    let _cse_temp_15 = tree_depth_of_node(&tree_a, 10)? != 0;
    if _cse_temp_15 {
        all_pass = false;
    }
    let _cse_temp_16 = tree_depth_of_node(&tree_a, 7)? != 2;
    if _cse_temp_16 {
        all_pass = false;
    }
    let _cse_temp_17 = tree_depth_of_node(&tree_a, 99)? != -1;
    if _cse_temp_17 {
        all_pass = false;
    }
    let balanced: Vec<i32> = vec![10, 5, 15, 3, 7, 12, 20];
    let bf: i32 = balance_factor(&balanced, 0)?;
    let _cse_temp_18 = bf < -1;
    let _cse_temp_19 = bf > 1;
    let _cse_temp_20 = (_cse_temp_18) || (_cse_temp_19);
    if _cse_temp_20 {
        all_pass = false;
    }
    let lca_tree: Vec<i32> = vec![20, 10, 30, 5, 15, 25, 35];
    let _cse_temp_21 = find_lca(&lca_tree, 5, 15)? != 10;
    if _cse_temp_21 {
        all_pass = false;
    }
    let _cse_temp_22 = find_lca(&lca_tree, 5, 35)? != 20;
    if _cse_temp_22 {
        all_pass = false;
    }
    let mut trie: std::collections::HashMap<String, std::collections::HashMap<String, String>> = {
        let map: HashMap<String, std::collections::HashMap<String, String>> = HashMap::new();
        map
    };
    ...
    let _cse_temp_23 = (trie_search(&trie, &"ca")?) || (trie_search(&trie, &"cats")?);
    if _cse_temp_23 {
        all_pass = false;
    }
    let ca_words: Vec<String> = trie_prefix_search(&trie, &"ca")?;
    let _cse_temp_24 = ca_words.len() as i32;
    let _cse_temp_25 = _cse_temp_24 != 2;
    if _cse_temp_25 {
        all_pass = false;
    }
    let empty_words: Vec<String> = trie_prefix_search(&trie, &"xyz")?;
    let _cse_temp_26 = empty_words.len() as i32;
    let _cse_temp_27 = _cse_temp_26 != 0;
    if _cse_temp_27 {
        all_pass = false;
    }
    let seg_arr: Vec<i32> = vec![1, 3, 5, 7, 9, 11];
    let mut seg: Vec<i32> = segment_tree_build(&seg_arr)?;
    let _cse_temp_28 = seg_arr.len() as i32;
    let n_seg: i32 = _cse_temp_28;
    let _cse_temp_29 = segment_tree_query(&seg, n_seg, 0, 3)? != 9;
    if _cse_temp_29 {
        all_pass = false;
    }
    let _cse_temp_30 = segment_tree_query(&seg, n_seg, 1, 4)? != 15;
    if _cse_temp_30 {
        all_pass = false;
    }
    seg = segment_tree_update(&seg, n_seg, 2, 10)?;
    let _cse_temp_31 = segment_tree_query(&seg, n_seg, 0, 3)? != 14;
    if _cse_temp_31 {
        all_pass = false;
    }
    let fen_arr: Vec<i32> = vec![3, 2, -1, 6, 5, 4, -3, 3, 7, 2, 3];
    let bit: Vec<i32> = fenwick_build(&fen_arr)?;
    let result: (i32, Vec<i32>) = fenwick_query_and_update(&bit, 4, 2, 5)?;
    let _cse_temp_32 = result.0 != 15;
    if _cse_temp_32 {
        all_pass = false;
    }
    let result2: (i32, Vec<i32>) = fenwick_query_and_update(&result.1, 4, 0, 0)?;
    let _cse_temp_33 = result2.0 != 20;
    if _cse_temp_33 {
        all_pass = false;
    }
    let sorted_vals: (Vec<i32>, Vec<i32>) = heap_push_pop(&vec![], &vec![9, 1, 5, 3, 7])?;
    let _cse_temp_34 = sorted_vals.0.len() as i32;
    let _cse_temp_35 = _cse_temp_34 != 5;
    if _cse_temp_35 {
        all_pass = false;
    }
    let _cse_temp_36 = sorted_vals
        .0
        .get(0usize)
        .cloned()
        .expect("IndexError: list index out of range")
        != 1;
    let _cse_temp_37 = sorted_vals
        .0
        .get(1usize)
        .cloned()
        .expect("IndexError: list index out of range")
    ...
    let heaped: Vec<i32> = heapify(&vec![9, 5, 6, 2, 3])?;
    let _cse_temp_39 = heaped
        .get(0usize)
        .cloned()
        .expect("IndexError: list index out of range")
        != 2;
    ...
    let orig: Vec<i32> = vec![10, 5, 15, 3, 7, -1, 20];
    let serialized: String = tree_serialize(&orig)?;
    let deserialized: Vec<i32> = tree_deserialize(&serialized)?;
    let _cse_temp_40 = deserialized.len() as i32;
    let _cse_temp_41 = _cse_temp_40 != 7;
    let _cse_temp_42 = (_cse_temp_41) || (_cse_temp_2);
    let _cse_temp_43 = deserialized
        .get(5usize)
        .cloned()
        .expect("IndexError: list index out of range")
        != -1;
    let _cse_temp_44 = (_cse_temp_42) || (_cse_temp_43);
    if _cse_temp_44 {
        all_pass = false;
    }
    let _cse_temp_45 = tree_diameter(&vec![1, 2, 3, 4, 5, 6, 7])? < 2;
    if _cse_temp_45 {
        all_pass = false;
    }
    let _cse_temp_46 = tree_diameter(&vec![])? != 0;
    if _cse_temp_46 {
        all_pass = false;
    }
    let ps_tree: Vec<i32> = vec![10, 5, 15, 3, 7, -1, 20];
    if _cse_temp_11 {
        all_pass = false;
    }
    if path_sum_exists(&ps_tree, 100).unwrap_or(false) {
        all_pass = false;
    }
    if !is_mirror(&vec![1, 2, 2, 3, 4, 4, 3])? {
        all_pass = false;
    }
    if is_mirror(&vec![1, 2, 2, -1, 3, -1, 3]).unwrap_or(false) {
        all_pass = false;
    }
    let levels: Vec<Vec<i32>> = level_order_traversal(&vec![1, 2, 3, 4, 5, 6, 7])?;
    let _cse_temp_47 = levels.len() as i32;
    let _cse_temp_48 = _cse_temp_47 != 3;
    if _cse_temp_48 {
        all_pass = false;
    }
    let _cse_temp_49 = _cse_temp_47 > 2;
    let _cse_temp_50 = _cse_temp_34 != 4;
    let _cse_temp_51 = (_cse_temp_49) && (_cse_temp_50);
    if _cse_temp_51 {
        all_pass = false;
    }
    let _cse_temp_52 = level_order_traversal(&vec![])?.len() as i32;
    let _cse_temp_53 = _cse_temp_52 != 0;
    if _cse_temp_53 {
        all_pass = false;
    }
    let depth_counts: std::collections::HashMap<i32, i32> =
        count_nodes_per_depth(&vec![1, 2, 3, 4, 5, 6, 7])?;
    let _cse_temp_54 = depth_counts.get(&0).is_none();
    let _cse_temp_55 = (_cse_temp_54) || (_cse_temp_36);
    if _cse_temp_55 {
        all_pass = false;
    }
    let _cse_temp_56 = depth_counts.get(&2).is_none();
    let _cse_temp_57 = depth_counts.get(&(2)).cloned().unwrap_or_default() != 4;
    let _cse_temp_58 = (_cse_temp_56) || (_cse_temp_57);
    if _cse_temp_58 {
        all_pass = false;
    }
    let ops: Vec<(i32, i32)> = vec![
        (0, 5),
        (0, 1),
        (0, 3),
        (0, 2),
    ...
    let pq_out: Vec<i32> = priority_queue_ops(&ops)?;
    let _cse_temp_59 = pq_out.len() as i32;
    let _cse_temp_60 = _cse_temp_59 != 4;
    if _cse_temp_60 {
        all_pass = false;
    }
    let _cse_temp_61 = (_cse_temp_36) || (_cse_temp_39);
    let _cse_temp_62 = (_cse_temp_61) || (_cse_temp_37);
    let _cse_temp_63 = pq_out
        .get(3usize)
        .cloned()
        .expect("IndexError: list index out of range")
        != 5;
    let _cse_temp_64 = (_cse_temp_62) || (_cse_temp_63);
    if _cse_temp_64 {
        all_pass = false;
    }
    Ok(all_pass)
...
mod tests {
    ...
    #[test]
    fn test_bt_preorder_examples() {
        assert_eq!(bt_preorder(vec![]), vec![]);
        assert_eq!(bt_preorder(vec![1]), vec![1]);
    }
    #[test]
    fn test_bt_postorder_examples() {
        assert_eq!(bt_postorder(vec![]), vec![]);
        assert_eq!(bt_postorder(vec![1]), vec![1]);
    }
    #[test]
    fn test_is_valid_bst_examples() {
        let _ = is_valid_bst(Default::default());
    }
    #[test]
    fn test_segment_tree_build_examples() {
        assert_eq!(segment_tree_build(vec![]), vec![]);
        assert_eq!(segment_tree_build(vec![1]), vec![1]);
    }
    #[test]
    fn test_fenwick_build_examples() {
        assert_eq!(fenwick_build(vec![]), vec![]);
        assert_eq!(fenwick_build(vec![1]), vec![1]);
    }
    #[test]
    fn test_heapify_examples() {
        assert_eq!(heapify(vec![]), vec![]);
        assert_eq!(heapify(vec![1]), vec![1]);
    }
    #[test]
    fn test_tree_diameter_examples() {
        assert_eq!(tree_diameter(&vec![]), 0);
        assert_eq!(tree_diameter(&vec![1]), 1);
        assert_eq!(tree_diameter(&vec![1, 2, 3]), 3);
    }
    #[test]
    fn test_is_mirror_examples() {
        let _ = is_mirror(Default::default());
    }
    #[test]
    fn test_level_order_traversal_examples() {
        assert_eq!(level_order_traversal(vec![]), vec![]);
        assert_eq!(level_order_traversal(vec![1]), vec![1]);
    }
    #[test]
    fn test_priority_queue_ops_examples() {
        assert_eq!(priority_queue_ops(vec![]), vec![]);
        assert_eq!(priority_queue_ops(vec![1]), vec![1]);
    }
}