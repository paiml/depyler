...
pub fn square_list(nums: &Vec<i32>) -> Vec<i32> {
    let mut result: Vec<i32> = vec![];
    for n in nums.iter().cloned() {
        result.push((n).py_mul(n));
    }
    result
}
#[doc = " Depyler: verified panic-free"]
...
pub fn main() {
    let data: Vec<i32> = square_list(&vec![1, 2, 3, 4, 5]);
...
mod tests {
    ...
    #[test]
    fn test_square_list_examples() {
        assert_eq!(square_list(vec![]), Vec::<i32>::new());
        assert_eq!(square_list(vec![1]), vec![1]);
    }
}