...
pub fn strictly_decreasing(nums: &Vec<i32>) -> Result<bool, Box<dyn std::error::Error>> {
    let mut i: i32 = 1;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    if strictly_decreasing(&vec![5, 4, 3, 2, 1]).unwrap_or(false) {
        println!("{}", "yes");
    } else {
        println!("{}", "no");
    }
...
mod tests {
    ...
    #[test]
    fn test_strictly_decreasing_examples() {
        let _ = strictly_decreasing(Default::default());
    }
}
