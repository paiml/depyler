...
#![allow(dead_code)]
const STR_BANANA: &'static str = "banana";
...
#[doc = "Compute the union of two lists treated as sets, returning sorted unique elements."]
#[doc = " Depyler: verified panic-free"]
pub fn set_union<'a, 'b>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> Vec<i32> {
    let mut seen: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    for x in a.iter().cloned() {
        seen.insert(x.clone(), true);
    }
    for x in b.iter().cloned() {
        seen.insert(x.clone(), true);
    }
    let mut result: Vec<i32> = vec![];
    for key in seen.keys().cloned() {
        result.push(key);
    }
    result.sort();
    result
}
#[doc = "Compute the intersection of two lists treated as sets."]
#[doc = " Depyler: verified panic-free"]
pub fn set_intersection<'b, 'a>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> Vec<i32> {
    let mut set_a: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    for x in a.iter().cloned() {
        set_a.insert(x.clone(), true);
    }
    let mut result: Vec<i32> = vec![];
    let mut added: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    for x in b.iter().cloned() {
        if (set_a.contains(&x)) && (added.get(&x).is_none()) {
            result.push(x);
            added.insert(x.clone(), true);
        }
    }
    result.sort();
    result
}
#[doc = "Compute elements in a but not in b(set difference)."]
#[doc = " Depyler: verified panic-free"]
pub fn set_difference<'a, 'b>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> Vec<i32> {
    let mut set_b: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    for x in b.iter().cloned() {
        set_b.insert(x.clone(), true);
    }
    let mut result: Vec<i32> = vec![];
    let mut added: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
    ...
    for x in a.iter().cloned() {
        if (set_b.get(&x).is_none()) && (added.get(&x).is_none()) {
            result.push(x);
            added.insert(x.clone(), true);
        }
    }
...
#[doc = "Compute the symmetric difference: elements in either but not both."]
#[doc = " Depyler: verified panic-free"]
pub fn set_symmetric_difference<'b, 'a>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> Vec<i32> {
    let mut set_a: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    ...
    for x in b.iter().cloned() {
        if (set_a.get(&x).is_none()) && (added.get(&x).is_none()) {
            result.push(x);
            added.insert(x.clone(), true);
        }
    }
...
#[doc = "Check if all elements of a are contained in b."]
#[doc = " Depyler: verified panic-free"]
pub fn is_subset<'a, 'b>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> bool {
    let mut set_b: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    ...
    for x in a.iter().cloned() {
        if set_b.get(&x).is_none() {
            return false;
        }
    }
    true
}
#[doc = "Check if a contains all elements of b."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_superset<'a, 'b>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> bool {
    is_subset(&b, &a)
}
#[doc = "Check if two lists treated as sets have no common elements."]
#[doc = " Depyler: verified panic-free"]
pub fn are_disjoint<'b, 'a>(a: &'a Vec<i32>, b: &'b Vec<i32>) -> bool {
    let mut set_a: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
...
#[doc = "Remove duplicates from a list preserving first-occurrence order."]
#[doc = " Depyler: verified panic-free"]
...
#[doc = "Count the number of unique elements in a list."]
#[doc = " Depyler: verified panic-free"]
pub fn count_unique(data: &Vec<i32>) -> i32 {
    ...
    count = 0;
    for __sanitized in seen.keys().cloned() {
        count = ((count).py_add(1i32)) as i32;
    }
    count
}
#[doc = "Compute the size of the power set of an n-element set(2^n)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn set_power_set_count(n: i32) -> i32 {
    let mut result: i32 = Default::default();
    result = 1;
    for __sanitized in 0..(n) {
        result = ((result).py_mul(2i32)) as i32;
    }
    result
}
#[doc = "Compute Jaccard similarity between two lists treated as sets.\n\n    J(A, B) = |A intersect B| / |A union B|. Returns 0.0 if both empty.\n    "]
#[doc = " Depyler: proven to terminate"]
pub fn jaccard_similarity_int<'b, 'a>(
    a: &'a Vec<i32>,
    b: &'b Vec<i32>,
) -> Result<f64, Box<dyn std::error::Error>> {
    let inter: Vec<i32> = set_intersection(&a, &b);
    let uni: Vec<i32> = set_union(&a, &b);
    let _cse_temp_0 = uni.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(0.0);
    }
    Ok(((inter.len() as i32) as f64).py_div((uni.len() as i32) as f64))
}
#[doc = "Partition unique elements into even and odd sets.\n\n    Returns [even_set, odd_set] where each is sorted and deduplicated.\n    "]
pub fn set_partition_even_odd(
    data: &Vec<i32>,
) -> Result<Vec<Vec<i32>>, Box<dyn std::error::Error>> {
    let mut evens: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    let mut odds: std::collections::HashMap<i32, bool> = {
        let map: HashMap<i32, bool> = HashMap::new();
        map
    };
    for x in data.iter().cloned() {
        if (x).py_mod(2i32) == 0 {
            evens.insert(x.clone(), true);
        } else {
            odds.insert(x.clone(), true);
        }
    }
    let mut even_list: Vec<i32> = vec![];
    for key in evens.keys().cloned() {
        even_list.push(key);
    }
    even_list.sort();
    let mut odd_list: Vec<i32> = vec![];
    for key in odds.keys().cloned() {
        odd_list.push(key);
    }
    odd_list.sort();
    Ok(vec![even_list.clone(), odd_list.clone()])
}
#[doc = "Compute the intersection of multiple lists treated as sets."]
#[doc = " Depyler: proven to terminate"]
pub fn multi_set_intersection(
    sets: &Vec<Vec<i32>>,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut result: Vec<i32> = Default::default();
    let _cse_temp_0 = sets.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(vec![]);
    }
...
#[doc = "Compute the union of multiple lists treated as sets."]
#[doc = " Depyler: proven to terminate"]
pub fn multi_set_union(sets: &Vec<Vec<i32>>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut result: Vec<i32> = Default::default();
    let _cse_temp_0 = sets.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
...
#[doc = "Compute union of two string lists treated as sets."]
#[doc = " Depyler: verified panic-free"]
pub fn string_set_union<'b, 'a>(a: &'a Vec<String>, b: &'b Vec<String>) -> Vec<String> {
    let mut seen: std::collections::HashMap<String, bool> = {
        let map: HashMap<String, bool> = HashMap::new();
        map
    };
    for x in a.iter().cloned() {
        seen.insert(x.to_string().clone(), true);
    }
    for x in b.iter().cloned() {
        seen.insert(x.to_string().clone(), true);
    }
...
#[doc = "Compute intersection of two string lists treated as sets."]
#[doc = " Depyler: verified panic-free"]
pub fn string_set_intersection<'b, 'a>(a: &'a Vec<String>, b: &'b Vec<String>) -> Vec<String> {
    let mut set_a: std::collections::HashMap<String, bool> = {
        let map: HashMap<String, bool> = HashMap::new();
        map
    };
    for x in a.iter().cloned() {
        set_a.insert(x.to_string().clone(), true);
    }
    let mut result: Vec<String> = vec![];
    let mut added: std::collections::HashMap<String, bool> = {
        let map: HashMap<String, bool> = HashMap::new();
        map
    };
    for x in b.iter().cloned() {
        if (set_a.contains(&x)) && (added.get(&x).is_none()) {
            result.push(x.clone());
            added.insert(x.to_string().clone(), true);
        }
    }
    result.sort();
    result
}
#[doc = "Comprehensive test exercising all set operation functions."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn test_all() -> Result<bool, Box<dyn std::error::Error>> {
    let u: Vec<i32> = set_union(&vec![1, 2, 3], &vec![3, 4, 5]);
    assert_eq!(u, vec![1, 2, 3, 4, 5]);
    assert_eq!(set_union(&vec![], &vec![1, 2]), vec![1, 2]);
    assert_eq!(set_union(&vec![1, 2], &vec![]), vec![1, 2]);
    let inter: Vec<i32> = set_intersection(&vec![1, 2, 3, 4], &vec![3, 4, 5, 6]);
    assert_eq!(inter, vec![3, 4]);
    assert_eq!(set_intersection(&vec![1, 2], &vec![3, 4]), vec![]);
    let diff: Vec<i32> = set_difference(&vec![1, 2, 3, 4], &vec![3, 4, 5]);
    assert_eq!(diff, vec![1, 2]);
    assert_eq!(set_difference(&vec![1, 2], &vec![1, 2]), vec![]);
    let sym: Vec<i32> = set_symmetric_difference(&vec![1, 2, 3], &vec![2, 3, 4]);
    assert_eq!(sym, vec![1, 4]);
    assert_eq!(set_symmetric_difference(&vec![1, 2], &vec![1, 2]), vec![]);
    assert_eq!(set_symmetric_difference(&vec![1], &vec![2]), vec![1, 2]);
    assert_eq!(is_subset(&vec![1, 2], &vec![1, 2, 3, 4]), true);
    assert_eq!(is_subset(&vec![1, 5], &vec![1, 2, 3]), false);
    assert_eq!(is_subset(&vec![], &vec![1, 2]), true);
    assert_eq!(is_superset(&vec![1, 2, 3, 4], &vec![1, 2]), true);
    assert_eq!(is_superset(&vec![1, 2], &vec![1, 2, 3]), false);
    assert_eq!(are_disjoint(&vec![1, 2], &vec![3, 4]), true);
    assert_eq!(are_disjoint(&vec![1, 2, 3], &vec![3, 4]), false);
    let uniq: Vec<i32> = unique_elements(&vec![3, 1, 2, 1, 3, 4, 2]);
    assert_eq!(uniq, vec![3, 1, 2, 4]);
    assert_eq!(count_unique(&vec![1, 2, 2, 3, 3, 3]), 3);
    assert_eq!(count_unique(&vec![]), 0);
    assert_eq!(set_power_set_count(0), 1);
    assert_eq!(set_power_set_count(3), 8);
    assert_eq!(set_power_set_count(5), 32);
    let j1: f64 = jaccard_similarity_int(&vec![1, 2, 3], &vec![2, 3, 4])?;
    assert!((j1 > 0.49) && (j1 < 0.51));
    let j2: f64 = jaccard_similarity_int(&vec![1, 2], &vec![3, 4])?;
    assert!(j2 < 0.01);
    let j3: f64 = jaccard_similarity_int(&vec![1, 2], &vec![1, 2])?;
    assert!(j3 > 0.99);
    let parts: Vec<Vec<i32>> = set_partition_even_odd(&vec![1, 2, 3, 4, 5, 6, 2, 4])?;
    assert_eq!(
        parts
            .get(0usize)
            .cloned()
    ...
    let multi_inter: Vec<i32> =
        multi_set_intersection(&vec![vec![1, 2, 3], vec![2, 3, 4], vec![3, 4, 5]])?;
    assert_eq!(multi_inter, vec![3]);
    assert_eq!(multi_set_intersection(&vec![]), vec![]);
    let multi_uni: Vec<i32> = multi_set_union(&vec![vec![1, 2], vec![3, 4], vec![2, 5]])?;
    assert_eq!(multi_uni, vec![1, 2, 3, 4, 5]);
    let su: Vec<String> = string_set_union(
        &vec!["apple".to_string(), STR_BANANA.to_string()],
        &vec![STR_BANANA.to_string(), "cherry".to_string()],
    );
    assert_eq!(su.len() as i32, 3);
    let si: Vec<String> = string_set_intersection(
        &vec![
            "apple".to_string(),
            STR_BANANA.to_string(),
            "cherry".to_string(),
        ],
        &vec![STR_BANANA.to_string(), "date".to_string()],
    );
    assert_eq!(si, vec![STR_BANANA.to_string()]);
    Ok(true)
}
#[doc = "Run all tests and report results."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let result: bool = test_all()?;
    if result {
        println!("{}", "All set operation tests passed!");
    }
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_count_unique_examples() {
        assert_eq!(count_unique(&vec![]), 0);
        assert_eq!(count_unique(&vec![1]), 1);
        assert_eq!(count_unique(&vec![1, 2, 3]), 3);
    }
    #[test]
    fn test_set_power_set_count_examples() {
        assert_eq!(set_power_set_count(0), 0);
        assert_eq!(set_power_set_count(1), 1);
        assert_eq!(set_power_set_count(-1), -1);
    }
    #[test]
    fn test_set_partition_even_odd_examples() {
        assert_eq!(set_partition_even_odd(vec![]), vec![]);
        assert_eq!(set_partition_even_odd(vec![1]), vec![1]);
    }
    #[test]
    fn test_multi_set_intersection_examples() {
        assert_eq!(multi_set_intersection(vec![]), vec![]);
        assert_eq!(multi_set_intersection(vec![1]), vec![1]);
    }
    #[test]
    fn test_multi_set_union_examples() {
        assert_eq!(multi_set_union(vec![]), vec![]);
        assert_eq!(multi_set_union(vec![1]), vec![1]);
    }
}