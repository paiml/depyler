...
pub fn count_negative(nums: &Vec<i32>) -> i32 {
...
pub fn main() {
    println!("{}", count_negative(&vec![-1, 2, -3, 4, 5, -6]));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_count_negative_examples() {
        assert_eq!(count_negative(&vec![]), 0);
        assert_eq!(count_negative(&vec![1]), 1);
        assert_eq!(count_negative(&vec![1, 2, 3]), 3);
    }
}
