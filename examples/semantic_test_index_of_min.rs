...
pub fn index_of_min(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    let mut min_idx: i32 = Default::default();
    min_idx = 0;
    let mut i: i32 = 1;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        index_of_min(&vec![3, 1, 4, 1, 5, 9, 2, 6]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_index_of_min_examples() {
        assert_eq!(index_of_min(&vec![]), 0);
        assert_eq!(index_of_min(&vec![1]), 1);
        assert_eq!(index_of_min(&vec![1, 2, 3]), 3);
    }
}