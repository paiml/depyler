...
pub fn sum_even(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        sum_even(&vec![1, 2, 3, 4, 5, 6]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_sum_even_examples() {
        assert_eq!(sum_even(&vec![]), 0);
        assert_eq!(sum_even(&vec![1]), 1);
        assert_eq!(sum_even(&vec![1, 2, 3]), 6);
    }
}