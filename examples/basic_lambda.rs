...
pub fn lambda_handler(
    _event: std::collections::HashMap<DepylerValue, DepylerValue>,
    _context: std::collections::HashMap<DepylerValue, DepylerValue>,
) -> HashMap<String, DepylerValue> {
    let status = 200;
    let message = "OK";
    {
        let mut map = HashMap::new();
        map.insert("statusCode".to_string(), DepylerValue::Int(status as i64));
        map.insert(
            "message".to_string(),
            DepylerValue::Str(message.to_string()),
        );
        map
    }
}
