...
pub fn main() {
    println!("{}", sum_positive(&vec![-1, 2, -3, 4, 5]));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_sum_positive_examples() {
        assert_eq!(sum_positive(&vec![]), 0);
        assert_eq!(sum_positive(&vec![1]), 1);
        assert_eq!(sum_positive(&vec![1, 2, 3]), 6);
    }
}
