...
pub fn count_valleys(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        count_valleys(&vec![3, 1, 4, 1, 5, 2, 6]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_count_valleys_examples() {
        assert_eq!(count_valleys(&vec![]), 0);
        assert_eq!(count_valleys(&vec![1]), 1);
        assert_eq!(count_valleys(&vec![1, 2, 3]), 3);
    }
}