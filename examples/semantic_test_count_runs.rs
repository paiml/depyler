...
pub fn count_runs(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        count_runs(&vec![1, 1, 2, 2, 2, 3, 3]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_count_runs_examples() {
        assert_eq!(count_runs(&vec![]), 0);
        assert_eq!(count_runs(&vec![1]), 1);
