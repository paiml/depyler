...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        find_first_even(&vec![1, 3, 5, 6, 7, 8]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_find_first_even_examples() {
        assert_eq!(find_first_even(&vec![]), 0);
        assert_eq!(find_first_even(&vec![1]), 1);
        assert_eq!(find_first_even(&vec![1, 2, 3]), 3);
    }
}