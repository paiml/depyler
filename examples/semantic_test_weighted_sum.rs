...
pub fn weighted_sum(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        weighted_sum(&vec![1, 2, 3, 4]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_weighted_sum_examples() {
        assert_eq!(weighted_sum(&vec![]), 0);
        assert_eq!(weighted_sum(&vec![1]), 1);
        assert_eq!(weighted_sum(&vec![1, 2, 3]), 6);
    }
}