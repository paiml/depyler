...
impl Point {
    ...
    pub fn distance_to(&self, other: &Point) -> f64 {
        let dx = self.x.clone() - (other.x as f64);
        let dy = self.y.clone() - (other.y as f64);
...
pub fn triangle_area_heron(a: f64, b: f64, c: f64) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = (a).py_add(b) <= c;
    let _cse_temp_1 = (a).py_add(c) <= b;
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    let _cse_temp_3 = (b).py_add(c) <= a;
    ...
    let _cse_temp_8 = (_cse_temp_7).py_mul((s).py_sub(b));
    let _cse_temp_9 = (_cse_temp_8).py_mul((s).py_sub(c));
...
#[doc = " Depyler: proven to terminate"]
pub fn line_intersection<'a, 'l1, 'c, 'b>(
    p1: &'a Point,
    p2: &'b Point,
    p3: &'c Point,
    p4: &'l1 Point,
    ...
    let _cse_temp_8 = (t).py_mul((y2).py_sub(y1));
    let intersection_y = (y1).py_add(_cse_temp_8);
