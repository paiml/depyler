...
pub fn mean(numbers: &Vec<f64>) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    Ok((numbers.iter().sum::<f64>()).py_div(numbers.len() as i32))
}
...
pub fn correlation<'a, 'b>(
    ...
    for i in 0..(n) {
        ...
        let dy = (y
            ...
            .expect("IndexError: list index out of range"))
        .py_sub(mean_y);
        numerator = (numerator).py_add((dx).py_mul(dy));
        sum_x_squared = (sum_x_squared).py_add((dx).py_mul(dx));
        sum_y_squared = (sum_y_squared).py_add((dy).py_mul(dy));
    }
    let _cse_temp_5 = (sum_x_squared).py_mul(sum_y_squared);
