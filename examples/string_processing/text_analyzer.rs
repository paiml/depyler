...
pub fn find_anagrams(words: &Vec<String>) -> Vec<Vec<String>> {
    ...
    for word in words.iter().cloned() {
        ...
        if groups.get(&sorted_chars).is_some() {
            groups
                .get(&DepylerValue::Int(sorted_chars as i64))
                .cloned()
                .unwrap_or_default()
                .push(word);
        } else {
...
pub fn longest_common_prefix(strings: &Vec<String>) -> Result<String, Box<dyn std::error::Error>> {
    ...
    for i in 0..(min_length) {
        ...
        for s in {
            ...
            if s.clone().py_index(DepylerValue::Int(i as i64)) != *char.to_string() {
                all_match = false;
                break;
            }
        }
        if all_match {
            prefix = (prefix).py_add(char);
        } else {
            break;
        }
    }
