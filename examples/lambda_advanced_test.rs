...
pub fn test_lambda_with_operations() -> (DepylerValue, DepylerValue, DepylerValue) {
    let calc = move |x: i64, y: i64, z: i64| ((x).py_add(y)).py_mul(z);
    let result1 = calc(2, 3, 4);
    let negate = move |x: i64| -x;
    let result2 = negate(5);
    let multiply = move |a: i64, b: i64| (a).py_mul(b);
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn test_lambda_as_argument() -> (DepylerValue, DepylerValue) {
    let numbers = vec![1, 2, 3, 4, 5];
    let apply_to_list = move |lst: &Vec<i32>, func: &dyn Fn(i32) -> i32| -> Vec<i32> {
        let mut result = vec![];
        for item in lst.iter().cloned() {
            result.push(DepylerValue::Str(format!("{:?}", func(item))));
        }
        return result;
    };
    let doubled = apply_to_list(&numbers, move |x: i64| (x).py_mul(2));
    let squared = apply_to_list(&numbers, move |x: i64| (x).py_mul(x));
