...
pub fn test_lambda_with_operations() -> (DepylerValue, DepylerValue, DepylerValue) {
    let calc = move |x, y, z| x.py_add(y).py_mul(z);
    let result1 = calc(2, 3, 4);
    let negate = move |x| -x;
    let result2 = negate(5);
    let multiply = move |a, b| a.py_mul(b);
...
pub fn test_lambda_as_argument() -> (DepylerValue, DepylerValue) {
    ...
    let doubled = apply_to_list(&numbers, move |x| x.py_mul(2));
    let squared = apply_to_list(&numbers, move |x| x.py_mul(x));
