...
impl ResourceManager {
    ...
    pub fn __exit__(&mut self, _exc_type: (), _exc_val: (), _exc_tb: ()) -> bool {
        self.is_open = false;
        return false;
    }
...
impl Counter {
    ...
    pub fn next(&mut self) -> i32 {
        if self.count.clone() < self.max_count.clone() {
...
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    let with_result: i32 = demo_with_statement();
    let iter_result: i32 = demo_iterator();
    let _ = (with_result).py_add(iter_result);
}
#[cfg(test)]
mod tests {
    use super::*;
