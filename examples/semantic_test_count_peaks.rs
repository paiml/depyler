...
pub fn count_peaks(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        count_peaks(&vec![1, 3, 2, 4, 1, 5, 2]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_count_peaks_examples() {
        assert_eq!(count_peaks(&vec![]), 0);
        assert_eq!(count_peaks(&vec![1]), 1);
        assert_eq!(count_peaks(&vec![1, 2, 3]), 3);
    }
}