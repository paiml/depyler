...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    if is_sorted(&vec![1, 2, 3, 4, 5]).unwrap_or(false) {
        println!("{}", "sorted");
    } else {
        println!("{}", "not sorted");
    }
...
mod tests {
    ...
    #[test]
    fn test_is_sorted_examples() {
        let _ = is_sorted(Default::default());
    }
}