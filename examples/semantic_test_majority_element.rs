...
pub fn majority_element(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    #[test]
    fn test_majority_element_examples() {
        assert_eq!(majority_element(&vec![]), 0);
        assert_eq!(majority_element(&vec![1]), 1);
        assert_eq!(majority_element(&vec![1, 2, 3]), 3);
    }
}
