...
pub fn count_zero(nums: &Vec<i32>) -> i32 {
...
pub fn main() {
    println!("{}", count_zero(&vec![0, 1, 0, 2, 0, 3]));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_count_zero_examples() {
        assert_eq!(count_zero(&vec![]), 0);
        assert_eq!(count_zero(&vec![1]), 1);
        assert_eq!(count_zero(&vec![1, 2, 3]), 3);
    }
}
