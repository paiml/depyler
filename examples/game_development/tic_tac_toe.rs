...
impl TicTacToe {
    pub fn new() -> Self {
    Self {
    board: Vec::new(), current_player: String::new()
}
} pub fn make_move(&mut self, row: i32, col: i32) -> bool {
    if ! self.is_valid_move(row, col) {
    return false;
    };
    self.board.clone().get_mut(& row).unwrap().insert(col, self.current_player.clone());
...
} #[derive(Debug, Clone)] pub struct AIPlayer {
    pub symbol: String, pub opponent: DepylerValue
}
impl AIPlayer {
    pub fn new(symbol: impl Into<String>) -> Self {
    Self {
    symbol: symbol.into(), opponent: Default::default()
}
} pub fn get_best_move(&self, game: & TicTacToe) -> (i32, i32) {
    let mut best_score = - 1000;
    let mut best_move  = (0, 0);
    for(row, col) in game.get_empty_positions() {
    game.board.get_mut(& row).unwrap().insert(col, self.symbol.clone());
    let score = self.minimax(game, 0, false);
    game.board.get_mut(& row).unwrap().insert(col, " ".to_string());
    if score>best_score {
...
pub fn minimax(&self, game: & TicTacToe, depth: i32, is_maximizing: bool) -> i32 {
    ...
    game.board.get_mut(& row).unwrap().insert(col, " ".to_string());
    let best_score  = ((best_score as f64)).max(score as f64);
    };
    return best_score;
    ...
    let mut best_score = 1000;
    for(row, col) in game.get_empty_positions() {
    game.board.get_mut(& row).unwrap().insert(col, self.opponent.clone());
    let score = self.minimax(game, depth + 1, true);
    game.board.get_mut(& row).unwrap().insert(col, " ".to_string());
    let best_score  = ((best_score as f64)).min (score as f64);
    };
    return best_score;
    };
   
}
} #[doc = "Play a complete game against computer"] pub fn play_computer_game() -> Result<String, Box<dyn std::error::Error>>{
    let mut game: TicTacToe = TicTacToe::new();
    let ai: AIPlayer = AIPlayer::new("O".to_string());
