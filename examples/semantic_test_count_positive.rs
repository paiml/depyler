...
pub fn count_positive(nums: &Vec<i32>) -> i32 {
...
pub fn main() {
    println!("{}", count_positive(&vec![-1, 2, -3, 4, 5, -6]));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_count_positive_examples() {
        assert_eq!(count_positive(&vec![]), 0);
        assert_eq!(count_positive(&vec![1]), 1);
        assert_eq!(count_positive(&vec![1, 2, 3]), 3);
    }
}
