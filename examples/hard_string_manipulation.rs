...
} #[doc = "Split a string by a single-character delimiter."] #[doc = " Depyler: verified panic-free"] pub fn string_split_by_char<'a, 'b>(text: & 'a str, delimiter: & 'b str) -> Vec<String>{
    let mut current: String = Default::default();
    let mut parts: Vec<String>= vec! [];
    current = STR_EMPTY.to_string().to_string();
    for ch in text.chars() {
    if ch.to_string() == (*delimiter) {
...
#[doc = "Join a list of strings with a separator."] #[doc = " Depyler: proven to terminate"] pub fn string_join<'b, 'a>(parts: & 'a Vec<String>, separator: & 'b str) -> Result<String, Box<dyn std::error::Error>>{
...
result = parts.get(0usize).cloned().expect("IndexError: list index out of range");
    for i in (1)..(parts.len() as i32) {
    result = format!("{}{}", format!("{}{}", result, separator), parts.get(i as usize).cloned().expect("IndexError: list index out of range"));
   
}
Ok(result.to_string())
}
#[doc = "Strip specified characters from both ends of a string."] pub fn string_strip_chars<'a, 'b>(text: & 'a str, chars: & 'b str) -> Result<String, Box<dyn std::error::Error>>{
    let mut result: String = Default::default();
...
#[doc = "Replace all occurrences of old substring with new substring."] pub fn string_replace_all<'b, 'a>(text: String, old: & 'a str, new: & 'b str) -> Result<String, Box<dyn std::error::Error>>{
    let mut result: String = Default::default();
    let mut r#match: bool = Default::default();
    let _cse_temp_0 = old.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
    return Ok(text.to_string().to_string());
...
result = STR_EMPTY.to_string().to_string();
    ...
    r#match = true;
    if(i).py_add(old.len() as i32)>text.len() as i32 {
    r#match = false;
   
}
else {
    for j in 0..(old.len() as i32) {
    if {
    let base = & text;
...
if r#match {
    result = format!("{}{}", result, new);
    i  = ((i).py_add(old.len() as i32)) as i32;
...
#[doc = "Find the first occurrence of pattern in text, return index or -1."] #[doc = " Depyler: proven to terminate"] pub fn string_find<'b, 'a>(text: & 'a str, pattern: & 'b str) -> Result<i32, Box<dyn std::error::Error>>{
...
let _cse_temp_2 = text.len() as i32;
    let _cse_temp_3 = _cse_temp_0>_cse_temp_2;
    if _cse_temp_3 {
    return Ok(- 1);
   
}
for i in 0..((((text.len() as i32) - (pattern.len() as i32) as i32)).py_add(1i32)) {
    let mut r#match: bool = true;
    for j in 0..(pattern.len() as i32) {
    if {
    let base = & text;
    let idx: i32  = (i).py_add(j);
    let actual_idx = if idx<0 {
...
#[doc = "Count non-overlapping occurrences of pattern in text."] pub fn string_count_occurrences<'a, 'b>(text: & 'a str, pattern: & 'b str) -> Result<i32, Box<dyn std::error::Error>>{
...
count = 0;
    let mut i: i32 = 0;
    while i<= (text.len() as i32) - (pattern.len() as i32) {
    let mut r#match: bool = true;
    for j in 0..(pattern.len() as i32) {
    if {
    let base = & text;
...
}
{
    ...
    i  = ((i).py_add(pattern.len() as i32)) as i32;
...
#[doc = "Check if text starts with the given prefix."] #[doc = " Depyler: proven to terminate"] pub fn string_starts_with<'a, 'b>(text: & 'a str, prefix: & 'b str) -> Result<bool, Box<dyn std::error::Error>>{
    let _cse_temp_0 = prefix.len() as i32;
...
#[doc = "Check if text ends with the given suffix."] #[doc = " Depyler: proven to terminate"] pub fn string_ends_with<'b, 'a>(text: & 'a str, suffix: & 'b str) -> Result<bool, Box<dyn std::error::Error>>{
    let _cse_temp_0 = suffix.len() as i32;
    let _cse_temp_1 = text.len() as i32;
    let _cse_temp_2 = _cse_temp_0>_cse_temp_1;
    if _cse_temp_2 {
    return Ok(false);
...
let offset: i32  = ((_cse_temp_1) - (_cse_temp_0)) as i32;
    for i in 0..(suffix.len() as i32) {
    if {
    let base = & text;
    let idx: i32  = (offset).py_add(i);
    let actual_idx = if idx<0 {
    base.chars().count().saturating_sub(idx.abs() as usize)
}
else {
...
#[doc = "Repeat a string count times."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn string_repeat(text: & str, count: i32) -> String {
    let mut result: String = Default::default();
    result = STR_EMPTY.to_string().to_string();
...
#[doc = "Reverse a string."] #[doc = " Depyler: proven to terminate"] pub fn string_reverse(text: & str) -> Result<String, Box<dyn std::error::Error>>{
    let mut result: String = Default::default();
    result = STR_EMPTY.to_string().to_string();
    for i in {
...
(- 1..(text.len() as i32) - (1i32)).rev().step_by(step.max(1))
}
{
    result = format!("{}{}", result, {
    let base = & text;
...
#[doc = "Check if a string is a palindrome."] pub fn string_is_palindrome(text: & str) -> Result<bool, Box<dyn std::error::Error>>{
    let mut left: i32 = 0;
    let _cse_temp_0 = text.len() as i32;
    let mut right: i32  = ((_cse_temp_0) - (1i32)) as i32;
    while left<right {
    if {
    let base = & text;
    let idx: i32 = left;
...
#[doc = "Pad a string on the left to reach the desired width."] #[doc = " Depyler: verified panic-free"] pub fn string_pad_left<'b, 'a>(text: & 'a str, width: i32, pad_char: & 'b str) -> String {
    let mut result: String = Default::default();
...
#[doc = "Pad a string on the right to reach the desired width."] #[doc = " Depyler: verified panic-free"] pub fn string_pad_right<'a, 'b>(text: & 'a str, width: i32, pad_char: & 'b str) -> String {
    let mut result: String = Default::default();
    result = text.clone().to_string();
    while(result.len() as i32)<width {
    result = format!("{}{}", result, pad_char);
   
}
result.to_string()
}
#[doc = "Center a string within a given width using pad characters."] #[doc = " Depyler: proven to terminate"] pub fn string_center(text: String, width: i32, pad_char: & str) -> Result<String, Box<dyn std::error::Error>>{
    let mut result: String = Default::default();
    let _cse_temp_0 = text.len() as i32;
    let _cse_temp_1 = _cse_temp_0>= width;
    if _cse_temp_1 {
    return Ok(text.to_string().to_string());
   
}
let total_pad: i32  = ((width) - (_cse_temp_0)) as i32;
    let _cse_temp_2 = {
    let a = total_pad;
    let b = 2;
    let q = a / b;
    ...
    let left_pad: i32 = _cse_temp_2;
    let right_pad: i32  = ((total_pad) - (left_pad)) as i32;
    result = STR_EMPTY.to_string().to_string();
    for __sanitized in 0..(left_pad) {
    result = format!("{}{}", result, pad_char);
   
}
result = format!("{}{}", result, text);
    for __sanitized in 0..(right_pad) {
    result = format!("{}{}", result, pad_char);
   
}
Ok(result.to_string())
}
#[doc = "Count words in a string(space-separated)."] #[doc = " Depyler: verified panic-free"] pub fn word_count(text: & str) -> i32 {
    let mut count: i32 = Default::default();
    let _cse_temp_0 = text.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
...
let words: Vec<String>= string_split_by_char(text, & " ");
    count = 0;
...
#[doc = "Count the frequency of each character in a string."] pub fn char_frequency(text: & str) -> Result<HashMap<String, i32>, Box<dyn std::error::Error>>{
...
#[doc = "Find the longest common prefix among a list of strings."] #[doc = " Depyler: proven to terminate"] pub fn longest_common_prefix(strings: & Vec<String>) -> Result<String, Box<dyn std::error::Error>>{
...
prefix = strings.get(0usize).cloned().expect("IndexError: list index out of range");
    for i in (1)..(strings.len() as i32) {
    let mut new_prefix: String = STR_EMPTY.to_string();
    let s: String = strings.get(i as usize).cloned().expect("IndexError: list index out of range");
    let mut j: i32 = 0;
...
#[doc = "Apply Caesar cipher to lowercase letters only, leaving other chars unchanged."] pub fn caesar_cipher(text: & str, shift: i32) -> Result<String, Box<dyn std::error::Error>>{
    let mut result: String = Default::default();
    result = STR_EMPTY.to_string().to_string();
    for ch in text.chars() {
    if(ch>= 'a') &&(ch <= 'z') {
    let code: i32  = ((ch as u32 as i32) - ("a".chars().next().expect("empty string") as i32)) as i32;
    let shifted: i32  = ((((code).py_add(shift) as i32)).py_mod(26i32)) as i32;
    result = format!("{}{}", result, char::from_u32(((shifted).py_add("a".chars().next().expect("empty string") as i32)) as u32).expect("builtin operation failed").to_string());
...
#[doc = "Comprehensive test exercising all string manipulation functions."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn test_all() -> Result<bool, Box<dyn std::error::Error>>{
    let parts: Vec<String>= string_split_by_char(& "hello,world,foo", & ",");
    assert_eq!(parts.len() as i32, 3);
    assert_eq!(parts.get(0usize).cloned().expect("IndexError: list index out of range"), "hello");
    assert_eq!(parts.get(1usize).cloned().expect("IndexError: list index out of range"), "world");
    assert_eq!(parts.get(2usize).cloned().expect("IndexError: list index out of range"), "foo");
    assert_eq!(string_split_by_char(& "abc", & ","), vec! ["abc".to_string()]);
    assert_eq!(string_join (& vec! ["a".to_string(), "b".to_string(), "c".to_string()], & "-"), "a-b-c");
    assert_eq!(string_join (& vec! ["hello".to_string()], & " "), "hello");
    assert_eq!(string_join (& vec! [], & ","), STR_EMPTY);
    assert_eq!(string_strip_chars(& "  hello  ", & " "), "hello");
    assert_eq!(string_strip_chars(& "xxhelloxx", & "x"), "hello");
    assert_eq!(string_strip_chars(& "abc", & "x"), "abc");
    assert_eq!(string_replace_all("hello world".to_string(), & "world", & "rust"), "hello rust");
    assert_eq!(string_replace_all("aabaa".to_string(), & "aa", & "x"), "xbx");
    assert_eq!(string_replace_all("abc".to_string(), & "d", & "e"), "abc");
    assert_eq!(string_find(& "hello world", & "world"), 6);
    assert_eq!(string_find(& "hello world", & "xyz"), - 1);
    assert_eq!(string_find(& "hello", & STR_EMPTY), 0);
    assert_eq!(string_find(& "abc", & "abcd"), - 1);
    assert_eq!(string_count_occurrences(& "abababab", & "ab"), 4);
    assert_eq!(string_count_occurrences(& "aaaa", & "aa"), 2);
    assert_eq!(string_count_occurrences(& "hello", & "xyz"), 0);
    assert_eq!(string_starts_with(& "hello world", & "hello"), true);
    assert_eq!(string_starts_with(& "hello", & "world"), false);
    assert_eq!(string_starts_with(& "hi", & "hello"), false);
    assert_eq!(string_ends_with(& "hello world", & "world"), true);
    assert_eq!(string_ends_with(& "hello", & "world"), false);
    assert_eq!(string_ends_with(& "hi", & "hello"), false);
    assert_eq!(string_repeat(& "ab", 3), "ababab");
    assert_eq!(string_repeat(& "x", 0), STR_EMPTY);
    assert_eq!(string_repeat(& "hi", 1), "hi");
    assert_eq!(string_reverse(& "hello"), "olleh");
    assert_eq!(string_reverse(& "a"), "a");
    assert_eq!(string_reverse(& STR_EMPTY), STR_EMPTY);
    assert_eq!(string_is_palindrome(& "racecar"), true);
    assert_eq!(string_is_palindrome(& "hello"), false);
    assert_eq!(string_is_palindrome(& "a"), true);
    assert_eq!(string_is_palindrome(& STR_EMPTY), true);
    assert_eq!(string_pad_left(& "42", 5, & "0"), "00042");
    assert_eq!(string_pad_left(& "hello", 3, & " "), "hello");
    assert_eq!(string_pad_right(& "hi", 5, & "."), "hi...");
    assert_eq!(string_pad_right(& "hello", 3, & " "), "hello");
    assert_eq!(string_center("hi".to_string(), 6, & "-"), "--hi--");
    assert_eq!(string_center("x".to_string(), 4, & "*"), "*x**");
    assert_eq!(string_center("long".to_string(), 2, & " "), "long");
    assert_eq!(word_count(& "hello world foo bar"), 4);
    assert_eq!(word_count(& "single"), 1);
    assert_eq!(word_count(& STR_EMPTY), 0);
    let freq: std::collections::HashMap<String, i32>= char_frequency(& "aabbc") ?;
    assert_eq!(freq.get("a").cloned().unwrap_or_default(), 2);
    assert_eq!(freq.get("b").cloned().unwrap_or_default(), 2);
    assert_eq!(freq.get("c").cloned().unwrap_or_default(), 1);
    assert_eq!(longest_common_prefix(& vec! ["flower".to_string(), "flow".to_string(), "flight".to_string()]), "fl");
    assert_eq!(longest_common_prefix(& vec! ["dog".to_string(), "racecar".to_string()]), STR_EMPTY);
    assert_eq!(longest_common_prefix(& vec! ["abc".to_string()]), "abc");
    assert_eq!(longest_common_prefix(& vec! []), STR_EMPTY);
    assert_eq!(caesar_cipher(& "abc", 1), "bcd");
    assert_eq!(caesar_cipher(& "xyz", 3), "abc");
    assert_eq!(caesar_cipher(& "hello world", 13), "uryyb jbeyq");
    assert_eq!(caesar_cipher(& caesar_cipher(& "hello", 13) ?, 13), "hello");
    Ok(true)
}
#[doc = "Run all tests and report results."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), Box<dyn std::error::Error>>{
    let result: bool = test_all() ?;
    if result {
    println!("{}", "All string manipulation tests passed!");
...
#[cfg(test)] mod tests {
    ...
    #[test] fn test_string_is_palindrome_examples() {
    let _ = string_is_palindrome(Default::default());
   
}
#[test] fn test_word_count_examples() {
