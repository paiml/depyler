...
pub fn first_positive(nums: &Vec<i32>) -> i32 {
...
pub fn main() {
    println!("{}", first_positive(&vec![-1, -2, 3, 4]));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_first_positive_examples() {
        assert_eq!(first_positive(&vec![]), 0);
        assert_eq!(first_positive(&vec![1]), 1);
        assert_eq!(first_positive(&vec![1, 2, 3]), 3);
    }
}
