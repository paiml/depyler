...
pub fn count_odd(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        count_odd(&vec![1, 2, 3, 4, 5, 6, 7]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_count_odd_examples() {
        assert_eq!(count_odd(&vec![]), 0);
        assert_eq!(count_odd(&vec![1]), 1);
        assert_eq!(count_odd(&vec![1, 2, 3]), 3);
    }
}