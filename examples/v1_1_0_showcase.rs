...
pub fn showcase_dictionary_assignment() -> (
    ...
    coords.insert(
        (10, 20).to_string(),
        DepylerValue::Str("location A".to_string().to_string()),
    );
    coords.insert(
        (30, 40).to_string(),
        DepylerValue::Str("location B".to_string().to_string()),
    );
    (nested, coords)
}
...
pub fn showcase_set_operations() -> (
    ...
    let intersection = _cse_temp_0.clone();
    let _cse_temp_1 = set1
        .union(&set2)
        .cloned()
        .collect::<std::collections::HashSet<_>>();
    let union = _cse_temp_1.clone();
    let difference = (set1).py_sub(set2);
    let _cse_temp_2 = set1
        .symmetric_difference(&set2)
        .cloned()
        .collect::<std::collections::HashSet<_>>();
    let symmetric_diff = _cse_temp_2.clone();
