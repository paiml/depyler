...
pub fn all_same(nums: &Vec<i32>) -> Result<bool, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    if all_same(&vec![5, 5, 5, 5]).unwrap_or(false) {
        println!("{}", "yes");
    } else {
        println!("{}", "no");
    }
...
mod tests {
    ...
    #[test]
    fn test_all_same_examples() {
        let _ = all_same(Default::default());
    }
}
