...
pub fn digit_sum(n: i32) -> Result<i32, Box<dyn std::error::Error>> {
    let _cse_temp_0 = n < 10;
    if _cse_temp_0 {
        return Ok(n);
    }
    Ok({
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", digit_sum(12345).expect("operation failed"));
    Ok(())
}
#[cfg(test)]
mod tests {
