...
#[doc = "Test environment variable access"] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn test_env_variable_access() -> String {
    let home: String = std::env::var("HOME".to_string()).unwrap_or_else(| _ | "/home/user".to_string());
    home.to_string()
}
...
(- 1..(path.len() as i32).py_sub(1)).rev().step_by(step.max(1))
}
{
    if {
    let base = & path;
...
(- 1..(filename.len() as i32).py_sub(1)).rev().step_by(step.max(1))
}
{
    if {
    let base = & filename;
