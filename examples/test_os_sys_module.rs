...
(- 1..(path.len() as i32).saturating_sub(1)).rev().step_by(step.max(1))
}
{
    if {
    let base = & path;
...
(- 1..(filename.len() as i32).saturating_sub(1)).rev().step_by(step.max(1))
}
{
    if {
    let base = & filename;
...
path = parts.get(0usize).cloned().expect("IndexError: list index out of range");
    ...
    path = format!("{}{}", format!("{}{}", path, STR__), parts.get(i as usize).cloned().expect("IndexError: list index out of range"));
