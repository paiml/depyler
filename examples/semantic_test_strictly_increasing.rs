...
pub fn strictly_increasing(nums: &Vec<i32>) -> Result<bool, Box<dyn std::error::Error>> {
    let mut i: i32 = 1;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    if strictly_increasing(&vec![1, 2, 3, 4, 5]).unwrap_or(false) {
        println!("{}", "yes");
    } else {
        println!("{}", "no");
    }
...
mod tests {
    ...
    #[test]
    fn test_strictly_increasing_examples() {
        let _ = strictly_increasing(Default::default());
    }
}
