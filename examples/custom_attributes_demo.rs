...
pub fn compute_hash(text: &str) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    for char in text.chars() {
        hash_val = (((hash_val).py_mul(31)).py_add(char as u32 as i32)).py_mod(
            ({ 2 } as i32)
...
pub fn main() {
    println!("{}", format!("add_numbers(5, 3) = {}", add_numbers(5, 3)));
    println!(
        "{}",
        format!("multiply_fast(4, 7) = {}", multiply_fast(4, 7))
    );
    println!(
        "{}",
        format!(
            "calculate_checksum([1, 2, 3, 4]) = {}",
            calculate_checksum(&vec![1, 2, 3, 4])
        )
    );
    println!(
        "{}",
