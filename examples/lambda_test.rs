...
pub fn test_simple_lambdas() -> (DepylerValue, DepylerValue, DepylerValue) {
    let add = move |x: i32, y: i32| (x).py_add(y);
    let square = move |x: i32| (x).py_mul(x);
...
pub fn test_lambda_in_expressions() -> (DepylerValue, DepylerValue) {
    let get_operation = move |is_add: bool| {
        if is_add {
            move |x: i32, y: i32| (x).py_add(y)
        } else {
            move |x: i32, y: i32| (x).py_sub(y)
        }
    };
...
pub fn test_lambda_with_closure() {
    ...
    let scale = move |x: i32| (x).py_mul(multiplier);
    let result = scale(7);
    let _ = result;
}
