...
pub fn test_simple_lambdas() -> (DepylerValue, DepylerValue, DepylerValue) {
    let add = move |x: i64, y: i64| (x).py_add(y);
    let square = move |x: i64| (x).py_mul(x);
    let constant = move || 42;
    let result1 = add(3, 5);
    let result2 = square(4);
    let result3 = constant();
    return (
        DepylerValue::from(result1),
        DepylerValue::from(result2),
        DepylerValue::from(result3),
    );
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn test_lambda_with_list_operations() -> (Vec<DepylerValue>, Vec<DepylerValue>, DepylerValue) {
    let numbers = vec![1, 2, 3, 4, 5];
    let squares = numbers.iter().map(|&x| (x).py_mul(x)).collect::<Vec<_>>();
    let evens = numbers
        .iter()
        .cloned()
        .filter(|&x| (x).py_mod(2) == 0)
        .collect::<Vec<_>>();
    let double = move |lst: Vec<i64>| {
        lst.iter()
            .cloned()
            .map(|x| (x).py_mul(2))
            .collect::<Vec<_>>()
    ...
    return (
        DepylerValue::from(squares),
        DepylerValue::from(evens),
        DepylerValue::from(doubled),
    );
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn test_lambda_in_expressions() -> (DepylerValue, DepylerValue) {
    let get_operation = move |is_add: bool| {
        if is_add {
            move |x: i64, y: i64| (x).py_add(y)
        } else {
            move |x: i64, y: i64| (x).py_sub(y)
        }
    };
    let add_op = get_operation(true);
    let sub_op = get_operation(false);
    return (
        DepylerValue::from(add_op(10, 5)),
        DepylerValue::from(sub_op(10, 5)),
    );
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn test_lambda_with_closure() {
    let multiplier = 3;
    let scale = move |x: i64| (x).py_mul(multiplier);
    let result = scale(7);
    let _ = result;
}
