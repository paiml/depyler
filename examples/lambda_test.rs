...
pub fn test_simple_lambdas() -> (DepylerValue, DepylerValue, DepylerValue) {
    let add = move |x, y| x.py_add(y);
    let square = move |x| x.py_mul(x);
...
pub fn test_lambda_with_list_operations() -> (Vec<DepylerValue>, Vec<DepylerValue>, DepylerValue) {
    ...
    let evens = numbers
        ...
        .collect::<Vec<_>>();
    let double = move |lst| lst.iter().cloned().map(|x| x.py_mul(2)).collect::<Vec<_>>();
...
pub fn test_lambda_with_closure() {
    let multiplier = 3;
    let scale = move |x| x.py_mul(multiplier);
    let result = scale(7);
    let _ = result;
}
