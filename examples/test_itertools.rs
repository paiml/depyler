...
pub fn group_consecutive(numbers: &Vec<i32>) -> Vec<Vec<i32>> {
    ...
    for (_k, g) in {
            ...
            .into_iter()
            .group_by(move |x: i32| {
                (x.clone().py_index(DepylerValue::Int(0 as i64)))
                    .py_sub(x.clone().py_index(DepylerValue::Int(1 as i64)))
            })
    } {
        groups.push(
            g.iter()
                .cloned()
                .map(|x| x.clone().py_index(DepylerValue::Int(1 as i64)))
                .collect::<Vec<_>>(),
        );
    }
    groups
