...
pub fn get_last(nums: &mut Vec<i32>) -> i32 {
    nums.pop().unwrap_or_default()
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    let mut data: Vec<i32> = vec![1, 2, 3, 4, 5];
    println!("{}", get_last(&mut data));
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn test_get_last_examples() {
        assert_eq!(get_last(&mut vec![]), 0);
        assert_eq!(get_last(&mut vec![1]), 1);
        assert_eq!(get_last(&mut vec![1, 2, 3]), 3);
    }
}