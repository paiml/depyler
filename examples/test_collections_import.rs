...
pub fn group_by_length(words: &Vec<String>) -> HashMap<i32, Vec<String>> {
    ...
    for word in words.iter().cloned() {
        groups
            .clone()
            .py_index(DepylerValue::Int(word.len() as i32 as i64))
            .push(word);
    }
    groups
        .into_iter()
...
pub fn sliding_window(data: &Vec<i32>, window_size: i32) -> Vec<Vec<i32>> {
    ...
    let mut window: Vec<DepylerValue> = VecDeque::from({
        let base = &*data;
    ...
    let mut windows: Vec<Vec<i32>> = vec![window.into_iter().collect::<Vec<_>>()];
    for item in {
        let base = &*data;
        ...
        window.push_back(DepylerValue::from(item));
