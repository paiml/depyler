...
pub fn equilibrium_index(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    ...
    while i < nums.len() as i32 {
        let right_sum: i32 = (((total) - (left_sum) as i32) - (
            nums.get(i as usize)
                .cloned()
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        equilibrium_index(&vec![1, 3, 5, 2, 2]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_equilibrium_index_examples() {
        assert_eq!(equilibrium_index(&vec![]), 0);
        assert_eq!(equilibrium_index(&vec![1]), 1);
        assert_eq!(equilibrium_index(&vec![1, 2, 3]), 3);
    }
}