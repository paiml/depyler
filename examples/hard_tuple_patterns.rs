...
#[doc = "Create and return a 3-element integer tuple."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn create_triple(x: i32, y: i32, z: i32) -> (i32, i32, i32) {
    (x, y, z)
}
#[doc = "Sum all elements of a 3-element tuple via indexing."]
#[doc = " Depyler: proven to terminate"]
pub fn access_triple(t: (i32, i32, i32)) -> Result<i32, Box<dyn std::error::Error>> {
    Ok({
        let _r: i32 = ((t.0).py_add(t.1) as i32).py_add(t.2);
        _r
    })
}
#[doc = "Create a mixed-type tuple of str, int, bool."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn mixed_type_tuple(name: &str, age: i32, active: bool) -> (String, i32, bool) {
    (name, age, active)
}
#[doc = "Extract and format fields from a mixed tuple."]
#[doc = " Depyler: proven to terminate"]
pub fn extract_mixed(record: &(String, i32, bool)) -> Result<String, Box<dyn std::error::Error>> {
    let name: String = record.0;
    let age: i32 = record.1;
    let active: bool = record.2;
    let status: String = if active { "active" } else { "inactive" };
    Ok(format!(
        "{}{}",
        format!(
            "{}{}",
            format!("{}{}", format!("{}{}", name, ":"), (age).to_string()),
            ":"
        ),
        status
    ))
}
#[doc = "Unpack a tuple in assignment and compute product."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn unpack_assignment(pair: (i32, i32)) -> i32 {
    let (a, b) = pair;
    (a).py_mul(b)
}
#[doc = "Unpack a 3-tuple and return weighted sum."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn unpack_triple(t: (i32, i32, i32)) -> i32 {
    let (x, y, z) = t;
    {
        let _r: i32 =
            (((x).py_mul(1i32) as i32).py_add((y).py_mul(10i32)) as i32).py_add((z).py_mul(100i32));
        _r
    }
}
#[doc = "Use enumerate to sum index*value products."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn enumerate_sum(items: &Vec<i32>) -> i32 {
    ...
    {
        let i = i as i32;
        total = ((total).py_add((i).py_mul(val))) as i32;
    }
    total
}
#[doc = "Compute dot product using zip to unpack pairs."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn zip_dot_product<'a, 'b>(xs: &'a Vec<i32>, ys: &'b Vec<i32>) -> i32 {
    let mut total: i32 = Default::default();
    total = 0;
    for (x, y) in xs.into_iter().zip(ys.into_iter()) {
        total = ((total).py_add((x).py_mul(y))) as i32;
    }
    total
}
#[doc = "Build a dictionary using tuples as keys."]
#[doc = " Depyler: proven to terminate"]
pub fn tuple_dict_keys<'b, 'a>(
    keys: &'a Vec<(i32, i32)>,
    values: &'b Vec<String>,
) -> Result<HashMap<(i32, i32), String>, Box<dyn std::error::Error>> {
    let mut result: std::collections::HashMap<(i32, i32), String> = {
...
#[doc = "Look up a value by tuple key in a dictionary."]
#[doc = " Depyler: proven to terminate"]
pub fn lookup_tuple_key(
    grid: &std::collections::HashMap<(i32, i32), String>,
    row: i32,
    col: i32,
) -> Result<String, Box<dyn std::error::Error>> {
    let key: (i32, i32) = (row, col);
    let _cse_temp_0 = grid.get(&key).is_some();
    if _cse_temp_0 {
        return Ok(grid
    ...
    Ok("not found".to_string().to_string())
}
#[doc = "Lexicographic tuple comparison returning -1, 0, or 1."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compare_tuples(a: (i32, i32), b: (i32, i32)) -> i32 {
    let _cse_temp_0 = a < b;
    if _cse_temp_0 {
        return -1;
    } else {
...
#[doc = "Named-tuple-like pattern: create a 2D point as a tuple."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_point(x: i32, y: i32) -> (i32, i32) {
    (x, y)
}
#[doc = "Compute Manhattan distance between two point-tuples."]
#[doc = " Depyler: proven to terminate"]
pub fn manhattan_distance(
    ...
    let _cse_temp_0 = ((p1.0) - (p2.0)) as i32;
    dx = _cse_temp_0;
    dy = _cse_temp_0;
    let _cse_temp_1 = dx < 0;
    if _cse_temp_1 {
        dx = -dx;
    }
    let _cse_temp_2 = dy < 0;
    if _cse_temp_2 {
        dy = -dy;
    }
    Ok((dx).py_add(dy))
}
#[doc = "Return quotient and remainder as a tuple."]
#[doc = " Depyler: proven to terminate"]
pub fn divmod_result(a: i32, b: i32) -> Result<(i32, i32), Box<dyn std::error::Error>> {
    let _cse_temp_0 = {
        let a = a;
        let b = b;
        let q = a / b;
    ...
    let quotient: i32 = _cse_temp_0;
    let _cse_temp_1 = ((a).py_mod(b)) as i32;
    let remainder: i32 = _cse_temp_1;
    Ok((quotient, remainder))
}
#[doc = "Return the min and max of a list as a tuple."]
pub fn min_max_list(nums: &Vec<i32>) -> Result<(i32, i32), Box<dyn std::error::Error>> {
...
#[doc = "Create a tuple containing a list and a bool."]
#[doc = " Depyler: verified panic-free"]
pub fn tuple_with_list(data: &Vec<i32>, flag: bool) -> (Vec<i32>, bool) {
...
#[doc = "Extract list from compound tuple and count elements."]
#[doc = " Depyler: proven to terminate"]
pub fn count_positive_from_compound(
    compound: &(Vec<i32>, bool),
) -> Result<i32, Box<dyn std::error::Error>> {
    let items: Vec<i32> = compound.0;
    Ok(items.len() as i32 as i32)
}
#[doc = "Access elements from a tuple of tuples and sum all."]
#[doc = " Depyler: proven to terminate"]
pub fn nested_tuple_access(
...
#[doc = "Build a nested tuple(pair of pairs)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn build_nested_tuple(a: i32, b: i32, c: i32, d: i32) -> ((i32, i32), (i32, i32)) {
    ((a, b), (c, d))
}
#[doc = "Slice first two elements from a 4-tuple."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn tuple_slice_first_two(t: (i32, i32, i32, i32)) -> (i32, i32) {
    (t.0, t.1)
}
#[doc = "Slice last two elements from a 4-tuple."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn tuple_slice_last_two(t: (i32, i32, i32, i32)) -> (i32, i32) {
    (t.2, t.3)
}
#[doc = "Sort a list of tuples by their second element(selection sort)."]
#[doc = " Depyler: proven to terminate"]
pub fn sort_tuples_by_second(
    pairs: &Vec<(i32, i32)>,
) -> Result<Vec<(i32, i32)>, Box<dyn std::error::Error>> {
    let mut result: Vec<(i32, i32)> = pairs.into_iter().collect::<Vec<_>>();
    ...
    for i in 0..(n) {
        ...
        if min_idx != i {
            let temp: (i32, i32) = result
                .get(i as usize)
                .cloned()
                .expect("IndexError: list index out of range");
            result[(i) as usize] = result
                .get(min_idx as usize)
                .cloned()
                .expect("IndexError: list index out of range");
            result[(min_idx) as usize] = temp;
        }
    }
    Ok(result)
}
#[doc = "Add tuples to a set and return the count of unique tuples."]
#[doc = " Depyler: verified panic-free"]
pub fn tuple_set_membership(elements: &Vec<(i32, i32)>) -> i32 {
    let mut seen: std::collections::HashSet<(i32, i32)> = std::collections::HashSet::<i32>::new();
    for t in elements.iter().cloned() {
        seen.insert(t);
    }
    seen.len() as i32 as i32
}
#[doc = "Return the lexicographically smaller tuple."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn min_tuple(a: (i32, i32), b: (i32, i32)) -> (i32, i32) {
    let _cse_temp_0 = a <= b;
    if _cse_temp_0 {
        return a;
    }
...
#[doc = "Return the lexicographically larger tuple."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn max_tuple(a: (i32, i32), b: (i32, i32)) -> (i32, i32) {
    let _cse_temp_0 = a >= b;
    if _cse_temp_0 {
        return a;
    }
...
#[doc = "Find the lexicographic min and max from a list of tuples."]
pub fn find_min_max_tuples(
    pairs: &Vec<(i32, i32)>,
) -> Result<((i32, i32), (i32, i32)), Box<dyn std::error::Error>> {
    let mut lo: (i32, i32) = Default::default();
    let mut hi: (i32, i32) = Default::default();
    lo = pairs
        .get(0usize)
        .cloned()
        .expect("IndexError: list index out of range");
...
#[doc = "Comprehensive test exercising all tuple patterns."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn test_all() -> Result<bool, Box<dyn std::error::Error>> {
    let triple: (i32, i32, i32) = create_triple(1, 2, 3);
    assert_eq!(access_triple(triple), 6);
    let record: (String, i32, bool) = mixed_type_tuple(&"Alice", 30, true);
    assert_eq!(extract_mixed(record), "Alice:30:active");
    let record2: (String, i32, bool) = mixed_type_tuple(&"Bob", 25, false);
    assert_eq!(extract_mixed(record2), "Bob:25:inactive");
    assert_eq!(unpack_assignment((3, 7)), 21);
    assert_eq!(unpack_triple((5, 3, 2)), 235);
    assert_eq!(
        enumerate_sum(&vec![10, 20, 30]),
        (((0i32).py_mul(10i32) as i32).py_add((1i32).py_mul(20i32)) as i32)
            .py_add((2i32).py_mul(30i32))
    );
    assert_eq!(zip_dot_product(&vec![1, 2, 3], &vec![4, 5, 6]), 32);
    let keys: Vec<(i32, i32)> = vec![(0, 0), (0, 1), (1, 0)];
    let vals: Vec<String> = vec![
        "origin".to_string(),
        "right".to_string(),
        "down".to_string(),
    ];
    let grid: std::collections::HashMap<(i32, i32), String> = tuple_dict_keys(&keys, &vals)?;
    assert_eq!(lookup_tuple_key(&grid, 0, 0), "origin");
    assert_eq!(lookup_tuple_key(&grid, 0, 1), "right");
    assert_eq!(lookup_tuple_key(&grid, 5, 5), "not found".to_string());
    assert_eq!(compare_tuples((1, 2), (1, 3)), -1);
    assert_eq!(compare_tuples((2, 0), (1, 9)), 1);
    assert_eq!(compare_tuples((3, 3), (3, 3)), 0);
    let p1: (i32, i32) = make_point(0, 0);
    let p2: (i32, i32) = make_point(3, 4);
    assert_eq!(manhattan_distance(p1, p2), 7);
    let (q, r) = divmod_result(17, 5)?;
    assert_eq!(q, 3);
    assert_eq!(r, 2);
    let (lo, hi) = min_max_list(&vec![3, 1, 4, 1, 5, 9, 2, 6])?;
    assert_eq!(lo, 1);
    assert_eq!(hi, 9);
    let compound: (Vec<i32>, bool) = tuple_with_list(&vec![-1, 2, -3, 4, 5], true);
    assert_eq!(count_positive_from_compound(compound.clone()), 3);
    assert_eq!(compound.1, true);
    let nested: ((i32, i32), (i32, i32)) = build_nested_tuple(1, 2, 3, 4);
    assert_eq!(nested_tuple_access(nested), 10);
    let four: (i32, i32, i32, i32) = (10, 20, 30, 40);
    let first_two: (i32, i32) = tuple_slice_first_two(four.clone());
    let last_two: (i32, i32) = tuple_slice_last_two(four);
    assert_eq!(first_two, (10, 20));
    assert_eq!(last_two, (30, 40));
    assert_eq!(access_triple((100, 200, 300)), 600);
    let unsorted: Vec<(i32, i32)> = vec![(3, 1), (1, 3), (2, 2)];
    let sorted_pairs: Vec<(i32, i32)> = sort_tuples_by_second(&unsorted)?;
    assert_eq!(
        sorted_pairs
            .get(0usize)
            .cloned()
    ...
    let with_dups: Vec<(i32, i32)> = vec![(1, 2), (3, 4), (1, 2), (5, 6), (3, 4)];
    assert_eq!(tuple_set_membership(&with_dups), 3);
    assert_eq!(min_tuple((1, 5), (1, 3)), (1, 3));
    assert_eq!(max_tuple((1, 5), (1, 3)), (1, 5));
    assert_eq!(min_tuple((2, 0), (1, 9)), (1, 9));
    assert_eq!(max_tuple((2, 0), (1, 9)), (2, 0));
    let (lo_t, hi_t) = find_min_max_tuples(&vec![(3, 1), (1, 5), (2, 2), (1, 3)])?;
    assert_eq!(lo_t, (1, 3));
    assert_eq!(hi_t, (3, 1));
    Ok(true)
}
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if result {
        println!("{}", "All tuple pattern tests passed!");
    }
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_enumerate_sum_examples() {
        assert_eq!(enumerate_sum(&vec![]), 0);
        assert_eq!(enumerate_sum(&vec![1]), 1);
        assert_eq!(enumerate_sum(&vec![1, 2, 3]), 6);
    }
    #[test]
    fn quickcheck_sort_tuples_by_second() {
        fn prop(pairs: Vec<()>) -> TestResult {
            let input_len = pairs.len();
            let result = sort_tuples_by_second(&pairs);
            ...
            let mut input_sorted = pairs.clone();
            input_sorted.sort();
            let mut result = sort_tuples_by_second(&pairs);
            result.sort();
    ...
    #[test]
    fn test_sort_tuples_by_second_examples() {
        assert_eq!(sort_tuples_by_second(vec![]), vec![]);
        assert_eq!(sort_tuples_by_second(vec![1]), vec![1]);
    }
    #[test]
    fn test_tuple_set_membership_examples() {
        assert_eq!(tuple_set_membership(&vec![]), 0);
        assert_eq!(tuple_set_membership(&vec![1]), 1);
        assert_eq!(tuple_set_membership(&vec![1, 2, 3]), 3);
    }
}