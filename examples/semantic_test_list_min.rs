...
pub fn find_min(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        find_min(&vec![3, 1, 4, 1, 5, 9, 2, 6]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_find_min_examples() {
        assert_eq!(find_min(&vec![]), 0);
        assert_eq!(find_min(&vec![1]), 1);
        assert_eq!(find_min(&vec![1, 2, 3]), 3);
    }
}