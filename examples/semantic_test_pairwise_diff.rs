...
pub fn pairwise_diff(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        pairwise_diff(&vec![1, 4, 2, 8, 3]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_pairwise_diff_examples() {
        assert_eq!(pairwise_diff(&vec![]), 0);
        assert_eq!(pairwise_diff(&vec![1]), 1);
        assert_eq!(pairwise_diff(&vec![1, 2, 3]), 3);
    }
}