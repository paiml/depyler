...
pub fn count_even_odd(nums: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    let mut odd: i32 = Default::default();
    let mut even: i32 = Default::default();
    even = 0;
    odd = 0;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!(
        "{}",
        count_even_odd(&vec![1, 2, 3, 4, 5, 6]).expect("operation failed")
    );
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_count_even_odd_examples() {
        assert_eq!(count_even_odd(&vec![]), 0);
        assert_eq!(count_even_odd(&vec![1]), 1);
        assert_eq!(count_even_odd(&vec![1, 2, 3]), 3);
    }
}