...
pub fn test_xml_iterate_children() {
    let xml_string =
        "\n    <root>\n        <a>1</a>\n        <b>2</b>\n        <c>3</c>\n    </root>\n    ";
    let root = ET::fromstring(xml_string);
    let mut tags: Vec<DepylerValue> = vec![];
    let mut texts: Vec<DepylerValue> = vec![];
    for child in root.iter().cloned() {
        tags.push(DepylerValue::Str(format!("{:?}", child.tag())));
        texts.push(DepylerValue::Str(format!("{:?}", child.text())));
    }
    assert_eq!(
        tags,
