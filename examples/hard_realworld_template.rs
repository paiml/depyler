...
} #[doc = "Find next {{ in template from start position. Returns -1 if not found."] pub fn tpl_find_open(tpl: & str, start: i32) -> Result<i32, Box<dyn std::error::Error>>{
    let mut idx: i32 = start.clone();
    while idx <(tpl.len() as i32) - (1i32) {
    if({ let base = & tpl;
    let idx: i32 = idx;
...
#[doc = "Find next }} in template from start position. Returns -1 if not found."] pub fn tpl_find_close(tpl: & str, start: i32) -> Result<i32, Box<dyn std::error::Error>>{
    let mut idx: i32 = start.clone();
    while idx <(tpl.len() as i32) - (1i32) {
    if({ let base = & tpl;
    let idx: i32 = idx;
...
#[doc = "Extract substring [start, end)."] pub fn tpl_extract(text: & str, start: i32, end: i32) -> Result<String, Box<dyn std::error::Error>>{
...
#[doc = "Trim whitespace from variable name."] pub fn tpl_trim(raw_name: & str) -> Result<String, Box<dyn std::error::Error>>{
    let mut start: i32 = Default::default();
    let mut end: i32 = Default::default();
    start = 0;
    while(start<raw_name.len() as i32) &&({ let base = & raw_name;
    let idx: i32 = start;
    let actual_idx = if idx<0 {
    base.chars().count().saturating_sub(idx.abs() as usize)
}
...
let _cse_temp_0 = raw_name.len() as i32;
    end = _cse_temp_0;
    while(end>start) &&({ let base = & raw_name;
...
tpl_extract(raw_name, start, end)
}
#[doc = "Look up variable value by name. Returns empty string if not found."] pub fn tpl_lookup<'c, 'a, 'b>(var_names: & 'a Vec<String>, var_values: & 'b Vec<String>, name: & 'c str) -> Result<String, Box<dyn std::error::Error>>{
    let mut idx: i32 = 0;
    while idx<var_names.len() as i32 {
    if var_names.get(idx as usize).cloned().expect("IndexError: list index out of range") == (*name) {
    return Ok(var_values.get(idx as usize).cloned().expect("IndexError: list index out of range"));
   
}
idx  = ((idx).py_add(1i32)) as i32;
   
}
Ok(STR_EMPTY.to_string())
}
#[doc = "Render template by replacing {{var}} with values."] #[doc = " Depyler: verified panic-free"] pub fn tpl_render<'c, 'a, 'b>(tpl: & 'a str, var_names: & 'b Vec<String>, var_values: & 'c Vec<String>) -> Result<String, Box<dyn std::error::Error>>{
    let mut result: String = Default::default();
    result = STR_EMPTY.to_string().to_string();
    let mut pos: i32 = 0;
    while pos<tpl.len() as i32 {
    let open_pos: i32 = tpl_find_open(tpl, pos) ?;
    if open_pos == - 1 {
    let rest: String = tpl_extract(tpl, pos, tpl.len() as i32) ?;
    result = format!("{}{}", result, rest);
    pos = tpl.len() as i32 as i32;
   
}
else {
    let before: String = tpl_extract(tpl, pos, open_pos) ?;
    result = format!("{}{}", result, before);
    let close_pos: i32 = tpl_find_close(tpl ,(open_pos).py_add(2i32)) ?;
    if close_pos == - 1 {
    let remaining: String = tpl_extract(tpl, open_pos, tpl.len() as i32) ?;
    result = format!("{}{}", result, remaining);
    pos = tpl.len() as i32 as i32;
   
}
else {
    let raw_name: String = tpl_extract(tpl ,(open_pos).py_add(2i32), close_pos) ?;
    let var_name: String = tpl_trim(& raw_name) ?;
    let replacement: String = tpl_lookup(& var_names, & var_values, & var_name) ?;
    result = format!("{}{}", result, replacement);
    pos  = ((close_pos).py_add(2i32)) as i32;
...
#[doc = "Count number of {{...}} placeholders."] #[doc = " Depyler: verified panic-free"] pub fn tpl_count_placeholders(tpl: & str) -> Result<i32, Box<dyn std::error::Error>>{
...
#[doc = "Extract all variable names from template placeholders."] #[doc = " Depyler: verified panic-free"] pub fn tpl_extract_names(tpl: & str) -> Result<Vec<String>, Box<dyn std::error::Error>>{
    ...
    let raw_name: String = tpl_extract(tpl ,(open_pos).py_add(2i32), close_pos) ?;
    let trimmed: String = tpl_trim(& raw_name) ?;
...
#[doc = "Check if template has any placeholders. Returns 1 if yes."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn tpl_has_placeholders(tpl: & str) -> Result<i32, Box<dyn std::error::Error>>{
    let _cse_temp_0 = tpl_find_open(tpl, 0) ? != - 1;
    if _cse_temp_0 {
    return Ok(1);
...
#[doc = "Test template engine module."] #[doc = " Depyler: proven to terminate"] pub fn test_module() -> Result<i32, Box<dyn std::error::Error>>{
    let mut passed: i32 = Default::default();
    passed = 0;
    let _cse_temp_0 = tpl_find_open(& "hello {{world}}", 0) ? == 6;
    if _cse_temp_0 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let _cse_temp_1 = tpl_find_close(& "hello {{world}}", 8) ? == 13;
    if _cse_temp_1 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let tpl: String = "Hello, {{name}}!".to_string();
    let result: String = tpl_render(& tpl, & vec! ["name".to_string()], & vec! ["Alice".to_string()]) ?;
    let _cse_temp_2 = result == "Hello, Alice!";
    if _cse_temp_2 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let tpl2: String = "{{greeting}}, {{ name }}! You are {{ age }}.".to_string();
    let names: Vec<String>= vec! ["greeting".to_string(), "name".to_string(), "age".to_string()];
    let vals: Vec<String>= vec! ["Hi".to_string(), "Bob".to_string(), "30".to_string()];
    let result2: String = tpl_render(& tpl2, & names, & vals) ?;
    let _cse_temp_3 = result2 == "Hi, Bob! You are 30.";
    if _cse_temp_3 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let _cse_temp_4 = tpl_count_placeholders(& tpl2) ? == 3;
    if _cse_temp_4 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let extracted: Vec<String>= tpl_extract_names(& tpl2) ?;
    let _cse_temp_5 = extracted.len() as i32;
    let _cse_temp_6 = _cse_temp_5 == 3;
    let _cse_temp_7 = extracted.get(0usize).cloned().expect("IndexError: list index out of range") == "greeting";
    let _cse_temp_8  = (_cse_temp_6) &&(_cse_temp_7);
    if _cse_temp_8 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let result3: String = tpl_render(& "{{missing}}", & vec! [], & vec! []) ?;
    let _cse_temp_9 = result3 == STR_EMPTY;
    if _cse_temp_9 {
    passed  = ((passed).py_add(1i32)) as i32;
   
}
let result4: String = tpl_render(& "plain text", & vec! [], & vec! []) ?;
    let _cse_temp_10 = result4 == "plain text";
    if _cse_temp_10 {
    passed  = ((passed).py_add(1i32)) as i32;
...
#[cfg(test)] mod tests {
    ...
    #[test] fn test_tpl_count_placeholders_examples() {
    assert_eq!(tpl_count_placeholders(""), 0);
    assert_eq!(tpl_count_placeholders("a"), 1);
    assert_eq!(tpl_count_placeholders("abc"), 3);
   
}
#[test] fn test_tpl_has_placeholders_examples() {
    assert_eq!(tpl_has_placeholders(""), 0);
    assert_eq!(tpl_has_placeholders("a"), 1);
    assert_eq!(tpl_has_placeholders("abc"), 3);
   
}
#[test] fn test_test_module_examples() {
    let _ = test_module();
