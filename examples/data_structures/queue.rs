...
impl Queue {
    ...
    pub fn dequeue(&mut self) -> Option<i32> {
        ...
        let item = self._items.clone()[self._front.clone() as usize];
        self._front = self._front.clone() + 1;
        if self._front.clone() > {
            let a = self._items.clone().len() as i32;
            let b = 2;
            let q = a / b;
            let r = a % b;
            let r_negative = r < 0;
            ...
            self._items = {
                let s = &self._items.clone();
                let len = s.chars().count() as isize;
                let start_idx = (self._front.clone()) as isize;
    ...
    pub fn front(&self) -> Option<i32> {
        ...
        return Some(self._items.clone()[self._front.clone() as usize]);
    }
    pub fn is_empty(&self) -> bool {
        return self._front.clone() >= (self._items.clone().len() as i32);
    }
    pub fn size(&self) -> i32 {
        return (self._items.clone().len() as i32).saturating_sub(self._front.clone());
    }
}
