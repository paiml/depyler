...
pub fn divide(a: i32, b: i32) -> Result<i32, Box<dyn std::error::Error>> {
    Ok({
        let a = a;
        let b = b;
        let q: i32 = a / b;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("{}", divide(17, 5).expect("operation failed"));
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_divide_examples() {
        assert_eq!(divide(0, 0), 0);
        assert_eq!(divide(1, 2), 3);
        assert_eq!(divide(-1, 1), 0);
    }
}