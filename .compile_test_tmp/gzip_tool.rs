...
enum Commands {
    #[default]
    __DepylerNone,
    Ratio { original: i32, compressed: i32 },
    Savings { original: i32, compressed: i32 },
}
#[derive(Default)]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Ratio { .. }));
    match &args.command {
        Some(Commands::Ratio {
            ref compressed,
            ref original,
        ...
        Some(Commands::Savings {
            ...
            let compressed = *compressed;
            let original = *original;
            println!("{}", (original - compressed) * 100 / original);
        }
