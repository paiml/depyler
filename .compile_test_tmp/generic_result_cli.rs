...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'abc'(tracked in DEPYLER-0424)"]
...
pub enum PyResult<T: Clone, E: Clone> {
    PyOk(T),
    PyErr(E),
}
impl<T: Clone, E: Clone> PyResult<T, E> {
    pub fn is_pyok(&self) -> bool {
        matches!(self, Self::PyOk(..))
    }
    pub fn is_pyerr(&self) -> bool {
        matches!(self, Self::PyErr(..))
    }
    pub fn new_pyok(value: T) -> Self {
        Self::PyOk(value)
    }
    pub fn new_pyerr(error: E) -> Self {
        Self::PyErr(error)
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct Either<T: Clone, U: Clone> {
    pub _left: Option<T>,
    pub _right: Option<U>,
    pub _is_left: bool,
}
impl<T: Clone, U: Clone> Either<T, U> {
    pub fn new(_left: Option<T>, _right: Option<U>, _is_left: bool) -> Self {
        Self {
            _left,
            _right,
            _is_left,
    ...
    pub fn left(value: T) -> Either<T, DepylerValue> {
        return Either::new();
    }
    pub fn right(value: U) -> Either<DepylerValue, U> {
        return Either::new();
    }
    pub fn is_left(&self) -> bool {
        return self._is_left.clone();
    }
    pub fn is_right(&self) -> bool {
        return !self._is_left.clone();
    }
    pub fn left_value(&self) -> T {
        if !self._is_left.clone() {
            panic!(
                "Exception: {}",
                ValueError::new("Called left_value on Right".to_string())
            );
        };
        return self._left.clone();
    }
    pub fn right_value(&self) -> U {
        if self._is_left.clone() {
            panic!(
                "Exception: {}",
                ValueError::new("Called right_value on Left".to_string())
            );
        };
        return self._right.clone();
    }
    pub fn map_left(&self, f: impl Fn(T) -> DepylerValue) -> Either<DepylerValue, U> {
        if self._is_left.clone() {
            return Either::left(f(self._left.clone()));
        };
        return self;
    }
    pub fn map_right(&self, f: impl Fn(U) -> DepylerValue) -> Either<T, DepylerValue> {
        if !self._is_left.clone() {
            return Either::right(f(self._right.clone()));
        };
        return self;
    }
    pub fn fold(
        &self,
        left_f: impl Fn(T) -> DepylerValue,
        right_f: impl Fn(U) -> DepylerValue,
    ) -> DepylerValue {
        if self._is_left.clone() {
            return left_f(self._left.clone());
        };
        return right_f(self._right.clone());
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct Validated<T: Clone, E: Clone> {
    pub _value: Option<T>,
    pub _errors: Option<Vec<E>>,
    pub _is_valid: bool,
}
impl<T: Clone, E: Clone> Validated<T, E> {
    pub fn new(_value: Option<T>, _errors: Option<Vec<E>>, _is_valid: bool) -> Self {
        Self {
            _value,
            _errors,
            _is_valid,
    ...
    pub fn valid(value: T) -> Validated<T, DepylerValue> {
        return Validated::new();
    }
    pub fn invalid(errors: Vec<E>) -> Validated<DepylerValue, E> {
        return Validated::new();
    }
    pub fn error(error: E) -> Validated<DepylerValue, E> {
        return Validated::new();
    }
    pub fn is_valid(&self) -> bool {
        return self._is_valid.clone();
    }
    pub fn value(&self) -> T {
        if !self._is_valid.clone() {
            panic!(
                "Exception: {}",
                ValueError::new("Called value on invalid".to_string())
            );
        };
    ...
    pub fn errors(&self) -> Vec<E> {
        ...
        return self._errors.clone() || vec![];
    }
}
#[doc = r" Stub for local import from module: #module_name"]
#[doc = r" DEPYLER-0615: Generated to allow standalone compilation"]
#[allow(dead_code, unused_variables)]
pub fn ABC<T: Default>(_args: impl std::any::Any) -> T {
    Default::default()
}
#[doc = r" Stub for local import from module: #module_name"]
#[doc = r" DEPYLER-0615: Generated to allow standalone compilation"]
#[allow(dead_code, unused_variables)]
pub fn abstractmethod<T: Default>(_args: impl std::any::Any) -> T {
    Default::default()
}
#[doc = r" Stub for local import from module: #module_name"]
#[doc = r" DEPYLER-0615: Generated to allow standalone compilation"]
...
#[doc = "Convert exception-throwing code to Result."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn try_catch<T: Clone>(f: impl Fn() -> T) -> Result<T, String> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(PyOk::new(f()));
    })() {
        Ok(_result) => {
            return _result;
        }
        Err(e) => {
            return PyErr::new((e).to_string());
        }
    }
}
#[doc = "Safely divide two numbers."]
#[doc = " Depyler: proven to terminate"]
pub fn safe_divide(a: f64, b: f64) -> Result<Result<f64, String>, Box<dyn std::error::Error>> {
...
#[doc = "Safely parse string to int."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn safe_parse_int(s: &str) -> Result<i32, String> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(PyOk::new(s.parse::<i32>().unwrap_or_default()));
    })() {
        Ok(_result) => {
            return _result;
        }
        Err(_) => {
            return PyErr::new(format!("invalid integer: {}", s));
        }
    }
}
#[doc = "Safely parse string to float."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn safe_parse_float(s: &str) -> Result<f64, String> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(PyOk::new(s.parse::<f64>().unwrap()));
    })() {
        Ok(_result) => {
            return _result;
        }
        Err(_) => {
            return PyErr::new(format!("invalid float: {}", s));
        }
    }
}
#[doc = "Collect list of Results into Result of list."]
#[doc = " Depyler: verified panic-free"]
pub fn collect_results<E: Clone, T: Clone>(results: &Vec<Result<T, E>>) -> Result<Vec<T>, E> {
    let mut values: Vec<T> = vec![];
    for result in results.iter().cloned() {
        if result.is_err() {
            return PyErr::new(result.unwrap_err());
        }
        values.push(result.unwrap());
    }
    PyOk::new(values)
}
#[doc = "Partition Results into successes and failures."]
#[doc = " Depyler: verified panic-free"]
pub fn partition_results<T: Clone, E: Clone>(results: &Vec<Result<T, E>>) -> (Vec<T>, Vec<E>) {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn combine_results<'b, 'a, T: Clone, U: Clone, E: Clone>(
    r1: &'a Result<T, E>,
    r2: &'b Result<U, E>,
    f: impl Fn(T, U) -> DepylerValue,
) -> Result<DepylerValue, E> {
    if r1.is_err() {
        return PyErr::new(r1.unwrap_err());
    }
    if r2.is_err() {
        return PyErr::new(r2.unwrap_err());
    }
    PyOk::new(f(r1.unwrap(), r2.unwrap()))
}
#[doc = "Combine two validated values, accumulating errors."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn combine_validated<'a, 'b, E: Clone, U: Clone, T: Clone>(
    v1: &'a Validated<T, E>,
    v2: &'b Validated<U, E>,
    f: impl Fn(T, U) -> DepylerValue,
) -> Validated<DepylerValue, E> {
    let _cse_temp_0 = (v1.is_valid()) && (v2.is_valid());
    if _cse_temp_0 {
        return Validated::valid(f(v1.value(), v2.value()));
    }
    let mut all_errors: Vec<E> = vec![];
    if !v1.is_valid() {
        all_errors.extend(v1.errors().iter().cloned());
    }
    if !v2.is_valid() {
        all_errors.extend(v2.errors().iter().cloned());
    }
    Validated::invalid(all_errors)
}
#[doc = "Validate number is positive."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn validate_positive(n: i32) -> Validated<i32, String> {
    let _cse_temp_0 = n > 0;
    if _cse_temp_0 {
        return Validated::valid(n);
    }
    Validated::error(format!("must be positive, got {}", n))
}
#[doc = "Validate string is non-empty."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn validate_non_empty(s: &str) -> Validated<String, String> {
    if !s.is_empty() {
        return Validated::valid(s);
    }
    Validated::error("must be non-empty".to_string())
}
#[doc = "Simulate Result operations."]
pub fn simulate_result(
    operations: &Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results_list: Vec<String> = vec![];
    let mut current: Result<i32, String> = PyOk::new(0);
        ...
        if cmd == "ok" {
            current = PyOk::new(
                (parts
                    .get(1usize)
                    .cloned()
                    .expect("IndexError: list index out of range")) as i32,
            );
            results_list.push("ok".to_string());
        } else {
            if cmd == "err" {
                current = PyErr::new(
                    parts
                ...
                results_list.push("err".to_string());
            } else {
                if cmd == "is_ok" {
                    results_list.push(if current.is_ok() {
                        "1".to_string()
                    } else {
                        "0".to_string()
                    });
                    ...
                    if cmd == "is_err" {
                        results_list.push(if current.is_err() {
                            "1".to_string()
                        } else {
                            "0".to_string()
                        });
                        ...
                        if cmd == "unwrap" {
                            results_list.push((current.unwrap_or(-1)).to_string());
                        } else {
                            if cmd == "map" {
                                current = current.map(move |x| {
                                    x * (parts
                                        .get(1usize)
                                        .cloned()
                                        .expect("IndexError: list index out of range"))
                                ...
                                if cmd == "divide" {
                                    ...
                                    current = safe_divide(a, b).map(|x| x as i32);
                                    if current.is_ok() {
                                        results_list.push((current.unwrap()).to_string());
                                    } else {
                                        results_list
                                            .push(format!("error:{}", current.unwrap_err()));
                                    }
                                } else {
                                    if cmd == "parse" {
                                        current = safe_parse_int(
                                            parts
                                                .get(1usize)
                                                .cloned()
                                                .expect("IndexError: list index out of range"),
                                        ...
                                        if current.is_ok() {
                                            ...
                                            results_list.push("error".to_string().to_string());
                                        }
                                    }
                                }
                            }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: generic_result_cli.py <command>[args...]");
        println!("{}", "Commands: divide, parse, validate");
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "divide";
    let mut result;
    if _cse_temp_2 {
        let _cse_temp_3 = _cse_temp_0 < 4;
        if _cse_temp_3 {
            eprintln!("{}", "Usage: divide <a><b>");
            std::process::exit(1i32)
        }
        result = safe_divide(
            std::env::args()
        ...
        if result.is_ok() {
            println!("{}", format!("Ok({})", result.unwrap()));
        } else {
            println!("{}", format!("Err({})", result.unwrap_err()));
        }
    } else {
        let _cse_temp_4 = cmd == "parse";
        if _cse_temp_4 {
            let _cse_temp_5 = _cse_temp_0 < 3;
            if _cse_temp_5 {
                eprintln!("{}", "Usage: parse <value>");
                std::process::exit(1i32)
            }
            ...
            let _cse_temp_6 = cmd == "validate";
            if _cse_temp_6 {
                let _cse_temp_7 = _cse_temp_0 < 3;
                if _cse_temp_7 {
                    eprintln!("{}", "Usage: validate <number>");
                    std::process::exit(1i32)
                }
                let parsed = safe_parse_int(
                    std::env::args()
                        .collect::<Vec<String>>()
                        .get(2usize)
                        .cloned()
                ...
                if parsed.is_err() {
                    println!("{}", format!("Err({})", parsed.unwrap_err()));
                    std::process::exit(1i32)
                }
                let validated = validate_positive(parsed.unwrap());
                if validated.is_valid() {
                    println!("{}", format!("Valid({})", validated.value()));
                } else {
                    println!("{}", format!("Invalid({})", validated.errors()));
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_simulate_result_examples() {
        assert_eq!(simulate_result(vec![]), vec![]);
        assert_eq!(simulate_result(vec![1]), vec![1]);
    }
}
