...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Hex { .. }));
    match &args.command {
        Some(Commands::Hex { ref num, .. }) => {
            let num = *num;
            println!("{}", format!("{:x}", num));
        }
        Some(Commands::Oct { ref num, .. }) => {
            let num = *num;
            println!("{}", format!("{:o}", num));
        }
        Some(Commands::Dec { ref hexval, .. }) => {
            let hexval = hexval.to_string();
