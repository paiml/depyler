...
pub fn cmd_create(content: &str, suffix: &str) {
    let _cse_temp_0 = if suffix.is_empty() {
        ".tmp".to_string()
    } else {
        suffix.to_string()
    };
    let suffix = _cse_temp_0;
...
#[doc = "Create temp directory. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_mkdir() {
    let d = tempfile::tempdir();
    println!("{}", format!("Directory: {:?}", d));
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
