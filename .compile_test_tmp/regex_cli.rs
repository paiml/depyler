...
pub struct NodeType {}
impl NodeType {
    pub const LITERAL: i32 = auto();
    pub const DOT: i32 = auto();
    pub const STAR: i32 = auto();
    pub const PLUS: i32 = auto();
    pub const QUESTION: i32 = auto();
    pub const CONCAT: i32 = auto();
    pub const ALTERNATION: i32 = auto();
    pub const GROUP: i32 = auto();
    pub const CHAR_CLASS: i32 = auto();
    pub const ANCHOR_START: i32 = auto();
    pub const ANCHOR_END: i32 = auto();
    pub fn new() -> Self {
        Self {}
    }
}
...
impl Lexer {
    ...
    pub fn peek(&self) -> String {
        if self.pos.clone() >= (self.pattern.clone().len() as i32) {
            return "".to_string();
        };
        return {
            let _base = &self.pattern.clone();
...
#[derive(Debug, Clone)]
pub struct Parser {
    pub lexer: DepylerValue,
}
impl Parser {
    pub fn new(_pattern: String) -> Self {
        Self {
    ...
    pub fn alternation(&self) -> Node {
        let left = self.concatenation();
        if self.lexer.clone().peek() == "|".to_string() {
            self.lexer.clone().advance();
            let right = self.alternation();
            return Node::new(NodeType::ALTERNATION);
        };
        return left;
    }
    pub fn concatenation(&self) -> Node {
        let mut nodes = vec![];
        while self.lexer.clone().peek() && !"|)".to_string().contains(&*self.lexer.clone().peek()) {
            nodes.push(self.quantifier());
        }
        if !nodes {
            return Node::new(NodeType::LITERAL);
        };
        if (nodes.len() as i32) == 1 {
            return {
                let _base = &nodes;
                let _idx = 0;
                if _idx < 0 {
        ...
        for node in {
            ...
            let result = Node::new(NodeType::CONCAT);
        }
        return result;
    }
    pub fn quantifier(&self) -> Node {
        let node = self.atom();
        if self.lexer.clone().peek() == "*".to_string() {
            self.lexer.clone().advance();
            return Node::new(NodeType::STAR);
        };
        if self.lexer.clone().peek() == "+".to_string() {
            self.lexer.clone().advance();
            return Node::new(NodeType::PLUS);
        };
        if self.lexer.clone().peek() == "?".to_string() {
            self.lexer.clone().advance();
            return Node::new(NodeType::QUESTION);
        };
        return node;
    }
    pub fn atom(&self) -> Node {
        let char = self.lexer.clone().peek();
        if char == "(".to_string() {
            self.lexer.clone().advance();
            ...
            return Node::new(NodeType::GROUP);
        };
        if char == "[".to_string() {
        ...
        if char == "^".to_string() {
            self.lexer.clone().advance();
            return Node::new(NodeType::ANCHOR_START);
        };
        if char == "$".to_string() {
            self.lexer.clone().advance();
            return Node::new(NodeType::ANCHOR_END);
        };
        if char == "\\".to_string() {
            self.lexer.clone().advance();
            let escaped = self.lexer.clone().advance();
            return Node::new(NodeType::LITERAL);
        };
        if char {
            self.lexer.clone().advance();
    ...
    pub fn char_class(&self) -> Node {
        ...
        while self.lexer.clone().peek() && self.lexer.clone().peek() != "]".to_string() {
            let char = self.lexer.clone().advance();
            if self.lexer.clone().peek() == "-".to_string()
                && self.lexer.clone().pos + 1 < (self.lexer.clone().pattern.len() as i32)
            {
                self.lexer.clone().advance();
                let end_char = self.lexer.clone().advance();
                for c in char.chars().next().unwrap() as i32
        ...
        return Node::new(NodeType::CHAR_CLASS);
    }
}
#[derive(Default)]
...
#[doc = "Match node against text starting at pos. Returns list of end positions."]
#[doc = " Depyler: proven to terminate"]
pub fn match_node<'a, 'b>(
    node: &'a Node,
    text: &'b str,
    pos: i32,
    ...
    let mut child: i32 = Default::default();
    let _cse_temp_0 = node.r#type == NodeType::LITERAL;
    if _cse_temp_0 {
        let _cse_temp_1 = text.len() as i32;
        let _cse_temp_2 = pos < _cse_temp_1;
        let _cse_temp_3 = {
            let base = text;
            let start_idx: i32 = pos;
            let stop_idx: i32 = pos
    ...
    if _cse_temp_0 {
        let _cse_temp_5 = text.len() as i32;
        let _cse_temp_6 = pos < _cse_temp_5;
        if _cse_temp_6 {
            return Ok(vec![pos + 1]);
        }
        return Ok(vec![]);
    ...
    if _cse_temp_0 {
        let _cse_temp_7 = text.len() as i32;
        let _cse_temp_8 = pos < _cse_temp_7;
        if _cse_temp_8 {
            let _cse_temp_9 = if node.value.is_empty() {
                "".to_string()
            } else {
                node.value.to_string()
            };
            let _cse_temp_10 = _cse_temp_9.contains(&*{
                let base = &text;
                let idx: i32 = pos;
            ...
            let in_class = _cse_temp_10;
            let _cse_temp_11 = (in_class) && (!node.negated);
            let _cse_temp_12 = (!in_class) && (node.negated);
    ...
    if _cse_temp_0 {
        let _cse_temp_15 = text.len() as i32;
        let _cse_temp_16 = pos == _cse_temp_15;
        if _cse_temp_16 {
            return Ok(vec![pos]);
        }
        return Ok(vec![]);
    ...
    if _cse_temp_0 {
        ...
        while !current.is_empty() {
            ...
            for p in current.iter().cloned() {
                for np in match_node(&child, text, p) {
                    if !results.contains(&np) {
                        results.push(np);
                        new_positions.push(DepylerValue::Str(format!("{:?}", np)));
                    }
                }
            }
    ...
    if _cse_temp_0 {
        child = node
            ...
            .cloned()
            .expect("IndexError: list index out of range");
        let first = match_node(&child, text, pos)?;
        if first.is_empty() {
            return Ok(vec![]);
        }
        results = first.into_iter().collect::<Vec<_>>();
        current = first.into_iter().collect::<Vec<_>>();
        while !current.is_empty() {
            let mut new_positions = vec![];
            for p in current.iter().cloned() {
                for np in match_node(&child, text, p) {
    ...
    if _cse_temp_0 {
        child = node
            ...
            .expect("IndexError: list index out of range");
        results = vec![pos];
        results.extend(match_node(&child, text, pos).iter().cloned());
        return Ok(results
            .into_iter()
    ...
    if _cse_temp_0 {
        ...
        let left_matches = match_node(&left, text, pos)?;
        results = vec![];
        for lp in left_matches.iter().cloned() {
            results.extend(match_node(&right, text, lp).iter().cloned());
        }
        return Ok(results);
    }
    if _cse_temp_0 {
        ...
        results.extend(match_node(&right, text, pos).iter().cloned());
        return Ok(results
            .into_iter()
            .collect::<std::collections::HashSet<_>>());
    }
...
pub fn r#match<'a, 'b>(
    ...
    let end_positions = match_node(&ast, text, 0)?;
    Ok(end_positions.contains(&text.len() as i32))
}
#[doc = "Find first match in text. Returns(start, end) or None."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn search<'a, 'b>(
    pattern: &'a str,
    ...
    let parser = Parser::new(pattern);
    let ast = parser.parse();
    for start in 0..(text.len() as i32 + 1) {
        let end_positions = match_node(&ast, text, start)?;
        if !end_positions.is_empty() {
            return Ok(Some((start, *end_positions.iter().max().unwrap())));
        }
    }
    Ok(None)
}
#[doc = "Find all non-overlapping matches."]
#[doc = " Depyler: verified panic-free"]
pub fn find_all<'b, 'a>(
    pattern: &'a str,
    text: &'b str,
    ...
    while pos <= text.len() as i32 {
        let end_positions = match_node(&ast, text, pos)?;
        if !end_positions.is_empty() {
            let end = *end_positions.iter().max().unwrap();
            if end > pos {
                matches.push(DepylerValue::Str(format!("{:?}", (pos, end))));
                pos = end;
            } else {
                pos = pos + 1;
            }
...
#[doc = "Replace all matches with replacement."]
#[doc = " Depyler: verified panic-free"]
pub fn replace<'b, 'a>(
    pattern: &'a str,
    text: String,
    ...
    let mut result = vec![];
    let mut last_end = 0;
    for (start, end) in matches.iter().cloned() {
        result.push(DepylerValue::Str(format!("{:?}", {
            let base = text;
            let start_idx: i32 = last_end;
            let stop_idx: i32 = start;
            let len = base.chars().count() as i32;
            let actual_start = if start_idx < 0 {
                (len + start_idx).max(0) as usize
            } else {
        ...
        result.push(DepylerValue::Str(replacement.to_string()));
        last_end = end;
    }
    result.push(DepylerValue::Str(format!("{:?}", {
        let base = text;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = args.mode == "match";
    if _cse_temp_0 {
        if r#match(&args.pattern, &text).unwrap_or(false) {
            println!("{}", "Match");
            return Ok(());
        }
        println!("{}", "No match");
        ...
        let _cse_temp_1 = args.mode == "search";
        let mut result;
        if _cse_temp_1 {
            result = Some(search(&args.pattern, &text)?);
            if let Some(ref result_val) = result {
                let (start, end) = result_val;
                println!(
                    "{}",
                    format!("Found at [{:?}:{:?}]: {}", start, end, {
                        let base = text;
            ...
            let _cse_temp_2 = args.mode == "findall";
            if _cse_temp_2 {
                let matches = find_all(&args.pattern, &text)?;
                for (start, end) in matches.iter().cloned() {
                    println!(
                        "{}",
                        format!("[{}:{}]: {}", start, end, {
                ...
                println!("{}", format!("Total: {} matches", matches.len() as i32));
            } else {
                let _cse_temp_3 = args.mode == "replace";
                if _cse_temp_3 {
                    let _cse_temp_4 = if args.replacement.is_empty() {
                        "".to_string()
                    } else {
                        args.replacement.to_string()
                    };
                    let repl = _cse_temp_4;
                    result = Some(replace(&args.pattern, &text, &repl)?);
                    println!("{:?}", result);
                } else {
                    let _cse_temp_5 = args.mode == "split";
                    if _cse_temp_5 {
                        let parts = split(&args.pattern, &text)?;
                        for (i, part) in parts
                            .iter()
                            .cloned()
                            .enumerate()
