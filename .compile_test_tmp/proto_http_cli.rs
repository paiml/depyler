...
impl HttpMethod {
    pub const GET: i32 = auto();
    pub const POST: i32 = auto();
    pub const PUT: i32 = auto();
    pub const DELETE: i32 = auto();
    pub const PATCH: i32 = auto();
    pub const HEAD: i32 = auto();
    pub const OPTIONS: i32 = auto();
    pub const TRACE: i32 = auto();
    pub const CONNECT: i32 = auto();
    pub fn new() -> Self {
        Self {}
    }
}
...
impl HttpRequest {
    ...
    pub fn content_length(&self) -> i32 {
        let cl = self.header("Content-Length".to_string());
        return if cl {
            cl.parse::<i32>().unwrap_or(0)
        } else {
            0
        };
    }
    pub fn content_type(&self) -> Option<String> {
        return Some(self.header("Content-Type".to_string()));
    }
    pub fn is_keep_alive(&self) -> bool {
        let conn = self.header("Connection".to_string());
        if conn {
            return conn.to_lowercase() == "keep-alive".to_string();
        };
        return self.version.clone() == "HTTP/1.1".to_string();
    }
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let header_bytes = lines.join("\r\n".to_string()).encode() + b"\r\n";
        return header_bytes + self.body.clone();
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl HttpResponse {
    ...
    pub fn is_redirect(&self) -> bool {
        return 300 <= self.status_code.clone() && self.status_code.clone() < 400;
    }
    pub fn is_error(&self) -> bool {
        return self.status_code.clone() >= 400;
    }
    pub fn to_bytes(&self) -> Vec<u8> {
        let mut lines = vec![format!(
            "{} {} {}",
...
impl HttpParser {
    ...
    pub fn parse_request(&mut self) -> Option<HttpRequest> {
        let header_end = self
            .buffer
            .clone()
            .find(b"\r\n\r\n")
        ...
        let header_bytes = {
            ...
            let stop_idx = (header_end) as isize;
        ...
        if !lines {
            panic!(
                "Exception: {}",
                HttpParseError::new("Empty request".to_string())
            );
        };
        let request_line = {
            let _base = &lines;
        ...
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
        if (request_line.len() as i32) < 3 {
            panic!(
                "Exception: {}",
                HttpParseError::new("Invalid request line".to_string())
            );
        };
        let (method_str, path, version) = (
            {
                let _base = &request_line;
                let _idx = 0;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
        ...
        if !self.METHODS.clone().contains_key(&method_str) {
            panic!(
                "Exception: {}",
                HttpParseError::new(format!("Unknown method: {}", method_str))
            );
        };
        let method = {
            let _base = &self.METHODS.clone();
            let _idx = method_str;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
        ...
        if content_length > 0 {
            if (self.buffer.clone().len() as i32) < body_start + content_length {
                return None;
            };
            request.body = {
                let s = &self.buffer.clone();
                ...
                let stop_idx = (body_start + content_length) as isize;
                let start = if start_idx < 0 {
                    (len + start_idx).max(0) as usize
                } else {
                    start_idx as usize
            ...
            self.buffer = {
                let s = &self.buffer.clone();
                let len = s.chars().count() as isize;
                let start_idx = (body_start + content_length) as isize;
                let start = if start_idx < 0 {
                    (len + start_idx).max(0) as usize
                } else {
                    start_idx as usize
    ...
    pub fn parse_response(&mut self) -> Option<HttpResponse> {
        let header_end = self
            .buffer
            .clone()
            .find(b"\r\n\r\n")
        ...
        if !lines {
            panic!(
                "Exception: {}",
                HttpParseError::new("Empty response".to_string())
            );
        };
        let status_line = {
            let _base = &lines;
        ...
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
        if (status_line.len() as i32) < 3 {
            panic!(
                "Exception: {}",
                HttpParseError::new("Invalid status line".to_string())
            );
        };
        let version = {
            let _base = &status_line;
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
        ...
        let mut response = HttpResponse::new(version, status_code, reason, headers);
        let body_start = header_end + 4;
...
#[doc = "Parse HTTP request."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_request(data: &Vec<u8>) -> Result<HttpRequest, HttpParseError> {
    let mut parser = HttpParser::new();
    parser.feed(data);
    let request = parser.parse_request();
    if request.is_none() {
        return Err(HttpParseError::new(
            "Incomplete request".to_string().to_string(),
        ));
    }
    Ok(request)
}
#[doc = "Parse HTTP response."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_response(data: &Vec<u8>) -> Result<HttpResponse, HttpParseError> {
    let mut parser = HttpParser::new();
    parser.feed(data);
    let response = parser.parse_response();
    if response.is_none() {
        return Err(HttpParseError::new(
            "Incomplete response".to_string().to_string(),
        ));
    }
    Ok(response)
}
#[doc = "Build HTTP request."]
#[doc = " Depyler: proven to terminate"]
pub fn build_request<'l1, 'b, 'c, 'a>(
    method: &'a str,
    path: &'b str,
    headers: &'c Option<std::collections::HashMap<String, String>>,
    body: &'l1 Vec<u8>,
) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let method_enum = HttpParser::METHODS.get(&method.to_uppercase()).cloned();
    ...
    let _cse_temp_0 = (headers.is_some())
        || ({
            let map: HashMap<String, String> = HashMap::new();
            map
        });
    let mut hdrs = _cse_temp_0.clone();
    let _cse_temp_1 = !hdrs.contains("Content-Length");
    let _cse_temp_2 = (body) && (_cse_temp_1);
    if _cse_temp_2 {
        let _cse_temp_3 = body.len() as i32;
        let _cse_temp_4 = (_cse_temp_3).to_string();
        hdrs.insert("Content-Length".to_string(), _cse_temp_4);
    }
    let request = HttpRequest::new(method_enum, path, "HTTP/1.1".to_string(), hdrs, body);
    Ok(request.to_bytes())
}
#[doc = "Build HTTP response."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn build_response<'a, 'c, 'b>(
    status_code: i32,
    reason: &'a str,
    headers: &'b Option<std::collections::HashMap<String, String>>,
    body: &'c Vec<u8>,
) -> Vec<u8> {
    let _cse_temp_0 = (headers.is_some())
        || ({
    ...
    let response = HttpResponse::new("HTTP/1.1".to_string(), status_code, reason, hdrs, body);
    response.to_bytes()
}
#[doc = "Simulate HTTP operations."]
pub fn simulate_http(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
        ...
        if cmd == "parse_request" {
            ...
            match (|| -> Result<(), Box<dyn std::error::Error>> {
                data = parts
                    ...
                    .replace(&b"\\r\\n", &b"\r\n");
                parser.feed(data);
                req = Some(parser.parse_request());
                if let Some(ref req_val) = req {
                    results.push(format!("{} {}", req_val.method.name, req_val.path));
                } else {
                    results.push("incomplete".to_string().to_string());
                }
                Ok(())
            })() {
                Ok(()) => {}
            ...
            if cmd == "parse_response" {
                ...
                match (|| -> Result<(), Box<dyn std::error::Error>> {
                    data = parts
                        ...
                        .replace(&b"\\r\\n", &b"\r\n");
                    parser.feed(data);
                    resp = Some(parser.parse_response());
                    if let Some(ref resp_val) = resp {
                        results.push(format!("{} {}", resp_val.status_code, resp_val.reason));
                    } else {
                        results.push("incomplete".to_string().to_string());
                    }
                    Ok(())
                ...
                if cmd == "build_request" {
                    let _split_parts = parts
                        ...
                        .map(|s| s.to_string())
                        .collect::<Vec<String>>();
                    let method = _split_parts.get(0).cloned().unwrap_or_default();
                    let path = _split_parts.get(1).cloned().unwrap_or_default();
                    data = build_request(&method, path.display().to_string(), &None)?;
                    results.push(
                    ...
                    if cmd == "build_response" {
                        let _split_parts = parts
                            ...
                            .map(|s| s.to_string())
                            .collect::<Vec<String>>();
                        let code = _split_parts.get(0).cloned().unwrap_or_default();
                        let reason = _split_parts.get(1).cloned().unwrap_or_default();
                        data = build_response((code) as i32, &reason, &None);
                        results.push(
                            String::from_utf8_lossy(&data)
                                .to_string()
                                .replace("\r\n", "\\r\\n"),
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: proto_http_cli.py <command>[args]");
        println!(
            "{}",
            "Commands: parse-request, parse-response, build-request, build-response"
        );
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "parse-request";
    ...
    if _cse_temp_4 {
        ...
        let mut req: Option<String> = None;
        match (|| -> Result<(), Box<dyn std::error::Error>> {
            req = Some(parse_request(&data)?);
            println!("{}", format!("Method: {}", req.method.name));
            println!("{}", format!("Path: {}", req.path));
            println!("{}", format!("Version: {}", req.version));
            for (k, v) in req
                .headers
                .iter()
        ...
        let _cse_temp_5 = cmd == "parse-response";
        let _cse_temp_6 = (_cse_temp_5) && (_cse_temp_3);
        if _cse_temp_6 {
            data = std::env::args()
                .collect::<Vec<String>>()
                .get(2usize)
            ...
            let mut resp: Option<String> = None;
            match (|| -> Result<(), Box<dyn std::error::Error>> {
                resp = Some(parse_response(&data)?);
                println!(
                    "{}",
                    format!("Status: {} {}", resp.status_code, resp.reason)
                );
                for (k, v) in resp
                    .headers
                    .iter()
...
mod tests {
    ...
    #[test]
    fn test_simulate_http_examples() {
        assert_eq!(simulate_http(vec![]), vec![]);
        assert_eq!(simulate_http(vec![1]), vec![1]);
    }
}
