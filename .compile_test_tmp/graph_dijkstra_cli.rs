...
pub struct WeightedGraph {
    pub _adj: std::collections::HashMap<i32, Vec<(i32, i32)>>,
    pub _directed: bool,
}
impl WeightedGraph {
    ...
    pub fn add_edge(&self, u: i32, v: i32, weight: i32) {
        self.add_vertex(u);
        self.add_vertex(v);
        {
    ...
    pub fn neighbors(&self, v: i32) -> Vec<(i32, i32)> {
...
enum Commands {
    ...
    Dijkstra { start: i32, edges: Vec<String> },
}
#[derive(Default)]
...
#[doc = "Dijkstra's algorithm - returns distances from start."]
...
pub fn dijkstra(graph: &WeightedGraph, start: i32) -> HashMap<i32, i32> {
    ...
    let heap: Vec<(i32, i32)> = vec![(0, start)];
    while !heap.is_empty() {
        let (dist, vertex) = {
            let heap = &mut (heap);
            if heap.is_empty() {
        ...
        if dist
            > distances
                .get(&vertex)
                .cloned()
                .unwrap_or("inf".to_string().parse::<f64>().unwrap())
        {
            continue;
        }
...
#[doc = "Dijkstra's algorithm - returns distance and path."]
pub fn dijkstra_path(
    ...
    let mut predecessors: std::collections::HashMap<i32, i32> = {
    ...
    while !heap.is_empty() {
        ...
        for (neighbor, weight) in graph.neighbors(vertex) {
            ...
            {
                distances.insert(neighbor.clone(), new_dist);
                predecessors.insert(neighbor.clone(), vertex);
                {
                    let heap = &mut (heap);
                    let item = (new_dist, neighbor);
                    heap.push(item);
    ...
    let _cse_temp_0 = distances.get(&end).is_none();
    if _cse_temp_0 {
        return Ok((-1, vec![]));
    }
    let mut path: Vec<i32> = vec![];
    let mut current = end.clone();
    while (predecessors.get(&current).is_some()) || (current == start) {
        path.push(current);
        if current == start {
            break;
        }
        current = predecessors
            .get(&DepylerValue::Int(current as i64))
            .cloned()
            .unwrap_or_default();
    }
    path.reverse();
    Ok((
        distances
            .get(&DepylerValue::Int(end as i64))
            .cloned()
            .unwrap_or_default(),
        path,
    ))
}
#[doc = "Bellman-Ford algorithm - handles negative weights."]
pub fn bellman_ford(
    graph: &WeightedGraph,
    start: i32,
) -> Result<Option<HashMap<i32, i32>>, Box<dyn std::error::Error>> {
    let vertices = graph.vertices();
    let mut distances: std::collections::HashMap<i32, i32> =
        vertices.iter().map(|k| (k.clone(), 1000000000)).collect();
    distances.insert(start.clone(), 0);
    for __sanitized in 0..((vertices.len() as i32).saturating_sub(1)) {
        for u in vertices.iter().cloned() {
            for (v, weight) in graph.neighbors(u) {
                if distances
                    .get(&DepylerValue::Int(u as i64))
                    .cloned()
                    .unwrap_or_default()
                    + weight
                    < distances
...
#[doc = "Shortest path in DAG using topological order."]
pub fn shortest_path_dag<'a, 'b>(
    graph: &'a WeightedGraph,
    start: i32,
    topo_order: &'b Vec<i32>,
) -> Result<HashMap<i32, i32>, Box<dyn std::error::Error>> {
    let mut start_found: bool = Default::default();
    let mut distances: std::collections::HashMap<i32, i32> = graph
        .vertices()
        .iter()
        .map(|k| (k.clone(), 1000000000))
        .collect();
    distances.insert(start.clone(), 0);
    start_found = false;
    for vertex in topo_order.iter().cloned() {
        if vertex == start {
            start_found = true;
        }
        if !start_found {
...
#[doc = "All pairs shortest paths using repeated Dijkstra."]
#[doc = " Depyler: verified panic-free"]
pub fn all_pairs_shortest(graph: &WeightedGraph) -> HashMap<(i32, i32), i32> {
    let mut result: std::collections::HashMap<(i32, i32), i32> = {
    ...
    for start in graph.vertices() {
        ...
        for (end, dist) in distances
            ...
            .collect::<Vec<_>>()
        {
            result.insert((start, end).to_string(), dist);
        }
    }
    result
}
#[doc = "Shortest path with at most k intermediate stops."]
#[doc = " Depyler: verified panic-free"]
pub fn shortest_path_k_stops(graph: &WeightedGraph, start: i32, end: i32, k: i32) -> i32 {
    ...
    let queue: Vec<(i32, i32, i32)> = vec![(0, start, 0)];
    result = 1000000000;
    while !queue.is_empty() {
        let (dist, vertex, stops) = {
            let heap = &mut (queue);
            if heap.is_empty() {
                panic!("heappop from empty heap");
            }
        ...
        if vertex == end {
            result = depyler_min((result).clone(), (dist).clone());
            continue;
        }
        if stops > k {
            continue;
        ...
        for (neighbor, weight) in graph.neighbors(vertex) {
            let new_dist = dist + weight;
            if (new_dist < distances.get(&neighbor).cloned().unwrap_or(1000000000)) || (stops < k) {
                distances.insert(neighbor.clone(), new_dist);
                {
                    let heap = &mut (queue);
                    let item = (new_dist, neighbor, stops + 1);
                    heap.push(item);
                    let mut pos = heap.len() - 1;
                    while pos > 0 {
                        let parent = (pos - 1) / 2;
...
#[doc = "Prim's algorithm for minimum spanning tree."]
pub fn min_spanning_tree_prim(
    graph: &WeightedGraph,
    ...
    let heap: Vec<(i32, i32, i32)> = vec![];
    for (neighbor, weight) in graph.neighbors(start) {
        {
            let heap = &mut (heap);
            let item = (weight, start, neighbor);
            heap.push(item);
            let mut pos = heap.len() - 1;
            while pos > 0 {
                let parent = (pos - 1) / 2;
    ...
    while (!heap.is_empty()) && ((visited.len() as i32) < graph.vertices().len() as i32) {
        let (weight, u, v) = {
            let heap = &mut (heap);
            if heap.is_empty() {
                panic!("heappop from empty heap");
...
#[doc = "Find graph center(vertex minimizing max distance to others)."]
pub fn graph_center(graph: &WeightedGraph) -> Result<i32, Box<dyn std::error::Error>> {
    let mut center: i32 = Default::default();
    let all_distances = all_pairs_shortest(&graph);
    let vertices = graph.vertices();
    let mut min_eccentricity = 1000000000;
    center = -1;
    for v in vertices.iter().cloned() {
        let mut eccentricity = 0;
        for u in vertices.iter().cloned() {
            if all_distances.get(&(v, u)).is_some() {
                eccentricity = depyler_max(
                    (eccentricity).clone(),
                    (all_distances
                        .get(&DepylerValue::Int((v, u) as i64))
                        .cloned()
                        .unwrap_or_default())
                    .clone(),
                );
...
#[doc = "Build graph from edge strings like '1-2:5'."]
#[doc = " Depyler: verified panic-free"]
pub fn build_weighted_graph(
    edges: &Vec<String>,
    directed: bool,
) -> Result<WeightedGraph, Box<dyn std::error::Error>> {
    let graph = WeightedGraph::new(directed);
    for edge in edges.iter().cloned() {
        ...
        let weight = if parts.len() as i32 > 1 {
            (parts
                .get(1usize)
                .cloned()
                .expect("IndexError: list index out of range")) as i32
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Dijkstra { .. }));
    match &args.command {
        Some(Commands::Dijkstra {
            ref edges,
            ref start,
            ..
        }) => {
            let start = *start;
            let mut graph = build_weighted_graph(&edges, false)?;
            let distances = dijkstra(&graph, start);
            println!("{}", format!("Distances: {:?}", distances));
        }
        Some(Commands::Path {
            ref edges,
            ref end,
            ...
            let start = *start;
            let mut graph = build_weighted_graph(&edges, false)?;
            let (dist, path) = dijkstra_path(&graph, start, end)?;
            println!("{}", format!("Distance: {:?}, Path: {:?}", dist, path));
        }
        Some(Commands::Mst { ref edges, .. }) => {
            let mut graph = build_weighted_graph(&edges, false)?;
            let edges = min_spanning_tree_prim(&graph)?;
            let _cse_temp_3 = edges.iter().cloned().map(|(_, _, w)| w).sum::<i32>();
            let total = _cse_temp_3;
            println!("{}", format!("MST weight: {}, Edges: {:?}", total, edges));
        }
