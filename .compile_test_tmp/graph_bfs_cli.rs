...
impl Graph {
    pub fn new(_directed: bool) -> Self {
        Self {
    ...
    pub fn vertex_count(&self) -> i32 {
        return self._adj.clone().len() as i32;
    }
}
#[derive(Default)]
enum Commands {
    ...
    Bfs { start: i32, edges: Vec<String> },
    
    Dfs { start: i32, edges: Vec<String> },
    
    Components { edges: Vec<String> },
...
pub fn bfs(graph: &Graph, start: i32) -> Vec<i32> {
    ...
    let mut queue: std::collections::VecDeque<i32> = VecDeque::from(vec![start]);
    while !queue.is_empty() {
        ...
        if !visited.contains(&vertex) {
            ...
            result.push(vertex);
            for neighbor in {
                let mut sorted_vec = graph.neighbors(vertex).iter().cloned().collect::<Vec<_>>();
...
#[doc = "Depth-first search traversal(iterative)."]
#[doc = " Depyler: verified panic-free"]
pub fn dfs(graph: &Graph, start: i32) -> Vec<i32> {
    ...
    while !stack.is_empty() {
        let vertex = stack.pop().unwrap_or_default();
        if !visited.contains(&vertex) {
            visited.insert(vertex);
            result.push(vertex);
            for neighbor in {
                let mut __sorted_result = graph.neighbors(vertex).clone();
...
#[doc = "Depth-first search traversal(recursive)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn dfs_recursive(graph: &Graph, start: i32) -> Vec<i32> {
    let visited: std::collections::HashSet<i32> = std::collections::HashSet::<i32>::new();
    let result: Vec<i32> = vec![];
    let dfs_visit = move |v: i32| -> () {
        visited.insert(v);
        result.push(v);
        for neighbor in {
            let mut sorted_vec = graph.neighbors(v).iter().cloned().collect::<Vec<_>>();
            sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            sorted_vec
        } {
            if !visited.contains(&neighbor) {
...
#[doc = "Find shortest path using BFS(unweighted)."]
#[doc = " Depyler: verified panic-free"]
pub fn bfs_shortest_path(graph: &Graph, start: i32, end: i32) -> Vec<i32> {
    ...
    let mut queue: std::collections::VecDeque<(i32, Vec<i32>)> =
        VecDeque::from(vec![(start, vec![start])]);
    while !queue.is_empty() {
        let (vertex, path) = queue.pop_front();
        for neighbor in graph.neighbors(vertex) {
            if neighbor == end {
                return path
                    .iter()
                    .chain(vec![neighbor].iter())
                    .cloned()
                    .collect::<Vec<_>>();
            }
            if !visited.contains(&neighbor) {
...
#[doc = "Calculate distances from start to all reachable vertices."]
pub fn bfs_distance(
    graph: &Graph,
    start: i32,
...
#[doc = "BFS with level grouping."]
#[doc = " Depyler: verified panic-free"]
pub fn bfs_level_order(graph: &Graph, start: i32) -> Vec<Vec<i32>> {
    let _cse_temp_0 = !graph.vertices().get(&start).is_some();
    if _cse_temp_0 {
        return vec![];
    }
    ...
    let mut current_level: Vec<i32> = vec![start];
    while !current_level.is_empty() {
        levels.push(current_level);
        let mut next_level: Vec<i32> = vec![];
        for vertex in current_level.iter().cloned() {
            for neighbor in {
                let mut sorted_vec = graph.neighbors(vertex).iter().cloned().collect::<Vec<_>>();
                sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                sorted_vec
...
#[doc = "Check if undirected graph is connected."]
#[doc = " Depyler: proven to terminate"]
pub fn is_connected(graph: &Graph) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    Ok(visited.len() as i32 == vertices.len() as i32)
}
#[doc = "Find all connected components."]
#[doc = " Depyler: verified panic-free"]
pub fn connected_components(graph: &Graph) -> Vec<Vec<i32>> {
    let mut visited: std::collections::HashSet<i32> = std::collections::HashSet::<i32>::new();
    let mut components: Vec<Vec<i32>> = vec![];
    for vertex in graph.vertices() {
        if !visited.contains(&vertex) {
            let component = bfs(&graph, vertex);
            for item in component {
                visited.insert(item);
            }
            components.push({
                let mut sorted_vec = component.iter().cloned().collect::<Vec<_>>();
                sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
                sorted_vec
            });
        }
...
#[doc = "Check if path exists between start and end."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn has_path(graph: &Graph, start: i32, end: i32) -> bool {
    bfs(&graph, start)
        .into_iter()
        .collect::<std::collections::HashSet<_>>()
        .contains(&end)
}
#[doc = "Detect cycle in undirected graph using DFS."]
#[doc = " Depyler: verified panic-free"]
pub fn cycle_detect_undirected(graph: &Graph) -> bool {
    let visited: std::collections::HashSet<i32> = std::collections::HashSet::<i32>::new();
    let dfs_cycle = move |v: i32, parent: i32| -> bool {
        visited.insert(v);
...
#[doc = "Check if graph is bipartite using BFS coloring."]
pub fn bipartite_check(graph: &Graph) -> Result<bool, Box<dyn std::error::Error>> {
...
#[doc = "Build graph from edge strings like '1-2'."]
#[doc = " Depyler: verified panic-free"]
pub fn build_graph(edges: &Vec<String>, directed: bool) -> Graph {
    let graph = Graph::new(directed);
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Bfs { .. }));
    match &args.command {
        Some(Commands::Bfs {
            ref edges,
            ref start,
            ...
            let start = *start;
            let mut graph = build_graph(&edges, false);
            let mut result = bfs(&graph, start);
            println!("{}", format!("BFS: {:?}", result));
        }
        Some(Commands::Dfs {
            ref edges,
            ref start,
            ...
            let start = *start;
            let mut graph = build_graph(&edges, false);
            let mut result = dfs(&graph, start);
            println!("{}", format!("DFS: {:?}", result));
        }
        Some(Commands::Path {
            ref edges,
            ref end,
            ...
            let start = *start;
            let mut graph = build_graph(&edges, false);
            let mut result = bfs_shortest_path(&graph, start, end);
            println!("{}", format!("Path: {:?}", result));
        }
        Some(Commands::Components { ref edges, .. }) => {
            let mut graph = build_graph(&edges, false);
            let mut result = connected_components(&graph);
            println!("{}", format!("Components: {:?}", result));
        }
...
mod tests {
    ...
    #[test]
    fn test_is_connected_examples() {
        let _ = is_connected(Default::default());
    }
    #[test]
    fn test_cycle_detect_undirected_examples() {
        let _ = cycle_detect_undirected(Default::default());
    }
    #[test]
    fn test_bipartite_check_examples() {
        let _ = bipartite_check(Default::default());
    }
}
