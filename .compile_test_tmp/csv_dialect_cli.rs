...
#[doc = r" Simple string split(NASA mode alternative to regex split)"] pub fn split(pattern: & str, text: & str) -> Vec<String>{
    ...
    #[doc = "Input CSV file(- for stdin)"] input: Option<String>, #[doc = "Field delimiter(default: ,)"] delimiter: String, #[doc = "Quote character(default: \")"] quote: String, #[doc = "Escape character(default: none)"] escape: String, #[doc = "Convert to this delimiter"] to_delimiter: Option<String>, #[doc = "Extract column N(0-indexed)"] column: Option<i32>, #[doc = "Count rows by column count"] count_columns: bool, #[doc = "Skip first line"] skip_header: bool
}
#[doc = "Parse a single CSV line with custom dialect."] pub fn parse_csv_line<'a, 'c, 'l1, 'b>(line: & 'a str, delimiter: & 'b str, quote: & 'c str, escape: & 'l1 str) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    let mut current: String = Default::default();
    let mut in_quotes: bool = Default::default();
    ...
    if((! escape.is_empty()) &&(char = = (* escape))) &&(i + 1<line.len() as i32) {
...
if(char = = (* delimiter)) &&(! in_quotes) {
    fields.push(DepylerValue::Str(current.to_string()));
    current = "".to_string();
    i = i + 1;
    continue;
   
}
current = format!("{}{}", current, char);
    i = i + 1;
...
#[doc = "Format fields as a CSV line."] #[doc = " Depyler: verified panic-free"] pub fn format_csv_line<'a, 'c, 'b>(fields: & 'a Vec<DepylerValue>, delimiter: & 'b str, quote: & 'c str, always_quote: bool) -> String {
    let mut result = vec! [];
    for field in fields.iter().cloned() {
    let needs_quote  = (((always_quote) ||(field.contains(& * delimiter))) ||(field.contains(& * quote))) ||(field.contains("\n"));
    if needs_quote {
    let escaped = field.replace(quote, & format!("{}{}", quote, quote));
    result.push(DepylerValue::Str(format!("{:?}", format!("{}{}{}", quote, escaped, quote))));
   
}
else {
    result.push(DepylerValue::Str(format!("{:?}", field)));
   
}
} result.join (delimiter)
}
#[doc = "Convert CSV from one delimiter to another."] #[doc = " Depyler: verified panic-free"] pub fn convert_delimiter<'c, 'l1, 'l2, 'a, 'b>(lines: & 'a Vec<DepylerValue>, from_delim: & 'b str, to_delim: & 'c str, quote: & 'l1 str, escape: & 'l2 str) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    let mut result = vec! [];
    for line in lines.iter().cloned() {
    if line.to_string().trim().to_string() {
    let fields = parse_csv_line(line.to_string().trim().to_string(), from_delim, quote, escape) ?;
    result.push(DepylerValue::Str(format!("{:?}", format_csv_line(& fields, to_delim, quote, false))));
   
}
} Ok(result)
}
#[doc = "Extract a single column from CSV."] #[doc = " Depyler: verified panic-free"] pub fn get_column<'l1, 'a, 'c, 'b>(lines: & 'a Vec<DepylerValue>, col_index: i32, delimiter: & 'b str, quote: & 'c str, escape: & 'l1 str) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    let mut result = vec! [];
    for line in lines.iter().cloned() {
    if line.to_string().trim().to_string() {
    let fields = parse_csv_line(line.to_string().trim().to_string(), delimiter, quote, escape) ?;
    if col_index<fields.len() as i32 {
    result.push(DepylerValue::Str(format!("{:?}", fields.get(col_index as usize).cloned().expect("IndexError: list index out of range"))));
...
#[doc = "Count rows by number of columns."] #[doc = " Depyler: verified panic-free"] pub fn count_columns<'a, 'c, 'l1, 'b>(lines: & 'a Vec<DepylerValue>, delimiter: & 'b str, quote: & 'c str, escape: & 'l1 str) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>>{
    let mut counts: std::collections::HashMap<DepylerValue, DepylerValue>= {
    ...
    counts.insert(n.to_string().clone(), DepylerValue::Str(format!("{:?}", counts.get(& n).cloned().unwrap_or(0) + 1)));
...
#[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), std::io::Error>{
    ...
    let has_to_delimiter = args.to_delimiter.is_some();
    let has_column = args.column.is_some();
    let _cse_temp_0 = args.input == "-";
    let _cse_temp_1  = (! has_input) ||(_cse_temp_0);
    if _cse_temp_1 {
    lines = {
    use std::io::BufRead;
    std::io::stdin ().lock().lines().collect::<Result<Vec<_>, _>>().unwrap() };
   
}
else {
    let mut f = std::fs::File::open(args.input) ?;
    lines = std::io::BufReader::new(f).lines().map(| l | l.unwrap_or_default()).collect::<Vec<_>>();
   
}
let _cse_temp_2  = (args.skip_header) &&(! lines.is_empty());
    if! _cse_temp_2.is_empty() {
    lines = {
    let base = & lines;
...
if has_to_delimiter {
    let result = convert_delimiter(& lines, & args.delimiter, & args.to_delimiter, & args.quote, & args.escape) ?;
    for line in result.iter().cloned() {
    println!("{:?}", line);
   
}
} else {
    if has_column {
    let values = get_column(& lines, args.column, & args.delimiter, & args.quote, & args.escape) ?;
    ...
    let counts = count_columns(& lines, & args.delimiter, & args.quote, & args.escape) ?;
    for(n, count) in {
    let mut sorted_vec = counts.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>().iter().cloned().collect::<Vec<_>>();
    sorted_vec.sort_by(| a, b | a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
    sorted_vec
}
{
    println!("{}", format!("{:?} columns: {:?} rows", n, count));
   
}
} else {
    for line in lines.iter().cloned() {
    if line.trim().to_string() {
    let fields = parse_csv_line(line.trim().to_string(), & args.delimiter, & args.quote, & args.escape) ?;
    println!("{}", format_csv_line(& fields, & args.delimiter, & args.quote, false));
