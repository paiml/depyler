...
pub enum BoolOrFloatUnion {
    Boolean(bool),
    Float(f64),
}
impl From<bool> for BoolOrFloatUnion {
    fn from(value: bool) -> Self {
        BoolOrFloatUnion::Boolean(value)
    }
}
impl From<f64> for BoolOrFloatUnion {
    fn from(value: f64) -> Self {
        BoolOrFloatUnion::Float(value)
    }
}
impl BoolOrFloatUnion {
    pub fn is_boolean(&self) -> bool {
        matches!(self, BoolOrFloatUnion::Boolean(_))
    }
    pub fn is_float(&self) -> bool {
        matches!(self, BoolOrFloatUnion::Float(_))
    }
    pub fn as_boolean(&self) -> Option<&bool> {
        match self {
            BoolOrFloatUnion::Boolean(value) => Some(value),
            _ => None,
        }
    }
    pub fn as_float(&self) -> Option<&f64> {
        match self {
            BoolOrFloatUnion::Float(value) => Some(value),
            _ => None,
        }
    }
}
...
impl TokenType {
    ...
    pub const NEQ: i32 = auto();
    pub const LT: i32 = auto();
    pub const LE: i32 = auto();
    pub const GT: i32 = auto();
    pub const GE: i32 = auto();
    pub const AND: i32 = auto();
    pub const OR: i32 = auto();
    pub const NOT: i32 = auto();
    pub const TRUE: i32 = auto();
    pub const FALSE: i32 = auto();
...
impl Token {
    pub fn new(type_: TokenType, value: DepylerValue) -> Self {
        Self { type_, value }
    }
}
#[derive(Debug, Clone)]
...
impl Lexer {
    ...
    pub fn number(&self) -> Token {
        let mut result = "".to_string();
        while !self.peek().is_empty() && self.peek().chars().all(|c| c.is_ascii_digit())
            || self.peek() == ".".to_string()
        {
            let result = result + self.advance();
        }
        return Token::new(TokenType::NUMBER, result.parse::<f64>().unwrap_or(0.0));
    }
    pub fn identifier(&self) -> Token {
        let mut result = "".to_string();
        while !self.peek().is_empty() && self.peek().chars().all(|c| c.is_alphanumeric())
            || self.peek() == "_".to_string()
        {
            let result = result + self.advance();
        }
        let lower = result.to_lowercase();
        if self.KEYWORDS.clone().contains_key(&lower) {
            let token_type = {
                let _base = &self.KEYWORDS.clone();
                let _idx = lower;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
            ...
            if token_type == TokenType::TRUE {
                return Token::new(token_type, true);
            };
            if token_type == TokenType::FALSE {
                return Token::new(token_type, false);
            };
            return Token::new(token_type, lower);
        };
        return Token::new(TokenType::IDENTIFIER, result);
    }
    pub fn tokenize(&self) -> Vec<Token> {
        let mut tokens = vec![];
        while self.pos.clone() < (self.text.clone().len() as i32) {
            ...
            if !char.is_empty() && char.chars().all(|c| c.is_ascii_digit())
                || char == ".".to_string()
                    && self.pos.clone() + 1 < (self.text.clone().len() as i32)
                    && !{
                        let _base = &self.text.clone();
                    ...
                    .all(|c| c.is_ascii_digit())
            {
                ...
                if !char.is_empty() && char.chars().all(|c| c.is_alphabetic())
                    || char == "_".to_string()
                {
                    ...
                    if char == "+".to_string() {
                        ...
                        if char == "-".to_string() {
                            ...
                            if char == "*".to_string() {
                                self.advance();
                                tokens.push(Token::new(TokenType::STAR, "*".to_string()));
                            } else {
                                if char == "/".to_string() {
                                    self.advance();
                                    tokens.push(Token::new(TokenType::SLASH, "/".to_string()));
                                } else {
                                    if char == "%".to_string() {
                                        self.advance();
                                        tokens
                                            .push(Token::new(TokenType::PERCENT, "%".to_string()));
                                    } else {
                                        if char == "^".to_string() {
                                            self.advance();
                                            ...
                                            if char == "(".to_string() {
                                                ...
                                                if char == ")".to_string() {
                                                    ...
                                                    if char == ",".to_string() {
                                                        ...
                                                        if char == "=".to_string()
                                                            && self.pos.clone() + 1
                                                                < (self.text.clone().len() as i32)
                                                            && {
                                                                let _base = &self.text.clone();
                                                                let _idx = self.pos.clone() + 1;
                                                                if _idx < 0 {
                                                                    _base[_base.len().wrapping_sub(
                                                                        (-_idx) as usize,
                                                                    )]
                                                                    .clone()
                                                                } else {
                                                                    ...
                                                                    } == "=".to_string()
                                                                {
                                                                        ...
                                                                        } == "=".to_string()
                                                                    {
                                                                        ...
                                                                        if char == "<".to_string() {
                                                                            ...
                                                                            {
                                                                                self.advance();
                                                                                tokens.push(Token::new(TokenType::GT, ">".to_string()));
                                                                            } else {
                                                                                self.advance();
                                                                            };
                                                                        };
...
impl NodeType {
    pub const NUMBER: i32 = auto();
    pub const VARIABLE: i32 = auto();
    pub const BOOLEAN: i32 = auto();
    pub const BINARY_OP: i32 = auto();
    pub const UNARY_OP: i32 = auto();
    pub const FUNCTION_CALL: i32 = auto();
    pub fn new() -> Self {
        Self {}
    }
}
...
pub struct ASTNode {
    pub type_: NodeType,
    pub value: DepylerValue,
    pub left: Option<ASTNode>,
    pub right: Option<ASTNode>,
...
impl Parser {
    ...
    pub fn peek(&self) -> Token {
        if self.pos.clone() >= (self.tokens.clone().len() as i32) {
            return Token::new(TokenType::EOF, "".to_string());
        };
        return {
            let _base = &self.tokens.clone();
            let _idx = self.pos.clone();
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn or_expr(&self) -> ASTNode {
        let mut left = self.and_expr();
        while self.peek().r#type == TokenType::OR {
            self.advance();
            let right = self.and_expr();
            let left = ASTNode::new(NodeType::BINARY_OP);
        }
        return left;
    }
    pub fn and_expr(&self) -> ASTNode {
        let mut left = self.not_expr();
        while self.peek().r#type == TokenType::AND {
            self.advance();
    ...
    pub fn not_expr(&self) -> ASTNode {
        if self.peek().r#type == TokenType::NOT {
            self.advance();
            let operand = self.not_expr();
            return ASTNode::new(NodeType::UNARY_OP);
        };
        return self.comparison();
    }
    pub fn comparison(&self) -> ASTNode {
        ...
        .contains(&self.peek().r#type)
        {
            let op_token = self.advance();
            let op_map = {
                let mut map = std::collections::HashMap::new();
                map.insert(TokenType::EQ, "==".to_string());
                map.insert(TokenType::NEQ, "!=".to_string());
                map.insert(TokenType::LT, "<".to_string());
                map.insert(TokenType::LE, "<=".to_string());
                map.insert(TokenType::GT, ">".to_string());
                map.insert(TokenType::GE, ">=".to_string());
                map
            };
    ...
    pub fn additive(&self) -> ASTNode {
        let mut left = self.multiplicative();
        while [TokenType::PLUS, TokenType::MINUS].contains(&self.peek().r#type) {
            let op_token = self.advance();
            let right = self.multiplicative();
            let op = if op_token.r#type == TokenType::PLUS {
                "+".to_string()
            } else {
                "-".to_string()
            };
    ...
    pub fn multiplicative(&self) -> ASTNode {
        let mut left = self.power();
        while [TokenType::STAR, TokenType::SLASH, TokenType::PERCENT].contains(&self.peek().r#type)
        {
            let op_token = self.advance();
            let right = self.power();
            let op_map = {
                let mut map = std::collections::HashMap::new();
                map.insert(TokenType::STAR, "*".to_string());
                map.insert(TokenType::SLASH, "/".to_string());
                map.insert(TokenType::PERCENT, "%".to_string());
                map
            };
            let left = ASTNode::new(NodeType::BINARY_OP);
        }
    ...
    pub fn power(&self) -> ASTNode {
        let left = self.unary();
        if self.peek().r#type == TokenType::CARET {
            self.advance();
            let right = self.power();
            return ASTNode::new(NodeType::BINARY_OP);
        };
        return left;
    }
    pub fn unary(&self) -> ASTNode {
        if self.peek().r#type == TokenType::MINUS {
            self.advance();
            let operand = self.unary();
            return ASTNode::new(NodeType::UNARY_OP);
        };
        if self.peek().r#type == TokenType::PLUS {
            self.advance();
            return self.unary();
        };
        return self.primary();
    ...
    pub fn primary(&self) -> ASTNode {
        ...
        if token.r#type == TokenType::NUMBER {
            self.advance();
            return ASTNode::new(NodeType::NUMBER);
        };
        if [TokenType::TRUE, TokenType::FALSE].contains(&token.r#type) {
            self.advance();
            return ASTNode::new(NodeType::BOOLEAN);
        };
        if token.r#type == TokenType::IDENTIFIER {
            self.advance();
            let name = token.value;
            if self.peek().r#type == TokenType::LPAREN {
                self.advance();
                let mut args = vec![];
                if self.peek().r#type != TokenType::RPAREN {
                    args.push(self.parse());
                    while self.peek().r#type == TokenType::COMMA {
                        self.advance();
                        args.push(self.parse());
                    }
                };
                ...
                return ASTNode::new(NodeType::FUNCTION_CALL);
            };
            return ASTNode::new(NodeType::VARIABLE);
        };
...
pub struct Evaluator {
    pub variables: DepylerValue,
}
impl Evaluator {
    pub fn new(variables: Option<std::collections::HashMap<String, f64>>) -> Self {
        Self { variables }
    }
    pub fn evaluate(&self, node: &ASTNode) -> DepylerValue {
        if node.r#type == NodeType::NUMBER {
            return node.value;
        };
        if node.r#type == NodeType::BOOLEAN {
            return node.value;
        };
        if node.r#type == NodeType::VARIABLE {
            let name = node.name;
            ...
            if self.BUILTIN_CONSTANTS.clone().contains_key(&name) {
                return self
                    .BUILTIN_CONSTANTS
                    .clone()
                    .get(&name)
            ...
            panic!(
                "Exception: {}",
                ValueError::new(format!("Unknown variable: {}", name))
            );
        };
        if node.r#type == NodeType::UNARY_OP {
            let operand = self.evaluate(node.right);
            if node.operator == "-".to_string() {
                return -operand;
            };
            if node.operator == "not".to_string() {
                return !operand;
            };
            panic!(
                "Exception: {}",
                ValueError::new(format!("Unknown unary operator: {}", node.operator))
            );
        };
        if node.r#type == NodeType::BINARY_OP {
            ...
            if node.operator == "/".to_string() {
                if right == 0 {
                    panic!(
                        "Exception: {}",
                        ValueError::new("Division by zero".to_string())
                    );
                };
                return left / right;
            };
            ...
            if node.operator == "^".to_string() {
                return {
                    if right >= 0 && (right as i64) <= (u32::MAX as i64) {
                        ({ left } as i32)
                            .checked_pow({ right } as u32)
                            .expect("Power operation overflowed")
                    } else {
                        ({ left } as f64).powf({ right } as f64) as i32
                    }
                };
            };
            if node.operator == "==".to_string() {
            ...
            if node.operator == "and".to_string() {
                return left && right;
            };
            if node.operator == "or".to_string() {
                return left || right;
            };
            panic!(
                "Exception: {}",
                ValueError::new(format!("Unknown operator: {}", node.operator))
            );
        };
        if node.r#type == NodeType::FUNCTION_CALL {
            ...
            let args = node
                ...
                .collect::<Vec<_>>();
            if self.BUILTIN_FUNCTIONS.clone().contains_key(&name) {
                return (self
                    .BUILTIN_FUNCTIONS
                    .clone()
                    .get(&name)
                    .cloned()
                    .unwrap_or_default())(args);
            };
            panic!(
                "Exception: {}",
                ValueError::new(format!("Unknown function: {}", name))
            );
        };
        panic!(
            "Exception: {}",
            ValueError::new(format!("Unknown node type: {}", node.r#type))
        );
    }
}
#[derive(Default)]
...
struct Args {
    ...
    #[doc = "Variable assignment(name=value)"]
    var: Vec<String>,
...
#[doc = "Tokenize expression string."]
...
#[doc = "Evaluate expression string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn evaluate<'a, 'b>(
    expr: &'a str,
    variables: &'b Option<std::collections::HashMap<String, f64>>,
) -> BoolOrFloatUnion {
...
#[doc = "Format result for display."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_result(value: &BoolOrFloatUnion) -> String {
    if true {
        return (value).to_string().to_lowercase();
    }
    let _cse_temp_0 = value.parse::<i32>().unwrap_or_default();
    let _cse_temp_1 = (*value) == _cse_temp_0;
...
pub fn main() {
    ...
    if args.var.is_some() {
        for v in args.var {
            let _split_parts = v
                ...
                .map(|s| s.to_string())
                .collect::<Vec<String>>();
            let mut name = _split_parts.get(0).cloned().unwrap_or_default();
            let value = _split_parts.get(1).cloned().unwrap_or_default();
            variables.as_mut().unwrap().insert(
                name.trim().to_string(),
                value.trim().to_string().parse::<f64>().unwrap(),
            );
        }
    }
    if args.interactive {
        ...
        loop {
            ...
            match (|| -> Result<(), Box<dyn std::error::Error>> {
                    ...
                    .any(|x| x))
                {
                    ...
                    if {
                        ...
                        variables
                            .as_mut()
                            .unwrap()
                            .insert(name.clone(), evaluate(&expr, variables));
                        println!(
                            "{}",
                            format!(
                                "{} = {}",
                ...
                result = Some(evaluate(&line, variables));
    ...
    match (|| -> Result<i32, Box<dyn std::error::Error>> {
        result = Some(evaluate(&expr, variables));
        println!("{}", format_result(result));
        return;
    })() {
        Ok(_result) => {
