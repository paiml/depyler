...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Upper { .. }));
    match &args.command {
        Some(Commands::Upper { ref text, .. }) => {
            println!("{}", text.to_uppercase());
        }
        Some(Commands::Lower { ref text, .. }) => {
            println!("{}", text.to_lowercase());
        }
        Some(Commands::Title { ref text, .. }) => {
            println!(
                "{}",
