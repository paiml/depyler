...
#![allow(dead_code)]
use std::f64 as math;
const STR_TARGET: &'static str = "target";
...
#[doc = "Mean squared error loss."]
#[doc = " Depyler: proven to terminate"]
pub fn mse_loss<'c, 'a, 'b>(
    pred: &'a Vec<f64>,
    target: &'b Vec<f64>,
    reduction: &'c str,
) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = pred.len() as i32;
    let _cse_temp_1 = target.len() as i32;
    let _cse_temp_2 = _cse_temp_0 != _cse_temp_1;
    if _cse_temp_2 {
        return Err(Box::new(ValueError::new("Shape mismatch".to_string())));
    }
    let squared_errors = (0..(pred.len() as i32))
        .into_iter()
        .map(|i| {
        ...
        .collect::<Vec<_>>();
    let _cse_temp_3 = (*reduction) == "sum";
    if _cse_temp_3 {
        return Ok(squared_errors.iter().sum::<f64>());
    } else {
        let _cse_temp_4 = (*reduction) == "mean";
        if _cse_temp_4 {
            return Ok(((squared_errors.iter().sum::<f64>()) as f64)
                / ((squared_errors.len() as i32) as f64));
        } else {
...
#[doc = "Gradient of MSE loss with respect to pred."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn mse_backward<'b, 'a>(pred: &'a Vec<f64>, target: &'b Vec<f64>) -> Vec<f64> {
    let _cse_temp_0 = pred.len() as i32;
    let n = _cse_temp_0;
    (0..(n))
        .into_iter()
...
#[doc = "L1 loss(mean absolute error)."]
#[doc = " Depyler: proven to terminate"]
pub fn l1_loss<'b, 'c, 'a>(
    pred: &'a Vec<f64>,
    target: &'b Vec<f64>,
    reduction: &'c str,
) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    let abs_errors = (0..(pred.len() as i32))
        .into_iter()
        .map(|i| {
            (pred
                .get(i as usize)
    ...
    let _cse_temp_3 = (*reduction) == "sum";
    if _cse_temp_3 {
        return Ok(abs_errors.iter().sum::<f64>());
    } else {
        let _cse_temp_4 = (*reduction) == "mean";
        if _cse_temp_4 {
            return Ok(
                ((abs_errors.iter().sum::<f64>()) as f64) / ((abs_errors.len() as i32) as f64)
            );
        } else {
            return Err(Box::new(ValueError::new(format!(
                "Unknown reduction: {}",
...
#[doc = "Cross-entropy loss with softmax."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cross_entropy_loss(logits: &Vec<f64>, target: i32) -> f64 {
    let _cse_temp_0 = *logits.iter().max().unwrap();
    ...
    let exp_logits = logits
        ...
        .collect::<Vec<_>>();
    let _cse_temp_1 = exp_logits.iter().sum::<f64>();
    let sum_exp = _cse_temp_1;
    let softmax = exp_logits
        .iter()
        .cloned()
        .map(|e| ((e) as f64) / ((sum_exp) as f64))
        .collect::<Vec<_>>();
    -(softmax
        .get(target as usize)
        .cloned()
...
pub fn cmd_forward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = data.get("pred").is_none();
    let _cse_temp_1 = data.get(STR_TARGET).is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'pred' or 'target'");
        std::process::exit(1);
    }
    let reduction = data.get("reduction").cloned().unwrap_or("mean");
    let mut loss: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        loss = mse_loss(
            data.get("pred").cloned().unwrap_or_default(),
            data.get("target").cloned().unwrap_or_default(),
            &reduction,
        )?;
        println!(
            "{}",
            format!("{:?}", {
                ...
                map.insert("loss".to_string().to_string(), loss);
                map
            })
        );
        Ok(())
...
pub fn cmd_backward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("grad".to_string().to_string(), grad);
            map
        })
    );
    Ok(())
}
#[doc = "Handle l1-forward subcommand."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_l1_forward() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
...
#[doc = "Handle cross-entropy subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_cross_entropy() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let _cse_temp_0 = data.get("logits").is_none();
    let _cse_temp_1 = data.get(STR_TARGET).is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'logits' or 'target'");
        std::process::exit(1);
    }
    let loss = cross_entropy_loss(
        data.get("logits").cloned().unwrap_or_default(),
        data.get("target").cloned().unwrap_or_default(),
    );
    println!(
        "{}",
