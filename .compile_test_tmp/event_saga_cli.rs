...
impl SagaState {
    ...
    pub const RUNNING: i32 = auto();
    pub const COMPLETED: i32 = auto();
    pub const COMPENSATING: i32 = auto();
    pub const COMPENSATED: i32 = auto();
    pub const FAILED: i32 = auto();
    pub fn new() -> Self {
        Self {}
    }
}
...
impl StepResult {
    pub fn new(success: bool, data: DepylerValue, error: Option<String>) -> Self {
        Self {
            success,
            data,
            error,
...
pub struct SagaStep {
    pub name: String,
    pub action: impl Fn() -> StepResult,
    pub compensate: impl Fn() -> StepResult,
    pub state: SagaState,
...
impl SagaStep {
    pub fn new(
        name: String,
        action: impl Fn() -> StepResult,
        compensate: impl Fn() -> StepResult,
        state: SagaState,
...
pub struct Saga {
    pub name: String,
    pub _steps: Vec<SagaStep>,
    pub _state: DepylerValue,
    pub _completed_steps: Vec<SagaStep>,
}
impl Saga {
    ...
    pub fn add_step(
        ...
        self._steps.push(SagaStep::new(name, action, compensate));
        return self;
    }
    pub fn execute(&mut self) -> bool {
        self._state = SagaState::RUNNING;
        self._completed_steps.clear();
        for step in self._steps.clone() {
            step.state = SagaState::RUNNING;
            step.result = step.action();
            if step.result.success {
                step.state = SagaState::COMPLETED;
                self._completed_steps.push(step);
            } else {
    ...
    pub fn _compensate(&mut self) {
        self._state = SagaState::COMPENSATING;
        for step in self._completed_steps.clone().iter().cloned().rev() {
    ...
    pub fn step_states(&self) -> std::collections::HashMap<String, SagaState> {
        return self
            ._steps
            .clone()
            .into_iter()
...
pub struct SagaBuilder {
    pub _saga: DepylerValue,
}
impl SagaBuilder {
    pub fn new(_name: String) -> Self {
        Self {
            _saga: Default::default(),
        }
    }
    pub fn step(
        &self,
        name: String,
        action: impl Fn() -> StepResult,
        compensate: Option<impl Fn() -> StepResult>,
    ) -> SagaBuilder {
        let comp = compensate || move || StepResult::new(true);
        self._saga.clone().add_step(name, action, comp);
        return self;
    }
    pub fn build(&self) -> Saga {
        return self._saga.clone();
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct OrderContext {
    ...
    pub payment_charged: bool,
    pub shipping_scheduled: bool,
}
impl OrderContext {
    pub fn new(
        order_id: String,
...
impl SagaEvent {
    pub fn new(saga_id: String, step_name: String, event_type: String, data: DepylerValue) -> Self {
        Self {
            saga_id,
            step_name,
            event_type,
...
pub struct ChoreographySaga {
    pub saga_id: String,
    pub _handlers: std::collections::HashMap<String, impl Fn(SagaEvent) -> Option<SagaEvent>>,
    pub _events: Vec<SagaEvent>,
    pub _compensations: Vec<impl Fn()>,
}
impl ChoreographySaga {
    pub fn new(saga_id: String) -> Self {
        Self {
            saga_id,
            _handlers: std::collections::HashMap::new(),
            _events: Vec::new(),
            _compensations: Vec::new(),
        }
    }
    pub fn on(
        &mut self,
        event_type: String,
        handler: impl Fn(SagaEvent) -> Option<SagaEvent>,
        compensation: Option<impl Fn()>,
    ) -> ChoreographySaga {
        self._handlers.clone().insert(event_type, handler);
        if compensation {
            self._compensations.push(compensation);
        };
        return self;
    }
    pub fn emit(&mut self, event: &SagaEvent) {
        self._events.push(event);
        if self._handlers.clone().contains_key(&event.event_type) {
            let next_event = ({
                let _base = &self._handlers.clone();
                let _idx = event.event_type;
                if _idx < 0 {
    ...
    pub fn compensate(&self) {
        for comp in self._compensations.clone().iter().cloned().rev() {
            comp();
        }
    }
    pub fn events(&self) -> Vec<SagaEvent> {
        return self._events.clone().into_iter().collect::<Vec<_>>();
    }
}
...
#[doc = "Create an order processing saga."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn create_order_saga(ctx: &OrderContext) -> Saga {
    let reserve_inventory = move || -> StepResult {
        ctx.inventory_reserved = true;
        return StepResult::new(true, "Inventory reserved".to_string().to_string(), None);
    };
    let release_inventory = move || -> StepResult {
        ctx.inventory_reserved = false;
        return StepResult::new(true, "Inventory released".to_string().to_string(), None);
    };
    let charge_payment = move || -> StepResult {
        if ctx.amount > 1000 {
            return StepResult::new(false, "Payment declined".to_string().to_string(), None);
        }
        ctx.payment_charged = true;
        return StepResult::new(true, "Payment charged".to_string().to_string(), None);
    };
    let refund_payment = move || -> StepResult {
        ctx.payment_charged = false;
        return StepResult::new(true, "Payment refunded".to_string().to_string(), None);
    };
    let schedule_shipping = move || -> StepResult {
        ctx.shipping_scheduled = true;
        return StepResult::new(true, "Shipping scheduled".to_string().to_string(), None);
    };
    let cancel_shipping = move || -> StepResult {
        ctx.shipping_scheduled = false;
        return StepResult::new(true, "Shipping cancelled".to_string().to_string(), None);
    };
    SagaBuilder::new(format!("order-{}", ctx.order_id))
        .step(
            "reserve_inventory".to_string(),
            reserve_inventory,
            release_inventory,
        )
        .step("charge_payment".to_string(), charge_payment, refund_payment)
        .step(
            "schedule_shipping".to_string(),
            schedule_shipping,
            cancel_shipping,
        )
        .build()
}
#[doc = "Simulate saga operations."]
pub fn simulate_saga(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for op in operations.iter().cloned() {
        ...
        if cmd == "order_success" {
            ctx = OrderContext::new("1".to_string().to_string(), 100.0, false, false, false);
            saga = create_order_saga(&ctx);
            success = saga.execute();
            results.push(format!("success={:?} state={}", success, saga.state().name));
        } else {
            if cmd == "order_fail" {
                ctx = OrderContext::new("2".to_string().to_string(), 2000.0, false, false, false);
                saga = create_order_saga(&ctx);
                success = saga.execute();
                results.push(format!(
                    "success={:?} state={} inventory={} payment={}",
                    success,
                    saga.state().name,
                    ctx.inventory_reserved,
                    ctx.payment_charged
                ));
            } else {
                if cmd == "builder" {
                    saga = SagaBuilder::new("test".to_string().to_string())
                        .step("step1".to_string(), move || {
                            StepResult::new(true, None, None)
                        })
                        .step("step2".to_string(), move || {
                            StepResult::new(true, None, None)
                        })
                        .build();
                    saga.execute();
                    results.push(format!("state={}", saga.state().name));
                }
            }
        }
    }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: event_saga_cli.py <command>");
    ...
    if _cse_temp_2 {
        let ctx1 = OrderContext::new(
            "ORD-001".to_string().to_string(),
            99.99,
            false,
            false,
            false,
        );
        let saga1 = create_order_saga(&ctx1);
        println!("{}", "Order 1(amount=$99.99):");
        let mut success = saga1.execute();
        println!("{}", format!("  Success: {:?}", success));
        println!("{}", format!("  State: {}", saga1.state().name));
        println!(
            "{}",
            format!(
                "  Context: inventory={}, payment={}",
                ctx1.inventory_reserved, ctx1.payment_charged
            )
        );
        println!();
        let ctx2 = OrderContext::new(
            "ORD-002".to_string().to_string(),
            1500.0,
            false,
            false,
            false,
        );
        let saga2 = create_order_saga(&ctx2);
        println!("{}", "Order 2(amount=$1500, will fail):");
        success = saga2.execute();
        println!("{}", format!("  Success: {:?}", success));
        println!("{}", format!("  State: {}", saga2.state().name));
        println!(
            "{}",
            format!(
                "  Context: inventory={}, payment={}",
                ctx2.inventory_reserved, ctx2.payment_charged
            )
        );
        println!("{}", format!("  Step states: {}", saga2.step_states()));
    } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_saga_examples() {
        assert_eq!(simulate_saga(vec![]), vec![]);
        assert_eq!(simulate_saga(vec![1]), vec![1]);
    }
}
