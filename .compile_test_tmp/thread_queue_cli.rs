...
const STR_SIZE: &'static str = "size";
#[derive(Debug, Clone)]
    ...
    pub fn put(&self, item: T) -> bool {
        {
            let _context = self._lock.clone();
            {
                if self._maxsize.clone() > 0
                    && (self._items.clone().len() as i32) >= self._maxsize.clone()
                {
                    return false;
                };
    ...
    pub fn clear(&self) -> i32 {
        {
            let _context = self._lock.clone();
            {
                let count = self._items.clone().len() as i32;
                self._items.clear();
                return count;
            }
        }
...
impl<T> PriorityQueue<T> {
    ...
    pub fn put(&self, priority: i32, item: T) {
        {
            let _context = self._lock.clone();
            {
                self._items.push((priority, item));
                self._items.clone().sort();
            }
        }
    }
    pub fn get(&self) -> Option<T> {
        {
            let _context = self._lock.clone();
            {
                if self._items.clone() {
                    return {
                        let _base = &self._items.remove(0 as usize);
                        let _idx = 1;
                        if _idx < 0 {
                            _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                        } else {
...
#[doc = "Simulate FIFO queue operations."]
pub fn simulate_fifo(
    maxsize: i32,
    ops: &Vec<String>,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let queue: ThreadSafeQueue<i32> = ThreadSafeQueue::new(maxsize);
...
#[doc = "Simulate LIFO queue operations."]
pub fn simulate_lifo(
    maxsize: i32,
    ops: &Vec<String>,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let stack: LifoQueue<i32> = LifoQueue::new(maxsize);
    let mut results: Vec<i32> = vec![];
    for op in ops.iter().cloned() {
        let mut val: i32;
        if op.starts_with("put:") {
...
#[doc = "Simulate priority queue operations."]
pub fn simulate_priority(ops: &Vec<String>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
...
#[doc = "Simulate deque operations."]
pub fn simulate_deque(
    maxsize: i32,
    ops: &Vec<String>,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let deque: std::collections::VecDeque<i32> = Deque::new(maxsize);
    let mut results: Vec<i32> = vec![];
    for op in ops.iter().cloned() {
        let mut val: i32;
        if op.starts_with("pf:") {
            val = (op
                .split(":")
                .map(|s| s.to_string())
                .collect::<Vec<String>>()
...
#[doc = "Simulate producer-consumer pattern."]
#[doc = " Depyler: verified panic-free"]
pub fn producer_consumer_sim(items: &Vec<i32>, batch_size: i32) -> Vec<i32> {
    ...
    while !queue.empty() {
        ...
        for __sanitized in 0..(batch_size) {
            let val = queue.get();
            if !val.is_null() {
                batch.push(val);
            }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Fifo { .. }));
    match &args.command {
        Some(Commands::Fifo {
            ref maxsize,
            ref ops,
            ...
            let mut results = simulate_fifo(maxsize, &ops)?;
        ...
        Some(Commands::Lifo {
            ...
            let maxsize = maxsize.clone();
            let mut results = simulate_lifo(maxsize, &ops)?;
            println!("{}", format!("Results: {:?}", results));
        }
        Some(Commands::Priority { ref ops, .. }) => {
            let mut results = simulate_priority(&ops)?;
            println!("{}", format!("Results: {:?}", results));
        }
        Some(Commands::Deque {
            ref maxsize,
            ...
            let maxsize = maxsize.clone();
            let mut results = simulate_deque(maxsize, &ops)?;
            println!("{}", format!("Results: {:?}", results));
        }
...
mod tests {
    ...
    #[test]
    fn test_simulate_priority_examples() {
        assert_eq!(simulate_priority(vec![]), vec![]);
        assert_eq!(simulate_priority(vec![1]), vec![1]);
    }
}
