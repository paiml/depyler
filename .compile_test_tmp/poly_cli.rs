...
impl Polynomial {
    ...
    pub fn degree(&self) -> i32 {
        return (self.coeffs.clone().len() as i32).saturating_sub(1);
    }
    pub fn to_string(&self) -> String {
        if self.coeffs.clone().is_empty() || self.coeffs.clone().iter().map(|c| c == 0).all(|x| x) {
            return "0".to_string();
        };
        let mut terms = vec![];
        for (i, c) in self
...
struct Args {
    ...
    #[doc = "Polynomial coefficients(comma-separated)"]
    p: Option<String>,
    
    #[doc = "Second polynomial"]
    q: Option<String>,
    
    #[doc = "Value to evaluate at"]
    x: Option<f64>,
    
    #[doc = "Points for interpolation(x1:y1,x2:y2,...)"]
...
pub fn add<'a, 'b>(
    p1: &'a Polynomial,
    p2: &'b Polynomial,
) -> Result<Polynomial, Box<dyn std::error::Error>> {
    let _cse_temp_0 = p1.coeffs.len() as i32;
    let _cse_temp_1 = depyler_max((_cse_temp_0).clone(), (_cse_temp_0).clone());
    let max_len = _cse_temp_1;
    let _cse_temp_2 = vec![0.0; max_len as usize];
...
pub fn multiply<'b, 'a>(
    ...
    let _cse_temp_0 = (!p1.coeffs) || (!p2.coeffs);
    if _cse_temp_0 {
        return Ok(Polynomial::new(vec![0]));
    }
    let _cse_temp_1 = p1.coeffs.len() as i32;
    let _cse_temp_2 = _cse_temp_1 + _cse_temp_1 - 1;
    let _cse_temp_3 = vec![0.0; _cse_temp_2 as usize];
...
#[doc = "Multiply polynomial by scalar."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn scalar_multiply(p: &Polynomial, scalar: f64) -> Polynomial {
    Polynomial::new(p.coeffs.into_iter().map(|c| c * scalar).collect::<Vec<_>>())
}
#[doc = "Evaluate polynomial at x using Horner's method."]
#[doc = " Depyler: proven to terminate"]
pub fn evaluate(p: &Polynomial, x: f64) -> Result<f64, Box<dyn std::error::Error>> {
    ...
    for i in {
        ...
        (-1..(p.coeffs.len() as i32).saturating_sub(2))
            .rev()
            .step_by(step.max(1))
    } {
        result = result * x
...
#[doc = "Calculate derivative of polynomial."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn derivative(p: &Polynomial) -> Polynomial {
    let _cse_temp_0 = p.coeffs.len() as i32;
    let _cse_temp_1 = _cse_temp_0 <= 1;
    if _cse_temp_1 {
        return Polynomial::new(vec![0]);
    }
    let result = ((1)..(p.coeffs.len() as i32))
        .into_iter()
        .map(|i| {
            i * p
                .coeffs
...
#[doc = "Calculate indefinite integral of polynomial."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn integrate(p: &Polynomial, constant: f64) -> Polynomial {
    let mut result = vec![constant];
    for (i, c) in p
        .coeffs
        .iter()
        .cloned()
...
#[doc = "Calculate definite integral from a to b."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn definite_integral(
    ...
    let integral = integrate(&p, 0i32);
    Ok(evaluate(&integral, b)? - evaluate(&integral, a)?)
}
#[doc = "Raise polynomial to power n."]
#[doc = " Depyler: proven to terminate"]
pub fn power(p: &Polynomial, n: i32) -> Result<Polynomial, Box<dyn std::error::Error>> {
    ...
    let _cse_temp_1 = n == 0;
    if _cse_temp_1 {
        return Ok(Polynomial::new(vec![1]));
    }
    result = Polynomial::new(p.coeffs.clone());
    for __sanitized in 0..(n - 1) {
        result = multiply(&result, &p)?;
    }
    Ok(result)
}
#[doc = "Polynomial long division. Returns(quotient, remainder)."]
#[doc = " Depyler: proven to terminate"]
pub fn divide<'a, 'b>(
    dividend: &'a Polynomial,
    divisor: &'b Polynomial,
) -> Result<(Polynomial, Polynomial), Box<dyn std::error::Error>> {
    if divisor.coeffs.into_iter().map(|c| c == 0).all(|x| x) {
        return Err(Box::new(ValueError::new(
            "Division by zero polynomial".to_string(),
        )));
    }
    let _cse_temp_0 = dividend.degree() < divisor.degree();
    if _cse_temp_0 {
        return Ok((Polynomial::new(vec![0]), dividend));
    }
    let _cse_temp_1 = dividend.degree() - divisor.degree();
    let _cse_temp_2 = vec![0.0; (_cse_temp_1 + 1) as usize];
    let mut quotient = _cse_temp_2.clone();
    let mut remainder = dividend.coeffs.clone();
    let divisor_lead = {
        let base = &divisor.coeffs;
        base.get(base.len().saturating_sub(1usize))
            .cloned()
            .unwrap_or_default()
    };
    for i in {
        ...
        (-1..(quotient.len() as i32).saturating_sub(1))
            .rev()
            .step_by(step.max(1))
    } {
        if remainder.len() as i32 > divisor.degree() + i {
            let coeff = {
                let base = &remainder;
                let idx: i32 = divisor.degree() + i;
                let actual_idx = if idx < 0 {
                    base.len().saturating_sub(idx.abs() as usize)
                } else {
                    idx as usize
            ...
            quotient.insert((i) as usize, coeff);
            for (j, c) in divisor
                .coeffs
                .iter()
                .cloned()
...
#[doc = "Calculate GCD of two polynomials using Euclidean algorithm."]
pub fn gcd(
    mut p1: Polynomial,
    p2: &mut Polynomial,
) -> Result<Polynomial, Box<dyn std::error::Error>> {
    while p2.coeffs.into_iter().map(|c| c != 0).any(|x| x) {
        let (__sanitized, remainder) = divide(&p1, &p2)?;
        p1 = p2;
        p2 = remainder;
    }
    let _cse_temp_0 = {
    ...
    let _cse_temp_1 = (p1.coeffs) && (_cse_temp_0);
    if _cse_temp_1 {
        let lead = {
            let base = &p1.coeffs;
            base.get(base.len().saturating_sub(1usize))
        ...
        return Ok(Polynomial::new(
            p1.coeffs.into_iter().map(|c| c / lead).collect::<Vec<_>>(),
        ));
    }
    Ok(p1)
}
#[doc = "Compose polynomials: outer(inner(x))."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compose<'a, 'b>(
    outer: &'a Polynomial,
    ...
    {
        let i = i as i32;
        let term = scalar_multiply(power(&inner, i), c);
        result = add(&result, &term)?;
    }
    Ok(result)
}
#[doc = "Find roots of quadratic polynomial."]
#[doc = " Depyler: proven to terminate"]
pub fn find_roots_quadratic(p: &Polynomial) -> Result<Vec<complex>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = p.degree() != 2;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new(
            "Polynomial must be quadratic".to_string(),
        )));
    }
    let (c, b, a) = (
        p.coeffs
    ...
    let _cse_temp_1 = b * b;
    let _cse_temp_2 = 4 * a;
    let _cse_temp_3 = _cse_temp_2 * c;
    let discriminant = _cse_temp_1 - _cse_temp_3;
    let _cse_temp_4 = discriminant >= 0;
    let mut sqrt_d;
    if _cse_temp_4 {
        let _cse_temp_5 = ({ discriminant } as f64).powf({ 0.5 } as f64);
        sqrt_d = _cse_temp_5;
        return Ok(vec![
            ((-b + sqrt_d) as f64) / ((2 * a) as f64),
            ((-b - sqrt_d) as f64) / ((2 * a) as f64),
        ]);
    } else {
        let _cse_temp_6 = ({ -discriminant } as f64).powf({ 0.5 } as f64);
        sqrt_d = _cse_temp_6;
        return Ok(vec![
            complex(-b / (2 * a), ((sqrt_d) as f64) / ((2 * a) as f64)),
            complex(-b / (2 * a), -sqrt_d / (2 * a)),
        ]);
    }
}
#[doc = "Find a root using Newton's method."]
#[doc = " Depyler: proven to terminate"]
pub fn find_root_newton(
    ...
    let dp = derivative(&p);
    let mut x = x0.clone();
    for __sanitized in 0..(max_iter) {
        let fx = evaluate(&p, x)?;
        if ((fx).abs() as f64) < tol {
            return Ok(x);
        }
        let dfx = evaluate(&dp, x)?;
        if ((dfx).abs() as f64) < 0.000000000000001 {
            return Err(Box::new(ValueError::new(
                "Derivative too small".to_string(),
            )));
        }
        x = x - ((fx) as f64) / ((dfx) as f64);
    }
    return Err(Box::new(ValueError::new("Did not converge".to_string())));
}
#[doc = "Lagrange polynomial interpolation."]
#[doc = " Depyler: proven to terminate"]
pub fn interpolate_lagrange(
    points: &Vec<(f64, f64)>,
) -> Result<Polynomial, Box<dyn std::error::Error>> {
    let mut result: DepylerValue = Default::default();
    let _cse_temp_0 = points.len() as i32;
    let n = _cse_temp_0;
    result = Polynomial::new(vec![0]);
    for i in 0..(n) {
        let (xi, yi) = points
        ...
        for j in 0..(n) {
            if i != j {
                ...
                let factor = Polynomial::new(vec![
                    ((-xj) as f64) / ((xi - xj) as f64),
                    ((1) as f64) / ((xi - xj) as f64),
                ]);
...
#[doc = "Create polynomial from its roots."]
#[doc = " Depyler: verified panic-free"]
pub fn from_roots(roots: &Vec<f64>) -> Result<Polynomial, Box<dyn std::error::Error>> {
    let mut result: DepylerValue = Default::default();
    result = Polynomial::new(vec![1]);
    for r in roots.iter().cloned() {
        let factor = Polynomial::new(vec![-r, 1]);
        result = multiply(&result, &factor)?;
    }
    Ok(result)
}
#[doc = "Parse polynomial from string like '1,2,3' -> 1 + 2x + 3x^2."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_polynomial(s: &str) -> Polynomial {
    let coeffs = s
        .split(",")
        .map(|s| s.to_string())
        .collect::<Vec<String>>()
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_p = args.p.is_some();
    let has_q = args.q.is_some();
    let has_x = args.x.is_some();
    let has_points = args.points.is_some();
    let _cse_temp_0 = args.mode == "demo";
    let mut p;
    let mut q;
    if _cse_temp_0 {
        println!("{}", "Polynomial Operations Demo\n");
        p = Polynomial::new(vec![1, 2, 3]);
        q = Polynomial::new(vec![2, 1]);
        println!("{}", format!("P(x) = {}", p));
        println!("{}", format!("Q(x) = {}", q));
        println!("{}", format!("\nP(2) = {:?}", evaluate(&p, 2f64)));
        println!("{}", format!("P + Q = {:?}", add(&p, &q)));
        println!("{}", format!("P * Q = {:?}", multiply(&p, &q)));
        println!("{}", format!("P' = {}", derivative(&p)));
        println!("{}", format!("∫P dx = {}", integrate(&p, 0i32)));
        let quadratic = Polynomial::new(vec![6, -5, 1]);
        println!(
            "{}",
            format!(
                "\nRoots of {}: {:?}",
                quadratic,
                find_roots_quadratic(&quadratic)
            )
        );
    } else {
        let mut result;
        if has_p {
            ...
            let _cse_temp_1 = args.mode == "eval";
            let _cse_temp_2 = (_cse_temp_1) && (has_x);
            if _cse_temp_2 {
                ...
                let _cse_temp_4 = (_cse_temp_3) && (args.q);
                if _cse_temp_4 {
                    q = parse_polynomial(&args.q);
                    result = add(&p, &q)?;
                    println!("{}", format!("P + Q = {}", result));
                } else {
                    let _cse_temp_5 = args.mode == "mult";
                    let _cse_temp_6 = (_cse_temp_5) && (args.q);
                    if _cse_temp_6 {
                        q = parse_polynomial(&args.q);
                        result = multiply(&p, &q)?;
                        println!("{}", format!("P * Q = {}", result));
                    } else {
                        let _cse_temp_7 = args.mode == "deriv";
                        if _cse_temp_7 {
                            result = derivative(&p);
                            println!("{}", format!("P' = {}", result));
                        } else {
                            let _cse_temp_8 = args.mode == "integ";
                            if _cse_temp_8 {
                                result = integrate(&p, 0i32);
                                println!("{}", format!("∫P dx = {}", result));
                            } else {
                                let _cse_temp_9 = args.mode == "roots";
                                let _cse_temp_10 = p.degree() == 2;
                                let _cse_temp_11 = (_cse_temp_9) && (_cse_temp_10);
                                if _cse_temp_11 {
                                    let roots = find_roots_quadratic(&p)?;
                                    println!("{}", format!("Roots: {:?}", roots));
                                } else {
                                    let _cse_temp_12 = args.mode == "divide";
                                    let _cse_temp_13 = (_cse_temp_12) && (args.q);
                                    if _cse_temp_13 {
                                        q = parse_polynomial(&args.q);
                                        let (quot, rem) = divide(&p, &q)?;
                                        println!("{}", format!("P / Q = {:?}", quot));
                                        println!("{}", format!("Remainder = {:?}", rem));
                                    }
                                }
                            }
                        }
            ...
            let _cse_temp_14 = args.mode == "interpolate";
            let _cse_temp_15 = (_cse_temp_14) && (args.points);
            if _cse_temp_15 {
                let mut points = vec![];
                for pt in args
                    .points
                    ...
                    .collect::<Vec<String>>()
                {
                    let (x, y) = pt
                        ...
                        .map(|s| s.to_string())
                        .collect::<Vec<String>>();
                    points.push(DepylerValue::Str(format!("{:?}", ((x) as f64, (y) as f64))));
                }
                result = interpolate_lagrange(&points)?;
                println!("{}", format!("Interpolating polynomial: {}", result));
            }
        }
    }
    Ok(())
