...
enum Commands {
    #[default]
    __DepylerNone,
    Minmax4 { a: f64, b: f64, c: f64, d: f64 },
    Minmax2 { a: f64, b: f64 },
    Minmax3 { a: f64, b: f64, c: f64 },
}
#[derive(Default)]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Minmax3 { .. }));
    match &args.command {
        Some(Commands::Minmax3 {
            ref a,
            ref b,
            ref c,
            ..
            ...
            let b = *b;
            let c = *c;
            let mut arr = Vector::from_slice(&[a as f32, b as f32, c as f32]);
            let mut min_val = arr.min().unwrap();
            let mut max_val = arr.max().unwrap();
            let mut denom = max_val - min_val;
            let mut result = if denom > 0f64 {
                Vector::from_vec(
                    Vector::from_vec(arr.as_slice().iter().map(|&x| x - min_val).collect())
                        .as_slice()
                        .iter()
                        .map(|&x| x / denom)
                        .collect(),
            ...
            println!(
                ...
                result
                    ...
                    .iter()
                    .cloned()
                    .map(|x| ((x as f64).round() as i32).to_string())
                    .collect::<Vec<_>>()
                    .join(" ")
            );
        }
        Some(Commands::Minmax4 {
            ref a,
            ref b,
            ref c,
            ref d,
            ...
            let c = *c;
            let d = *d;
            let mut arr = Vector::from_slice(&[a as f32, b as f32, c as f32, d as f32]);
            let mut min_val = arr.min().unwrap();
            let mut max_val = arr.max().unwrap();
            let mut denom = max_val - min_val;
            let mut result = if denom > 0f64 {
        ...
        Some(Commands::Minmax2 { ref a, ref b, .. }) => {
            let a = *a;
            let b = *b;
            let mut arr = Vector::from_slice(&[a as f32, b as f32]);
            let mut min_val = arr.min().unwrap();
            let mut max_val = arr.max().unwrap();
            let mut denom = max_val - min_val;
            let mut result = if denom > 0f64 {
