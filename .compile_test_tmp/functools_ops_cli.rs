...
pub static add_10: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| partial(add, 10).unwrap());
pub static multiply_by_2: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| partial(multiply, 2).unwrap());
pub static square: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| partial(power, 2).unwrap());
pub static cube: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| partial(power, 3).unwrap());
pub static format_dollars: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| partial(format_value, "$".to_string(), "".to_string()).unwrap());
pub static format_percent: std::sync::LazyLock<String> =
    std::sync::LazyLock::new(|| partial(format_value, "".to_string(), "%".to_string()).unwrap());
...
#[doc = "Format value with prefix and suffix."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_value(prefix: String, value: i32, suffix: String) -> String {
    format!("{}{}{}", prefix, value, suffix)
}
#[doc = "Sum using reduce."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_sum(items: &Vec<i32>) -> i32 {
    std::iter::Iterator::fold(add, items, 0)
}
#[doc = "Product using reduce."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_product(items: &Vec<i32>) -> i32 {
    std::iter::Iterator::fold(multiply, items, 1)
}
#[doc = "Max using reduce."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_max(items: &Vec<i32>) -> Option<i32> {
    if items.is_empty() {
        return None;
    }
    Some(std::iter::Iterator::fold(
        move |a, b| if a > b { a } else { b },
        items,
    ))
}
#[doc = "Min using reduce."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_min(items: &Vec<i32>) -> Option<i32> {
    if items.is_empty() {
        return None;
    }
    Some(std::iter::Iterator::fold(
...
#[doc = "Concatenate strings using reduce."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_concat(items: &Vec<String>) -> String {
    std::iter::Iterator::fold(move |a, b| a + b, items, "")
}
#[doc = "Flatten using reduce."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_flatten(lists: &Vec<Vec<i32>>) -> Vec<i32> {
    std::iter::Iterator::fold(move |a, b| a + b, lists, vec![])
}
#[doc = "Compose multiple functions."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compose(funcs: &[String]) -> Box<dyn Fn(i32) -> i32> {
    let composed = move |x: i32| -> i32 {
        let mut result = x.clone();
        for _f in funcs.iter().cloned().rev() {
            result = f(result);
        }
        return result;
    };
...
#[doc = "Pipe functions(left to right)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn pipe(funcs: &[String]) -> Box<dyn Fn(i32) -> i32> {
    let piped = move |x: i32| -> i32 {
        let mut result = x.clone();
        for _f in funcs.iter().cloned() {
            result = f(result);
        }
        return result;
    };
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_cache_info() -> HashMap<String, DepylerValue> {
    {
        let mut map = HashMap::new();
        map.insert("fibonacci".to_string(), fibonacci.cache_info()._asdict());
        map.insert("factorial".to_string(), factorial.cache_info()._asdict());
        map
    }
}
#[doc = "Create partial application."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
#[doc = "Curry a 2-argument function."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn curry_2(func: impl Fn(i32, i32) -> i32) -> Box<dyn Fn(i32) -> &dyn Fn(i32) -> i32> {
    let curried = move |a: i32| -> &dyn Fn(i32) -> i32 {
        let inner = move |b: i32| -> i32 {
            return func(a, b);
        };
        return inner;
    };
...
#[doc = "Uncurry a curried function."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn uncurry_2(_func: &dyn Fn(i32) -> &dyn Fn(i32) -> i32) -> impl Fn(i32, i32) -> i32 {
    uncurried
}
#[doc = "Map-reduce pattern."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn map_reduce<'a, 'b>(
    items: &'a Vec<i32>,
    mapper: impl Fn(i32) -> i32,
    reducer: impl Fn(i32, i32) -> i32,
    initial: i32,
) -> i32 {
    let mapped = items
        .as_slice()
        ...
        .map(|x| mapper(x))
        .collect::<Vec<_>>();
    std::iter::Iterator::fold(reducer, mapped, initial)
}
#[doc = "Filter-reduce pattern."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn filter_reduce<'a, 'b>(
    items: &'a Vec<i32>,
    ...
    let filtered = items
        ...
        .map(|x| x)
        .collect::<Vec<_>>();
    std::iter::Iterator::fold(reducer, filtered, initial)
}
#[doc = "Partition by predicate."]
#[doc = " Depyler: verified panic-free"]
pub fn partition_by(items: &Vec<i32>, predicate: impl Fn(i32) -> bool) -> (Vec<i32>, Vec<i32>) {
    let mut true_list: Vec<i32> = vec![];
    let mut false_list: Vec<i32> = vec![];
...
#[doc = "Group items by key function."]
#[doc = " Depyler: verified panic-free"]
pub fn group_by(items: &Vec<i32>, key_func: impl Fn(i32) -> i32) -> HashMap<i32, Vec<i32>> {
    let mut result: std::collections::HashMap<i32, Vec<i32>> = {
    ...
    for item in items.iter().cloned() {
        ...
        result.get(&key).cloned().unwrap_or_default().push(item);
    }
    result
}
#[doc = "Scan(cumulative reduce)."]
#[doc = " Depyler: verified panic-free"]
pub fn scan(items: &Vec<i32>, func: impl Fn(i32, i32) -> i32, initial: i32) -> Vec<i32> {
...
#[doc = "Take while predicate is true."]
#[doc = " Depyler: verified panic-free"]
pub fn take_while(items: &Vec<i32>, predicate: impl Fn(i32) -> bool) -> Vec<i32> {
    let mut result: Vec<i32> = vec![];
    for item in items.iter().cloned() {
        if !predicate(item) {
            break;
...
#[doc = "Drop while predicate is true."]
#[doc = " Depyler: verified panic-free"]
pub fn drop_while(items: &Vec<i32>, predicate: impl Fn(i32) -> bool) -> Vec<i32> {
    let mut result: Vec<i32> = vec![];
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Partial { .. }));
    match &args.command {
        Some(Commands::Partial {
            ...
            let add10 = add10.clone();
            let mul2 = mul2.clone();
            let square = square.clone();
            if add10.is_some() {
                println!("{}", format!("add_10({}) = {}", add10, add_10(add10)));
            }
            if mul2.is_some() {
                println!(
                    "{}",
                    format!("multiply_by_2({}) = {}", mul2, multiply_by_2(mul2))
                );
            }
            if square.is_some() {
                println!("{}", format!("square({}) = {}", square, square(square)));
            }
        }
        Some(Commands::Reduce {
            ...
            let _cse_temp_2 = if op.is_empty() {
                "sum".to_string()
            } else {
                op.to_string()
            };
            let op = _cse_temp_2;
            let _cse_temp_3 = op == "sum";
            if _cse_temp_3 {
                println!("{}", format!("Sum: {}", reduce_sum(&items)));
            } else {
                let _cse_temp_4 = op == "product";
                if _cse_temp_4 {
                    println!("{}", format!("Product: {}", reduce_product(&items)));
                } else {
                    let _cse_temp_5 = op == "max";
                    if _cse_temp_5 {
                        println!("{}", format!("Max: {:?}", reduce_max(&items)));
                    } else {
                        let _cse_temp_6 = op == "min";
                        if _cse_temp_6 {
                            println!("{}", format!("Min: {:?}", reduce_min(&items)));
                        }
                    }
                }
            }
        }
        Some(Commands::Cached {
            ref fact, ref fib, ..
        }) => {
            let fact = fact.clone();
            let fib = fib.clone();
            if fib.is_some() {
                println!("{}", format!("fib({}) = {}", fib, fibonacci(fib)));
            }
            if fact.is_some() {
                println!("{}", format!("fact({}) = {}", fact, factorial(fact)));
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_reduce_flatten_examples() {
        assert_eq!(reduce_flatten(vec![]), vec![]);
        assert_eq!(reduce_flatten(vec![1]), vec![1]);
    }
    #[test]
