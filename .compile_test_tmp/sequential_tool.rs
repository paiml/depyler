...
#[doc = "Linear layer forward."]
#[doc = " Depyler: proven to terminate"]
pub fn linear_forward<'b, 'a, 'c>(
    x: &'a Vec<f64>,
    weight: &'b Vec<Vec<f64>>,
    bias: &'c Vec<f64>,
) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let mut out = vec![];
    for i in 0..(weight.len() as i32) {
        let mut val = bias
            .get(i as usize)
            .cloned()
            .expect("IndexError: list index out of range");
        for j in 0..(x.len() as i32) {
            val = val
                + weight
                    .get(i as usize)
                    .cloned()
        ...
        out.push(DepylerValue::Float(val as f64));
    }
    Ok(out)
}
#[doc = "ReLU activation."]
...
#[doc = " Depyler: proven to terminate"]
pub fn relu_forward(x: &Vec<f64>) -> Vec<f64> {
    x.as_slice()
        .iter()
        .cloned()
        .map(|xi| (0.0 as f64).max(xi as f64))
        .collect::<Vec<_>>()
}
#[doc = "Sigmoid activation."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn sigmoid_forward(x: &Vec<f64>) -> Vec<f64> {
    x.as_slice()
        .iter()
        .cloned()
        .map(|xi| {
            if xi >= 0 {
                ((1.0) as f64) / ((1.0 + (-xi as f64).exp()) as f64)
            } else {
                (((xi as f64).exp()) as f64) / ((1.0 + (xi as f64).exp()) as f64)
            }
        })
        .collect::<Vec<_>>()
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn tanh_forward(x: &Vec<f64>) -> Vec<f64> {
    x.as_slice()
        .iter()
        .cloned()
        .map(|xi| (xi as f64).tanh())
        .collect::<Vec<_>>()
}
#[doc = "Forward pass through sequential layers."]
pub fn forward<'a, 'b>(
    x: &'a Vec<f64>,
    layers: &'b Vec<std::collections::HashMap<String, DepylerValue>>,
) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let mut output: Vec<f64> = Default::default();
    output = x;
    for layer in layers.iter().cloned() {
        let layer_type = layer.get("type").cloned().unwrap_or_default();
        if layer_type == "linear".to_string() {
            output = linear_forward(
                &output,
                layer.get("weight").cloned().unwrap_or_default(),
                layer.get("bias").cloned().unwrap_or_default(),
            )?;
        } else {
            if layer_type == "relu" {
...
#[doc = "Initialize linear layer."]
#[doc = " Depyler: proven to terminate"]
pub fn init_linear(
    in_features: f64,
    out_features: i32,
) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>> {
    let std = (((2.0) as f64) / ((in_features) as f64) as f64).sqrt();
    let _cse_temp_0 = (3.0 as f64).sqrt() * std;
    let bound = _cse_temp_0;
    let weight = (0..(out_features))
        ...
        .collect::<Vec<_>>();
    let _cse_temp_1 = ((1.0) as f64) / (((in_features as f64).sqrt()) as f64);
    let bias_bound = _cse_temp_1;
    let bias = (0..(out_features))
        .into_iter()
        .map(|_| -bias_bound as f64)
        .collect::<Vec<_>>();
    Ok({
        let mut map = HashMap::new();
        map.insert("type".to_string(), "linear".to_string().to_string());
        map.insert("weight".to_string(), weight);
        map.insert("bias".to_string(), bias);
        map
    })
}
#[doc = "Build sequential model from architecture spec."]
pub fn build<'b, 'a>(
    architecture: &'a Vec<std::collections::HashMap<String, DepylerValue>>,
    random_state: &'b Option<i32>,
) -> Result<Vec<HashMap<String, DepylerValue>>, Box<dyn std::error::Error>> {
    if random_state.is_some() {
        ();
    }
    let mut layers = vec![];
    for spec in architecture.iter().cloned() {
        if spec.get("type").cloned().unwrap_or_default() == "linear".to_string() {
            layers.push(DepylerValue::Str(format!(
                "{:?}",
                init_linear(
                    spec.get("in").cloned().unwrap_or_default(),
                    spec.get("out").cloned().unwrap_or_default()
                )
            )));
        } else {
            layers.push(DepylerValue::Str(format!("{:?}", {
                ...
                map.insert(
                    "type".to_string(),
                    spec.get("type").cloned().unwrap_or_default(),
                );
                map
            })));
        }
...
#[doc = "Handle forward subcommand."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_forward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_1 = data.get("layers").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'x' or 'layers'");
        std::process::exit(1);
    }
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        output = forward(
            ...
            data.get("layers").cloned().unwrap_or_default(),
        )?;
        println!(
            "{}",
            format!("{:?}", {
                let mut map = HashMap::new();
                map.insert("output".to_string().to_string(), output);
                map
            })
        );
        Ok(())
...
#[doc = "Handle build subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_build() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let _cse_temp_0 = data.get("architecture").is_none();
    if _cse_temp_0 {
        eprintln!("{}", "Error: Missing 'architecture'");
        std::process::exit(1);
    }
    let random_state = data.get("random_state").cloned().unwrap_or(None);
    let layers = build(
        data.get("architecture").cloned().unwrap_or_default(),
        &random_state,
    )?;
    println!(
        "{}",
        format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("layers".to_string(), layers);
            map
        })
    );
    Ok(())
...
mod tests {
    ...
    #[test]
    fn test_relu_forward_examples() {
        assert_eq!(relu_forward(vec![]), vec![]);
        assert_eq!(relu_forward(vec![1]), vec![1]);
    }
    #[test]
    fn test_sigmoid_forward_examples() {
        assert_eq!(sigmoid_forward(vec![]), vec![]);
        assert_eq!(sigmoid_forward(vec![1]), vec![1]);
    }
    #[test]
    fn test_tanh_forward_examples() {
        assert_eq!(tanh_forward(vec![]), vec![]);
        assert_eq!(tanh_forward(vec![1]), vec![1]);
    }
}
