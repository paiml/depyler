...
const STR__: &'static str = "_";
const STR_FOR: &'static str = "for";
...
impl TokenType {
    pub const TEXT: i32 = auto();
    pub const VAR_OPEN: i32 = auto();
    pub const VAR_CLOSE: i32 = auto();
    pub const BLOCK_OPEN: i32 = auto();
    pub const BLOCK_CLOSE: i32 = auto();
...
#[doc = "Tokenize template string."]
pub fn tokenize(template: &str) -> Result<Vec<Token>, Box<dyn std::error::Error>> {
    let mut tokens = vec![];
    let mut i = 0;
    while i < template.len() as i32 {
        if {
            let base = template;
            let start_idx: i32 = i;
        ...
        {
            tokens.push(DepylerValue::Str(format!(
                "{:?}",
                Token::new(TokenType::VAR_OPEN, "{{".to_string())
            )));
            i = i + 2;
            while (i < template.len() as i32)
                && ({
            ...
            } == "}}"
            {
                tokens.push(DepylerValue::Str(format!(
                    "{:?}",
                    Token::new(TokenType::VAR_CLOSE, "}}".to_string())
                )));
                i = i + 2;
            }
        } else {
            ...
            } == "{%"
            {
                tokens.push(DepylerValue::Str(format!(
                    "{:?}",
                    Token::new(TokenType::BLOCK_OPEN, "{%".to_string())
                )));
                i = i + 2;
                while (i < template.len() as i32)
                    && ({
                ...
                } == "%}"
                {
                    tokens.push(DepylerValue::Str(format!(
                        "{:?}",
                        Token::new(TokenType::BLOCK_CLOSE, "%}".to_string())
                    )));
                    i = i + 2;
                }
            } else {
...
#[doc = "Resolve variable from context."]
pub fn resolve_var<'b, 'a>(
    name: &'a str,
    context: &'b std::collections::HashMap<DepylerValue, DepylerValue>,
) -> Result<DepylerValue, Box<dyn std::error::Error>> {
    let mut value: std::collections::HashMap<DepylerValue, DepylerValue> = Default::default();
    let parts = name
        .split(".")
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
    ...
    for part in parts.iter().cloned() {
        if (true) && (value.get(&part).is_some()) {
            value = value.get(&part).cloned().unwrap_or_default();
        } else {
            if (true) && (part.chars().all(|c| c.is_numeric())) {
                value = value
                    .get(&DepylerValue::Int(
                        part.parse::<i32>().unwrap_or_default() as i64
                    ))
                    .cloned()
                    .unwrap_or_default();
            } else {
...
#[doc = "Apply filter to value."]
#[doc = " Depyler: proven to terminate"]
pub fn apply_filter<'a, 'b>(
    value: &'a DepylerValue,
    filter_name: &'b str,
) -> Result<String, Box<dyn std::error::Error>> {
    let _cse_temp_0 = (*filter_name) == "upper";
    if _cse_temp_0 {
        return Ok((value).to_string().to_uppercase());
    }
    let _cse_temp_1 = (*filter_name) == "lower";
    if _cse_temp_1 {
        return Ok((value).to_string().to_lowercase());
    }
    let _cse_temp_2 = (*filter_name) == "title";
    if _cse_temp_2 {
        return Ok((value)
            .to_string()
            .split_whitespace()
    ...
    let _cse_temp_3 = (*filter_name) == "strip";
    if _cse_temp_3 {
        return Ok((value).to_string().trim().to_string());
    }
    let _cse_temp_4 = (*filter_name) == "length";
    if _cse_temp_4 {
        return Ok((if hasattr(value, "__len__".to_string()) {
            value.len() as i32
        } else {
            0
        })
    ...
    let _cse_temp_5 = (*filter_name) == "reverse";
    if _cse_temp_5 {
        if true {
            return Ok({
                let base = value;
        ...
        if true {
            return Ok(value.iter().cloned().rev().into_iter().collect::<Vec<_>>());
        }
    }
    let _cse_temp_6 = (*filter_name) == "first";
    if _cse_temp_6 {
        let _cse_temp_7 = value.len() as i32;
        let _cse_temp_8 = _cse_temp_7 > 0;
        let _cse_temp_9 = (true) && (_cse_temp_8);
        if _cse_temp_9 {
            return Ok(value
                .get(0usize)
                .cloned()
    ...
    let _cse_temp_10 = (*filter_name) == "last";
    if _cse_temp_10 {
        let _cse_temp_11 = value.len() as i32;
        let _cse_temp_12 = _cse_temp_11 > 0;
        let _cse_temp_13 = (true) && (_cse_temp_12);
        if _cse_temp_13 {
            return Ok(value.get(&-1).cloned().unwrap_or_default());
        }
    }
    let _cse_temp_14 = (*filter_name) == "default";
    if _cse_temp_14 {
        return Ok(value);
    }
    let _cse_temp_15 = (*filter_name) == "join";
    if _cse_temp_15 {
        if true {
            return Ok(value
                .iter()
...
#[doc = "Render template with context."]
pub fn render<'a, 'b>(
    template: &'a str,
    context: &'b std::collections::HashMap<DepylerValue, DepylerValue>,
) -> Result<String, Box<dyn std::error::Error>> {
    ...
    while i < tokens.len() as i32 {
        let token = tokens
            ...
            .cloned()
            .expect("IndexError: list index out of range");
        if token.r#type == TokenType::TEXT {
            result.push(DepylerValue::Str(format!("{:?}", token.value)));
            i = i + 1;
        } else {
            let mut value;
            if token.r#type == TokenType::VAR_OPEN {
                i = i + 1;
                                ...
                                == TokenType::PIPE
                            {
                                ...
                                if (i < tokens.len() as i32)
                                    && (tokens
                                        ...
                                        == TokenType::IDENTIFIER)
                                {
                                    filters.push(DepylerValue::Str(format!(
                                        "{:?}",
                                        tokens
                                            .get(i as usize)
                                            .cloned()
                ...
                if token.r#type == TokenType::BLOCK_OPEN {
                    i = i + 1;
                    while (i < tokens.len() as i32)
                        && (tokens
                            .get(i as usize)
                    ...
                    {
                        let mut block_var: String = Default::default();
                        let mut block_iterable: String = Default::default();
                        let mut block_type: String = Default::default();
                        if tokens
                            .get(i as usize)
                            .cloned()
                            .expect("IndexError: list index out of range")
                            ...
                            == TokenType::IDENTIFIER
                        {
                            if block_type.is_none() {
                                ...
                                if (block_type == STR_FOR) && (block_var.is_none()) {
                                    block_var = tokens
                                        .get(i as usize)
                                        .cloned()
                                        .expect("IndexError: list index out of range")
                                            ...
                                            == "in")
                                    {
                                        ...
                                        if block_type == STR_FOR {
                                            block_iterable = tokens
                                                .get(i as usize)
                                                .cloned()
                                                .expect("IndexError: list index out of range")
                                                .value;
                                        } else {
                                            if (block_type == "if") && (block_var.is_none()) {
                                                block_var = tokens
                                                    .get(i as usize)
                                                    .cloned()
                                                    .expect("IndexError: list index out of range")
                    ...
                    let mut depth: i32;
                    let mut body_start;
                    let mut body_end: i32;
                    if ((block_type == STR_FOR) && (block_var)) && (block_iterable) {
                        depth = 1;
                        body_start = i;
                        while (i < tokens.len() as i32) && (depth > 0) {
                            if tokens
                                .get(i as usize)
                                .cloned()
                                .expect("IndexError: list index out of range")
                        ...
                        let items = resolve_var(block_iterable, &context)?;
                        if true {
                            for item in items.iter().cloned() {
                                let mut loop_context = context.clone();
                                loop_context.insert(
                                    block_var.to_string().clone(),
                                    DepylerValue::Str(format!("{:?}", item)),
                                );
                                let mut body_tokens = {
                                    let base = &tokens;
                                    let start_idx = (body_start) as isize;
                                    let stop_idx = (body_end) as isize;
                                ...
                                let mut body_template = reconstruct_template(&body_tokens)?;
                                result.push(DepylerValue::Str(format!(
                                    "{:?}",
                                    render(&body_template, &loop_context)
                                )));
                            }
                        }
                    } else {
                        if (block_type == "if") && (block_var) {
                            depth = 1;
                            body_start = i;
                            while (i < tokens.len() as i32) && (depth > 0) {
                                if tokens
                            ...
                            if ["endfor".to_string(), "endif".to_string()].contains(&block_type) {}
                        }
                    }
                } else {
                    i = i + 1;
...
#[doc = "Reconstruct template string from tokens."]
pub fn reconstruct_template(tokens: &Vec<Token>) -> Result<String, Box<dyn std::error::Error>> {
    let mut i: i32 = Default::default();
    let mut result = vec![];
    i = 0;
    while i < tokens.len() as i32 {
...
pub fn main() -> Result<(), std::io::Error> {
    let mut template: String = Default::default();
    let mut context: String = Default::default();
    let args = Args::default();
    let has_template = args.template.is_some();
    let has_file = args.file.is_some();
    let has_context = args.context.is_some();
    let has_context_file = args.context_file.is_some();
    if has_file {
    ...
    if has_context_file {
        let mut f = std::fs::File::open(args.context_file)?;
        context = std::collections::HashMap::<String, DepylerValue>::new();
    } else {
        if has_context {
            context = std::collections::HashMap::<String, DepylerValue>::new();
        } else {
