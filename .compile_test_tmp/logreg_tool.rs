...
const STR_COEF: &'static str = "coef";
const STR_INTERCEPT: &'static str = "intercept";
...
#[doc = "Sigmoid activation function."]
#[doc = " Depyler: proven to terminate"]
pub fn sigmoid(x: f64) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = x >= 0f64;
    if _cse_temp_0 {
        return Ok(((1.0) as f64) / ((1.0 + (-x as f64).exp()) as f64));
    } else {
        let exp_x = (x as f64).exp();
        return Ok(((exp_x) as f64) / ((1.0 + exp_x) as f64));
    }
}
#[doc = "Fit logistic regression using gradient descent.\n\n    Uses batch gradient descent to minimize cross-entropy loss.\n    "]
#[doc = " Depyler: proven to terminate"]
pub fn fit<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    ...
    let mut intercept: f64 = Default::default();
    ...
    if _cse_temp_5 {
        return Err(Box::new(ValueError::new(format!(
            "Dimension mismatch: X has {} samples, y has {}",
            &X.len() as i32,
            y.len() as i32
        ))));
    }
    ...
    let _cse_temp_6 = X
        ...
        .len() as i32;
    let n_features = _cse_temp_6;
    let _cse_temp_7 = vec![0.0; n_features as usize];
    let mut coef = _cse_temp_7.clone();
    intercept = 0.0;
    for __sanitized in 0..(max_iter) {
        let mut grad_coef = vec![0.0; n_features as usize];
        let mut grad_intercept = 0.0;
        for i in 0..(n_samples) {
            let mut z = intercept.clone();
            ...
            let error = pred
                - y.get(i as usize)
                    .cloned()
                    .expect("IndexError: list index out of range");
            grad_intercept = grad_intercept + error;
            for j in 0..(n_features) {
                {
                    let _key = j;
                    let _old_val = grad_coef.get(&_key).cloned().unwrap_or_default();
                    grad_coef.insert(
                        _key,
                        _old_val
                            + error
        ...
        intercept = intercept - ((learning_rate * grad_intercept) as f64) / ((n_samples) as f64);
        for j in 0..(n_features) {
            {
                let _key = j;
                let _old_val = coef.get(&_key).cloned().unwrap_or_default();
                coef.insert(
                    _key,
                    _old_val
                        - ((learning_rate
    ...
    Ok({
        ...
        map.insert(
            STR_COEF.to_string(),
            DepylerValue::Str(format!("{:?}", coef)),
        );
        map.insert(
            STR_INTERCEPT.to_string(),
            DepylerValue::Float(intercept as f64),
        );
        map
    })
}
#[doc = "Predict probabilities for class 1."]
pub fn predict_proba<'b, 'a>(
    X: &'a Vec<Vec<f64>>,
    coef: &'b Vec<f64>,
    intercept: f64,
) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let mut probabilities = vec![];
    for row in X.iter().cloned() {
        let mut z = intercept.clone();
        for (i, val) in row.iter().cloned().enumerate().map(|(i, x)| (i as i32, x)) {
            let i = i as i32;
            z = z + val
                * coef
                    .get(i as usize)
        ...
        probabilities.push(DepylerValue::Str(format!("{:?}", sigmoid(z))));
    }
    Ok(probabilities)
}
#[doc = "Predict class labels(0 or 1)."]
...
pub fn predict<'a, 'b>(
    ...
    let probs = predict_proba(&&X, &coef, intercept)?;
    Ok(probs
        .as_slice()
        .iter()
        .cloned()
        .map(|p| if (p as f64) >= 0.5 { 1 } else { 0 })
        .collect::<Vec<_>>())
}
...
pub fn score<'b, 'c, 'a>(
    ...
    let predictions = predict(&&X, &coef, intercept)?;
...
pub fn cmd_fit() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        ...
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON input: {:?}", e));
            std::process::exit(1);
        }
    };
    let _cse_temp_0 = data.get(STR_X).is_none();
    if _cse_temp_0 {
        eprintln!("{}", "Error: Missing required field 'X'");
        std::process::exit(1);
    }
    let _cse_temp_1 = data.get("y").is_none();
    if _cse_temp_1 {
        eprintln!("{}", "Error: Missing required field 'y'");
        std::process::exit(1);
    }
    let learning_rate = data.get("learning_rate").cloned().unwrap_or(0.1);
    let max_iter = data.get("max_iter").cloned().unwrap_or(1000);
...
pub fn cmd_predict() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for field in required.iter().cloned() {
        if data.get(&field).is_none() {
            eprintln!("{}", format!("Error: Missing required field '{}'", field));
            std::process::exit(1);
        }
    }
    let predictions = predict(
        data.get("X").cloned().unwrap_or_default(),
        data.get("coef").cloned().unwrap_or_default(),
        data.get("intercept").cloned().unwrap_or_default(),
    )?;
    println!(
        "{}",
        format!("{:?}", {
...
#[doc = "Handle predict-proba subcommand."]
pub fn cmd_predict_proba() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON input: {:?}", e));
    ...
    let probabilities = predict_proba(
        data.get("X").cloned().unwrap_or_default(),
        data.get("coef").cloned().unwrap_or_default(),
        data.get("intercept").cloned().unwrap_or_default(),
    )?;
    println!(
        ...
        format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("probabilities".to_string().to_string(), probabilities);
            map
        })
    );
    Ok(())
...
pub fn cmd_fit_predict() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut fit_result: Option<String> = None;
    let mut result: std::collections::HashMap<String, i32> = Default::default();
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        fit_result = fit(
        ...
        predictions = predict(
            ...
            fit_result.get("coef").cloned().unwrap_or_default(),
            fit_result.get("intercept").cloned().unwrap_or_default(),
        )?;
        result = {
            let mut map = HashMap::new();
            map.insert(
...
#[doc = "Handle fit-score subcommand."]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_fit_score() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON input: {:?}", e));
    ...
    let required = vec![STR_X.to_string(), "y".to_string()];
    for field in required.iter().cloned() {
        if data.get(&field).is_none() {
            eprintln!("{}", format!("Error: Missing required field '{}'", field));
            std::process::exit(1);
    ...
    let learning_rate = data.get("learning_rate").cloned().unwrap_or(0.1);
    let max_iter = data.get("max_iter").cloned().unwrap_or(1000);
    let mut accuracy: Option<String> = None;
    let mut result: std::collections::HashMap<String, i32> = Default::default();
    let mut fit_result: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        fit_result = fit(
...
pub fn main() {
    ...
    predict_proba_parser.set_defaults();
    score_parser.set_defaults();
    fit_predict_parser.set_defaults();
    fit_score_parser.set_defaults();
