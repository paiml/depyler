...
struct Args {
    ...
    #[doc = "Capture and display output"]
    capture: bool,
    
    
    #[doc = "Exit with error if command fails"]
    check: bool,
    
    #[doc = "Working directory for command"]
    cwd: Option<String>,
...
pub fn run_command(
    ...
    let mut result: CompletedProcess;
    if capture {
        result = {
            let cmd_list = cmd;
...
pub fn main() {
    ...
    let has_cwd = args.cwd.is_some();
    let has_version = args.version.is_some();
    let _cse_temp_0 = vec![args.command]
        .iter()
        .chain(args.args.iter())
        .cloned()
        .collect::<Vec<_>>();
    let cmd = _cse_temp_0;
    println!("{}", format!("Running: {}", cmd.join(" ").display()));
    if has_cwd {
        println!(
            "{}",
    ...
    let mut returncode: Option<String> = None;
    let mut stdout: Option<String> = None;
    let mut stderr: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        (returncode, stdout, stderr) = run_command(&cmd, args.capture, args.check, args.cwd);
        println!("{}", format!("Exit code: {:?}", returncode));
        if args.capture {
            if let Some(ref stdout_val) = stdout {
                println!("{}", format!("Output:\n{:?}", stdout_val));
            }
            if let Some(ref stderr_val) = stderr {
                eprintln!("{}", format!("Errors:\n{:?}", stderr_val));
            }
        }
        Ok(())
    })() {
        Ok(()) => {}
        Err(e) => {
            eprintln!("{}", format!("Command failed with exit code {}", 1));
            std::process::exit(1);
        }
    }
}
