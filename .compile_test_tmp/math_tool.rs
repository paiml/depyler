...
#[doc = "Compute sine of x radians.\n\n   >>>compute_sin (0.0)\n    0.0\n   >>>abs(compute_sin (3.14159265359 / 2) - 1.0)<0.0001\n    True\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compute_sin(x: f64) -> f64 {
    (x as f64).sin()
}
#[doc = "Compute cosine of x radians.\n\n   >>>compute_cos(0.0)\n    1.0\n   >>>abs(compute_cos(math.pi) + 1.0)<0.0001\n    True\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compute_cos(x: f64) -> f64 {
    (x as f64).cos()
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compute_ceil(x: f64) -> i32 {
    (x as f64).ceil()
}
#[doc = "Compute floor.\n\n   >>>compute_floor(2.0)\n    2\n   >>>compute_floor(2.9)\n    2\n   >>>compute_floor(-2.1)\n    -3\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compute_floor(x: f64) -> i32 {
    (x as f64).floor()
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Sin { .. }));
    match &args.command {
        Some(Commands::Sin { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_sin(x));
        }
        Some(Commands::Cos { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_cos(x));
        }
        Some(Commands::Sqrt { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_sqrt(x));
        }
        Some(Commands::Ceil { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_ceil(x));
        }
        Some(Commands::Floor { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_floor(x));
        }
