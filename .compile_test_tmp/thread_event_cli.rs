...
#[derive(Debug)]
pub struct Signal {
    pub _event: std::sync::Condvar,
    pub _signal_count: i32,
    ...
    pub fn set(&self) {
        {
            ...
            {
                self._signal_count = self._signal_count.clone() + 1;
            }
        }
        self._event.clone().set();
    }
    pub fn clear(&mut self) {
        self._event.clear();
    }
    pub fn is_set(&self) -> bool {
        return self._event.clone().is_set();
    }
    pub fn wait(&self, timeout: Option<f64>) -> bool {
        return self._event.clone().wait(timeout);
    }
    pub fn signal_count(&self) -> i32 {
        {
            let _context = self._lock.clone();
            {
                return self._signal_count.clone();
            }
        }
    }
}
...
impl Gate {
    ...
    pub fn wait_for_open(&self, timeout: Option<f64>) -> bool {
        {
            let _context = self._lock.clone();
            {
                self._waiters = self._waiters.clone() + 1;
            }
        }
        let result = self._event.clone().wait(timeout);
        {
            let _context = self._lock.clone();
            {
                self._waiters = self._waiters.clone() - 1;
...
impl Latch {
    ...
    pub fn count_down(&self) -> i32 {
        {
            let _context = self._lock.clone();
            {
                if self._count.clone() > 0 {
    ...
    pub fn reset(&self) {
        {
            let _context = self._lock.clone();
            {
                self._count = self._initial.clone();
                self._event.clear();
            }
        }
    }
...
impl PulseEvent {
    ...
    pub fn pulse(&mut self) {
        {
            let _context = self._lock.clone();
            {
                self._pulse_count = self._pulse_count.clone() + 1;
            }
        }
        self._event.clone().set();
        self._event.clear();
    ...
    pub fn pulse_count(&self) -> i32 {
        {
            let _context = self._lock.clone();
            {
                return self._pulse_count.clone();
            }
        }
    }
}
...
impl BarrierSimulator {
    ...
    pub fn wait(&self) -> i32 {
        {
            let _context = self._lock.clone();
            {
                let index = self._waiting.clone();
                self._waiting = self._waiting.clone() + 1;
                if self._waiting.clone() >= self._parties.clone() {
                    self._generation = self._generation.clone() + 1;
                    self._waiting = 0;
                };
                return index;
            }
    ...
    pub fn parties(&self) -> i32 {
        return self._parties.clone();
    }
    pub fn n_waiting(&self) -> i32 {
        {
            let _context = self._lock.clone();
            {
                return self._waiting.clone();
            }
        }
    }
...
enum Commands {
    ...
    Signal { ops: Vec<String> },
    
    Barrier { parties: i32, waits: i32 },
}
#[derive(Default)]
...
#[doc = "Simulate signal operations."]
#[doc = " Depyler: verified panic-free"]
pub fn simulate_signal(ops: &Vec<String>) -> Vec<bool> {
    ...
    for op in ops.iter().cloned() {
        if op == "set" {
            ...
            if op == "clear" {
                ...
                if op == "check" {
                    results.push(signal.is_set());
                } else {
                    if op == "count" {
                        results.push(signal.signal_count() > 0);
                    }
                }
            }
        }
...
#[doc = "Simulate gate operations."]
#[doc = " Depyler: verified panic-free"]
pub fn simulate_gate(ops: &Vec<String>) -> Vec<bool> {
    ...
    for op in ops.iter().cloned() {
        if op == "open" {
            ...
            if op == "close" {
                ...
                if op == "check" {
                    results.push(gate.is_open());
                } else {
                    if op == "pass" {
                        results.push(gate.pass_through());
                    }
                }
            }
        }
...
#[doc = "Simulate latch operations."]
#[doc = " Depyler: verified panic-free"]
pub fn simulate_latch(count: i32, ops: &Vec<String>) -> Vec<i32> {
    ...
    for op in ops.iter().cloned() {
        if op == "down" {
            ...
            if op == "count" {
                results.push(latch.get_count());
            } else {
                if op == "open" {
                    results.push(if latch.is_open() { 1 } else { 0 });
                } else {
                    if op == "reset" {
                        latch.reset();
                    }
...
#[doc = "Simulate barrier with given parties and waits."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn simulate_barrier(parties: i32, waits: i32) -> (i32, i32) {
    let barrier = BarrierSimulator::new(parties);
    for __sanitized in 0..(waits) {
        barrier.wait();
    }
    (barrier.n_waiting(), barrier.generation())
}
#[doc = "Execute event sequence and return states."]
#[doc = " Depyler: verified panic-free"]
pub fn event_sequence(sequence: &Vec<String>) -> Vec<bool> {
    let mut event = std::thread::Condvar();
    let mut states: Vec<bool> = vec![];
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Signal { .. }));
    match &args.command {
        Some(Commands::Signal { ref ops, .. }) => {
            let mut results = simulate_signal(&ops);
            println!("{}", format!("States: {:?}", results));
        }
        Some(Commands::Gate { ref ops, .. }) => {
            let mut results = simulate_gate(&ops);
            println!("{}", format!("States: {:?}", results));
        }
        Some(Commands::Latch {
            ref count, ref ops, ..
            ...
            let mut results = simulate_latch(count, &ops);
            println!("{}", format!("Results: {:?}", results));
        }
        Some(Commands::Barrier {
            ref parties,
            ref waits,
            ..
        }) => {
            let parties = *parties;
            let waits = *waits;
            let (waiting, gen) = simulate_barrier(parties, waits);
            println!(
                "{}",
                format!("Waiting: {:?}, Generation: {:?}", waiting, gen)
            );
        }
...
mod tests {
    ...
    #[test]
    fn test_simulate_signal_examples() {
        assert_eq!(simulate_signal(vec![]), vec![]);
        assert_eq!(simulate_signal(vec![1]), vec![1]);
    }
    #[test]
    fn test_simulate_gate_examples() {
        assert_eq!(simulate_gate(vec![]), vec![]);
        assert_eq!(simulate_gate(vec![1]), vec![1]);
    }
    #[test]
    fn test_event_sequence_examples() {
        assert_eq!(event_sequence(vec![]), vec![]);
        assert_eq!(event_sequence(vec![1]), vec![1]);
    }
}
