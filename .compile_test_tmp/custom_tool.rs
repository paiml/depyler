...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Port { .. }));
    match &args.command {
        Some(Commands::Port { ref value, .. }) => {
            let value = *value;
            let _cse_temp_1 = value >= 1;
            let _cse_temp_2 = value <= 65535;
            let _cse_temp_3 = (_cse_temp_1) && (_cse_temp_2);
            if _cse_temp_3 {
                println!("{}", format!("Valid port: {}", value));
            }
        }
        Some(Commands::Percent { ref value, .. }) => {
            let value = *value;
            println!("{}", ((value) as f64) / ((100.0) as f64));
        }
        Some(Commands::Bytes { ref value, .. }) => {
            let value = *value;
            println!("{}", format!("{} KB", value / 1024));
        }
