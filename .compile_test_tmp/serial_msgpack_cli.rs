...
pub const FIXINT_POS_MAX: i32 = 127;
pub const FIXINT_NEG_MIN: i32 = 224;
pub const FIXMAP_MAX: i32 = 143;
pub const FIXARRAY_MAX: i32 = 159;
pub const FIXSTR_MAX: i32 = 191;
pub const NIL: i32 = 192;
pub const FALSE: i32 = 194;
pub const TRUE: i32 = 195;
pub const BIN8: i32 = 196;
pub const BIN16: i32 = 197;
pub const BIN32: i32 = 198;
pub const FLOAT32: i32 = 202;
pub const FLOAT64: i32 = 203;
pub const UINT8: i32 = 204;
pub const UINT16: i32 = 205;
pub const UINT32: i32 = 206;
pub const UINT64: i32 = 207;
pub const INT8: i32 = 208;
pub const INT16: i32 = 209;
pub const INT32: i32 = 210;
pub const INT64: i32 = 211;
pub const STR8: i32 = 217;
pub const STR16: i32 = 218;
pub const STR32: i32 = 219;
pub const ARRAY16: i32 = 220;
pub const ARRAY32: i32 = 221;
pub const MAP16: i32 = 222;
pub const MAP32: i32 = 223;
...
impl DecodeResult {
    pub fn new(value: DepylerValue, bytes_consumed: i32) -> Self {
        Self {
            value,
            bytes_consumed,
        }
...
impl MsgPackEncoder {
    ...
    pub fn encode(&mut self, value: &DepylerValue) -> Vec<u8> {
        self.buffer.clear();
        self._encode_value(value);
        return vec![0u8; (self.buffer.clone()) as usize];
    }
    pub fn _encode_value(&mut self, value: &DepylerValue) {
        if value.is_none() {
            self.buffer.push(NIL);
        } else {
            if true {
                self.buffer.push(if value { TRUE } else { FALSE });
            } else {
                if true {
    ...
    pub fn _encode_int(&mut self, value: i32) {
        if 0 <= value && value <= FIXINT_POS_MAX {
            self.buffer.push(value);
        } else {
            if -32 <= value && value < 0 {
                self.buffer.push(value & 255);
            } else {
                if 0 <= value && value <= 255 {
                    self.buffer.push(UINT8);
                    self.buffer.push(value);
                } else {
                    if 0 <= value && value <= 65535 {
                        self.buffer.push(UINT16);
                        self.buffer.extend(r#struct.pack(">H".to_string(), value));
                    } else {
                        if 0 <= value && value <= 4294967295 {
                            self.buffer.push(UINT32);
                            self.buffer.extend(r#struct.pack(">I".to_string(), value));
                        } else {
                            if 0 <= value && value <= 0 {
                                self.buffer.push(UINT64);
                                self.buffer.extend(r#struct.pack(">Q".to_string(), value));
                            } else {
                                if -128 <= value && value < 0 {
                                    self.buffer.push(INT8);
                                    self.buffer.extend(r#struct.pack(">b".to_string(), value));
                                } else {
                                    if -32768 <= value && value < 0 {
                                        self.buffer.push(INT16);
                                        self.buffer.extend(r#struct.pack(">h".to_string(), value));
                                    } else {
                                        if -2147483648 <= value && value < 0 {
                                            self.buffer.push(INT32);
                                            self.buffer
                                                .extend(r#struct.pack(">i".to_string(), value));
                                        } else {
                                            self.buffer.push(INT64);
                                            self.buffer
                                                .extend(r#struct.pack(">q".to_string(), value));
                                        };
                                    };
                                };
                            };
    ...
    pub fn _encode_float(&mut self, value: f64) {
        self.buffer.push(FLOAT64);
        self.buffer.extend(r#struct.pack(">d".to_string(), value));
    }
    pub fn _encode_str(&mut self, value: String) {
        let data = value.encode("utf-8".to_string());
        let length = data.len() as i32;
        if length <= 31 {
            self.buffer.push(160 | length);
        } else {
            if length <= 255 {
                self.buffer.push(STR8);
                self.buffer.push(length);
            } else {
                if length <= 65535 {
                    self.buffer.push(STR16);
                    self.buffer.extend(r#struct.pack(">H".to_string(), length));
                } else {
                    self.buffer.push(STR32);
                    self.buffer.extend(r#struct.pack(">I".to_string(), length));
                };
            };
        };
        self.buffer.extend(data);
    }
    pub fn _encode_bin(&mut self, value: &Vec<u8>) {
        let length = value.len() as i32;
        if length <= 255 {
            self.buffer.push(BIN8);
            self.buffer.push(length);
        } else {
            if length <= 65535 {
                self.buffer.push(BIN16);
                self.buffer.extend(r#struct.pack(">H".to_string(), length));
            } else {
                self.buffer.push(BIN32);
                self.buffer.extend(r#struct.pack(">I".to_string(), length));
            };
        };
        self.buffer.extend(value);
    }
    pub fn _encode_array(&mut self, value: Vec<DepylerValue>) {
        let length = value.len() as i32;
        if length <= 15 {
            self.buffer.push(144 | length);
        } else {
            if length <= 65535 {
                self.buffer.push(ARRAY16);
                self.buffer.extend(r#struct.pack(">H".to_string(), length));
            } else {
                self.buffer.push(ARRAY32);
                self.buffer.extend(r#struct.pack(">I".to_string(), length));
            };
        };
        for item in value {
    ...
    pub fn _encode_map(&mut self, value: std::collections::HashMap<String, DepylerValue>) {
        let length = value.len() as i32;
        if length <= 15 {
            self.buffer.push(128 | length);
        } else {
            if length <= 65535 {
                self.buffer.push(MAP16);
                self.buffer.extend(r#struct.pack(">H".to_string(), length));
            } else {
                self.buffer.push(MAP32);
                self.buffer.extend(r#struct.pack(">I".to_string(), length));
            };
        };
...
impl MsgPackDecoder {
    ...
    pub fn decode(&mut self) -> DepylerValue {
        if self.pos.clone() >= (self.data.clone().len() as i32) {
            panic!(
                "Exception: {}",
                ValueError::new("Unexpected end of data".to_string())
            );
        };
        let marker = {
            let _base = &self.data.clone();
        ...
        if 128 <= marker && marker <= FIXMAP_MAX {
            return self._decode_map(marker & 15);
        };
        if 144 <= marker && marker <= FIXARRAY_MAX {
            return self._decode_array(marker & 15);
        };
        if 160 <= marker && marker <= FIXSTR_MAX {
            return self._decode_str(marker & 31);
        };
        if marker >= FIXINT_NEG_MIN {
            return {
                let _base = &r#struct.unpack(">b".to_string(), vec![0u8; (vec![marker]) as usize]);
                let _idx = 0;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
        ...
        panic!(
            "Exception: {}",
            ValueError::new(format!("Unknown marker: 0x{}", marker))
        );
    }
    pub fn _read_bytes(&mut self, n: i32) -> Vec<u8> {
        if self.pos.clone() + n > (self.data.clone().len() as i32) {
            panic!(
                "Exception: {}",
                ValueError::new("Unexpected end of data".to_string())
            );
        ...
        let result = {
            ...
            let start_idx = (self.pos.clone()) as isize;
            let stop_idx = (self.pos.clone() + n) as isize;
        ...
        self.pos = self.pos.clone() + n;
        return result;
    }
    pub fn _read_uint8(&self) -> i32 {
        return {
            let _base = &self._read_bytes(1);
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _read_uint16(&self) -> i32 {
        return {
            let _base = &r#struct.unpack(">H".to_string(), self._read_bytes(2));
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _read_uint32(&self) -> i32 {
        return {
            let _base = &r#struct.unpack(">I".to_string(), self._read_bytes(4));
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _read_uint64(&self) -> i32 {
        return {
            let _base = &r#struct.unpack(">Q".to_string(), self._read_bytes(8));
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _read_int8(&self) -> i32 {
        return {
            let _base = &r#struct.unpack(">b".to_string(), self._read_bytes(1));
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _read_float32(&self) -> f64 {
        return {
            let _base = &r#struct.unpack(">f".to_string(), self._read_bytes(4));
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _read_float64(&self) -> f64 {
        return {
            let _base = &r#struct.unpack(">d".to_string(), self._read_bytes(8));
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn _decode_str(&self, length: i32) -> String {
        return self._read_bytes(length).decode("utf-8".to_string());
    }
    pub fn _decode_bin(&self, length: i32) -> Vec<u8> {
        return vec![0u8; (self._read_bytes(length)) as usize];
    }
    pub fn _decode_array(&self, length: i32) -> Vec<DepylerValue> {
        return 0..length
            .into_iter()
            .map(|_| self.decode())
            .collect::<Vec<_>>();
    }
    pub fn _decode_map(&self, length: i32) -> std::collections::HashMap<String, DepylerValue> {
        let mut result = {
...
#[doc = "Encode value to MessagePack."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn msgpack_encode(value: &DepylerValue) -> Vec<u8> {
    MsgPackEncoder::new().as_bytes().to_vec()
}
#[doc = "Decode MessagePack data."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn msgpack_decode(data: &Vec<u8>) -> DepylerValue {
    String::from_utf8_lossy(&MsgPackDecoder::new(data)).to_string()
}
#[doc = "Calculate encoded size."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn msgpack_size(value: &DepylerValue) -> i32 {
    msgpack_encode(&value).len() as i32 as i32
}
...
#[doc = "Compare two values by their encoded form."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compare_encoded<'b, 'a>(val1: &'a DepylerValue, val2: &'b DepylerValue) -> bool {
    msgpack_encode(&val1) == msgpack_encode(&val2)
}
#[doc = "Encode multiple values as an array."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_batch(values: &Vec<DepylerValue>) -> Vec<u8> {
    msgpack_encode(&values)
}
#[doc = "Decode array of values."]
#[doc = " Depyler: proven to terminate"]
pub fn decode_batch(data: &Vec<u8>) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>> {
    let result = msgpack_decode(&data);
    if !true {
        return Err(Box::new(ValueError::new("Expected array".to_string())));
    }
    Ok(result)
}
#[doc = "Simulate MessagePack operations."]
pub fn simulate_msgpack(
    ...
    for op in operations.iter().cloned() {
        ...
        let mut encoded;
        if cmd == "encode" {
            value = std::collections::HashMap::<String, DepylerValue>::new();
            encoded = msgpack_encode(&value);
            context.insert("encoded".to_string(), encoded);
            results.push(DepylerValue::Str(format!("{:?}", bytes_to_hex(&encoded))));
        } else {
            if cmd == "decode" {
                ...
                if cmd == "size" {
                    results.push(DepylerValue::Str(format!(
                        "{:?}",
                        (context.get("encoded").cloned().unwrap_or(b"").len() as i32).to_string()
                    )));
                } else {
                    if cmd == "roundtrip" {
                        value = std::collections::HashMap::<String, DepylerValue>::new();
                        encoded = msgpack_encode(&value);
                        let decoded = msgpack_decode(&encoded);
                        results.push(DepylerValue::Str(format!(
                            "{:?}",
                            if value == decoded {
                                "ok".to_string()
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: serial_msgpack_cli.py <command>[args...]");
        println!("{}", "Commands: encode, decode, size");
    ...
    if _cse_temp_2 {
        ...
        let _cse_temp_3 = cmd == "decode";
        if _cse_temp_3 {
            data = {
                let mut content = String::new();
                std::io::stdin().buffer.read_to_string(&mut content)?;
                content
            };
            let decoded = msgpack_decode(&data);
            println!("{}", format!("{:?}", decoded));
        } else {
            let _cse_temp_4 = cmd == "size";
            if _cse_temp_4 {
                data = std::collections::HashMap::<String, DepylerValue>::new();
                encoded = msgpack_encode(&data);
                println!("{}", encoded.len() as i32);
            } else {
                let _cse_temp_5 = cmd == "hex";
                if _cse_temp_5 {
                    data = std::collections::HashMap::<String, DepylerValue>::new();
                    encoded = msgpack_encode(&data);
                    println!("{}", bytes_to_hex(&encoded));
                }
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_simulate_msgpack_examples() {
        assert_eq!(simulate_msgpack(vec![]), vec![]);
        assert_eq!(simulate_msgpack(vec![1]), vec![1]);
    }
}
