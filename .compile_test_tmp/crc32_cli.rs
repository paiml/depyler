...
#![allow(dead_code)]
pub const CRC32_POLY: i32 = 3988292384;
pub static CRC32_TABLE: std::sync::LazyLock<Vec<i32>> = std::sync::LazyLock::new(|| vec![]);
use std::io::Read;
use std::io::Write;
...
struct Args {
    ...
    #[doc = "Verify checksums from file"]
    check: Option<String>,
    
    
    #[doc = "Calculate checksum of string"]
    string: Option<String>,
    
    
    #[doc = "Verify file against checksum"]
    verify: Option<String>,
    
    
    #[doc = "Output raw binary checksum"]
    binary: bool,
    
    
    #[doc = "Quiet mode(exit code only)"]
    quiet: bool,
}
#[doc = "Initialize CRC32 lookup table."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn init_crc32_table() {
    if CRC32_TABLE {
        return;
    }
    for i in 0..(256) {
        let mut crc = i.clone();
        for __sanitized in 0..(8) {
            if crc & 1 {
...
#[doc = "Update CRC32 with data."]
pub fn crc32_update(mut crc: i32, data: &Vec<u8>) -> Result<i32, Box<dyn std::error::Error>> {
    init_crc32_table();
    for byte in data.iter().cloned() {
        crc = {
            let base = &CRC32_TABLE;
            let idx: i32 = (crc ^ byte) & 255;
...
#[doc = "Calculate CRC32 of data."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn crc32(data: &Vec<u8>) -> Result<i32, Box<dyn std::error::Error>> {
    Ok(crc32_update(4294967295, &data)? ^ 4294967295)
}
#[doc = "Calculate CRC32 of file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn crc32_file(path: &str, chunk_size: i32) -> Result<i32, std::io::Error> {
...
#[doc = "Format CRC32 value as hex string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_crc32(value: i32) -> String {
    format!("{}", value)
}
#[doc = "Parse CRC32 from hex string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_crc32(hex_str: &str) -> Result<Option<i32>, Box<dyn std::error::Error>> {
    {
        return Ok(Some(i64::from_str_radix(&hex_str, 16).unwrap()));
        return Ok(None);
    }
}
#[doc = "Verify file checksum matches expected."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn verify_checksum(path: &str, expected: i32) -> Result<bool, Box<dyn std::error::Error>> {
    let actual = crc32_file(path, 65536i32)?;
    Ok(actual == expected)
}
#[doc = "Generate checksum file content."]
#[doc = " Depyler: verified panic-free"]
pub fn generate_checksum_file(
    ...
    for path in paths.iter().cloned() {
        ...
        let checksum = crc32_file(&path, 65536i32)?;
        lines.push(DepylerValue::Str(format!(
            "{:?}",
            format!("{}  {}", format_crc32(checksum), path)
        )));
    }
    Ok(lines)
}
#[doc = "Parse checksum file content.\n\n    Returns list of(checksum, path) tuples.\n    "]
pub fn parse_checksum_file(
        ...
        .collect::<Vec<String>>()
    {
        ...
        let parts = line
            .splitn((1 + 1) as usize, &None)
            .map(|s| s.to_string())
            .collect::<Vec<String>>();
        if parts.len() as i32 != 2 {
            continue;
        }
        let checksum = parse_crc32(
            parts
                .get(0usize)
                .cloned()
                .expect("IndexError: list index out of range"),
...
#[doc = "Verify checksums from checksum file.\n\n    Returns list of(path, passed, message) tuples.\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn verify_checksum_file(content: &str) -> Result<Vec<(String, bool, String)>, std::io::Error> {
    let mut results = vec![];
    let entries = parse_checksum_file(content)?;
    for (expected, path) in entries.iter().cloned() {
        if !std::path::Path::new(&path).exists() {
            results.push(DepylerValue::Str(format!(
                "{:?}",
                (path, false, "NOT FOUND".to_string().to_string())
            )));
            continue;
        }
        let mut actual: Option<String> = None;
        match (|| -> Result<(), Box<dyn std::error::Error>> {
            actual = Some(crc32_file(&path, 65536i32)?);
            if actual.unwrap_or_default() == expected {
                results.push(DepylerValue::Str(format!(
                    "{:?}",
                    (path, true, "OK".to_string().to_string())
                )));
            } else {
                results.push(DepylerValue::Str(format!(
                    "{:?}",
                    (path, false, format!("FAILED(got {})", format_crc32(actual)))
                )));
            }
            Ok(())
        })() {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut all_passed: bool = Default::default();
    let mut checksum: DepylerValue = Default::default();
    let args = Args::default();
    let has_check = args.check.is_some();
    let has_string = args.string.is_some();
    let has_verify = args.verify.is_some();
    if has_check {
        match (|| -> Result<(), Box<dyn std::error::Error>> {
            let mut f = std::fs::File::open(args.check)?;
            let content = {
        ...
        let results = verify_checksum_file(&content)?;
        let mut failed = 0;
        for (path, passed, message) in results.iter().cloned() {
            if !args.quiet {
                println!("{}", format!("{}: {}", path, message));
            }
            if !passed {
                failed = failed + 1;
            }
        }
        let _cse_temp_0 = failed > 0;
        if _cse_temp_0 {
            if !args.quiet {
                println!("{}", format!("\n{} failed", failed));
            }
    ...
    if has_string {
        checksum = crc32(&args.string.as_bytes().to_vec())?;
        if args.binary {
            std::io::stdout()
                .buffer
                .write_all(checksum.to_bytes(4, "little".to_string()).as_bytes())
                .unwrap();
        } else {
            println!("{}", format_crc32(checksum));
        }
        return Ok(());
    }
    if has_verify {
        ...
        let expected = parse_crc32(&args.verify)?;
        if expected.is_none() {
            eprintln!(
                "{}",
        ...
        for path in args.files {
            if !std::path::Path::new(&path).is_file() {
                if !args.quiet {
                    println!("{}", format!("{}: NOT FOUND", path));
                }
                all_passed = false;
                continue;
            }
            let passed = verify_checksum(&path, expected)?;
            if !args.quiet {
                ...
                println!("{}", format!("{}: {}", path, status));
            }
            if !passed {
                all_passed = false;
            }
        }
        let _ = if all_passed { 0 } else { 1 };
        return Ok(());
    }
    if !args.files {
        let data = {
    ...
    for path in args.files {
        if !std::path::Path::new(&path).is_file() {
            eprintln!("{}", format!("{}: NOT FOUND", path));
            continue;
        }
        match (|| -> Result<(), Box<dyn std::error::Error>> {
            checksum = crc32_file(&path, 65536i32)?;
            if args.binary {
                std::io::stdout()
                    .buffer
                    .write_all(checksum.to_bytes(4, "little".to_string()).as_bytes())
                ...
                println!("{}", format!("{}  {}", format_crc32(checksum), path));
            }
            Ok(())
        })() {
            Ok(()) => {}
            Err(e) => {
                eprintln!("{}", format!("{}: {:?}", path, e));
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn test_generate_checksum_file_examples() {
        assert_eq!(generate_checksum_file(vec![]), vec![]);
        assert_eq!(generate_checksum_file(vec![1]), vec![1]);
    }
}
