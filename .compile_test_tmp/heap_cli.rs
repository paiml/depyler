...
impl MinHeap {
    ...
    pub fn left_child(&self, i: i32) -> i32 {
        return 2 * i + 1;
    }
    pub fn right_child(&self, i: i32) -> i32 {
        return 2 * i + 2;
    }
    pub fn push(&mut self, value: i32) {
        self.heap.push(value);
        self._sift_up((self.heap.clone().len() as i32).saturating_sub(1));
    }
    pub fn pop(&self) -> Option<i32> {
        if self.heap.clone().is_empty() {
            return None;
        };
        if (self.heap.clone().len() as i32) == 1 {
            return Some(self.heap.pop().unwrap_or_default());
        };
        let root = {
            let _base = &self.heap.clone();
            let _idx = 0;
            if _idx < 0 {
        ...
        self.heap
            .clone()
            .insert(0, self.heap.pop().unwrap_or_default());
        self._sift_down(0);
        return Some(root);
    }
    pub fn peek(&self) -> Option<i32> {
        return Some(if self.heap.clone() {
            {
                let _base = &self.heap.clone();
                let _idx = 0;
                if _idx < 0 {
        ...
        while i > 0 {
            ...
            if {
            ...
            } {
                {
                    ...
                    self.heap.clone()[(i) as usize] = _swap_temp.0;
                    self.heap.clone()[(parent) as usize] = _swap_temp.1;
                }
                let i = parent;
            } else {
                break;
    ...
    pub fn _sift_down(&self, i: i32) {
        let size = self.heap.clone().len() as i32;
        while true {
            let mut smallest = i;
            ...
            if smallest != i {
                {
                    ...
                    self.heap.clone()[(i) as usize] = _swap_temp.0;
                    self.heap.clone()[(smallest) as usize] = _swap_temp.1;
                }
                let i = smallest;
            } else {
                break;
    ...
    pub fn size(&self) -> i32 {
        return self.heap.clone().len() as i32;
    }
    pub fn is_empty(&self) -> bool {
        return (self.heap.clone().len() as i32) == 0;
    }
    pub fn to_list(&self) -> Vec<i32> {
        return self.heap.clone().clone();
    }
}
#[derive(Debug, Clone)]
pub struct MaxHeap {
    pub min_heap: DepylerValue,
}
impl MaxHeap {
    pub fn new() -> Self {
        Self {
    ...
    pub fn pop(&self) -> Option<i32> {
        let result = self.min_heap.pop().unwrap_or_default();
        return Some(if result.is_some() { -result } else { None });
    }
    pub fn peek(&self) -> Option<i32> {
        let result = self.min_heap.clone().peek();
        return Some(if result.is_some() { -result } else { None });
    }
    pub fn size(&self) -> i32 {
        return self.min_heap.clone().size();
    }
    pub fn is_empty(&self) -> bool {
        return self.min_heap.clone().is_empty();
    }
    pub fn to_list(&self) -> Vec<i32> {
        return self
            .min_heap
...
impl PriorityQueue {
    pub fn new(max_priority: bool) -> Self {
        Self {
            entries: Vec::new(),
            max_priority,
        }
    }
    pub fn push(&mut self, priority: i32, value: String) {
        self.entries.push((priority, value));
        self._sift_up((self.entries.clone().len() as i32).saturating_sub(1));
    }
    pub fn pop(&self) -> Option<(i32, String)> {
        if self.entries.clone().is_empty() {
            return None;
        };
        if (self.entries.clone().len() as i32) == 1 {
            return Some(self.entries.pop().unwrap_or_default());
        };
        let root = {
            let _base = &self.entries.clone();
            let _idx = 0;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
        ...
        self.entries
            .clone()
            .insert(0, self.entries.pop().unwrap_or_default());
        self._sift_down(0);
        return Some(root);
    }
    pub fn peek(&self) -> Option<(i32, String)> {
        return Some(if self.entries.clone() {
            {
                let _base = &self.entries.clone();
                let _idx = 0;
                if _idx < 0 {
    ...
    pub fn _compare(&self, a: (i32, String), b: (i32, String)) -> bool {
        if self.max_priority.clone() {
            return a.0 > b.0;
        };
        return a.0 < b.0;
    }
        ...
        while i > 0 {
            ...
            if self._compare(
            ...
            ) {
                {
                    ...
                    self.entries.clone()[(i) as usize] = _swap_temp.0;
                    self.entries.clone()[(parent) as usize] = _swap_temp.1;
                }
                let i = parent;
            } else {
                break;
    ...
    pub fn _sift_down(&self, i: i32) {
        let size = self.entries.clone().len() as i32;
        while true {
            let mut best = i;
            let left = 2 * i + 1;
            let right = 2 * i + 2;
            ...
            if best != i {
                {
                    ...
                    self.entries.clone()[(i) as usize] = _swap_temp.0;
                    self.entries.clone()[(best) as usize] = _swap_temp.1;
                }
                let i = best;
            } else {
                break;
    ...
    pub fn size(&self) -> i32 {
        return self.entries.clone().len() as i32;
    }
    pub fn is_empty(&self) -> bool {
        return (self.entries.clone().len() as i32) == 0;
    }
}
#[derive(Default)]
...
struct Args {
    ...
    #[doc = "K for k-smallest/largest"]
    k: i32,
}
#[doc = "Convert array to min heap in-place."]
...
#[doc = "Helper for heapify."]
#[doc = " Depyler: proven to terminate"]
pub fn _heapify_down(arr: &Vec<i32>, n: i32, i: i32) -> Result<(), Box<dyn std::error::Error>> {
    let mut smallest: i32 = Default::default();
    smallest = i;
    let _cse_temp_0 = 2 * i;
    let left = _cse_temp_0 + 1;
    let right = _cse_temp_0 + 2;
    let _cse_temp_1 = left < n;
    let _cse_temp_2 = arr
        .get(left as usize)
        .cloned()
    ...
    let _cse_temp_4 = right < n;
    let _cse_temp_5 = (_cse_temp_4) && (_cse_temp_2);
    if _cse_temp_5 {
        smallest = right;
    }
    let _cse_temp_6 = smallest != i;
    if _cse_temp_6 {
        let _swap_temp = (
            arr.get(smallest as usize)
                .cloned()
                .expect("IndexError: list index out of range"),
            arr.get(i as usize)
                .cloned()
        ...
        arr.insert(smallest, _swap_temp.1);
        _heapify_down(&arr, n, smallest);
    }
    Ok(())
}
#[doc = "Sort array using heap sort."]
#[doc = " Depyler: proven to terminate"]
pub fn heap_sort(arr: &Vec<i32>, reverse: bool) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    if arr.is_empty() {
        return Ok(vec![]);
    }
    let result = arr.clone();
    ...
    for i in {
        ...
        result[(0) as usize] = _swap_temp.0;
        result.insert(i, _swap_temp.1);
        _heap_sort_down(&result, i, 0, reverse);
    }
    Ok(result)
}
#[doc = "Helper for heap sort."]
#[doc = " Depyler: proven to terminate"]
pub fn _heap_sort_down(
    arr: &Vec<i32>,
    n: i32,
    i: i32,
    reverse: bool,
) -> Result<(), Box<dyn std::error::Error>> {
    let mut extreme: i32 = Default::default();
    extreme = i;
    let _cse_temp_0 = 2 * i;
    let left = _cse_temp_0 + 1;
    let right = _cse_temp_0 + 2;
    if reverse {
        ...
        let _cse_temp_9 = right < n;
        let _cse_temp_10 = (_cse_temp_9) && (_cse_temp_7);
        if _cse_temp_10 {
            extreme = right;
        }
    }
    let _cse_temp_11 = extreme != i;
    if _cse_temp_11 {
        let _swap_temp = (
            arr.get(extreme as usize)
                .cloned()
                .expect("IndexError: list index out of range"),
            arr.get(i as usize)
                .cloned()
        ...
        arr.insert(i, _swap_temp.0);
        arr.insert(extreme, _swap_temp.1);
        _heap_sort_down(&arr, n, extreme, reverse);
    }
    Ok(())
}
#[doc = "Find k smallest elements."]
#[doc = " Depyler: verified panic-free"]
pub fn k_smallest(arr: &Vec<i32>, k: i32) -> Vec<i32> {
    let _cse_temp_0 = k <= 0;
    let _cse_temp_1 = (_cse_temp_0) || (arr.is_empty());
    if _cse_temp_1 {
        return vec![];
    }
    ...
    let mut result = vec![];
    for __sanitized in 0..(depyler_min((k).clone(), (arr.len() as i32).clone())) {
        let val = heap.pop();
        if val.is_some() {
            result.push(DepylerValue::Str(format!("{:?}", val)));
        }
    }
    result
}
#[doc = "Find k largest elements."]
#[doc = " Depyler: verified panic-free"]
pub fn k_largest(arr: &Vec<i32>, k: i32) -> Vec<i32> {
    let _cse_temp_0 = k <= 0;
    let _cse_temp_1 = (_cse_temp_0) || (arr.is_empty());
    if _cse_temp_1 {
        return vec![];
...
#[doc = "Merge k sorted arrays using a min heap."]
pub fn merge_k_sorted_arrays(
    arrays: &Vec<Vec<i32>>,
    ...
    let mut result = vec![];
    let mut heap: Vec<(i32, i32, i32)> = vec![];
    for (i, arr) in arrays
        .iter()
        .cloned()
        .enumerate()
    ...
    while !heap.is_empty() {
        ...
        if !heap.is_empty() {
            _heapify_tuple(&heap, heap.len() as i32, 0);
        }
    }
    Ok(result)
}
#[doc = "Heapify for tuple comparisons."]
#[doc = " Depyler: proven to terminate"]
pub fn _heapify_tuple(
    arr: &Vec<(i32, i32, i32)>,
    n: i32,
    i: i32,
) -> Result<(), Box<dyn std::error::Error>> {
...
#[doc = "Check if array represents a valid heap."]
#[doc = " Depyler: proven to terminate"]
pub fn is_valid_heap(arr: &Vec<i32>, max_heap: bool) -> Result<bool, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = args.mode == "heap";
    let mut result;
    if _cse_temp_0 {
        ...
        let _cse_temp_1 = args.mode == "sort";
        if _cse_temp_1 {
            result = heap_sort(&values, args.max)?;
            println!(
                "{}",
                format!(
                    "Sorted: {}",
            ...
            let _cse_temp_2 = args.mode == "ksmallest";
            if _cse_temp_2 {
                ...
                let _cse_temp_3 = args.mode == "klargest";
                if _cse_temp_3 {
                    ...
                    let _cse_temp_4 = args.mode == "validate";
                    if _cse_temp_4 {
                        if is_valid_heap(&values, args.max).unwrap_or(false) {
                            let heap_type = if args.max {
                                "max".to_string()
                            } else {
                                "min".to_string()
                            };
                            println!("{}", format!("Valid {} heap: Yes", heap_type));
                        } else {
                            println!("{}", "Valid heap: No");
                        }
                    }
                }
            }
