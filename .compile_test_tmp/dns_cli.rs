...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'dataclasses'(tracked in DEPYLER-0424)"] pub static DNS_TYPES: std::sync::LazyLock<std::collections::HashMap<DepylerValue, DepylerValue>>= std::sync::LazyLock::new(|| {
    let mut map = HashMap::new();
    map.insert(DepylerValue::Int(1 as i64), DepylerValue::Str("A".to_string().to_string()));
    map.insert(DepylerValue::Int(2 as i64), DepylerValue::Str("NS".to_string().to_string()));
    map.insert(DepylerValue::Int(5 as i64), DepylerValue::Str("CNAME".to_string().to_string()));
    map.insert(DepylerValue::Int(6 as i64), DepylerValue::Str("SOA".to_string().to_string()));
    map.insert(DepylerValue::Int(12 as i64), DepylerValue::Str("PTR".to_string().to_string()));
    map.insert(DepylerValue::Int(15 as i64), DepylerValue::Str("MX".to_string().to_string()));
    map.insert(DepylerValue::Int(16 as i64), DepylerValue::Str("TXT".to_string().to_string()));
    map.insert(DepylerValue::Int(28 as i64), DepylerValue::Str("AAAA".to_string().to_string()));
    map });
    pub const DNS_TYPES_REVERSE: std::collections::HashMap<DepylerValue, DepylerValue>= DNS_TYPES.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>().into_iter().map(|(k, v) | {
    let _v = k;
   (v, _v) }).collect::<std::collections::HashMap<_, _>>();
    pub static DNS_CLASSES: std::sync::LazyLock<std::collections::HashMap<DepylerValue, DepylerValue>>= std::sync::LazyLock::new(|| {
    let mut map = HashMap::new();
    map.insert(DepylerValue::Int(1 as i64), DepylerValue::Str("IN".to_string().to_string()));
    map.insert(DepylerValue::Int(3 as i64), DepylerValue::Str("CH".to_string().to_string()));
    map.insert(DepylerValue::Int(4 as i64), DepylerValue::Str("HS".to_string().to_string()));
    map });
...
} #[derive(Debug, Clone, PartialEq)] pub struct DNSHeader {
    pub id: i32, pub qr: i32, pub opcode: i32, pub aa: i32, pub tc: i32, pub rd: i32, pub ra: i32, pub z: i32, pub rcode: i32, pub qdcount: i32, pub ancount: i32, pub nscount: i32, pub arcount: i32
}
impl DNSHeader {
    pub fn new(id: i32, qr: i32, opcode: i32, aa: i32, tc: i32, rd: i32, ra: i32, z: i32, rcode: i32, qdcount: i32, ancount: i32, nscount: i32, arcount: i32) -> Self {
    Self {
    id, qr, opcode, aa, tc, rd, ra, z, rcode, qdcount, ancount, nscount, arcount
}
}
}
#[derive(Debug, Clone, PartialEq)] pub struct DNSQuestion {
    pub name: String, pub qtype: i32, pub qclass: i32
}
impl DNSQuestion {
    pub fn new(name: String, qtype: i32, qclass: i32) -> Self {
    Self {
    name, qtype, qclass
}
}
}
#[derive(Debug, Clone, PartialEq)] pub struct DNSRecord {
    pub name: String, pub rtype: i32, pub rclass: i32, pub ttl: i32, pub rdlength: i32, pub rdata: Vec<u8>
}
impl DNSRecord {
    pub fn new(name: String, rtype: i32, rclass: i32, ttl: i32, rdlength: i32, rdata: Vec<u8>) -> Self {
    Self {
...
#[derive(Default)] struct Args {
    #[doc = "Operation mode"] mode: String, #[doc = "Domain name for query"] domain: Option<String>, #[doc = "DNS record type"] type: String, #[doc = "Hex-encoded DNS message to parse"] hex: Option<String>
}
#[doc = r" Stub for local import from module: #module_name"] #[doc = r" DEPYLER-0615: Generated to allow standalone compilation"] #[allow(dead_code, unused_variables)] pub fn dataclass<T: Default>(_args: impl std::any::Any) -> T {
    Default::default()
}
#[doc = "Parse DNS header from bytes."] #[doc = " Depyler: proven to terminate"] pub fn parse_header(data: & Vec<u8>) -> Result<DNSHeader, Box<dyn std::error::Error>>{
    let _cse_temp_0 = data.len() as i32;
    let _cse_temp_1 = _cse_temp_0<12;
    if _cse_temp_1 {
    return Err(Box::new(ValueError::new("DNS header too short".to_string())));
   
}
let _cse_temp_2 = data.get(0usize).cloned().expect("IndexError: list index out of range") <<8;
    let _cse_temp_3 = _cse_temp_2 | data.get(1usize).cloned().expect("IndexError: list index out of range");
    let id_ = _cse_temp_3;
    let flags = _cse_temp_3;
    let _cse_temp_4 = flags>>15;
    let _cse_temp_5 = _cse_temp_4 & 1;
    let qr = _cse_temp_5;
    let _cse_temp_6 = flags>>11;
    let _cse_temp_7 = _cse_temp_6 & 15;
    let opcode = _cse_temp_7;
    let _cse_temp_8 = flags>>10;
    let _cse_temp_9 = _cse_temp_8 & 1;
    let aa = _cse_temp_9;
    let _cse_temp_10 = flags>>9;
    let _cse_temp_11 = _cse_temp_10 & 1;
    let tc = _cse_temp_11;
    let _cse_temp_12 = flags>>8;
    let _cse_temp_13 = _cse_temp_12 & 1;
    let rd = _cse_temp_13;
    let _cse_temp_14 = flags>>7;
    let _cse_temp_15 = _cse_temp_14 & 1;
    let ra = _cse_temp_15;
    let _cse_temp_16 = flags>>4;
    let _cse_temp_17 = _cse_temp_16 & 7;
    let z = _cse_temp_17;
    let _cse_temp_18 = flags & 15;
    let rcode = _cse_temp_18;
    let qdcount = _cse_temp_3;
    let ancount = _cse_temp_3;
    let nscount = _cse_temp_3;
    let arcount = _cse_temp_3;
    Ok(DNSHeader::new(id_, qr, opcode, aa, tc, rd, ra, z, rcode, qdcount, ancount, nscount, arcount))
}
#[doc = "Encode DNS header to bytes."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn encode_header(header: & DNSHeader) -> Vec<u8>{
    let id_bytes = vec! [header.id>>8, header.id & 255].into_iter().map(| x | x as u8).collect::<Vec<u8>>();
    let _cse_temp_0 = header.qr <<15;
    let _cse_temp_1 = header.opcode <<11;
    let _cse_temp_2 = _cse_temp_0 | _cse_temp_1;
    let _cse_temp_3 = header.aa <<10;
    let _cse_temp_4 = _cse_temp_2 | _cse_temp_3;
    let _cse_temp_5 = header.tc <<9;
    let _cse_temp_6 = _cse_temp_4 | _cse_temp_5;
    let _cse_temp_7 = header.rd <<8;
    let _cse_temp_8 = _cse_temp_6 | _cse_temp_7;
    let _cse_temp_9 = header.ra <<7;
    let _cse_temp_10 = _cse_temp_8 | _cse_temp_9;
    let _cse_temp_11 = header.z <<4;
    let _cse_temp_12 = _cse_temp_10 | _cse_temp_11;
    let _cse_temp_13 = _cse_temp_12 | header.rcode;
    let flags = _cse_temp_13;
    let flags_bytes = vec! [flags>>8, flags & 255].into_iter().map(| x | x as u8).collect::<Vec<u8>>();
    let counts = vec! [header.qdcount>>8, header.qdcount & 255, header.ancount>>8, header.ancount & 255, header.nscount>>8, header.nscount & 255, header.arcount>>8, header.arcount & 255].into_iter().map(| x | x as u8).collect::<Vec<u8>>();
    id_bytes + flags_bytes + counts
}
#[doc = "Parse DNS domain name with compression support."] pub fn parse_name(data: & Vec<u8>, offset: i32) -> Result <(String, i32), Box<dyn std::error::Error>>{
    let mut pos: i32 = Default::default();
    let mut original_pos: i32 = Default::default();
    let mut jumped: bool = Default::default();
    let mut labels = vec! [];
    pos = offset;
    jumped = false;
    original_pos = offset;
...
let length = data.get(pos as usize).cloned().expect("IndexError: list index out of range");
    if length & 192 == 192 {
    if pos + 1>= data.len() as i32 {
    break;
   
}
let pointer  = (length & 63) <<8 | {
    let base = & data;
    let idx: i32 = pos + 1;
...
pos = pos + 1;
    if pos + length>data.len() as i32 {
    break;
   
}
let label = String::from_utf8_lossy(& {
    let base = & data;
    let start_idx  = (pos) as isize;
    let stop_idx  = (pos + length) as isize;
   ...
   (base.len() as isize + stop_idx).max(0) as usize
    ...
    labels.push(DepylerValue::Str(label.to_string()));
    pos = pos + length;
...
#[doc = "Encode domain name to DNS format."] #[doc = " Depyler: verified panic-free"] pub fn encode_name(name: & str) -> Vec<u8>{
    let mut result: DepylerValue = Default::default();
    let _cse_temp_0  = (* name) == ".";
    let _cse_temp_1  = (name.is_empty()) ||(_cse_temp_0);
    if _cse_temp_1 {
    return b"\0";
   
}
result = b"";
    for label in name.trim_end_matches(| c: char | ".".contains(c)).to_string().split(".").map(| s | s.to_string()).collect::<Vec<String>>() {
    let encoded = label.as_bytes().to_vec();
    result = format!("{}{}", result, format!("{}{}", vec! [encoded.len() as i32].into_iter().map(| x | x as u8).collect::<Vec<u8>>(), encoded));
   
}
format!("{}{}", result, b"\0")
}
#[doc = "Parse DNS question section."] #[doc = " Depyler: proven to terminate"] pub fn parse_question(data: & Vec<u8>, offset: i32) -> Result <(DNSQuestion, i32), Box<dyn std::error::Error>>{
    let(name, pos) = parse_name(& data, offset) ?;
    let _cse_temp_0 = data.len() as i32;
    let _cse_temp_1 = pos + 4>_cse_temp_0;
    if _cse_temp_1 {
    return Err(Box::new(ValueError::new("Question section too short".to_string())));
   
}
let _cse_temp_2 = data.get(pos as usize).cloned().expect("IndexError: list index out of range") <<8;
    let _cse_temp_3 = _cse_temp_2 | {
    let base = & data;
    let idx: i32 = pos + 1;
    let actual_idx = if idx<0 {
    base.len().saturating_sub(idx.abs() as usize)
    ...
    idx as usize };
    base.get(actual_idx).cloned().expect("IndexError: list index out of range") };
    let qtype = _cse_temp_3;
    let qclass = _cse_temp_3;
    Ok((DNSQuestion::new(name, qtype, qclass), pos + 4))
}
#[doc = "Encode DNS question to bytes."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn encode_question(question: & DNSQuestion) -> Vec<u8>{
    let name_bytes = encode_name(question.name);
    let type_bytes = vec! [question.qtype>>8, question.qtype & 255].into_iter().map(| x | x as u8).collect::<Vec<u8>>();
    let class_bytes = vec! [question.qclass>>8, question.qclass & 255].into_iter().map(| x | x as u8).collect::<Vec<u8>>();
    name_bytes + type_bytes + class_bytes
}
#[doc = "Parse DNS resource record."] #[doc = " Depyler: proven to terminate"] pub fn parse_record(data: & Vec<u8>, offset: i32) -> Result <(DNSRecord, i32), Box<dyn std::error::Error>>{
    let(name, mut pos) = parse_name(& data, offset) ?;
    let _cse_temp_0 = data.len() as i32;
    let _cse_temp_1 = pos + 10>_cse_temp_0;
    if _cse_temp_1 {
    return Err(Box::new(ValueError::new("Record too short".to_string())));
...
let _cse_temp_2 = data.get(pos as usize).cloned().expect("IndexError: list index out of range") <<8;
    ...
    idx as usize };
    base.get(actual_idx).cloned().expect("IndexError: list index out of range") };
    let rtype = _cse_temp_3;
    let rclass = _cse_temp_3;
    let _cse_temp_4 = {
    let base = & data;
    let idx: i32 = pos + 4;
    let actual_idx = if idx<0 {
...
<<16;
    ...
    base.get(actual_idx).cloned().expect("IndexError: list index out of range") };
    let ttl = _cse_temp_8;
    let rdlength = _cse_temp_3;
    pos = pos + 10;
    let _cse_temp_9 = pos + rdlength>_cse_temp_0;
    if _cse_temp_9 {
    return Err(Box::new(ValueError::new("Record data too short".to_string())));
   
}
let rdata = {
    let base = & data;
    let start_idx  = (pos) as isize;
    let stop_idx  = (pos + rdlength) as isize;
    let start = if start_idx<0 {
   (base.len() as isize + start_idx).max(0) as usize
}
else {
   ...
   (base.len() as isize + stop_idx).max(0) as usize
    ...
    Ok((DNSRecord::new(name, rtype, rclass, ttl, rdlength, rdata), pos + rdlength))
}
#[doc = "Format resource data for display."] #[doc = " Depyler: proven to terminate"] pub fn format_rdata<'b, 'a>(rtype: i32, rdata: & 'a Vec<u8>, full_data: & 'b Option<Vec<u8>>) -> Result<String, Box<dyn std::error::Error>>{
    let mut __sanitized: DepylerValue = Default::default();
    let mut name: DepylerValue = Default::default();
    let _cse_temp_0 = rtype == 1;
    let _cse_temp_1 = rdata.len() as i32;
    let _cse_temp_2 = _cse_temp_1 == 4;
    let _cse_temp_3  = (_cse_temp_0) &&(_cse_temp_2);
    if _cse_temp_3 {
    return Ok(format!("{}.{}.{}.{}", rdata.get(0usize).cloned().expect("IndexError: list index out of range"), rdata.get(1usize).cloned().expect("IndexError: list index out of range"), rdata.get(2usize).cloned().expect("IndexError: list index out of range"), rdata.get(3usize).cloned().expect("IndexError: list index out of range")));
   
}
let _cse_temp_4 = rtype == 28;
    let _cse_temp_5 = _cse_temp_1 == 16;
    let _cse_temp_6  = (_cse_temp_4) &&(_cse_temp_5);
    if _cse_temp_6 {
    let mut parts = vec! [];
    for i in {
...
{
    parts.push(DepylerValue::Str(format!("{:?}", format!("{}{}", rdata.get(i as usize).cloned().expect("IndexError: list index out of range"), {
    let base = & rdata;
    let idx: i32 = i + 1;
    let actual_idx = if idx<0 {
    base.len().saturating_sub(idx.abs() as usize)
...
let _cse_temp_7 = [2, 5, 12].contains(& rtype);
    if _cse_temp_7 {
    if let Some(ref full_data_val) = full_data {
   (name, __sanitized) = parse_name(full_data_val, full_data_val.iter().position(| x | x == & rdata).map(| i | i as i32).expect("ValueError: value is not in list")) ?;
    return Ok(name.to_string());
   
}
return Ok(rdata.bytes().map(| b | format!("{:02x}", b)).collect::<String>());
   
}
let _cse_temp_8 = rtype == 15;
    let _cse_temp_9 = _cse_temp_1>= 3;
    let _cse_temp_10  = (_cse_temp_8) &&(_cse_temp_9);
    if _cse_temp_10 {
    let _cse_temp_11 = rdata.get(0usize).cloned().expect("IndexError: list index out of range") <<8;
    let _cse_temp_12 = _cse_temp_11 | rdata.get(1usize).cloned().expect("IndexError: list index out of range");
    let preference = _cse_temp_12;
    if let Some(ref full_data_val) = full_data {
   (name, __sanitized) = parse_name(full_data_val, full_data_val.iter().position(| x | x == & rdata).map(| i | i as i32).expect("ValueError: value is not in list") + 2) ?;
    return Ok(format!("{} {:?}", preference, name));
   
}
return Ok(format!("{} {}", preference, {
    let base = & rdata;
    let start_idx  = (2) as isize;
    let start = if start_idx<0 {
   (base.len() as isize + start_idx).max(0) as usize
}
else {
...
}.bytes().map(| b | format!("{:02x}", b)).collect::<String>()));
   
}
let _cse_temp_13 = rtype == 16;
    if _cse_temp_13 {
    let mut texts = vec! [];
    let mut pos = 0;
    while pos<rdata.len() as i32 {
    let length = rdata.get(pos as usize).cloned().expect("IndexError: list index out of range");
    pos = pos + 1;
    if pos + length <= rdata.len() as i32 {
    texts.push(DepylerValue::Str(format!("{:?}", String::from_utf8_lossy(& {
    let base = & rdata;
    let start_idx  = (pos) as isize;
    let stop_idx  = (pos + length) as isize;
    let start = if start_idx<0 {
...
return Ok(texts.iter().cloned().map(| t | format!("\"{:?}\"", t)).collect::<Vec<_>>().join (" "));
   
}
Ok(rdata.bytes().map(| b | format!("{:02x}", b)).collect::<String>())
}
#[doc = "Build a DNS query message."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn build_query(domain: String, qtype: & str, id_: i32) -> Vec<u8>{
    let header = DNSHeader::new(id_, 0, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0);
    let type_num = DNS_TYPES_REVERSE.get(& qtype.to_uppercase()).cloned().unwrap_or(1);
    let question = DNSQuestion::new(domain, type_num, 1);
    encode_header(& header) + encode_question(& question)
}
#[doc = "Parse complete DNS message."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn parse_message(data: & Vec<u8>) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>>{
    let mut pos: i32 = Default::default();
    let mut record: DepylerValue = Default::default();
    let header = parse_header(& data) ?;
    let result = {
    let mut map = HashMap::new();
    map.insert("header".to_string(), DepylerValue::Str(format!("{:?}", {
    let mut map = HashMap::new();
    map.insert("id".to_string(), DepylerValue::Int(header.id as i64));
    map.insert("qr".to_string(), DepylerValue::Str(format!("{:?}", if header.qr {
    "response".to_string()
}
else {
    "query" })));
    map.insert("opcode".to_string(), DepylerValue::Int(header.opcode as i64));
    map.insert("rcode".to_string(), DepylerValue::Int(header.rcode as i64));
    map.insert("flags".to_string(), DepylerValue::Str(format!("{:?}", {
    let mut map = HashMap::new();
    map.insert("aa".to_string(), DepylerValue::Str(format!("{:?}", header.aa!= 0)));
    map.insert("tc".to_string(), DepylerValue::Str(format!("{:?}", header.tc!= 0)));
    map.insert("rd".to_string(), DepylerValue::Str(format!("{:?}", header.rd!= 0)));
    map.insert("ra".to_string(), DepylerValue::Str(format!("{:?}", header.ra!= 0)));
    map })));
    map })));
    map.insert("questions".to_string(), DepylerValue::Str(format!("{:?}", vec! [])));
    map.insert("answers".to_string(), DepylerValue::Str(format!("{:?}", vec! [])));
    map.insert("authority".to_string(), DepylerValue::Str(format!("{:?}", vec! [])));
    map.insert("additional".to_string(), DepylerValue::Str(format!("{:?}", vec! [])));
    map };
    pos = 12;
    for __sanitized in 0..(header.qdcount) {
    let(question, mut pos) = parse_question(& data, pos) ?;
    result.get("questions").cloned().unwrap_or_default().push({ let mut map = HashMap::new();
    map.insert(std::borrow::Cow::Borrowed("name").to_string(), DepylerValue::Str(question.name.to_string()));
    map.insert("type".to_string().to_string(), DepylerValue::Str(format!("{:?}", DNS_TYPES.get(& question.qtype).cloned().unwrap_or((question.qtype).to_string()))));
    map.insert("class".to_string().to_string(), DepylerValue::Str(format!("{:?}", DNS_CLASSES.get(& question.qclass).cloned().unwrap_or((question.qclass).to_string()))));
    map });
   
}
for __sanitized in 0..(header.ancount) {
   (record, pos) = parse_record(& data, pos) ?;
    result.get("answers").cloned().unwrap_or_default().push({ let mut map = HashMap::new();
    map.insert(std::borrow::Cow::Borrowed("name").to_string(), DepylerValue::Str(record.name.to_string()));
    map.insert("type".to_string().to_string(), DepylerValue::Str(format!("{:?}", DNS_TYPES.get(& record.rtype).cloned().unwrap_or((record.rtype).to_string()))));
    map.insert("class".to_string().to_string(), DepylerValue::Str(format!("{:?}", DNS_CLASSES.get(& record.rclass).cloned().unwrap_or((record.rclass).to_string()))));
    map.insert("ttl".to_string().to_string(), DepylerValue::Int(record.ttl as i64));
    map.insert("data".to_string().to_string(), DepylerValue::Str(format!("{:?}", format_rdata(record.rtype, record.rdata, & data))));
    map });
   
}
for __sanitized in 0..(header.nscount) {
   (record, pos) = parse_record(& data, pos) ?;
    result.get("authority").cloned().unwrap_or_default().push({ let mut map = HashMap::new();
    map.insert(std::borrow::Cow::Borrowed("name").to_string(), DepylerValue::Str(record.name.to_string()));
    map.insert("type".to_string().to_string(), DepylerValue::Str(format!("{:?}", DNS_TYPES.get(& record.rtype).cloned().unwrap_or((record.rtype).to_string()))));
    map.insert("ttl".to_string().to_string(), DepylerValue::Int(record.ttl as i64));
    map });
...
for __sanitized in 0..(header.arcount) {
   (record, pos) = parse_record(& data, pos) ?;
    result.get("additional").cloned().unwrap_or_default().push({ let mut map = HashMap::new();
    map.insert(std::borrow::Cow::Borrowed("name").to_string(), DepylerValue::Str(record.name.to_string()));
    map.insert("type".to_string().to_string(), DepylerValue::Str(format!("{:?}", DNS_TYPES.get(& record.rtype).cloned().unwrap_or((record.rtype).to_string()))));
    map.insert("ttl".to_string().to_string(), DepylerValue::Int(record.ttl as i64));
    map });
...
#[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), Box<dyn std::error::Error>>{
    ...
    let has_domain = args.domain.is_some();
    let has_hex = args.hex.is_some();
    let _cse_temp_0 = args.mode == "query";
    let _cse_temp_1  = (_cse_temp_0) &&(args.domain);
    if _cse_temp_1 {
    let query = build_query(args.domain, & args.r#type, 1234i32);
    println!("{}", format!("DNS Query for {}({}):", {
    match & args.domain {
    Some(v) =>format!("{}", v), None =>"None".to_string() ,
}
}, args.r#type));
    println!("{}", format!("Hex: {}", query.bytes().map(| b | format!("{:02x}", b)).collect::<String>()));
    println!("{}", format!("Length: {} bytes", query.len() as i32));
   
}
else {
    let _cse_temp_2 = args.mode == "parse";
    let _cse_temp_3  = (_cse_temp_2) &&(args.hex);
    if _cse_temp_3 {
    let data  = (args.hex.replace(" ", "")).as_bytes().chunks(2).map(| c | u8::from_str_radix(std::str::from_utf8(c).unwrap(), 16).unwrap()).collect::<Vec<u8>>();
    let result = parse_message(& data) ?;
    println!("{}", format!("ID: {}", result.get("header").cloned().unwrap_or_default().get("id").cloned().unwrap_or_default()));
    println!("{}", format!("Type: {}", result.get("header").cloned().unwrap_or_default().get("qr").cloned().unwrap_or_default()));
    println!("{}", format!("Opcode: {}", result.get("header").cloned().unwrap_or_default().get("opcode").cloned().unwrap_or_default()));
    println!("{}", format!("Response code: {}", result.get("header").cloned().unwrap_or_default().get("rcode").cloned().unwrap_or_default()));
    for q in result.get("questions").cloned().unwrap_or_default().as_array().unwrap_or(& vec! []).iter().cloned() {
    println!("{}", format!("Question: {} {} {}", q.get("name").cloned().unwrap_or_default(), q.get("type").cloned().unwrap_or_default(), q.get("class").cloned().unwrap_or_default()));
   
}
for a in result.get("answers").cloned().unwrap_or_default().as_array().unwrap_or(& vec! []).iter().cloned() {
    println!("{}", format!("Answer: {} {} TTL={} -> {}", a.get("name").cloned().unwrap_or_default(), a.get("type").cloned().unwrap_or_default(), a.get("ttl").cloned().unwrap_or_default(), a.get("data").cloned().unwrap_or_default()));
   
}
} else {
    let _cse_temp_4 = args.mode == "encode-name";
    let _cse_temp_5  = (_cse_temp_4) &&(args.domain);
    if _cse_temp_5 {
    let encoded = encode_name(& args.domain);
    println!("{}", format!("Domain: {}", {
    match & args.domain {
    Some(v) =>format!("{}", v), None =>"None".to_string() ,
}
}));
    println!("{}", format!("Encoded: {}", encoded.bytes().map(| b | format!("{:02x}", b)).collect::<String>()));
   
}
else {
    println!("{}", "Usage: dns_cli.py --mode query --domain example.com");
    println!("{}", "       dns_cli.py --mode parse --hex <hex_data>");
