...
#[doc = "Compute absolute value of integer.\n\n   >>>compute_abs_int(-10)\n    10\n   >>>compute_abs_int(0)\n    0\n   >>>compute_abs_int(42)\n    42\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compute_abs_int(x: i32) -> i32 {
    let _cse_temp_0 = x < 0;
    if _cse_temp_0 {
        return -x;
    }
...
#[doc = "Compute absolute value of float.\n\n   >>>compute_abs_float(-2.5)\n    2.5\n   >>>compute_abs_float(0.0)\n    0.0\n   >>>compute_abs_float(1.5)\n    1.5\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compute_abs_float(x: f64) -> f64 {
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Int { .. }));
    match &args.command {
        Some(Commands::Int { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_abs_int(x));
        }
        Some(Commands::Float { ref x, .. }) => {
            let x = *x;
            println!("{}", compute_abs_float(x));
        }
...
mod tests {
    ...
    #[test]
    fn quickcheck_compute_abs_int() {
        fn prop(x: i32) -> TestResult {
            let result = compute_abs_int(x.clone());
            if result < 0 {
    ...
    #[test]
    fn test_compute_abs_int_examples() {
        assert_eq!(compute_abs_int(0), 0);
        assert_eq!(compute_abs_int(1), 1);
        assert_eq!(compute_abs_int(-1), 1);
        assert_eq!(compute_abs_int(i32::MIN + 1), i32::MAX);
    }
    #[test]
    fn quickcheck_compute_abs_float() {
        fn prop(x: f64) -> TestResult {
            let result = compute_abs_float(x.clone());
            if result < 0 {
                return TestResult::failed();
            }
            TestResult::passed()
