...
pub fn replace_char<'b, 'c, 'a>(text: &'a str, old: &'b str, new: &'c str) -> String {
    text.replace(old, new)
}
#[doc = "Replace each character with new.\n\n   >>>replace_all(\"abc\", \"X\")\n    'XXX'\n   >>>replace_all(\"\", \"X\")\n    ''\n   >>>replace_all(\"hello\", \"*\")\n    '*****'\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn replace_all<'a, 'b>(text: &'a str, new: &'b str) -> String {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Char { .. }));
    match &args.command {
        Some(Commands::Char {
            ref new,
            ref old,
            ...
            println!("{}", replace_char(&text, &old, &new));
        }
        Some(Commands::First {
            ref new, ref text, ..
        }) => {
