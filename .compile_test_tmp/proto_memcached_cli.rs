...
impl Opcode {
    ...
    pub const SET: i32 = 1;
    pub const ADD: i32 = 2;
    pub const REPLACE: i32 = 3;
    pub const DELETE: i32 = 4;
    pub const INCREMENT: i32 = 5;
    pub const DECREMENT: i32 = 6;
    pub const QUIT: i32 = 7;
    pub const FLUSH: i32 = 8;
    pub const GETQ: i32 = 9;
    pub const NOOP: i32 = 10;
    pub const VERSION: i32 = 11;
    pub const GETK: i32 = 12;
    pub const GETKQ: i32 = 13;
    pub const APPEND: i32 = 14;
    pub const PREPEND: i32 = 15;
    pub fn new() -> Self {
        Self {}
    }
}
...
impl Status {
    ...
    pub const KEY_EXISTS: i32 = 2;
    pub const VALUE_TOO_LARGE: i32 = 3;
    pub const INVALID_ARGUMENTS: i32 = 4;
    pub const ITEM_NOT_STORED: i32 = 5;
    pub const NON_NUMERIC_VALUE: i32 = 6;
    pub const UNKNOWN_COMMAND: i32 = 129;
    pub const OUT_OF_MEMORY: i32 = 130;
    pub fn new() -> Self {
        Self {}
    }
}
...
pub struct Magic {}
impl Magic {
    pub const REQUEST: i32 = 128;
    pub const RESPONSE: i32 = 129;
    pub fn new() -> Self {
        Self {}
    }
}
...
pub struct RequestHeader {
    pub opcode: Opcode,
    pub key_length: i32,
    pub extras_length: i32,
    pub data_type: i32,
    pub vbucket_id: i32,
    pub total_body_length: i32,
    pub opaque: i32,
    pub cas: i32,
}
impl RequestHeader {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        return vec![
            ...
            (vec![
                Magic::REQUEST,
                self.opcode.clone(),
                self.key_length.clone() >> 8 & 255,
                self.key_length.clone() & 255,
                self.extras_length.clone(),
                self.data_type.clone(),
                self.vbucket_id.clone() >> 8 & 255,
                self.vbucket_id.clone() & 255,
                self.total_body_length.clone() >> 24 & 255,
                self.total_body_length.clone() >> 16 & 255,
                self.total_body_length.clone() >> 8 & 255,
                self.total_body_length.clone() & 255,
                self.opaque.clone() >> 24 & 255,
                self.opaque.clone() >> 16 & 255,
                self.opaque.clone() >> 8 & 255,
                self.opaque.clone() & 255,
                self.cas.clone() >> 56 & 255,
                self.cas.clone() >> 48 & 255,
                self.cas.clone() >> 40 & 255,
                self.cas.clone() >> 32 & 255,
                self.cas.clone() >> 24 & 255,
                self.cas.clone() >> 16 & 255,
                self.cas.clone() >> 8 & 255,
                self.cas.clone() & 255
            ]) as usize
        ];
    }
...
impl ResponseHeader {
    ...
    pub fn from_bytes(data: &Vec<u8>) -> ResponseHeader {
        if (data.len() as i32) < HEADER_SIZE {
            panic!(
                "Exception: {}",
                ValueError::new("Insufficient data for header".to_string())
            );
        };
        if data.get(&0).cloned().unwrap_or_default() != Magic::RESPONSE {
            panic!(
                "Exception: {}",
...
impl GetRequest {
    pub fn new(key: String, opaque: i32) -> Self {
        Self { key, opaque }
    }
    pub fn to_bytes(&self) -> Vec<u8> {
        let key_bytes = self.key.clone().encode("utf-8".to_string());
...
impl SetRequest {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        ...
        let extras = vec![
            0u8;
            (vec![
                self.flags.clone() >> 24 & 255,
                self.flags.clone() >> 16 & 255,
                self.flags.clone() >> 8 & 255,
                self.flags.clone() & 255,
                self.expiration.clone() >> 24 & 255,
                self.expiration.clone() >> 16 & 255,
                self.expiration.clone() >> 8 & 255,
                self.expiration.clone() & 255
            ]) as usize
        ];
        let header = RequestHeader::new();
        return header.to_bytes() + extras + key_bytes + self.value.clone();
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl IncrDecrRequest {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let key_bytes = self.key.clone().encode("utf-8".to_string());
        let extras = self.delta.clone().to_bytes(8, "big".to_string())
            + self.initial.clone().to_bytes(8, "big".to_string())
            + self.expiration.clone().to_bytes(4, "big".to_string());
        let opcode = if self.is_increment.clone() {
            Opcode::INCREMENT
        } else {
            Opcode::DECREMENT
        };
...
impl Response {
    pub fn new(header: ResponseHeader, extras: Vec<u8>, key: Vec<u8>, value: Vec<u8>) -> Self {
        Self {
            header,
            extras,
            key,
    ...
    pub fn from_bytes(data: &Vec<u8>) -> Response {
        let header = ResponseHeader::from_bytes(data);
        let mut pos = HEADER_SIZE;
        let extras = {
            let s = &data;
            let len = s.chars().count() as isize;
            let start_idx = (pos) as isize;
            let stop_idx = (pos + header.extras_length) as isize;
        ...
        let mut pos = pos + header.extras_length;
        let key = {
            let s = &data;
            let len = s.chars().count() as isize;
            let start_idx = (pos) as isize;
            let stop_idx = (pos + header.key_length) as isize;
            let start = if start_idx < 0 {
                (len + start_idx).max(0) as usize
            } else {
                start_idx as usize
        ...
        let mut pos = pos + header.key_length;
        let value_length = header.total_body_length - header.extras_length - header.key_length;
        let value = {
            let s = &data;
            let len = s.chars().count() as isize;
            let start_idx = (pos) as isize;
            let stop_idx = (pos + value_length) as isize;
            let start = if start_idx < 0 {
                (len + start_idx).max(0) as usize
            } else {
                start_idx as usize
        ...
        return Self::new(header, extras, key, value);
    }
    pub fn is_success(&self) -> bool {
        return self.header.clone().status == Status::NO_ERROR;
    }
}
#[derive(Debug, Clone)]
...
impl MemcachedClient {
    ...
    pub fn encode_get(key: String) -> Vec<u8> {
        return GetRequest::new(key).to_bytes();
    }
    pub fn encode_set(key: String, value: &Vec<u8>, flags: i32, expiry: i32) -> Vec<u8> {
        return SetRequest::new(key, value, flags, expiry).to_bytes();
    }
    pub fn encode_delete(key: String) -> Vec<u8> {
        return DeleteRequest::new(key).to_bytes();
    }
    pub fn encode_incr(key: String, delta: i32) -> Vec<u8> {
        return IncrDecrRequest::new(key, delta).to_bytes();
    }
    pub fn encode_decr(key: String, delta: i32) -> Vec<u8> {
        return IncrDecrRequest::new(key, delta).to_bytes();
    }
    ...
    pub fn decode_response(data: &Vec<u8>) -> Response {
...
#[doc = "Simulate memcached operations."]
pub fn simulate_memcached(
    ...
    for op in operations.iter().cloned() {
        ...
        if cmd == "get" {
            ...
            if cmd == "set" {
                ...
                data = client.encode_set(key, value.as_bytes().to_vec());
                results.push(format!("SET key={} len={}", key, data.len() as i32));
            } else {
                if cmd == "delete" {
                    data = client.encode_delete(
                        parts
                            .get(1usize)
                            .cloned()
                            .expect("IndexError: list index out of range"),
                    ...
                    if cmd == "incr" {
                        ...
                        results.push(format!("INCR key={} len={}", key, data.len() as i32));
                    } else {
                        if cmd == "decr" {
                            key = if parts.len() as i32 > 1 {
                                parts
                            ...
                            results.push(format!("DECR key={} len={}", key, data.len() as i32));
                        } else {
                            if cmd == "flush" {
                                data = client.encode_flush();
                                results.push(format!("FLUSH len={}", data.len() as i32));
                            } else {
                                if cmd == "noop" {
                                    data = client.encode_noop();
                                    results.push(format!("NOOP len={}", data.len() as i32));
                                } else {
                                    if cmd == "version" {
                                        data = client.encode_version();
                                        results.push(format!("VERSION len={}", data.len() as i32));
                                    }
                                }
                            }
                        }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: proto_memcached_cli.py <command>[args]");
    ...
    if _cse_temp_4 {
        ...
        println!("{}", format!("GET request: {} bytes", data.len() as i32));
        println!(
            "{}",
            format!(
                "Hex: {}",
        ...
        if _cse_temp_7 {
            ...
            println!("{}", format!("SET request: {} bytes", data.len() as i32));
        } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_memcached_examples() {
        assert_eq!(simulate_memcached(vec![]), vec![]);
        assert_eq!(simulate_memcached(vec![1]), vec![1]);
    }
}
