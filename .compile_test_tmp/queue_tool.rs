...
enum Commands {
    #[default]
    __DepylerNone,
    First { items: Vec<String> },
    Last { items: Vec<String> },
}
#[derive(Default)]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::First { .. }));
    match &args.command {
        Some(Commands::First { ref items, .. }) => {
            println!(
                "{}",
                items
                    .get(0usize)
