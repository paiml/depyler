...
struct Args {
    ...
    #[doc = "Use regex for find/replace"]
    regex: bool,
    ...
    #[doc = "Trim N chars from start"]
    trim_start: Option<i32>,
    
    #[doc = "Trim N chars from end"]
    trim_end: Option<i32>,
    
    
    #[doc = "Change extension"]
    ext: Option<String>,
    
    
    #[doc = "Filter files by regex"]
    filter: Option<String>,
    
    
    #[doc = "Process subdirectories"]
    recursive: bool,
    
    
    #[doc = "Preview without renaming"]
    dry_run: bool,
}
#[doc = "Apply find/replace pattern to filename."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn apply_pattern<'a, 'b, 'c>(
    filename: &'a str,
    find: &'b str,
    replace: &'c str,
    use_regex: bool,
    ...
    if use_regex {
        return DepylerRegexMatch::sub(find, replace, filename);
    }
    filename.replace(find, replace)
}
#[doc = "Apply case transformation to filename."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn apply_case(filename: String, case: &str) -> String {
    let (name, ext) = {
    ...
    let _cse_temp_0 = (*case) == "lower";
    if _cse_temp_0 {
        return name.to_lowercase() + ext.to_lowercase();
    } else {
        let _cse_temp_1 = (*case) == "upper";
        if _cse_temp_1 {
            return name.to_uppercase() + ext.to_uppercase();
        } else {
            let _cse_temp_2 = (*case) == "title";
            if _cse_temp_2 {
                return name
                    .split_whitespace()
                    .map(|word| {
                ...
                let _cse_temp_3 = (*case) == "snake";
                let mut s;
                if _cse_temp_3 {
                    s = DepylerRegexMatch::sub("([A-Z])", "_\\1", name);
                    s = DepylerRegexMatch::sub("[\\s-]+", "_", s);
                    s = DepylerRegexMatch::sub("_+", "_", s);
                    return s
                        .trim_matches(|c: char| "_".to_string().contains(c))
                        .to_string()
                        .to_lowercase()
                        + ext.to_lowercase();
                } else {
                    let _cse_temp_4 = (*case) == "kebab";
                    if _cse_temp_4 {
                        s = DepylerRegexMatch::sub("([A-Z])", "-\\1", name);
                        s = DepylerRegexMatch::sub("[\\s_]+", "-", s);
                        s = DepylerRegexMatch::sub("-+", "-", s);
                        return s
                            .trim_matches(|c: char| "-".to_string().contains(c))
                            .to_string()
                            .to_lowercase()
                            + ext.to_lowercase();
                    }
...
pub fn apply_numbering(
    ...
    let _cse_temp_0 = (*position) == "prefix";
    if _cse_temp_0 {
        return format!("{:?}_{:?}{:?}", num, name, ext);
    } else {
        let _cse_temp_1 = (*position) == "suffix";
        if _cse_temp_1 {
            return format!("{:?}_{:?}{:?}", name, num, ext);
        }
    }
    filename.to_string()
}
#[doc = "Trim characters from filename."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn apply_trim<'a, 'b>(filename: &'a str, chars: i32, position: &'b str) -> String {
    ...
    let _cse_temp_0 = (*position) == "start";
    if _cse_temp_0 {
        name = {
            let base = &name;
            let start_idx = (chars) as isize;
        ...
        let _cse_temp_1 = (*position) == "end";
        if _cse_temp_1 {
            name = if chars > 0 {
                {
                    let base = &name;
                    let stop_idx = (-chars) as isize;
    ...
    name.iter().chain(ext.iter()).cloned().collect::<Vec<_>>()
}
#[doc = "Change file extension."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn apply_extension(filename: &str, mut new_ext: String) -> String {
    let (name, __sanitized) = {
        let p = std::path::Path::new(&filename);
        let stem = p
            .file_stem()
            .and_then(|s| s.to_str())
    ...
    if !new_ext.starts_with(".") {
        new_ext = format!("{}{}", ".", new_ext);
    }
    name + new_ext
}
#[doc = "Preview renames without applying.\n\n    Returns list of(old_name, new_name) tuples.\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn preview_rename(
    files: &Vec<String>,
    transform_fn: impl Fn(i32, i32) -> i32,
) -> Vec<(String, String)> {
    let mut result = vec![];
    ...
    {
        let i = i as i32;
        let dirname = std::path::Path::new(&filepath)
            .parent()
            .and_then(|p| p.to_str())
            .unwrap_or("")
            .to_string();
        let filename = std::path::Path::new(&filepath)
            ...
            .unwrap_or("")
            .to_string();
        let new_filename = transform_fn(filename, i);
        if new_filename != filename {
            let new_path = std::path::PathBuf::from(dirname)
                .join(new_filename)
                .to_string_lossy()
                .to_string();
            result.push(DepylerValue::Str(format!("{:?}", (filepath, new_path))));
        }
    }
    result
}
#[doc = "Check for naming conflicts."]
#[doc = " Depyler: verified panic-free"]
pub fn check_conflicts(renames: &Vec<(String, String)>) -> Vec<String> {
    let mut conflicts = vec![];
    let mut new_names = {
        let mut map = HashMap::new();
        map
    };
    for (old, new) in renames.iter().cloned() {
        if new_names.get(&new).is_some() {
            conflicts.push(DepylerValue::Str(format!(
                "{:?}",
                format!(
                    "Conflict: {} and {} -> {}",
                    old,
                    new_names.get(&new).cloned().unwrap_or_default(),
                    new
                )
            )));
        }
...
#[doc = " Depyler: verified panic-free"]
pub fn execute_renames(renames: &Vec<(String, String)>, dry_run: bool) -> (i32, i32) {
    let mut errors: i32 = Default::default();
    let mut success: i32 = Default::default();
    success = 0;
    errors = 0;
    for (old, new) in renames.iter().cloned() {
        if dry_run {
        ...
        let mut new_dir: Option<String> = None;
        match (|| -> Result<(), Box<dyn std::error::Error>> {
            ...
            if (new_dir.is_some()) && (!std::path::Path::new(&new_dir).exists()) {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn list_files<'a, 'b>(directory: &'a str, pattern: &'b str, recursive: bool) -> Vec<String> {
    let mut files = vec![];
    if recursive {
        for (root, __sanitized, filenames) in std::fs::read_dir(directory)
            .into_iter()
            .filter_map(|e| e.ok())
            ...
            .collect::<Vec<_>>()
        {
            for filename in filenames.iter().cloned() {
                if (!pattern.is_empty()) && (DepylerRegexMatch::search(pattern, filename).is_none())
                {
                    continue;
                }
                files.push(DepylerValue::Str(format!(
                    "{:?}",
            ...
            .collect::<Vec<_>>()
        {
            ...
            if (!pattern.is_empty()) && (DepylerRegexMatch::search(pattern, entry).is_none()) {
                continue;
            }
            files.push(DepylerValue::Str(path.to_string()));
        }
    }
    {
        let mut sorted_vec = files.iter().cloned().collect::<Vec<_>>();
...
#[doc = " Depyler: verified panic-free"]
pub fn main() {
    let mut files: Vec<DepylerValue> = Default::default();
    let args = Args::default();
    let has_path = args.path.is_some();
    let has_find = args.find.is_some();
    let has_case = args.case.is_some();
    let has_trim_start = args.trim_start.is_some();
    let has_trim_end = args.trim_end.is_some();
    let has_ext = args.ext.is_some();
    ...
    if std::path::Path::new(&args.path).is_file() {
        ...
        if std::path::Path::new(&args.path).is_dir() {
            ...
            eprintln!("{}", format!("Error: {} not found", args.path));
    ...
    let transform = move |filename: &str, index: i32| -> String {
    ...
    let renames = preview_rename(&files, transform);
    if renames.is_empty() {
        println!("{}", "No files to rename");
        return;
    }
    let conflicts = check_conflicts(&renames);
    ...
    for (old, new) in renames.iter().cloned() {
        ...
        let new_name = std::path::Path::new(&new)
            ...
            .unwrap_or("")
            .to_string();
        println!("{}", format!("  {:?} -> {:?}", old_name, new_name));
    }
    let (success, errors) = execute_renames(&renames, args.dry_run);
    println!(
        "{}",
        format!("\nRenamed: {:?}, Errors: {:?}", success, errors)
    );
    let _ = if errors == 0 { 0 } else { 1 };
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    #[test]
    fn test_check_conflicts_examples() {
        assert_eq!(check_conflicts(vec![]), vec![]);
        assert_eq!(check_conflicts(vec![1]), vec![1]);
    }
}
