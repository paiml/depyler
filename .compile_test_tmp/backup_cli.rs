...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'shutil'(tracked in DEPYLER-0424)"]
use std::io::Read;
use std::io::Write;
#[derive(Debug, Clone)]
...
enum Commands {
    ...
    List {
        ...
        #[doc = "Backup directory"]
        dir: Option<String>,
        
        #[doc = "Filter by pattern"]
    ...
    Backup {
        ...
        #[doc = "Verify backup checksum"]
        verify: bool,
    },
    
    Restore {
        #[doc = "Backup file or 'latest'"]
        backup: String,
        ...
        #[doc = "Pattern for latest lookup"]
        pattern: Option<String>,
        
        #[doc = "Verify restore checksum"]
        verify: bool,
    },
}
#[derive(Default)]
...
#[doc = "Get current timestamp for backup naming."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_timestamp() -> String {
    format!("{:?}", DepylerDateTime::now())
}
#[doc = "Parse timestamp from backup name."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_timestamp(name: &str) -> Result<Option<DepylerDateTime>, Box<dyn std::error::Error>> {
    let parts = name
        .split("_")
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
    ...
    let mut date_part: i32 = Default::default();
    let mut time_part: i32 = Default::default();
    let mut micro_part: String = Default::default();
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        date_part = {
            let base = &parts;
            base.get(base.len().saturating_sub(3usize))
                .cloned()
                .unwrap_or_default()
        };
        time_part = {
            let base = &parts;
            base.get(base.len().saturating_sub(2usize))
                .cloned()
                .unwrap_or_default()
        };
        micro_part = {
...
#[doc = "Calculate SHA256 checksum of file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_file_checksum(path: &str) -> Result<String, Box<dyn std::error::Error>> {
    let mut hasher = {
...
#[doc = "Create backup filename with timestamp."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn create_backup_name<'b, 'a>(source: &'a str, backup_dir: &'b str) -> String {
    let basename = std::path::Path::new(&source)
        ...
        .to_string();
    let (name, ext) = {
        let p = std::path::Path::new(&basename);
        let stem = p
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("")
    ...
    let backup_name = format!("{:?}_{:?}{:?}", name, timestamp, ext);
    std::path::PathBuf::from(backup_dir)
        .join(backup_name)
        .to_string_lossy()
        .to_string()
}
#[doc = "Create a backup of a file.\n\n    Returns backup path on success, empty string on failure.\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn backup_file<'b, 'a>(
    source: &'a str,
    backup_dir: &'b str,
    verify: bool,
    ...
    let backup_path = create_backup_name(source, backup_dir);
    let mut src_checksum: Option<String> = None;
    let mut bak_checksum: Option<String> = None;
    match (|| -> Result<String, Box<dyn std::error::Error>> {
        {
            std::fs::copy(&source, &backup_path).unwrap();
            backup_path.clone()
        };
        if verify {
            src_checksum = Some(get_file_checksum(source)?);
            bak_checksum = Some(get_file_checksum(backup_path.display().to_string())?);
            if src_checksum != bak_checksum {
                std::fs::remove_file(backup_path).unwrap();
                return Ok("".to_string());
            }
        }
        return Ok(backup_path.to_string());
    })() {
        Ok(_result) => {
            return Ok(_result);
        }
...
#[doc = "List backups in directory, optionally filtered by pattern.\n\n    Returns list of(path, timestamp) tuples sorted by date.\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn list_backups<'b, 'a>(
    backup_dir: &'a str,
    pattern: &'b str,
) -> Result<Vec<(String, DepylerDateTime)>, Box<dyn std::error::Error>> {
    ...
    for entry in std::fs::read_dir(backup_dir)?
        ...
        .collect::<Vec<_>>()
    {
        if (!pattern.is_empty()) && (!entry.contains(&*pattern)) {
            continue;
        }
        ...
        if let Some(ref timestamp_val) = timestamp {
            backups.push(DepylerValue::Str(format!("{:?}", (path, timestamp_val))));
        }
    }
    Ok({
        let mut __sorted_result = backups.clone();
...
#[doc = "Get the most recent backup matching pattern."]
#[doc = " Depyler: proven to terminate"]
pub fn get_latest_backup<'a, 'b>(
    backup_dir: &'a str,
    pattern: &'b str,
) -> Result<String, Box<dyn std::error::Error>> {
    let backups = list_backups(backup_dir, pattern)?;
    if backups.is_empty() {
        return Ok("".to_string());
    }
    Ok(backups
...
#[doc = "Restore a backup to destination."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn restore_backup<'b, 'a>(
    backup_path: &'a str,
    dest: &'b str,
    verify: bool,
    ...
    let mut src_checksum: Option<String> = None;
    let mut dst_checksum: Option<String> = None;
    match (|| -> Result<bool, Box<dyn std::error::Error>> {
        ...
        if verify {
            src_checksum = Some(get_file_checksum(backup_path)?);
            dst_checksum = Some(get_file_checksum(dest)?);
...
#[doc = "Remove old backups, keeping only the most recent N.\n\n    Returns number of backups removed.\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn prune_backups<'a, 'b>(
    backup_dir: &'a str,
    pattern: &'b str,
    keep: i32,
) -> Result<i32, std::io::Error> {
    let mut removed: i32 = Default::default();
    let backups = list_backups(backup_dir, pattern)?;
    let _cse_temp_0 = backups.len() as i32;
    let _cse_temp_1 = _cse_temp_0 <= keep;
    if _cse_temp_1 {
        return Ok(0);
    }
    removed = 0;
    for (path, __sanitized) in {
        let base = &backups;
        let start_idx = (keep) as isize;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut backup_path: DepylerValue = Default::default();
    ();
    ();
    ();
    ();
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Backup { .. }));
    match &args.command {
        Some(Commands::Backup {
            ...
            let verify = *verify;
            let result = backup_file(&source, &dest, verify)?;
            if result {
                println!("{}", format!("Created backup: {:?}", result));
                return Ok(());
            }
        ...
        Some(Commands::List {
            ...
            let dir = dir.as_deref().unwrap_or_default();
            let pattern = pattern.as_deref().unwrap_or_default();
            let backups = list_backups(&dir, &pattern)?;
            if backups.is_empty() {
                println!("{}", "No backups found");
                return Ok(());
            }
            for (path, timestamp) in backups.iter().cloned() {
                ...
                println!(
                    "{}",
                    format!("{} {:?} {:?}", format!("{:?}", timestamp), size, name)
                );
            }
            Ok(())
        }
        Some(Commands::Restore {
            ...
            let pattern = pattern.as_deref().unwrap_or_default();
            let verify = *verify;
            let _cse_temp_3 = backup == "latest";
            if _cse_temp_3 {
                backup_path = get_latest_backup(&dir, &pattern)?;
                if !backup_path {
            ...
            if restore_backup(backup_path.display().to_string(), &dest, verify).unwrap_or(false) {
                println!("{}", format!("Restored {:?} to {}", backup_path, dest));
                return Ok(());
            }
        ...
        Some(Commands::Prune {
            ...
            let keep = keep.clone();
            let pattern = pattern.as_deref().unwrap_or_default();
            let removed = prune_backups(&dir, &pattern, keep)?;
            println!("{}", format!("Removed {:?} old backups", removed));
            Ok(())
        }
        _ => {}
    }
