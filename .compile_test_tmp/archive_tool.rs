...
enum Commands {
    #[default]
    __DepylerNone,
    Create { output: String, files: Vec<String> },
    List { archive: String },
}
#[derive(Default)]
...
#[doc = "List zip contents. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_list(archive: &str) {
    let mut _context = zipfile.ZipFile(archive, "r".to_string());
    let z = _context.__enter__();
    for name in z.namelist() {
        println!("{}", name);
    }
}
#[doc = "Create zip archive. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_create(files: &[String], output: &str) {
    let mut _context = zipfile.ZipFile(output, "w".to_string());
    let z = _context.__enter__();
    for f in files {
        z.write(
            f,
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::List { .. }));
    match &args.command {
        Some(Commands::List { ref archive, .. }) => {
            cmd_list(archive);
        }
