...
#[doc = "Calculate GINI impurity."]
pub fn gini_impurity(y: &Vec<i32>) -> Result<f64, Box<dyn std::error::Error>> {
    let mut impurity: f64 = Default::default();
    let _cse_temp_0 = y.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(0.0);
    }
    let classes = y.into_iter().collect::<std::collections::HashSet<_>>();
    impurity = 1.0;
    for c in classes.iter().cloned() {
        ...
        impurity = impurity - {
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ...
                ({ p } as f64).powf({ 2 } as f64) as f64
            }
        };
    }
    Ok(impurity)
}
#[doc = "Create bootstrap sample."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn bootstrap_sample<'b, 'a>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
) -> (Vec<Vec<f64>>, Vec<i32>) {
    let _cse_temp_0 = &X.len() as i32;
    let n = _cse_temp_0;
    let indices = (0..(n)).into_iter().map(|_| 0).collect::<Vec<_>>();
    let X_boot = indices
        .iter()
        .cloned()
        .map(|i| {
...
#[doc = "Find best split using GINI criterion with feature subsampling."]
pub fn best_split<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    max_features: i32,
) -> Result<((), (), (), ()), Box<dyn std::error::Error>> {
    let mut best_right_idx: DepylerValue = Default::default();
    let mut best_left_idx: DepylerValue = Default::default();
    let mut best_threshold: DepylerValue = Default::default();
    let mut best_feature: DepylerValue = Default::default();
    let _cse_temp_0 = &X.len() as i32;
    let n_samples = _cse_temp_0;
    ...
    let _cse_temp_1 = n_samples == 0;
    if _cse_temp_1 {
        return Ok((None, None, None, None));
    }
    let _cse_temp_2 = "inf".parse::<f64>().unwrap();
    let mut best_gini = _cse_temp_2.clone();
    let all_features = (0..(n_features)).collect::<Vec<_>>();
    let features_to_check =
        all_features[..depyler_min((max_features).clone(), (n_features).clone()) as usize].to_vec();
    for feature in features_to_check.iter().cloned() {
        let values = {
            let mut sorted_vec = (0..(n_samples))
                .into_iter()
                .map(|i| {
                    X.get(i as usize)
                        .cloned()
        ...
        let thresholds = (0..((values.len() as i32).saturating_sub(1)))
            .into_iter()
            .map(|i| {
                (values
                    .get(i as usize)
            ...
            .collect::<Vec<_>>();
        for threshold in thresholds.iter().cloned() {
            let left_idx = (0..(n_samples))
                .into_iter()
                .filter(|i| {
                    let i = i.clone();
                    X.get(i as usize)
            ...
            let right_idx = (0..(n_samples))
                ...
                .map(|i| i)
                .collect::<Vec<_>>();
            if (left_idx.len() as i32 == 0) || (right_idx.len() as i32 == 0) {
                continue;
            }
            let left_y = left_idx
                .iter()
            ...
            let right_y = right_idx
                ...
                .collect::<Vec<_>>();
            let gini = ((left_y.len() as i32 * gini_impurity(&left_y)?
                + right_y.len() as i32 * gini_impurity(&right_y)?) as f64)
                / ((n_samples) as f64);
            if gini < best_gini {
                best_gini = gini;
    ...
    Ok((best_feature, best_threshold, best_left_idx, best_right_idx))
}
#[doc = "Recursively build decision tree."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn build_tree<'a, 'b>(
    ...
    let mut class_counts: std::collections::HashMap<DepylerValue, DepylerValue> =
        Default::default();
    let mut majority_class: i32 = Default::default();
    let classes = y.into_iter().collect::<std::collections::HashSet<_>>();
    let _cse_temp_0 = classes.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 1;
    let _cse_temp_2 = depth >= max_depth;
    let _cse_temp_3 = (_cse_temp_1) || (_cse_temp_2);
    let _cse_temp_4 = y.len() as i32;
    let _cse_temp_5 = _cse_temp_4 < 2;
    ...
    if _cse_temp_6 {
        ...
        for yi in y.iter().cloned() {
            class_counts.insert(
                yi.to_string().clone(),
                class_counts.get(&yi).cloned().unwrap_or(0) + 1,
            );
        }
        majority_class = if !class_counts.is_empty() {
            *class_counts
                .keys()
                .cloned()
                .collect::<Vec<_>>()
        ...
        return Ok({
            ...
            map.insert("class".to_string(), majority_class);
            map
        });
    }
    let (feature, threshold, left_idx, right_idx) = best_split(&&X, &y, max_features)?;
    if feature.is_none() {
        class_counts = {
            let map: HashMap<String, ()> = HashMap::new();
            map
    ...
    Ok({
        let mut map = HashMap::new();
        map.insert("feature".to_string(), feature);
        map.insert("threshold".to_string(), threshold);
        map.insert(
            "left".to_string(),
            build_tree(&left_X, &left_y, depth + 1, max_depth, max_features)
                .expect("build_tree failed"),
        );
        map.insert(
            "right".to_string(),
            build_tree(&right_X, &right_y, depth + 1, max_depth, max_features)
                .expect("build_tree failed"),
        );
        map
    })
}
#[doc = "Predict class using single tree."]
#[doc = " Depyler: proven to terminate"]
pub fn predict_single_tree<'a, 'b>(
    tree: &'a std::collections::HashMap<String, DepylerValue>,
    x: &'b Vec<f64>,
) -> Result<i32, Box<dyn std::error::Error>> {
    let _cse_temp_0 = tree.get("class").is_some();
    if _cse_temp_0 {
        return Ok(tree.get("class").cloned().unwrap_or_default());
    }
    let _cse_temp_1 = {
        let base = &x;
        let idx: i32 = tree.get("feature").cloned().unwrap_or_default();
        let actual_idx = if idx < 0 {
            base.len().saturating_sub(idx.abs() as usize)
        } else {
            idx as usize
        ...
        base.get(actual_idx)
            .cloned()
            .expect("IndexError: list index out of range")
    } <= tree.get("threshold").cloned().unwrap_or_default();
    if _cse_temp_1 {
        return Ok(predict_single_tree(
            tree.get("left").cloned().unwrap_or_default(),
            &x,
        ));
    }
    Ok(predict_single_tree(
        tree.get("right").cloned().unwrap_or_default(),
        &x,
    ))
}
#[doc = "Fit random forest classifier."]
#[doc = " Depyler: proven to terminate"]
pub fn fit<'a, 'b, 'c>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    n_estimators: i32,
    max_depth: i32,
    random_state: &'c Option<i32>,
) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = &X.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    let _cse_temp_2 = y.len() as i32;
    let _cse_temp_3 = _cse_temp_2 == 0;
    let _cse_temp_4 = (_cse_temp_1) || (_cse_temp_3);
    if _cse_temp_4 {
        return Err(Box::new(ValueError::new("Empty input data".to_string())));
    }
    ...
    let _cse_temp_5 = X
        ...
        .expect("IndexError: list index out of range")
        .len() as i32;
    let n_features = _cse_temp_5;
    let _cse_temp_6 = ({ n_features } as f64).powf({ 0.5 } as f64);
    let _cse_temp_7 = (_cse_temp_6) as i32;
    let _cse_temp_8 = depyler_max((1).clone(), (_cse_temp_7).clone());
    let max_features = _cse_temp_8;
    let mut trees = vec![];
    for __sanitized in 0..(n_estimators) {
        let (X_boot, y_boot) = bootstrap_sample(&&X, &y);
        let tree = build_tree(&X_boot, &y_boot, 0, max_depth, max_features)?;
        trees.push(DepylerValue::Str(format!("{:?}", tree)));
    }
    Ok({
        let mut map = HashMap::new();
        map.insert("trees".to_string(), trees);
        map
    })
}
#[doc = "Predict class labels using majority voting."]
#[doc = " Depyler: verified panic-free"]
pub fn predict<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    trees: &'b Vec<std::collections::HashMap<String, DepylerValue>>,
) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut predictions = vec![];
    for x in X.iter().cloned() {
        let mut votes = {
            let mut map = HashMap::new();
            map
        };
        for tree in trees.iter().cloned() {
            let pred = predict_single_tree(&tree, &x)?;
            votes.insert(
                pred.to_string().clone(),
                DepylerValue::Str(format!("{:?}", votes.get(&pred).cloned().unwrap_or(0) + 1)),
            );
        }
        let majority = *votes
            .keys()
            ...
            .max()
            .unwrap();
        predictions.push(DepylerValue::Int(majority as i64));
    }
    Ok(predictions)
}
#[doc = "Calculate accuracy score."]
#[doc = " Depyler: proven to terminate"]
pub fn score<'b, 'a, 'c>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    trees: &'c Vec<std::collections::HashMap<String, DepylerValue>>,
) -> Result<f64, Box<dyn std::error::Error>> {
    let predictions = predict(&&X, &trees)?;
    let _cse_temp_0 = (0..(y.len() as i32))
        .into_iter()
        .filter(|&i| {
            predictions
                .get(i as usize)
        ...
        .map(|i| 1)
        .sum::<f64>();
    let correct = _cse_temp_0;
    Ok(((correct) as f64) / ((y.len() as i32) as f64))
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_fit() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
            std::process::exit(1);
        }
    };
    let _cse_temp_0 = data.get("X").is_none();
    let _cse_temp_1 = data.get("y").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'X' or 'y'");
        std::process::exit(1);
    }
    let n_estimators = data.get("n_estimators").cloned().unwrap_or(10);
    let max_depth = data.get("max_depth").cloned().unwrap_or(10);
    let random_state = data.get("random_state").cloned().unwrap_or(None);
    let mut result: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        result = fit(
            data.get("X").cloned().unwrap_or_default(),
            data.get("y").cloned().unwrap_or_default(),
            &n_estimators,
            &max_depth,
            &random_state,
        )?;
...
#[doc = " Depyler: proven to terminate"]
pub fn cmd_predict() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let _cse_temp_0 = data.get("X").is_none();
    let _cse_temp_1 = data.get("trees").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'X' or 'trees'");
        std::process::exit(1);
    }
    let predictions = predict(
        data.get("X").cloned().unwrap_or_default(),
        data.get("trees").cloned().unwrap_or_default(),
    )?;
    println!(
        "{}",
        format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("predictions".to_string().to_string(), predictions);
            map
        })
    );
    Ok(())
...
#[doc = " Depyler: verified panic-free"]
pub fn cmd_fit_predict() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    for field in required.iter().cloned() {
        if data.get(&field).is_none() {
            eprintln!("{}", format!("Error: Missing '{}'", field));
            std::process::exit(1);
        }
    }
    let n_estimators = data.get("n_estimators").cloned().unwrap_or(10);
    let max_depth = data.get("max_depth").cloned().unwrap_or(10);
    let random_state = data.get("random_state").cloned().unwrap_or(42);
    let mut result: Option<String> = None;
    let mut predictions: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        result = fit(
            data.get("X_train").cloned().unwrap_or_default(),
            data.get("y_train").cloned().unwrap_or_default(),
            &n_estimators,
            &max_depth,
            &random_state,
        )?;
        predictions = predict(
            data.get("X_test").cloned().unwrap_or_default(),
            result.get("trees").cloned().unwrap_or_default(),
        )?;
        println!(
            "{}",
            format!("{:?}", {
...
#[doc = " Depyler: proven to terminate"]
pub fn cmd_score() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let _cse_temp_1 = data.get("y").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    let _cse_temp_3 = data.get("trees").is_none();
    let _cse_temp_4 = (_cse_temp_2) || (_cse_temp_3);
    if _cse_temp_4 {
        eprintln!("{}", "Error: Missing 'X', 'y', or 'trees'");
        std::process::exit(1);
    }
    let accuracy = score(
        data.get("X").cloned().unwrap_or_default(),
    ...
    println!(
        ...
        format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("accuracy".to_string().to_string(), accuracy);
            map
        })
    );
    Ok(())
