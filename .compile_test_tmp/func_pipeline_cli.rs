...
impl<T: Clone> Pipeline<T> {
    ...
    pub fn map<U: Clone>(&self, f: impl Fn(T) -> U) -> Pipeline<U> {
        return Pipeline::new(f(self.value.clone()));
    }
    pub fn tap(&self, f: impl Fn(T)) -> Pipeline<T> {
        f(self.value.clone());
        return self;
    }
    pub fn filter(&self, pred: impl Fn(T) -> bool) -> Pipeline<Option<T>> {
        if pred(self.value.clone()) {
            return Pipeline::new(self.value.clone());
        };
        return Pipeline::new(None);
    }
    pub fn get(&self) -> T {
        return self.value.clone();
    }
    pub fn get_or(&self, default: T) -> T {
        return if self.value.clone().is_some() {
            self.value.clone()
        } else {
            default
        };
...
#[doc = "Create a pipeline that applies functions left to right.\n\n    pipe(f, g, h)(x) == h(g(f(x)))\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn pipe(funcs: &[String]) -> Box<dyn Fn(()) -> ()> {
    let pipeline = move |value: ()| {
        let mut result = value.clone();
        for _func in funcs.iter().cloned() {
            result = func(result);
        }
        return result;
    };
...
#[doc = "Create a composition that applies functions right to left.\n\n    compose(f, g, h)(x) == f(g(h(x)))\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compose(funcs: &[String]) -> Box<dyn Fn(()) -> ()> {
    let composed = move |value: ()| {
        let mut result = value.clone();
        for _func in funcs.iter().cloned().rev() {
            result = func(result);
        }
        return result;
    };
...
#[doc = "Return a function that always returns value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn constant<T: Clone>(value: T) -> Box<dyn Fn(()) -> ()> {
    let r#const = move |args: ()| {
        return value;
    };
    r#const
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn flip<V: Clone, T: Clone, U: Clone>(f: impl Fn(T, U) -> V) -> Box<dyn Fn((), ()) -> ()> {
    let flipped = move |b: (), a: ()| -> () {
        return f(a, b);
    };
    Box::new(flipped)
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn apply<T: Clone, U: Clone>(f: impl Fn(T) -> U, x: &T) -> U {
    f(x)
}
#[doc = "Execute a side effect and return the original value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn tap<T: Clone>(f: impl Fn(T)) -> Box<dyn Fn(()) -> ()> {
    let tapped = move |x: ()| -> () {
        f(x);
        return x;
    };
...
#[doc = "Create a pipeline starting with value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn pipeline<T: Clone>(value: &T) -> Pipeline<T> {
    Pipeline::new(value)
}
#[doc = "Create a function that adds n."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn add(n: i32) -> impl Fn(i32) -> i32 {
    move |x| x + n
}
#[doc = "Create a function that multiplies by n."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
#[doc = " Depyler: proven to terminate"]
pub fn is_even(x: i32) -> Result<bool, Box<dyn std::error::Error>> {
    Ok(x % 2 == 0)
}
...
#[doc = "Add prefix to string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn prefix<'a>(p: &'a str) -> impl Fn(String) -> String + 'a {
    move |s| format!("{}{}", p, s)
}
#[doc = "Add suffix to string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn suffix<'a>(s: &'a str) -> impl Fn(String) -> String + 'a {
    move |x| x + s
}
#[doc = "Split string by separator."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn split_by<'a>(sep: &'a str) -> impl Fn(String) -> Vec<String> + 'a {
    move |s| s.split(sep).map(|s| s.to_string()).collect::<Vec<String>>()
}
#[doc = "Join list by separator."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn join_by<'a>(sep: &'a str) -> impl Fn(String) -> String + 'a {
    move |lst| lst.join(sep)
}
#[doc = "Map function over list."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn map_list<T: Clone, U: Clone>(f: impl Fn(T) -> U) -> impl Fn(T) -> Vec<U> {
    move |lst| lst.iter().cloned().map(|x| f(x)).collect::<Vec<_>>()
}
#[doc = "Filter list by predicate."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn filter_list<T: Clone>(pred: impl Fn(T) -> bool) -> impl Fn(T) -> Vec<T> {
    move |lst| {
        lst.iter()
            .cloned()
            .filter(|x| {
...
#[doc = "Reduce list with function."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reduce_list<T: Clone>(f: impl Fn(T, T) -> T, initial: &T) -> Box<dyn Fn(Vec<()>) -> ()> {
    let reducer = move |lst: &Vec<()>| -> () {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn head<T: Clone>(lst: &Vec<T>) -> Result<Option<T>, Box<dyn std::error::Error>> {
    Ok(if !lst.is_empty() {
        Some(
            lst.get(0usize)
                .cloned()
...
#[doc = "Get all but first element."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn tail<T: Clone>(lst: &Vec<T>) -> Vec<T> {
    if !lst.is_empty() {
        {
            let base = &lst;
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn take<T: Clone>(n: i32) -> impl Fn(T) -> Vec<T> {
    move |lst| {
        let base = &lst;
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn drop<T: Clone>(n: i32) -> impl Fn(T) -> Vec<T> {
    move |lst| {
        let base = &lst;
...
#[doc = "Simulate pipeline operations."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn simulate_pipeline(_operations: Vec<String>) -> Vec<String> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: func_pipeline_cli.py <command>[args]");
...
mod tests {
    ...
    #[test]
    fn test_is_even_examples() {
        let _ = is_even(Default::default());
    }
    #[test]
    fn test_is_positive_examples() {
        let _ = is_positive(Default::default());
    }
    #[test]
    fn test_tail_examples() {
        assert_eq!(tail(vec![]), vec![]);
        assert_eq!(tail(vec![1]), vec![1]);
    }
    #[test]
    fn test_simulate_pipeline_examples() {
        assert_eq!(simulate_pipeline(vec![]), vec![]);
        assert_eq!(simulate_pipeline(vec![1]), vec![1]);
    }
}
