...
pub struct DirectedGraph {
    pub _adj: std::collections::HashMap<i32, Vec<i32>>,
    pub _in_degree: std::collections::HashMap<i32, i32>,
}
impl DirectedGraph {
    pub fn new() -> Self {
        Self {
            _adj: std::collections::HashMap::new(),
            _in_degree: std::collections::HashMap::new(),
        }
    }
    pub fn add_vertex(&self, v: i32) {
        if !self._adj.clone().contains_key(&v) {
            self._adj.clone().insert(v, vec![]);
            self._in_degree.clone().insert(v, 0);
        };
    }
    pub fn add_edge(&self, u: i32, v: i32) {
        self.add_vertex(u);
        self.add_vertex(v);
        {
            let _base = &self._adj.clone();
            let _idx = u;
            if _idx < 0 {
        ...
        .push(v);
        self._in_degree.clone().insert(
            v,
            {
                let _base = &self._in_degree.clone();
                let _idx = v;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
    ...
    pub fn neighbors(&self, v: i32) -> Vec<i32> {
        return self._adj.clone().get(v, vec![]);
    }
    pub fn vertices(&self) -> Vec<i32> {
        return self._adj.clone().keys().cloned().collect::<Vec<_>>();
    }
    pub fn in_degree(&self, v: i32) -> i32 {
        return self._in_degree.clone().get(v, 0);
    }
}
#[derive(Default)]
enum Commands {
    ...
    Levels { edges: Vec<String> },
    
    Sort { edges: Vec<String> },
    
    Longest { edges: Vec<String> },
...
#[doc = "Kahn's algorithm - BFS-based topological sort."]
pub fn topo_sort_kahn(
    graph: &DirectedGraph,
    ...
    let mut in_degree = graph
        ...
        .into_iter()
        .map(|v| {
            let _v = graph.in_degree(v);
            (v, _v)
        })
        .collect::<std::collections::HashMap<_, _>>();
    let mut queue: std::collections::VecDeque<i32> = VecDeque::from(
        graph
            .vertices()
            .into_iter()
            .filter(|v| {
    ...
    while !queue.is_empty() {
        ...
        for neighbor in graph.neighbors(vertex) {
            ...
            if in_degree
                .get(&DepylerValue::Int(neighbor as i64))
                .cloned()
                .unwrap_or_default()
                == 0
            {
...
#[doc = "DFS-based topological sort."]
#[doc = " Depyler: verified panic-free"]
pub fn topo_sort_dfs(graph: &DirectedGraph) -> Option<Vec<i32>> {
    let visited: std::collections::HashSet<i32> = std::collections::HashSet::<i32>::new();
    let temp_mark: std::collections::HashSet<i32> = std::collections::HashSet::<i32>::new();
    let mut result: Vec<i32> = vec![];
    let has_cycle = false;
...
#[doc = "Find all possible topological orderings."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn all_topo_sorts(graph: &DirectedGraph) -> Vec<Vec<i32>> {
    let in_degree = graph
        .vertices()
        .into_iter()
        .map(|v| {
    ...
    let backtrack = move || -> () {
        ...
        if candidates.is_empty() {
            if current.len() as i32 == graph.vertices().len() as i32 {
                result.push(current.clone());
            }
            return;
        }
        for v in {
            let mut sorted_vec = candidates.iter().cloned().collect::<Vec<_>>();
...
#[doc = "Check if directed graph has a cycle."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn has_cycle(graph: &DirectedGraph) -> Result<bool, Box<dyn std::error::Error>> {
    Ok(topo_sort_kahn(&graph).is_none())
}
#[doc = "Find longest path in DAG."]
pub fn longest_path_dag(graph: &DirectedGraph) -> Result<i32, Box<dyn std::error::Error>> {
    let topo = topo_sort_kahn(&graph)?;
    if topo.is_none() {
        return Ok(-1);
    }
    let mut distances: std::collections::HashMap<i32, i32> =
        graph.vertices().iter().map(|k| (k.clone(), 0)).collect();
    for vertex in topo.iter().cloned() {
        for neighbor in graph.neighbors(vertex) {
            distances.insert(
                neighbor.clone(),
                depyler_max(
                    ...
                    .clone(),
                    (distances
                        .get(&DepylerValue::Int(vertex as i64))
                        .cloned()
                        .unwrap_or_default()
                        + 1)
                    .clone(),
...
#[doc = "Find critical path(longest path with node weights)."]
pub fn critical_path<'b, 'a>(
    graph: &'a DirectedGraph,
    durations: &'b std::collections::HashMap<i32, i32>,
) -> Result<(i32, Vec<i32>), Box<dyn std::error::Error>> {
    let mut max_time: i32 = Default::default();
    let mut end_vertex: i32 = Default::default();
    let topo = topo_sort_kahn(&graph)?;
    if topo.is_none() {
        return Ok((-1, vec![]));
    }
    let mut earliest: std::collections::HashMap<i32, i32> = {
        let map: HashMap<i32, i32> = HashMap::new();
        map
    };
    let mut predecessor: std::collections::HashMap<i32, i32> = {
        let map: HashMap<i32, i32> = HashMap::new();
        map
    };
    for vertex in topo.iter().cloned() {
        ...
        for v in graph.vertices() {
            if graph.neighbors(v).get(&vertex).is_some() {
                let pred_time = earliest.get(&v).cloned().unwrap_or(0)
                    + durations.get(&v).cloned().unwrap_or(0);
                if pred_time.unwrap_or_default() > max_pred {
                    max_pred = pred_time.clone();
                    best_pred = v;
                }
            }
        }
        earliest.insert(vertex.clone(), max_pred);
        if best_pred >= 0 {
            predecessor.insert(vertex.clone(), best_pred);
        }
    }
    max_time = 0;
    end_vertex = -1;
    for v in topo.iter().cloned() {
        let completion = earliest
            .get(&DepylerValue::Int(v as i64))
            .cloned()
            .unwrap_or_default()
            + durations.get(&v).cloned().unwrap_or(0);
    ...
    let mut current = end_vertex.clone();
    while current >= 0 {
        path.push(current);
        current = predecessor.get(&current).cloned().unwrap_or(-1);
    }
    path.reverse();
    Ok((max_time, path))
}
#[doc = "Group vertices by dependency level(parallelizable groups)."]
pub fn dependency_levels(
    ...
    while !current.is_empty() {
        levels.push({
            let mut sorted_vec = current.iter().cloned().collect::<Vec<_>>();
            sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            sorted_vec
        });
        let mut next_level: Vec<i32> = vec![];
        for v in current.iter().cloned() {
            for neighbor in graph.neighbors(v) {
                {
                    let _key = neighbor;
                    let _old_val = in_degree.get(&_key).cloned().unwrap_or_default();
...
#[doc = "Check if graph is a DAG."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_dag(graph: &DirectedGraph) -> Result<bool, Box<dyn std::error::Error>> {
    Ok(!has_cycle(&graph))
}
#[doc = "Find all source vertices(in-degree 0)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn source_vertices(graph: &DirectedGraph) -> Vec<i32> {
    {
        let mut sorted_vec = graph
            .vertices()
            .into_iter()
            .filter(|v| {
                let v = v.clone();
...
#[doc = "Find all sink vertices(out-degree 0)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn sink_vertices(graph: &DirectedGraph) -> Vec<i32> {
    {
        let mut sorted_vec = graph
            .vertices()
            .into_iter()
            .filter(|v| {
                let v = v.clone();
                graph.neighbors(v).len() as i32 == 0
            })
            .map(|v| v)
            .collect::<Vec<_>>()
            .iter()
...
#[doc = "Build directed graph from edge strings like '1-2'."]
#[doc = " Depyler: verified panic-free"]
pub fn build_directed_graph(edges: &Vec<String>) -> DirectedGraph {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Sort { .. }));
    match &args.command {
        Some(Commands::Sort { ref edges, .. }) => {
            let mut graph = build_directed_graph(&edges);
            let result = topo_sort_kahn(&graph)?;
            if let Some(ref result_val) = result {
                println!("{}", format!("Topological order: {:?}", result_val));
            } else {
                println!("{}", "Cycle detected - no valid ordering");
            }
        }
        Some(Commands::Cycle { ref edges, .. }) => {
            let mut graph = build_directed_graph(&edges);
        ...
        Some(Commands::Levels { ref edges, .. }) => {
            let mut graph = build_directed_graph(&edges);
            let levels = dependency_levels(&graph)?;
            println!("{}", format!("Levels: {:?}", levels));
        }
        Some(Commands::Longest { ref edges, .. }) => {
            let mut graph = build_directed_graph(&edges);
            let length = longest_path_dag(&graph)?;
            println!("{}", format!("Longest path: {:?}", length));
        }
...
mod tests {
    ...
    #[test]
    fn quickcheck_topo_sort_kahn() {
        fn prop(graph: ()) -> TestResult {
            let result = topo_sort_kahn(graph.clone());
            ...
            input_sorted.sort();
            let mut result = topo_sort_kahn(graph.clone());
            result.sort();
    ...
    #[test]
    fn test_has_cycle_examples() {
        let _ = has_cycle(Default::default());
    }
    #[test]
    fn test_is_dag_examples() {
        let _ = is_dag(Default::default());
    }
}
