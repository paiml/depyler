...
#[doc = "Integer division(floor division).\n\n   >>>integer_divide(10, 3)\n    3\n   >>>integer_divide(9, 3)\n    3\n   >>>integer_divide(-10, 3)\n    -4\n    "]
#[doc = " Depyler: proven to terminate"]
pub fn integer_divide(a: i32, b: i32) -> Result<i32, Box<dyn std::error::Error>> {
    Ok({
        let a = a;
        let b = b;
        let q = a / b;
...
#[doc = " Depyler: proven to terminate"]
pub fn modulo(a: i32, b: i32) -> Result<i32, Box<dyn std::error::Error>> {
    Ok(a % b)
}
#[doc = "Return both quotient and remainder.\n\n   >>>divmod_pair(10, 3)\n   (3, 1)\n   >>>divmod_pair(20, 7)\n   (2, 6)\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn divmod_pair(a: i32, b: i32) -> (i32, i32) {
    (
        {
            let a = a;
            let b = b;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Calc { .. }));
    match &args.command {
        Some(Commands::Calc { ref a, ref b, .. }) => {
            let a = *a;
            let b = *b;
            let (mut q, mut r) = divmod_pair(a, b);
            println!("{}", format!("{:?} {:?}", q, r));
        }
        Some(Commands::Quot { ref a, ref b, .. }) => {
            let a = *a;
            let b = *b;
            println!("{:?}", integer_divide(a, b));
        }
        Some(Commands::Rem { ref a, ref b, .. }) => {
            let a = *a;
            let b = *b;
            println!("{:?}", modulo(a, b));
        }
...
mod tests {
    ...
    #[test]
    fn test_integer_divide_examples() {
        assert_eq!(integer_divide(0, 0), 0);
        assert_eq!(integer_divide(1, 2), 3);
        assert_eq!(integer_divide(-1, 1), 0);
    }
    #[test]
    fn test_modulo_examples() {
        assert_eq!(modulo(0, 0), 0);
        assert_eq!(modulo(1, 2), 3);
        assert_eq!(modulo(-1, 1), 0);
    }
}
