...
impl ValidationError {
    ...
    pub fn to_string(&self) -> String {
        return format!("{}: {}", self.field.clone(), self.message.clone());
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl ParseError {
    pub fn new(input: String, reason: String) -> Self {
        Self { input, reason }
    }
    pub fn to_string(&self) -> String {
        return format!(
...
#[doc = "Create a Left value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn left<L: Clone, R: Clone>(value: &L) -> Either<L, R> {
    Left::new(value)
}
#[doc = "Create a Right value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn right<L: Clone, R: Clone>(value: &R) -> Either<L, R> {
    Right::new(value)
}
#[doc = "Wrap a function that may raise."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn try_except<R: Clone>(f: impl Fn() -> R) -> Either<String, R> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(Right::new(f()));
    })() {
        Ok(_result) => {
            return _result;
        }
        Err(e) => {
            return Left::new((e).to_string());
        }
    }
}
#[doc = "Parse string to int."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_int(s: &str) -> Either<ParseError, i32> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(Right::new(s.parse::<i32>().unwrap_or_default()));
    })() {
        Ok(_result) => {
            return _result;
        }
        Err(_) => {
            return Left::new(ParseError::new(
                s,
                "not a valid integer".to_string().to_string(),
            ));
        }
    }
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_float(s: &str) -> Either<ParseError, f64> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(Right::new(s.parse::<f64>().unwrap()));
    })() {
        Ok(_result) => {
            return _result;
        }
        Err(_) => {
            return Left::new(ParseError::new(
                s,
                "not a valid float".to_string().to_string(),
            ));
        }
    }
}
#[doc = "Parse string to positive int."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_positive(s: &str) -> Either<String, i32> {
    parse_int(s).flat_map(move |n| {
        if n > 0 {
            Right::new(n)
        } else {
            Left::new(format!("{:?} is not positive", n))
        }
    })
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn validate_non_empty<'a, 'b>(
    field: &'a str,
    value: &'b str,
) -> Either<ValidationError, String> {
    if value.trim().to_string() {
        return Right::new(value.trim().to_string());
    }
    Left::new(ValidationError::new(field, "cannot be empty".to_string()))
}
#[doc = "Validate string minimum length."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn validate_min_length<'b, 'a>(
    field: &'a str,
    value: &'b str,
    min_len: i32,
) -> Either<ValidationError, String> {
    let _cse_temp_0 = value.len() as i32;
    let _cse_temp_1 = _cse_temp_0 >= min_len;
    if _cse_temp_1 {
        return Right::new(value);
    }
    Left::new(ValidationError::new(
        field,
...
pub fn validate_email(
    value: &str,
) -> Result<Either<ValidationError, String>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = value.contains("@");
    let _cse_temp_1 = {
        let base = &value
            .split("@")
            .map(|s| s.to_string())
    ...
    if _cse_temp_2 {
        return Ok(Right::new(value));
    }
    Ok(Left::new(ValidationError::new(
        "email".to_string(),
        "invalid email format".to_string(),
    )))
}
#[doc = "Validate number is in range."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn validate_range(
    ...
    Left::new(ValidationError::new(
        field,
        format!("must be between {} and {}", min_val, max_val),
    ))
}
#[doc = "Convert list of Eithers to Either of list."]
#[doc = " Depyler: verified panic-free"]
pub fn sequence<R: Clone, L: Clone>(eithers: &Vec<Either<L, R>>) -> Either<L, Vec<R>> {
    let mut results: Vec<R> = vec![];
    for e in eithers.iter().cloned() {
        if e.is_left() {
            return e;
        }
        results.push(e.get_or(None));
    }
    Right::new(results)
}
#[doc = "Map function over list, collecting into Either."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn traverse<R: Clone, T: Clone, L: Clone>(
    f: impl Fn(T) -> Either<L, R>,
    lst: &Vec<T>,
) -> Either<L, Vec<R>> {
    sequence(lst.iter().cloned().map(|x| f(x)).collect::<Vec<_>>())
}
#[doc = "Partition list of Eithers into lefts and rights."]
#[doc = " Depyler: verified panic-free"]
pub fn partition_eithers<R: Clone, L: Clone>(eithers: &Vec<Either<L, R>>) -> (Vec<L>, Vec<R>) {
    let mut lefts: Vec<L> = vec![];
    let mut rights: Vec<R> = vec![];
    for e in eithers.iter().cloned() {
        if e.is_left() {
            lefts.push(e.fold(move |x| x, move |x| x));
        } else {
            rights.push(e.get_or(None));
        }
    }
    (lefts, rights)
}
#[doc = "Simulate either operations."]
pub fn simulate_either(
    operations: &Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut e: Either<String, i32> = Default::default();
                        ...
                        if cmd == "validate_email" {
                            ...
                            if cmd == "map" {
                                let (mut val, func) = parts
                                    ...
                                    .map(|s| s.to_string())
                                    .collect::<Vec<String>>();
                                e = right(val.parse::<i32>().unwrap_or_default());
                                if func == "double" {
                                    e = e.map(move |x| x * 2);
                                }
                                results.push(format!("{:?}", e));
                                ...
                                if cmd == "get_or" {
                                    ...
                                    results.push((e.get_or((default) as i32)).to_string());
                                } else {
                                    if cmd == "fold" {
                                        val = parts
                                            .get(1usize)
                                        ...
                                        let result = e.fold(
                                            move |err| format!("Error: {:?}", err),
                                            move |val| format!("Success: {}", val),
                                        );
                                        results.push(result);
                                    }
                                }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: func_either_cli.py <command>[args]");
    ...
    if _cse_temp_4 {
        ...
        let _cse_temp_5 = cmd == "validate-email";
...
mod tests {
    ...
    #[test]
    fn test_simulate_either_examples() {
        assert_eq!(simulate_either(vec![]), vec![]);
        assert_eq!(simulate_either(vec![1]), vec![1]);
    }
}
