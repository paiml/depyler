...
#[doc = "2D convolution forward pass."]
#[doc = " Depyler: proven to terminate"]
pub fn conv2d<'b, 'c, 'a>(
    x: &'a Vec<DepylerValue>,
    kernel: &'b Vec<DepylerValue>,
    bias: &'c Vec<DepylerValue>,
    stride: i32,
    padding: i32,
    ...
    let _cse_temp_0 = 2 * padding;
    let _cse_temp_1 = h + _cse_temp_0 - kh;
    let _cse_temp_2 = {
        let a = _cse_temp_1;
        let b = stride;
        let q = a / b;
    ...
    let out_h = _cse_temp_2 + 1;
    let _cse_temp_3 = w + _cse_temp_0 - kw;
    let _cse_temp_4 = {
        let a = _cse_temp_3;
        let b = stride;
        let q = a / b;
    ...
    let out_w = _cse_temp_4 + 1;
    let output = (0..(out_ch))
        .into_iter()
        .map(|_| {
            (0..(out_h))
                .into_iter()
                .map(|_| (0..(out_w)).into_iter().map(|_| 0.0).collect::<Vec<_>>())
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>();
    for oc in 0..(out_ch) {
        for oh in 0..(out_h) {
            for ow in 0..(out_w) {
                let mut val = if !bias.is_empty() {
                    bias.get(oc as usize)
                        .cloned()
                        .expect("IndexError: list index out of range")
                } else {
                    DepylerValue::Int(0 as i64)
                };
                for ic in 0..(in_ch) {
                    for kh_i in 0..(kh) {
                        for kw_i in 0..(kw) {
                            let ih = oh * stride + kh_i - padding;
                            let iw = ow * stride + kw_i - padding;
...
pub fn cmd_forward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        output = conv2d(
            ...
            data.get("kernel").cloned().unwrap_or_default(),
            data.get("bias").cloned().unwrap_or(vec![0]),
            1i32,
            0i32,
        )?;
        println!(
