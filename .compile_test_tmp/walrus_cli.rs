...
enum Commands {
    ...
    Find {
        ...
        #[doc = "Minimum word length(default: 5)"]
        min_len: Option<i32>,
    },
...
#[doc = "Return length if>5, else 0."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn check_length(text: &str) -> i32 {
...
#[doc = "Count words using walrus in comprehension."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_words(text: &str) -> i32 {
    ...
    let long_words = words
        ...
        .collect::<Vec<_>>();
    long_words.len() as i32 as i32
}
#[doc = "Find first word longer than min_len."]
#[doc = " Depyler: verified panic-free"]
pub fn find_first_long(text: &str, min_len: i32) -> String {
    let words = text
        .split_whitespace()
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
...
#[doc = "Main entry point with walrus operator examples."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    ();
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Check { .. }));
    match &args.command {
        Some(Commands::Check { ref text, .. }) => {
            let result = check_length(&text);
            let _cse_temp_1 = result > 0;
            if _cse_temp_1 {
                println!("{}", format!("Length: {:?}", result));
            } else {
                println!("{}", "Too short");
            }
        }
        Some(Commands::Count { ref text, .. }) => {
            ...
            println!("{}", format!("Long words: {:?}", count));
        }
        Some(Commands::Find {
            ref min_len,
            ref text,
            ...
            let found = find_first_long(&text, min_len);
            println!("{}", format!("Found: {:?}", found));
        }
...
mod tests {
    ...
    #[test]
    fn test_check_length_examples() {
        assert_eq!(check_length(""), 0);
        assert_eq!(check_length("a"), 1);
        assert_eq!(check_length("abc"), 3);
    }
    #[test]
