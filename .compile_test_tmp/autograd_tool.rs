...
#![allow(dead_code)]
const STR_VALUE: &'static str = "value";
...
#[doc = "Compute gradient numerically using central difference."]
...
pub fn numerical_gradient(
    ...
    Ok(((f(x + h) - f(x - h)) as f64) / ((2f64 * h) as f64))
}
#[doc = "Gradient of x^n is n * x^(n-1)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn backward_power(x: f64, n: i32) -> f64 {
    (n as f64) * ({ x } as f64).powf({ n - 1 } as f64)
}
#[doc = "Gradient of sum is all ones."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn backward_sum(x: &Vec<f64>) -> Vec<f64> {
    vec![1.0; (x.len() as i32) as usize]
}
#[doc = "Gradient of ax + b is a."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn backward_linear(a: f64, _b: f64, _x: f64) -> f64 {
    a
}
#[doc = "Gradient of x*w: d/dx = w, d/dw = x."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn backward_product(x: f64, w: f64) -> (f64, f64) {
    (w, x)
}
#[doc = "Parse expression and compute gradients."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_and_differentiate(
    mut expression: String,
    variables: &std::collections::HashMap<DepylerValue, DepylerValue>,
    ...
    let mut grad: DepylerValue = Default::default();
    expression = expression.trim().to_string();
    let _cse_temp_0 = expression.contains("**");
    if _cse_temp_0 {
        parts = expression
            .split("**")
            .map(|s| s.to_string())
            .collect::<Vec<String>>();
        let _cse_temp_1 = parts
            .get(0usize)
            .cloned()
        ...
        if _cse_temp_1 {
            let _cse_temp_2 = parts
                ...
                .unwrap_or_default();
            let n = _cse_temp_2;
            x = variables.get("x").cloned().unwrap_or_default();
            grad = backward_power(x, n);
            return Ok({
                let mut map = HashMap::new();
                map.insert("grad".to_string(), DepylerValue::Float(grad as f64));
                map.insert(
                    STR_VALUE.to_string(),
    ...
    let _cse_temp_3 = expression == "sum";
    if _cse_temp_3 {
        x = variables.get("x").cloned().unwrap_or_default();
        grad = backward_sum(&x);
        let _cse_temp_4 = x.iter().sum::<i32>();
        let value = _cse_temp_4;
        return Ok({
            let mut map = HashMap::new();
            map.insert("grad".to_string(), DepylerValue::Str(format!("{:?}", grad)));
            map.insert(STR_VALUE.to_string(), DepylerValue::Int(value as i64));
            map
        });
    }
    let _cse_temp_5 = expression.contains("x * w");
    let _cse_temp_6 = expression.contains("x*w");
    let _cse_temp_7 = (_cse_temp_5) || (_cse_temp_6);
    if _cse_temp_7 {
        x = variables.get("x").cloned().unwrap_or_default();
        let w = variables.get("w").cloned().unwrap_or_default();
        let (grad_x, grad_w) = backward_product(x, w);
        return Ok({
            let mut map = HashMap::new();
            map.insert(
                "grad_x".to_string(),
                DepylerValue::Str(format!("{:?}", grad_x)),
            );
            map.insert(
                "grad_w".to_string(),
                DepylerValue::Str(format!("{:?}", grad_w)),
            );
            map.insert(
                STR_VALUE.to_string(),
                DepylerValue::Str(format!("{:?}", x * w)),
            );
            map
        });
    }
    let _cse_temp_8 = expression.contains("*x");
    if _cse_temp_8 {
        let _cse_temp_9 = expression.contains("+ ");
        if _cse_temp_9 {
            parts = expression
                .split("+")
                .map(|s| s.to_string())
...
#[doc = "Handle backward subcommand."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_backward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = data.get("expression").is_none();
    if _cse_temp_0 {
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        result =
            parse_and_differentiate(data.get("expression").cloned().unwrap_or_default(), &data)?;
...
mod tests {
    ...
    #[test]
    fn test_backward_sum_examples() {
        assert_eq!(backward_sum(vec![]), vec![]);
        assert_eq!(backward_sum(vec![1]), vec![1]);
    }
}
