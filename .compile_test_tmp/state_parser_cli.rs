...
impl Token {
    pub fn new(type_: TokenType, value: DepylerValue, pos: i32) -> Self {
        Self { type_, value, pos }
    }
}
#[derive(Debug, Clone)]
pub struct Lexer {
...
impl Lexer {
    ...
    pub fn tokenize(&mut self) -> Vec<Token> {
        ...
        while self.pos.clone() < (self.text.clone().len() as i32) {
            ...
            if !char.is_empty() && char.chars().all(|c| c.is_ascii_digit())
                || char == ".".to_string() && self._peek_digit()
            {
                tokens.push(self._read_number(start_pos));
            } else {
                if !char.is_empty() && char.chars().all(|c| c.is_alphabetic())
                    || char == "_".to_string()
                {
                    tokens.push(self._read_identifier(start_pos));
                } else {
                    if char == "+".to_string() {
                        tokens.push(Token::new(TokenType::PLUS, "+".to_string(), start_pos));
                        self.pos = self.pos.clone() + 1;
                    } else {
                        if char == "-".to_string() {
                            tokens.push(Token::new(TokenType::MINUS, "-".to_string(), start_pos));
                            self.pos = self.pos.clone() + 1;
                        } else {
                            if char == "*".to_string() {
        ...
        tokens.push(Token::new(TokenType::EOF, None, self.pos.clone()));
        return tokens;
    }
    ...
    pub fn _peek_digit(&self) -> bool {
        return self.pos.clone() + 1 < (self.text.clone().len() as i32)
            && !{
                let _base = &self.text.clone();
    ...
    pub fn _read_number(&mut self, start_pos: i32) -> Token {
        ...
        let value = {
            ...
            let start_idx = (start_pos) as isize;
    ...
    pub fn _read_identifier(&mut self, start_pos: i32) -> Token {
        while self.pos.clone() < (self.text.clone().len() as i32) {
            let char = {
                let _base = &self.text.clone();
                let _idx = self.pos.clone();
            ...
            if !char.is_empty() && char.chars().all(|c| c.is_alphanumeric())
                || char == "_".to_string()
            {
                self.pos = self.pos.clone() + 1;
            } else {
        ...
        return Token::new(TokenType::IDENTIFIER, value, start_pos);
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct Expr {}
...
impl BinaryExpr {
    pub fn new(left: Expr, op: String, right: Expr) -> Self {
        Self { left, op, right }
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl UnaryExpr {
    pub fn new(op: String, operand: Expr) -> Self {
        Self { op, operand }
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl PrattParser {
    ...
    pub fn parse_precedence(&self, precedence: &Precedence) -> Expr {
        let mut token = self.advance();
        let mut left = self.prefix(token);
        if left.is_none() {
            panic!(
                "Exception: {}",
                ValueError::new(format!("Unexpected token: {}", token))
            );
        };
        while precedence.value < self.get_precedence(self.current()).value {
            let token = self.advance();
            let left = self.infix(token, left);
        }
        return left;
    }
    pub fn prefix(&self, token: &Token) -> Option<Expr> {
        if token.r#type == TokenType::NUMBER {
            return Some(NumberExpr::new(token.value));
        };
        if token.r#type == TokenType::IDENTIFIER {
            if self.check(TokenType::LPAREN) {
                return Some(self.call_expr(token.value));
            };
            return Some(IdentifierExpr::new(token.value));
        };
        if token.r#type == TokenType::LPAREN {
            let expr = self.parse_precedence(Precedence::NONE);
            self.consume(TokenType::RPAREN);
            return Some(GroupExpr::new(expr));
        };
        if token.r#type == TokenType::MINUS {
            let operand = self.parse_precedence(Precedence::UNARY);
            return Some(UnaryExpr::new("-".to_string(), operand));
        };
        return None;
    }
    pub fn infix(&self, token: &Token, left: &Expr) -> Expr {
        if [
            TokenType::PLUS,
            TokenType::MINUS,
            TokenType::STAR,
        ...
        .contains(&token.r#type)
        {
            ...
            return BinaryExpr::new(left, token.value, right);
        };
        if token.r#type == TokenType::CARET {
            let right =
                self.parse_precedence(Precedence::new(self.get_precedence(token).value - 1));
            return BinaryExpr::new(left, "^".to_string(), right);
        };
        return left;
    }
    pub fn call_expr(&self, name: String) -> CallExpr {
        self.consume(TokenType::LPAREN);
        let mut args = vec![];
        if !self.check(TokenType::RPAREN) {
            args.push(self.parse_precedence(Precedence::NONE));
            while self.r#match(TokenType::COMMA) {
                args.push(self.parse_precedence(Precedence::NONE));
            }
        };
        self.consume(TokenType::RPAREN);
        return CallExpr::new(name, args);
    }
    pub fn get_precedence(&self, token: &Token) -> Precedence {
        let precedences = {
            let mut map = std::collections::HashMap::new();
            map.insert(TokenType::PLUS, Precedence::TERM);
            map.insert(TokenType::MINUS, Precedence::TERM);
            map.insert(TokenType::STAR, Precedence::FACTOR);
            map.insert(TokenType::SLASH, Precedence::FACTOR);
            map.insert(TokenType::CARET, Precedence::UNARY);
            map.insert(TokenType::LPAREN, Precedence::CALL);
            map
        };
        return precedences.get(token.r#type, Precedence::NONE);
    }
    ...
    pub fn advance(&mut self) -> Token {
        ...
        if self.pos.clone() < (self.tokens.clone().len() as i32).saturating_sub(1) {
            self.pos = self.pos.clone() + 1;
        };
        return token;
    }
    pub fn check(&self, r#type: &TokenType) -> bool {
        return self.current().r#type == r#type;
    }
    pub fn r#match(&self, r#type: &TokenType) -> bool {
    ...
    pub fn consume(&self, r#type: &TokenType) -> Token {
        if !self.check(r#type) {
            panic!(
                "Exception: {}",
...
pub struct Evaluator {
    pub env: DepylerValue,
    pub functions: std::collections::HashMap<String, impl Fn() -> f64>,
}
impl Evaluator {
    pub fn new(env: Option<std::collections::HashMap<String, f64>>) -> Self {
        Self {
            env,
    ...
    pub fn evaluate(&self, expr: &Expr) -> f64 {
        ...
        if true {
            if !self.env.clone().contains_key(&expr.name) {
                panic!(
                    "Exception: {}",
                    ValueError::new(format!("Undefined variable: {}", expr.name))
                );
            };
            return {
                let _base = &self.env.clone();
                let _idx = expr.name;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
        ...
        if true {
            ...
            let ops = {
                let mut map = std::collections::HashMap::new();
                map.insert("+".to_string(), move |a, b| a + b);
                map.insert("-".to_string(), move |a, b| a - b);
                map.insert("*".to_string(), move |a, b| a * b);
                map.insert("/".to_string(), move |a, b| {
                    if b != 0 {
                        a / b
                    } else {
                        "inf".to_string().parse::<f64>().unwrap_or(0.0)
                    }
                });
                map.insert("^".to_string(), move |a, b| {
                    if b >= 0 && (b as i64) <= (u32::MAX as i64) {
                        ({ a } as i32)
        ...
        if true {
            ...
            if expr.op == "-".to_string() {
                return -operand;
            };
            return operand;
        };
        if true {
            if !self.functions.clone().contains_key(&expr.name) {
                panic!(
                    "Exception: {}",
                    ValueError::new(format!("Undefined function: {}", expr.name))
                );
            };
            let args = expr
                .args
                ...
                .collect::<Vec<_>>();
            return ({
                let _base = &self.functions.clone();
                let _idx = expr.name;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
        ...
        panic!(
            "Exception: {}",
            ValueError::new(format!("Unknown expression type: {}", r#type(expr)))
        );
    }
}
#[derive(Debug, Clone)]
...
impl Printer {
    ...
    pub fn to_string(&self, expr: &Expr) -> String {
        if true {
            if expr.value == expr.value.parse::<i32>().unwrap_or(0) {
                return expr.value.parse::<i32>().unwrap_or(0).to_string();
            };
        ...
        if true {
            let left = self.to_string(expr.left);
            let right = self.to_string(expr.right);
            return format!("({} {} {})", left, expr.op, right);
        };
        if true {
            let operand = self.to_string(expr.operand);
            return format!("({}{})", expr.op, operand);
        };
        if true {
            let args = expr
                .args
                .iter()
                .map(|arg| self.to_string(arg))
                .join(", ".to_string());
    ...
    pub fn to_rpn(&self, expr: &Expr) -> String {
        if true {
            if expr.value == expr.value.parse::<i32>().unwrap_or(0) {
                return expr.value.parse::<i32>().unwrap_or(0).to_string();
            };
        ...
        if true {
            let left = self.to_rpn(expr.left);
            let right = self.to_rpn(expr.right);
            return format!("{} {} {}", left, right, expr.op);
        };
        if true {
            let operand = self.to_rpn(expr.operand);
            return format!("{} {}", operand, expr.op);
        };
        if true {
...
#[doc = "Parse expression string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse(text: &str) -> Expr {
    let mut lexer = Lexer::new(text);
    let tokens = lexer.tokenize();
    let parser = PrattParser::new(tokens);
    parser.parse()
}
#[doc = "Parse and evaluate expression."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn evaluate<'a, 'b>(
    text: &'a str,
    env: &'b Option<std::collections::HashMap<String, f64>>,
) -> f64 {
    let expr = parse(text);
    Evaluator::new(env).evaluate(expr)
}
#[doc = "Parse and convert to string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn to_string(text: &str) -> String {
    let expr = parse(text);
    Printer::new().to_string(expr)
}
#[doc = "Parse and convert to RPN."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn to_rpn(text: &str) -> String {
    let expr = parse(text);
    Printer::new().to_rpn(expr)
}
#[doc = "Simulate parser operations."]
pub fn simulate_parser(
    operations: &Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results = vec![];
    let mut env: std::collections::HashMap<String, f64> = {
    ...
    for op in operations.iter().cloned() {
        ...
        if cmd == "eval" {
            ...
            match (|| -> Result<(), Box<dyn std::error::Error>> {
                ...
                results.push(DepylerValue::Str(format!("{:?}", (value).to_string())));
                Ok(())
            })() {
                Ok(()) => {}
                Err(e) => {
                    results.push(DepylerValue::Str(format!("{:?}", format!("error:{:?}", e))));
                }
            }
        } else {
            if cmd == "set" {
                ...
                if cmd == "string" {
                    match (|| -> Result<(), Box<dyn std::error::Error>> {
                        ...
                        Err(e) => {
                            results
                                .push(DepylerValue::Str(format!("{:?}", format!("error:{:?}", e))));
                        }
                    }
                } else {
                    if cmd == "rpn" {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: state_parser_cli.py <command><expr>");
        println!("{}", "Commands: eval, string, rpn");
    ...
    let expr = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "eval";
    let mut result;
    if _cse_temp_2 {
        result = evaluate(&expr, &None);
        println!("{}", result);
    } else {
        let _cse_temp_3 = cmd == "string";
        if _cse_temp_3 {
            result = to_string(&expr);
            println!("{}", result);
        } else {
            let _cse_temp_4 = cmd == "rpn";
            if _cse_temp_4 {
                result = to_rpn(&expr);
                println!("{}", result);
            } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_parser_examples() {
        assert_eq!(simulate_parser(vec![]), vec![]);
        assert_eq!(simulate_parser(vec![1]), vec![1]);
    }
}
