...
#![allow(dead_code)]
pub static SAFE_CHARS: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
...
pub const PATH_SAFE: std::collections::HashSet<String> = SAFE_CHARS
    | "!$&'()*+,;=:@/"
        .to_string()
        .into_iter()
        .collect::<std::collections::HashSet<_>>();
pub const QUERY_SAFE: std::collections::HashSet<String> = SAFE_CHARS
    | "!$&'()*+,;=:@/?"
        .to_string()
        .into_iter()
        .collect::<std::collections::HashSet<_>>();
...
struct Args {
    #[doc = "Text to encode/decode"]
    text: Option<String>,
    
    
    #[doc = "Decode instead of encode"]
    decode: bool,
    
    
    #[doc = "Encode/decode as URL path"]
    path: bool,
    
    
    #[doc = "Encode/decode as query string"]
    query: bool,
    
    
    #[doc = "Encode query parameter(can repeat)"]
    param: Vec<String>,
    
    
    #[doc = "Use + for spaces(form encoding)"]
    plus: bool,
    
    
    #[doc = "Auto-detect encode/decode"]
    auto: bool,
}
#[doc = "Encode a single byte as percent-encoded."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn percent_encode_byte(byte: i32) -> String {
    format!("%{}", byte)
}
#[doc = "Percent-encode a string.\n\n    Args:\n        text: String to encode\n        safe: Set of safe characters(default: unreserved chars)\n        space_as_plus: Encode spaces as +(for form data)\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn percent_encode<'b, 'a>(
    text: &'a str,
    safe: &'b mut Option<std::collections::HashSet<String>>,
    space_as_plus: bool,
) -> String {
    if safe.is_none() {
        safe = Some(SAFE_CHARS);
    }
    let mut result = vec![];
    for char in text.chars() {
        if (char.to_string() == " ") && (space_as_plus) {
            result.push(DepylerValue::Str("+".to_string()));
        } else {
            if safe.contains(&*char) {
                result.push(DepylerValue::Str(format!("{:?}", char)));
            } else {
                for byte in char.as_bytes().to_vec() {
...
#[doc = "Decode percent-encoded string."]
pub fn percent_decode(
    ...
    while i < text.len() as i32 {
        ...
        if (char.to_string() == "+") && (plus_as_space) {
            result.push(DepylerValue::Str(" ".to_string()));
            i = i + 1;
        } else {
            if (char.to_string() == "%") && (i + 2 < text.len() as i32) {
                let mut byte: i32 = Default::default();
                match (|| -> Result<(), Box<dyn std::error::Error>> {
                    byte = i64::from_str_radix(
                        &{
                            let base = text;
                            let start_idx: i32 = i + 1;
                            let stop_idx: i32 = i + 3;
                    ...
                    Ok(()) => {}
                    Err(_) => {
                        result.push(DepylerValue::Str(char.to_string()));
                        i = i + 1;
                    }
                }
            } else {
    ...
    match (|| -> Result<String, Box<dyn std::error::Error>> {
        return Ok(String::from_utf8_lossy(&result.join("").as_bytes().to_vec()).to_string());
    })() {
        Ok(_result) => {
            return Ok(_result);
        }
...
#[doc = "Encode a single query parameter."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_query_param<'b, 'a>(key: &'a str, value: &'b str) -> String {
    let encoded_key = percent_encode(key, &mut true, false);
    let encoded_value = percent_encode(value, &mut true, false);
    format!("{:?}={:?}", encoded_key, encoded_value)
}
#[doc = "Encode query parameters into query string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_query_string(params: &Vec<(String, String)>) -> String {
    params
        .iter()
        .cloned()
        .map(|(k, v)| encode_query_param(&k, &v))
        .collect::<Vec<_>>()
        .join("&")
}
#[doc = "Parse query string into list of(key, value) pairs."]
#[doc = " Depyler: verified panic-free"]
pub fn parse_query_string(
    query: &str,
) -> Result<Vec<(String, String)>, Box<dyn std::error::Error>> {
    let mut key: DepylerValue = Default::default();
...
#[doc = "Encode URL path, preserving structure."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_path(path: &str) -> String {
    ...
    parts
        .iter()
        .cloned()
        .map(|p| percent_encode(&p, &mut SAFE_CHARS, false))
        .collect::<Vec<_>>()
        .join("/")
}
#[doc = "Decode URL path."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn decode_path(path: &str) -> Result<String, Box<dyn std::error::Error>> {
    Ok(percent_decode(path, false))
}
#[doc = "Check if text contains percent-encoding."]
pub fn is_encoded(text: &str) -> Result<bool, Box<dyn std::error::Error>> {
    let mut i = 0;
    while i < text.len() as i32 {
        if {
            let base = &text;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut key: DepylerValue = Default::default();
    let mut params: Vec<DepylerValue> = Default::default();
    ...
    if args.param.is_some() {
        ...
        for p in args.param {
            ...
            params.push(DepylerValue::Str(format!("{:?}", (key, value))));
        }
        println!("{}", encode_query_string(&params));
        return Ok(());
    }
    if !has_text {
        text = {
    ...
    if args.decode {
        if args.query {
            ...
            for (key, value) in params.iter().cloned() {
                println!("{}", format!("{}={}", key, value));
            }
        } else {
            if !args.path.is_empty() {
                println!("{:?}", decode_path(&text));
            } else {
                println!("{:?}", percent_decode(&text, args.plus));
            }
        }
    } else {
        if args.query {
            println!("{}", percent_encode(&text, &mut true, false));
        } else {
            if !args.path.is_empty() {
                println!("{}", encode_path(&text));
            } else {
                println!("{}", percent_encode(&text, &mut args.plus, false));
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn test_is_encoded_examples() {
        let _ = is_encoded(Default::default());
    }
}
