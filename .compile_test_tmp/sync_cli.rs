...
struct Args {
    ...
    #[doc = "Delete files in dest not in source"]
    delete: bool,
    
    
    #[doc = "Use hash comparison instead of mtime"]
    hash: bool,
    
    
    #[doc = "Show what would be done without making changes"]
    dry_run: bool,
    
    
    #[doc = "Show status without syncing"]
    status: bool,
}
#[doc = "Calculate MD5 hash of file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_file_hash(path: &str) -> Result<String, Box<dyn std::error::Error>> {
    let mut hasher = {
                use md5::Digest;
    ...
    match (|| -> Result<String, Box<dyn std::error::Error>> {
        ...
        loop {
            let chunk = {
                let mut _read_buf = vec![0u8; 8192];
...
#[doc = "Get file modification time."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_file_mtime(path: &str) -> f64 {
    {
...
#[doc = "List all files in directory recursively."]
#[doc = " Depyler: verified panic-free"]
        ...
        .collect::<Vec<_>>()
    {
        for filename in filenames.iter().cloned() {
            ...
            let rel_path = {
                ...
                let start_obj = std::path::Path::new(directory);
                path_obj
                    .strip_prefix(start_obj)
                    .map(|p| p.to_string_lossy().to_string())
                    .unwrap_or_else(|_| full_path.to_string())
            };
            files.push(DepylerValue::Str(format!("{:?}", rel_path)));
        }
    }
    {
...
#[doc = "Compare two files. Returns True if different."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compare_files<'b, 'a>(
    src: &'a str,
    dst: &'b str,
    use_hash: bool,
    ...
    let src_size = std::fs::metadata(&src).unwrap().len() as i64;
    let dst_size = std::fs::metadata(&dst).unwrap().len() as i64;
    let _cse_temp_0 = src_size != dst_size;
    if _cse_temp_0 {
        return Ok(true);
    }
    if use_hash {
        return Ok(get_file_hash(src)? != get_file_hash(dst)?);
    }
    Ok(get_file_mtime(src) > get_file_mtime(dst))
}
#[doc = "Find files to add, update, and delete.\n\n    Returns(to_add, to_update, to_delete).\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn find_changes<'a, 'b>(
    src_dir: &'a str,
    dst_dir: &'b str,
    use_hash: bool,
    ...
    let dst_files = list_files(dst_dir)
        .into_iter()
        .collect::<std::collections::HashSet<_>>();
    let to_add = src_files
        ...
        .collect::<std::collections::HashSet<_>>()
    {
        let src_path = std::path::PathBuf::from(src_dir)
            ...
            .to_string_lossy()
            .to_string();
        let dst_path = std::path::PathBuf::from(dst_dir)
            .join(f)
            .to_string_lossy()
            .to_string();
        if compare_files(&src_path, &dst_path, use_hash).unwrap_or(false) {
            to_update.push(DepylerValue::Str(format!("{:?}", f)));
        }
    }
    Ok((
        {
            let mut sorted_vec = to_add.iter().cloned().collect::<Vec<_>>();
            sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            sorted_vec
        },
        {
            let mut sorted_vec = to_update.iter().cloned().collect::<Vec<_>>();
            sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            sorted_vec
        },
        {
            let mut sorted_vec = to_delete.iter().cloned().collect::<Vec<_>>();
            sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
            sorted_vec
        },
    ))
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn sync_file<'a, 'b>(src: &'a str, dst: &'b str, dry_run: bool) -> bool {
    if dry_run {
        return true;
    }
    let mut dst_dir: Option<String> = None;
    match (|| -> Result<bool, Box<dyn std::error::Error>> {
        ...
        if (dst_dir.is_some()) && (!std::path::Path::new(&dst_dir).exists()) {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn delete_file(path: &str, dry_run: bool) -> bool {
    if dry_run {
        return true;
    }
    match (|| -> Result<bool, Box<dyn std::error::Error>> {
...
#[doc = "Sync src_dir to dst_dir.\n\n    Returns(added, updated, deleted).\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn sync_directories<'a, 'b>(
    src_dir: &'a str,
    dst_dir: &'b str,
    delete: bool,
    use_hash: bool,
    ...
    let mut updated: i32 = Default::default();
    let mut added: i32 = Default::default();
    let mut src: String = Default::default();
    let (to_add, to_update, to_delete) = find_changes(src_dir, dst_dir, use_hash)?;
    added = 0;
    updated = 0;
    let mut deleted = 0;
    for f in to_add.iter().cloned() {
        src = std::path::PathBuf::from(src_dir)
            .join(f)
            .to_string_lossy()
            .to_string();
        dst = std::path::PathBuf::from(dst_dir)
            .join(f)
            .to_string_lossy()
            .to_string();
    ...
    for f in to_update.iter().cloned() {
        src = std::path::PathBuf::from(src_dir)
            .join(f)
            .to_string_lossy()
            .to_string();
    ...
    if delete {
        for f in to_delete.iter().cloned() {
            dst = std::path::PathBuf::from(dst_dir)
                .join(f)
                .to_string_lossy()
                .to_string();
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !std::path::Path::new(&args.source).is_dir() {
        eprintln!("{}", format!("Error: {} is not a directory", args.source));
        std::process::exit(1i32)
    }
    if args.status {
        if !std::path::Path::new(&args.dest).is_dir() {
            println!("{}", format!("Destination does not exist: {}", args.dest));
            return Ok(());
        }
        let (to_add, to_update, to_delete) = find_changes(&args.source, &args.dest, args.hash)?;
        println!("{}", format!("To add: {} files", to_add.len() as i32));
        for f in to_add.iter().cloned() {
            println!("{}", format!("  + {:?}", f));
        }
        println!("{}", format!("To update: {} files", to_update.len() as i32));
        for f in to_update.iter().cloned() {
            println!("{}", format!("  ~ {:?}", f));
        }
        println!(
            "{}",
            format!("Would delete: {} files", to_delete.len() as i32)
        );
        for f in to_delete.iter().cloned() {
            println!("{}", format!("  - {:?}", f));
        }
    ...
    let (added, updated, deleted) = sync_directories(
        &args.source,
        &args.dest,
        args.delete,
        args.hash,
