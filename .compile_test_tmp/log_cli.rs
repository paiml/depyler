...
#![allow(dead_code)]
pub const LOG_PATTERN: String = "(\\S+)\\s+(\\S+)\\s+(\\S+)\\s+\\[([^\\]]+)\\]\\s+\"(\\S+)\\s+(\\S+)\\s+(\\S+)\"\\s+(\\d+)\\s+(\\S+)\\s*(?:\"([^\"]*)\"\\s*)?(?:\"([^\"]*)\")?".to_string().to_string();
...
struct Args {
    #[doc = "Log file to parse(- for stdin)"]
    ...
    #[doc = "Filter by HTTP method"]
    method: Option<String>,
    
    
    #[doc = "Show statistics instead of entries"]
    stats: bool,
    
    
    #[doc = "Show top N IPs by request count"]
    top_ips: Option<i32>,
}
#[doc = "Parse a single log line into components."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_log_line(line: &str) -> Option<HashMap<String, DepylerValue>> {
    let r#match = LOG_PATTERN.find(line.trim().to_string());
    if r#match.is_none() {
        return None;
    }
...
#[doc = "Filter log entries by status code."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn filter_by_status(entries: &Vec<DepylerValue>, status_code: i32) -> Vec<DepylerValue> {
    entries
        .iter()
        .cloned()
        .filter(|e| {
            let e = e.clone();
            e.get("status").cloned().unwrap_or_default() == status_code
        })
        .map(|e| e)
        .collect::<Vec<_>>()
}
#[doc = "Filter log entries by HTTP method."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn filter_by_method<'a, 'b>(
    entries: &'a Vec<DepylerValue>,
    method: &'b str,
) -> Vec<DepylerValue> {
    entries
        .iter()
        ...
        .filter(|e| {
            let e = e.clone();
            e.get("method").cloned().unwrap_or_default().to_uppercase() == method.to_uppercase()
        })
        .map(|e| e)
        .collect::<Vec<_>>()
}
#[doc = "Count requests per IP address."]
pub fn count_by_ip(
    entries: &Vec<DepylerValue>,
    ...
    for entry in entries.iter().cloned() {
        let ip = entry.get("ip").cloned().unwrap_or_default();
        counts.insert(
            ip.to_string().clone(),
            DepylerValue::Str(format!("{:?}", counts.get(&ip).cloned().unwrap_or(0) + 1)),
        );
    }
    Ok(counts)
}
#[doc = "Count requests per status code."]
pub fn count_by_status(
    ...
    for entry in entries.iter().cloned() {
        ...
        counts.insert(
            status.to_string().clone(),
            DepylerValue::Str(format!(
                "{:?}",
                counts.get(&status).cloned().unwrap_or(0) + 1
            )),
        );
    }
    Ok(counts)
}
#[doc = "Format a log entry for output."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_entry<'b, 'a>(
    entry: &'a std::collections::HashMap<DepylerValue, DepylerValue>,
    output_format: &'b str,
) -> String {
    let _cse_temp_0 = (*output_format) == "json";
    if _cse_temp_0 {
        return format!("{:?}", entry);
    } else {
        let _cse_temp_1 = (*output_format) == "csv";
        if _cse_temp_1 {
            return format!(
                "{},{},{},{},{}",
                entry.get("ip").cloned().unwrap_or_default(),
                entry.get("method").cloned().unwrap_or_default(),
                entry.get("path").cloned().unwrap_or_default(),
                entry.get("status").cloned().unwrap_or_default(),
...
pub fn main() -> Result<(), std::io::Error> {
    let mut entry: DepylerValue = Default::default();
    let mut lines: Vec<String> = Default::default();
    let mut entries: Vec<DepylerValue> = Default::default();
    let args = Args::default();
    let has_logfile = args.logfile.is_some();
    let has_status = args.status.is_some();
    let has_method = args.method.is_some();
    let has_top_ips = args.top_ips.is_some();
    let _cse_temp_0 = args.logfile == "-";
    let _cse_temp_1 = (!has_logfile) || (_cse_temp_0);
    if _cse_temp_1 {
        lines = {
            use std::io::BufRead;
            std::io::stdin()
        ...
        let mut f = std::fs::File::open(args.logfile)?;
        lines = std::io::BufReader::new(f)
            .lines()
    ...
    for line in lines.iter().cloned() {
        entry = Some(parse_log_line(&line));
        if let Some(ref entry_val) = entry {
            entries.push(DepylerValue::Str(format!("{:?}", entry_val)));
        } else {
    ...
    if args.stats {
        println!("{}", format!("Total entries: {}", entries.len() as i32));
        println!("{}", format!("Parse errors: {}", parse_errors));
        let status_counts = count_by_status(&entries)?;
        println!("{}", "Status codes:");
        for (status, count) in {
            let mut sorted_vec = status_counts
                .iter()
        ...
        if has_top_ips {
            let ip_counts = count_by_ip(&entries)?;
            let sorted_ips = {
                let mut __sorted_result = ip_counts
                    .iter()
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect::<Vec<_>>()
                    .clone();
            ...
            for (ip, count) in {
                let base = &sorted_ips;
                let stop_idx = (args.top_ips) as isize;
                ...
                println!("{}", format!("{:?}: {:?}", ip, count));
            }
        } else {
            for entry in entries.iter().cloned() {
                println!("{}", format_entry(entry, &args.format));
            }
        }
    }
    Ok(())
