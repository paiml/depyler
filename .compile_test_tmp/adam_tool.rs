...
pub fn adam_step<'b, 'a, 'l1, 'c>(
    ...
    for i in 0..(params.len() as i32) {
        ...
        let m_hat = ((m_i) as f64) / ((1f64 - ({ beta1 } as f64).powf({ t } as f64)) as f64);
        let v_hat = ((v_i) as f64) / ((1f64 - ({ beta2 } as f64).powf({ t } as f64)) as f64);
        let param_i = params
            .get(i as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            - ((lr * m_hat) as f64) / (((v_hat as f64).sqrt() + eps) as f64);
        new_params.push(DepylerValue::Float(param_i as f64));
        new_m.push(DepylerValue::Float(m_i as f64));
        new_v.push(DepylerValue::Float(v_i as f64));
    }
    Ok((new_params, new_m, new_v))
}
#[doc = "Initialize Adam optimizer state."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn init_adam(n_params: i32) -> HashMap<String, DepylerValue> {
    {
        let mut map = HashMap::new();
        map.insert(
            "m".to_string(),
            DepylerValue::Str(format!("{:?}", vec![0.0; n_params as usize])),
        );
        map.insert(
            "v".to_string(),
            DepylerValue::Str(format!("{:?}", vec![0.0; n_params as usize])),
        );
        map.insert("t".to_string(), DepylerValue::Int(0 as i64));
        map
    }
}
#[doc = "Handle step subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_step() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = data.get("params").is_none();
    let _cse_temp_1 = data.get("grads").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'params' or 'grads'");
        std::process::exit(1);
    }
    let params = data.get("params").cloned().unwrap_or_default();
    let grads = data.get("grads").cloned().unwrap_or_default();
    let lr = data.get("lr").cloned().unwrap_or(0.001);
    let beta1 = data.get("beta1").cloned().unwrap_or(0.9);
    let beta2 = data.get("beta2").cloned().unwrap_or(0.999);
    let eps = data.get("eps").cloned().unwrap_or(0.00000001);
    let m = data
        .get("m")
        .cloned()
        .unwrap_or(vec![0.0; (params.len() as i32) as usize]);
    let v = data
        .get("v")
        .cloned()
        .unwrap_or(vec![0.0; (params.len() as i32) as usize]);
    let t = data.get("t").cloned().unwrap_or(1);
    let _cse_temp_3 = lr == 0f64;
    if _cse_temp_3 {
        println!(
            "{}",
            format!("{:?}", {
    ...
    let (new_params, new_m, new_v) =
        adam_step(&params, &grads, &m, &v, &t, &lr, &beta1, &beta2, &eps)?;
    println!(
        "{}",
        format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("params".to_string(), new_params);
            map.insert("m".to_string(), new_m);
            map.insert("v".to_string(), new_v);
            map.insert("t".to_string(), t + 1);
            map
        })
    );
    Ok(())
}
#[doc = "Handle init subcommand."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_init() {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
    ...
    let n_params = data.get("n_params").cloned().unwrap_or(1);
    let lr = data.get("lr").cloned().unwrap_or(0.001);
    let mut state = init_adam(&n_params);
    state.insert("lr".to_string(), DepylerValue::Str(format!("{:?}", lr)));
