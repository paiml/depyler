...
struct Args {
    ...
    #[doc = "Calculate as of date(default: today)"]
    as_of: Option<String>,
    ...
    #[doc = "Show next birthday"]
    next: bool,
    
    
    #[doc = "Show zodiac sign"]
    zodiac: bool,
...
#[doc = "Parse date string into date object."]
#[doc = " Depyler: verified panic-free"]
pub fn parse_date(date_str: &str) -> Option<DepylerDate> {
    let formats = vec![
        "%Y-%m-%d".to_string(),
        "%d/%m/%Y".to_string(),
        "%m/%d/%Y".to_string(),
...
#[doc = "Calculate age in years, months, days."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn calculate_age<'b, 'a>(
    birth_date: &'a DepylerDate,
    as_of: &'b mut Option<DepylerDate>,
) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>> {
    let mut years: i32 = Default::default();
    let mut months: i32 = Default::default();
    let mut days: DepylerValue = Default::default();
    if as_of.is_none() {
        as_of = Some(DepylerDate::today());
    }
    let _cse_temp_0 = as_of.year() as i32 - birth_date.year() as i32;
    years = _cse_temp_0;
    let _cse_temp_1 = (as_of.month() as i32, as_of.day() as i32)
        < (birth_date.month() as i32, birth_date.day() as i32);
    if _cse_temp_1 {
        years = years - 1;
    }
    months = _cse_temp_0;
    let _cse_temp_2 = (as_of.day() as i32) < birth_date.day() as i32;
    if _cse_temp_2 {
        months = months - 1;
    }
    let _cse_temp_3 = months < 0;
    if _cse_temp_3 {
        months = months + 12;
    }
    days = _cse_temp_0;
    let _cse_temp_4 = days < 0;
    if _cse_temp_4 {
        let prev_month = if as_of.month() as i32 > 1 {
            as_of.month() as i32 - 1
        } else {
            12
        };
        let prev_year = if as_of.month() as i32 > 1 {
            as_of.year() as i32
        } else {
            as_of.year() as i32 - 1
        };
    ...
    Ok({
        ...
        map.insert("years".to_string(), DepylerValue::Int(years as i64));
        map.insert("months".to_string(), DepylerValue::Int(months as i64));
        map.insert("days".to_string(), DepylerValue::Str(format!("{:?}", days)));
        map
    })
}
...
pub fn days_in_month(year: i32, month: i32) -> Result<i32, Box<dyn std::error::Error>> {
    let _cse_temp_0 = vec![1, 3, 5, 7, 8, 10, 12].contains(&month);
    if _cse_temp_0 {
        return Ok(31);
    }
    if _cse_temp_0 {
...
#[doc = "Calculate total days lived."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn total_days<'b, 'a>(birth_date: &'a DepylerDate, as_of: &'b mut Option<DepylerDate>) -> i32 {
    if as_of.is_none() {
        as_of = Some(DepylerDate::today());
    }
    *as_of - *birth_date.days() as i32
}
#[doc = "Calculate next birthday."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn next_birthday<'a, 'b>(
    birth_date: &'a DepylerDate,
    as_of: &'b mut Option<DepylerDate>,
) -> DepylerDate {
    if as_of.is_none() {
    ...
    let mut this_year: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        this_year = Some(DepylerDate::new(
            as_of.year() as i32 as u32,
            birth_date.month() as i32 as u32,
            birth_date.day() as i32 as u32,
        ));
        Ok(())
    })() {
        Ok(()) => {}
    ...
    let _cse_temp_0 = this_year > (*as_of);
    if _cse_temp_0 {
        return this_year;
    }
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(DepylerDate::new(
            as_of.year() as i32 + 1 as u32,
            birth_date.month() as i32 as u32,
            birth_date.day() as i32 as u32,
        ));
    })() {
        ...
        Err(_) => {
            return DepylerDate::new(as_of.year() as i32 + 1 as u32, 3 as u32, 1 as u32);
        }
    }
}
#[doc = "Days until next birthday."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn days_until_birthday<'a, 'b>(
    birth_date: &'a DepylerDate,
    as_of: &'b mut Option<DepylerDate>,
) -> i32 {
    if as_of.is_none() {
    ...
    next_birthday(&birth_date, &mut as_of) - *as_of.days() as i32
}
#[doc = "Get zodiac sign for birth date."]
pub fn zodiac_sign(birth_date: &DepylerDate) -> Result<String, Box<dyn std::error::Error>> {
    let month = birth_date.month() as i32;
    let day = birth_date.day() as i32;
    let signs = vec![
        (1, 20, "Capricorn".to_string().to_string()),
        (2, 19, "Aquarius".to_string()),
        (3, 20, "Pisces".to_string()),
        (4, 20, "Aries".to_string()),
        (5, 21, "Taurus".to_string()),
        (6, 21, "Gemini".to_string()),
        (7, 22, "Cancer".to_string()),
        (8, 23, "Leo".to_string()),
        (9, 23, "Virgo".to_string()),
        (10, 23, "Libra".to_string()),
        (11, 22, "Scorpio".to_string()),
        (12, 22, "Sagittarius".to_string()),
    ];
    for (end_month, end_day, sign) in signs.iter().cloned() {
        ...
        if month < end_month {
            let idx = signs
                .iter()
                .position(|x| x == &(end_month, end_day, sign))
                .map(|i| i as i32)
    ...
    Ok("Capricorn".to_string())
}
#[doc = "Get day of week name."]
#[doc = " Depyler: proven to terminate"]
pub fn day_of_week(d: &DepylerDate) -> Result<String, Box<dyn std::error::Error>> {
    let days = vec![
        "Monday".to_string(),
        "Tuesday".to_string(),
        "Wednesday".to_string(),
...
#[doc = "Format age dict for display."]
#[doc = " Depyler: proven to terminate"]
pub fn format_age(
    age: &std::collections::HashMap<DepylerValue, DepylerValue>,
) -> Result<String, Box<dyn std::error::Error>> {
    let mut parts = vec![];
    let _cse_temp_0 = age.get("years").cloned().unwrap_or_default() > 0;
    if _cse_temp_0 {
        parts.push(DepylerValue::Str(format!(
            "{:?}",
            format!(
                "{} year{}",
                age.get("years").cloned().unwrap_or_default(),
                if age.get("years").cloned().unwrap_or_default() != 1 {
                    "s".to_string()
                } else {
                    "".to_string()
                }
    ...
    if _cse_temp_0 {
        parts.push(DepylerValue::Str(format!(
            ...
            format!(
                "{} month{}",
                age.get("months").cloned().unwrap_or_default(),
                if age.get("months").cloned().unwrap_or_default() != 1 {
                    "s".to_string()
                } else {
                    "".to_string()
                }
    ...
    let _cse_temp_1 = (_cse_temp_0) || (parts.is_empty());
    if _cse_temp_1 {
        parts.push(DepylerValue::Str(format!(
            "{:?}",
            format!(
                "{} day{}",
                age.get("days").cloned().unwrap_or_default(),
                if age.get("days").cloned().unwrap_or_default() != 1 {
                    "s".to_string()
                } else {
                    "".to_string()
                }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut days: DepylerValue = Default::default();
    let mut as_of: DepylerValue = Default::default();
    let mut nb: DepylerValue = Default::default();
    let args = Args::default();
    let has_as_of = args.as_of.is_some();
    let birth = parse_date(&args.birthdate);
    if birth.is_none() {
        eprintln!("{}", format!("Invalid date format: {}", args.birthdate));
        std::process::exit(1i32)
    }
    if has_as_of {
        as_of = Some(parse_date(&args.as_of));
        if as_of.is_none() {
            eprintln!(
                "{}",
    ...
    let age = calculate_age(birth, &mut as_of)?;
    if args.json {
        let result = {
            let mut map = HashMap::new();
            map.insert("birth_date".to_string(), (birth).unwrap().to_string());
            map.insert("age".to_string(), age);
            map.insert("total_days".to_string(), total_days(birth, &mut as_of));
            map.insert(
                "next_birthday".to_string(),
                (next_birthday(birth, &mut as_of)).to_string(),
            );
            map.insert(
                "days_until_birthday".to_string(),
                days_until_birthday(birth, &mut as_of),
            );
            map.insert(
                "zodiac".to_string(),
                zodiac_sign(birth).expect("zodiac_sign failed"),
            );
            map.insert(
                "birth_day".to_string(),
                day_of_week(birth).expect("day_of_week failed"),
            );
            map
        };
    ...
    if args.days() as i32 {
        println!("{}", total_days(birth, &mut as_of));
        return Ok(());
    }
    if args.next {
        nb = next_birthday(birth, &mut as_of);
        days = days_until_birthday(birth, &mut as_of);
        println!("{}", format!("Next birthday: {:?}({:?} days)", nb, days));
        return Ok(());
    }
    if args.zodiac {
        println!("{:?}", zodiac_sign(birth));
        return Ok(());
    }
    println!(
        "{}",
        format!("Birth date: {:?}({:?})", birth, day_of_week(birth))
    );
    println!("{}", format!("Age: {:?}", format_age(&age)));
    println!(
        "{}",
        format!("Total days: {}", total_days(birth, &mut as_of))
    );
    println!("{}", format!("Zodiac: {:?}", zodiac_sign(birth)));
    nb = next_birthday(birth, &mut as_of);
    days = days_until_birthday(birth, &mut as_of);
    println!(
        "{}",
        format!("Next birthday: {:?}({:?} days away)", nb, days)
    );
    Ok(())
}
#[cfg(test)]
