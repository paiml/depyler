...
#[doc = "Base64 encode. Depyler: proven to terminate"]
...
pub fn cmd_encode() {
    ...
    .trim()
    .to_string();
    let encoded = format!("{:?}", data.as_bytes().to_vec())
        .into_bytes();
    println!("{}", encoded);
}
#[doc = "Base64 decode. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_decode() {
    let data = {
...
#[doc = "URL-safe base64 encode. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_urlsafe_encode() {
    let data = {
        use std::io::Read;
        let mut buffer = String::new();
        std::io::stdin().read_to_string(&mut buffer).unwrap();
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Encode { .. }));
    match &args.command {
