...
enum Commands {
    ...
    Filter { threshold: i32, items: Vec<i32> },
    
    Adder { n: i32, x: i32 },
    
    Multiplier { n: i32, x: i32 },
...
#[doc = "Create function that adds n."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_adder(n: i32) -> Box<dyn Fn(i32) -> i32> {
    let adder = move |x: i32| -> i32 {
        return x + n;
    };
    Box::new(adder)
}
#[doc = "Create function that multiplies by n."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_multiplier(n: i32) -> Box<dyn Fn(i32) -> i32> {
    let multiplier = move |x: i32| -> i32 {
        return x * n;
    };
    Box::new(multiplier)
}
#[doc = "Create function that raises to power exp."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_power(exp: i32) -> Box<dyn Fn(i32) -> i32> {
    let power = move |x: i32| -> i32 {
        return {
            if exp >= 0 && (exp as i64) <= (u32::MAX as i64) {
                ...
                ({ x } as f64).powf({ exp } as f64) as i32
            }
        };
    };
    Box::new(power)
}
#[doc = "Create range checker function."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_range_checker(min_val: i32, max_val: i32) -> Box<dyn Fn(i32) -> bool> {
    let checker = move |x: i32| -> bool {
        return (min_val <= x) && (x <= max_val);
    };
    Box::new(checker)
}
#[doc = "Create string formatter with prefix."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_prefix_formatter(prefix: String) -> Box<dyn Fn(String) -> String> {
    let formatter = move |s: &str| -> String {
        return format!("{}{}", prefix, s);
    };
    Box::new(formatter)
}
#[doc = "Create string formatter with suffix."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_suffix_formatter(suffix: String) -> Box<dyn Fn(String) -> String> {
    let formatter = move |s: &str| -> String {
        return format!("{}{}", s, suffix);
    };
    Box::new(formatter)
}
#[doc = "Create counter that increments each call."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_counter() -> Box<dyn Fn() -> i32> {
    let count = vec![0];
    let counter = move || -> i32 {
        count.insert(
            (0) as usize,
            count
                .get(0usize)
...
#[doc = "Create accumulator with add and get functions."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_accumulator(initial: i32) -> (impl Fn(i32) -> i32, impl Fn() -> i32) {
    let total = vec![initial];
    let add = move |x: i32| -> i32 {
        total.insert(
            (0) as usize,
            total
                .get(0usize)
...
#[doc = "Create filter for values above threshold."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_threshold_filter(threshold: i32) -> Box<dyn Fn(Vec<i32>) -> Vec<i32>> {
    let filter_fn = move |items: &Vec<i32>| -> Vec<i32> {
        return items
            .as_slice()
            .iter()
            .cloned()
...
#[doc = "Create validator that checks all rules."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_validator(rules: &Vec<impl Fn(String) -> bool>) -> Box<dyn Fn(String) -> bool> {
    let validate = move |value: &str| -> bool {
        return rules.iter().cloned().map(|rule| rule(&value)).all(|x| x);
    };
    Box::new(validate)
}
#[doc = "Create chain of transformers."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_transformer_chain(fns: &Vec<impl Fn(i32) -> i32>) -> Box<dyn Fn(i32) -> i32> {
    let transform = move |x: i32| -> i32 {
        let mut result = x.clone();
        for _fn in fns.iter().cloned() {
            result = r#fn(result);
        }
        return result;
    };
...
#[doc = "Create simple cache with set and get."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_cache() -> (impl Fn(String, i32), impl Fn(String) -> Option<i32>) {
    let cache: std::collections::HashMap<String, i32> = {
        let map: HashMap<String, i32> = HashMap::new();
        map
    };
    let set_value = move |key: &str, value: i32| -> () {
        cache.insert(key.to_string().clone(), value);
    };
    let get_value = move |key: &str| -> Option<i32> {
        return cache.get(&key).cloned();
    };
    (set_value, get_value)
}
#[doc = "Create rate limiter that allows max_calls."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_rate_limiter(max_calls: i32) -> Box<dyn Fn() -> bool> {
    let calls = vec![0];
    let check = move || -> bool {
        if calls
            .get(0usize)
...
#[doc = "Create toggle function."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_toggler(initial: bool) -> Box<dyn Fn() -> bool> {
    let state = vec![initial];
    let toggle = move || -> bool {
        state.insert(
            (0) as usize,
            !state
                .get(0usize)
...
#[doc = "Create function that runs inner function only once."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_once() -> Box<dyn Fn(&dyn Fn() -> i32) -> &dyn Fn() -> i32> {
    let once_wrapper = move |r#fn: &dyn Fn() -> i32| -> &dyn Fn() -> i32 {
        let called = vec![false];
        let result: Vec<i32> = vec![];
        let wrapped = move || -> i32 {
            if !called
                .get(0usize)
                .cloned()
                .expect("IndexError: list index out of range")
    ...
    Box::new(once_wrapper)
}
#[doc = "Create comparator function for dict key."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_comparator(
    key: &str,
) -> Box<
    dyn Fn(std::collections::HashMap<String, i32>, std::collections::HashMap<String, i32>) -> i32,
> {
    let compare = move |a: &std::collections::HashMap<String, i32>,
                        b: &std::collections::HashMap<String, i32>|
          -> i32 {
        let va = a.get(key).cloned().unwrap_or(0);
        let vb = b.get(key).cloned().unwrap_or(0);
        if va < vb {
            return -1;
        } else {
            if va > vb {
...
#[doc = "Create function that gets with default."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_default_factory(
    default: i32,
) -> Box<dyn Fn(std::collections::HashMap<String, i32>, String) -> i32> {
    let getter = move |d: &std::collections::HashMap<String, i32>, key: &str| -> i32 {
        return d.get(&key).cloned().unwrap_or(default);
    };
    Box::new(getter)
}
#[doc = "Create bounded counter with inc, dec, get."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_bounded_counter(
    min_val: i32,
    max_val: i32,
) -> (impl Fn() -> i32, impl Fn() -> i32, impl Fn() -> i32) {
    let value = vec![min_val];
    let inc = move || -> i32 {
        if value
            .get(0usize)
            .cloned()
...
#[doc = "Create string builder with append and build."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn make_string_builder() -> (impl Fn(String), impl Fn() -> String) {
    let parts: Vec<String> = vec![];
    let append = move |s: &str| -> () {
        parts.push(s);
    };
    let build = move || -> String {
        return parts.join("");
    };
    (append, build)
}
#[doc = "Compose two functions:(f.g)(x) = f(g(x))."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn compose(f: impl Fn(i32) -> i32, g: impl Fn(i32) -> i32) -> Box<dyn Fn(i32) -> i32> {
    let composed = move |x: i32| -> i32 {
        return f(g(x));
    };
    Box::new(composed)
}
#[doc = "Apply function n times."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn apply_n_times(f: impl Fn(i32) -> i32, n: i32) -> Box<dyn Fn(i32) -> i32> {
    let applied = move |x: i32| -> i32 {
        let mut result = x.clone();
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Adder { .. }));
    match &args.command {
        Some(Commands::Adder { ref n, ref x, .. }) => {
            let n = *n;
            let x = *x;
            let adder = make_adder(n);
            println!("{}", format!("{} + {} = {}", x, n, adder(x)));
        }
        Some(Commands::Multiplier { ref n, ref x, .. }) => {
            let n = *n;
            let x = *x;
            let multiplier = make_multiplier(n);
            println!("{}", format!("{} * {} = {}", x, n, multiplier(x)));
        }
        Some(Commands::Counter { ref calls, .. }) => {
            let calls = calls.clone();
            let counter = make_counter();
            for __sanitized in 0..(calls) {
        ...
        Some(Commands::Filter {
            ...
            let filter_fn = make_threshold_filter(threshold);
            let result = filter_fn(items);
