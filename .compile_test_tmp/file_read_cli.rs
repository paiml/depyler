...
#[doc = "Read file as list of lines."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_lines(path: &str) -> Result<Vec<String>, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    return Ok(std::io::BufReader::new(f)
        .lines()
        .map(|l| l.unwrap_or_default())
        .collect::<Vec<_>>());
}
#[doc = "Read file as list of stripped lines."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_lines_stripped(path: &str) -> Result<Vec<String>, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    return Ok(std::io::BufReader::new(f)
        .lines()
        .map(|l| l.unwrap_or_default())
...
#[doc = "Read specific line from file(1-indexed)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_line(path: &str, line_num: i32) -> Result<String, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    for (i, line) in f
        .iter()
        .cloned()
...
#[doc = "Read first n lines from file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_first_n_lines(path: &str, n: i32) -> Result<Vec<String>, std::io::Error> {
    let mut lines = vec![];
    let mut f = std::fs::File::open(&path)?;
    for (i, line) in std::io::BufReader::new(f)
        .lines()
        .map(|l| l.unwrap_or_default())
        .enumerate()
        .map(|(i, x)| (i as i32, x))
...
#[doc = "Read last n lines from file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_last_n_lines(path: &str, n: i32) -> Result<Vec<String>, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
...
#[doc = "Count number of lines in file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_lines(path: &str) -> Result<i32, std::io::Error> {
    let mut count = 0;
    let mut f = std::fs::File::open(&path)?;
    for __sanitized in std::io::BufReader::new(f)
        .lines()
        .map(|l| l.unwrap_or_default())
    {
        count = count + 1;
...
#[doc = "Count number of words in file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_words(path: &str) -> Result<i32, std::io::Error> {
    let mut count = 0;
    let mut f = std::fs::File::open(&path)?;
...
#[doc = "Count number of characters in file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_chars(path: &str) -> Result<i32, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    return Ok({
        let mut content = String::new();
        f.read_to_string(&mut content)?;
...
#[doc = "Read file as binary."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_binary(path: &str) -> Result<Vec<u8>, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    return Ok({
        let mut content = String::new();
        f.read_to_string(&mut content)?;
...
#[doc = "Read file in chunks."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_chunks(path: &str, chunk_size: i32) -> Result<Vec<String>, std::io::Error> {
    let mut chunks = vec![];
    let mut f = std::fs::File::open(&path)?;
    loop {
        let chunk = {
            let mut _read_buf = vec![0u8; chunk_size];
        ...
        chunks.push(DepylerValue::Str(chunk.to_string()));
    }
    Ok(chunks)
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn search_in_file<'b, 'a>(
    path: &'a str,
    pattern: &'b str,
) -> Result<Vec<(i32, String)>, std::io::Error> {
    let mut matches = vec![];
    let mut f = std::fs::File::open(&path)?;
    for (i, line) in f
        .iter()
        ...
        .map(|(i, x)| ((i + 1 as usize) as i32, x))
    {
        ...
        if line.contains(&*pattern) {
            matches.push(DepylerValue::Str(format!(
                "{:?}",
                (i, line.trim_end().to_string())
            )));
        }
    }
    Ok(matches)
}
#[doc = "Read simple CSV file without csv module."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_csv_simple<'a, 'b>(
    path: &'a str,
    delimiter: &'b str,
) -> Result<Vec<Vec<String>>, std::io::Error> {
    let mut rows = vec![];
    let mut f = std::fs::File::open(&path)?;
    for mut line in std::io::BufReader::new(f)
        .lines()
        .map(|l| l.unwrap_or_default())
    {
        line = line.trim().to_string();
        if !line.is_empty() {
            rows.push(DepylerValue::Str(format!(
                "{:?}",
...
#[doc = "Read key=value file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_key_value<'b, 'a>(
    path: &'a str,
    delimiter: &'b str,
...
#[doc = "Check if file exists by trying to open it."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn file_exists(path: &str) -> Result<bool, Box<dyn std::error::Error>> {
    {
        let mut _context = std::fs::File::open(&path)?;
        return Ok(true);
        return Ok(false);
    }
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_file_size(path: &str) -> Result<i32, std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    f.seek(0, 2);
    return Ok(f.tell());
}
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        match &args.command {
            ...
            Some(Commands::Lines {
                ...
                if n.is_some() {
                    if tail {
                        lines = read_last_n_lines(&file, n)?;
                    } else {
                        lines = read_first_n_lines(&file, n)?;
                    }
                } else {
                    lines = read_lines_stripped(&file)?;
                }
            ...
            Some(Commands::Count {
                ...
                let words = *words;
                if lines {
                    println!("{}", format!("Lines: {:?}", count_lines(&file)));
                }
                if words {
                    println!("{}", format!("Words: {:?}", count_words(&file)));
                }
                if chars {
                    println!("{}", format!("Chars: {:?}", count_chars(&file)));
                }
            ...
            Some(Commands::Search {
                ...
                matches = search_in_file(&file, &pattern)?;
                for (line_num, line) in matches.iter().cloned() {
                    println!("{}", format!("{}: {}", line_num, line));
                }
                println!("{}", format!("Found {} matches", matches.len() as i32));
            }
            Some(Commands::Size { ref file, .. }) => {
                size = Some(get_file_size(&file)?);
                println!("{}", format!("{:?} bytes", size));
            }
            Some(Commands::Exists { ref file, .. }) => {
                if file_exists(&file).unwrap_or(false) {
                    println!("{}", format!("'{}' exists", file));
                } else {
                    println!("{}", format!("'{}' does not exist", file));
                }
            }
...
mod tests {
    ...
    #[test]
    fn test_count_lines_examples() {
        assert_eq!(count_lines(""), 0);
        assert_eq!(count_lines("a"), 1);
        assert_eq!(count_lines("abc"), 3);
    }
    #[test]
    ...
    #[test]
    fn test_count_chars_examples() {
        assert_eq!(count_chars(""), 0);
        assert_eq!(count_chars("a"), 1);
        assert_eq!(count_chars("abc"), 3);
    }
    #[test]
    fn test_file_exists_examples() {
        let _ = file_exists(Default::default());
    }
    #[test]
