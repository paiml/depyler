...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'contextlib'(tracked in DEPYLER-0424)"]
use std::io::Read;
use std::io::Write;
...
pub fn redirect_stdout(_args: impl std::any::Any) -> () {}
#[doc = r" Python FileNotFoundError stub for contextlib.suppress()"]
#[allow(dead_code)]
pub struct FileNotFoundError;
#[doc = r" Python PermissionError stub for contextlib.suppress()"]
#[allow(dead_code)]
pub struct PermissionError;
#[doc = r" Python OSError stub for contextlib.suppress()"]
#[allow(dead_code)]
pub struct OSError;
#[doc = "Create temp file. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_tempfile(content: &str) {
    let mut f = tempfile::NamedTempFile::new().unwrap();
    f.write_all(content.as_bytes()).unwrap();
    println!(
        "{}",
        format!("Created: {}", f.path().to_string_lossy().to_string())
    );
}
#[doc = "Redirect stdout to file. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_redirect(message: &str, output: &str) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::create(&output)?;
    let mut _context = redirect_stdout(f);
    println!("{}", message);
    println!("{}", format!("Written to: {}", output));
    Ok(())
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Tempfile { .. }));
    match &args.command {
        Some(Commands::Tempfile { ref content, .. }) => {
            let content = content.as_deref().unwrap_or_default();
            cmd_tempfile(content);
        }
