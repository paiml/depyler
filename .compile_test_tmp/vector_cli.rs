...
impl Vector {
    ...
    pub fn len(&self) -> i32 {
        return self.components.clone().len() as i32;
    }
    pub fn index(&self, i: i32) -> f64 {
        return {
            let _base = &self.components.clone();
            let _idx = i;
            if _idx < 0 {
...
struct Args {
    ...
    #[doc = "First vector(comma-separated)"]
    v1: Option<String>,
    
    #[doc = "Second vector(comma-separated)"]
    v2: Option<String>,
}
...
pub fn add<'a, 'b>(v1: &'a Vector, v2: &'b Vector) -> Result<Vector, Box<dyn std::error::Error>> {
    let _cse_temp_0 = v1.len() as i32;
    let _cse_temp_1 = v2.len() as i32;
...
#[doc = " Depyler: proven to terminate"]
pub fn scalar_multiply(v: &Vector, scalar: f64) -> Vector {
    Vector::new(
        v.components
            .into_iter()
            .map(|c| c * scalar)
...
#[doc = "Calculate dot product."]
#[doc = " Depyler: proven to terminate"]
pub fn dot<'a, 'b>(v1: &'a Vector, v2: &'b Vector) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = v1.len() as i32;
    let _cse_temp_1 = v2.len() as i32;
    let _cse_temp_2 = _cse_temp_0 != _cse_temp_1;
    if _cse_temp_2 {
...
#[doc = "Calculate cross product(3D only)."]
#[doc = " Depyler: proven to terminate"]
pub fn cross<'a, 'b>(v1: &'a Vector, v2: &'b Vector) -> Result<Vector, Box<dyn std::error::Error>> {
    let _cse_temp_0 = v1.len() as i32;
    let _cse_temp_1 = _cse_temp_0 != 3;
    let _cse_temp_2 = v2.len() as i32;
...
#[doc = "Calculate vector magnitude(Euclidean norm)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn magnitude(v: &Vector) -> f64 {
    (v.components.into_iter().map(|c| c * c).sum::<f64>() as f64).sqrt()
}
#[doc = "Normalize vector to unit length."]
#[doc = " Depyler: proven to terminate"]
pub fn normalize(v: &Vector) -> Result<Vector, Box<dyn std::error::Error>> {
    let mag = magnitude(&v);
    let _cse_temp_0 = mag == 0f64;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new(
    ...
    Ok(Vector::new(
        v.components
            .into_iter()
            .map(|c| ((c) as f64) / ((mag) as f64))
            .collect::<Vec<_>>(),
    ))
}
#[doc = "Calculate Euclidean distance between vectors."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn distance<'a, 'b>(v1: &'a Vector, v2: &'b Vector) -> Result<f64, Box<dyn std::error::Error>> {
    Ok(magnitude(subtract(&v1, &v2)))
}
#[doc = "Calculate angle between vectors in radians."]
#[doc = " Depyler: proven to terminate"]
pub fn angle_between<'b, 'a>(
    v1: &'a Vector,
    v2: &'b Vector,
) -> Result<f64, Box<dyn std::error::Error>> {
    let mag1 = magnitude(&v1);
    let mag2 = magnitude(&v2);
    let _cse_temp_0 = mag1 == 0f64;
    let _cse_temp_1 = mag2 == 0f64;
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        return Err(Box::new(ValueError::new(
    ...
    let _cse_temp_3 = mag1 * mag2;
    let _cse_temp_4 = ((dot(&v1, &v2)?) as f64) / ((_cse_temp_3) as f64);
    let mut cos_angle = _cse_temp_4.clone();
    let _cse_temp_5 = depyler_min((1).clone(), (cos_angle).clone());
    let _cse_temp_6 = depyler_max((-1).clone(), (_cse_temp_5).clone());
    cos_angle = _cse_temp_6;
    Ok((cos_angle as f64).acos())
}
#[doc = "Project v onto another vector."]
#[doc = " Depyler: proven to terminate"]
pub fn project<'a, 'b>(
    v: &'a Vector,
    onto: &'b Vector,
) -> Result<Vector, Box<dyn std::error::Error>> {
    let onto_mag_sq = dot(&onto, &onto)?;
    let _cse_temp_0 = onto_mag_sq == 0f64;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new(
            "Cannot project onto zero vector".to_string(),
        )));
    }
    let _cse_temp_1 = ((dot(&v, &onto)?) as f64) / ((onto_mag_sq) as f64);
    let scalar = _cse_temp_1;
    Ok(scalar_multiply(&onto, scalar))
}
#[doc = "Calculate rejection of v from another vector."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reject<'b, 'a>(
    v: &'a Vector,
    ...
    Ok(subtract(&v, project(&v, &from_)))
}
#[doc = "Reflect vector across a plane defined by normal."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn reflect<'a, 'b>(
    v: &'a Vector,
    ...
    Ok(subtract(&v, scalar_multiply(&n, 2f64 * dot(&v, &n)?)))
}
#[doc = "Linear interpolation between vectors."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn lerp<'b, 'a>(
    v1: &'a Vector,
    ...
    Ok(add(scalar_multiply(&v1, 1f64 - t), scalar_multiply(&v2, t)))
}
...
#[doc = "Calculate scalar triple product: v1 ·(v2 × v3)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn triple_scalar<'b, 'a, 'c>(
    v1: &'a Vector,
    v2: &'b Vector,
    v3: &'c Vector,
) -> Result<f64, Box<dyn std::error::Error>> {
    Ok(dot(&v1, cross(&v2, &v3)))
}
#[doc = "Calculate vector triple product: v1 ×(v2 × v3)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn triple_vector<'a, 'c, 'b>(
    v1: &'a Vector,
    v2: &'b Vector,
    v3: &'c Vector,
) -> Result<Vector, Box<dyn std::error::Error>> {
    Ok(cross(&v1, cross(&v2, &v3)))
}
#[doc = "Check if two vectors are orthogonal."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_orthogonal<'a, 'b>(
    v1: &'a Vector,
    v2: &'b Vector,
    tolerance: f64,
) -> Result<bool, Box<dyn std::error::Error>> {
    Ok(((dot(&v1, &v2)).abs() as f64) < tolerance)
}
#[doc = "Check if two vectors are parallel."]
#[doc = " Depyler: proven to terminate"]
pub fn is_parallel<'a, 'b>(
    v1: &'a Vector,
    v2: &'b Vector,
    tolerance: f64,
) -> Result<bool, Box<dyn std::error::Error>> {
    let mut ratio: i32 = Default::default();
    let _cse_temp_0 = v1.len() as i32;
    let _cse_temp_1 = v2.len() as i32;
    let _cse_temp_2 = _cse_temp_0 != _cse_temp_1;
    if _cse_temp_2 {
    ...
    if _cse_temp_3 {
        let c = cross(&v1, &v2)?;
        return Ok(magnitude(&c) < tolerance);
    }
    let _cse_temp_4 = magnitude(&v2) == 0f64;
    if _cse_temp_4 {
        return Ok(magnitude(&v1) == 0f64);
    }
...
#[doc = "Calculate outer product(returns matrix)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn outer_product<'b, 'a>(v1: &'a Vector, v2: &'b Vector) -> Vec<Vec<f64>> {
    (0..(v1.len() as i32))
        .into_iter()
        .map(|i| {
            (0..(v2.len() as i32))
...
#[doc = "Calculate scalar component of v in given direction."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn component_in_direction<'a, 'b>(
    v: &'a Vector,
    direction: &'b Vector,
) -> Result<f64, Box<dyn std::error::Error>> {
    Ok(dot(&v, normalize(&direction)))
}
#[doc = "Gram-Schmidt orthogonalization."]
#[doc = " Depyler: verified panic-free"]
pub fn gram_schmidt(vectors: &Vec<Vector>) -> Result<Vec<Vector>, Box<dyn std::error::Error>> {
    if vectors.is_empty() {
        return Ok(vec![]);
    }
    let mut orthogonal = vec![];
    for v in vectors.iter().cloned() {
        let mut result = v.clone();
        for u in orthogonal.iter().cloned() {
            result = subtract(&result, project(&v, u))?;
        }
        if magnitude(&result) > 0.0000000001 {
            orthogonal.push(DepylerValue::Str(format!("{:?}", result)));
        }
    }
    Ok(orthogonal)
}
#[doc = "Orthonormalize a set of vectors."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn orthonormalize(vectors: &Vec<Vector>) -> Result<Vec<Vector>, Box<dyn std::error::Error>> {
...
#[doc = "Calculate centroid of a set of vectors."]
pub fn centroid(vectors: &Vec<Vector>) -> Result<Vector, Box<dyn std::error::Error>> {
    if vectors.is_empty() {
        return Err(Box::new(ValueError::new(
            "Need at least one vector".to_string(),
        )));
    }
    let _cse_temp_0 = vectors.len() as i32;
    let n = _cse_temp_0;
    let _cse_temp_1 = vectors
        .get(0usize)
        .cloned()
        .expect("IndexError: list index out of range")
        .len() as i32;
    let dim = _cse_temp_1;
    let _cse_temp_2 = vec![0.0; dim as usize];
    let mut result = _cse_temp_2.clone();
    for v in vectors.iter().cloned() {
        for i in 0..(dim) {
            {
                let _key = i;
...
#[doc = "Calculate p-norm of vector."]
#[doc = " Depyler: proven to terminate"]
pub fn norm_p(v: &Vector, p: f64) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = p <= 0f64;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new("p must be positive".to_string())));
    }
    let _cse_temp_1 = "inf".parse::<f64>().unwrap();
    let _cse_temp_2 = p == _cse_temp_1;
    if _cse_temp_2 {
        return Ok(v
            .components
            .into_iter()
    ...
    Ok(({
        v.components
            .into_iter()
            .map(|c| ({ (c).abs() } as f64).powf({ p } as f64))
            .sum::<f64>()
    } as f64)
        .powf({ ((1) as f64) / ((p) as f64) } as f64))
}
#[doc = "Calculate cosine similarity between vectors."]
#[doc = " Depyler: proven to terminate"]
pub fn cosine_similarity<'a, 'b>(
    ...
    Ok(((dot(&v1, &v2)?) as f64) / ((mag1 * mag2) as f64))
}
#[doc = "Parse vector from comma-separated string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_v1 = args.v1.is_some();
    let has_v2 = args.v2.is_some();
    let _cse_temp_0 = args.mode == "demo";
    let mut v2;
    let mut v1;
    if _cse_temp_0 {
        println!("{}", "Vector Operations Demo\n");
        v1 = Vector::new(vec![1.0, 2.0, 3.0]);
        v2 = Vector::new(vec![4.0, 5.0, 6.0]);
        println!("{}", format!("v1 = {}", v1));
        println!("{}", format!("v2 = {}", v2));
        println!("{}", format!("\nv1 + v2 = {:?}", add(&v1, &v2)));
        println!("{}", format!("v1 - v2 = {:?}", subtract(&v1, &v2)));
        println!("{}", format!("v1 · v2 = {:?}", dot(&v1, &v2)));
        println!("{}", format!("v1 × v2 = {:?}", cross(&v1, &v2)));
        println!("{}", format!("|v1| = {}", magnitude(&v1)));
        println!("{}", format!("normalize(v1) = {:?}", normalize(&v1)));
        println!(
            "{}",
            format!(
                "angle(v1, v2) = {}°",
                (angle_between(&v1, &v2) as f64).to_degrees()
            )
        );
        println!("{}", format!("distance(v1, v2) = {:?}", distance(&v1, &v2)));
        println!("{}", format!("project(v1, v2) = {:?}", project(&v1, &v2)));
        println!(
            "{}",
            format!("cosine_similarity = {:?}", cosine_similarity(&v1, &v2))
        );
    } else {
        if has_v1 {
            v1 = parse_vector(&args.v1);
            let _cse_temp_1 = args.mode == "mag";
            if _cse_temp_1 {
                println!("{}", format!("|v| = {}", magnitude(&v1)));
            } else {
                let _cse_temp_2 = args.mode == "normalize";
                if _cse_temp_2 {
                    println!("{}", format!("normalize(v) = {:?}", normalize(&v1)));
                } else {
                    if has_v2 {
                        v2 = parse_vector(&args.v2);
                        let _cse_temp_3 = args.mode == "add";
                        if _cse_temp_3 {
                            println!("{}", format!("v1 + v2 = {:?}", add(&v1, &v2)));
                        } else {
                            let _cse_temp_4 = args.mode == "dot";
                            if _cse_temp_4 {
                                println!("{}", format!("v1 · v2 = {:?}", dot(&v1, &v2)));
                            } else {
                                let _cse_temp_5 = args.mode == "cross";
                                if _cse_temp_5 {
                                    println!("{}", format!("v1 × v2 = {:?}", cross(&v1, &v2)));
                                } else {
                                    let _cse_temp_6 = args.mode == "angle";
                                    if _cse_temp_6 {
                                        let angle = angle_between(&v1, &v2)?;
                                        println!(
                                            "{}",
                                            format!("angle = {}°", (angle as f64).to_degrees())
                                        );
                                    } else {
                                        let _cse_temp_7 = args.mode == "project";
                                        if _cse_temp_7 {
                                            println!(
                                                "{}",
                                                format!(
                                            ...
                                            let _cse_temp_8 = args.mode == "distance";
                                            if _cse_temp_8 {
                                                println!(
                                                    "{}",
                                                    format!("distance = {:?}", distance(&v1, &v2))
                                                );
                                            }
                                        }
                                    }
...
mod tests {
    ...
    #[test]
    fn quickcheck_magnitude() {
        fn prop(v: ()) -> TestResult {
            let result = magnitude(v.clone());
            if result < 0 {
    ...
    #[test]
    fn test_gram_schmidt_examples() {
        assert_eq!(gram_schmidt(vec![]), vec![]);
        assert_eq!(gram_schmidt(vec![1]), vec![1]);
    }
    #[test]
    fn quickcheck_orthonormalize() {
        fn prop(vectors: Vec<()>) -> TestResult {
            let once = orthonormalize(&vectors);
            let twice = orthonormalize(once.clone());
            if once != twice {
                return TestResult::failed();
            }
            TestResult::passed()
    ...
    #[test]
    fn test_orthonormalize_examples() {
        assert_eq!(orthonormalize(vec![]), vec![]);
        assert_eq!(orthonormalize(vec![1]), vec![1]);
    }
}
