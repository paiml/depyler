...
enum Commands {
    ...
    Parse {
        #[doc = "Date string(YYYY-MM-DD or ISO)"]
        date: String,
    },
...
#[doc = "Display current time. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_now(format: &str) {
    let now = DepylerDateTime::now();
    let _cse_temp_0 = format == "iso";
    if _cse_temp_0 {
        println!("{}", format!("{:?}", now));
    } else {
        let _cse_temp_1 = format == "date";
        if _cse_temp_1 {
            println!("{}", format!("{:?}", now));
        } else {
            println!("{}", format!("{:?}", now));
...
#[doc = "Convert Unix timestamp. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_timestamp(timestamp: &str) {
    let dt = DepylerDateTime::fromtimestamp((timestamp) as i32 as f64);
    println!("{}", format!("{:?}", dt));
}
#[doc = "Parse date string. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_parse(date: &str) {
    let _cse_temp_0 = date.contains("T");
    let mut dt;
    if _cse_temp_0 {
    ...
    println!("{}", format!("Parsed: {}", format!("{:?}", dt)));
}
#[doc = "Calculate duration. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_duration(days: i32, hours: i32) {
    let mut delta;
    if days {
        delta = DepylerTimeDelta::new(0, 0, 0);
        println!(
            "{}",
            format!(
                "{} days = {} hours",
...
#[doc = "Main entry point. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    ();
    ...
    match &args.command {
        Some(Commands::Now { ref format, .. }) => {
            cmd_now(format);
        }
        Some(Commands::Timestamp { ref timestamp, .. }) => {
            cmd_timestamp(timestamp);
        }
        Some(Commands::Parse { ref date, .. }) => {
            cmd_parse(date);
        }
        Some(Commands::Duration {
            ref days,
            ref hours,
            ..
        }) => {
