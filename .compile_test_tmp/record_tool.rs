...
#[doc = "Create point. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_point(distance: bool, x: i32, y: i32) {
    let p = Point::new(x, y);
    println!("{}", format!("Point({}, {})", p.x, p.y));
    if distance {
        let dist = ({
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ...
                ({ p.x } as f64).powf({ 2 } as f64) as i32
            }
        } + {
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ({ p.y } as i32)
                ...
                ({ p.y } as f64).powf({ 2 } as f64) as i32
            }
        } as f64)
            .sqrt();
        println!("{}", format!("Distance from origin: {:?}", dist));
    }
}
#[doc = "Create color. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_color(b: i32, g: i32, r: i32) {
    let c = Color::new(r, g, b);
    println!("{}", format!("RGB({}, {}, {})", c.r, c.g, c.b));
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Point { .. }));
    match &args.command {
        Some(Commands::Point {
            ref x,
            ref y,
