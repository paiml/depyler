...
#[doc = "Write string to file, return bytes written."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn write_file<'b, 'a>(path: &'a str, content: &'b str) -> Result<i32, std::io::Error> {
    let mut f = std::fs::File::create(&path)?;
    return Ok(f.write_all(content.as_bytes()).unwrap());
}
#[doc = "Write lines to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn write_lines<'a, 'b>(path: &'a str, lines: &'b Vec<String>) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::create(&path)?;
...
#[doc = "Write lines using writelines method."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn writelines<'a, 'b>(path: &'a str, lines: &'b Vec<String>) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::create(&path)?;
    f.writelines(
        lines
            .iter()
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn append_file<'a, 'b>(path: &'a str, content: &'b str) -> Result<i32, std::io::Error> {
...
#[doc = "Append single line to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn append_line<'a, 'b>(path: &'a str, line: &'b str) -> Result<(), std::io::Error> {
    let mut f = std::fs::OpenOptions::new()
        .append(true)
        .create(true)
        .open(&path)?;
...
#[doc = "Append multiple lines to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn append_lines<'a, 'b>(path: &'a str, lines: &'b Vec<String>) -> Result<(), std::io::Error> {
    let mut f = std::fs::OpenOptions::new()
        .append(true)
        .create(true)
        .open(&path)?;
...
#[doc = "Write binary data to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn write_binary<'a, 'b>(path: &'a str, data: &'b Vec<u8>) -> Result<i32, std::io::Error> {
    let mut f = std::fs::File::create(&path)?;
    return Ok(f.write_all(data.as_bytes()).unwrap());
}
#[doc = "Append binary data to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn append_binary<'b, 'a>(path: &'a str, data: &'b Vec<u8>) -> Result<i32, std::io::Error> {
    let mut f = std::fs::OpenOptions::new()
        .append(true)
        .create(true)
        .open(&path)?;
...
#[doc = "Write simple CSV file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn write_csv_simple<'c, 'b, 'a>(
    path: &'a str,
    rows: &'b Vec<Vec<String>>,
    delimiter: &'c str,
) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::create(&path)?;
    for row in rows.iter().cloned() {
        f.write_all(format!("{}{}", row.join(delimiter), "\n").as_bytes())
            .unwrap();
    }
    Ok(())
}
#[doc = "Write key-value file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn write_key_value<'a, 'b>(
    path: &'a str,
    data: &'b std::collections::HashMap<String, String>,
    delimiter: String,
) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::create(&path)?;
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn copy_file<'a, 'b>(src: &'a str, dst: &'b str) -> Result<i32, std::io::Error> {
    let mut f_src = std::fs::File::open(&src)?;
    let content = {
        let mut content = String::new();
        f_src.read_to_string(&mut content)?;
        content
    };
    let mut f_dst = std::fs::File::create(&dst)?;
    return Ok(f_dst.write_all(content.as_bytes()).unwrap());
}
#[doc = "Truncate file to size bytes."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn truncate_file(path: &str, size: i32) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    f.truncate(size);
    Ok(())
}
#[doc = "Clear file contents."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn clear_file(path: &str) -> Result<(), std::io::Error> {
    let mut _context = std::fs::File::create(&path)?;
    Ok(())
}
#[doc = "Overwrite specific line in file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn overwrite_line<'a, 'b>(
    path: &'a str,
    line_num: i32,
    new_content: &'b str,
) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
    let mut lines = std::io::BufReader::new(f)
        .lines()
    ...
    if _cse_temp_3 {
        lines.insert((line_num - 1) as usize, format!("{}{}", new_content, "\n"));
    }
    let mut f = std::fs::File::create(&path)?;
    f.writelines(lines);
    Ok(())
}
#[doc = "Insert line at position."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn insert_line<'b, 'a>(
    path: &'a str,
    line_num: i32,
    content: &'b str,
) -> Result<(), std::io::Error> {
    ...
    let mut lines = std::io::BufReader::new(f)
        ...
        .map(|l| l.unwrap_or_default())
        .collect::<Vec<_>>();
    lines.insert(line_num - 1 as usize, format!("{}{}", content, "\n"));
    let mut f = std::fs::File::create(&path)?;
    f.writelines(lines);
    Ok(())
}
#[doc = "Delete line at position."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn delete_line(path: &str, line_num: i32) -> Result<(), std::io::Error> {
    let mut f = std::fs::File::open(&path)?;
...
#[doc = "Replace all occurrences in file, return count."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn replace_in_file<'b, 'c, 'a>(
    path: &'a str,
    old: &'b str,
    new: &'c str,
...
#[doc = "Atomic write using temp file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn atomic_write<'b, 'a>(path: &'a str, content: &'b str) -> Result<(), std::io::Error> {
    let temp_path = format!("{}{}", path, ".tmp");
    let mut f = std::fs::File::create(&temp_path)?;
    f.write_all(content.as_bytes()).unwrap();
    let mut f_tmp = std::fs::File::open(&temp_path)?;
    let data = {
        let mut content = String::new();
        f_tmp.read_to_string(&mut content)?;
        content
    };
    let mut f = std::fs::File::create(&path)?;
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        match &args.command {
            Some(Commands::Write {
                ...
                n = Some(write_file(&file, &content)?);
                println!("{}", format!("Wrote {:?} bytes to {}", n, file));
            }
            Some(Commands::Append {
                ...
                n = Some(append_file(&file, &content)?);
                println!("{}", format!("Appended {:?} bytes to {}", n, file));
            }
            Some(Commands::Copy {
                ref dst, ref src, ..
            }) => {
                n = Some(copy_file(&src, &dst)?);
                println!(
                    "{}",
                    format!("Copied {:?} bytes from {} to {}", n, src, dst)
                );
            }
            Some(Commands::Clear { ref file, .. }) => {
                clear_file(&file);
                println!("{}", format!("Cleared {}", file));
            }
            Some(Commands::Replace {
                ...
                count = Some(replace_in_file(&file, &old, &new)?);
            ...
            Some(Commands::Insert {
                ...
                insert_line(&file, line_num, &content);
                println!("{}", format!("Inserted line at {}", line_num));
            }
            Some(Commands::Delete {
                ...
                println!("{}", format!("Deleted line {}", line_num));
            }
