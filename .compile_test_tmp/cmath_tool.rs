...
enum Commands {
    #[default]
    __DepylerNone,
    Phase { real: f64, imag: f64 },
    Abs { real: f64, imag: f64 },
    Polar { real: f64, imag: f64 },
}
#[derive(Default)]
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Abs { .. }));
    match &args.command {
        Some(Commands::Abs {
            ref imag, ref real, ..
        }) => {
            let imag = *imag;
            let real = *real;
            println!("{}", (real * real + imag * imag as f64).sqrt());
        }
        Some(Commands::Phase {
            ref imag, ref real, ..
        }) => {
            let imag = *imag;
            let real = *real;
            println!("{}", (imag as f64).atan2(real as f64));
        }
        Some(Commands::Polar {
            ref imag, ref real, ..
        }) => {
            let imag = *imag;
            let real = *real;
            let r = (real * real + imag * imag as f64).sqrt();
            let theta = (imag as f64).atan2(real as f64);
            println!("{}", format!("{:?} {:?}", r, theta));
        }
