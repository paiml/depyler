...
#[doc = "Copy file. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_copy(dst: &str, src: &str) {
    {
        std::fs::copy(&src, &dst).unwrap();
        dst.clone()
    };
    println!("{}", format!("Copied: {} -> {}", src, dst));
}
#[doc = "Move file. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_move(dst: &str, src: &str) {
    {
        std::fs::rename(&src, &dst).unwrap();
        dst.clone()
    };
    println!("{}", format!("Moved: {} -> {}", src, dst));
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Copy { .. }));
    match &args.command {
        Some(Commands::Copy {
            ref src, ref dst, ..
        }) => {
