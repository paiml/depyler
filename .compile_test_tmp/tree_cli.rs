...
struct Args {
    ...
    #[doc = "Directory to display"]
    path: Option<String>,
    
    #[doc = "Maximum depth(-1 for unlimited)"]
    depth: i32,
    ...
    #[doc = "Filter by extension"]
    ext: Option<String>,
    
    
    #[doc = "Show count summary only"]
    count: bool,
    
    
    #[doc = "Show total size only"]
    total_size: bool,
}
...
#[doc = "Walk directory and return list of(path, depth, is_dir)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn walk_directory(
    ...
    let result = vec![];
    let walk = move |current_path: &str, depth: i32| -> () {
        ...
        for entry in entries.iter().cloned() {
            ...
            let full_path = std::path::PathBuf::from(current_path)
                .join(entry)
                .to_string_lossy()
                .to_string();
            ...
            result.push(DepylerValue::Str(format!(
                "{:?}",
                (full_path, depth, is_dir)
            )));
            if is_dir {
                walk(full_path.display().to_string(), depth + 1);
            }
        }
    };
    walk(path.to_string(), 0);
    result
}
#[doc = "Format entries as tree with box-drawing characters."]
#[doc = " Depyler: proven to terminate"]
pub fn format_tree<'a, 'b>(
    entries: &'a Vec<(String, i32, bool)>,
    base_path: &'b str,
    show_size: bool,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut line: String = Default::default();
    let mut lines = vec![];
    let _cse_temp_0 = base_path
        .trim_end_matches(|c: char| "/".contains(c))
        .to_string()
        .len() as i32;
    let base_len = _cse_temp_0 + 1;
    for (i, _nested) in entries
        .iter()
        .cloned()
        .enumerate()
        .map(|(i, x)| (i as i32, x))
    {
        ...
        let name = {
            let base = &path;
            let start_idx = (base_len) as isize;
        ...
        for j in (i + 1)..(entries.len() as i32) {
            if entries
                .get(j as usize)
                .cloned()
        ...
        if (show_size) && (!is_dir) {
            let size = get_file_size(path.display().to_string());
            line = format!("{}{}({})", prefix, display_name, format_size(size));
        } else {
            line = format!("{}{}", prefix, display_name);
            if is_dir {
                line = format!("{}{}", line, "/");
            }
        }
...
#[doc = "Count directories and files."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_entries(entries: &Vec<(String, i32, bool)>) -> (i32, i32) {
    let _cse_temp_0 = entries
        .iter()
        .cloned()
        .filter(|(_, _, is_dir)| is_dir)
        .map(|(_, _, is_dir)| 1)
        .sum::<i32>();
    let dirs = _cse_temp_0;
    let files = _cse_temp_0;
    (dirs, files)
}
#[doc = "Calculate total size of all files."]
#[doc = " Depyler: verified panic-free"]
pub fn total_size(entries: &Vec<(String, i32, bool)>) -> i32 {
    let mut total: i32 = Default::default();
    total = 0;
    for (path, __sanitized, is_dir) in entries.iter().cloned() {
        if !is_dir {
            total = total + get_file_size(&path);
        }
    }
    total
}
#[doc = "Filter entries by file extension."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn filter_by_extension(
    entries: &Vec<(String, i32, bool)>,
    mut ext: String,
) -> Vec<(String, i32, bool)> {
    if !ext.starts_with(".") {
        ext = format!("{}{}", ".", ext);
    ...
    entries
        .iter()
        .cloned()
        .filter(|(path, depth, is_dir)| {
            let (path, depth, is_dir) = (path, depth, is_dir).clone();
            (is_dir) || (path.ends_with(ext))
        })
        .map(|(path, depth, is_dir)| (path, depth, is_dir))
        .collect::<Vec<_>>()
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut entries: DepylerValue = Default::default();
    ...
    entries = walk_directory(&args.path, args.depth, args.all, args.dirs);
    if has_ext {
    ...
    if args.count {
        ...
        println!("{}", format!("{:?} directories, {:?} files", dirs, files));
    } else {
        if args.total_size {
            let size = total_size(&entries);
            println!("{}", format_size(size));
        } else {
            println!("{}", args.path);
            for line in format_tree(&entries, &args.path, args.size) {
                println!("{}", line);
            }
            (dirs, files) = count_entries(&entries);
...
mod tests {
    ...
    #[test]
    fn test_total_size_examples() {
        assert_eq!(total_size(&vec![]), 0);
        assert_eq!(total_size(&vec![1]), 1);
        assert_eq!(total_size(&vec![1, 2, 3]), 3);
    }
}
