...
const STR_ERROR__MISSING__X_: &'static str = "Error: Missing 'x'";
...
#[doc = "Sigmoid activation: 1 /(1 + exp(-x))."]
#[doc = " Depyler: verified panic-free"]
pub fn sigmoid(x: &Vec<f64>) -> Vec<f64> {
    ...
    for xi in x.iter().cloned() {
        if xi >= 0f64 {
            ...
            let exp_x = (xi as f64).exp();
            result.push(DepylerValue::Str(format!(
                "{:?}",
                ((exp_x) as f64) / ((1.0 + exp_x) as f64)
            )));
        }
    }
    result
...
pub fn softmax(x: &Vec<f64>) -> Vec<f64> {
    let _cse_temp_0 = *x.iter().max().unwrap();
    let max_x = _cse_temp_0;
    let exp_x = x
        .as_slice()
        .iter()
        .cloned()
        .map(|xi| (xi - max_x as f64).exp())
        .collect::<Vec<_>>();
    let _cse_temp_1 = exp_x.iter().sum::<i32>();
...
#[doc = "Handle relu subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_relu() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_0 {
        eprintln!("{}", STR_ERROR__MISSING__X_);
        std::process::exit(1);
    }
    let output = relu(data.get("x").cloned().unwrap_or_default());
    println!(
        "{}",
        format!("{:?}", {
...
#[doc = "Handle sigmoid subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_sigmoid() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let output = sigmoid(data.get("x").cloned().unwrap_or_default());
    println!(
        "{}",
        format!("{:?}", {
            let mut map = HashMap::new();
...
#[doc = "Handle tanh subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_tanh() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let output = tanh(data.get("x").cloned().unwrap_or_default());
    println!(
        "{}",
        format!("{:?}", {
            let mut map = HashMap::new();
...
#[doc = "Handle softmax subcommand."]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_softmax() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let output = softmax(data.get("x").cloned().unwrap_or_default());
    println!(
        "{}",
        format!("{:?}", {
            let mut map = HashMap::new();
...
mod tests {
    ...
    #[test]
    fn test_relu_examples() {
        assert_eq!(relu(vec![]), vec![]);
        assert_eq!(relu(vec![1]), vec![1]);
    }
    #[test]
    fn test_sigmoid_examples() {
        assert_eq!(sigmoid(vec![]), vec![]);
        assert_eq!(sigmoid(vec![1]), vec![1]);
    }
    #[test]
    fn test_tanh_examples() {
        assert_eq!(tanh(vec![]), vec![]);
        assert_eq!(tanh(vec![1]), vec![1]);
    }
    #[test]
