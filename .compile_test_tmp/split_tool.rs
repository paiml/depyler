...
#[doc = "Split string by underscores.\n\n   >>>split_underscore(\"hello_world_test\")\n    ['hello', 'world', 'test']\n   >>>split_underscore(\"a_b_c\")\n    ['a', 'b', 'c']\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn split_underscore(text: &str) -> Vec<String> {
    text.split("_")
        .map(|s| s.to_string())
        .collect::<Vec<String>>()
}
#[doc = "Split string by dashes.\n\n   >>>split_dash(\"foo-bar-baz\")\n    ['foo', 'bar', 'baz']\n   >>>split_dash(\"2025-11-29\")\n    ['2025', '11', '29']\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn split_dash(text: &str) -> Vec<String> {
    text.split("-")
        .map(|s| s.to_string())
        .collect::<Vec<String>>()
}
#[doc = "Split string by dots.\n\n   >>>split_dot(\"192.168.1.1\")\n    ['192', '168', '1', '1']\n   >>>split_dot(\"a.b.c\")\n    ['a', 'b', 'c']\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn split_dot(text: &str) -> Vec<String> {
    text.split(".")
        .map(|s| s.to_string())
        .collect::<Vec<String>>()
}
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &args.command {
        Some(Commands::Underscore { ref text, .. }) => {
            let mut parts = split_underscore(&text);
            println!(
                "{}",
                format!(
                    "{}{}",
        ...
        Some(Commands::Dash { ref text, .. }) => {
            let mut parts = split_dash(&text);
            println!(
                "{}",
                format!(
                    "{}{}",
        ...
        Some(Commands::Dot { ref text, .. }) => {
            let mut parts = split_dot(&text);
            println!(
                "{}",
                format!(
                    "{}{}",
