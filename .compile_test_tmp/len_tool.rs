...
pub fn string_length(text: &str) -> i32 {
    text.len() as i32 as i32
}
#[doc = "Count digits in integer.\n\n   >>>digit_count(0)\n    1\n   >>>digit_count(123)\n    3\n   >>>digit_count(-456)\n    3\n    "]
#[doc = " Depyler: proven to terminate"]
pub fn digit_count(num: i32) -> Result<i32, Box<dyn std::error::Error>> {
...
#[doc = "Count underscore-separated words.\n\n   >>>word_count(\"hello\")\n    1\n   >>>word_count(\"a_b\")\n    2\n   >>>word_count(\"one_two_three\")\n    3\n    "]
pub fn word_count(text: &str) -> Result<i32, Box<dyn std::error::Error>> {
    let mut count: i32 = Default::default();
    count = 1;
    let mut i = 0;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::String { .. }));
    match &args.command {
        Some(Commands::String { ref text, .. }) => {
            println!("{}", string_length(&text));
        }
        Some(Commands::Digits { ref num, .. }) => {
            let num = *num;
...
mod tests {
    ...
    #[test]
    fn test_string_length_examples() {
        assert_eq!(string_length(""), 0);
        assert_eq!(string_length("a"), 1);
        assert_eq!(string_length("abc"), 3);
    }
    #[test]
    fn test_digit_count_examples() {
