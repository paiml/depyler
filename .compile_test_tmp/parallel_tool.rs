...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'concurrent.futures'(tracked in DEPYLER-0424)"]
...
enum Commands {
    #[default]
    __DepylerNone,
    Sum { numbers: Vec<String> },
    Square { numbers: Vec<String> },
}
#[derive(Default)]
...
pub fn ThreadPoolExecutor<T: Default>(_args: impl std::any::Any) -> T {
    Default::default()
}
#[doc = "Square a number. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn square(n: i32) {
    let _ = n * n;
}
#[doc = "Sum numbers in parallel. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_sum(numbers: &[String]) {
    let numbers = numbers.into_iter().map(|n| (n) as i32).collect::<Vec<_>>();
    let mut _context = ThreadPoolExecutor::new(4);
    let executor = _context.__enter__();
    let results = executor.map(move |x| x, numbers).collect::<Vec<_>>();
    println!("{}", format!("Sum: {}", results.iter().sum::<i32>()));
}
#[doc = "Square numbers in parallel. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_square(numbers: &[String]) {
    let numbers = numbers.into_iter().map(|n| (n) as i32).collect::<Vec<_>>();
    let mut _context = ThreadPoolExecutor::new(4);
    let executor = _context.__enter__();
    let results = executor.map(square, numbers).collect::<Vec<_>>();
    for (n, r) in numbers.into_iter().zip(results.into_iter()) {
        println!("{}", format!("{:?}^2 = {:?}", n, r));
    }
}
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Sum { .. }));
    match &args.command {
        Some(Commands::Sum { ref numbers, .. }) => {
            let numbers = numbers.unwrap_or_default();
            cmd_sum(numbers);
        }
        Some(Commands::Square { ref numbers, .. }) => {
            let numbers = numbers.unwrap_or_default();
            cmd_square(numbers);
        }
