...
pub fn asdict<T: Default>(_args: impl std::any::Any) -> T {
    Default::default()
}
#[doc = r" Stub for local import from module: #module_name"]
#[doc = r" DEPYLER-0615: Generated to allow standalone compilation"]
...
#[doc = "Create a person record. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_create(age: i32, email: &str, json: bool, name: &str) {
    ...
    if json {
        println!("{}", format!("{:?}", asdict(person)));
    } else {
        println!("{}", format!("Name: {}", person.name));
        println!("{}", format!("Age: {}", person.age));
        if !person.email.is_empty() {
            println!("{}", format!("Email: {}", person.email));
        }
    }
}
...
pub fn main() {
    ...
    match &args.command {
        Some(Commands::Create {
            ...
            let age = age.clone();
            let json = *json;
            cmd_create(name, age, email, json);
        }
