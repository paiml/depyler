...
#[doc = "Batch normalization forward pass."]
#[doc = " Depyler: proven to terminate"]
pub fn batchnorm<'c, 'a, 'b>(
    x: &'a Vec<Vec<f64>>,
    gamma: &'b Vec<f64>,
    beta: &'c Vec<f64>,
    eps: f64,
    ...
    for i in 0..(n_samples) {
            ...
            let x_norm =
                ((x.get(i as usize)
                    ...
                    / (((var
                        ...
                        + eps as f64)
                        .sqrt()) as f64);
            row.push(DepylerValue::Str(format!(
                "{:?}",
                gamma
                    .get(j as usize)
                    .cloned()
        ...
        output.push(DepylerValue::Str(format!("{:?}", row)));
    }
    Ok(output)
}
...
pub fn cmd_forward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        output = batchnorm(
            ...
            data.get("gamma").cloned().unwrap_or_default(),
            data.get("beta").cloned().unwrap_or_default(),
            0.00001f64,
        )?;
        println!(
            "{}",
