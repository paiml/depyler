...
use std::f64 as math;
pub static GAUSS_LEGENDRE_2: std::sync::LazyLock<Vec<DepylerValue>> =
    std::sync::LazyLock::new(|| vec![(-0.5773502691896257, 1.0), (0.5773502691896257, 1.0)]);
pub static GAUSS_LEGENDRE_3: std::sync::LazyLock<Vec<DepylerValue>> =
...
pub static GAUSS_LEGENDRE_5: std::sync::LazyLock<Vec<DepylerValue>> =
    std::sync::LazyLock::new(|| {
        vec![
...
pub struct IntegrationResult {
    ...
    pub n_evaluations: i32,
}
impl IntegrationResult {
    pub fn new(value: f64, error_estimate: f64, n_evaluations: i32) -> Self {
        Self {
            value,
            error_estimate,
            n_evaluations,
...
#[doc = "Trapezoidal rule integration."]
...
pub fn trapezoidal(
    ...
    let mut total: DepylerValue = Default::default();
    let _cse_temp_0 = ((b - a) as f64) / ((n) as f64);
    let h = _cse_temp_0;
    let _cse_temp_1 = f(a) + f(b);
    let _cse_temp_2 = ((_cse_temp_1) as f64) / ((2) as f64);
    total = _cse_temp_2;
    for i in (1)..(n) {
        total = total + f(a + (i as f64) * h);
    }
    Ok(total * h)
}
#[doc = "Simpson's rule integration(n must be even)."]
#[doc = " Depyler: proven to terminate"]
pub fn simpson(
    f: impl Fn(f64) -> f64,
    a: f64,
    ...
    let _cse_temp_0 = n % 2;
    let _cse_temp_1 = _cse_temp_0 != 0f64;
    if _cse_temp_1 {
        n = n + 1f64;
    }
    let _cse_temp_2 = ((b - a) as f64) / ((n) as f64);
    let h = _cse_temp_2;
    let _cse_temp_3 = f(a) + f(b);
    total = _cse_temp_3;
    for i in (1)..(n) {
        let x = a + (i as f64) * h;
        if i % 2 == 0 {
            total = total + 2f64 * f(x);
        } else {
            total = total + 4f64 * f(x);
        }
    }
    Ok(((total * h) as f64) / ((3) as f64))
}
#[doc = "Simpson's 3/8 rule integration(n must be multiple of 3)."]
pub fn simpson38(
    f: impl Fn(f64) -> f64,
    a: f64,
    b: f64,
    ...
    for i in (1)..(n) {
        ...
        if i % 3 == 0 {
            ...
            total = total + 3f64 * f(x);
        }
    }
    Ok(((total * 3f64 * h) as f64) / ((8) as f64))
}
#[doc = "Midpoint rule integration."]
#[doc = " Depyler: proven to terminate"]
pub fn midpoint(
    f: impl Fn(f64) -> f64,
    a: f64,
    ...
    total = 0.0;
    for i in 0..(n) {
        let x_mid = a + ((i as f64) + 0.5) * h;
        total = total + f(x_mid);
    }
    Ok(total * h)
}
#[doc = "Romberg integration with Richardson extrapolation."]
#[doc = " Depyler: proven to terminate"]
pub fn romberg(
    f: impl Fn(f64) -> f64,
    a: f64,
    ...
    let mut n_evals: i32 = Default::default();
    let R = (0..(max_iter + 1))
        .into_iter()
        .map(|_| vec![0.0; (max_iter + 1) as usize])
        .collect::<Vec<_>>();
    n_evals = 2;
    let _cse_temp_0 = f(a) + f(b);
    let _cse_temp_1 = (b - a) * _cse_temp_0;
    let _cse_temp_2 = ((_cse_temp_1) as f64) / ((2) as f64);
    R.get_mut(&0).unwrap().insert((0) as usize, _cse_temp_2);
    for i in (1)..(max_iter + 1) {
        let h = ((b - a) as f64)
            / (({
                if i >= 0 && (i as i64) <= (u32::MAX as i64) {
                    ({ 2 } as i32)
                        .checked_pow({ i } as u32)
                        .expect("Power operation overflowed")
                } else {
                    ({ 2 } as f64).powf({ i } as f64) as i32
                }
            }) as f64);
        let n_points = {
            if i - 1 >= 0 && (i - 1 as i64) <= (u32::MAX as i64) {
                ({ 2 } as i32)
                    .checked_pow({ i - 1 } as u32)
                    .expect("Power operation overflowed")
            } else {
                ({ 2 } as f64).powf({ i - 1 } as f64) as i32
            }
        };
        let inner_sum = ((1)..(n_points + 1))
            .into_iter()
            .map(|k| f(a + (2 * k - 1) * h))
            .sum::<i32>();
        R.get_mut(&i).unwrap().insert(
            (0) as usize,
            {
                let base = &R;
                let idx: i32 = i - 1;
        ...
        n_evals = n_evals + n_points;
        for j in (1)..(i + 1) {
            R.get_mut(&i).unwrap().insert(
                (j) as usize,
                ({
                    if j >= 0 && (j as i64) <= (u32::MAX as i64) {
                        ({ 4 } as i32)
                            .checked_pow({ j } as u32)
                            .expect("Power operation overflowed")
                    } else {
                        ({ 4 } as f64).powf({ j } as f64) as i32
                    }
                } * {
                    let base = &R
                        .get(i as usize)
    ...
    Ok(IntegrationResult::new(
        R.get(max_iter as usize)
            .cloned()
            .expect("IndexError: list index out of range")
            .get(max_iter as usize)
            .cloned()
            .expect("IndexError: list index out of range"),
        (R.get(max_iter as usize)
            .cloned()
...
#[doc = "Gaussian quadrature integration."]
pub fn gauss_legendre(
    ...
    let mut total: f64 = Default::default();
    let mut nodes_weights: DepylerValue = Default::default();
    let _cse_temp_0 = n_points == 2;
    if _cse_temp_0 {
        nodes_weights = GAUSS_LEGENDRE_2;
    } else {
        let _cse_temp_1 = n_points == 3;
        if _cse_temp_1 {
    ...
    let _cse_temp_2 = ((b - a) as f64) / ((2) as f64);
    let c1 = _cse_temp_2;
    let _cse_temp_3 = ((b + a) as f64) / ((2) as f64);
    let c2 = _cse_temp_3;
    total = 0.0;
    for (node, weight) in nodes_weights.iter().cloned() {
        let x = c1 * node + c2;
        total = total + weight * f(x);
    }
    Ok(total * c1)
}
#[doc = "Adaptive Simpson's rule."]
#[doc = " Depyler: proven to terminate"]
pub fn adaptive_simpson(
    ...
    let mut n_evals = vec![0];
    let simpson_helper = move |a: f64, b: f64, fa: f64, fb: f64, s: f64, depth: i32| -> f64 {
        let c = ((a + b) as f64) / ((2) as f64);
        let fc = f(c);
        n_evals.insert(
            (0) as usize,
            n_evals
        ...
        let d = ((a + c) as f64) / ((2) as f64);
        let e = ((c + b) as f64) / ((2) as f64);
        let fd = f(d);
        let fe = f(e);
        n_evals.insert(
            (0) as usize,
        ...
        let h = b - a;
        let s_left = ((h) as f64) / ((12) as f64) * (fa + 4f64 * fd + fc);
        let s_right = ((h) as f64) / ((12) as f64) * (fc + 4f64 * fe + fb);
        let s_new = s_left + s_right;
        if (depth <= 0) || (((s_new - s).abs() as f64) < 15f64 * tol) {
            return s_new + ((s_new - s) as f64) / ((15) as f64);
        }
        return simpson_helper(a, c, fa, fc, s_left, depth - 1)
            + simpson_helper(c, b, fc, fb, s_right, depth - 1);
    };
    let fa = f(a);
    let fb = f(b);
    let fc = f(((a + b) as f64) / ((2) as f64));
    n_evals.insert((0) as usize, 3);
    let _cse_temp_0 = 4f64 * fc;
    let _cse_temp_1 = fa + _cse_temp_0 + fb;
    let _cse_temp_2 = (b - a) * _cse_temp_1;
    let _cse_temp_3 = ((_cse_temp_2) as f64) / ((6) as f64);
    let s = _cse_temp_3;
    let result = simpson_helper(a, b, fa, fb, s, max_depth);
    Ok(IntegrationResult::new(
        result,
        tol,
        n_evals
...
pub fn monte_carlo(
    ...
    let mut samples = vec![];
    for __sanitized in 0..(n_samples) {
        state = (1103515245f64 * state + 12345f64)
            % ({ 2 } as i32)
                .checked_pow({ 31 } as u32)
                .expect("Power operation overflowed");
        let u = ((state) as f64)
            / ((({ 2 } as i32)
                .checked_pow({ 31 } as u32)
        ...
        let x = a + u * (b - a);
        samples.push(DepylerValue::Str(format!("{:?}", f(x))));
    }
    let _cse_temp_0 = samples.iter().sum::<i32>();
    let _cse_temp_1 = ((_cse_temp_0) as f64) / ((n_samples) as f64);
    let mean = _cse_temp_1;
    let _cse_temp_2 = (b - a) * mean;
    let integral = _cse_temp_2;
    let _cse_temp_3 = samples
        .iter()
        .cloned()
        .map(|s| ({ s - mean } as f64).powf({ 2 } as f64))
        .sum::<i32>();
    let _cse_temp_4 = ((_cse_temp_3) as f64) / ((n_samples - 1f64) as f64);
    let variance = _cse_temp_4;
    let _cse_temp_5 = (b - a) * (((variance) as f64) / ((n_samples) as f64) as f64).sqrt();
    let error = _cse_temp_5;
    Ok(IntegrationResult::new(integral, error, n_samples))
}
#[doc = "Double integral using trapezoidal rule."]
#[doc = " Depyler: proven to terminate"]
pub fn double_integral(
    ...
    let mut total: f64 = Default::default();
    let _cse_temp_0 = ((bx - ax) as f64) / ((nx) as f64);
    let hx = _cse_temp_0;
    let _cse_temp_1 = ((by - ay) as f64) / ((ny) as f64);
    let hy = _cse_temp_1;
    total = 0.0;
    for i in 0..(nx + 1f64) {
        let x = ax + (i as f64) * hx;
        let wx = if (i == 0) || (((i as f64) as f64) == nx) {
            0.5
        } else {
            1.0
        };
        for j in 0..(ny + 1f64) {
            ...
            let wy = if (j == 0) || (((j as f64) as f64) == ny) {
                0.5
            } else {
                1.0
            };
            total = total + wx * wy * f(x, y);
        }
    }
    Ok(total * hx * hy)
}
#[doc = "Integrate from a to infinity using transformation."]
...
pub fn improper_integral(
    ...
    let _cse_temp_0 = (*method) == "exponential";
    if _cse_temp_0 {
        let g = move |t: f64| -> f64 {
            if t >= 1f64 {
                return 0.0;
            }
            let x = a - (1f64 - t as f64).ln();
            return ((f(x)) as f64) / ((1f64 - t) as f64);
        };
        return Ok(simpson(g, 0f64, 0.9999, n));
    } else {
        let g = move |t: f64| -> f64 {
            if t >= 1f64 {
                return 0.0;
            }
            let x = a + ((t) as f64) / ((1f64 - t) as f64);
            return ((f(x)) as f64) / ((({ 1f64 - t } as f64).powf({ 2 } as f64)) as f64);
        };
        return Ok(simpson(g, 0f64, 0.9999, n));
    }
}
#[doc = "Simulate integration operations."]
pub fn simulate_integrate(
    ...
    for op in operations.iter().cloned() {
        ...
        let mut result;
        if cmd == "trapezoidal" {
            result = trapezoidal(f, 0f64, 1f64, 100f64)?;
            results.push(format!("{}", result));
        } else {
            if cmd == "simpson" {
                result = simpson(f, 0f64, 1f64, 100f64)?;
                results.push(format!("{}", result));
            } else {
                if cmd == "gauss" {
                    result = gauss_legendre(f, 0f64, 1f64, 5)?;
                    results.push(format!("{}", result));
                } else {
                    if cmd == "romberg" {
                        result = romberg(f, 0f64, 1f64, 10i32, 0.0000000001f64)?;
                        results.push(format!("{}", result.value));
                    } else {
                        if cmd == "sin" {
                            result = simpson(f64::sin, 0f64, std::f64::consts::PI, 100f64)?;
                            results.push(format!("{}", result));
                        }
                    }
                }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: num_integrate_cli.py <command>");
    ...
    if _cse_temp_2 {
        ...
        println!("{}", "Integrating x^2 from 0 to 1(exact = 1/3):");
        println!(
            "{}",
            format!(
                "Trapezoidal(n=10):  {:?}",
                trapezoidal(f, 0f64, 1f64, 10f64)
            )
        );
        println!(
            "{}",
            format!(
                "Trapezoidal(n=100): {:?}",
                trapezoidal(f, 0f64, 1f64, 100f64)
            )
        );
        println!(
            "{}",
            format!("Simpson(n=10):      {:?}", simpson(f, 0f64, 1f64, 10f64))
        );
        println!(
            "{}",
            format!("Simpson(n=100):     {:?}", simpson(f, 0f64, 1f64, 100f64))
        );
        println!(
            "{}",
            format!("Gauss-Legendre(5):  {:?}", gauss_legendre(f, 0f64, 1f64, 5))
        );
        let result = romberg(f, 0f64, 1f64, 10i32, 0.0000000001f64)?;
        println!(
            "{}",
            format!(
                "Romberg:             {}(evals={})",
                result.value, result.n_evaluations
            )
        );
        result = adaptive_simpson(f, 0f64, 1f64, 0.0000000001f64, 50i32)?;
        println!(
            "{}",
            format!(
                "Adaptive Simpson:    {}(evals={})",
                result.value, result.n_evaluations
            )
        );
        ...
        println!(
            ...
            format!(
                "Simpson: {:?}",
                simpson(f64::sin, 0f64, std::f64::consts::PI, 100f64)
            )
        );
    } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_integrate_examples() {
        assert_eq!(simulate_integrate(vec![]), vec![]);
        assert_eq!(simulate_integrate(vec![1]), vec![1]);
    }
}
