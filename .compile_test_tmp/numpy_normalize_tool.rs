...
enum Commands {
    ...
    __DepylerNone,
    Norm2 { a: f64, b: f64 },
    Norm3 { a: f64, b: f64, c: f64 },
    Norm4 { a: f64, b: f64, c: f64, d: f64 },
}
#[derive(Default)]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Norm3 { .. }));
    match &args.command {
        Some(Commands::Norm3 {
            ref a,
            ref b,
            ...
            let mut norm = (arr).norm_l2().unwrap();
            let mut result = if norm > 0f64 {
                Vector::from_vec(arr.as_slice().iter().map(|&x| x / norm).collect())
            } else {
                arr
            };
            println!(
        ...
        Some(Commands::Norm2 { ref a, ref b, .. }) => {
            let a = *a;
            let b = *b;
