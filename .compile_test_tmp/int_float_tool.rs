...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Toint { .. }));
    match &args.command {
        Some(Commands::Toint { ref x, .. }) => {
            let x = *x;
            println!("{}", (x) as i32);
        }
        Some(Commands::Tofloat { ref x, .. }) => {
            let x = *x;
            println!("{}", (x) as f64);
        }
        Some(Commands::Parse { ref text, .. }) => {
            println!("{}", text.parse::<i32>().unwrap_or_default());
        }
