...
pub struct Transition<S: Clone, E: Clone> {
    pub from_state: S,
    pub event: E,
    pub to_state: S,
    pub action: Option<impl Fn()>,
    pub guard: Option<impl Fn() -> bool>,
}
impl<S: Clone, E: Clone> Transition<S, E> {
    pub fn new(
        from_state: S,
        event: E,
        to_state: S,
        action: Option<impl Fn()>,
        guard: Option<impl Fn() -> bool>,
    ) -> Self {
        Self {
            from_state,
            event,
...
#[derive(Debug, Clone, PartialEq)]
pub struct FSM<S: Clone, E: Clone> {
    pub initial_state: S,
    pub current_state: S,
    pub transitions: Vec<Transition<S, E>>,
    pub on_enter: std::collections::HashMap<S, impl Fn()>,
    pub on_exit: std::collections::HashMap<S, impl Fn()>,
    pub history: Vec<S>,
}
impl<S: Clone, E: Clone> FSM<S, E> {
    pub fn new(
        initial_state: S,
        current_state: S,
        transitions: Vec<Transition<S, E>>,
        on_enter: std::collections::HashMap<S, impl Fn()>,
        on_exit: std::collections::HashMap<S, impl Fn()>,
        history: Vec<S>,
    ) -> Self {
        Self {
            initial_state,
    ...
    pub fn add_transition(
        ...
        self.transitions
            .push(Transition::new(from_state, event, to_state, action, guard));
    }
    pub fn set_on_enter(&self, state: S, callback: impl Fn()) {
        self.on_enter.clone().insert(state, callback);
    }
    pub fn set_on_exit(&self, state: S, callback: impl Fn()) {
        self.on_exit.clone().insert(state, callback);
    }
    pub fn process(&mut self, event: E) -> bool {
        for trans in self.transitions.clone() {
            if trans.from_state == self.current_state.clone() && trans.event == event {
                ...
                if self
                    .on_exit
                    .clone()
                    .contains_key(&self.current_state.clone())
                {
                    ({
                        let _base = &self.on_exit.clone();
                        let _idx = self.current_state.clone();
                        if _idx < 0 {
                ...
                self.history.push(trans.to_state);
                if self.on_enter.clone().contains_key(&trans.to_state) {
                    ({
                        let _base = &self.on_enter.clone();
                        let _idx = trans.to_state;
                        if _idx < 0 {
                            _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                        } else {
                            _base[_idx as usize].clone()
    ...
    pub fn can_process(&self, event: E) -> bool {
        for trans in self.transitions.clone() {
            if trans.from_state == self.current_state.clone() && trans.event == event {
                if trans.guard.is_none() || trans.guard() {
                    return true;
                };
            };
        }
    ...
    pub fn available_events(&self) -> Vec<E> {
        let mut events = vec![];
        for trans in self.transitions.clone() {
            if trans.from_state == self.current_state.clone() {
                if trans.guard.is_none() || trans.guard() {
                    events.push(trans.event);
                };
            };
        }
        return events;
    ...
    pub fn reset(&mut self) {
        ...
        self.history = vec![self.initial_state.clone()];
    }
}
#[derive(Debug, Clone)]
pub struct TrafficLight {
    pub fsm: FSM<String, String>,
}
impl TrafficLight {
    pub fn new() -> Self {
        Self {
    ...
    pub fn tick(&self) -> String {
        self.fsm.clone().process("timer".to_string());
        return self.fsm.clone().current_state;
    }
...
impl Turnstile {
    ...
    pub fn _accept_coin(&mut self) {
        self.coins_inserted = self.coins_inserted.clone() + 1;
    }
    pub fn _pass_through(&self) {
        {}
    }
    pub fn insert_coin(&self) -> bool {
        return self.fsm.clone().process("coin".to_string());
    }
    pub fn push(&self) -> bool {
        return self.fsm.clone().process("push".to_string());
    }
    pub fn state(&self) -> String {
        return self.fsm.clone().current_state;
    }
...
impl Connection {
    ...
    pub fn _setup_transitions(&self) {
        self.fsm.clone().add_transition(
            "closed".to_string(),
            "open".to_string(),
            "opening".to_string(),
        );
    ...
    pub fn open(&self) -> bool {
        return self.fsm.clone().process("open".to_string());
    }
    pub fn connected(&self) -> bool {
        return self.fsm.clone().process("connected".to_string());
    }
    pub fn close(&self) -> bool {
        return self.fsm.clone().process("close".to_string());
    }
    pub fn error(&self) -> bool {
        return self.fsm.clone().process("error".to_string());
    }
    pub fn closed(&self) -> bool {
        return self.fsm.clone().process("closed".to_string());
    }
    pub fn state(&self) -> String {
        return self.fsm.clone().current_state;
    }
...
impl Order {
    ...
    pub fn confirm(&self) -> bool {
        return self.fsm.clone().process("confirm".to_string());
    }
    pub fn pay(&mut self) -> bool {
        let result = self.fsm.clone().process("pay".to_string());
        if result {
            self._paid = true;
        };
        return result;
    }
    pub fn ship(&self) -> bool {
        return self.fsm.clone().process("ship".to_string());
    }
    pub fn deliver(&self) -> bool {
        return self.fsm.clone().process("deliver".to_string());
    }
    pub fn cancel(&self) -> bool {
        return self.fsm.clone().process("cancel".to_string());
    }
    pub fn state(&self) -> String {
        return self.fsm.clone().current_state;
    }
...
#[derive(Debug, Clone, PartialEq)]
pub struct StateBuilder<S: Clone, E: Clone> {
    pub initial: S,
    pub transitions: Vec<(S, E, S)>,
}
impl<S: Clone, E: Clone> StateBuilder<S, E> {
    pub fn new(initial: S, transitions: Vec<(S, E, S)>) -> Self {
        Self {
            initial,
            transitions,
        }
    }
    pub fn add(&mut self, from_state: S, event: E, to_state: S) -> StateBuilder<S, E> {
        self.transitions.push((from_state, event, to_state));
        return self;
    }
    pub fn build(&self) -> FSM<S, E> {
        let fsm = FSM::new(self.initial.clone());
        for (from_s, event, to_s) in self.transitions.clone() {
...
#[doc = "Validate FSM for common issues."]
#[doc = " Depyler: verified panic-free"]
pub fn validate_fsm<'b, 'a>(
    fsm: &'a FSM<DepylerValue, DepylerValue>,
    states: &'b Vec<DepylerValue>,
) -> Vec<String> {
    let mut errors = vec![];
    for trans in fsm.transitions {
        if !states.contains(&trans.from_state) {
            errors.push(DepylerValue::Str(format!(
                "{:?}",
                format!("Invalid from_state: {}", trans.from_state)
            )));
        }
        if !states.contains(&trans.to_state) {
            errors.push(DepylerValue::Str(format!(
                "{:?}",
                format!("Invalid to_state: {}", trans.to_state)
            )));
        }
    }
    let _cse_temp_0 = !states.contains(&fsm.initial_state);
    if _cse_temp_0 {
        errors.push(DepylerValue::Str(format!(
            "{:?}",
            format!("Invalid initial_state: {}", fsm.initial_state)
        )));
    }
    errors
}
#[doc = "Simulate FSM from transition definitions."]
#[doc = " Depyler: verified panic-free"]
pub fn simulate_fsm<'b, 'a>(
    transitions: &'a Vec<String>,
    events: &'b Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut event: DepylerValue = Default::default();
    let mut fsm: FSM<String, String> = FSM::new(if !transitions.is_empty() {
        transitions
            .get(0usize)
            .cloned()
    ...
    for trans in transitions.iter().cloned() {
        let parts = trans
            .split("-")
            .map(|s| s.to_string())
            .collect::<Vec<String>>();
        if parts.len() as i32 == 3 {
            let (from_s, event, to_s) = parts;
            fsm.add_transition(from_s, event, to_s);
        }
    }
    let mut results = vec![];
    for event in events.iter().cloned() {
        if event == "state" {
            results.push(DepylerValue::Str(format!("{:?}", fsm.current_state)));
        } else {
            if event == "history" {
                results.push(DepylerValue::Str(format!("{:?}", fsm.history.join(","))));
            } else {
                if event == "available" {
                    results.push(DepylerValue::Str(format!(
                        "{:?}",
                        fsm.available_events().join(",")
                    )));
                } else {
                    let success = fsm.process(event);
                    results.push(DepylerValue::Str(format!(
                        "{:?}",
                        if success {
                            "ok".to_string()
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: state_fsm_cli.py <command>[args...]");
        println!("{}", "Commands: traffic, turnstile, order, simulate");
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "traffic";
    if _cse_temp_2 {
        let light = TrafficLight::new();
        println!("{}", format!("Initial: {}", light.state()));
        for __sanitized in 0..(6) {
            light.tick();
            println!("{}", format!("After tick: {}", light.state()));
        }
    } else {
        let _cse_temp_3 = cmd == "turnstile";
        if _cse_temp_3 {
            let turnstile = Turnstile::new();
            println!("{}", format!("Initial: {}", turnstile.state()));
            println!(
                "{}",
                format!(
                    "Push(should fail): {} -> {}",
            ...
            let _cse_temp_4 = cmd == "order";
            if _cse_temp_4 {
                let mut order = Order::new();
                println!("{}", format!("Initial: {}", order.state()));
                order.confirm();
                println!("{}", format!("After confirm: {}", order.state()));
                order.pay();
                println!("{}", format!("After pay: {}", order.state()));
                order.ship();
                println!("{}", format!("After ship: {}", order.state()));
                order.deliver();
                println!("{}", format!("After deliver: {}", order.state()));
            } else {
                let _cse_temp_5 = cmd == "simulate";
                if _cse_temp_5 {
                    let _cse_temp_6 = _cse_temp_0 < 4;
                    if _cse_temp_6 {
                        eprintln!("{}", "Usage: simulate <transitions><events>");
                        std::process::exit(1i32)
                    }
                    let transitions = std::env::args()
                        .collect::<Vec<String>>()
                        .get(2usize)
                        .cloned()
                        .expect("IndexError: list index out of range")
                    ...
                    let events = std::env::args()
                        ...
                        .map(|s| s.to_string())
                        .collect::<Vec<String>>();
                    let results = simulate_fsm(&transitions, &events)?;
