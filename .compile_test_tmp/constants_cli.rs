...
pub const NEG_INF: f64 = -f64::INFINITY;
...
#[doc = "Calculate circle circumference."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn circle_circumference(radius: f64) -> f64 {
    2 * PI * radius
}
#[doc = "Calculate circle area."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn circle_area(radius: f64) -> f64 {
    PI * radius * radius
}
#[doc = "Calculate sphere volume."]
#[doc = " Depyler: proven to terminate"]
pub fn sphere_volume(radius: f64) -> Result<f64, Box<dyn std::error::Error>> {
    Ok(1 * PI * ({ radius } as f64).powf({ 3 } as f64))
}
#[doc = "Calculate sphere surface area."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn sphere_surface_area(radius: f64) -> f64 {
    4 * PI * ({ radius } as f64).powf({ 2 } as f64)
}
#[doc = "Convert degrees to radians."]
#[doc = " Depyler: proven to terminate"]
pub fn degrees_to_radians(degrees: f64) -> Result<f64, Box<dyn std::error::Error>> {
    Ok(((degrees * PI) as f64) / ((180) as f64))
}
#[doc = "Convert radians to degrees."]
#[doc = " Depyler: proven to terminate"]
pub fn radians_to_degrees(radians: f64) -> Result<f64, Box<dyn std::error::Error>> {
    Ok(((radians * 180f64) as f64) / ((PI) as f64))
}
#[doc = "Check if value is finite(not inf or nan)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_finite_value(x: f64) -> bool {
    (x as f64).is_finite()
}
...
pub fn is_infinite(x: f64) -> bool {
    (x as f64).is_infinite()
}
#[doc = "Check if value is NaN."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_nan_value(x: f64) -> bool {
    (x as f64).is_nan()
}
#[doc = "Check if positive infinity."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_positive_inf(x: f64) -> bool {
    ((x as f64).is_infinite()) && (x > 0f64)
}
#[doc = "Check if negative infinity."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_negative_inf(x: f64) -> bool {
    ((x as f64).is_infinite()) && (x < 0f64)
}
#[doc = "Safe division handling division by zero."]
#[doc = " Depyler: proven to terminate"]
pub fn safe_divide(a: f64, b: f64) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = b == 0f64;
    if _cse_temp_0 {
        let _cse_temp_1 = a > 0f64;
        if _cse_temp_1 {
            return Ok(INF);
        }
        let _cse_temp_2 = a < 0f64;
        if _cse_temp_2 {
            return Ok(NEG_INF);
        }
        return Ok(NAN);
...
#[doc = "Safe logarithm handling non-positive values."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
#[doc = "Safe square root handling negative values."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
#[doc = "Clamp value to range [min_val, max_val]."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn clamp(value: f64, min_val: f64, max_val: f64) -> f64 {
    ...
    depyler_max(
        (min_val).clone(),
        (depyler_min((max_val).clone(), (value).clone())).clone(),
    )
}
...
#[doc = "Inverse linear interpolation."]
#[doc = " Depyler: proven to terminate"]
pub fn inverse_lerp(a: f64, b: f64, value: f64) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = a == b;
    if _cse_temp_0 {
        return Ok(0.0);
    }
    Ok(((value - a) as f64) / ((b - a) as f64))
}
#[doc = "Normalize angle to [0, 2*pi)."]
#[doc = " Depyler: verified panic-free"]
pub fn normalize_angle(mut angle: f64) -> f64 {
    while angle < 0f64 {
        angle = angle + TAU;
    }
    while angle >= (TAU as f64) {
        angle = angle - TAU;
    }
    angle
}
#[doc = "Return golden ratio phi."]
#[doc = " Depyler: proven to terminate"]
pub fn golden_ratio() -> Result<f64, Box<dyn std::error::Error>> {
    Ok(((1 + (5 as f64).sqrt()) as f64) / ((2) as f64))
}
#[doc = "Approximate Euler-Mascheroni constant."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn euler_mascheroni() -> f64 {
    let n = 1000000;
    let _cse_temp_0 = ((1)..(n + 1))
        .into_iter()
        .map(|i| ((1) as f64) / ((i) as f64))
        .sum::<f64>();
    let h = _cse_temp_0;
    h - (n as f64).ln()
}
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Pi { .. }));
    match &args.command {
        Some(Commands::Pi { .. }) => {
            println!("{}", format!("π = {:?}", PI));
        }
        ...
        Some(Commands::Tau { .. }) => {
            println!("{}", format!("τ = {:?}", TAU));
        }
        Some(Commands::Inf { .. }) => {
            println!("{}", format!("∞ = {:?}", INF));
        }
        Some(Commands::Golden { .. }) => {
            println!("{}", format!("φ = {:?}", golden_ratio()));
        }
        Some(Commands::Circle {
            ref area,
            ref circumference,
            ref radius,
            ..
            ...
            let radius = *radius;
            if area {
                println!("{}", format!("Area: {}", circle_area(radius)));
            }
            if circumference {
                println!(
                    "{}",
                    format!("Circumference: {}", circle_circumference(radius))
                );
            }
            let _cse_temp_6 = (!area) && (!circumference);
            if _cse_temp_6 {
                println!("{}", format!("Area: {}", circle_area(radius)));
                println!(
                    "{}",
        ...
        Some(Commands::Sphere {
            ...
            let volume = *volume;
            if volume {
                println!("{}", format!("Volume: {:?}", sphere_volume(radius)));
            }
            if surface {
                println!(
                    "{}",
                    format!("Surface area: {}", sphere_surface_area(radius))
                );
            }
            let _cse_temp_8 = (!volume) && (!surface);
            if _cse_temp_8 {
                println!("{}", format!("Volume: {:?}", sphere_volume(radius)));
                println!(
                    "{}",
        ...
        Some(Commands::Convert {
            ...
            if to_radians {
                println!(
                    "{}",
                    format!("{}° = {:?} rad", value, degrees_to_radians(value))
                );
            } else {
                if to_degrees {
                    println!(
                        "{}",
                        format!("{} rad = {:?}°", value, radians_to_degrees(value))
                    );
                } else {
                    println!(
                        "{}",
        ...
        Some(Commands::Check { ref value, .. }) => {
            let value = *value;
            println!("{}", format!("Value: {}", value));
            println!("{}", format!("  is_finite: {}", is_finite_value(value)));
            println!("{}", format!("  is_infinite: {}", is_infinite(value)));
            println!("{}", format!("  is_nan: {}", is_nan_value(value)));
        }
...
mod tests {
    ...
    #[test]
    fn test_is_finite_value_examples() {
        let _ = is_finite_value(Default::default());
    }
    #[test]
    fn test_is_infinite_examples() {
        let _ = is_infinite(Default::default());
    }
    #[test]
    fn test_is_nan_value_examples() {
        let _ = is_nan_value(Default::default());
    }
    #[test]
    fn test_is_positive_inf_examples() {
        let _ = is_positive_inf(Default::default());
    }
    #[test]
    fn test_is_negative_inf_examples() {
        let _ = is_negative_inf(Default::default());
    }
    #[test]
    fn quickcheck_normalize_angle() {
        fn prop(angle: f64) -> TestResult {
            let once = normalize_angle(angle.clone());
            let twice = normalize_angle(once.clone());
            if once != twice {
