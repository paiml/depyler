...
struct Args {
    ...
    #[doc = "Directory to scan"]
    path: Option<String>,
    
    
    #[doc = "Minimum file size in bytes"]
    min_size: i32,
    
    #[doc = "Filter by extension(can repeat)"]
    ext: Vec<String>,
    
    
    #[doc = "Quick mode(hash only first 64KB)"]
    quick: bool,
    
    
    #[doc = "Show summary only"]
    summary: bool,
...
#[doc = "Calculate hash of file content.\n\n    If quick=True, only hash first 64KB for speed.\n    "]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_file_hash(path: &str, quick: bool) -> Result<String, Box<dyn std::error::Error>> {
    let mut hasher = {
    ...
    match (|| -> Result<String, Box<dyn std::error::Error>> {
        ...
        if quick {
            let mut chunk = {
                let mut _read_buf = vec![0u8; 65536];
                let _n = f.read(&mut _read_buf).unwrap_or(0);
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_file_size(path: &str) -> i32 {
    {
        return std::fs::metadata(&path).unwrap().len() as i64;
        return -1;
    }
}
#[doc = "Scan directory for files matching criteria."]
#[doc = " Depyler: verified panic-free"]
pub fn scan_directory<'a, 'b>(
    path: &'a str,
    min_size: i32,
    extensions: &'b Option<Vec<String>>,
) -> Vec<String> {
    let mut files = vec![];
    for (root, __sanitized, filenames) in std::fs::read_dir(path)
        .into_iter()
        .filter_map(|e| e.ok())
        ...
        .collect::<Vec<_>>()
    {
        for filename in filenames.iter().cloned() {
            let full_path = std::path::PathBuf::from(root)
                .join(filename)
                .to_string_lossy()
                .to_string();
            if let Some(ref extensions_val) = extensions {
                let (__sanitized, ext) = {
                ...
                if !extensions_val.contains(&*ext.to_lowercase()) {
                    continue;
                }
            }
            ...
            files.push(DepylerValue::Str(full_path.to_string()));
        }
    }
    files
}
#[doc = "Group files by size."]
#[doc = " Depyler: verified panic-free"]
pub fn group_by_size(files: &Vec<String>) -> HashMap<i32, Vec<String>> {
    ...
    for path in files.iter().cloned() {
        ...
        if groups.get(&size).is_none() {
            groups.insert(size.clone(), vec![]);
        }
        groups
            .get(&DepylerValue::Int(size as i64))
            .cloned()
            .unwrap_or_default()
            .push(path);
    }
...
#[doc = "Find duplicate files by content.\n\n    Returns list of duplicate groups.\n    "]
#[doc = " Depyler: verified panic-free"]
pub fn find_duplicates(
    ...
    for path in candidates.iter().cloned() {
        let file_hash = get_file_hash(&path, quick)?;
        if !file_hash {
            continue;
        }
        if hash_groups.get(&file_hash).is_none() {
            hash_groups.insert(file_hash.to_string().clone(), vec![]);
        }
        hash_groups
            .get(&DepylerValue::Int(file_hash as i64))
            .cloned()
            .unwrap_or_default()
            .push(path);
    }
...
#[doc = "Calculate wasted space from duplicates."]
pub fn calculate_waste(duplicates: &Vec<Vec<String>>) -> Result<i32, Box<dyn std::error::Error>> {
    let mut total: i32 = Default::default();
    total = 0;
    for group in duplicates.iter().cloned() {
        if (group.len() as i32) < 2 {
            continue;
        }
        ...
        total = total + size * (group.len() as i32).saturating_sub(1);
    }
    Ok(total)
}
#[doc = "Format size in human-readable format."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_size(size: i32) -> String {
    let _cse_temp_0 = size < 1024;
    if _cse_temp_0 {
        return format!("{}B", size);
    }
    let _cse_temp_1 = size < 1048576;
    if _cse_temp_1 {
        return format!("{}KB", size / 1024);
    }
    let _cse_temp_2 = size < 1073741824;
    if _cse_temp_2 {
        return format!("{}MB", size / (1024 * 1024));
    }
    format!("{}GB", size / (1024 * 1024 * 1024))
}
#[doc = "Determine which file to keep(shortest path)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_keep_candidate(group: &Vec<String>) -> String {
    *group.iter().min().unwrap()
}
#[doc = "Format duplicates for display."]
#[doc = " Depyler: proven to terminate"]
pub fn format_duplicates<'a, 'b>(
    duplicates: &'a Vec<Vec<String>>,
    base_path: &'b str,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut lines = vec![];
    for (i, group) in duplicates
        .iter()
        .cloned()
        .enumerate()
        .map(|(i, x)| ((i + 1 as usize) as i32, x))
    {
        ...
        let keep = get_keep_candidate(&group);
        for path in {
            let mut sorted_vec = group.iter().cloned().collect::<Vec<_>>();
            ...
            if !base_path.is_empty() {
                display = {
                    ...
                    let start_obj = std::path::Path::new(base_path);
                    path_obj
        ...
        lines.push(DepylerValue::Str("".to_string().to_string()));
    }
    Ok(lines)
}
#[doc = " Depyler: verified panic-free"]
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut waste: DepylerValue = Default::default();
    let mut extensions: Vec<String> = Default::default();
    let args = Args::default();
    let has_path = args.path.is_some();
    if !std::path::Path::new(&args.path).is_dir() {
        eprintln!("{}", format!("Error: {} is not a directory", args.path));
    ...
    let files = scan_directory(&args.path, args.min_size, &extensions);
    let duplicates = find_duplicates(&files, args.quick)?;
    if args.json {
        let output = {
            let mut map = HashMap::new();
            map.insert("groups".to_string(), duplicates);
            map.insert("total_groups".to_string(), duplicates.len() as i32);
            map.insert(
                "total_duplicates".to_string(),
                duplicates
                    .iter()
                    .cloned()
                    .map(|g| (g.len() as i32).saturating_sub(1))
                    .sum::<i32>(),
            );
            map.insert(
                "wasted_bytes".to_string(),
                calculate_waste(&duplicates).expect("calculate_waste failed"),
            );
            map
        };
    ...
    if args.summary {
        let _cse_temp_0 = duplicates
            .iter()
            .cloned()
            .map(|g| g.len() as i32)
            .sum::<i32>();
        let total_files = _cse_temp_0;
        waste = calculate_waste(&duplicates)?;
        println!(
            "{}",
            format!("Duplicate groups: {}", duplicates.len() as i32)
        );
        println!("{}", format!("Total duplicate files: {}", total_files));
        println!("{}", format!("Wasted space: {}", format_size(waste)));
        return Ok(());
    }
    ...
    waste = calculate_waste(&duplicates)?;
    println!("{}", format!("Total wasted space: {}", format_size(waste)));
    Ok(())
}
#[cfg(test)]
mod tests {
    ...
    #[test]
    fn test_get_file_size_examples() {
        assert_eq!(get_file_size(""), 0);
        assert_eq!(get_file_size("a"), 1);
        assert_eq!(get_file_size("abc"), 3);
    }
    #[test]
    fn test_calculate_waste_examples() {
        assert_eq!(calculate_waste(&vec![]), 0);
        assert_eq!(calculate_waste(&vec![1]), 1);
        assert_eq!(calculate_waste(&vec![1, 2, 3]), 3);
    }
}
