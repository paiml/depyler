...
#[doc = "Find best split using GINI criterion."]
pub fn best_split<'a, 'b>(
    ...
    let features_to_check =
        (0..(depyler_min((n_features).clone(), (max_features).clone()))).collect::<Vec<_>>();
...
pub fn build_tree<'b, 'a>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    depth: i32,
    max_depth: i32,
) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>> {
    let mut majority_class: DepylerValue = Default::default();
    ...
    let (feature, threshold, left_idx, right_idx) = best_split(
        &&X,
        &y,
        X.get(0usize)
            .cloned()
    ...
    Ok({
        ...
        map.insert(
            "left".to_string(),
            build_tree(&left_X, &left_y, depth + 1, max_depth).expect("build_tree failed"),
        );
        map.insert(
            "right".to_string(),
            build_tree(&right_X, &right_y, depth + 1, max_depth).expect("build_tree failed"),
        );
        map
    })
}
#[doc = "Predict class for single sample."]
#[doc = " Depyler: proven to terminate"]
pub fn predict_single<'b, 'a>(
...
#[doc = "Fit decision tree classifier."]
#[doc = " Depyler: proven to terminate"]
pub fn fit<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    ...
    let _cse_temp_5 = _cse_temp_0 != _cse_temp_2;
    if _cse_temp_5 {
        return Err(Box::new(ValueError::new(
            "X and y must have same length".to_string(),
        )));
    }
    let tree = build_tree(&&X, &y, 0, max_depth)?;
    Ok({
        let mut map = HashMap::new();
...
#[doc = "Predict class labels."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn predict<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    tree: &'b std::collections::HashMap<String, DepylerValue>,
) -> Vec<i32> {
    X.iter()
        .cloned()
        .map(|x| predict_single(&tree, &x))
        .collect::<Vec<_>>()
}
...
pub fn score<'a, 'b, 'c>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<i32>,
    tree: &'c std::collections::HashMap<String, DepylerValue>,
) -> Result<f64, Box<dyn std::error::Error>> {
    let predictions = predict(&&X, &tree);
...
pub fn cmd_predict() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = data.get("X").is_none();
    let _cse_temp_1 = data.get("tree").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    if _cse_temp_2 {
        eprintln!("{}", "Error: Missing 'X' or 'tree'");
        std::process::exit(1);
    }
    let predictions = predict(
        data.get("X").cloned().unwrap_or_default(),
        data.get("tree").cloned().unwrap_or_default(),
    );
    println!(
        "{}",
        format!("{:?}", {
...
#[doc = "Handle fit-predict subcommand."]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_fit_predict() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        predictions = predict(
            ...
            result.get("tree").cloned().unwrap_or_default(),
        );
        println!(
            "{}",
            format!("{:?}", {
...
pub fn cmd_score() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_1 = data.get("y").is_none();
    let _cse_temp_2 = (_cse_temp_0) || (_cse_temp_1);
    let _cse_temp_3 = data.get("tree").is_none();
    let _cse_temp_4 = (_cse_temp_2) || (_cse_temp_3);
    if _cse_temp_4 {
        eprintln!("{}", "Error: Missing 'X', 'y', or 'tree'");
        std::process::exit(1);
    }
    let accuracy = score(
        data.get("X").cloned().unwrap_or_default(),
