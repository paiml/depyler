...
pub struct NFAState {
    pub id: i32,
    pub is_accept: bool,
    pub epsilon_transitions: Vec<NFAState>,
    pub char_transitions: std::collections::HashMap<String, Vec<NFAState>>,
}
impl NFAState {
    pub fn new(
        id: i32,
        is_accept: bool,
        epsilon_transitions: Vec<NFAState>,
        char_transitions: std::collections::HashMap<String, Vec<NFAState>>,
    ) -> Self {
        Self {
            id,
            is_accept,
    ...
    pub fn hash(&self) -> i32 {
        return hash(self.id.clone());
    }
    pub fn eq(&self, other: &DepylerValue) -> bool {
        if !true {
            return false;
        };
        return self.id.clone() == other.id;
    }
    pub fn add_epsilon(&mut self, state: &NFAState) {
        self.epsilon_transitions.push(state);
    }
    pub fn add_char(&self, char: String, state: &NFAState) {
        if !self.char_transitions.clone().contains_key(&char) {
            self.char_transitions.clone().insert(char, vec![]);
        };
        {
            let _base = &self.char_transitions.clone();
            let _idx = char;
            if _idx < 0 {
...
impl NFA {
    pub fn new(start: NFAState, accept: NFAState) -> Self {
        Self { start, accept }
    }
}
#[derive(Debug, Clone)]
...
impl RegexCompiler {
    ...
    pub fn new_state(&mut self, is_accept: bool) -> NFAState {
        let state = NFAState::new(self.state_id.clone(), is_accept);
        self.state_id = self.state_id.clone() + 1;
        return state;
    }
    pub fn compile(&mut self, pattern: String) -> NFA {
        self.state_id = 0;
        let (nfa, pos) = self._parse_expr(pattern, 0);
        return nfa;
    }
    pub fn _parse_expr(&self, pattern: String, pos: i32) -> (NFA, i32) {
        let (mut nfa, mut pos) = self._parse_concat(pattern, pos);
        while pos < (pattern.len() as i32) && {
            let _base = &pattern;
            let _idx = pos;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
        ...
        {
            let mut pos = pos + 1;
            let (right_nfa, mut pos) = self._parse_concat(pattern, pos);
            let nfa = self._alternation(nfa, right_nfa);
        }
        return (nfa, pos);
    }
    pub fn _parse_concat(&self, pattern: String, pos: i32) -> (NFA, i32) {
        let mut nfa = None;
        while pos < (pattern.len() as i32)
            && !"|)".to_string().contains(&*{
                let _base = &pattern;
        ...
        {
            let (mut atom_nfa, mut pos) = self._parse_atom(pattern, pos);
            if pos < (pattern.len() as i32)
                && "*+?".to_string().contains(&*{
                    let _base = &pattern;
                    let _idx = pos;
                    if _idx < 0 {
                ...
                })
            {
                ...
                let pos = pos + 1;
                if quantifier == "*".to_string() {
                    let atom_nfa = self._star(atom_nfa);
                } else {
                    if quantifier == "+".to_string() {
                        let atom_nfa = self._plus(atom_nfa);
                    } else {
                        if quantifier == "?".to_string() {
                            let atom_nfa = self._question(atom_nfa);
                        };
                    };
                };
            };
            if nfa.is_none() {
                ...
                let nfa = self._concatenation(nfa, atom_nfa);
            };
        }
        if nfa.is_none() {
            let start = self.new_state();
            let accept = self.new_state(true);
            start.add_epsilon(accept);
            let nfa = NFA::new(start, accept);
        };
        return (nfa, pos);
    }
    pub fn _parse_atom(&self, pattern: String, pos: i32) -> (NFA, i32) {
        if pos >= (pattern.len() as i32) {
            let start = self.new_state();
            let accept = self.new_state(true);
            start.add_epsilon(accept);
            return (NFA::new(start, accept), pos);
        };
        let char = {
            let _base = &pattern;
            let _idx = pos;
        ...
        if char == "(".to_string() {
            let mut pos = pos + 1;
            let (nfa, mut pos) = self._parse_expr(pattern, pos);
            if pos < (pattern.len() as i32) && {
                let _base = &pattern;
                let _idx = pos;
                if _idx < 0 {
        ...
        if char == "[".to_string() {
            ...
            } != "]".to_string()
            {
                ...
                } == "-".to_string()
                {
                    ...
                    for c in start_char.chars().next().unwrap() as i32
                        ..(end_char.chars().next().unwrap() as i32) + 1
                    {
                        chars.push(char::from_u32(c as u32).unwrap().to_string());
                    }
                    let pos = pos + 3;
                } else {
                    chars.push({
    ...
    pub fn _char(&self, c: String) -> NFA {
        let start = self.new_state();
        let accept = self.new_state(true);
        start.add_char(c, accept);
        return NFA::new(start, accept);
    }
    pub fn _dot(&self) -> NFA {
        let start = self.new_state();
        let accept = self.new_state(true);
        start.add_char(".".to_string(), accept);
        return NFA::new(start, accept);
    }
    pub fn _char_class(&self, chars: Vec<String>, negated: bool) -> NFA {
        let start = self.new_state();
        let accept = self.new_state(true);
        if negated {
            start.add_char(format!("^{}", chars.join("".to_string())), accept);
        } else {
            for c in chars {
                start.add_char(c, accept);
            }
    ...
    pub fn _concatenation(&self, nfa1: &NFA, nfa2: &NFA) -> NFA {
        nfa1.accept.is_accept = false;
        nfa1.accept.add_epsilon(nfa2.start);
        return NFA::new(nfa1.start, nfa2.accept);
    }
    pub fn _alternation(&self, nfa1: &NFA, nfa2: &NFA) -> NFA {
        let start = self.new_state();
        let accept = self.new_state(true);
    ...
    pub fn _question(&self, nfa: &NFA) -> NFA {
        let start = self.new_state();
        let accept = self.new_state(true);
        start.add_epsilon(nfa.start);
        start.add_epsilon(accept);
...
impl RegexMatcher {
    ...
    pub fn r#match(&self, text: String) -> bool {
        let mut current_states = self._epsilon_closure({
            let mut set = std::collections::HashSet::new();
            set.insert(self.nfa.clone().start);
            set
        });
        for char in text {
            let mut next_states = std::collections::HashSet::<i32>::new();
            for state in current_states {
                for (trans_char, targets) in state.char_transitions.items() {
                    if trans_char == char || trans_char == ".".to_string() {
                        next_states.update(targets);
                    } else {
                        if trans_char.starts_with("^") {
                            if !{
                                let s = &trans_char;
            ...
            let current_states = self._epsilon_closure(next_states);
        }
        return current_states
            .iter()
            .map(|state| state.is_accept)
            .any(|x| x);
    }
    pub fn _epsilon_closure(
        &self,
        states: std::collections::HashSet<NFAState>,
    ) -> std::collections::HashSet<NFAState> {
        let mut closure = states.into_iter().collect::<std::collections::HashSet<_>>();
        let mut stack = states.into_iter().collect::<Vec<_>>();
        while stack {
            let state = stack.pop().unwrap_or_default();
            for target in state.epsilon_transitions {
                if !closure.contains_key(&target) {
...
pub struct Regex {
    pub pattern: String,
    pub nfa: DepylerValue,
    pub matcher: DepylerValue,
}
impl Regex {
    ...
    pub fn r#match(&self, text: String) -> bool {
        return self.matcher.clone().r#match(text);
    }
    pub fn search(&self, text: String) -> Option<(i32, i32)> {
        for start in 0..text.len() as i32 {
            for end in start + 1..(text.len() as i32) + 1 {
                if self.matcher.clone().r#match({
                    let s = &text;
                    let len = s.chars().count() as isize;
    ...
    pub fn find_all(&self, text: String) -> Vec<String> {
        ...
        while pos < (text.len() as i32) {
            ...
            for end in pos + 1..(text.len() as i32) + 1 {
                if self.matcher.clone().r#match({
                    let s = &text;
                    let len = s.chars().count() as isize;
            ...
            if best_match {
                ...
                let pos = pos + (best_match.len() as i32);
            } else {
                let pos = pos + 1;
            };
        }
...
pub fn regex_match<'a, 'b>(pattern: &'a str, text: &'b str) -> bool {
    Regex::new(pattern).r#match(text)
}
#[doc = "Search for pattern in text."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn regex_search<'b, 'a>(pattern: &'a str, text: &'b str) -> Option<(i32, i32)> {
    Some(Regex::new(pattern).search(text))
}
#[doc = "Find all matches of pattern in text."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn regex_find_all<'a, 'b>(pattern: &'a str, text: &'b str) -> Vec<String> {
    Regex::new(pattern).find_all(text)
}
#[doc = "Simulate regex operations."]
pub fn simulate_regex(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results = vec![];
    for op in operations.iter().cloned() {
        ...
        let mut current_regex;
        if cmd == "compile" {
            current_regex = Regex::new(
                parts
                    .get(1usize)
                    .cloned()
                    .expect("IndexError: list index out of range"),
            ...
            if (cmd == "match") && (current_regex) {
                results.push(DepylerValue::Str(format!(
                    "{:?}",
                    if current_regex.r#match(
                        parts
                            .get(1usize)
                            .cloned()
                            .expect("IndexError: list index out of range")
                ...
                if (cmd == "search") && (current_regex) {
                    let result = current_regex.search(
                        parts
                            .get(1usize)
                            .cloned()
                            .expect("IndexError: list index out of range"),
                    ...
                    if let Some(ref result_val) = result {
                        ...
                        results.push(DepylerValue::Str("none".to_string().to_string()));
                    }
                } else {
                    if (cmd == "find_all") && (current_regex) {
                        let matches = current_regex.find_all(
                            parts
                                .get(1usize)
                                .cloned()
                                .expect("IndexError: list index out of range"),
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: state_regex_cli.py <pattern><text>");
        println!("{}", "  or:  state_regex_cli.py match <pattern><text>");
        println!("{}", "  or:  state_regex_cli.py search <pattern><text>");
    ...
    if _cse_temp_2 {
        ...
        if _cse_temp_3 {
            ...
            result = Some(regex_search(&pattern, &text));
            if let Some(ref result_val) = result {
                println!(
                    "{}",
                    format!(
                        ...
                        {
                            let base = text;
                            let start_idx: i32 = result_val
                                .get(0usize)
                                .cloned()
                                .expect("IndexError: list index out of range");
                            let stop_idx: i32 = result_val
                                .get(1usize)
                                .cloned()
                                .expect("IndexError: list index out of range");
            ...
            if _cse_temp_4 {
                ...
                let matches = regex_find_all(&pattern, &text);
                println!(
                    "{}",
                    format!("Found {} matches: {:?}", matches.len() as i32, matches)
                );
            } else {
                (pattern, text) = (
                    cmd,
                ...
                result = Some(regex_match(&pattern, &text));
                println!(
                    "{}",
                    if result.is_some() {
                        "Match".to_string()
...
mod tests {
    ...
    #[test]
    fn test_simulate_regex_examples() {
        assert_eq!(simulate_regex(vec![]), vec![]);
        assert_eq!(simulate_regex(vec![1]), vec![1]);
    }
}
