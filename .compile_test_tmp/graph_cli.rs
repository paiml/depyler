...
} #[derive(Debug, Clone)] pub struct Graph {
    pub directed: bool, pub adj: std::collections::HashMap<String, Vec<String>>, pub weights: std::collections::HashMap <(String, String), i32>
}
impl Graph {
    pub fn new(directed: bool) -> Self {
    Self {
    directed, adj: std::collections::HashMap::new(), weights: std::collections::HashMap::new()
}
} pub fn add_vertex(&self, v: String) {
    if! self.adj.clone().contains_key(& v) {
    self.adj.clone().insert(v, vec! []);
    };
   
}
pub fn add_edge(&self, u: String, v: String, weight: i32) {
    self.add_vertex(u);
    self.add_vertex(v);
    {
    let _base = &self.adj.clone();
    let _idx = u;
    if _idx<0 {
    ...
    self.weights.clone().insert((u, v), weight);
    if! self.directed.clone() {
    { let _base = &self.adj.clone();
    let _idx = v;
    if _idx<0 {
    _base [_base.len().wrapping_sub((- _idx) as usize)].clone()
...
pub fn edges(&self) -> Vec <(String, String, i32)>{
    ...
    let edge = if! self.directed.clone() {
   (((u as f64)).min (v as f64) ,((u as f64)).max(v as f64))
}
else {
   (u, v) };
    if! seen.contains_key(& edge) {
    seen.insert(edge);
    result.push((u, v, self.weights.clone().get((u, v), 1)));
    };
    };
    };
    return result;
...
pub fn neighbors(&self, v: String) -> Vec<String>{
    return self.adj.clone().get(v, vec! []);
   
}
} #[derive(Default)] struct Args {
    #[doc = "Edges: u-v or u-v:weight"] edges: Vec<String>, #[doc = "Directed graph"] directed: bool, #[doc = "Operation mode"] mode: String, #[doc = "Start vertex"] start: String, #[doc = "End vertex"] end: String
}
#[doc = "Breadth-first search traversal."] #[doc = " Depyler: verified panic-free"] pub fn bfs<'b, 'a>(graph: & 'a Graph, start: & 'b str) -> Vec<String>{
    let _cse_temp_0 = graph.adj.get(start).is_none();
    if _cse_temp_0 {
    return vec! [];
...
visited.insert(v);
    result.push(DepylerValue::Str(format!("{:?}", v)));
...
#[doc = "Depth-first search traversal(iterative)."] #[doc = " Depyler: verified panic-free"] pub fn dfs<'a, 'b>(graph: & 'a Graph, start: & 'b str) -> Vec<String>{
    let _cse_temp_0 = graph.adj.get(start).is_none();
    if _cse_temp_0 {
    return vec! [];
...
visited.insert(v);
    result.push(DepylerValue::Str(format!("{:?}", v)));
    for neighbor in graph.neighbors(v).iter().cloned().rev() {
    if! visited.contains(& neighbor) {
    stack.push(neighbor);
...
#[doc = "Depth-first search traversal(recursive)."] #[doc = " Depyler: verified panic-free"] pub fn dfs_recursive<'a, 'b, 'c>(graph: & 'a Graph, start: & 'b str, visited: & 'c mut Option<std::collections::HashSet<String>>) -> Vec<String>{
    if visited.is_none() {
    visited = std::collections::HashSet::<i32>::new();
   
}
let _cse_temp_0 = graph.adj.get(start).is_none();
    let _cse_temp_1 = visited.contains(start);
...
visited.insert(start);
    ...
    result.extend(dfs_recursive(& graph, & neighbor, &mut visited).iter().cloned());
   
}
result
}
#[doc = "Find shortest path using BFS(unweighted)."] #[doc = " Depyler: verified panic-free"] pub fn shortest_path_bfs<'a, 'c, 'b>(graph: & 'a Graph, start: & 'b str, end: & 'c str) -> Vec<String>{
    let _cse_temp_0 = graph.adj.get(start).is_none();
    let _cse_temp_1 = graph.adj.get(end).is_none();
    let _cse_temp_2  = (_cse_temp_0) ||(_cse_temp_1);
    if _cse_temp_2 {
    return vec! [];
   
}
let _cse_temp_3 = start == end;
    if _cse_temp_3 {
    return vec! [start];
...
let mut visited = {
    ...
    set.insert(start);
    set };
    let mut queue = VecDeque::from(vec! [(start, vec! [start])]);
    while! queue.is_empty() {
    let(v, path) = queue.pop_front();
    for neighbor in graph.neighbors(v) {
    if neighbor = = (* end) {
    return path.iter().chain (vec! [neighbor].iter()).cloned().collect::<Vec<_>>();
   
}
if! visited.contains(& neighbor) {
    visited.insert(neighbor);
    queue.push_back((neighbor, path.iter().chain (vec! [neighbor].iter()).cloned().collect::<Vec<_>>()));
...
#[doc = "Find shortest path using Dijkstra's algorithm(weighted)."] pub fn dijkstra<'a, 'c, 'b>(graph: & 'a Graph, start: & 'b str, end: & 'c str) -> Result <(Vec<String>, i32), Box<dyn std::error::Error>>{
...
let mut distances = graph.vertices().into_iter().map(| v | {
    let _v = "inf".parse::<f64>().unwrap();
   (v, _v) }).collect::<std::collections::HashMap<_, _>>();
    distances.insert(start.to_string().clone(), 0);
    let mut previous: std::collections::HashMap<String, Option<String>>= graph.vertices().iter().map(| k |(k.clone() ,())).collect();
    let mut unvisited = graph.vertices().into_iter().collect::<std::collections::HashSet<_>>();
    while! unvisited.is_empty() {
    current = Some(* unvisited.iter().min ().unwrap());
    if(distances.get(& DepylerValue::Int(current as i64)).cloned().unwrap_or_default() as f64) == "inf".parse::<f64>().unwrap() {
    break;
   
}
if current.unwrap_or_default() = = (* end) {
    break;
...
if! unvisited.remove(& current) {
    ...
    let weight = graph.weights.get(&(current, neighbor)).cloned().unwrap_or(1);
    let alt = distances.get(& DepylerValue::Int(current as i64)).cloned().unwrap_or_default() + weight;
    if alt <(distances.get(& DepylerValue::Int(neighbor as i64)).cloned().unwrap_or_default() as f64) {
    distances.insert(neighbor.to_string().clone(), alt);
    previous.insert(neighbor.to_string().clone(), current);
   
}
}
}
} let _cse_temp_3 = "inf".parse::<f64>().unwrap();
    let _cse_temp_4  = (distances.get(end).cloned().unwrap_or_default() as f64) == _cse_temp_3;
    if _cse_temp_4 {
    return Ok((vec! [], - 1));
...
let mut path = vec! [];
    ...
    path.push(DepylerValue::Str(current.to_string()));
    current = previous.get(& current).cloned().unwrap_or_default();
   
}
Ok((path.iter().cloned().rev().into_iter().collect::<Vec<_>>() ,(distances.get(end).cloned().unwrap_or_default()) as i32))
}
#[doc = "Detect if graph has a cycle."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn has_cycle(graph: & Graph) -> Result<bool, Box<dyn std::error::Error>>{
    if graph.directed {
    return Ok(has_cycle_directed(graph));
   
}
Ok(has_cycle_undirected(graph))
}
#[doc = "Detect cycle in directed graph using colors."] pub fn has_cycle_directed(graph: & Graph) -> Result<bool, Box<dyn std::error::Error>>{
    let(WHITE, GRAY, BLACK)  = (0, 1, 2);
    let color = graph.vertices().iter().map(| k |(k.clone(), WHITE)).collect();
    let dfs_visit = move | v: & str | -> bool {
    color.insert((v) as usize, GRAY);
    for neighbor in graph.neighbors(v) {
    if color.get(neighbor as usize).cloned().expect("IndexError: list index out of range") == GRAY {
    return true;
   
}
if(color.get(neighbor as usize).cloned().expect("IndexError: list index out of range") == WHITE) &&(dfs_visit(neighbor)) {
    return true;
   
}
} color.insert((v) as usize, BLACK);
    return false;
    };
    for v in graph.vertices() {
    if color.get(& v).cloned().unwrap_or_default() == WHITE {
    if dfs_visit(& v) {
    return Ok(true);
...
#[doc = "Detect cycle in undirected graph."] #[doc = " Depyler: verified panic-free"] pub fn has_cycle_undirected(graph: & Graph) -> bool {
    let visited = std::collections::HashSet::<i32>::new();
    let dfs_visit = move | v: & str, parent: Option<String>| -> bool {
    visited.insert(v);
    for neighbor in graph.neighbors(v) {
    if! visited.contains(& neighbor) {
    ...
    if neighbor!= parent.unwrap_or_default() {
    return true;
...
#[doc = "Topological sort using Kahn's algorithm."] pub fn topological_sort(graph: & Graph) -> Result<Vec<String>, Box<dyn std::error::Error>>{
    if! graph.directed {
    return Ok(vec! []);
   
}
let mut in_degree = graph.vertices().iter().map(| k |(k.clone(), 0)).collect();
    for u in graph.adj {
    for v in graph.adj.get(& DepylerValue::Int(u as i64)).cloned().unwrap_or_default() {
    { let _key = v;
...
let mut queue = VecDeque::from(graph.vertices().into_iter().filter(| v | {
    let v = v.clone();
    in_degree.get(v as usize).cloned().expect("IndexError: list index out of range") == 0 }).map(| v | v).collect::<Vec<_>>());
    let mut result = vec! [];
    while! queue.is_empty() {
    let v = queue.pop_front();
    result.push(DepylerValue::Str(format!("{:?}", v)));
...
let _cse_temp_0 = result.len() as i32;
    let _cse_temp_1 = graph.vertices().len() as i32;
    let _cse_temp_2 = _cse_temp_0!= _cse_temp_1;
    if _cse_temp_2 {
    return Ok(vec! []);
...
#[doc = "Find connected components in undirected graph."] #[doc = " Depyler: verified panic-free"] pub fn connected_components(graph: & Graph) -> Vec<Vec<String>>{
    let mut visited = std::collections::HashSet::<i32>::new();
    let mut components = vec! [];
    for v in graph.vertices() {
    if! visited.contains(& v) {
    let component = bfs(& graph, & v);
    for node in component.iter().cloned() {
    visited.insert(node);
   
}
components.push(DepylerValue::Str(format!("{:?}", component)));
   
}
} components
}
#[doc = "Check if graph is bipartite."] pub fn is_bipartite(graph: & Graph) -> Result<bool, Box<dyn std::error::Error>>{
    let mut color = {
...
let mut queue = VecDeque::from(vec! [start]);
    color.insert(start.to_string().clone(), DepylerValue::Str(format!("{:?}", 0)));
    while! queue.is_empty() {
    let v = queue.pop_front();
    for neighbor in graph.neighbors(v) {
    if color.get(& neighbor).is_none() {
    color.insert(neighbor.to_string().clone(), DepylerValue::Str(format!("{:?}", 1 - color.get(& DepylerValue::Int(v as i64)).cloned().unwrap_or_default())));
    queue.push_back(neighbor);
   
}
else {
    if color.get(& DepylerValue::Int(neighbor as i64)).cloned().unwrap_or_default() == color.get(& DepylerValue::Int(v as i64)).cloned().unwrap_or_default() {
    return Ok(false);
...
#[doc = "Parse edge strings like 'a-b' or 'a-b:5'(weighted)."] #[doc = " Depyler: verified panic-free"] pub fn parse_edges(edge_strs: & Vec<String>) -> Vec <(String, String, i32)>{
    let mut edges = vec! [];
    for edge_str in edge_strs.iter().cloned() {
    let mut v;
    let mut u;
    if edge_str.contains(":") {
    let _split_parts = edge_str.rsplitn((1 + 1) as usize, ":").map(| s | s.to_string()).collect::<Vec<String>>();
    let edge_part = _split_parts.get(0).cloned().unwrap_or_default();
    let weight = _split_parts.get(1).cloned().unwrap_or_default();
   (u, v) = edge_part.split("-").map(| s | s.to_string()).collect::<Vec<String>>();
    edges.push(DepylerValue::Str(format!("{:?}" ,(u, v ,(weight) as i32))));
   
}
else {
   (u, v) = edge_str.split("-").map(| s | s.to_string()).collect::<Vec<String>>();
    edges.push(DepylerValue::Str(format!("{:?}" ,(u, v, 1))));
...
#[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), Box<dyn std::error::Error>>{
    ...
    let graph = Graph::new(args.directed);
...
} let _cse_temp_0 = args.mode == "bfs";
    let mut result;
    let mut start;
    if _cse_temp_0 {
    let _cse_temp_1  = (args.start) ||(if graph.vertices() {
    graph.vertices().get(0usize).cloned().expect("IndexError: list index out of range")
}
else {
    "".to_string() });
    start = _cse_temp_1;
    result = bfs(& graph, & start);
    println!("{}", format!("BFS from {}: {}", start, result.join (" -> ").display()));
   
}
else {
    let _cse_temp_2 = args.mode == "dfs";
    if _cse_temp_2 {
    let _cse_temp_3  = (args.start) ||(if graph.vertices() {
    graph.vertices().get(0usize).cloned().expect("IndexError: list index out of range")
}
else {
    "".to_string() });
    ...
    println!("{}", format!("DFS from {}: {}", start, result.join (" -> ").display()));
   
}
else {
    let _cse_temp_4 = args.mode == "path";
    let mut path;
    if _cse_temp_4 {
    let _cse_temp_5  = (! args.start) ||(! args.end);
    if _cse_temp_5 {
    println!("{}", "Error: --start and --end required for path");
    std::process::exit(1i32)
}
path = shortest_path_bfs(& graph, & args.start, & args.end);
    if! path.is_empty() {
    println!("{}", format!("Path: {}", path.join (" -> ").display()));
    ...
    let _cse_temp_6 = args.mode == "dijkstra";
    if _cse_temp_6 {
    let _cse_temp_7  = (! args.start) ||(! args.end);
    if _cse_temp_7 {
    println!("{}", "Error: --start and --end required for dijkstra");
    std::process::exit(1i32)
}
let(mut path, dist) = dijkstra(& graph, & args.start, & args.end) ?;
    if! path.is_empty() {
    println!("{}", format!("Path: {}(distance: {:?})", path.join (" -> ").display(), dist));
    ...
    let _cse_temp_8 = args.mode == "cycle";
    if _cse_temp_8 {
    if has_cycle(& graph).unwrap_or(false) {
    println!("{}", "Cycle: Yes");
    ...
    let _cse_temp_9 = args.mode == "topo";
    if _cse_temp_9 {
    result = topological_sort(& graph) ?;
    if! result.is_empty() {
    println!("{}", format!("Topological order: {}", result.join (" -> ").display()));
   
}
else {
    println!("{}", "No valid topological order(cycle or undirected)");
   
}
} else {
    let _cse_temp_10 = args.mode == "components";
    if _cse_temp_10 {
    let components = connected_components(& graph);
    println!("{}", format!("Components: {}", components.len() as i32));
    for(i, comp) in components.iter().cloned().enumerate().map(|(i, x) |(i as i32, x)) {
    let i = i as i32;
    println!("{}", format!("  {}: {}", i + 1, comp.join (", ").display()));
   
}
} else {
    let _cse_temp_11 = args.mode == "bipartite";
    if _cse_temp_11 {
    if is_bipartite(& graph).unwrap_or(false) {
    println!("{}", "Bipartite: Yes");
   
}
else {
    println!("{}", "Bipartite: No");
...
#[cfg(test)] mod tests {
    ...
    #[test] fn test_has_cycle_examples() {
    let _ = has_cycle(Default::default());
   
}
#[test] fn test_has_cycle_directed_examples() {
    let _ = has_cycle_directed(Default::default());
   
}
#[test] fn test_has_cycle_undirected_examples() {
    let _ = has_cycle_undirected(Default::default());
   
}
#[test] fn quickcheck_topological_sort() {
    fn prop(graph :()) -> TestResult {
    let result = topological_sort(graph.clone());
    ...
    input_sorted.sort();
    let mut result = topological_sort(graph.clone());
    result.sort();
...
#[test] fn test_is_bipartite_examples() {
    let _ = is_bipartite(Default::default());
   
}
#[test] fn test_parse_edges_examples() {
    assert_eq!(parse_edges(vec! []), vec! []);
    assert_eq!(parse_edges(vec! [1]), vec! [1]);
