...
pub fn read_text(path: &str, encoding: String) -> String {
    std::fs::read_to_string(&std::path::PathBuf::from(&path)).unwrap()
}
#[doc = "Write text to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn write_text<'a, 'b>(path: &'a str, content: &'b str, encoding: String) -> i32 {
    std::fs::write(&std::path::PathBuf::from(&path), content).unwrap()
}
#[doc = "Read file lines as list."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_lines_text(path: &str, encoding: String) -> Vec<String> {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
        .collect::<Vec<String>>()
}
#[doc = "Read file lines keeping line endings."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn read_lines_keep_ends(path: &str, encoding: String) -> Vec<String> {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
...
#[doc = "Count characters in file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_characters(path: &str, encoding: String) -> i32 {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .len() as i32 as i32
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_lines_text(path: &str, encoding: String) -> i32 {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
...
#[doc = "Count words in file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn count_words_text(path: &str, encoding: String) -> i32 {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .split_whitespace()
        .map(|s| s.to_string())
...
#[doc = "Get first n lines."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn head_lines(path: &str, n: i32, encoding: String) -> Vec<String> {
    let lines = std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
...
#[doc = "Get last n lines."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn tail_lines(path: &str, n: i32, encoding: String) -> Vec<String> {
    let lines = std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
...
#[doc = "Get specific line(1-indexed)."]
#[doc = " Depyler: proven to terminate"]
pub fn get_line(
    path: &str,
    line_num: i32,
    ...
    let lines = std::fs::read_to_string(&std::path::PathBuf::from(&path))
        ...
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
    let _cse_temp_0 = 1 <= line_num;
    let _cse_temp_1 = lines.len() as i32;
    let _cse_temp_2 = line_num <= _cse_temp_1;
    let _cse_temp_3 = (_cse_temp_0) && (_cse_temp_2);
    if _cse_temp_3 {
        return Ok({
            let base = &lines;
            let idx: i32 = line_num - 1;
...
#[doc = "Search for pattern in file, return matching lines with numbers."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn search_text<'b, 'a>(
    path: &'a str,
    pattern: &'b str,
    encoding: String,
) -> Vec<(i32, String)> {
        ...
        .map(|(i, x)| ((i + 1 as usize) as i32, x))
    {
        ...
        if line.contains(&*pattern) {
            results.push(DepylerValue::Str(format!("{:?}", (i, line))));
        }
    }
    results
}
#[doc = "Replace text in file, return count of replacements."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn replace_text<'a, 'b, 'c>(
    path: &'a str,
    old: &'b str,
    new: &'c str,
    encoding: String,
    ...
    let count = content.matches(&*old).count() as i32;
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn append_text<'a, 'b>(path: &'a str, content: &'b str, encoding: String) -> i32 {
    let p = std::path::PathBuf::from(&path);
...
#[doc = "Prepend text to file."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn prepend_text<'a, 'b>(path: &'a str, content: &'b str, encoding: String) -> i32 {
    let p = std::path::PathBuf::from(&path);
    let existing = if p.exists() {
        std::fs::read_to_string(&p).unwrap()
    } else {
...
#[doc = "Insert content at specific line."]
#[doc = " Depyler: proven to terminate"]
pub fn insert_at_line(
    ...
    let p = std::path::PathBuf::from(&path);
    let mut lines = std::fs::read_to_string(&p)
        .unwrap()
        .lines()
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
    let _cse_temp_0 = (!lines.is_empty())
        && (!{
            let base = &lines;
    ...
    if !_cse_temp_0.is_empty() {
        ...
        lines.insert((-1) as usize, _cse_temp_1);
    }
    if !content.ends_with("\n") {
        content = format!("{}{}", content, "\n");
    }
    lines.insert(line_num - 1 as usize, content);
    Ok(std::fs::write(&p, lines.join("")).unwrap())
}
#[doc = "Delete specific line, return deleted content."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn delete_at_line(path: &str, line_num: i32, encoding: String) -> String {
    let p = std::path::PathBuf::from(&path);
    let mut lines = std::fs::read_to_string(&p)
        .unwrap()
        .lines()
    ...
    let _cse_temp_3 = (_cse_temp_0) && (_cse_temp_2);
    if _cse_temp_3 {
        let deleted = lines.remove(line_num - 1 as usize);
        std::fs::write(&p, lines.join("")).unwrap();
        return deleted
...
#[doc = "Read and strip all lines."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn strip_lines(path: &str, encoding: String) -> Vec<String> {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
...
#[doc = "Read non-empty lines."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn filter_empty_lines(path: &str, encoding: String) -> Vec<String> {
    std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
...
#[doc = "Simple encoding detection(checks for BOM)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn detect_encoding(path: &str) -> String {
    let data = std::fs::read(&std::path::PathBuf::from(&path)).unwrap();
...
#[doc = "Convert file between encodings."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn convert_encoding<'a, 'b>(
    src: &'a str,
    dst: &'b str,
    src_enc: String,
    dst_enc: String,
) -> i32 {
    let content = std::fs::read_to_string(&std::path::PathBuf::from(&src)).unwrap();
    std::fs::write(&std::path::PathBuf::from(&dst), content).unwrap()
}
#[doc = "Normalize line endings(\\n, \\r\\n, or \\r)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn normalize_line_endings<'a, 'b>(path: &'a str, ending: &'b str, encoding: String) -> i32 {
    let mut normalized: String = Default::default();
    let p = std::path::PathBuf::from(&path);
    let content = std::fs::read_to_string(&p).unwrap();
    normalized = content.replace("\r\n", "\n").replace("\r", "\n");
    let _cse_temp_0 = (*ending) != "\n";
    if _cse_temp_0 {
        normalized = normalized.replace("\n", ending);
    }
    std::fs::write(&p, normalized).unwrap()
}
#[doc = "Ensure file ends with newline."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn ensure_newline_at_end(path: &str, encoding: String) -> bool {
    let p = std::path::PathBuf::from(&path);
    let content = std::fs::read_to_string(&p).unwrap();
    let _cse_temp_0 = (content) && (!content.ends_with("\n"));
    if _cse_temp_0 {
...
#[doc = "Remove trailing whitespace from each line."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn remove_trailing_whitespace(path: &str, encoding: String) -> i32 {
    let p = std::path::PathBuf::from(&path);
    ...
    .unwrap();
    let _cse_temp_0 = lines
        .into_iter()
        .zip(stripped.into_iter())
        .into_iter()
...
#[doc = "Join all lines with separator."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn join_lines<'b, 'a>(path: &'a str, separator: &'b str, encoding: String) -> String {
    let lines = std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
...
#[doc = "Split file into multiple files."]
#[doc = " Depyler: proven to terminate"]
pub fn split_into_files<'a, 'b>(
    path: &'a str,
    output_dir: &'b str,
    lines_per_file: i32,
    encoding: String,
) -> Result<i32, Box<dyn std::error::Error>> {
    let mut file_count: i32 = Default::default();
    let lines = std::fs::read_to_string(&std::path::PathBuf::from(&path))
        .unwrap()
        .lines()
        .map(|s| s.to_string())
    ...
    file_count = 0;
    for i in {
        let step = lines_per_file as usize;
        if step == 0 {
            panic!("range() arg 3 must not be zero");
        }
        (0..lines.len() as i32).step_by(step)
    } {
        let chunk = {
            let base = &lines;
            let start_idx = (i) as isize;
            let stop_idx = (i + lines_per_file) as isize;
            let start = if start_idx < 0 {
                (base.len() as isize + start_idx).max(0) as usize
            } else {
                start_idx as usize
        ...
        let out_file = out_path.join(format!("part_{}.txt", file_count));
        std::fs::write(&out_file, format!("{}{}", chunk.join("\n"), "\n")).unwrap();
...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Read { .. }));
    match &args.command {
        Some(Commands::Read {
            ref encoding,
            ref path,
            ...
            let content = read_text(&path, encoding);
            println!("{}", content);
        }
        Some(Commands::Write {
            ref content,
            ...
            let mut n = write_text(&path, &content, encoding);
            println!("{}", format!("Wrote {:?} bytes", n));
        }
        Some(Commands::Head {
            ref n, ref path, ..
        }) => {
            let n = n.clone();
            let mut lines = head_lines(&path, n, "utf-8".to_string());
        ...
        Some(Commands::Tail {
            ...
            let n = n.clone();
            let mut lines = tail_lines(&path, n, "utf-8".to_string());
            for line in lines.iter().cloned() {
                println!("{}", line);
            }
        }
        Some(Commands::Count { ref path, .. }) => {
            let chars = count_characters(&path, "utf-8".to_string());
            let mut lines = count_lines_text(&path, "utf-8".to_string());
            let words = count_words_text(&path, "utf-8".to_string());
            println!("{}", format!("Characters: {:?}", chars));
            println!("{}", format!("Lines: {:?}", lines));
            println!("{}", format!("Words: {:?}", words));
        }
        Some(Commands::Search {
            ...
            let results = search_text(&path, &pattern, "utf-8".to_string());
            for (line_num, line) in results.iter().cloned() {
                println!("{}", format!("{}: {}", line_num, line));
            }
        }
        Some(Commands::Replace {
            ...
            let count = replace_text(&path, &old, &new, "utf-8".to_string());
            println!("{}", format!("Replaced {:?} occurrences", count));
        }
        Some(Commands::Convert {
            ref dst, ref src, ..
        }) => {
            let mut n = convert_encoding(&src, &dst, args.src_enc, args.dst_enc);
            println!("{}", format!("Converted {:?} bytes", n));
        }
...
mod tests {
    ...
    #[test]
    fn quickcheck_normalize_line_endings() {
        fn prop(path: String, ending: String, encoding: String) -> TestResult {
            let once =
                normalize_line_endings((&*path).into(), (&*ending).into(), (&*encoding).into());
        ...
        quickcheck(prop as fn(String, String, String) -> TestResult);
    }
}
