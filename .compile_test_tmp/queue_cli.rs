...
impl TaskStatus {
    pub const PENDING: i32 = "pending".to_string();
    pub const RUNNING: i32 = "running".to_string();
    pub const COMPLETED: i32 = "completed".to_string();
    pub const FAILED: i32 = "failed".to_string();
    pub fn new() -> Self {
        Self {}
    }
}
...
pub struct Task {
    ...
    pub status: DepylerValue,
    pub created_at: DepylerValue,
    pub started_at: Option<DepylerDateTime>,
    pub completed_at: Option<DepylerDateTime>,
...
impl Task {
    pub fn new(
        ...
        name: String,
        priority: i32,
        data: Option<std::collections::HashMap<DepylerValue, DepylerValue>>,
    ) -> Self {
        Self {
            task_id,
            name,
    ...
    pub fn to_dict(&self) -> std::collections::HashMap<DepylerValue, DepylerValue> {
        return {
            ...
            map.insert("task_id".to_string(), self.task_id.clone());
            map.insert("name".to_string(), self.name.clone());
            map.insert("priority".to_string(), self.priority.clone());
            map.insert("data".to_string(), self.data.clone());
            map.insert("status".to_string(), self.status.clone().value);
            map.insert(
                "created_at".to_string(),
                self.created_at.clone().isoformat(),
            );
            map.insert(
                "started_at".to_string(),
                if self.started_at.clone() {
                    self.started_at.clone().isoformat()
                } else {
                    None
                },
            );
            map.insert(
                ...
                if self.completed_at.clone() {
                    self.completed_at.clone().isoformat()
                } else {
                    None
                },
            );
    ...
    pub fn from_dict(data: std::collections::HashMap<DepylerValue, DepylerValue>) -> Task {
        let mut task = Self::new();
        task.status = TaskStatus::new(data.get("status".to_string(), "pending".to_string()));
        if data.get("created_at".to_string()) {
            task.created_at = datetime.fromisoformat(
                data.get(&"created_at".to_string())
                    .cloned()
                    .unwrap_or_default(),
            );
        };
        if data.get("started_at".to_string()) {
            task.started_at = datetime.fromisoformat(
                data.get(&"started_at".to_string())
                    .cloned()
                    .unwrap_or_default(),
            );
        };
        if data.get("completed_at".to_string()) {
            task.completed_at = datetime.fromisoformat(
                data.get(&"completed_at".to_string())
                    .cloned()
                    .unwrap_or_default(),
            );
        };
        task.result = data.get("result".to_string());
        task.error = data.get("error".to_string());
        return task;
    }
}
#[derive(Debug, Clone)]
...
impl TaskQueue {
    ...
    pub fn add(
        ...
        let task_id = format!("task_{}", self.next_id.clone());
        self.next_id = self.next_id.clone() + 1;
        let task = Task::new(task_id, name, priority, data);
        self.tasks.clone().insert(task_id, task);
        return task;
    }
    pub fn get(&self, task_id: String) -> Option<Task> {
        return Some(self.tasks.clone().get(task_id));
    }
    ...
    pub fn start(&self, task_id: String) -> bool {
    ...
    pub fn complete(&self, task_id: String, result: Option<String>) -> bool {
        let mut task = self.get(task_id);
        if !task || task.status != TaskStatus::RUNNING {
            return false;
        };
        task.status = TaskStatus::COMPLETED;
        task.completed_at = std::time::SystemTime::now();
        task.result = result;
        return true;
    }
    pub fn fail(&self, task_id: String, error: String) -> bool {
        let mut task = self.get(task_id);
        if !task || task.status != TaskStatus::RUNNING {
            return false;
        };
    ...
    pub fn list_by_status(&self, status: Option<TaskStatus>) -> Vec<Task> {
        if status.is_none() {
            return self.tasks.clone().values().cloned().collect::<Vec<_>>();
        };
        return self
            .tasks
            .clone()
    ...
    pub fn count_by_status(&self) -> std::collections::HashMap<String, i32> {
        let mut counts = TaskStatus
            .into_iter()
            .map(|s| (s.value, 0))
            .collect::<std::collections::HashMap<_, _>>();
        for task in self.tasks.clone().values() {
            counts.insert(
                task.status.value,
                {
                    let _base = &counts;
                    let _idx = task.status.value;
                    if _idx < 0 {
                        _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                    } else {
                        _base[_idx as usize].clone()
    ...
    pub fn clear_completed(&self) -> i32 {
        let to_remove = self
            ...
            .filter(|t| {
                let t = t.clone();
                [TaskStatus::COMPLETED, TaskStatus::FAILED].contains(&t.status)
            })
            .map(|t| t.task_id)
            .collect::<Vec<_>>();
        ...
        return to_remove.len() as i32;
    }
    pub fn to_dict(&self) -> std::collections::HashMap<DepylerValue, DepylerValue> {
        return {
            let mut map = std::collections::HashMap::new();
            map.insert("next_id".to_string(), self.next_id.clone());
            map.insert(
                "tasks".to_string(),
                self.tasks
                    .clone()
    ...
    pub fn from_dict(data: std::collections::HashMap<DepylerValue, DepylerValue>) -> TaskQueue {
        let mut queue = Self::new();
        queue.next_id = data.get("next_id".to_string(), 1);
        for task_data in data.get("tasks".to_string(), vec![]) {
            let task = Task::from_dict(task_data);
            queue.tasks.insert(task.task_id, task);
        }
        return queue;
    }
}
...
enum Commands {
    ...
    Add {
        ...
        #[doc = "Task priority(higher = more urgent)"]
        priority: Option<i32>,
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        queue = Some(TaskQueue::from_dict(std::collections::HashMap::<
            String,
            DepylerValue,
        >::new()));
        Ok(())
    ...
    match &args.command {
        Some(Commands::Add {
            ...
            let task = queue.add(name, priority, data);
            save_queue();
            println!("{}", format!("Created {}: {}", task.task_id, task.name));
        }
        Some(Commands::List { ref status, .. }) => {
            ...
            let tasks = queue.list_by_status(status);
            if !tasks {
                println!("{}", "No tasks");
            } else {
                for task in tasks.iter().cloned() {
                    println!(
                        ...
                        format!(
                            "{}: {} [{}](priority: {})",
                            task.task_id, task.name, task.status.value, task.priority
                        )
                    );
                }
            }
        ...
        Some(Commands::Next { .. }) => {
            let task = queue.next_pending();
            if task {
                println!("{}", format!("{}: {}", task.task_id, task.name));
                println!("{}", format!("{:?}", task.to_dict()));
            } else {
                println!("{}", "No pending tasks");
            }
        }
        Some(Commands::Start { ref task_id, .. }) => {
            let task_id = task_id.to_string();
            if queue.start(task_id) {
                save_queue();
                println!("{}", format!("Started {}", task_id));
            } else {
                eprintln!("{}", format!("Cannot start {}", task_id));
        ...
        Some(Commands::Complete {
            ...
            let task_id = task_id.to_string();
            if queue.complete(task_id, result) {
                save_queue();
                println!("{}", format!("Completed {}", task_id));
            } else {
                eprintln!("{}", format!("Cannot complete {}", task_id));
                std::process::exit(1i32)
            }
        }
        Some(Commands::Fail {
            ...
            let task_id = task_id.to_string();
            if queue.fail(task_id, error) {
                save_queue();
                println!("{}", format!("Failed {}: {}", task_id, error));
            } else {
                eprintln!("{}", format!("Cannot fail {}", task_id));
                std::process::exit(1i32)
            }
        }
        Some(Commands::Status { .. }) => {
            let counts = queue.count_by_status();
            let _cse_temp_7 = counts.values().cloned().sum::<i32>();
            let total = _cse_temp_7;
            println!("{}", format!("Total: {} tasks", total));
            for (status, count) in counts
                .iter()
                .map(|(k, v)| (k.clone(), v.clone()))
                .collect::<Vec<_>>()
            {
                println!("{}", format!("  {}: {:?}", status, count));
            }
        }
        Some(Commands::Clear { .. }) => {
            let removed = queue.clear_completed();
            save_queue();
            println!("{}", format!("Removed {:?} tasks", removed));
        }
