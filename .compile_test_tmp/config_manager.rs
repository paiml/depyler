...
pub static DEFAULT_CONFIG: std::sync::LazyLock<
    std::collections::HashMap<DepylerValue, DepylerValue>,
> = std::sync::LazyLock::new(|| {
    let mut map = HashMap::new();
    map.insert(
        DepylerValue::Str("database".to_string().to_string()),
        DepylerValue::Str(format!("{:?}", {
            let mut map = HashMap::new();
            map.insert(
                DepylerValue::Str("host".to_string().to_string()),
                DepylerValue::Str("localhost".to_string().to_string()),
            );
            map.insert(
                DepylerValue::Str("port".to_string().to_string()),
                DepylerValue::Int(5432 as i64),
            );
            map
        })),
    );
    map.insert(
        DepylerValue::Str("logging".to_string().to_string()),
        DepylerValue::Str(format!("{:?}", {
            let mut map = HashMap::new();
            map.insert(
            ...
            map.insert(
                ...
                "app.log".to_string().to_string(),
            );
            map
        })),
    );
    map.insert(
        DepylerValue::Str("features".to_string().to_string()),
        DepylerValue::Str(format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("debug".to_string().to_string(), false);
            map.insert("verbose".to_string().to_string(), false);
            map
        })),
    );
    map
...
enum Commands {
    ...
    Get {
        #[doc = "Config key using dot notation(e.g., database.host)"]
        key: String,
    },
    ...
    Set {
        #[doc = "Config key using dot notation"]
        key: String,
        #[doc = "Config value to set"]
        value: String,
    },
...
struct Args {
    ...
    #[doc = "Path to config file(default: config.json)"]
    config: String,
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn save_config<'a, 'b>(path: &'a str, config: &'b DepylerValue) -> Result<(), std::io::Error> {
...
pub fn get_nested_value<'a, 'b>(
    ...
    for k in keys.iter().cloned() {
        if (true) && (value.contains(&*k)) {
            value = value.get(&k).cloned().unwrap_or_default();
        } else {
            return Ok(None);
        }
    }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Init { .. }));
    match &args.command {
        Some(Commands::Init { .. }) => {
    ...
    let _cse_temp_1 = matches!(args.command, Some(Commands::List { .. }));
    match &args.command {
        Some(Commands::List { .. }) => {
            println!("{}", format!("{:?}", config));
        }
        Some(Commands::Get { ref key, .. }) => {
        ...
        Some(Commands::Set {
            ...
            save_config(args.config.clone(), &config);
            println!("{}", format!("Set {} = {}", key, value));
        }
