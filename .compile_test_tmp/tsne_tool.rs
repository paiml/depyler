...
#[doc = "Calculate Euclidean distance."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn euclidean_distance<'b, 'a>(a: &'a Vec<f64>, b: &'b Vec<f64>) -> f64 {
    ((0..(a.len() as i32))
        .into_iter()
        .map(|i| {
...
pub fn compute_pairwise_distances(
    ...
    for i in 0..(n) {
        for j in (i + 1)..(n) {
            ...
            D.get_mut(&i).unwrap().insert((j) as usize, d);
            D.get_mut(&j).unwrap().insert((i) as usize, d);
        }
    }
    Ok(D)
}
#[doc = "Compute joint probabilities P_ij using Gaussian kernel."]
#[doc = " Depyler: proven to terminate"]
pub fn compute_p_ij(
    D: &Vec<Vec<f64>>,
    perplexity: f64,
    ...
    let P = (0..(n))
        ...
        .map(|_| vec![0.0; n as usize])
        .collect::<Vec<_>>();
    let _cse_temp_1 = ((perplexity) as f64) / ((3.0) as f64);
    let sigma = _cse_temp_1;
    for i in 0..(n) {
        for j in 0..(n) {
            if i != j {
                P.get_mut(&i).unwrap().insert(
                    (j) as usize,
                    (((-{
                        if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                            ({
                        ...
                        / ((2f64 * ({ sigma } as f64).powf({ 2 } as f64)) as f64) as f64)
                        .exp(),
                );
            }
        }
    ...
    for i in 0..(n) {
        for j in (i + 1)..(n) {
            let avg = ((P
                ...
                / ((2) as f64);
            P.get_mut(&i).unwrap().insert((j) as usize, avg);
            P.get_mut(&j).unwrap().insert((i) as usize, avg);
        }
    }
    Ok(P)
}
#[doc = "Compute Q_ij using Student t-distribution."]
#[doc = " Depyler: proven to terminate"]
pub fn compute_q_ij(Y: &Vec<Vec<f64>>) -> Result<Vec<Vec<f64>>, Box<dyn std::error::Error>> {
        ...
        for j in 0..(n) {
            if i != j {
                let d_sq = (0..(Y
                    ...
                    .sum::<i32>();
                Q.get_mut(&i).unwrap().insert(
                    (j) as usize,
                    ((1.0) as f64) / ((1.0 + (d_sq as f64)) as f64),
                );
            }
        }
    }
...
#[doc = "Fit t-SNE and transform data."]
#[doc = " Depyler: proven to terminate"]
pub fn fit_transform<'b, 'a>(
    X: &'a Vec<Vec<f64>>,
    n_components: i32,
    mut perplexity: f64,
    n_iter: f64,
    ...
    let _cse_temp_2 = ((n_samples - 1) as f64) / ((3) as f64);
    let _cse_temp_3 = depyler_min((perplexity).clone(), (_cse_temp_2).clone());
    perplexity = _cse_temp_3;
    let _cse_temp_4 = perplexity < 1;
    if _cse_temp_4 {
        perplexity = 1;
    }
    ...
    let Y = (0..(n_samples))
        ...
        .collect::<Vec<_>>();
    let D = compute_pairwise_distances(&&X)?;
    let P = compute_p_ij(&&D, perplexity)?;
    for iteration in 0..(n_iter) {
        let Q = compute_q_ij(&&Y)?;
        let grad = (0..(n_samples))
            .into_iter()
            .map(|_| vec![0.0; n_components as usize])
            .collect::<Vec<_>>();
            ...
            for j in 0..(n_samples) {
                if i != j {
                    ...
                    let d_sq = (0..(n_components))
                        ...
                        .sum::<i32>();
                    let factor = ((4.0 * pq_diff) as f64) / ((1.0 + (d_sq as f64)) as f64);
                    for k in 0..(n_components) {
                        grad.get_mut(&i).unwrap().insert(
                            k.to_string().clone(),
                            grad.get(i as usize)
                                .cloned()
                                .expect("IndexError: list index out of range")
        ...
        let lr = learning_rate * (1.0 - ((iteration) as f64) / ((n_iter) as f64));
        for i in 0..(n_samples) {
            for k in 0..(n_components) {
                Y.get_mut(&i).unwrap().insert(
                    k.to_string().clone(),
                    Y.get(i as usize)
                        .cloned()
                        .expect("IndexError: list index out of range")
...
#[doc = "Handle fit-transform subcommand."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_fit_transform() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_0 {
        eprintln!("{}", "Error: Missing 'X'");
        std::process::exit(1);
    }
    let n_components = data.get("n_components").cloned().unwrap_or(2);
    let perplexity = data.get("perplexity").cloned().unwrap_or(30.0);
    let n_iter = data.get("n_iter").cloned().unwrap_or(250);
    let learning_rate = data.get("learning_rate").cloned().unwrap_or(200.0);
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!(
            ...
            format!("{:?}", {
                ...
                map.insert("X_embedded".to_string().to_string(), Y);
                map
            })
        );
        Ok(())
...
mod tests {
    ...
    #[test]
    fn test_compute_pairwise_distances_examples() {
        assert_eq!(compute_pairwise_distances(vec![]), vec![]);
        assert_eq!(compute_pairwise_distances(vec![1]), vec![1]);
    }
    #[test]
    fn test_compute_q_ij_examples() {
        assert_eq!(compute_q_ij(vec![]), vec![]);
        assert_eq!(compute_q_ij(vec![1]), vec![1]);
    }
}
