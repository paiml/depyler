...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Both { .. }));
    match &args.command {
        Some(Commands::Both { ref text, .. }) => {
            println!(
                "{}",
                text.trim_matches(|c: char| "_".to_string().contains(c))
                    .to_string()
            );
        }
        Some(Commands::Left { ref text, .. }) => {
            println!(
                "{}",
                text.trim_start_matches(|c: char| "_".to_string().contains(c))
                    .to_string()
            );
        }
        Some(Commands::Right { ref text, .. }) => {
            println!(
                "{}",
                text.trim_end_matches(|c: char| "_".to_string().contains(c))
                    .to_string()
            );
        }
