...
pub static DEFAULT_KEYWORDS: std::sync::LazyLock<std::collections::HashSet<DepylerValue>> =
    std::sync::LazyLock::new(|| {
        ...
        set.insert("break".to_string());
        set.insert("continue".to_string());
        set.insert("fn".to_string());
        ...
        set.insert("struct".to_string());
        set.insert("enum".to_string());
        set.insert("import".to_string());
        set.insert("export".to_string());
        set
    });
...
impl TokType {
    ...
    pub const CHAR: i32 = auto();
    pub const BOOL: i32 = auto();
    pub const IDENTIFIER: i32 = auto();
    pub const KEYWORD: i32 = auto();
    pub const PLUS: i32 = auto();
    pub const MINUS: i32 = auto();
    pub const STAR: i32 = auto();
    pub const SLASH: i32 = auto();
    pub const PERCENT: i32 = auto();
    pub const CARET: i32 = auto();
    pub const AMPERSAND: i32 = auto();
    pub const PIPE: i32 = auto();
    pub const TILDE: i32 = auto();
    pub const BANG: i32 = auto();
    pub const QUESTION: i32 = auto();
    pub const EQ: i32 = auto();
    pub const NE: i32 = auto();
    pub const LT: i32 = auto();
    pub const LE: i32 = auto();
    pub const GT: i32 = auto();
    pub const GE: i32 = auto();
    pub const ASSIGN: i32 = auto();
    pub const PLUS_ASSIGN: i32 = auto();
    pub const MINUS_ASSIGN: i32 = auto();
    pub const STAR_ASSIGN: i32 = auto();
    pub const SLASH_ASSIGN: i32 = auto();
    pub const LPAREN: i32 = auto();
    pub const RPAREN: i32 = auto();
    pub const LBRACKET: i32 = auto();
    pub const RBRACKET: i32 = auto();
    pub const LBRACE: i32 = auto();
    pub const RBRACE: i32 = auto();
    pub const COMMA: i32 = auto();
    pub const DOT: i32 = auto();
    pub const COLON: i32 = auto();
    pub const SEMICOLON: i32 = auto();
    pub const ARROW: i32 = auto();
    pub const NEWLINE: i32 = auto();
    pub const COMMENT: i32 = auto();
    pub const WHITESPACE: i32 = auto();
    pub const EOF: i32 = auto();
    pub const ERROR: i32 = auto();
    pub fn new() -> Self {
        Self {}
    }
}
...
impl Token {
    pub fn new(type_: TokType, value: DepylerValue, line: i32, column: i32, length: i32) -> Self {
        Self {
            type_,
            value,
            line,
...
impl Lexer {
    pub fn new(source: String, config: Option<LexerConfig>) -> Self {
        Self {
            source,
            config,
            pos: 0,
    ...
    pub fn scan_token(&mut self) {
        ...
        if " \t\r".to_string().contains(&*char) {
            if !self.config.clone().skip_whitespace {
                self.add_token(TokType::WHITESPACE, char, start_line, start_col, 1);
            };
            return ();
        };
        if char == "\n".to_string() {
            self.line = self.line.clone() + 1;
            self.column = 1;
            if !self.config.clone().skip_whitespace {
                self.add_token(TokType::NEWLINE, "\n".to_string(), start_line, start_col, 1);
            };
            return ();
        };
        if char == "/".to_string() && self.peek() == "/".to_string() {
            self.advance();
            let comment = self.read_until("\n".to_string());
            if !self.config.clone().skip_comments {
                self.add_token(
                    TokType::COMMENT,
                    comment,
                    start_line,
                    start_col,
                    (comment.len() as i32) + 2,
                );
            };
            return ();
        };
        if char == "/".to_string() && self.peek() == "*".to_string() {
            ...
            if !self.config.clone().skip_comments {
                self.add_token(
                    ...
                    start_line,
                    start_col,
                    (comment.len() as i32) + 4,
                );
            };
            return ();
        };
        if self.config.clone().string_delimiters.contains_key(&char) {
            let string = self.read_string(char);
            self.add_token(
                TokType::STRING,
                string,
                start_line,
        ...
        if !char.is_empty() && char.chars().all(|c| c.is_ascii_digit()) {
            let (number, is_float) = self.read_number(char);
            let tok_type = if is_float {
                TokType::FLOAT
            } else {
                TokType::INTEGER
            };
            self.add_token(
                ...
                start_line,
                start_col,
                self.pos.clone() - start_pos,
            );
            return ();
        };
        if !char.is_empty() && char.chars().all(|c| c.is_alphabetic()) || char == "_".to_string() {
            let identifier = self.read_identifier(char);
            if self.config.clone().keywords.contains_key(&identifier) {
                self.add_token(
                    TokType::KEYWORD,
                    identifier,
                    start_line,
                    start_col,
                    identifier.len() as i32,
                );
            } else {
                if ["true".to_string(), "false".to_string()].contains(&identifier) {
                    self.add_token(
                        TokType::BOOL,
        ...
        let token = self.scan_operator(char, start_line, start_col);
        if token {
    ...
    pub fn scan_operator(&self, char: String, line: i32, col: i32) -> Option<Token> {
        let two_char = if !self.is_at_end() {
            char + self.peek()
        } else {
            char
        };
        let two_char_ops = {
            ...
            map.insert("==".to_string(), TokType::EQ);
            map.insert("!=".to_string(), TokType::NE);
            map.insert("<=".to_string(), TokType::LE);
            map.insert(">=".to_string(), TokType::GE);
            map.insert("+=".to_string(), TokType::PLUS_ASSIGN);
            map.insert("-=".to_string(), TokType::MINUS_ASSIGN);
            map.insert("*=".to_string(), TokType::STAR_ASSIGN);
            map.insert("/=".to_string(), TokType::SLASH_ASSIGN);
            map.insert(" -> ".to_string(), TokType::ARROW);
            map
        };
        if two_char_ops.contains_key(&two_char) {
            self.advance();
            return Some(Token::new(
                {
                    let _base = &two_char_ops;
                    let _idx = two_char;
                    if _idx < 0 {
        ...
        let single_char_ops = {
            let mut map = std::collections::HashMap::new();
            map.insert("+".to_string(), TokType::PLUS);
            map.insert("-".to_string(), TokType::MINUS);
            map.insert("*".to_string(), TokType::STAR);
            map.insert("/".to_string(), TokType::SLASH);
            map.insert("%".to_string(), TokType::PERCENT);
            map.insert("^".to_string(), TokType::CARET);
            map.insert("&".to_string(), TokType::AMPERSAND);
            map.insert("|".to_string(), TokType::PIPE);
            map.insert("~".to_string(), TokType::TILDE);
            map.insert("!".to_string(), TokType::BANG);
            map.insert("?".to_string(), TokType::QUESTION);
            map.insert("<".to_string(), TokType::LT);
            map.insert(">".to_string(), TokType::GT);
            map.insert("=".to_string(), TokType::ASSIGN);
            map.insert("(".to_string(), TokType::LPAREN);
            map.insert(")".to_string(), TokType::RPAREN);
            map.insert("[".to_string(), TokType::LBRACKET);
            map.insert("]".to_string(), TokType::RBRACKET);
            map.insert("{".to_string(), TokType::LBRACE);
            map.insert("}".to_string(), TokType::RBRACE);
            map.insert(",".to_string(), TokType::COMMA);
            map.insert(".".to_string(), TokType::DOT);
            map.insert(":".to_string(), TokType::COLON);
            map.insert(";".to_string(), TokType::SEMICOLON);
            map
        };
        if single_char_ops.contains_key(&char) {
            return Some(Token::new(
                {
                    let _base = &single_char_ops;
                    let _idx = char;
                    if _idx < 0 {
                        _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                    } else {
    ...
    pub fn read_string(&self, quote: String) -> String {
        let mut chars = vec![];
        while !self.is_at_end() && self.peek() != quote {
            ...
            if char == "\\".to_string() {
                ...
                let escape_map = {
                    ...
                    map.insert("t".to_string(), "\t".to_string());
                ...
                chars.push(escape_map.get(escaped, escaped));
            } else {
                chars.push(char);
            };
        }
    ...
    pub fn read_number(&self, first: String) -> (DepylerValue, bool) {
        let mut chars = vec![first];
        let mut is_float = false;
        while !self.is_at_end()
            && !self.peek().is_empty()
            && self.peek().chars().all(|c| c.is_ascii_digit())
            || self.peek() == ".".to_string()
        {
        ...
        if !self.is_at_end() && self.peek().to_lowercase() == "e".to_string() {
            chars.push(self.advance());
            if !self.is_at_end() && "+-".to_string().contains(&*self.peek()) {
                chars.push(self.advance());
            };
            while !self.is_at_end()
                && !self.peek().is_empty()
        ...
        let value = chars.join("".to_string());
        return if is_float {
            (value.parse::<f64>().unwrap_or(0.0), true)
        } else {
            (value.parse::<i32>().unwrap_or(0), false)
        };
    }
    pub fn read_identifier(&self, first: String) -> String {
        let mut chars = vec![first];
        while !self.is_at_end()
            && !self.peek().is_empty()
            && self.peek().chars().all(|c| c.is_alphanumeric())
            || self.peek() == "_".to_string()
        {
            chars.push(self.advance());
        }
        return chars.join("".to_string());
    }
    pub fn read_until(&self, delimiter: String) -> String {
        let mut chars = vec![];
    ...
    pub fn read_multi_line_comment(&mut self) -> String {
        let mut chars = vec![];
        while !self.is_at_end() {
            if self.peek() == "*".to_string()
                && self.pos.clone() + 1 < (self.source.clone().len() as i32)
                && {
                    let _base = &self.source.clone();
    ...
    pub fn advance(&mut self) -> String {
        ...
        self.column = self.column.clone() + 1;
        return char;
    }
    ...
    pub fn is_at_end(&self) -> bool {
        return self.pos.clone() >= (self.source.clone().len() as i32);
    }
    pub fn add_token(
        &mut self,
        r#type: &TokType,
        ...
        self.tokens
            .push(Token::new(r#type, value, line, col, length));
    }
}
...
#[doc = "Get summary of token types."]
#[doc = " Depyler: verified panic-free"]
pub fn token_summary(tokens: &Vec<Token>) -> HashMap<String, i32> {
    let mut summary: std::collections::HashMap<String, i32> = {
        let map: HashMap<String, i32> = HashMap::new();
        map
    };
    for token in tokens.iter().cloned() {
        let name = token.r#type.name;
        summary.insert(
            name.to_string().clone(),
            summary.get(&name).cloned().unwrap_or(0) + 1,
        );
    }
    summary
}
#[doc = "Format tokens as string."]
#[doc = " Depyler: verified panic-free"]
pub fn format_tokens(tokens: &Vec<Token>) -> String {
    let mut lines = vec![];
    for token in tokens.iter().cloned() {
        if token.r#type == TokType::EOF {
            continue;
        }
        lines.push(DepylerValue::Str(format!(
            ...
            format!(
                "{}:{} {} {}",
                token.line, token.column, token.r#type.name, token.value
            )
        )));
    }
    lines.join("\n")
}
#[doc = "Simulate lexer operations."]
pub fn simulate_lexer(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results = vec![];
        ...
        if cmd == "tokenize" {
            ...
            if cmd == "count" {
                let count = tokens
                    ...
                    .map(|t| 1)
                    .sum::<i32>();
                results.push(DepylerValue::Str(format!("{:?}", (count).to_string())));
            } else {
                if cmd == "types" {
                    let types = tokens
                        .iter()
                        ...
                        .filter(|t| {
                            let t = t.clone();
                            t.r#type != TokType::EOF
                        })
                        .map(|t| t.r#type.name)
                        .collect::<Vec<_>>();
                    results.push(DepylerValue::Str(format!("{:?}", types.join(","))));
                } else {
                    if cmd == "values" {
                        let values = tokens
                            .iter()
                            ...
                            .collect::<Vec<_>>();
                        results.push(DepylerValue::Str(format!("{:?}", values.join(","))));
                    } else {
                        if cmd == "summary" {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: state_lexer_cli.py <command>[args...]");
        println!("{}", "Commands: tokenize, summary, format");
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "tokenize";
    let mut source: String;
    let mut tokens;
    if _cse_temp_2 {
        source = if (std::env::args().collect::<Vec<String>>().len() as i32) < 3 {
            {
                use std::io::Read;
        ...
        for token in tokens.iter().cloned() {
            if token.r#type != TokType::EOF {
                println!("{}", format!("{}: {}", token.r#type.name, token.value));
            }
        }
    } else {
        let _cse_temp_3 = cmd == "summary";
        if _cse_temp_3 {
            source = if (std::env::args().collect::<Vec<String>>().len() as i32) < 3 {
                {
                    use std::io::Read;
            ...
            for (name, count) in {
                ...
                println!("{}", format!("{:?}: {:?}", name, count));
            }
        } else {
            let _cse_temp_4 = cmd == "format";
            if _cse_temp_4 {
                source = if (std::env::args().collect::<Vec<String>>().len() as i32) < 3 {
                    {
                        use std::io::Read;
...
mod tests {
    ...
    #[test]
    fn test_simulate_lexer_examples() {
        assert_eq!(simulate_lexer(vec![]), vec![]);
        assert_eq!(simulate_lexer(vec![1]), vec![1]);
    }
}
