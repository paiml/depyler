...
struct Args {
    ...
    #[doc = "Mean for transform"]
    mean: f64,
    
    
    #[doc = "Std for transform"]
    std: f64,
    
    
    #[doc = "Value to transform"]
    value: f64,
}
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let x2 = args.x2;
    let x3 = args.x3;
    let _cse_temp_0 = args.mode == "fit";
    let mut std;
    let mut mean;
    if _cse_temp_0 {
        let _cse_temp_1 = x0 + x1 + x2;
        let _cse_temp_2 = ((_cse_temp_1 + x3) as f64) / ((4.0) as f64);
        mean = _cse_temp_2;
        let diff0 = x0 - mean;
        let diff1 = x1 - mean;
        let diff2 = x2 - mean;
        let diff3 = x3 - mean;
        let _cse_temp_3 = diff0 * diff0;
        let _cse_temp_4 = diff1 * diff1;
        let _cse_temp_5 = diff2 * diff2;
        let _cse_temp_6 = _cse_temp_3 + _cse_temp_4 + _cse_temp_5;
        let _cse_temp_7 = diff3 * diff3;
        let _cse_temp_8 = ((_cse_temp_6 + _cse_temp_7) as f64) / ((4.0) as f64);
        let var = _cse_temp_8;
        let _cse_temp_9 = ((var) as f64) / ((2.0) as f64);
        std = _cse_temp_9;
        let _cse_temp_10 = var > 0.0;
        if _cse_temp_10 {
            let mut i = 0.0;
            while i < 10.0 {
                std = ((std + ((var) as f64) / ((std) as f64)) as f64) / ((2.0) as f64);
                i = i + 1.0;
            }
        } else {
            std = 0.0;
        }
        println!("{}", format!("mean={} std={}", mean, std));
    } else {
        let _cse_temp_11 = args.mode == "transform";
        if _cse_temp_11 {
            mean = args.mean;
            std = args.std;
            let value = args.value;
            let _cse_temp_12 = std > 0.0;
            let mut scaled: f64;
            if _cse_temp_12 {
                let _cse_temp_13 = ((value - mean) as f64) / ((std) as f64);
                scaled = _cse_temp_13;
            } else {
                scaled = 0.0;
            }
            println!("{}", format!("scaled={}", scaled));
        }
    }
    Ok(())
}
