...
impl UnionFind {
    ...
    pub fn union(&mut self, x: i32, y: i32) -> bool {
        let mut root_x = self.find(x).map(|i| i as i64).unwrap_or(-1);
        let mut root_y = self.find(y).map(|i| i as i64).unwrap_or(-1);
        if root_x == root_y {
            return false;
        };
        if {
            let _base = &self._rank.clone();
            let _idx = root_x;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
            ...
            let (root_x, root_y) = (root_y, root_x);
        };
        self._parent.clone().insert(root_y, root_x);
        self._size.clone().insert(
            root_x,
            {
                let _base = &self._size.clone();
                let _idx = root_x;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
    ...
    pub fn connected(&self, x: i32, y: i32) -> bool {
        return self.find(x).map(|i| i as i64).unwrap_or(-1)
            == self.find(y).map(|i| i as i64).unwrap_or(-1);
    }
    pub fn component_count(&self) -> i32 {
        return self._count.clone();
    }
    pub fn component_size(&self, x: i32) -> i32 {
        return {
            let _base = &self._size.clone();
            let _idx = self.find(x).map(|i| i as i64).unwrap_or(-1);
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
                _base[_idx as usize].clone()
    ...
    pub fn get_components(&self) -> Vec<Vec<i32>> {
        let mut components = {
            let mut map = std::collections::HashMap::new();
            map
        };
        for i in 0..self._parent.clone().len() as i32 {
            let root = self.find(i).map(|i| i as i64).unwrap_or(-1);
            if !components.contains_key(&root) {
                components.insert(root, vec![]);
            };
            {
                let _base = &components;
                let _idx = root;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
        ...
        return components.values().cloned().collect::<Vec<_>>();
    }
}
#[derive(Debug, Clone)]
...
impl WeightedUnionFind {
    ...
    pub fn find(&self, x: i32) -> (i32, i32) {
        if {
            let _base = &self._parent.clone();
            let _idx = x;
            if _idx < 0 {
        ...
        let (root, w) = self
            ...
            .map(|i| i as i64)
            .unwrap_or(-1);
        self._parent.clone().insert(x, root);
        self._weight.clone().insert(
            x,
            {
                let _base = &self._weight.clone();
                let _idx = x;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
                } else {
    ...
    pub fn union(&self, x: i32, y: i32, w: i32) -> bool {
        let (root_x, wx) = self.find(x).map(|i| i as i64).unwrap_or(-1);
        let (root_y, wy) = self.find(y).map(|i| i as i64).unwrap_or(-1);
        if root_x == root_y {
            return false;
        };
        if {
            ...
            self._parent.clone().insert(root_x, root_y);
            self._weight.clone().insert(root_x, wy - wx - w);
        } else {
            self._parent.clone().insert(root_y, root_x);
            self._weight.clone().insert(root_y, wx - wy + w);
            if {
                let _base = &self._rank.clone();
                let _idx = root_x;
                if _idx < 0 {
    ...
    pub fn diff(&self, x: i32, y: i32) -> Option<i32> {
        let (root_x, wx) = self.find(x).map(|i| i as i64).unwrap_or(-1);
        let (root_y, wy) = self.find(y).map(|i| i as i64).unwrap_or(-1);
        if root_x != root_y {
            return None;
...
#[doc = "Kruskal's MST algorithm using Union-Find."]
#[doc = " Depyler: verified panic-free"]
pub fn kruskal_mst(n: i32, edges: &Vec<(i32, i32, i32)>) -> Vec<(i32, i32, i32)> {
    let mut uf = UnionFind::new(n);
    let sorted_edges = {
        let mut __sorted_result = edges.clone();
        __sorted_result.sort_by_key(|e| {
            e.get(2usize)
                .cloned()
    ...
    let mut mst: Vec<(i32, i32, i32)> = vec![];
    for (u, v, w) in sorted_edges.iter().cloned() {
        if !uf.connected(u, v) {
            uf.union(u, v);
            mst.push((u, v, w));
            if mst.len() as i32 == n - 1 {
                break;
            }
        }
    }
...
#[doc = "Count connected components in grid(1 = land, 0 = water)."]
#[doc = " Depyler: proven to terminate"]
pub fn count_islands(grid: &Vec<Vec<i32>>) -> Result<i32, Box<dyn std::error::Error>> {
    let _cse_temp_0 = (grid.is_empty())
        || (!grid
            .get(0usize)
            .cloned()
            .expect("IndexError: list index out of range"));
    ...
    let index = move |r: i32, c: i32| -> i32 {
        return r * cols + c;
    };
    for r in 0..(rows) {
        for c in 0..(cols) {
    ...
    let roots: std::collections::HashSet<i32> = std::collections::HashSet::<i32>::new();
    for r in 0..(rows) {
        for c in 0..(cols) {
            if grid
                .get(r as usize)
                ...
                == 1
            {
                roots.insert(uf.find(&index(r, c)).map(|i| i as i32).unwrap_or(-1));
            }
        }
    }
    Ok(roots.len() as i32 as i32)
}
#[doc = "Check if graph is connected."]
#[doc = " Depyler: verified panic-free"]
pub fn is_connected_graph(n: i32, edges: &Vec<(i32, i32)>) -> bool {
    let mut uf = UnionFind::new(n);
    for (u, v) in edges.iter().cloned() {
        uf.union(u, v);
    }
    uf.component_count() == 1
}
#[doc = "Find edge that creates cycle(redundant connection)."]
#[doc = " Depyler: verified panic-free"]
pub fn redundant_connection(n: i32, edges: &Vec<(i32, i32)>) -> Option<(i32, i32)> {
    let mut uf = UnionFind::new(n);
    for (u, v) in edges.iter().cloned() {
        if uf.connected(u, v) {
            return Some((u, v));
...
#[doc = "Find earliest time when all nodes are connected."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn earliest_connection(n: i32, edges: &Vec<(i32, i32)>) -> i32 {
    let mut uf = UnionFind::new(n);
    for (i, _nested) in edges
        .iter()
        .cloned()
        .enumerate()
        .map(|(i, x)| (i as i32, x))
...
#[doc = "Find size of largest connected component."]
#[doc = " Depyler: verified panic-free"]
pub fn largest_component(n: i32, edges: &Vec<(i32, i32)>) -> i32 {
    ...
    max_size = 0;
    for i in 0..(n) {
        max_size = depyler_max((max_size).clone(), (uf.component_size(i)).clone());
    }
    max_size
}
#[doc = "Simulate Union-Find operations."]
pub fn simulate_uf(n: i32, ops: &Vec<String>) -> Result<Vec<i32>, Box<dyn std::error::Error>> {
    let mut uf = UnionFind::new(n);
        ...
        if cmd == "union" {
            ...
            if cmd == "find" {
                x = (parts
                    ...
                    .cloned()
                    .expect("IndexError: list index out of range")) as i32;
                results.push(uf.find(&x).map(|i| i as i32).unwrap_or(-1));
            } else {
                if cmd == "connected" {
                    (x, y) = (
                        (parts
                    ...
                    results.push(if uf.connected(x, y) { 1 } else { 0 });
                } else {
                    if cmd == "count" {
                        results.push(uf.component_count());
                    } else {
                        if cmd == "size" {
                            x = (parts
                                .get(1usize)
                                ...
                                .expect("IndexError: list index out of range"))
                                as i32;
                            results.push(uf.component_size(x));
                        }
                    }
                }
            }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Ops { .. }));
    match &args.command {
        Some(Commands::Ops { ref n, ref ops, .. }) => {
            let n = *n;
            let results = simulate_uf(n, &ops)?;
        ...
        Some(Commands::Mst {
            ...
            let mut edges = vec![];
            for e in edges {
                let mut parts = e.split(":").map(|s| s.to_string()).collect::<Vec<String>>();
                let verts = parts
                    .get(0usize)
                    .cloned()
                    .expect("IndexError: list index out of range")
                ...
                let w = (parts
                    ...
                    .cloned()
                    .expect("IndexError: list index out of range")) as i32;
                edges.push(DepylerValue::Str(format!("{:?}", (u, v, w))));
            }
            let mst = kruskal_mst(n, &edges);
            let _cse_temp_2 = mst.iter().cloned().map(|(_, _, w)| w).sum::<i32>();
            let total = _cse_temp_2;
            println!("{}", format!("MST weight: {}", total));
        }
        Some(Commands::Connected {
            ...
            for e in edges {
                let mut parts = e.split("-").map(|s| s.to_string()).collect::<Vec<String>>();
                edges.push(DepylerValue::Str(format!(
                    "{:?}",
                    (
                        (parts
                            .get(0usize)
            ...
            let connected = is_connected_graph(n, &edges);
            println!("{}", format!("Connected: {:?}", connected));
        }
...
mod tests {
    ...
    #[test]
    fn test_count_islands_examples() {
        assert_eq!(count_islands(&vec![]), 0);
        assert_eq!(count_islands(&vec![1]), 1);
        assert_eq!(count_islands(&vec![1, 2, 3]), 3);
    }
}
