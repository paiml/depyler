...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_0 {
        ...
        let x2 = args.x2;
        let y2 = args.y2;
        let _cse_temp_1 = x1 == x2;
        if _cse_temp_1 {
            coef = 0.0;
            intercept = y1;
        } else {
            let _cse_temp_2 = (y2 - y1) / (x2 - x1);
            coef = _cse_temp_2;
            let _cse_temp_3 = coef * x1;
            intercept = y1 - _cse_temp_3;
        }
        println!("{}", format!("coef={} intercept={}", coef, intercept));
    } else {
        let _cse_temp_4 = args.mode == "predict";
        if _cse_temp_4 {
            let x = args.x;
            coef = args.coef;
            intercept = args.intercept;
            let _cse_temp_5 = coef * x;
            let y = _cse_temp_5 + intercept;
            println!("{}", format!("prediction={}", y));
        }
    }
    Ok(())
}
