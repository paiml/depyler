...
enum Commands {
    #[default]
    __DepylerNone,
    Check { typename: String, value: String },
}
#[derive(Default)]
...
pub fn main() {
    ...
    match &args.command {
        Some(Commands::Check { ref typename, .. }) => {
            let typename = typename.to_string();
            let _cse_temp_1 = typename == "int";
            if _cse_temp_1 {
                println!("{}", "valid int");
            } else {
                let _cse_temp_2 = typename == "float";
                if _cse_temp_2 {
                    println!("{}", "valid float");
                } else {
                    let _cse_temp_3 = typename == "str";
                    if _cse_temp_3 {
                        println!("{}", "valid str");
                    }
                }
