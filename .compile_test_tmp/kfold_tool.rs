...
#[doc = "Generate K-fold train/test splits."]
pub fn kfold_split(
    n_samples: i32,
    n_splits: i32,
) -> Result<Vec<HashMap<String, Vec<i32>>>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = n_splits > n_samples;
    if _cse_temp_0 {
    ...
    let mut fold_sizes = _cse_temp_1.clone();
    for i in 0..(n_samples % n_splits) {
        {
            let _key = i;
            let _old_val = fold_sizes.get(&_key).cloned().unwrap_or_default();
            fold_sizes.insert(_key, _old_val + 1);
        }
    }
    let indices = (0..(n_samples)).collect::<Vec<_>>();
    let mut folds = vec![];
    let mut current = 0;
    for fold_size in fold_sizes.iter().cloned() {
        let test_idx = {
            let base = &indices;
            let start_idx = (current) as isize;
            let stop_idx = (current + fold_size) as isize;
        ...
        .chain(
            {
                let base = &indices;
                let start_idx = (current + fold_size) as isize;
                let start = if start_idx < 0 {
                    (base.len() as isize + start_idx).max(0) as usize
                } else {
                    start_idx as usize
        ...
        .cloned()
        .collect::<Vec<_>>();
        folds.push(DepylerValue::Str(format!("{:?}", {
            let mut map = HashMap::new();
            map.insert("train".to_string().to_string(), train_idx);
            map.insert("test".to_string().to_string(), test_idx);
            map
        })));
...
#[doc = "Simple linear regression fit."]
#[doc = " Depyler: proven to terminate"]
pub fn linear_regression_fit<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<f64>,
    ...
    let _cse_temp_2 = X
        ...
        .len() as i32;
    let n_features = _cse_temp_2;
    let _cse_temp_3 = n_features == 1;
    if _cse_temp_3 {
        let _cse_temp_4 = (0..(n))
            .into_iter()
            .map(|i| {
            ...
            .sum::<i32>();
        let sum_x = _cse_temp_4;
        let _cse_temp_5 = y.iter().sum::<i32>();
        let sum_y = _cse_temp_5;
        let sum_xy = _cse_temp_4;
        let sum_xx = _cse_temp_4;
        let _cse_temp_6 = n * sum_xx;
        let _cse_temp_7 = {
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ...
                ({ sum_x } as f64).powf({ 2 } as f64) as i32
            }
        };
        let denom = _cse_temp_6 - _cse_temp_7;
        let _cse_temp_8 = (denom).abs();
        let _cse_temp_9 = ((_cse_temp_8 as f64) as f64) < 0.0000000001;
        if _cse_temp_9 {
            return Ok((
                vec![0.0],
                if n > 0 {
                    ((sum_y) as f64) / ((n) as f64)
                } else {
                    0.0
                },
            ));
        }
        let _cse_temp_10 = n * sum_xy;
        let _cse_temp_11 = sum_x * sum_y;
        let _cse_temp_12 = ((_cse_temp_10 - _cse_temp_11) as f64) / ((denom) as f64);
        let coef = _cse_temp_12;
        let _cse_temp_13 = coef * sum_x;
        let _cse_temp_14 = ((sum_y - _cse_temp_13) as f64) / ((n) as f64);
        let intercept = _cse_temp_14;
        return Ok((vec![coef], intercept));
    }
    Ok((vec![0.0; n_features as usize], 0.0))
}
#[doc = "Linear regression predict."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn linear_regression_predict<'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    coef: &'b Vec<f64>,
    intercept: f64,
) -> Vec<f64> {
...
#[doc = "Calculate R² score."]
#[doc = " Depyler: proven to terminate"]
pub fn r2_score<'a, 'b>(
    y_true: &'a Vec<f64>,
    y_pred: &'b Vec<f64>,
) -> Result<f64, Box<dyn std::error::Error>> {
    let _cse_temp_0 = y_true.len() as i32;
    let _cse_temp_1 = _cse_temp_0 == 0;
    if _cse_temp_1 {
        return Ok(0.0);
    }
    let _cse_temp_2 = y_true.iter().sum::<f64>();
    let _cse_temp_3 = ((_cse_temp_2) as f64) / ((_cse_temp_0) as f64);
    let y_mean = _cse_temp_3;
    let _cse_temp_4 = (0..(y_true.len() as i32))
        .into_iter()
        .map(|i| {
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ({
        ...
        .sum::<f64>();
    let ss_res = _cse_temp_4;
    let ss_tot = _cse_temp_4;
    let _cse_temp_5 = ss_tot == 0;
    if _cse_temp_5 {
        return Ok(if ss_res == 0 { 1.0 } else { 0.0 });
    }
    Ok(1.0 - ((ss_res) as f64) / ((ss_tot) as f64))
}
#[doc = "Perform cross-validation and return scores."]
#[doc = " Depyler: verified panic-free"]
pub fn cross_val_score<'c, 'a, 'b>(
    X: &'a Vec<Vec<f64>>,
    y: &'b Vec<f64>,
    model: &'c str,
    n_splits: i32,
) -> Result<Vec<f64>, Box<dyn std::error::Error>> {
    let mut score: f64 = Default::default();
    let folds = kfold_split(&X.len() as i32, n_splits)?;
    let mut scores = vec![];
    for fold in folds.iter().cloned() {
        ...
        if (*model) == "linear_regression" {
            let (coef, intercept) = linear_regression_fit(&X_train, &y_train)?;
            let y_pred = linear_regression_predict(&X_test, &coef, intercept);
            score = r2_score(&y_test, &y_pred)?;
        } else {
            score = 0.0;
        }
        scores.push(DepylerValue::Float(score as f64));
    }
    Ok(scores)
}
#[doc = "Handle split subcommand."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_split() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = data.get("n_samples").is_none();
    if _cse_temp_0 {
        eprintln!("{}", "Error: Missing 'n_samples'");
        std::process::exit(1);
    }
    let n_splits = data.get("n_splits").cloned().unwrap_or(5);
    let mut folds: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        folds = kfold_split(
            data.get("n_samples").cloned().unwrap_or_default(),
            &n_splits,
        )?;
        println!(
            "{}",
            format!("{:?}", {
                let mut map = HashMap::new();
                map.insert("folds".to_string().to_string(), folds);
                map
            })
        );
        Ok(())
...
#[doc = "Handle cross-val-score subcommand."]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_cross_val_score() -> Result<(), Box<dyn std::error::Error>> {
    let data = match serde_json::from_reader::<_, String>(std::io::stdin()) {
        Ok(__json_data) => __json_data,
        Err(e) => {
            eprintln!("{}", format!("Error: Invalid JSON: {:?}", e));
    ...
    let required = vec!["X".to_string(), "y".to_string(), "model".to_string()];
    ...
    let mut mean_score: i32 = Default::default();
    let mut std_score: Option<String> = None;
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        scores = cross_val_score(
            data.get("X").cloned().unwrap_or_default(),
            data.get("y").cloned().unwrap_or_default(),
            data.get("model").cloned().unwrap_or_default(),
            &n_splits,
        )?;
        mean_score = scores.iter().sum::<i32>() / scores.len() as i32;
        std_score = Some(
            (scores
                .as_slice()
                .iter()
                ...
                .map(|s| {
                    if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                        ...
                        ({ s - mean_score } as f64).powf({ 2 } as f64) as i32
                    }
                })
                .sum::<i32>()
                / scores.len() as i32 as f64)
                .sqrt(),
        );
        println!(
            "{}",
            format!("{:?}", {
                let mut map = HashMap::new();
                map.insert("scores".to_string().to_string(), scores);
                map.insert("mean_score".to_string().to_string(), mean_score);
                map.insert("std_score".to_string().to_string(), std_score);
                map
            })
        );
        Ok(())
