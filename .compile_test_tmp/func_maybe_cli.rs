...
#[doc = "Create a Some value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn some<T: Clone>(value: &T) -> Maybe<T> {
    PySome::new(value)
}
#[doc = "Create a Nothing value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn nothing<T: Clone>() -> Maybe<T> {
    Nothing::new()
}
#[doc = "Create Maybe from nullable value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn maybe<T: Clone>(value: &Option<T>) -> Maybe<T> {
    if value.is_none() {
        return Nothing::new();
    }
    PySome::new(value)
}
#[doc = "Create Maybe from optional value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn from_optional<T: Clone>(value: &Option<T>) -> Maybe<T> {
    maybe(value)
}
#[doc = "Safe division returning Maybe."]
#[doc = " Depyler: proven to terminate"]
pub fn safe_div(a: f64, b: f64) -> Result<Maybe<f64>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = b == 0f64;
    if _cse_temp_0 {
        return Ok(Nothing::new());
    }
    Ok(PySome::new(((a) as f64) / ((b) as f64)))
}
#[doc = "Safe string to int conversion."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn safe_int(s: &str) -> Maybe<i32> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(PySome::new(s.parse::<i32>().unwrap_or_default()));
    })() {
        Ok(_result) => {
            return _result;
        }
...
#[doc = "Safe string to float conversion."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn safe_float(s: &str) -> Maybe<f64> {
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        return Ok(PySome::new(s.parse::<f64>().unwrap()));
    })() {
        Ok(_result) => {
            return _result;
        }
...
#[doc = "Get first element safely."]
#[doc = " Depyler: proven to terminate"]
pub fn safe_head<T: Clone>(lst: &Vec<T>) -> Result<Maybe<T>, Box<dyn std::error::Error>> {
    if !lst.is_empty() {
        return Ok(PySome::new(
            lst.get(0usize)
                .cloned()
...
#[doc = "Get last element safely."]
#[doc = " Depyler: proven to terminate"]
pub fn safe_last<T: Clone>(lst: &Vec<T>) -> Result<Maybe<T>, Box<dyn std::error::Error>> {
    if !lst.is_empty() {
        return Ok(PySome::new({
            let base = &lst;
...
#[doc = "Get dict value safely."]
#[doc = " Depyler: proven to terminate"]
pub fn safe_get<'b, 'a, T: Clone>(
    d: &'a std::collections::HashMap<String, T>,
    key: &'b str,
) -> Result<Maybe<T>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = d.get(key).is_some();
    if _cse_temp_0 {
        return Ok(PySome::new(d.get(key).cloned().unwrap_or_default()));
    }
    Ok(Nothing::new())
}
#[doc = "Get list element by index safely."]
#[doc = " Depyler: proven to terminate"]
...
#[doc = "Lift a function to work with Maybe."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn lift<U: Clone, T: Clone>(f: impl Fn(T) -> U) -> Box<dyn Fn(Maybe<()>) -> Maybe<()>> {
    let lifted = move |m: Maybe<()>| -> Maybe<()> {
        return m.map(f);
    };
    Box::new(lifted)
}
#[doc = "Lift a binary function to work with Maybe."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn lift2<T: Clone, U: Clone, V: Clone>(
    f: impl Fn(T, U) -> V,
) -> Box<dyn Fn(Maybe<()>, Maybe<()>) -> Maybe<()>> {
    let lifted = move |m1: Maybe<()>, m2: Maybe<()>| -> Maybe<()> {
        return m1.flat_map(move |a| m2.map(move |b| f(a, b)));
    };
    Box::new(lifted)
}
#[doc = "Convert list of Maybes to Maybe of list."]
#[doc = " Depyler: verified panic-free"]
pub fn sequence<T: Clone>(maybes: &Vec<Maybe<T>>) -> Maybe<Vec<T>> {
    let mut results: Vec<T> = vec![];
    for m in maybes.iter().cloned() {
        if m.is_none() {
            return Nothing::new();
        }
        results.push(m.get());
...
#[doc = "Map function over list, collecting into Maybe."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn traverse<T: Clone, U: Clone>(f: impl Fn(T) -> Maybe<U>, lst: &Vec<T>) -> Maybe<Vec<U>> {
    sequence(lst.iter().cloned().map(|x| f(x)).collect::<Vec<_>>())
}
#[doc = "Extract all Some values from list."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cat_maybes<T: Clone>(maybes: &Vec<Maybe<T>>) -> Vec<T> {
    maybes
        .iter()
        .cloned()
        .filter(|m| {
...
#[doc = "Simulate maybe operations."]
pub fn simulate_maybe(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut m: Maybe<i32> = Default::default();
        ...
        if cmd == "some" {
            ...
            if cmd == "nothing" {
                ...
                if cmd == "maybe" {
                    ...
                    m = maybe(&Some(if val != 0 { (val) as i32 } else { None }));
                    results.push(format!("{:?}", m));
                } else {
                    if cmd == "safe_div" {
                        let (a, b) = parts
                        ...
                        if cmd == "safe_int" {
                            ...
                            if cmd == "map" {
                                ...
                                if cmd == "filter" {
                                    ...
                                    m = some((val) as i32);
                                    if pred == "even" {
                                        m = m.filter(move |x| x % 2 == 0);
                                    }
                                    results.push(format!("{:?}", m));
                                } else {
                                    if cmd == "get_or" {
                                        let (mut val, default) = parts
                                            .get(1usize)
                                            .cloned()
                                            .expect("IndexError: list index out of range")
                                            .split(",")
                                        ...
                                        results.push((m.get_or((default) as i32)).to_string());
                                    } else {
                                        if cmd == "sequence" {
                                            let vals = parts
                                                .get(1usize)
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: func_maybe_cli.py <command>[args]");
    ...
    if _cse_temp_4 {
        ...
        let _cse_temp_5 = cmd == "safe-int";
        let _cse_temp_6 = _cse_temp_0 > 2;
...
mod tests {
    ...
    #[test]
    fn test_cat_maybes_examples() {
        assert_eq!(cat_maybes(vec![]), vec![]);
        assert_eq!(cat_maybes(vec![1]), vec![1]);
    }
    #[test]
    fn test_simulate_maybe_examples() {
        assert_eq!(simulate_maybe(vec![]), vec![]);
        assert_eq!(simulate_maybe(vec![1]), vec![1]);
    }
}
