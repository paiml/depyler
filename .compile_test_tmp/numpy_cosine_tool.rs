...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Cosine3 { .. }));
    match &args.command {
        Some(Commands::Cosine3 {
            ref a1,
            ref a2,
            ref a3,
            ref b1,
            ...
            let b2 = *b2;
            let b3 = *b3;
            let mut a = Vector::from_slice(&[a1 as f32, a2 as f32, a3 as f32]);
            let mut b = Vector::from_slice(&[b1 as f32, b2 as f32, b3 as f32]);
            let mut dot = a.dot(&b).unwrap();
            let mut norm_a = (a).norm_l2().unwrap();
            let mut norm_b = (b).norm_l2().unwrap();
            let mut result = if (norm_a > 0f64) && (norm_b > 0f64) {
                ((dot) as f64) / ((norm_a * norm_b) as f64)
            } else {
                0f64
            };
            println!("{}", (result as f64).round() as i32);
        }
        Some(Commands::Cosine2 {
            ref a1,
            ref a2,
            ref b1,
            ref b2,
            ...
            let b1 = *b1;
            let b2 = *b2;
            let mut a = Vector::from_slice(&[a1 as f32, a2 as f32]);
            let mut b = Vector::from_slice(&[b1 as f32, b2 as f32]);
            let mut dot = a.dot(&b).unwrap();
            let mut norm_a = (a).norm_l2().unwrap();
            let mut norm_b = (b).norm_l2().unwrap();
            let mut result = if (norm_a > 0f64) && (norm_b > 0f64) {
        ...
        Some(Commands::Cosine4 {
            ...
            let b3 = *b3;
            let b4 = *b4;
            let mut a = Vector::from_slice(&[a1 as f32, a2 as f32, a3 as f32, a4 as f32]);
            let mut b = Vector::from_slice(&[b1 as f32, b2 as f32, b3 as f32, b4 as f32]);
            let mut dot = a.dot(&b).unwrap();
            let mut norm_a = (a).norm_l2().unwrap();
            let mut norm_b = (b).norm_l2().unwrap();
            let mut result = if (norm_a > 0f64) && (norm_b > 0f64) {
