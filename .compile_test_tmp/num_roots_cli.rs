...
impl RootResult {
    pub fn new(root: f64, iterations: i32, converged: bool, error: f64) -> Self {
        Self {
            root,
            iterations,
            converged,
...
#[doc = "Find root using bisection method."]
...
pub fn bisection(
    ...
    tol: f64,
    max_iter: i32,
) -> Result<RootResult, Box<dyn std::error::Error>> {
    ...
    for i in 0..(max_iter) {
        ...
        let fc = f(c);
        if (((fc).abs() as f64) < tol) || (((b - a) as f64) / ((2) as f64) < tol) {
            return Ok(RootResult::new(c, i + 1, true, (fc).abs()));
        }
        if fc * f(a) < 0f64 {
            b = c;
        } else {
    ...
    Ok(RootResult::new(
        ...
        max_iter,
        false,
        (f(((a + b) as f64) / ((2) as f64))).abs(),
    ))
}
#[doc = "Find root using Newton-Raphson method."]
#[doc = " Depyler: proven to terminate"]
pub fn newton(
    f: impl Fn(f64) -> f64,
    df: impl Fn(f64) -> f64,
    ...
    for i in 0..(max_iter) {
        let fx = f(x);
        let dfx = df(x);
        if ((dfx).abs() as f64) < 0.00000000000001 {
            return Ok(RootResult::new(x, i + 1, false, (fx).abs()));
        }
        let x_new = x - ((fx) as f64) / ((dfx) as f64);
        if ((x_new - x).abs() as f64) < tol {
            return Ok(RootResult::new(x_new, i + 1, true, (f(x_new)).abs()));
        }
        x = x_new;
    }
    Ok(RootResult::new(x, max_iter, false, (f(x)).abs()))
}
#[doc = "Find root using secant method."]
#[doc = " Depyler: proven to terminate"]
pub fn secant(
    f: impl Fn(f64) -> f64,
    mut x0: f64,
    ...
    for i in 0..(max_iter) {
        let (f0, f1) = (f(x0), f(x1));
        if ((f1 - f0).abs() as f64) < 0.00000000000001 {
            return Ok(RootResult::new(x1, i + 1, false, (f1).abs()));
        }
        let x_new = x1 - ((f1 * (x1 - x0)) as f64) / ((f1 - f0) as f64);
        if ((x_new - x1).abs() as f64) < tol {
            return Ok(RootResult::new(x_new, i + 1, true, (f(x_new)).abs()));
        }
        (x0, x1) = (x1, x_new);
    }
    Ok(RootResult::new(x1, max_iter, false, (f(x1)).abs()))
}
#[doc = "Find root using regula falsi(false position) method."]
#[doc = " Depyler: proven to terminate"]
pub fn regula_falsi(
    f: impl Fn(f64) -> f64,
    mut a: f64,
    ...
    let mut c: DepylerValue = Default::default();
    let _cse_temp_0 = f(a) * f(b);
    let _cse_temp_1 = _cse_temp_0 > 0f64;
    if _cse_temp_1 {
        return Err(Box::new(ValueError::new(
    ...
    for i in 0..(max_iter) {
        let (fa, fb) = (f(a), f(b));
        c = ((a * fb - b * fa) as f64) / ((fb - fa) as f64);
        let fc = f(c);
        if ((fc).abs() as f64) < tol {
            return Ok(RootResult::new(c, i + 1, true, (fc).abs()));
        }
        if fc * fa < 0f64 {
            b = c;
    ...
    let _cse_temp_2 = a * f(b);
    let _cse_temp_3 = b * f(a);
    let _cse_temp_4 = f(b) - f(a);
    let _cse_temp_5 = ((_cse_temp_2 - _cse_temp_3) as f64) / ((_cse_temp_4) as f64);
    c = _cse_temp_5;
    Ok(RootResult::new(c, max_iter, false, (f(c)).abs()))
}
#[doc = "Find root using Brent's method."]
#[doc = " Depyler: proven to terminate"]
pub fn brent(
    f: impl Fn(f64) -> f64,
    mut a: f64,
    ...
    let _cse_temp_2 = (f(a)).abs();
    let _cse_temp_3 = (f(b)).abs();
    ...
    for i in 0..(max_iter) {
        ...
        if ((fb).abs() as f64) < tol {
            return Ok(RootResult::new(b, i + 1, true, (fb).abs()));
        }
        if (fa != fc) && (fb != fc) {
            s = ((a * fb * fc) as f64) / (((fa - fb) * (fa - fc)) as f64)
                + ((b * fa * fc) as f64) / (((fb - fa) * (fb - fc)) as f64)
                + ((c * fa * fb) as f64) / (((fc - fa) * (fc - fb)) as f64);
        } else {
            s = b - ((fb * (b - a)) as f64) / ((fb - fa) as f64);
        }
        let cond1 = if a < b {
            (s < ((3f64 * a + b) as f64) / ((4) as f64)) || (s > b)
        } else {
            (s > ((3f64 * a + b) as f64) / ((4) as f64)) || (s < b)
        };
        let cond2 = (mflag) && ((s - b).abs() >= (b - c).abs() / 2);
        let cond3 = if i > 0 {
            (!mflag) && ((s - b).abs() >= (c - d).abs() / 2)
        } else {
            false
        };
        let cond4 = (mflag) && (((b - c).abs() as f64) < tol);
        let cond5 = if i > 0 {
            (!mflag) && (((c - d).abs() as f64) < tol)
        } else {
            false
        };
        if ((((cond1) || (cond2)) || (cond3)) || (cond4)) || (cond5) {
            s = ((a + b) as f64) / ((2) as f64);
            mflag = true;
        } else {
            mflag = false;
        }
    ...
    Ok(RootResult::new(b, max_iter, false, (f(b)).abs()))
}
#[doc = "Find fixed point x = g(x)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn fixed_point(g: impl Fn(f64) -> f64, x0: f64, tol: f64, max_iter: i32) -> RootResult {
    let mut x: f64 = Default::default();
    x = x0;
    for i in 0..(max_iter) {
        let x_new = g(x);
        if ((x_new - x).abs() as f64) < tol {
            return RootResult::new(x_new, i + 1, true, (x_new - g(x_new)).abs());
        }
        x = x_new;
    }
    RootResult::new(x, max_iter, false, (x - g(x)).abs())
}
#[doc = "Find all roots in interval by searching subintervals."]
#[doc = " Depyler: proven to terminate"]
pub fn find_all_roots(
    ...
    let mut roots = vec![];
    let _cse_temp_0 = ((b - a) as f64) / ((n_intervals) as f64);
    let dx = _cse_temp_0;
    for i in 0..(n_intervals) {
        let x0 = a + (i as f64) * dx;
        let x1 = a + ((i + 1) as f64) * dx;
        if f(x0) * f(x1) <= 0f64 {
            let mut result: Option<String> = None;
            match (|| -> Result<(), Box<dyn std::error::Error>> {
                result = Some(bisection(f, x0, x1, tol, 100i32)?);
                if result.converged {
                    if !roots
                        .iter()
                        .cloned()
                        .map(|r| ((r - result.root).abs() as f64) < tol * 10f64)
                        .any(|x| x)
                    {
                        roots.push(DepylerValue::Str(format!("{:?}", result.root)));
                    }
                }
                Ok(())
            })() {
    ...
    Ok({
        let mut sorted_vec = roots.iter().cloned().collect::<Vec<_>>();
        sorted_vec.sort_by(|a, b| a.partial_cmp(b).unwrap_or(std::cmp::Ordering::Equal));
        sorted_vec
    })
}
#[doc = "Simulate root finding operations."]
pub fn simulate_roots(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for op in operations.iter().cloned() {
        ...
        let mut result;
        if cmd == "bisection" {
            result = bisection(f, 1f64, 2f64, 0.0000000001f64, 100i32)?;
            results.push(format!("root={} iter={}", result.root, result.iterations));
        } else {
            if cmd == "newton" {
                result = newton(f, df, 1.5, 0.0000000001f64, 100i32)?;
                results.push(format!("root={} iter={}", result.root, result.iterations));
            } else {
                if cmd == "secant" {
                    result = secant(f, 1f64, 2f64, 0.0000000001f64, 100i32)?;
                    results.push(format!("root={} iter={}", result.root, result.iterations));
                } else {
                    if cmd == "sin_roots" {
                        let roots = find_all_roots(f64::sin, 0f64, 10f64, 100i32, 0.0000000001f64)?;
                        results.push(format!(
                            "roots={}",
                            roots
                                .as_slice()
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: num_roots_cli.py <command>");
    ...
    if _cse_temp_2 {
        ...
        println!("{}", "Finding root of x^3 - x - 2:");
        let result = bisection(f, 1f64, 2f64, 0.0000000001f64, 100i32)?;
        println!(
            "{}",
            format!(
                "Bisection: root={}, iterations={}",
...
mod tests {
    ...
    #[test]
    fn test_simulate_roots_examples() {
        assert_eq!(simulate_roots(vec![]), vec![]);
        assert_eq!(simulate_roots(vec![1]), vec![1]);
    }
}
