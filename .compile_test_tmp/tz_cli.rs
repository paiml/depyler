...
#![allow(dead_code)]
pub static TIMEZONE_OFFSETS: std::sync::LazyLock<std::collections::HashMap<String, i32>> =
    std::sync::LazyLock::new(|| {
        let mut map = HashMap::new();
        map.insert("UTC".to_string().to_string(), 0);
        map.insert("GMT".to_string().to_string(), 0);
        map.insert("EST".to_string().to_string(), -5);
        map.insert("EDT".to_string().to_string(), -4);
        map.insert("CST".to_string().to_string(), -6);
        map.insert("CDT".to_string().to_string(), -5);
        map.insert("MST".to_string().to_string(), -7);
        map.insert("MDT".to_string().to_string(), -6);
        map.insert("PST".to_string().to_string(), -8);
        map.insert("PDT".to_string().to_string(), -7);
        map.insert("CET".to_string().to_string(), 1);
        map.insert("CEST".to_string().to_string(), 2);
        map.insert("JST".to_string().to_string(), 9);
        map.insert("IST".to_string().to_string(), 5);
        map.insert("AEST".to_string().to_string(), 10);
        map.insert("AEDT".to_string().to_string(), 11);
        map
    });
...
struct Args {
    ...
    #[doc = "List available timezones"]
    list: bool,
    
    
    #[doc = "Show time difference between zones"]
    diff: bool,
}
#[doc = "Get timezone from name or offset string."]
#[doc = " Depyler: proven to terminate"]
pub fn get_timezone(mut name: String) -> Result<Option<timezone>, Box<dyn std::error::Error>> {
    let mut hours: i32 = Default::default();
    name = name.to_uppercase();
    let _cse_temp_0 = TIMEZONE_OFFSETS.get(name).is_some();
    if _cse_temp_0 {
        hours = TIMEZONE_OFFSETS.get(name).cloned().unwrap_or_default();
        return Ok(Some(timezone(DepylerTimeDelta::new(0, 0, 0))));
    }
    let _cse_temp_1 = (name.starts_with("+")) || (name.starts_with("-"));
    if _cse_temp_1 {
        let mut parts: Vec<String> = Default::default();
        let mut sign: i32 = Default::default();
        let mut minutes: i32 = Default::default();
        let mut total_minutes: i32 = Default::default();
...
#[doc = "Parse time string into datetime."]
#[doc = " Depyler: verified panic-free"]
pub fn parse_time<'b, 'a>(time_str: &'a str, tz: &'b Option<timezone>) -> Option<DepylerDateTime> {
    ...
    for fmt in formats.iter().cloned() {
        ...
        match (|| -> Result<String, Box<dyn std::error::Error>> {
            ...
            if dt.year() as i32 == 1900 {
                now = Some(DepylerDateTime::now());
                dt = dt.replace();
            }
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn convert_time<'a, 'b>(dt: &'a mut DepylerDateTime, to_tz: &'b timezone) -> DepylerDateTime {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_time(dt: &DepylerDateTime, include_tz: bool) -> String {
    let base = format!("{:?}", dt);
    let _cse_temp_0 = (include_tz) && (dt.tzinfo);
    if _cse_temp_0 {
        let offset = format!("{:?}", dt);
        return format!("{} {}", base, offset);
    }
    base.to_string()
}
#[doc = "List available timezone names."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn list_timezones() -> Vec<String> {
    {
        let mut sorted_vec = TIMEZONE_OFFSETS
            .keys()
            .cloned()
            .collect::<Vec<_>>()
            .iter()
...
#[doc = "Get current time in specified timezone."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_current_time(tz: &timezone) -> DepylerDateTime {
    DepylerDateTime::now()
}
#[doc = "Calculate difference between two timezones."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn time_difference<'b, 'a>(tz1: &'a timezone, tz2: &'b timezone) -> DepylerTimeDelta {
    let now = DepylerDateTime::now();
    let t1 = now.astimezone(tz1);
    let t2 = now.astimezone(tz2);
    let _cse_temp_0 = (t1.utcoffset()) || (DepylerTimeDelta::new(0 as i64, 0, 0));
    let offset1 = _cse_temp_0;
    let offset2 = _cse_temp_0;
    offset2 - offset1
}
#[doc = "Format timedelta as offset string."]
#[doc = " Depyler: proven to terminate"]
pub fn format_offset(td: &DepylerTimeDelta) -> Result<String, Box<dyn std::error::Error>> {
    let _cse_temp_0 = (td.total_seconds()) as i32;
    let mut total_seconds = _cse_temp_0.clone();
    let sign = if total_seconds >= 0 {
        "+".to_string()
    } else {
        "-".to_string()
    };
    let _cse_temp_1 = (total_seconds).abs();
    total_seconds = _cse_temp_1;
    let _cse_temp_2 = {
        let a = total_seconds;
        let b = 3600;
        let q = a / b;
    ...
    let hours = _cse_temp_2;
    let _cse_temp_3 = total_seconds % 3600;
    let _cse_temp_4 = {
        let a = _cse_temp_3;
        let b = 60;
        let q = a / b;
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let has_time = args.time.is_some();
    if args.list {
        println!("{}", "Available timezones:");
        for tz_name in list_timezones() {
            let mut offset = TIMEZONE_OFFSETS.get(&tz_name).cloned().unwrap_or_default();
            let sign = if offset >= 0 {
                "+".to_string()
            } else {
                "".to_string()
            };
            println!("{}", format!("  {:?} {}{:?}:00", tz_name, sign, offset));
        }
        return Ok(());
    }
    let from_tz = get_timezone(args.from_tz)?;
    let to_tz = get_timezone(args.to_tz)?;
    if from_tz.is_none() {
        eprintln!("{}", format!("Unknown timezone: {}", args.from_tz));
        std::process::exit(1i32)
    }
    if to_tz.is_none() {
        eprintln!("{}", format!("Unknown timezone: {}", args.to_tz));
        std::process::exit(1i32)
    }
    if args.diff {
        let diff = time_difference(from_tz, to_tz);
        let mut offset = format_offset(&diff)?;
        println!(
            "{}",
            format!("{} is {:?} from {}", args.to_tz, offset, args.from_tz)
        );
        return Ok(());
    }
    let _cse_temp_0 = args.time.to_lowercase() == "now";
    let _cse_temp_1 = (!args.time) || (_cse_temp_0);
    if _cse_temp_1 {
        dt = get_current_time(from_tz);
    } else {
        dt = Some(parse_time(&args.time, from_tz));
        if dt.is_none() {
            eprintln!("{}", format!("Invalid time format: {}", args.time));
            std::process::exit(1i32)
        }
    }
    let result = convert_time(&mut dt, to_tz);
    println!("{}", format!("{}: {}", args.from_tz, format_time(dt, true)));
    println!(
        "{}",
        format!("{}: {}", args.to_tz, format_time(&result, true))
    );
    Ok(())
}
