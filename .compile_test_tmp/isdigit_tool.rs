...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Digit { .. }));
    match &args.command {
        Some(Commands::Digit { ref text, .. }) => {
            let mut result = true;
            let mut i = 0;
        ...
        Some(Commands::Alpha { ref text, .. }) => {
            ...
            while i < text.len() as i32 {
                ...
                let is_lower = ((c).as_str() >= "a") && ((c).as_str() <= "z");
                let is_upper = ((c).as_str() >= "A") && ((c).as_str() <= "Z");
