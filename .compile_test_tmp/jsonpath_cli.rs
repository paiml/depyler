...
} #[derive(Debug, Clone)] pub struct PathType {
   
}
impl PathType {
    pub const ROOT: i32 = auto();
    pub const CHILD: i32 = auto();
    pub const RECURSIVE: i32 = auto();
    pub const INDEX: i32 = auto();
    pub const SLICE: i32 = auto();
    pub const WILDCARD: i32 = auto();
    pub const FILTER: i32 = auto();
    pub fn new() -> Self {
    Self {
...
#[derive(Debug, Clone, PartialEq)] pub struct PathSegment {
    pub type_: PathType, pub value: DepylerValue
}
impl PathSegment {
    pub fn new(type_: PathType, value: DepylerValue) -> Self {
    Self {
    type_, value
}
}
...
#[derive(Default)] struct Args {
    #[doc = "JSONPath expression"] path: Option<String>, #[doc = "JSON file to query"] file: Option<String>, #[doc = "JSON data string"] data: Option<String>, #[doc = "Operation mode"] mode: String
}
...
#[doc = "Parse JSONPath string into segments."] pub fn parse_path(path: & str) -> Result<Vec<PathSegment>, Box<dyn std::error::Error>>{
    ...
    if char == "$" {
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::ROOT, None))));
    i = i + 1;
    ...
    let mut key: String;
    if char == "." {
    if(i + 1<path.len() as i32) &&({ let base = & path;
    let idx: i32 = i + 1;
    let actual_idx = if idx<0 {
    base.chars().count().saturating_sub(idx.abs() as usize)
}
...
== ".") {
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::RECURSIVE, None))));
    i = i + 2;
    ...
    i = i + 1;
    start = i;
    while(i<path.len() as i32) &&(! ".[]".contains(& * {
    let base = & path;
    let idx: i32 = i;
    let actual_idx = if idx<0 {
    base.chars().count().saturating_sub(idx.abs() as usize)
...
if i>start {
   ...
   (len + stop_idx).max(0) as usize
    ...
    if key == "*" {
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::WILDCARD, None))));
   
}
else {
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::CHILD, key))));
    ...
    while(i<path.len() as i32) &&(bracket_depth>0) {
    if {
    let base = & path;
    let idx: i32 = i;
    let actual_idx = if idx<0 {
...
let content = {
   ...
   (len + stop_idx).max(0) as usize
    ...
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::FILTER, {
    let base = content;
    let start_idx: i32 = 1;
    let len = base.chars().count() as i32;
    let actual_start = if start_idx<0 {
...
. trim().to_string()))));
    ...
    let start_idx = if parts.get(0usize).cloned().expect("IndexError: list index out of range") {
   (parts.get(0usize).cloned().expect("IndexError: list index out of range")) as i32
}
else {
    None };
    let end_idx = if(parts.len() as i32>1) &&(parts.get(1usize).cloned().expect("IndexError: list index out of range")) {
   (parts.get(1usize).cloned().expect("IndexError: list index out of range")) as i32
}
else {
    None };
    let step = if(parts.len() as i32>2) &&(parts.get(2usize).cloned().expect("IndexError: list index out of range")) {
   (parts.get(2usize).cloned().expect("IndexError: list index out of range")) as i32
}
else {
    None };
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::SLICE ,(start_idx, end_idx, step)))));
   
}
else {
    if(content.chars().all(| c | c.is_numeric())) ||((content.starts_with("-")) &&({ let base = content;
    let start_idx: i32 = 1;
    let len = base.chars().count() as i32;
    let actual_start = if start_idx<0 {
   (len + start_idx).max(0) as usize
...
. chars().all(| c | c.is_numeric()))) {
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::INDEX, content.parse::<i32>().unwrap_or_default()))));
   
}
else {
    key = content.trim_matches(| c: char | "'\"".to_string().contains(c)).to_string();
    segments.push(DepylerValue::Str(format!("{:?}", PathSegment::new(PathType::CHILD, key))));
...
#[doc = "Execute JSONPath query on data."] #[doc = " Depyler: verified panic-free"] pub fn query<'b, 'a>(data: & 'a DepylerValue, path: & 'b str) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    let mut results: Vec<DepylerValue>= Default::default();
    let segments = parse_path(path) ?;
    results = vec! [data];
    for segment in segments.iter().cloned() {
    let mut new_results = vec! [];
    for item in results.iter().cloned() {
    new_results.extend(apply_segment(item, segment).iter().cloned());
...
#[doc = "Apply a single path segment to data."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn apply_segment<'b, 'a>(data: & 'a DepylerValue, segment: & 'b PathSegment) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    let _cse_temp_0 = segment.r#type == PathType::ROOT;
    if _cse_temp_0 {
    return Ok(vec! [data]);
   
}
if _cse_temp_0 {
    let _cse_temp_1 = data.get(& segment.value).is_some();
    let _cse_temp_2  = (true) &&(_cse_temp_1);
    if _cse_temp_2 {
    return Ok(vec! [{ let base = & data;
    let idx: i32 = segment.value;
    let actual_idx = if idx<0 {
...
if _cse_temp_0 {
    ...
    let _cse_temp_3 = - data.len() as i32 <= idx;
    let _cse_temp_4 = data.len() as i32;
    let _cse_temp_5 = idx<_cse_temp_4;
    let _cse_temp_6  = (_cse_temp_3) &&(_cse_temp_5);
    let _cse_temp_7  = (true) &&(_cse_temp_6);
    if _cse_temp_7 {
    return Ok(vec! [data.get(idx as usize).cloned().expect("IndexError: list index out of range")]);
...
if _cse_temp_0 {
    if true {
    let(start, end, step) = segment.value;
    return Ok(vec! [{ let base = data;
   ...
   (base.len() as isize + stop_idx).max(0) as usize
    ...
    base [start..stop.min (base.len())].iter().step_by(step as usize).cloned().collect::<Vec<_>>()
}
else {
    let abs_step  = (- step) as usize;
    if start<base.len() {
    base [start..stop.min (base.len())].iter().rev().step_by(abs_step).cloned().collect::<Vec<_>>()
}
else {
    Vec::new()
}
...
if _cse_temp_0 {
    if true {
    return Ok(data.values().cloned().collect::<Vec<_>>());
...
if _cse_temp_0 {
    return Ok(recursive_descent(data));
   
}
if _cse_temp_0 {
    return Ok(apply_filter(data ,(segment.value).to_string()));
   
}
Ok(vec! [])
}
#[doc = "Get all descendants of data."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn recursive_descent(data: & DepylerValue) -> Vec<DepylerValue>{
    let mut results = vec! [data];
    if true {
    for value in data.values().cloned().collect::<Vec<_>>() {
    results.extend(recursive_descent(& value).iter().cloned());
   
}
} else {
    if true {
    for item in data.iter().cloned() {
    results.extend(recursive_descent(& item).iter().cloned());
...
#[doc = "Apply filter expression to array."] #[doc = " Depyler: verified panic-free"] pub fn apply_filter(data: & DepylerValue, mut expr: String) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    if! true {
    return Ok(vec! []);
   
}
let mut results = vec! [];
    expr = expr.trim_matches(| c: char | "()".to_string().contains(c)).to_string();
    for item in data.iter().cloned() {
    if evaluate_filter(item, & expr).unwrap_or(false) {
    results.push(DepylerValue::Str(format!("{:?}", item)));
   
}
} Ok(results)
}
#[doc = "Evaluate filter expression on item."] #[doc = " Depyler: proven to terminate"] pub fn evaluate_filter(item: & DepylerValue, mut expr: String) -> Result<bool, Box<dyn std::error::Error>>{
    expr = expr.trim().to_string();
    let _cse_temp_0 = expr.contains("@.");
    if _cse_temp_0 {
    expr = expr.replace("@.", "");
    for op in vec! [">=".to_string(), "<=".to_string(), "!=".to_string(), "==".to_string(), ">".to_string(), "<".to_string()] {
    if expr.contains(& * op) {
    let _split_parts = expr.splitn((1 + 1) as usize, & op).map(| s | s.to_string()).collect::<Vec<String>>();
    let mut left = _split_parts.get(0).cloned().unwrap_or_default();
    let mut right = _split_parts.get(1).cloned().unwrap_or_default();
    left = left.trim().to_string();
    right = right.trim().to_string().trim_matches(| c: char | "'\"".to_string().contains(c)).to_string();
    if(true) &&(item.contains(& * left)) {
    let mut left_val = item.get(& left).cloned().unwrap_or_default();
    let mut right_val: f64 = Default::default();
    match(|| -> Result <(), Box<dyn std::error::Error>>{
    right_val = if right.contains(".") {
    right.parse::<f64>().unwrap()
}
else {
    right.parse::<i32>().unwrap_or_default() };
    left_val  = (left_val) as f64;
    Ok(()) })() {
    Ok(()) =>{
...
, Err(_) =>{
    ...
    return Ok(left_val = = (right_val as f64));
...
} return Ok((true) &&(item.contains(& * expr)));
   
}
Ok(false)
}
#[doc = "Get single value from path(returns first result or None)."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn get_value < 'a, 'b>(data: & 'a DepylerValue, path: & 'b str) -> Result<DepylerValue, Box<dyn std::error::Error>>{
    let results = query(& data, path) ?;
    Ok(if! results.is_empty() {
    results.get(0usize).cloned().expect("IndexError: list index out of range")
}
else {
    None })
}
#[doc = "Get all values matching path."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn get_all<'b, 'a>(data: & 'a DepylerValue, path: & 'b str) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>>{
    Ok(query(& data, path))
}
#[doc = "Check if path exists in data."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn exists<'b, 'a>(data: & 'a DepylerValue, path: & 'b str) -> Result<bool, Box<dyn std::error::Error>>{
    Ok(query(& data, path).len() as i32>0)
}
#[doc = "Count matches for path."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn count<'b, 'a>(data: & 'a DepylerValue, path: & 'b str) -> Result<i32, Box<dyn std::error::Error>>{
    Ok(query(& data, path).len() as i32 as i32)
}
#[doc = "Get keys of object."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn keys(data: & DepylerValue) -> Vec<String>{
    if true {
...
#[doc = "Get values of object or array."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn values(data: & DepylerValue) -> Vec<DepylerValue>{
    if true {
    return data.values().cloned().collect::<Vec<_>>();
...
#[doc = "Flatten nested structure."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn flatten(data: & DepylerValue) -> Vec<DepylerValue>{
    let mut results = vec! [];
    if true {
    for value in data.values().cloned().collect::<Vec<_>>() {
    results.extend(flatten(& value).iter().cloned());
   
}
} else {
    if true {
    for item in data.iter().cloned() {
    results.extend(flatten(& item).iter().cloned());
   
}
} else {
    results.push(DepylerValue::Str(format!("{:?}", data)));
...
#[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), std::io::Error>{
    ...
    data = std::collections::HashMap::<String, DepylerValue>::new();
...
} let _cse_temp_0 = args.mode == "keys";
    if _cse_temp_0 {
    result = keys(& data);
...
let _cse_temp_1 = args.mode == "values";
    if _cse_temp_1 {
    result = values(& data);
    println!("{}", format!("{:?}", result));
    return Ok(());
...
let _cse_temp_2 = args.mode == "flatten";
    if _cse_temp_2 {
    result = flatten(& data);
    println!("{}", format!("{:?}", result));
    return Ok(());
...
if! args.path {
    println!("{}", "Error: path required for query mode");
    std::process::exit(1i32)
}
let _cse_temp_3 = args.mode == "query";
    if _cse_temp_3 {
    let results = query(& data, & args.path) ?;
    let _cse_temp_4 = results.len() as i32;
    let _cse_temp_5 = _cse_temp_4 == 1;
    if _cse_temp_5 {
    println!("{}", format!("{:?}", results.get(0usize).cloned().expect("IndexError: list index out of range")));
    ...
    let _cse_temp_6 = args.mode == "exists";
    if _cse_temp_6 {
    println!("{}", if exists(& data, & args.path) {
    "true".to_string()
}
else {
    "false".to_string() });
    ...
    let _cse_temp_7 = args.mode == "count";
    if _cse_temp_7 {
    println!("{:?}", count(& data, & args.path));
