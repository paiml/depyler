...
pub enum Iter<T: Clone> {
    ListIter(Vec<T>, i32),
    RangeIter(i32, i32, i32, Option<i32>),
    MapIter(Iter<T>, impl Fn(T) -> U),
    FilterIter(Iter<T>, impl Fn(T) -> bool),
    TakeIter(Iter<T>, i32, i32),
    SkipIter(Iter<T>, i32, i32),
    EnumerateIter(Iter<T>, i32),
    ZipIter(Iter<T>, Iter<U>),
    ChainIter(Iter<T>, Iter<T>, bool),
    RepeatIter(T, Option<i32>, i32),
    CycleIter(Vec<T>, i32, Option<i32>, i32),
}
impl<T: Clone> Iter<T> {
    pub fn is_listiter(&self) -> bool {
        matches!(self, Self::ListIter(..))
    }
    pub fn is_rangeiter(&self) -> bool {
        matches!(self, Self::RangeIter(..))
    }
    pub fn is_mapiter(&self) -> bool {
        matches!(self, Self::MapIter(..))
    }
    pub fn is_filteriter(&self) -> bool {
        matches!(self, Self::FilterIter(..))
    }
    pub fn is_takeiter(&self) -> bool {
        matches!(self, Self::TakeIter(..))
    }
    pub fn is_skipiter(&self) -> bool {
        matches!(self, Self::SkipIter(..))
    }
    pub fn is_enumerateiter(&self) -> bool {
        matches!(self, Self::EnumerateIter(..))
    }
    pub fn is_zipiter(&self) -> bool {
        matches!(self, Self::ZipIter(..))
    }
    pub fn is_chainiter(&self) -> bool {
        matches!(self, Self::ChainIter(..))
    }
    pub fn is_repeatiter(&self) -> bool {
        matches!(self, Self::RepeatIter(..))
    }
    pub fn is_cycleiter(&self) -> bool {
        matches!(self, Self::CycleIter(..))
    }
}
...
#[doc = "Create iterator from list."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn iter_from_list<T: Clone>(items: &Vec<T>) -> Iter<T> {
    ListIter::new(items, 0i32)
}
#[doc = "Create range iterator."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn iter_range(start: i32, end: i32, step: i32) -> Iter {
    RangeIter::new(start, end, step, None)
}
#[doc = "Create repeat iterator."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn iter_repeat<'a, 'b, T: Clone>(value: &'a T, count: &'b Option<i32>) -> Iter<T> {
    RepeatIter::new(value, count, 0i32)
}
#[doc = "Create cycle iterator."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn iter_cycle<T: Clone>(items: &Vec<T>, max_cycles: Option<i32>) -> Iter<T> {
    CycleIter::new(items, max_cycles, None, 0i32)
}
#[doc = "Partition iterator into two lists."]
#[doc = " Depyler: verified panic-free"]
pub fn partition_iter<T: Clone>(iter: &Iter<T>, predicate: impl Fn(T) -> bool) -> (Vec<T>, Vec<T>) {
    let mut left: Vec<T> = vec![];
    let mut right: Vec<T> = vec![];
    loop {
        let (item, ok) = iter.next();
        if !ok {
            break;
        }
        if predicate(item) {
...
#[doc = "Group iterator items by key function."]
#[doc = " Depyler: verified panic-free"]
pub fn group_by_iter<T: Clone>(
    iter: &Iter<T>,
    key_fn: impl Fn(T) -> DepylerValue,
) -> HashMap<DepylerValue, Vec<T>> {
    let mut groups: std::collections::HashMap<DepylerValue, Vec<T>> = {
        let map: HashMap<(), Vec<()>> = HashMap::new();
        map
    };
    loop {
        let (item, ok) = iter.next();
...
#[doc = "Simulate iterator operations."]
pub fn simulate_iterator(
    operations: &Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results = vec![];
    let mut current_iter: Iter<i32> = ListIter::new(vec![], 0i32);
    for op in operations.iter().cloned() {
        ...
        if cmd == "list" {
            items = parts
                ...
                .map(|x| (x) as i32)
                .collect::<Vec<_>>();
            current_iter = ListIter::new(items, 0i32);
            ...
            if cmd == "range" {
                ...
                let step = if params.len() as i32 > 2 {
                    (params
                        .get(2usize)
                        .cloned()
                        .expect("IndexError: list index out of range")) as i32
                ...
                if cmd == "map" {
                    let factor = (parts
                        ...
                        .expect("IndexError: list index out of range"))
                        as i32;
                    current_iter = current_iter.map(move |x, f| x * f);
                    results.push(DepylerValue::Str("ok".to_string().to_string()));
                } else {
                    if cmd == "filter" {
                        let threshold = (parts
                            ...
                            .expect("IndexError: list index out of range"))
                            as i32;
                        current_iter = current_iter.filter(move |x, t| x > t);
                        results.push(DepylerValue::Str("ok".to_string().to_string()));
                    } else {
                        if cmd == "take" {
                            current_iter = current_iter.take(
                                (parts
                                    .get(1usize)
                                    .cloned()
                                    .expect("IndexError: list index out of range"))
                            ...
                            if cmd == "skip" {
                                current_iter = current_iter.skip(
                                    (parts
                                        .get(1usize)
                                        .cloned()
                                        .expect("IndexError: list index out of range"))
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: generic_iterator_cli.py <command>[args...]");
        println!("{}", "Commands: range, list, demo");
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "range";
    let mut items: Vec<i32>;
    if _cse_temp_2 {
        let _cse_temp_3 = _cse_temp_0 < 4;
        if _cse_temp_3 {
            eprintln!("{}", "Usage: range <start><end>");
            std::process::exit(1i32)
        }
        let (start, end) = (
            (std::env::args()
        ...
        items = iter_range(start, end, 1i32).collect();
        println!(
            "{}",
            items
                .as_slice()
        ...
        let _cse_temp_4 = cmd == "list";
        let mut result;
        if _cse_temp_4 {
            let _cse_temp_5 = _cse_temp_0 < 3;
            if _cse_temp_5 {
                eprintln!("{}", "Usage: list <items>");
                std::process::exit(1i32)
            }
            items = std::env::args()
                .collect::<Vec<String>>()
            ...
            let _cse_temp_6 = cmd == "demo";
            if _cse_temp_6 {
                result = iter_range(1, 11, 1i32)
                    .map(move |x| x * x)
                    .filter(move |x| x % 2 == 0)
                    .take(3)
                    .collect();
                println!(
                    "{}",
                    format!("Squares of 1-10, even, first 3: {:?}", result)
                );
                let total = iter_range(1, 101, 1i32).sum();
                println!("{}", format!("Sum of 1-100: {:?}", total));
                let mut chained = iter_range(1, 4, 1i32)
                    .chain(iter_range(10, 13, 1i32))
                    .collect();
                println!("{}", format!("Chained: {:?}", chained));
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn test_simulate_iterator_examples() {
        assert_eq!(simulate_iterator(vec![]), vec![]);
        assert_eq!(simulate_iterator(vec![1]), vec![1]);
    }
}
