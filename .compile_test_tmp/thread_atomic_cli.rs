...
impl AtomicInt {
    ...
    pub fn add(&self, delta: i32) -> i32 {
        {
            let _context = self._lock.clone();
            {
                self._value = self._value.clone() + delta;
                return self._value.clone();
            }
        }
    }
    ...
    pub fn compare_and_swap(&self, expected: i32, new_value: i32) -> bool {
        {
            let _context = self._lock.clone();
            {
                if self._value.clone() == expected {
...
impl AtomicBool {
    ...
    pub fn get_and_set(&self, new_value: bool) -> bool {
        {
            let _context = self._lock.clone();
            {
                let old = self._value.clone();
...
pub struct AtomicReference {
    pub _value: DepylerValue,
    pub _lock: std::sync::Mutex<()>,
}
impl AtomicReference {
    pub fn new(_value: DepylerValue) -> Self {
        Self {
            _value: Default::default(),
            _lock: Default::default(),
        }
    }
    pub fn get(&self) -> DepylerValue {
        {
            let _context = self._lock.clone();
            {
                return self._value.clone();
    ...
    pub fn set(&self, value: &DepylerValue) {
        {
            let _context = self._lock.clone();
            {
                self._value = value;
    ...
    pub fn compare_and_swap(&self, expected: &DepylerValue, new_value: &DepylerValue) -> bool {
        {
            let _context = self._lock.clone();
            {
                if self._value.clone() == expected {
    ...
    pub fn get_and_set(&self, new_value: &DepylerValue) -> DepylerValue {
        {
            let _context = self._lock.clone();
            {
                let old = self._value.clone();
...
#[derive(Debug, Clone)]
pub struct SpinLock {
    pub _locked: AtomicBool,
}
impl SpinLock {
    pub fn new() -> Self {
        Self {
            _locked: Default::default(),
        }
    }
    pub fn try_lock(&self) -> bool {
        return self._locked.clone().compare_and_swap(false, true);
    }
    pub fn unlock(&self) {
        self._locked.clone().set(false);
    }
    pub fn is_locked(&self) -> bool {
        return self._locked.clone().get();
    }
}
#[derive(Debug, Clone)]
pub struct AtomicCounter {
    pub _value: AtomicInt,
    pub _increments: AtomicInt,
    pub _decrements: AtomicInt,
}
impl AtomicCounter {
    pub fn new() -> Self {
        Self {
            _value: Default::default(),
            _increments: Default::default(),
            _decrements: Default::default(),
        }
    }
    pub fn increment(&self) -> i32 {
        self._increments.clone().increment();
        return self._value.clone().increment();
    }
    pub fn decrement(&self) -> i32 {
        self._decrements.clone().increment();
        return self._value.clone().decrement();
    }
    pub fn get(&self) -> i32 {
        return self._value.clone().get();
    }
    pub fn reset(&self) -> i32 {
        return self._value.clone().get_and_set(0);
    }
    pub fn stats(&self) -> std::collections::HashMap<String, i32> {
        return {
            let mut map = std::collections::HashMap::new();
            map.insert("value".to_string(), self._value.clone().get());
            map.insert("increments".to_string(), self._increments.clone().get());
            map.insert("decrements".to_string(), self._decrements.clone().get());
            map
        };
    }
}
...
#[doc = "Execute atomic operations and return result."]
pub fn atomic_ops(initial: i32, ops: &Vec<String>) -> Result<i32, Box<dyn std::error::Error>> {
    let mut atom = AtomicInt::new(initial);
...
#[doc = "Execute CAS sequence and return results."]
#[doc = " Depyler: verified panic-free"]
pub fn cas_sequence(initial: i32, operations: &Vec<(i32, i32)>) -> Vec<bool> {
    let atom = AtomicInt::new(initial);
    let mut results: Vec<bool> = vec![];
    for (expected, new_val) in operations.iter().cloned() {
        let success = atom.compare_and_swap(expected, new_val);
        results.push(success);
    }
    results
}
#[doc = "Execute spinlock sequence."]
#[doc = " Depyler: verified panic-free"]
pub fn spinlock_sequence(ops: &Vec<String>) -> Vec<bool> {
    let lock = SpinLock::new();
    let mut results: Vec<bool> = vec![];
    for op in ops.iter().cloned() {
        if op == "try" {
            results.push(lock.try_lock());
        } else {
            if op == "unlock" {
                lock.unlock();
            } else {
                if op == "check" {
                    results.push(lock.is_locked());
                }
            }
        }
    }
...
#[doc = "Simulate counter with statistics."]
#[doc = " Depyler: verified panic-free"]
pub fn counter_simulation(ops: &Vec<String>) -> HashMap<String, i32> {
...
#[doc = "Find max using atomic compare-and-swap."]
pub fn atomic_max(values: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    if values.is_empty() {
        return Ok(0);
    }
    let result = AtomicInt::new(
        values
            .get(0usize)
            .cloned()
            .expect("IndexError: list index out of range"),
...
#[doc = "Find min using atomic compare-and-swap."]
pub fn atomic_min(values: &Vec<i32>) -> Result<i32, Box<dyn std::error::Error>> {
    if values.is_empty() {
        return Ok(0);
    }
    let result = AtomicInt::new(
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &args.command {
        Some(Commands::Ops {
            ...
            let mut result = atomic_ops(initial, &ops)?;
        ...
        Some(Commands::Cas {
            ...
            for op in operations {
                let parts = op
                    ...
                    .map(|s| s.to_string())
                    .collect::<Vec<String>>();
                operations.push(DepylerValue::Str(format!(
                    "{:?}",
                    (
                        (parts
                            .get(0usize)
            ...
            let results = cas_sequence(initial, &operations);
            println!("{}", format!("Results: {:?}", results));
        }
        Some(Commands::Counter { ref ops, .. }) => {
            let stats = counter_simulation(&ops);
            println!("{}", format!("Stats: {:?}", stats));
        }
        Some(Commands::Max { ref values, .. }) => {
            let values = values.clone();
            let mut result = atomic_max(&values)?;
            println!("{}", format!("Max: {:?}", result));
        }
...
mod tests {
    ...
    #[test]
    fn test_spinlock_sequence_examples() {
        assert_eq!(spinlock_sequence(vec![]), vec![]);
        assert_eq!(spinlock_sequence(vec![1]), vec![1]);
    }
    #[test]
    fn test_atomic_max_examples() {
        assert_eq!(atomic_max(&vec![]), 0);
        assert_eq!(atomic_max(&vec![1]), 1);
        assert_eq!(atomic_max(&vec![1, 2, 3]), 3);
    }
    #[test]
    fn test_atomic_min_examples() {
        assert_eq!(atomic_min(&vec![]), 0);
        assert_eq!(atomic_min(&vec![1]), 1);
        assert_eq!(atomic_min(&vec![1, 2, 3]), 3);
    }
}
