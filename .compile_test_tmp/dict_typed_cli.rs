...
#[doc = "Count word occurrences with typed dict."]
pub fn create_word_counts(text: &str) -> Result<HashMap<String, i32>, Box<dyn std::error::Error>> {
        ...
        .collect::<Vec<String>>()
    {
        ...
        if counts.get(&word_lower).is_some() {
            {
                let _key = word_lower;
            ...
            counts.insert(word_lower.to_string().clone(), 1);
        }
    }
    Ok(counts)
}
#[doc = "Get unique words as typed list."]
...
pub fn get_unique_words(text: &str) -> Vec<String> {
    ...
    {
        let word_lower = word.to_lowercase();
        if seen.get(&word_lower).is_none() {
            seen.insert(word_lower.to_string().clone(), true);
            words.push(word_lower);
        }
    }
    words
}
#[doc = "Format counts for display."]
#[doc = " Depyler: verified panic-free"]
pub fn format_counts(counts: &std::collections::HashMap<String, i32>) -> String {
    ...
    for word in {
        ...
        items.push(format!(
            "{:?}:{}",
            word,
            counts.get(&word).cloned().unwrap_or_default()
        ));
    }
    items.join(",")
}
#[doc = "Main entry point with typed dict examples."]
#[doc = " Depyler: verified panic-free"]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Count { .. }));
    match &args.command {
        Some(Commands::Count { ref text, .. }) => {
            let mut counts = create_word_counts(&text)?;
            let result = format_counts(&counts);
            println!("{}", format!("Counts: {:?}", result));
        }
        Some(Commands::Unique { ref text, .. }) => {
            let words = get_unique_words(&text);
            println!("{}", format!("Unique: {}", words.join(",").display()));
        }
        Some(Commands::Stats { ref text, .. }) => {
            let mut counts = create_word_counts(&text)?;
            let unique = get_unique_words(&text);
            let _cse_temp_3 = counts.values().cloned().sum::<i32>();
            let total = _cse_temp_3;
            println!("{}", format!("Total: {}", total));
            println!("{}", format!("Unique: {}", unique.len() as i32));
        }
