...
pub enum Observer<T: Clone> {
    FunctionObserver(DepylerValue, DepylerValue, DepylerValue),
    Subject(Vec<Observer<T>>, bool, Option<Box<dyn std::error::Error>>),
}
impl<T: Clone> Observer<T> {
    pub fn is_functionobserver(&self) -> bool {
        matches!(self, Self::FunctionObserver(..))
    }
...
pub struct Subscription {
    pub _unsubscribe: impl Fn(),
    pub _closed: bool,
}
impl Subscription {
    pub fn new(_unsubscribe: impl Fn(), _closed: bool) -> Self {
        Self {
            _unsubscribe,
            _closed,
        }
...
impl<T: Clone> BehaviorSubject<T> {
    ...
    pub fn on_next(&mut self, value: T) {
        self._value = value;
        super_().on_next(value);
    }
    pub fn subscribe(&self, args: Vec<String>) -> Subscription {
        let sub = super_().subscribe(args);
        if args && {
            let _base = &args;
            let _idx = 0;
            if _idx < 0 {
...
#[doc = "Create Observable from values."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn of<T: Clone>(values: &[String]) -> Observable<T> {
    let subscribe_fn = move |observer: Observer<()>| -> &dyn Fn() {
...
#[doc = "Create Observable from list."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn from_list<T: Clone>(lst: &Vec<T>) -> Observable<T> {
    of(&[&lst])
}
#[doc = "Create empty Observable."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn empty() -> Observable<DepylerValue> {
    let subscribe_fn = move |observer: Observer<String>| -> &dyn Fn() {
        observer.on_complete();
        return move || None;
    };
    Observable::new(subscribe_fn)
}
#[doc = "Simulate observable operations."]
pub fn simulate_observable(
    operations: &Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results: Vec<String> = vec![];
    let mut received: Vec<DepylerValue> = vec![];
    for op in operations.iter().cloned() {
        ...
        if cmd == "of" {
            ...
            let obs = of(&&values);
            obs.subscribe();
            results.push(format!("emitted={}", values.len() as i32));
        } else {
            if cmd == "map" {
                values = parts
                    .get(1usize)
                    ...
                    .map(|x| (x) as i32)
                    .collect::<Vec<_>>();
                of(&&values).map(move |x| x * 2).subscribe();
                results.push("mapped".to_string().to_string());
            } else {
                if cmd == "filter" {
                    values = parts
                        .get(1usize)
                        ...
                        .map(|x| (x) as i32)
                        .collect::<Vec<_>>();
                    of(&&values).filter(move |x| x % 2 == 0).subscribe();
                    results.push("filtered".to_string().to_string());
                } else {
                    if cmd == "received" {
                        ...
                        if cmd == "subject" {
                            let mut subj: Subject<i32> = Subject::new();
                            subj.subscribe();
                            subj.on_next(1);
                            subj.on_next(2);
                            subj.on_complete();
                            results.push("subject_done".to_string().to_string());
                        }
                    }
                }
            }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: event_observable_cli.py <command>");
...
mod tests {
    ...
    #[test]
    fn test_simulate_observable_examples() {
        assert_eq!(simulate_observable(vec![]), vec![]);
        assert_eq!(simulate_observable(vec![1]), vec![1]);
    }
}
