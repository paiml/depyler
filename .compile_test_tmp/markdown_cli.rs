...
struct Args {
    #[doc = "Input markdown file(- for stdin)"]
    input: Option<String>,
    
    #[doc = "Output HTML file(default: stdout)"]
    output: Option<String>,
    
    
    #[doc = "Wrap output in HTML document"]
    wrap: bool,
}
#[doc = "Convert markdown headers to HTML."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_headers(mut text: String) -> String {
    for i in {
        let step = (-1 as i32).abs() as usize;
        if step == 0 {
            panic!("range() arg 3 must not be zero");
        }
        (0..6).rev().step_by(step.max(1))
    } {
        let pattern = format!(
            "{}{}",
        ...
        text = DepylerRegexMatch::sub(pattern, format!("<h{}>\\1</h{}>", i, i), text);
    }
    text.to_string()
}
#[doc = "Convert **bold** to <strong>."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_bold(text: &str) -> String {
    DepylerRegexMatch::sub("\\*\\*(.+?)\\*\\*", "<strong>\\1</strong>", text)
}
#[doc = "Convert *italic* to <em>."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_italic(text: &str) -> String {
    DepylerRegexMatch::sub("\\*(.+?)\\*", "<em>\\1</em>", text)
}
#[doc = "Convert `code` to <code>."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_code(text: &str) -> String {
    DepylerRegexMatch::sub("`(.+?)`", "<code>\\1</code>", text)
}
#[doc = "Convert [text](url) to <a href>."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_links(text: &str) -> String {
    DepylerRegexMatch::sub("\\[(.+?)\\]\\((.+?)\\)", "<a href=\"\\2\">\\1</a>", text)
}
#[doc = "Convert unordered lists to HTML."]
#[doc = " Depyler: verified panic-free"]
pub fn parse_lists(text: &str) -> String {
    let mut in_list: bool = Default::default();
    let lines = text
        .split("\n")
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
    ...
    for line in lines.iter().cloned() {
        if line.starts_with("- ") {
            if !in_list {
                result.push(DepylerValue::Str("<ul>".to_string().to_string()));
                in_list = true;
            }
            ...
            result.push(DepylerValue::Str(line.to_string()));
        }
    }
    if in_list {
        result.push(DepylerValue::Str("</ul>".to_string().to_string()));
...
#[doc = "Wrap standalone text in <p>tags."]
#[doc = " Depyler: verified panic-free"]
pub fn parse_paragraphs(text: &str) -> String {
    let lines = text
        .split("\n")
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
...
#[doc = "Convert markdown text to HTML."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
pub fn main() -> Result<(), std::io::Error> {
    let mut markdown: String = Default::default();
