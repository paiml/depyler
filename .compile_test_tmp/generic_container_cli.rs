...
pub struct PyBox<T: Clone> {
    pub value: T,
}
impl<T: Clone> PyBox<T> {
    ...
    pub fn map<U: Clone>(&self, f: impl Fn(T) -> U) -> PyBox<U> {
        return PyBox::new(f(self.value.clone()));
    }
    pub fn flat_map<U: Clone>(&self, f: impl Fn(T) -> Box<U>) -> PyBox<U> {
        return f(self.value.clone());
    }
}
#[derive(Debug, Clone, PartialEq)]
pub enum PyOption<T: Clone> {
    PySome(T),
    NoneValue,
}
impl<T: Clone> PyOption<T> {
    pub fn is_pysome(&self) -> bool {
        matches!(self, Self::PySome(..))
    }
    pub fn is_nonevalue(&self) -> bool {
        matches!(self, Self::NoneValue(..))
    }
    pub fn new_pysome(value: T) -> Self {
        Self::PySome(value)
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct PyVec<T: Clone> {
    pub items: Vec<T>,
}
impl<T: Clone> PyVec<T> {
    ...
    pub fn pop(&self) -> PyOption<T> {
        if self.items.clone() {
            return PySome::new(self.items.pop().unwrap_or_default());
        };
        return NoneValue::new();
    }
    pub fn get(&self, index: i32) -> PyOption<T> {
        if 0 <= index && index < (self.items.clone().len() as i32) {
            return PySome::new({
    ...
    pub fn first(&self) -> PyOption<T> {
        return self.get(0);
    }
    pub fn last(&self) -> PyOption<T> {
        if self.items.clone() {
            return self.get((self.items.clone().len() as i32).saturating_sub(1));
        };
        return NoneValue::new();
    }
    pub fn map<U: Clone>(&self, f: impl Fn(T) -> U) -> PyVec<U> {
        return PyVec::new(
            self.items
                .clone()
                .into_iter()
    ...
    pub fn filter(&self, predicate: impl Fn(T) -> bool) -> PyVec<T> {
        return PyVec::new(
            self.items
                .clone()
                .into_iter()
    ...
    pub fn fold<U: Clone>(&self, initial: U, f: impl Fn(U, T) -> U) -> U {
        let mut result = initial;
        for item in self.items.clone() {
            let result = f(result, item);
        }
        return result;
    }
    pub fn find(&self, predicate: impl Fn(T) -> bool) -> PyOption<T> {
        for item in self.items.clone() {
            if predicate(item) {
                return PySome::new(item);
            };
        }
        return NoneValue::new();
    }
    ...
    pub fn iter(&self) -> VecIterator<T> {
        return VecIterator::new(self.items.clone());
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl<T: Clone> VecIterator<T> {
    pub fn new(items: Vec<T>, index: i32) -> Self {
        Self { items, index }
    }
    pub fn iter(&self) -> VecIterator<T> {
        return self;
    ...
    pub fn next(&mut self) -> T {
        if self.index.clone() >= (self.items.clone().len() as i32) {
            panic!("Exception: {}", StopIteration);
        };
        let item = {
            let _base = &self.items.clone();
            let _idx = self.index.clone();
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
                _base[_idx as usize].clone()
        ...
        self.index = self.index.clone() + 1;
        return item;
    }
    pub fn next(&mut self) -> PyOption<T> {
        if self.index.clone() >= (self.items.clone().len() as i32) {
            return NoneValue::new();
        };
        let item = {
...
impl<T: Clone, U: Clone> Pair<T, U> {
    ...
    pub fn map_first(&self, f: impl Fn(T) -> DepylerValue) -> Pair<DepylerValue, U> {
        return Pair::new(f(self.first.clone()), self.second.clone());
    }
    pub fn map_second(&self, f: impl Fn(U) -> DepylerValue) -> Pair<T, DepylerValue> {
        return Pair::new(self.first.clone(), f(self.second.clone()));
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct Triple<T: Clone, U: Clone> {
    pub first: T,
    pub second: U,
    pub third: DepylerValue,
}
impl<T: Clone, U: Clone> Triple<T, U> {
    pub fn new(first: T, second: U, third: DepylerValue) -> Self {
        Self {
            first,
            second,
            third,
    ...
    pub fn to_tuple(&self) -> (T, U, DepylerValue) {
        return (self.first.clone(), self.second.clone(), self.third.clone());
    }
}
...
#[doc = "Create Option from nullable value."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn option_from<T: Clone>(value: &Option<T>) -> Option<T> {
    if value.is_none() {
        return NoneValue::new();
    }
    PySome::new(value)
}
#[doc = "Zip two vectors with a combining function."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn zip_with<'a, 'b, T: Clone, U: Clone>(
    vec1: &'a Vec<T>,
    vec2: &'b Vec<U>,
    f: impl Fn(T, U) -> DepylerValue,
) -> Vec<DepylerValue> {
    let mut result: Vec<DepylerValue> = vec![];
    for i in 0..(depyler_min((vec1.len() as i32).clone(), (vec2.len() as i32).clone())) {
        let a = vec1.get(&i).cloned().unwrap();
        let b = vec2.get(&i).cloned().unwrap();
        result.push(f(a.as_ref().unwrap(), b.as_ref().unwrap()));
    }
    PyVec::new(result)
}
#[doc = "Flatten nested Vec."]
#[doc = " Depyler: verified panic-free"]
pub fn flatten<T: Clone>(nested: &Vec<Vec<T>>) -> Vec<T> {
    let mut result: Vec<T> = vec![];
    for inner in nested.items {
        result.extend(inner.items.iter().cloned());
    }
    PyVec::new(result)
}
#[doc = "Partition Vec into two based on predicate."]
#[doc = " Depyler: verified panic-free"]
pub fn partition<T: Clone>(vec: &Vec<T>, predicate: impl Fn(T) -> bool) -> Pair<Vec<T>, Vec<T>> {
    ...
    Pair::new(PyVec::new(left), PyVec::new(right))
}
#[doc = "Simulate container operations."]
pub fn simulate_container(
    operations: &Vec<String>,
) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    let mut results = vec![];
    let mut vec: Vec<i32> = PyVec::new();
    let mut opt: Option<i32> = NoneValue::new();
                                ...
                                if cmd == "unwrap" {
                                    ...
                                    if cmd == "is_some" {
                                        ...
                                        if cmd == "first" {
                                            results.push(DepylerValue::Str(format!(
                                                "{:?}",
                                                (vec.first().unwrap_or(-1)).to_string()
                                            )));
                                        } else {
                                            if cmd == "last" {
                                                results.push(DepylerValue::Str(format!(
                                                    "{:?}",
                                                    (vec.last().unwrap_or(-1)).to_string()
                                                )));
                                            }
                                        }
                                    }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut opt: Option<String> = Default::default();
    let _cse_temp_0 = std::env::args().collect::<Vec<String>>().len() as i32;
    let _cse_temp_1 = _cse_temp_0 < 2;
    if _cse_temp_1 {
        println!("{}", "Usage: generic_container_cli.py <command>[args...]");
        println!("{}", "Commands: box, option, vec");
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "box";
    if _cse_temp_2 {
        let _cse_temp_3 = _cse_temp_0 < 3;
        if _cse_temp_3 {
            eprintln!("{}", "Usage: box <value>");
            std::process::exit(1i32)
        }
        let r#box = PyBox::new(
            std::env::args()
        ...
        println!("{}", format!("Box({})", r#box.unwrap()));
    } else {
        let _cse_temp_4 = cmd == "option";
        if _cse_temp_4 {
            ...
            if opt.is_some() {
                println!("{}", format!("Some({})", opt.unwrap()));
            } else {
                println!("{}", "None");
            }
        } else {
            let _cse_temp_6 = cmd == "vec";
            if _cse_temp_6 {
                let mut vec: Vec<String> = PyVec::new();
                for arg in {
                ...
                println!("{}", format!("Vec({})", vec.items.join(", ").display()));
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn test_simulate_container_examples() {
        assert_eq!(simulate_container(vec![]), vec![]);
        assert_eq!(simulate_container(vec![1]), vec![1]);
    }
}
