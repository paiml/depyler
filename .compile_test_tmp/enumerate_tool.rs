...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Index { .. }));
    match &args.command {
        Some(Commands::Index { ref text, .. }) => {
            result = "".to_string();
            let mut idx = 0;
            while idx < text.len() as i32 {
                ...
                result = format!(
                    "{}{}",
                    format!("{}{}", format!("{}{}", result, (idx).to_string()), ":"),
                    {
                        let base = &text;
                        let idx: i32 = idx;
        ...
        Some(Commands::Start {
            ...
            while idx < text.len() as i32 {
                ...
                result = format!(
                    ...
                    format!(
                        "{}{}",
                        format!("{}{}", result, (offset + idx).to_string()),
                        ":"
                    ),
                    {
                        let base = &text;
        ...
        Some(Commands::Reverse { ref text, .. }) => {
            result = "".to_string();
