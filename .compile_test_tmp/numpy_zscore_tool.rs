...
enum Commands {
    #[default]
    __DepylerNone,
    Zscore3 { a: f64, b: f64, c: f64 },
    Zscore4 { a: f64, b: f64, c: f64, d: f64 },
    Zscore2 { a: f64, b: f64 },
}
#[derive(Default)]
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Zscore3 { .. }));
    match &args.command {
        Some(Commands::Zscore3 {
            ref a,
            ref b,
            ref c,
            ..
            ...
            let mut mean = arr.mean().unwrap();
            let mut std = arr.stddev().unwrap();
            let mut result = if std > 0f64 {
                Vector::from_vec(
                    Vector::from_vec(arr.as_slice().iter().map(|&x| x - mean).collect())
                        .as_slice()
                        .iter()
                        .map(|&x| x / std)
                        .collect(),
        ...
        Some(Commands::Zscore2 { ref a, ref b, .. }) => {
            let a = *a;
            let b = *b;
