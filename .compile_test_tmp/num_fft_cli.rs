...
impl Complex {
    ...
    pub fn add(&self, other: &Complex) -> Complex {
        return Complex::new(
            self.real.clone() + other.real,
            self.imag.clone() + other.imag,
        );
    }
    pub fn sub(&self, other: &Complex) -> Complex {
        return Complex::new(
            self.real.clone() - (other.real as f64),
            self.imag.clone() - (other.imag as f64),
        );
    }
    pub fn mul(&self, other: &Complex) -> Complex {
        return Complex::new(
            self.real.clone() * other.real - self.imag.clone() * other.imag,
            self.real.clone() * other.imag + self.imag.clone() * other.real,
        );
    }
    pub fn div(&self, other: f64) -> Complex {
        return Complex::new(self.real.clone() / other, self.imag.clone() / other);
    }
    pub fn magnitude(&self) -> f64 {
        return ({
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ({ self.real.clone() } as i32)
                    .checked_pow({ 2 } as u32)
                    .expect("Power operation overflowed")
            } else {
                ({ self.real.clone() } as f64).powf({ 2 } as f64) as i32
            }
        } + {
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ({ self.imag.clone() } as i32)
                    .checked_pow({ 2 } as u32)
                    .expect("Power operation overflowed")
            } else {
                ({ self.imag.clone() } as f64).powf({ 2 } as f64) as i32
            }
        } as f64)
            .sqrt();
    }
    pub fn phase(&self) -> f64 {
        return math.atan2(self.imag.clone(), self.real.clone());
    }
    pub fn conjugate(&self) -> Complex {
        return Complex::new(self.real.clone(), -self.imag.clone());
    }
    pub fn fmt(&self) -> String {
        if self.imag.clone() >= 0f64 {
            return format!("{}+{}j", self.real.clone(), self.imag.clone());
        };
        return format!("{}{}j", self.real.clone(), self.imag.clone());
    }
}
...
#[doc = "e^(j*theta) = cos(theta) + j*sin (theta)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn exp_j(theta: f64) -> Complex {
    Complex::new((theta as f64).cos(), (theta as f64).sin())
}
#[doc = "Cooley-Tukey FFT(radix-2)."]
pub fn fft(x: &mut Vec<Complex>) -> Result<Vec<Complex>, Box<dyn std::error::Error>> {
    let mut n: DepylerValue = Default::default();
    ...
    let _cse_temp_2 = n & n - 1;
    let _cse_temp_3 = _cse_temp_2 != 0;
    if _cse_temp_3 {
        let _cse_temp_4 = 1 << n - 1.bit_length();
        let next_pow2 = _cse_temp_4;
        let _cse_temp_5 = vec![Complex::new(0, 0); (next_pow2 - n) as usize];
        x = x
            .iter()
            .chain(_cse_temp_5.iter())
            .cloned()
            .collect::<Vec<_>>();
        n = next_pow2;
    }
    let bits = n - 1.bit_length();
    let _cse_temp_6 = vec![Complex::new(0, 0); n as usize];
    let mut result = _cse_temp_6.clone();
    for i in 0..(n) {
        let rev = i64::from_str_radix(
            &{
                let base = {
                    let s = {
                        let base = &format!("0b{:b}", i);
    ...
    while length <= n {
        ...
        let w_n = exp_j(-2 * std::f64::consts::PI / length);
        for i in {
            let step = length as usize;
            if step == 0 {
            ...
            for j in 0..(half) {
                ...
                result.insert((i + j) as usize, u + t);
                result.insert((i + j + half) as usize, u - t);
                w = w * w_n;
            }
        }
        length = length * 2;
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn ifft(x: &Vec<Complex>) -> Result<Vec<Complex>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = x.len() as i32;
    let n = _cse_temp_0;
    let conj = x.iter().cloned().map(|c| c.conjugate()).collect::<Vec<_>>();
    let result = fft(&mut conj)?;
    Ok(result
        .iter()
        .cloned()
        .map(|c| c.conjugate() / n)
...
#[doc = "Direct DFT(O(n^2), for reference)."]
#[doc = " Depyler: proven to terminate"]
pub fn dft(x: &Vec<Complex>) -> Result<Vec<Complex>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = x.len() as i32;
    let n = _cse_temp_0;
    let mut result = vec![];
    for k in 0..(n) {
        let mut total = Complex::new(0, 0);
        for j in 0..(n) {
            let angle = -2 * std::f64::consts::PI * k * j / n;
            total = total
                + x.get(j as usize)
                    .cloned()
                    .expect("IndexError: list index out of range")
        ...
        result.push(DepylerValue::Str(format!("{:?}", total)));
    }
    Ok(result)
}
#[doc = "Get magnitude spectrum from FFT result."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn magnitude_spectrum(x: &Vec<Complex>) -> Vec<f64> {
    x.iter().cloned().map(|c| c.magnitude()).collect::<Vec<_>>()
}
#[doc = "Get phase spectrum from FFT result."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn phase_spectrum(x: &Vec<Complex>) -> Vec<f64> {
    x.iter().cloned().map(|c| c.phase()).collect::<Vec<_>>()
}
#[doc = "Get power spectrum(magnitude squared)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn power_spectrum(x: &Vec<Complex>) -> Vec<f64> {
    x.iter()
        .cloned()
        .map(|c| {
            if 2 >= 0 && (2 as i64) <= (u32::MAX as i64) {
                ...
                ({ c.magnitude() } as f64).powf({ 2 } as f64) as i32
            }
        })
        .collect::<Vec<_>>()
}
#[doc = "Get frequency bin centers."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn frequency_bins(n: i32, sample_rate: f64) -> Vec<f64> {
    (0..(n))
        .into_iter()
        .map(|k| ((k * sample_rate) as f64) / ((n) as f64))
        .collect::<Vec<_>>()
}
#[doc = "Generate signal from frequency components."]
#[doc = " Depyler: proven to terminate"]
pub fn generate_signal<'b, 'a, 'c>(
    frequencies: &'a Vec<f64>,
    amplitudes: &'b Vec<f64>,
    phases: &'c Vec<f64>,
    n_samples: i32,
    sample_rate: f64,
) -> Result<Vec<Complex>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = vec![Complex::new(0, 0); n_samples as usize];
    let mut signal = _cse_temp_0.clone();
    for i in 0..(n_samples) {
        let t = ((i) as f64) / ((sample_rate) as f64);
        let mut val = 0.0;
        for (freq, amp, phase) in frequencies
            .into_iter()
            .zip(amplitudes.into_iter())
            .zip(phases.into_iter())
        {
            val = val + amp * (2 * std::f64::consts::PI * freq * t + phase as f64).cos();
        }
        signal.insert((i) as usize, Complex::new(val, 0f64));
    }
    Ok(signal)
}
#[doc = "Find dominant frequency and its magnitude."]
#[doc = " Depyler: proven to terminate"]
pub fn find_dominant_frequency(
    spectrum: &Vec<Complex>,
    sample_rate: f64,
) -> Result<(f64, f64), Box<dyn std::error::Error>> {
    let _cse_temp_0 = spectrum.len() as i32;
    let n = _cse_temp_0;
    let magnitudes = magnitude_spectrum(&{
        let base = &spectrum;
        let stop_idx = ({
            let a = n;
            let b = 2;
    ...
    let _cse_temp_1 = *0..(magnitudes.len() as i32).iter().max().unwrap();
    let max_idx = _cse_temp_1;
    let _cse_temp_2 = (max_idx as f64) * sample_rate;
    let _cse_temp_3 = ((_cse_temp_2) as f64) / ((n) as f64);
    let freq = _cse_temp_3;
    Ok((
        freq,
        magnitudes
...
#[doc = "Convolution using FFT."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn convolution_fft<'b, 'a>(
    a: &'a Vec<Complex>,
    b: &'b Vec<Complex>,
) -> Result<Vec<Complex>, Box<dyn std::error::Error>> {
    let _cse_temp_0 = a.len() as i32;
    let _cse_temp_1 = b.len() as i32;
    let _cse_temp_2 = _cse_temp_0 + _cse_temp_1 - 1;
    let n = _cse_temp_2;
    let _cse_temp_3 = 1 << n - 1.bit_length();
    let next_pow2 = _cse_temp_3;
    let _cse_temp_4 = vec![Complex::new(0, 0f64); (next_pow2 - _cse_temp_0) as usize];
    let a_padded = a
        .iter()
        .chain(_cse_temp_4.iter())
        .cloned()
        .collect::<Vec<_>>();
    let _cse_temp_5 = vec![Complex::new(0, 0f64); (next_pow2 - _cse_temp_1) as usize];
    let b_padded = b
        .iter()
        .chain(_cse_temp_5.iter())
        .cloned()
        .collect::<Vec<_>>();
    let fft_a = fft(&mut a_padded)?;
    let fft_b = fft(&mut b_padded)?;
    let product = fft_a
        .into_iter()
        .zip(fft_b.into_iter())
        .into_iter()
...
#[doc = "Simulate FFT operations."]
pub fn simulate_fft(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for op in operations.iter().cloned() {
        ...
        let cmd = parts
            ...
            .cloned()
            .expect("IndexError: list index out of range");
        let mut signal: Vec<DepylerValue>;
        ...
        if cmd == "fft" {
            ...
            signal = values
                ...
                .iter()
                .cloned()
                .map(|v| Complex::new(v, 0f64))
                .collect::<Vec<_>>();
            spectrum = fft(&mut signal)?;
            let mags = spectrum
                .iter()
                .cloned()
                .map(|c| format!("{}", c.magnitude()))
                .collect::<Vec<_>>();
            results.push((mags).to_string());
        } else {
            if cmd == "ifft" {
                values = parts
                    .get(1usize)
                ...
                let reals = recovered
                    ...
                    .map(|c| format!("{}", c.real))
                    .collect::<Vec<_>>();
                results.push((reals).to_string());
            } else {
                if cmd == "dominant" {
                    signal = generate_signal(&vec![10.0], &vec![1.0], &vec![0.0], 64, 100.0)?;
                    spectrum = fft(&mut signal)?;
                    let (freq, mag) = find_dominant_frequency(&spectrum, 100.0)?;
                    results.push(format!("freq={:?}", freq));
                }
            }
        }
    }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: num_fft_cli.py <command>");
    ...
    if _cse_temp_2 {
        ...
        let spectrum = fft(&mut signal)?;
        let (freq, mag) = find_dominant_frequency(&spectrum, 100.0)?;
        println!(
            "{}",
            format!("Dominant frequency: {:?} Hz(magnitude: {:?})", freq, mag)
        );
        let recovered = ifft(&spectrum)?;
        println!(
            "{}",
...
mod tests {
    ...
    #[test]
    fn test_fft_examples() {
        assert_eq!(fft(vec![]), vec![]);
        assert_eq!(fft(vec![1]), vec![1]);
    }
    #[test]
    fn test_ifft_examples() {
        assert_eq!(ifft(vec![]), vec![]);
        assert_eq!(ifft(vec![1]), vec![1]);
    }
    #[test]
    fn test_dft_examples() {
        assert_eq!(dft(vec![]), vec![]);
        assert_eq!(dft(vec![1]), vec![1]);
    }
    #[test]
    fn quickcheck_magnitude_spectrum() {
        fn prop(x: Vec<()>) -> TestResult {
            let result = magnitude_spectrum(&x);
            if result < 0 {
    ...
    #[test]
    fn test_magnitude_spectrum_examples() {
        assert_eq!(magnitude_spectrum(vec![]), vec![]);
        assert_eq!(magnitude_spectrum(vec![1]), vec![1]);
    }
    #[test]
    fn test_phase_spectrum_examples() {
        assert_eq!(phase_spectrum(vec![]), vec![]);
        assert_eq!(phase_spectrum(vec![1]), vec![1]);
    }
    #[test]
    fn test_power_spectrum_examples() {
        assert_eq!(power_spectrum(vec![]), vec![]);
        assert_eq!(power_spectrum(vec![1]), vec![1]);
    }
    #[test]
    fn test_simulate_fft_examples() {
        assert_eq!(simulate_fft(vec![]), vec![]);
        assert_eq!(simulate_fft(vec![1]), vec![1]);
    }
}
