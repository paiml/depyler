...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'secrets'(tracked in DEPYLER-0424)"] #[doc = "// NOTE: Map Python module 'string'(tracked in DEPYLER-0424)"] use std::collections::HashMap;
...
, Token {
    r#type: Option<String>, bytes: Option<i32>
}
, Randbelow {
    n: i32
}
...
#[doc = "Generate secure token. Depyler: proven to terminate"] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn cmd_token(bytes: i32, type: & str) {
    let _cse_temp_0 = r#type == "hex";
    if _cse_temp_0 {
    println!("{}", {
...
#[doc = "Generate secure password. Depyler: proven to terminate"] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn cmd_password() {
    let password  = (0..(length)).into_iter().map(| _ | {
    use rand::seq::SliceRandom;
    * alphabet.choose(&mut rand::thread_rng()).unwrap() }).collect::<Vec<_>>().join ("");
    println!("{}", password);
   
}
#[doc = "Generate random int below n. Depyler: proven to terminate"] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn cmd_randbelow(n: i32) {
    println!("{}", {
    use rand::Rng;
...
#[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () {
   ...
   ({ let mut map = HashMap::new();
    map.insert("token".to_string().to_string(), cmd_token);
    map.insert("password".to_string().to_string(), cmd_password);
    map.insert("randbelow".to_string().to_string(), cmd_randbelow);
    map
}
. get(& DepylerValue::Int(args.command as i64)).cloned().unwrap_or_default())(args);
    }
