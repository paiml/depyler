...
pub fn dropout(
    ...
    for xi in x.iter().cloned() {
        if (0.5_f64 as f64) < p {
            output.push(DepylerValue::Float(0.0 as f64));
        } else {
            output.push(DepylerValue::Str(format!("{:?}", xi * scale)));
        }
    }
    Ok(output)
}
...
pub fn cmd_forward() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match (|| -> Result<(), Box<dyn std::error::Error>> {
        output = dropout(
            ...
            data.get("p").cloned().unwrap_or(0.5),
            data.get("training").cloned().unwrap_or(true),
            &Some(data.get("random_state").cloned().unwrap_or(None)),
        )?;
        println!(
            "{}",
            format!("{:?}", {
