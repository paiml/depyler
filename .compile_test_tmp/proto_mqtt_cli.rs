...
pub const MqttPacket: () = ConnectPacket
    | ConnackPacket
    | PublishPacket
    | SubscribePacket
    | SubackPacket
...
impl PacketType {
    ...
    pub const CONNACK: i32 = 2;
    pub const PUBLISH: i32 = 3;
    pub const PUBACK: i32 = 4;
    pub const PUBREC: i32 = 5;
    pub const PUBREL: i32 = 6;
    pub const PUBCOMP: i32 = 7;
    pub const SUBSCRIBE: i32 = 8;
    pub const SUBACK: i32 = 9;
    pub const UNSUBSCRIBE: i32 = 10;
    pub const UNSUBACK: i32 = 11;
    pub const PINGREQ: i32 = 12;
    pub const PINGRESP: i32 = 13;
...
pub struct QoS {}
impl QoS {
    pub const AT_MOST_ONCE: i32 = 0;
    pub const AT_LEAST_ONCE: i32 = 1;
    pub const EXACTLY_ONCE: i32 = 2;
    pub fn new() -> Self {
        Self {}
    }
}
...
impl ConnectReturnCode {
    ...
    pub const UNACCEPTABLE_PROTOCOL: i32 = 1;
    pub const IDENTIFIER_REJECTED: i32 = 2;
    pub const SERVER_UNAVAILABLE: i32 = 3;
    pub const BAD_CREDENTIALS: i32 = 4;
    pub const NOT_AUTHORIZED: i32 = 5;
    pub fn new() -> Self {
        Self {}
    }
}
...
impl SubscribePacket {
    pub fn new(packet_id: i32, topics: Vec<(String, QoS)>) -> Self {
...
impl SubackPacket {
    pub fn new(packet_id: i32, return_codes: Vec<i32>) -> Self {
        Self {
            packet_id,
            return_codes,
        }
...
impl UnsubscribePacket {
    pub fn new(packet_id: i32, topics: Vec<String>) -> Self {
        Self { packet_id, topics }
    }
}
#[derive(Debug, Clone)]
...
impl MqttEncoder {
    ...
    pub fn encode_remaining_length(length: i32) -> Vec<u8> {
        ...
        return vec![0u8; (result) as usize];
    }
    pub fn encode_string(s: String) -> Vec<u8> {
        let encoded = s.encode("utf-8".to_string());
        return (encoded.len() as i32).to_bytes(2, "big".to_string()) + encoded;
    }
    pub fn encode_connect(packet: &ConnectPacket) -> Vec<u8> {
        let mut var_header = Vec::<u8>::new();
        var_header.extend(MqttEncoder::encode_string("MQTT".to_string()));
        var_header.push(4);
        let mut flags = 0;
        ...
        if packet.will_topic {
            ...
            let mut flags = flags | packet.will_qos & 3 << 3;
        ...
        var_header.extend(packet.keep_alive.to_bytes(2, "big".to_string()));
        let mut payload = Vec::<u8>::new();
        payload.extend(MqttEncoder::encode_string(packet.client_id));
        if packet.will_topic {
            payload.extend(MqttEncoder::encode_string(packet.will_topic));
            let will_msg = packet.will_message || b"";
            payload.extend((will_msg.len() as i32).to_bytes(2, "big".to_string()));
        ...
        let remaining = vec![0u8; (var_header) as usize] + vec![0u8; (payload) as usize];
        let mut fixed = vec![0u8; (vec![PacketType::CONNECT << 4]) as usize];
        let mut fixed = fixed + MqttEncoder::encode_remaining_length(remaining.len() as i32);
        return fixed + remaining;
    }
    pub fn encode_connack(packet: &ConnackPacket) -> Vec<u8> {
        let flags = if packet.session_present { 1 } else { 0 };
        return vec![0u8; (vec![PacketType::CONNACK << 4, 2, flags, packet.return_code]) as usize];
    }
    pub fn encode_publish(packet: &PublishPacket) -> Vec<u8> {
        let mut flags = packet.qos << 1;
        if packet.retain {
            let flags = flags | 1;
        };
        if packet.dup {
        ...
        let mut var_header = MqttEncoder::encode_string(packet.topic);
        if packet.qos > 0 && packet.packet_id.is_some() {
            let var_header = var_header + packet.packet_id.to_bytes(2, "big".to_string());
        };
        let remaining = var_header + packet.payload;
        let mut fixed = vec![0u8; (vec![PacketType::PUBLISH << 4 | flags]) as usize];
        let mut fixed = fixed + MqttEncoder::encode_remaining_length(remaining.len() as i32);
        return fixed + remaining;
    }
    pub fn encode_subscribe(packet: &SubscribePacket) -> Vec<u8> {
        let var_header = packet.packet_id.to_bytes(2, "big".to_string());
        let mut payload = Vec::<u8>::new();
        for (topic, qos) in packet.topics {
            payload.extend(MqttEncoder::encode_string(topic));
            payload.push(qos);
        }
        let remaining = var_header + vec![0u8; (payload) as usize];
        let mut fixed = vec![0u8; (vec![PacketType::SUBSCRIBE << 4 | 2]) as usize];
        let mut fixed = fixed + MqttEncoder::encode_remaining_length(remaining.len() as i32);
        return fixed + remaining;
    }
    pub fn encode_pingreq() -> Vec<u8> {
        return vec![0u8; (vec![PacketType::PINGREQ << 4, 0]) as usize];
    }
    pub fn encode_pingresp() -> Vec<u8> {
        return vec![0u8; (vec![PacketType::PINGRESP << 4, 0]) as usize];
    }
    pub fn encode_disconnect() -> Vec<u8> {
        return vec![0u8; (vec![PacketType::DISCONNECT << 4, 0]) as usize];
    }
}
#[derive(Debug, Clone)]
...
impl MqttDecoder {
    ...
    pub fn feed(&mut self, data: &Vec<u8>) {
        self.buffer = self.buffer.clone() + data;
    }
    pub fn decode_remaining_length(&self, pos: i32) -> (i32, i32) {
        let mut multiplier = 1;
        let mut value = 0;
        let mut consumed = 0;
        while pos + consumed < (self.buffer.clone().len() as i32) {
            let byte = {
                let _base = &self.buffer.clone();
                let _idx = pos + consumed;
                if _idx < 0 {
            ...
            let value = value + byte & 127 * multiplier;
            let consumed = consumed + 1;
            if byte & 128 == 0 {
                return (value, consumed);
            };
            let multiplier = multiplier * 128;
            if multiplier > 128 * 128 * 128 {
                panic!(
                    "Exception: {}",
                    MqttError::new("Malformed remaining length".to_string())
                );
            };
        }
        return (-1, 0);
    }
    pub fn decode_string(&self, pos: i32) -> (String, i32) {
        if pos + 2 > (self.buffer.clone().len() as i32) {
            panic!(
                "Exception: {}",
                MqttError::new("Incomplete string length".to_string())
            );
        };
        ...
        if pos + 2 + length > (self.buffer.clone().len() as i32) {
            panic!(
                "Exception: {}",
                MqttError::new("Incomplete string data".to_string())
            );
        };
        let s = {
            let s = &self.buffer.clone();
            let len = s.chars().count() as isize;
            let start_idx = (pos + 2) as isize;
            let stop_idx = (pos + 2 + length) as isize;
            let start = if start_idx < 0 {
                (len + start_idx).max(0) as usize
            } else {
                start_idx as usize
    ...
    pub fn decode(&self) -> (MqttPacket, i32) {
        if (self.buffer.clone().len() as i32) < 2 {
            return (None, 0);
        };
        let fixed_byte = {
            let _base = &self.buffer.clone();
        ...
        let packet_type = PacketType::new(fixed_byte >> 4);
        let flags = fixed_byte & 15;
        let (remaining_length, length_bytes) = self.decode_remaining_length(1);
        if remaining_length < 0 {
            return (None, 0);
        };
        let total_length = 1 + length_bytes + remaining_length;
        if (self.buffer.clone().len() as i32) < total_length {
            return (None, 0);
        };
        let mut pos = 1 + length_bytes;
        if packet_type == PacketType::CONNACK {
            ...
            let return_code = ConnectReturnCode::new({
                let _base = &self.buffer.clone();
                let _idx = pos + 1;
                if _idx < 0 {
                    _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            ...
            return (
                ConnackPacket::new(session_present, return_code),
                total_length,
            );
        } else {
            if packet_type == PacketType::PUBLISH {
                let (topic, consumed) = self.decode_string(pos);
                let mut pos = pos + consumed;
                let qos = QoS::new(flags >> 1 & 3);
                let retain = !flags & 1.is_empty();
                let dup = !flags & 8.is_empty();
                let mut packet_id = None;
                if qos > 0 {
                    let packet_id = i64::from_be_bytes({
                        let mut arr = [0u8; 8];
                        let bytes: &[u8] = {
                            let s = &self.buffer.clone();
                            let len = s.chars().count() as isize;
                ...
                let payload_end = 1 + length_bytes + remaining_length;
                let payload = {
                    let s = &self.buffer.clone();
                    let len = s.chars().count() as isize;
                    let start_idx = (pos) as isize;
                    let stop_idx = (payload_end) as isize;
                    let start = if start_idx < 0 {
                        (len + start_idx).max(0) as usize
                    } else {
                        start_idx as usize
                ...
                return (
                    PublishPacket::new(topic, payload, qos, retain, dup, packet_id),
                    total_length,
                );
            } else {
                if packet_type == PacketType::SUBACK {
                    ...
                    let pos = pos + 2;
                    let end = 1 + length_bytes + remaining_length;
                    let return_codes = {
                        let s = &self.buffer.clone();
                        let len = s.chars().count() as isize;
                        let start_idx = (pos) as isize;
                    ...
                    .into_iter()
                    .collect::<Vec<_>>();
                    return (SubackPacket::new(packet_id, return_codes), total_length);
                } else {
    ...
    pub fn consume(&mut self, length: i32) {
        self.buffer = {
            let s = &self.buffer.clone();
            let len = s.chars().count() as isize;
            let start_idx = (length) as isize;
            let start = if start_idx < 0 {
                (len + start_idx).max(0) as usize
            } else {
                start_idx as usize
...
#[doc = "Encode CONNECT packet."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_connect(client_id: &str) -> Vec<u8> {
    let packet = ConnectPacket::new(client_id, true, 60i32, None, None, None, None, false);
    MqttEncoder::encode_connect(packet)
}
#[doc = "Encode PUBLISH packet."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_publish<'a, 'b>(topic: &'a str, payload: &'b Vec<u8>, qos: i32) -> Vec<u8> {
    let packet = PublishPacket::new(
        topic,
        payload,
        QoS::new(qos, 1i32, 2i32),
        false,
        false,
        None,
    );
    MqttEncoder::encode_publish(packet)
}
#[doc = "Encode SUBSCRIBE packet."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_subscribe(packet_id: i32, topics: &Vec<(String, i32)>) -> Vec<u8> {
    let packet = SubscribePacket::new(
        packet_id,
        topics
            .iter()
            .cloned()
            .map(|(t, q)| (t, QoS::new(q, 1i32, 2i32)))
            .collect::<Vec<_>>(),
    );
    MqttEncoder::encode_subscribe(packet)
}
#[doc = "Decode MQTT packet."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn decode_packet(data: &Vec<u8>) -> MqttPacket {
    let mut decoder = MqttDecoder::new();
    decoder.feed(data);
    let (packet, __sanitized) = String::from_utf8_lossy(&decoder).to_string();
    packet
}
#[doc = "Simulate MQTT operations."]
pub fn simulate_mqtt(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for op in operations.iter().cloned() {
        ...
        if cmd == "connect" {
            ...
            results.push(format!("CONNECT len={}", data.len() as i32));
        } else {
            let mut topic: i32;
            if cmd == "publish" {
                let _split_parts = parts
                    ...
                    .map(|s| s.to_string())
                    .collect::<Vec<String>>();
                let mut topic = _split_parts.get(0).cloned().unwrap_or_default();
                let payload = _split_parts.get(1).cloned().unwrap_or_default();
                data = encode_publish(&topic, &payload.as_bytes().to_vec(), 0i32);
                results.push(format!("PUBLISH len={}", data.len() as i32));
            } else {
                if cmd == "subscribe" {
                    topic = parts
                        .get(1usize)
                        .cloned()
                        .expect("IndexError: list index out of range");
                    data = encode_subscribe(1, &vec![(topic, 0)]);
                    results.push(format!("SUBSCRIBE len={}", data.len() as i32));
                } else {
                    if cmd == "ping" {
                        data = MqttEncoder::encode_pingreq();
                        results.push(format!("PINGREQ len={}", data.len() as i32));
                    } else {
                        if cmd == "disconnect" {
                            data = MqttEncoder::encode_disconnect();
                            results.push(format!("DISCONNECT len={}", data.len() as i32));
                        }
                    }
                }
            }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: proto_mqtt_cli.py <command>[args]");
    ...
    if _cse_temp_4 {
        ...
        println!("{}", format!("CONNECT packet: {} bytes", data.len() as i32));
    } else {
        let _cse_temp_5 = cmd == "publish";
        ...
        if _cse_temp_7 {
            ...
            println!("{}", format!("PUBLISH packet: {} bytes", data.len() as i32));
        } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_mqtt_examples() {
        assert_eq!(simulate_mqtt(vec![]), vec![]);
        assert_eq!(simulate_mqtt(vec![1]), vec![1]);
    }
}
