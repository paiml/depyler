...
#[doc = "Count word frequencies. Depyler: proven to terminate"]
...
pub fn cmd_count() {
    ...
    let words = text
        ...
        .map(|s| s.to_string())
        .collect::<Vec<String>>();
    let counter = words.into_iter().fold(HashMap::new(), |mut acc, item| {
...
#[doc = "Show top N words. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_top(n: i32) {
    let text = {
        use std::io::Read;
        let mut buffer = String::new();
        std::io::stdin().read_to_string(&mut buffer).unwrap();
    ...
    for (word, count) in {
        let mut entries: Vec<_> = counter.iter().map(|(k, v)| (k.clone(), *v)).collect();
        entries.sort_by(|a, b| b.1.cmp(&a.1));
        entries.into_iter().take(n as usize).collect::<Vec<_>>()
    } {
        println!("{}", format!("{:?}: {:?}", word, count));
    }
}
#[doc = "Count character frequencies. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_chars() {
    let text = {
        use std::io::Read;
    ...
    .trim()
    .to_string();
    let counter = text.chars().fold(HashMap::new(), |mut acc, item| {
        *acc.entry(item).or_insert(0) += 1;
        acc
    });
    for (char, count) in {
