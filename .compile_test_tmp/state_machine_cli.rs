...
impl<C: Clone> State<C> {
    pub fn is_idlestate(&self) -> bool {
        matches!(self, Self::IdleState(..))
    }
    pub fn is_selectingstate(&self) -> bool {
        matches!(self, Self::SelectingState(..))
    }
    pub fn is_insufficientstate(&self) -> bool {
        matches!(self, Self::InsufficientState(..))
    }
    pub fn is_dispensingstate(&self) -> bool {
        matches!(self, Self::DispensingState(..))
    }
    pub fn is_changestate(&self) -> bool {
        matches!(self, Self::ChangeState(..))
    }
    pub fn is_draftstate(&self) -> bool {
        matches!(self, Self::DraftState(..))
    }
    pub fn is_reviewstate(&self) -> bool {
        matches!(self, Self::ReviewState(..))
    }
    pub fn is_approvedstate(&self) -> bool {
        matches!(self, Self::ApprovedState(..))
    }
    pub fn is_publishedstate(&self) -> bool {
        matches!(self, Self::PublishedState(..))
    }
    pub fn is_archivedstate(&self) -> bool {
        matches!(self, Self::ArchivedState(..))
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct StateMachine<C: Clone> {
    pub context: C,
    pub initial_state: State<C>,
    pub current_state: State<C>,
    pub history: Vec<String>,
    pub listeners: Vec<impl Fn(String, String)>,
}
impl<C: Clone> StateMachine<C> {
    pub fn new(
        context: C,
        initial_state: State<C>,
        current_state: State<C>,
        history: Vec<String>,
        listeners: Vec<impl Fn(String, String)>,
    ) -> Self {
        Self {
            context,
            initial_state,
    ...
    pub fn process(&mut self, event: String) -> bool {
        let next_state = self
            .current_state
            .clone()
            .handle(self.context.clone(), event);
        if next_state.is_some() {
            let old_name = self.current_state.clone().name();
            self.current_state.clone().on_exit(self.context.clone());
            self.current_state = next_state;
            self.history.push(next_state.name());
            next_state.on_enter(self.context.clone());
    ...
    pub fn add_listener(&mut self, listener: impl Fn(String, String)) {
        self.listeners.push(listener);
    }
    pub fn state_name(&self) -> String {
        return self.current_state.clone().name();
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct VendingContext {
    pub balance: i32,
    pub selected_item: String,
    pub item_prices: std::collections::HashMap<String, i32>,
...
impl VendingContext {
    pub fn new(
        ...
        item_prices: std::collections::HashMap<String, i32>,
...
pub struct VendingMachine {
    pub context: DepylerValue,
    pub machine: DepylerValue,
}
impl VendingMachine {
    pub fn new() -> Self {
        Self {
    ...
    pub fn insert_coin(&self, amount: i32) -> bool {
        return self.machine.clone().process(format!("insert:{}", amount));
    }
    pub fn select_item(&self, item: String) -> bool {
        return self.machine.clone().process(format!("select:{}", item));
    }
    pub fn cancel(&self) -> bool {
        return self.machine.clone().process("cancel".to_string());
    }
    pub fn take_item(&self) -> bool {
        return self.machine.clone().process("take".to_string());
    }
    pub fn take_change(&self) -> bool {
        return self.machine.clone().process("take_change".to_string());
    }
    pub fn state(&self) -> String {
        return self.machine.clone().state_name;
    }
    pub fn balance(&self) -> i32 {
        return self.context.clone().balance;
    }
    pub fn dispensed(&self) -> String {
        return self.context.clone().dispensed;
    }
}
#[derive(Debug, Clone, PartialEq)]
...
impl DocumentWorkflow {
    pub fn new(_title: String, _author: String) -> Self {
        Self {
            context: Default::default(),
            machine: Default::default(),
        }
    }
    pub fn edit(&self, content: String) {
        self.context.clone().content = content;
    }
    pub fn submit(&self) -> bool {
        return self.machine.clone().process("submit".to_string());
    }
    pub fn approve(&self) -> bool {
        return self.machine.clone().process("approve".to_string());
    }
    pub fn reject(&self) -> bool {
        return self.machine.clone().process("reject".to_string());
    }
    pub fn publish(&self) -> bool {
        return self.machine.clone().process("publish".to_string());
    }
    pub fn archive(&self) -> bool {
        return self.machine.clone().process("archive".to_string());
    }
    pub fn state(&self) -> String {
        return self.machine.clone().state_name;
    }
...
#[doc = "Simulate state machine operations."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn simulate_machine<'a, 'b>(machine_type: &'a str, events: &'b Vec<String>) -> Vec<String> {
    let mut results = vec![];
    let _cse_temp_0 = (*machine_type) == "vending";
    if _cse_temp_0 {
        let vm = VendingMachine::new();
        for event in events.iter().cloned() {
            if event == "state" {
                results.push(DepylerValue::Str(format!("{:?}", vm.state())));
            } else {
                if event == "balance" {
                    results.push(DepylerValue::Str(format!(
                        "{:?}",
                        (vm.balance()).to_string()
                    )));
                } else {
                    if event == "dispensed" {
                        results.push(DepylerValue::Str(format!(
                            "{:?}",
                            if vm.dispensed().is_empty() {
                                "none".to_string().to_string()
                            } else {
                                vm.dispensed().to_string()
                            }
                        )));
                    } else {
                        if event.starts_with("insert:") {
                            vm.insert_coin(
                                (event
                                    .split(":")
                                    .map(|s| s.to_string())
                            ...
                            if event.starts_with("select:") {
                                ...
                                if event == "take" {
                                    ...
                                    if event == "take_change" {
                                        vm.take_change();
                                        results
                                            .push(DepylerValue::Str("ok".to_string().to_string()));
                                    } else {
                                        if event == "cancel" {
                                            vm.cancel();
                                            results.push(DepylerValue::Str(
        ...
        let _cse_temp_1 = (*machine_type) == "document";
        if _cse_temp_1 {
            let doc = DocumentWorkflow::new("Test".to_string(), "Author".to_string());
            for event in events.iter().cloned() {
                if event == "state" {
                    results.push(DepylerValue::Str(format!("{:?}", doc.state())));
                } else {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: state_machine_cli.py <command>[args...]");
        println!("{}", "Commands: vending, document");
    ...
    let cmd = std::env::args()
        ...
        .cloned()
        .expect("IndexError: list index out of range");
    let _cse_temp_2 = cmd == "vending";
    if _cse_temp_2 {
        let vm = VendingMachine::new();
        println!("{}", format!("Initial state: {}", vm.state()));
        vm.insert_coin(50);
        println!(
            "{}",
            format!(
                "After insert 50: state={}, balance={}",
                vm.state(),
                vm.balance()
            )
        );
        ...
        vm.select_item("cola".to_string());
        println!(
            "{}",
            format!(
                "After select cola: state={}, dispensed={}",
                vm.state(),
                vm.dispensed()
            )
        );
        vm.take_item();
        println!("{}", format!("After take: state={}", vm.state()));
    } else {
        let _cse_temp_3 = cmd == "document";
        if _cse_temp_3 {
            let doc = DocumentWorkflow::new("My Document".to_string(), "Alice".to_string());
            println!("{}", format!("Initial state: {}", doc.state()));
            doc.edit("Hello World".to_string());
            doc.submit();
            println!("{}", format!("After submit: {}", doc.state()));
            doc.approve();
            println!("{}", format!("After approve: {}", doc.state()));
            doc.publish();
            println!("{}", format!("After publish: {}", doc.state()));
        }
    }
    Ok(())
}
