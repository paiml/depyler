...
#[doc = "Sort list ascending(bubble sort).\n\n   >>>sort_asc([3, 1, 2])\n    [1, 2, 3]\n   >>>sort_asc([5, 4, 3, 2, 1])\n    [1, 2, 3, 4, 5]\n   >>>sort_asc([1, 1, 1])\n    [1, 1, 1]\n    "]
pub fn sort_asc(nums: &Vec<DepylerValue>) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>> {
...
#[doc = "Sort list descending(bubble sort).\n\n   >>>sort_desc([3, 1, 2])\n    [3, 2, 1]\n   >>>sort_desc([1, 2, 3, 4, 5])\n    [5, 4, 3, 2, 1]\n    "]
pub fn sort_desc(
    nums: &Vec<DepylerValue>,
...
#[doc = "Sort strings alphabetically.\n\n   >>>sort_alpha([\"c\", \"a\", \"b\"])\n    ['a', 'b', 'c']\n   >>>sort_alpha([\"zebra\", \"apple\", \"mango\"])\n    ['apple', 'mango', 'zebra']\n    "]
pub fn sort_alpha(
    words: &Vec<DepylerValue>,
) -> Result<Vec<DepylerValue>, Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Asc { .. }));
    match &args.command {
        Some(Commands::Asc {
            ref a,
            ref b,
            ...
            let d = *d;
            let e = *e;
            let mut nums = sort_asc(&vec![a, b, c, d, e])?;
            println!(
                "{}",
                format!(
                    "{} {} {} {} {}",
        ...
        Some(Commands::Desc {
            ...
            let d = *d;
            let e = *e;
            let mut nums = sort_desc(&vec![a, b, c, d, e])?;
            println!(
                "{}",
                format!(
                    "{} {} {} {} {}",
        ...
        Some(Commands::Alpha {
            ...
            let words = sort_alpha(&vec![a, b, c])?;
            println!(
                "{}",
                format!(
                    "{} {} {}",
...
mod tests {
    ...
    fn quickcheck_sort_asc() {
        fn prop(nums: Vec<()>) -> TestResult {
            let input_len = nums.len();
            let result = sort_asc(&nums);
    ...
    #[test]
    fn quickcheck_sort_alpha() {
        fn prop(words: Vec<()>) -> TestResult {
            let input_len = words.len();
            let result = sort_alpha(&words);
            if result.len() != input_len {
                return TestResult::failed();
            }
            let result = sort_alpha(&words);
            ...
            let mut input_sorted = words.clone();
            input_sorted.sort();
            let mut result = sort_alpha(&words);
            result.sort();
            if input_sorted != result {
                return TestResult::failed();
            }
    ...
    #[test]
    fn test_sort_alpha_examples() {
        assert_eq!(sort_alpha(vec![]), vec![]);
        assert_eq!(sort_alpha(vec![1]), vec![1]);
    }
}
