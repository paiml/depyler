...
impl CacheStats {
    pub fn new(hits: i32, misses: i32, evictions: i32, size: i32, capacity: i32) -> Self {
        Self {
            hits,
            misses,
            evictions,
    ...
    pub fn hit_rate(&self) -> f64 {
        let total = self.hits.clone() + self.misses.clone();
        return if total > 0 {
            self.hits.clone() / total
        } else {
            0.0
        };
    }
...
pub struct Node<K: Clone, V: Clone> {
    pub key: K,
    pub value: V,
    pub prev: Option<Node_K__V_>,
    pub next: Option<Node_K__V_>,
}
impl<K: Clone, V: Clone> Node<K, V> {
    pub fn new(key: K, value: V, prev: Option<Node_K__V_>, next: Option<Node_K__V_>) -> Self {
        Self {
            key,
            value,
            prev,
...
pub struct LRUCache<K: Clone, V: Clone> {
    pub _capacity: i32,
    pub _cache: std::collections::HashMap<K, Node<K, V>>,
    pub _head: Node<K, V>,
    pub _tail: Node<K, V>,
    pub _stats: DepylerValue,
}
impl<K: Clone, V: Clone> LRUCache<K, V> {
    pub fn new(_capacity: i32) -> Self {
        Self {
            _capacity: 0,
            _cache: std::collections::HashMap::new(),
            _head: Default::default(),
            _tail: Default::default(),
            _stats: Default::default(),
        }
    }
    pub fn get(&self, key: K) -> Option<V> {
        if self._cache.clone().contains_key(&key) {
            let node = self._cache.clone().get(&key).cloned().unwrap_or_default();
            self._move_to_front(node);
            self._stats.clone().hits = self._stats.clone().hits + 1;
            return Some(node.value);
        };
        self._stats.clone().misses = self._stats.clone().misses + 1;
        return None;
    }
    pub fn put(&self, key: K, value: V) -> Option<V> {
        let mut evicted = None;
        if self._cache.clone().contains_key(&key) {
            let mut node = self._cache.clone().get(&key).cloned().unwrap_or_default();
            node.value = value;
            self._move_to_front(node);
        } else {
            if (self._cache.clone().len() as i32) >= self._capacity.clone() {
                let evicted = self._evict_lru();
            };
            let node = Node::new(key, value);
            self._cache.clone().insert(key, node);
            self._add_to_front(node);
            self._stats.clone().size = self._cache.clone().len() as i32;
        };
        return Some(evicted);
    }
    pub fn remove(&self, key: K) -> Option<V> {
        ...
        let node = self._cache.remove(key as usize);
        self._remove_node(node);
        self._stats.clone().size = self._cache.clone().len() as i32;
        return Some(node.value);
    }
    pub fn contains(&self, key: K) -> bool {
        return self._cache.clone().contains_key(&key);
    }
    ...
    pub fn clear(&mut self) {
        self._cache.clear();
        self._head.clone().next = self._tail.clone();
        self._tail.clone().prev = self._head.clone();
        self._stats.clone().size = 0;
    }
    pub fn size(&self) -> i32 {
        return self._cache.clone().len() as i32;
    }
    ...
    pub fn keys(&self) -> Vec<K> {
        let mut result = vec![];
        let mut node = self._head.clone().next;
        while node && node != self._tail.clone() {
            result.push(node.key);
            let node = node.next;
        }
        return result;
    }
    pub fn _add_to_front(&self, node: Node<K, V>) {
        node.prev = self._head.clone();
        node.next = self._head.clone().next;
        if self._head.clone().next {
            self._head.clone().next.prev = node;
        };
        self._head.clone().next = node;
    }
    pub fn _remove_node(&self, node: Node<K, V>) {
        if node.prev {
    ...
    pub fn _move_to_front(&self, node: Node<K, V>) {
        self._remove_node(node);
        self._add_to_front(node);
    }
    pub fn _evict_lru(&self) -> Option<V> {
        let lru = self._tail.clone().prev;
        if lru && lru != self._head.clone() {
            self._remove_node(lru);
            {}
            self._stats.clone().evictions = self._stats.clone().evictions + 1;
...
#[derive(Debug, Clone)]
pub struct TTLCache<K: Clone, V: Clone> {
    pub _cache: DepylerValue,
    pub _ttl: i32,
    pub _current_time: i32,
    _phantom: std::marker::PhantomData<K>,
}
impl<K: Clone, V: Clone> TTLCache<K, V> {
    pub fn new(_capacity: i32, _ttl: i32) -> Self {
        Self {
    ...
    pub fn get(&mut self, key: K) -> Option<V> {
        let entry = self._cache.clone().get(key);
        if entry.is_none() {
            return None;
        };
        let (value, expire_time) = entry;
        if self._current_time.clone() >= expire_time {
            if let Some(pos) = self._cache.iter().position(|x| x == &key) {
                self._cache.remove(pos);
            } else {
    ...
    pub fn put(&self, key: K, value: V) {
        let expire_time = self._current_time.clone() + self._ttl.clone();
        self._cache.clone().put(key, (value, expire_time));
    }
    pub fn tick(&mut self, seconds: i32) {
        self._current_time = self._current_time.clone() + seconds;
    }
...
#[doc = "Decorator to memoize function with LRU cache."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn memoize(cache: &LRUCache<DepylerValue, DepylerValue>) -> Box<dyn Fn(Callable) -> Callable> {
    let decorator = move |func: Callable| -> Callable {
        let wrapper = move |args: String| -> String {
            let key = args;
            let mut result = cache.get(&key).cloned();
            if result.is_none() {
...
#[doc = "Simulate LRU cache operations."]
pub fn simulate_lru(operations: &Vec<String>) -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for op in operations.iter().cloned() {
        ...
        if cmd == "put" {
            let (key, mut value) = parts
                ...
                .map(|s| s.to_string())
                .collect::<Vec<String>>();
            let evicted = cache.put(key, value.parse::<i32>().unwrap_or_default());
            results.push(format!("evicted={:?}", evicted));
        } else {
            if cmd == "get" {
                value = cache.get(
                    parts
                ...
                results.push(format!("value={:?}", value));
            } else {
                if cmd == "remove" {
                    value = cache.remove(
                        parts
                    ...
                    results.push(format!("removed={:?}", value));
                } else {
                    if cmd == "size" {
                        results.push((cache.size()).to_string());
                    } else {
                        if cmd == "keys" {
                            results.push((cache.keys()).to_string());
                        } else {
                            if cmd == "stats" {
                                let s = cache.stats();
                                results.push(format!(
                                ...
                                if cmd == "clear" {
                                    cache.clear();
                                    results.push("cleared".to_string().to_string());
                                }
                            }
                        }
                    }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: mem_lru_cli.py <command>[args]");
    ...
    if _cse_temp_2 {
        ...
        println!("{}", format!("Keys: {:?}", cache.keys()));
        println!("{}", format!("Get a: {}", cache.get("a".to_string())));
        println!("{}", format!("Keys after get: {:?}", cache.keys()));
        cache.put("d".to_string(), 4);
        println!("{}", format!("After adding d: {:?}", cache.keys()));
        println!("{}", format!("Stats: {}", cache.stats()));
    } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_lru_examples() {
        assert_eq!(simulate_lru(vec![]), vec![]);
        assert_eq!(simulate_lru(vec![1]), vec![1]);
    }
}
