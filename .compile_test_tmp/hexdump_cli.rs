...
struct Args {
    ...
    #[doc = "Don't show ASCII column"]
    no_ascii: bool,
    
    
    #[doc = "Reverse: convert hex to binary"]
    reverse: bool,
    
    
    #[doc = "Output as C array"]
    c_array: Option<String>,
    
    
    
    #[doc = "Extract strings(min length)"]
    strings: Option<i32>,
    
    
    #[doc = "Find hex pattern in data"]
    find: Option<String>,
    
    #[doc = "Output file"]
    output: Option<String>,
}
#[doc = "Format a single line of hex dump."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_hex_line(
    offset: i32,
    data: &Vec<u8>,
    bytes_per_line: i32,
    show_ascii: bool,
    ...
    let mut parts = vec![];
    parts.push(DepylerValue::Str(format!("{:?}", format!("{}", offset))));
    let mut hex_parts = vec![];
    for (i, byte) in data.iter().cloned().enumerate().map(|(i, x)| (i as i32, x)) {
        let i = i as i32;
        hex_parts.push(DepylerValue::Str(format!("{:?}", format!("{:?}", byte))));
        if i == 7 {
            hex_parts.push(DepylerValue::Str("".to_string().to_string()));
        }
    }
    let _cse_temp_0 = data.len() as i32;
    let _cse_temp_1 = _cse_temp_0 < bytes_per_line;
    if _cse_temp_1 {
        for i in (data.len() as i32)..(bytes_per_line) {
            hex_parts.push(DepylerValue::Str("  ".to_string().to_string()));
            if i == 7 {
                hex_parts.push(DepylerValue::Str("".to_string().to_string()));
            }
    ...
    parts.push(DepylerValue::Str(format!("{:?}", hex_parts.join(" "))));
    if show_ascii {
        let mut ascii_chars = vec![];
        for byte in data.iter().cloned() {
            if (32 <= byte) && (byte < 127) {
                ascii_chars.push(DepylerValue::Str(format!(
                    "{:?}",
                    char::from_u32((byte) as u32).unwrap().to_string()
                )));
            } else {
                ascii_chars.push(DepylerValue::Str(".".to_string().to_string()));
            }
        }
        parts.push(DepylerValue::Str(format!(
            ...
            format!(
                "{}{}",
                format!("{}{}", "|".to_string(), ascii_chars.join("")),
                "|".to_string()
            )
        )));
    }
...
pub fn hexdump(
    ...
    for i in {
        let step = bytes_per_line as usize;
        if step == 0 {
            panic!("range() arg 3 must not be zero");
        }
        (0..data.len() as i32).step_by(step)
    } {
        let chunk = {
            let base = &data;
            let start_idx = (i) as isize;
            let stop_idx = (i + bytes_per_line) as isize;
        ...
        lines.push(DepylerValue::Str(format!(
            "{:?}",
            format_hex_line(offset, &chunk, bytes_per_line, show_ascii)
        )));
        offset = offset + bytes_per_line;
    }
    lines
}
#[doc = "Generate C array from binary data."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn hexdump_c_array(data: &Vec<u8>, name: String, bytes_per_line: i32) -> Vec<String> {
    ...
    for i in {
        ...
        let hex_values = chunk
            .as_slice()
            .iter()
            .cloned()
            .map(|b| format!("0x{:?}", b))
            .collect::<Vec<_>>();
        line = format!("{}{}", "    ", hex_values.join(", "));
        if i + bytes_per_line < data.len() as i32 {
            line = format!("{}{}", line, ",");
        }
    ...
    lines.push(DepylerValue::Str(format!(
        "{:?}",
        format!("unsigned int {}_len = {};", name, data.len() as i32)
    )));
    lines
}
#[doc = "Parse hex string to bytes."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_hex(mut hex_str: String) -> Vec<u8> {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn bytes_to_hex<'a, 'b>(data: &'a Vec<u8>, separator: &'b str) -> String {
    data.as_slice()
        .iter()
        .cloned()
        .map(|b| format!("{:?}", b))
...
#[doc = "Find all occurrences of pattern in data."]
#[doc = " Depyler: verified panic-free"]
pub fn find_pattern<'a, 'b>(data: &'a Vec<u8>, pattern: &'b Vec<u8>) -> Vec<i32> {
    let mut positions = vec![];
    let mut start = 0;
    loop {
        let pos = data[start as usize..]
            .find(&pattern)
            .map(|i| (i + start as usize) as i32)
            .unwrap_or(-1);
        if pos.unwrap_or_default() == -1 {
            break;
        }
        positions.push(DepylerValue::Str(format!("{:?}", pos)));
        start = pos + 1;
    }
    positions
}
#[doc = "Extract printable ASCII strings from binary data."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn extract_strings(data: &Vec<u8>, min_length: i32) -> Vec<(i32, String)> {
    let mut start_pos: i32 = Default::default();
    ...
    for (i, byte) in data.iter().cloned().enumerate().map(|(i, x)| (i as i32, x)) {
        ...
        if (32 <= byte) && (byte < 127) {
            ...
            current.push(DepylerValue::Str(format!(
                "{:?}",
                char::from_u32((byte) as u32).unwrap().to_string()
            )));
        } else {
            if current.len() as i32 >= min_length {
                strings.push(DepylerValue::Str(format!(
                    "{:?}",
                    (start_pos, current.join(""))
                )));
            }
            current = vec![];
        }
    ...
    let _cse_temp_1 = _cse_temp_0 >= min_length;
    if _cse_temp_1 {
        strings.push(DepylerValue::Str(format!(
            "{:?}",
            (start_pos, current.join(""))
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut lines: DepylerValue = Default::default();
    let args = Args::default();
    let has_input = args.input.is_some();
    let has_length = args.length.is_some();
    let has_c_array = args.c_array.is_some();
    let has_strings = args.strings.is_some();
    ...
    if !args.reverse {
        let _cse_temp_2 = args.skip > 0;
        if _cse_temp_2 {
            data = {
                let base = &data;
                let start_idx = (args.skip) as isize;
                let start = if start_idx < 0 {
                    (base.len() as isize + start_idx).max(0) as usize
                } else {
                    start_idx as usize
        ...
        if has_length {
            data = {
                let base = &data;
                let stop_idx = (args.length) as isize;
                let stop = if stop_idx < 0 {
                    (base.len() as isize + stop_idx).max(0) as usize
                } else {
                    stop_idx as usize
    ...
    if args.reverse {
        ...
        match (|| -> Result<i32, Box<dyn std::error::Error>> {
            result = Some(parse_hex(data));
            if has_output {
                let mut f = std::fs::File::create(args.output)?;
                f.write_all(result.as_ref().unwrap().as_bytes()).unwrap();
            } else {
                std::io::stdout()
                    .buffer
                    .write_all(result.as_ref().unwrap().as_bytes())
                    .unwrap();
            }
            return Ok(());
        })() {
            ...
            Err(e) => {
                eprintln!("{}", format!("Invalid hex: {:?}", e));
                std::process::exit(1i32)
            }
        }
    }
    if has_strings {
        let strings = extract_strings(&data, args.strings);
        output = strings
            .iter()
            .cloned()
            .map(|(offset, s)| format!("{:?}  {:?}", offset, s))
            .collect::<Vec<_>>()
            .join("\n");
        if has_output {
            let mut f = std::fs::File::create(args.output)?;
    ...
    if has_find {
        ...
        match (|| -> Result<(), Box<dyn std::error::Error>> {
            pattern = Some(parse_hex(args.find));
            Ok(())
        })() {
            Ok(()) => {}
            Err(e) => {
                eprintln!("{}", format!("Invalid pattern: {:?}", e));
                std::process::exit(1i32)
            }
        }
        let positions = find_pattern(&data, pattern);
        if !positions.is_empty() {
            println!(
                "{}",
                format!("Found {} occurrence(s):", positions.len() as i32)
            );
            for pos in positions.iter().cloned() {
                println!("{}", format!("  0x{}", pos));
            }
        } else {
    ...
    lines = hexdump(&data, args.columns, !args.no_ascii, args.skip);
    output = lines.join("\n");
    if has_output {
        let mut f = std::fs::File::create(args.output)?;
        f.write_all(output.as_bytes()).unwrap();
