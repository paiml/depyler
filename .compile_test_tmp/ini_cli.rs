...
struct Args {
    ...
    #[doc = "List keys in a section"]
    keys: Option<String>,
    
    #[doc = "Output file(default: stdout)"]
    output: Option<String>,
}
#[doc = "Parse INI content into nested dict."]
#[doc = " Depyler: verified panic-free"]
pub fn parse_ini(content: &str) -> HashMap<String, DepylerValue> {
    let mut value: String = Default::default();
    let mut current_section: String = Default::default();
    let mut result: std::collections::HashMap<DepylerValue, DepylerValue> = {
        let mut map = HashMap::new();
        map
    };
    current_section = STR_EMPTY;
        ...
        .collect::<Vec<String>>()
    {
        ...
        if (line.starts_with("[")) && (line.ends_with("]")) {
            ...
            if result.get(&current_section).is_none() {
                result.insert(
                    current_section.to_string().clone(),
        ...
        if line.contains("=") {
            ...
            if !current_section.is_empty() {
                result.get_mut(&current_section).unwrap().insert(
                    key.to_string().clone(),
                    DepylerValue::Str(format!("{:?}", value)),
                );
            } else {
                if result.get(STR_EMPTY).is_none() {
                    result.insert(
                        STR_EMPTY.to_string(),
                        DepylerValue::Str(format!(
                            "{:?}",
                ...
                result.get_mut(&STR_EMPTY).unwrap().insert(
                    key.to_string().clone(),
                    DepylerValue::Str(format!("{:?}", value)),
                );
            }
...
#[doc = "Format dict back to INI format."]
pub fn format_ini(
    data: &std::collections::HashMap<String, String>,
) -> Result<String, Box<dyn std::error::Error>> {
    let mut lines = vec![];
    let _cse_temp_0 = data.get(STR_EMPTY).is_some();
    if _cse_temp_0 {
        ...
        {
            lines.push(DepylerValue::Str(STR_EMPTY.to_string()));
        }
    }
...
#[doc = "Get a value from parsed INI data."]
#[doc = " Depyler: proven to terminate"]
pub fn get_value<'a, 'b, 'c>(
    data: &'a std::collections::HashMap<String, String>,
    section: &'b str,
    key: &'c str,
) -> Result<String, Box<dyn std::error::Error>> {
    let _cse_temp_0 = data.get(section).is_none();
    if _cse_temp_0 {
        return Ok(STR_EMPTY);
    }
    let _cse_temp_1 = !data
        .get(section)
        .cloned()
        .unwrap_or_default()
        .contains(&*key);
...
#[doc = "Set a value in parsed INI data."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn set_value<'a, 'b>(
    mut data: std::collections::HashMap<String, String>,
    section: &'a str,
    key: String,
    value: &'b str,
    ...
    data.get_mut(&section).unwrap().insert(
        key.to_string().clone(),
        DepylerValue::Str(format!("{:?}", value)),
    );
    data
}
#[doc = "List all sections in INI data."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
...
#[doc = "List all keys in a section."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn list_keys<'b, 'a>(
    data: &'a std::collections::HashMap<String, String>,
    section: &'b str,
) -> Vec<String> {
    let _cse_temp_0 = data.get(section).is_none();
...
pub fn main() -> Result<(), std::io::Error> {
    let mut section: String = Default::default();
    ...
    let mut data = parse_ini(&content);
    let mut value;
    if has_get {
        let _cse_temp_2 = args.get.contains(".");
        if _cse_temp_2 {
            let _split_parts = args
                .get
                .splitn((1 + 1) as usize, ".")
                .map(|s| s.to_string())
                .collect::<Vec<String>>();
            let mut section = _split_parts.get(0).cloned().unwrap_or_default();
            let mut key = _split_parts.get(1).cloned().unwrap_or_default();
        } else {
            (section, key) = (STR_EMPTY.to_string(), args.get);
        }
        ...
        let mut output;
        if has_set {
            let _cse_temp_3 = !args.set.contains("=");
            if _cse_temp_3 {
                eprintln!("{}", "Error: --set requires SECTION.KEY=VALUE format");
            ...
            let _split_parts = args
                ...
                .map(|s| s.to_string())
                .collect::<Vec<String>>();
            let path = _split_parts.get(0).cloned().unwrap_or_default();
            let mut value = _split_parts.get(1).cloned().unwrap_or_default();
            let _cse_temp_4 = path.contains(".");
            if _cse_temp_4 {
                let _split_parts = path
                    .splitn((1 + 1) as usize, ".")
                    .map(|s| s.to_string())
                    .collect::<Vec<String>>();
                let mut section = _split_parts.get(0).cloned().unwrap_or_default();
                ...
                (section, key) = (STR_EMPTY.to_string(), path);
            }
