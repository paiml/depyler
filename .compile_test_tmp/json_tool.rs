...
pub fn main() {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Escape { .. }));
    match &args.command {
        Some(Commands::Escape { ref text, .. }) => {
            let mut result = text.replace("\"", "\\\"");
            println!("{}", result);
        }
        Some(Commands::Unescape { ref text, .. }) => {
            let mut result = text.replace("\\\"", "\"");
            println!("{}", result);
        }
        Some(Commands::Validate { ref text, .. }) => {
            let _cse_temp_3 = text.contains("{");
            let _cse_temp_4 = text.contains("}");
