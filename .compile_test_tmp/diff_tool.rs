...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'difflib'(tracked in DEPYLER-0424)"]
...
#[doc = "Show unified diff. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_diff(file1: &str, file2: &str) -> Result<(), std::io::Error> {
    let mut f1 = std::fs::File::open(&file1)?;
    let lines1 = std::io::BufReader::new(f1)
        .lines()
        .map(|l| l.unwrap_or_default())
        .collect::<Vec<_>>();
    let mut f2 = std::fs::File::open(&file2)?;
    let lines2 = std::io::BufReader::new(f2)
        .lines()
        .map(|l| l.unwrap_or_default())
        .collect::<Vec<_>>();
    let diff = difflib.unified_diff(lines1, lines2);
    for line in diff.iter().cloned() {
        println!("{}", line);
    }
    Ok(())
}
#[doc = "Calculate similarity ratio. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_ratio(str1: &str, str2: &str) {
    let ratio = difflib.SequenceMatcher(None, str1, str2).ratio();
    println!("{}", format!("Similarity: {:?}", ratio));
}
#[doc = "Find close matches. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_close(candidates: &[String], word: &str) {
    let matches = difflib.get_close_matches(word, candidates);
...
pub fn main() {
    ...
    ({
        ...
        map.insert("diff".to_string().to_string(), cmd_diff);
        map.insert("ratio".to_string().to_string(), cmd_ratio);
        map.insert("close".to_string().to_string(), cmd_close);
        map
    }
    .get(&DepylerValue::Int(args.command as i64))
    .cloned()
