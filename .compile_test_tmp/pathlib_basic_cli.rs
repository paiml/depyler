...
#[doc = "Create Path object from string."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn create_path(path_str: &str) -> std::path::PathBuf {
    std::path::PathBuf::from(&path_str)
}
#[doc = "Get file name."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_name(path: &std::path::PathBuf) -> String {
    path.file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("")
        .to_string()
}
#[doc = "Get file name without suffix."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_stem(path: &std::path::PathBuf) -> String {
    path.file_stem()
        .and_then(|n| n.to_str())
        .unwrap_or("")
        .to_string()
}
#[doc = "Get file suffix/extension."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_suffix(path: &std::path::PathBuf) -> String {
    path.extension()
        .map(|e| format!(".{}", e.to_str().unwrap()))
        .unwrap_or_default()
}
#[doc = "Get all suffixes(e.g., .tar.gz)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_suffixes(path: &std::path::PathBuf) -> Vec<String> {
    path.suffixes
}
#[doc = "Get parent directory."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_parent(path: &std::path::PathBuf) -> std::path::PathBuf {
    path.parent().map(|p| p.to_path_buf()).unwrap_or_default()
}
#[doc = "Get all parent directories."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_parents(path: &std::path::PathBuf) -> Vec<std::path::PathBuf> {
    path.parents.into_iter().collect::<Vec<_>>()
}
#[doc = "Get path components."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_parts(path: &std::path::PathBuf) -> (String, DepylerValue) {
    path.components()
        .map(|c| c.as_os_str().to_str().unwrap().to_string())
        .collect::<Vec<_>>()
}
#[doc = "Get anchor(root + drive)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_anchor(path: &std::path::PathBuf) -> String {
    path.anchor
}
#[doc = "Get root."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn get_root(path: &std::path::PathBuf) -> String {
    path.root
}
#[doc = "Check if path is absolute."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_absolute(path: &std::path::PathBuf) -> bool {
    path.is_absolute()
}
#[doc = "Check if path is relative."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_relative(path: &std::path::PathBuf) -> bool {
    !path.is_absolute()
}
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_symlink(path: &std::path::PathBuf) -> bool {
    path.is_symlink()
}
#[doc = "Resolve path to absolute."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn resolve(path: &std::path::PathBuf) -> std::path::PathBuf {
    path.canonicalize().unwrap()
}
#[doc = "Expand ~ to home directory."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn expanduser(path: &std::path::PathBuf) -> std::path::PathBuf {
    path.expanduser()
}
#[doc = "Get absolute path."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn absolute(path: &std::path::PathBuf) -> std::path::PathBuf {
    path.canonicalize().unwrap()
}
#[doc = "Join path components."]
pub fn join_paths<'b, 'a>(
    base: &'a std::path::PathBuf,
    parts: &[String],
) -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    let mut result: std::path::PathBuf = Default::default();
    result = base;
...
#[doc = "Return path with different name."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn with_name<'a, 'b>(path: &'a std::path::PathBuf, name: &'b str) -> std::path::PathBuf {
    path.with_file_name(name)
}
#[doc = "Return path with different stem."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn with_stem<'a, 'b>(path: &'a std::path::PathBuf, stem: &'b str) -> std::path::PathBuf {
    {
        let p = &path;
        let ext = p
            .extension()
...
#[doc = "Return path with different suffix."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn with_suffix<'a, 'b>(path: &'a std::path::PathBuf, suffix: &'b str) -> std::path::PathBuf {
    path.with_extension(suffix.trim_start_matches('.'))
}
#[doc = "Get path relative to other."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn relative_to<'a, 'b>(
    path: &'a std::path::PathBuf,
    other: &'b std::path::PathBuf,
) -> std::path::PathBuf {
    path.strip_prefix(other).unwrap().to_path_buf()
}
#[doc = "Check if path is relative to other."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn is_relative_to<'a, 'b>(path: &'a std::path::PathBuf, other: &'b std::path::PathBuf) -> bool {
    path.is_relative_to(other)
}
#[doc = "Check if path matches pattern."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn match_pattern<'a, 'b>(path: &'a std::path::PathBuf, pattern: &'b str) -> bool {
    path.find(pattern)
}
#[doc = "Return path with forward slashes."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn as_posix(path: &std::path::PathBuf) -> String {
    path.as_posix()
}
#[doc = "Return path as file:// URI."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn as_uri(path: &std::path::PathBuf) -> String {
    path.as_uri()
}
...
#[doc = "Parse path and return all components."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn parse_path_info(path_str: &str) -> HashMap<String, DepylerValue> {
    let path = std::path::PathBuf::from(&path_str);
    {
        let mut map = HashMap::new();
        map.insert(
            "path".to_string(),
            DepylerValue::Str(format!("{:?}", (path).display().to_string())),
        );
        map.insert(
            "name".to_string(),
        ...
        map.insert(
            "suffixes".to_string(),
            DepylerValue::Str(format!("{:?}", path.suffixes)),
        );
        map.insert(
            "parent".to_string(),
            DepylerValue::Str(format!(
        ...
        map.insert(
            ...
            DepylerValue::Str(format!("{:?}", path.is_absolute())),
        );
        map.insert(
            "anchor".to_string(),
            DepylerValue::Str(format!("{:?}", path.anchor)),
        );
        map
    }
}
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Info { .. }));
    match &args.command {
        Some(Commands::Info { ref path, .. }) => {
            let info = parse_path_info(&path);
        ...
        Some(Commands::Exists { ref path, .. }) => {
            let mut path = std::path::PathBuf::from(&path);
            if path.exists() {
                println!("{}", format!("'{}' exists", path));
            } else {
                println!("{}", format!("'{}' does not exist", path));
            }
        }
        Some(Commands::Type { ref path, .. }) => {
            let mut path = std::path::PathBuf::from(&path);
            if !path.exists() {
        ...
        Some(Commands::Parent {
            ref n, ref path, ..
        }) => {
            let n = n.clone();
            let mut path = std::path::PathBuf::from(&path);
            for __sanitized in 0..(n) {
                path = path.parent().map(|p| p.to_path_buf()).unwrap_or_default();
            }
            println!("{}", path.display());
        }
        Some(Commands::Name { ref path, .. }) => {
            let mut path = std::path::PathBuf::from(&path);
            println!(
                "{}",
                format!(
        ...
        Some(Commands::Resolve { ref path, .. }) => {
            let mut path = std::path::PathBuf::from(&path);
...
mod tests {
    ...
    #[test]
    fn quickcheck_is_absolute() {
        fn prop(path: ()) -> TestResult {
            let result = is_absolute(path.clone());
            if result < 0 {
    ...
    #[test]
    fn test_is_absolute_examples() {
        let _ = is_absolute(Default::default());
    }
    #[test]
    fn test_is_relative_examples() {
        let _ = is_relative(Default::default());
    }
    #[test]
    fn test_exists_examples() {
        let _ = exists(Default::default());
    }
    #[test]
    fn test_is_file_examples() {
        let _ = is_file(Default::default());
    }
    #[test]
    fn test_is_dir_examples() {
        let _ = is_dir(Default::default());
    }
    #[test]
    fn test_is_symlink_examples() {
        let _ = is_symlink(Default::default());
    }
    #[test]
    fn quickcheck_absolute() {
        fn prop(path: ()) -> TestResult {
            let result = absolute(path.clone());
            if result < 0 {
                return TestResult::failed();
            }
            TestResult::passed()
