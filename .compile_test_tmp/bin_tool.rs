...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = matches!(args.command, Some(Commands::Tobin { .. }));
    match &args.command {
        Some(Commands::Tobin { ref num, .. }) => {
            let num = *num;
            println!("{}", format!("{:b}", num));
        }
        Some(Commands::Frombin { ref binstr, .. }) => {
            let binstr = binstr.to_string();
            println!("{}", i64::from_str_radix(&binstr, 2).unwrap());
        }
        Some(Commands::Bits { ref num, .. }) => {
            let num = *num;
            let mut count = 0;
            let mut n = num.clone();
            while n > 0 {
