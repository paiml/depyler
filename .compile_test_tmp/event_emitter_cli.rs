...
pub struct EventStats {
    ...
    pub handlers_invoked: i32,
    pub handlers_registered: i32,
}
impl EventStats {
    pub fn new(events_emitted: i32, handlers_invoked: i32, handlers_registered: i32) -> Self {
        Self {
            events_emitted,
            handlers_invoked,
            handlers_registered,
...
pub struct EventEmitter {
    pub _handlers: std::collections::HashMap<String, Vec<Handler>>,
    pub _once_handlers: std::collections::HashMap<String, Vec<Handler>>,
...
impl EventEmitter {
    pub fn new() -> Self {
        Self {
            ...
            _once_handlers: std::collections::HashMap::new(),
            _stats: Default::default(),
        }
    }
    pub fn on(&self, event: String, handler: &Handler) -> EventEmitter {
        ...
        .push(handler);
        self._stats.clone().handlers_registered = self._stats.clone().handlers_registered + 1;
        return self;
    }
    pub fn once(&self, event: String, handler: &Handler) -> EventEmitter {
        if !self._once_handlers.clone().contains_key(&event) {
            self._once_handlers.clone().insert(event, vec![]);
        };
        {
            let _base = &self._once_handlers.clone();
            let _idx = event;
            if _idx < 0 {
                _base[_base.len().wrapping_sub((-_idx) as usize)].clone()
            } else {
    ...
    pub fn off(&self, event: String, handler: Option<Handler>) -> EventEmitter {
        if handler.is_none() {
            if self._handlers.clone().contains_key(&event) {
                self._stats.clone().handlers_registered = self._stats.clone().handlers_registered
                    - ({
                        let _base = &self._handlers.clone();
                        let _idx = event;
                        if _idx < 0 {
            ...
            .contains_key(&handler)
            {
                ...
                self._stats.clone().handlers_registered =
                    self._stats.clone().handlers_registered - 1;
            };
            if self._once_handlers.clone().contains_key(&event) && {
                let _base = &self._once_handlers.clone();
                let _idx = event;
    ...
    pub fn emit(&self, event: String, args: Vec<String>) -> bool {
        let mut invoked = false;
        self._stats.clone().events_emitted = self._stats.clone().events_emitted + 1;
        if self._handlers.clone().contains_key(&event) {
            for handler in {
                let _base = &self._handlers.clone();
                let _idx = event;
                ...
                handler(args);
                self._stats.clone().handlers_invoked = self._stats.clone().handlers_invoked + 1;
                let invoked = true;
            }
        };
        if self._once_handlers.clone().contains_key(&event) {
            let handlers = self._once_handlers.remove(event as usize);
            self._stats.clone().handlers_registered =
                self._stats.clone().handlers_registered - (handlers.len() as i32);
    ...
    pub fn listeners(&self, event: String) -> Vec<Handler> {
        let mut result = vec![];
        if self._handlers.clone().contains_key(&event) {
            result.extend({
                let _base = &self._handlers.clone();
    ...
    pub fn listener_count(&self, event: String) -> i32 {
        return self.listeners(event).len() as i32;
    }
    pub fn event_names(&self) -> Vec<String> {
        let names = self
            ...
            .cloned()
            .collect();
        return names.into_iter().collect::<Vec<_>>();
    }
    pub fn remove_all_listeners(&mut self, event: Option<String>) -> EventEmitter {
        if event.is_none() {
            self._handlers.clear();
            self._once_handlers.clear();
            self._stats.clone().handlers_registered = 0;
        } else {
            self.off(event);
        };
        return self;
    ...
    pub fn stats(&self) -> EventStats {
        return EventStats::new();
    }
}
#[derive(Debug, Clone)]
pub struct TypedEventEmitter {
    pub _emitter: DepylerValue,
}
impl TypedEventEmitter {
    pub fn new() -> Self {
        Self {
            _emitter: Default::default(),
        }
    }
    pub fn on(&self, event: String, handler: &Handler) -> TypedEventEmitter {
        self._emitter.clone().on(event, handler);
        return self;
    }
    pub fn emit(&self, event: String, data: &DepylerValue) -> bool {
        return self._emitter.clone().emit(event, data);
    }
}
#[derive(Debug, Clone, PartialEq)]
pub struct Event {
...
impl Event {
    pub fn new(name: String, data: DepylerValue, timestamp: f64) -> Self {
        Self {
            name,
            data,
            timestamp,
...
impl AsyncEventEmitter {
    ...
    pub fn on(&self, event: String, handler: &Handler) -> AsyncEventEmitter {
        self._emitter.clone().on(event, handler);
        return self;
    }
    pub fn emit(&mut self, event: String, args: Vec<String>) {
        self._pending.push(Event::new(event, args));
    }
    pub fn process(&self) -> i32 {
        let mut count = 0;
        while self._pending.clone() {
            let event = self._pending.remove(0 as usize);
            if true {
                self._emitter.clone().emit(event.name, event.data);
            } else {
                self._emitter.clone().emit(event.name, event.data);
            };
            let count = count + 1;
    ...
    pub fn pending_count(&self) -> i32 {
        return self._pending.clone().len() as i32;
    }
}
...
#[doc = "Simulate event emitter operations."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn simulate_emitter(_operations: Vec<String>) -> Vec<String> {
    let results: Vec<String> = vec![];
    let emitter = EventEmitter::new();
    let values: Vec<DepylerValue> = vec![];
    let capture = move |args: String| -> () {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if _cse_temp_1 {
        println!("{}", "Usage: event_emitter_cli.py <command>[args]");
    ...
    if _cse_temp_2 {
        ...
        emitter.on("message".to_string(), move |msg| received.push(msg));
        emitter.on("message".to_string(), move |msg| {
            println!("{}", format!("Handler 2: {:?}", msg))
        });
        emitter.emit("message".to_string(), "Hello!".to_string());
        emitter.emit("message".to_string(), "World!".to_string());
        println!("{}", format!("Received: {:?}", received));
        println!("{}", format!("Stats: {}", emitter.stats()));
    } else {
...
mod tests {
    ...
    #[test]
    fn test_simulate_emitter_examples() {
        assert_eq!(simulate_emitter(vec![]), vec![]);
        assert_eq!(simulate_emitter(vec![1]), vec![1]);
    }
}
