...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'dataclasses'(tracked in DEPYLER-0424)"] #[doc = "// NOTE: Map Python module 'enum'(tracked in DEPYLER-0424)"] use std::collections::HashMap;
...
} #[derive(Debug, Clone)] pub struct RuleType {
   
}
impl RuleType {
    pub const REQUIRED: i32 = auto();
    pub const TYPE: i32 = auto();
    pub const MIN: i32 = auto();
    pub const MAX: i32 = auto();
    pub const MIN_LENGTH: i32 = auto();
    pub const MAX_LENGTH: i32 = auto();
    pub const PATTERN: i32 = auto();
    pub const ENUM: i32 = auto();
    pub const CUSTOM: i32 = auto();
    pub fn new() -> Self {
    Self {
...
#[derive(Debug, Clone)] pub struct ValueType {
   
}
impl ValueType {
    pub const STRING: i32 = auto();
    pub const INT: i32 = auto();
    pub const FLOAT: i32 = auto();
    pub const BOOL: i32 = auto();
    pub const LIST: i32 = auto();
    pub const DICT: i32 = auto();
...
#[derive(Debug, Clone, PartialEq)] pub struct Rule {
    pub rule_type: RuleType, pub value: DepylerValue
}
impl Rule {
    pub fn new(rule_type: RuleType, value: DepylerValue) -> Self {
    Self {
    rule_type, value
}
}
}
#[derive(Debug, Clone, PartialEq)] pub struct FieldSchema {
    pub name: String, pub value_type: ValueType, pub rules: Vec<Rule>, pub nested: Option<std::collections::HashMap<String, FieldSchema>>
}
impl FieldSchema {
    pub fn new(name: String, value_type: ValueType, rules: Vec<Rule>, nested: Option<std::collections::HashMap<String, FieldSchema>>) -> Self {
    Self {
    name, value_type, rules, nested
}
}
}
#[derive(Debug, Clone, PartialEq)] pub struct ValidationError {
    pub path: String, pub message: String, pub rule_type: Option<RuleType>
}
impl ValidationError {
    pub fn new(path: String, message: String, rule_type: Option<RuleType>) -> Self {
    Self {
    path, message, rule_type
}
}
}
#[derive(Default)] enum Commands {
    ...
    __DepylerNone,
    Unflatten {
    #[doc = "Flat config file"] config: String
}
, Diff {
    #[doc = "First config file"] config1: String, #[doc = "Second config file"] config2: String
}
, Validate {
    #[doc = "Config file(JSON)"] config: String, #[doc = "Schema file(JSON)"] schema: Option<String>
}
, Merge {
    #[doc = "Base config file"] base: String, #[doc = "Override config file"] r#override: String
}
, Flatten {
    #[doc = "Config file"] config: String
}
...
#[doc = "Get ValueType for a Python value."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn get_value_type(value: & DepylerValue) -> ValueType {
    if true {
...
#[doc = "Check if value matches expected type."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn type_matches<'a, 'b>(value: & 'a DepylerValue, expected: & 'b ValueType) -> bool {
    let _cse_temp_0  = (* expected) == ValueType::ANY;
    if _cse_temp_0 {
    return true;
   
}
let actual = get_value_type(& value);
    let _cse_temp_1 = actual == ValueType::INT;
...
#[doc = "Validate a field against its schema."] #[doc = " Depyler: verified panic-free"] pub fn validate_field<'c, 'a, 'b>(value: & 'a DepylerValue, schema: & 'b FieldSchema, path: & 'c str) -> Vec<ValidationError>{
    let mut errors = vec! [];
    for rule in schema.rules {
    if rule.rule_type == RuleType::REQUIRED {
    if value.is_none() {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Field '{}' is required", path), RuleType::REQUIRED))));
    return errors;
...
if rule.rule_type == RuleType::TYPE {
    if! type_matches(& value, schema.value_type) {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Expected {}, got {}", schema.value_type.name.to_lowercase(), get_value_type(& value).name.to_lowercase()), RuleType::TYPE))));
   
}
} else {
    if rule.rule_type == RuleType::MIN {
    if(true) &&((* value)<rule.value) {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Value {} is less than minimum {}", value, rule.value), RuleType::MIN))));
   
}
} else {
    if rule.rule_type == RuleType::MAX {
    if(true) &&((* value)>rule.value) {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Value {} is greater than maximum {}", value, rule.value), RuleType::MAX))));
   
}
} else {
    if rule.rule_type == RuleType::MIN_LENGTH {
    if(hasattr(value, "__len__".to_string())) &&((value.len() as i32)<rule.value) {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Length {} is less than minimum {}", value.len() as i32, rule.value), RuleType::MIN_LENGTH))));
   
}
} else {
    if rule.rule_type == RuleType::MAX_LENGTH {
    if(hasattr(value, "__len__".to_string())) &&(value.len() as i32>rule.value) {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Length {} is greater than maximum {}", value.len() as i32, rule.value), RuleType::MAX_LENGTH))));
   
}
} else {
    if rule.rule_type == RuleType::PATTERN {
    if(true) &&(DepylerRegexMatch::match_start(rule.value, value).is_none()) {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Value does not match pattern '{}'", rule.value), RuleType::PATTERN))));
   
}
} else {
    if rule.rule_type == RuleType::ENUM {
    if! rule.value.get(& value).is_some() {
    errors.push(DepylerValue::Str(format!("{:?}", ValidationError::new(path, format!("Value must be one of: {}", rule.value), RuleType::ENUM))));
...
let _cse_temp_0  = (schema.nested) &&(true);
    if _cse_temp_0 {
    for(field_name, field_schema) in schema.nested.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>() {
    let field_value = value.get(& field_name).cloned();
    let field_path = if! path.is_empty() {
    format!("{}.{:?}", path, field_name)
}
else {
    field_name };
    errors.extend(validate_field(field_value.as_ref().unwrap(), & field_schema, & field_path).iter().cloned());
   
}
} errors
}
#[doc = "Validate configuration against schema."] #[doc = " Depyler: verified panic-free"] pub fn validate<'a, 'b>(config: & 'a std::collections::HashMap<DepylerValue, DepylerValue>, schema: & 'b std::collections::HashMap<String, FieldSchema>) -> Vec<ValidationError>{
    let mut errors = vec! [];
    for(field_name, field_schema) in schema.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>() {
    let value = config.get(& field_name).cloned();
    errors.extend(validate_field(value, & field_schema, & field_name).iter().cloned());
   
}
errors
}
#[doc = "Create a field schema with rules."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn create_field<'l2, 'l5, 'c, 'b, 'l1, 'l3, 'l4, 'a>(value_type: & 'a ValueType, required: bool, min_val: & 'b Option<f64>, max_val: & 'c Option<f64>, min_length: & 'l1 Option<i32>, max_length: & 'l2 Option<i32>, pattern: & 'l3 Option<String>, r#enum: & 'l4 Option<Vec<DepylerValue>>, nested: & 'l5 Option<std::collections::HashMap<String, FieldSchema>>) -> FieldSchema {
    let mut rules = vec! [];
    if required {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::REQUIRED, None))));
   
}
rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::TYPE, None))));
    if min_val.is_some() {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::MIN, min_val))));
   
}
if max_val.is_some() {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::MAX, max_val))));
   
}
if min_length.is_some() {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::MIN_LENGTH, min_length))));
   
}
if max_length.is_some() {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::MAX_LENGTH, max_length))));
   
}
if pattern.is_some() {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::PATTERN, pattern))));
   
}
if r#enum.is_some() {
    rules.push(DepylerValue::Str(format!("{:?}", Rule::new(RuleType::ENUM, r#enum))));
   
}
FieldSchema::new("".to_string(), value_type, rules, nested)
}
#[doc = "Parse type string to ValueType."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn parse_type(type_str: & str) -> ValueType {
    let type_map = {
    let mut map = HashMap::new();
    map.insert("string".to_string(), ValueType::STRING);
    map.insert("str".to_string(), ValueType::STRING);
    map.insert("int".to_string(), ValueType::INT);
    map.insert("integer".to_string(), ValueType::INT);
    map.insert("float".to_string(), ValueType::FLOAT);
    map.insert("number".to_string(), ValueType::FLOAT);
    map.insert("bool".to_string(), ValueType::BOOL);
    map.insert("boolean".to_string(), ValueType::BOOL);
    map.insert("list".to_string(), ValueType::LIST);
    map.insert("array".to_string(), ValueType::LIST);
    map.insert("dict".to_string(), ValueType::DICT);
    map.insert("object".to_string(), ValueType::DICT);
    map.insert("any".to_string(), ValueType::ANY);
    map };
    type_map.get(& type_str.to_lowercase()).cloned().unwrap_or(ValueType::ANY)
}
#[doc = "Parse schema from dictionary."] pub fn parse_schema_dict(schema_dict: & std::collections::HashMap<DepylerValue, DepylerValue>) -> Result<HashMap<String, FieldSchema>, Box<dyn std::error::Error>>{
    let mut nested: DepylerValue = Default::default();
    let mut result = {
    let map: HashMap<String ,()>= HashMap::new();
    map };
    for(field_name, field_def) in schema_dict.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>() {
    if true {
    result.insert(field_name.to_string().clone(), create_field(parse_type(& field_def), false, & None, & None, & None, & None, & None, & None, & None));
   
}
else {
    if true {
    let value_type = parse_type(field_def.get("type").cloned().unwrap_or("any"));
    let required = field_def.get("required").cloned().unwrap_or(false);
    let min_val = field_def.get("min").cloned();
    let max_val = field_def.get("max").cloned();
    let min_length = field_def.get("minLength").cloned();
    let max_length = field_def.get("maxLength").cloned();
    let pattern = field_def.get("pattern").cloned();
    let r#enum = field_def.get("enum").cloned();
    if field_def.contains("properties") {
    * nested = Some(parse_schema_dict(field_def.get("properties").cloned().unwrap_or_default()) ?);
   
}
let mut field_schema = create_field(& value_type, required, min_val.as_ref().unwrap(), max_val.as_ref().unwrap(), min_length.as_ref().unwrap(), max_length.as_ref().unwrap(), pattern.as_ref().unwrap(), r#enum.as_ref().unwrap(), nested);
    field_schema.name = field_name;
    result.insert(field_name.to_string().clone(), field_schema);
...
#[doc = "Deep merge two configurations."] pub fn merge_configs<'a, 'b>(base: & 'a std::collections::HashMap<DepylerValue, DepylerValue>, r#override: & 'b std::collections::HashMap<DepylerValue, DepylerValue>) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>>{
    let mut result = base.into_iter().collect::<std::collections::HashMap<_, _>>();
    for(key, value) in r#override.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>() {
    if((result.get(& key).is_some()) &&(true)) &&(true) {
    result.insert(key.to_string().clone(), DepylerValue::Str(format!("{:?}", merge_configs(result.get(& key).cloned().unwrap_or_default(), & value) ?)));
   
}
else {
    result.insert(key.to_string().clone(), DepylerValue::Str(format!("{:?}", value)));
   
}
} Ok(result)
}
#[doc = "Flatten nested config to dot notation."] #[doc = " Depyler: verified panic-free"] pub fn flatten_config<'b, 'a>(config: & 'a std::collections::HashMap<DepylerValue, DepylerValue>, prefix: & 'b str) -> HashMap<String, DepylerValue>{
    let mut result = {
    let map: HashMap<String ,()>= HashMap::new();
    map };
    for(key, value) in config.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>() {
    ...
    key };
    if true {
    for(k, v) in (flatten_config(& value, & full_key)).iter() {
    result.insert(k.clone(), v.clone());
    };
   
}
else {
    result.insert(full_key.to_string().clone(), value);
   
}
} result
}
#[doc = "Unflatten dot notation to nested dict."] pub fn unflatten_config(flat: & std::collections::HashMap<String, DepylerValue>) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>>{
    let result = {
    let mut map = HashMap::new();
    map };
    for(key, value) in flat.iter().map(|(k, v) |(k.clone(), v.clone())).collect::<Vec<_>>() {
...
}
{
    ...
    current.insert(part.to_string().clone(), DepylerValue::Str(format!("{:?}", format!("{:?}", {
    let mut map = HashMap::new();
    map }))));
   
}
current = current.get(& DepylerValue::Int(part as i64)).cloned().unwrap_or_default();
   
}
current.insert({ let base = & parts;
    base.get(base.len().saturating_sub(1usize)).cloned().unwrap_or_default()
}
. to_string(), DepylerValue::Str(format!("{:?}", value)));
   
}
Ok(result)
}
#[doc = "Find differences between two configurations."] pub fn diff_configs<'b, 'a>(config1: & 'a std::collections::HashMap<DepylerValue, DepylerValue>, config2: & 'b std::collections::HashMap<DepylerValue, DepylerValue>) -> Result<HashMap<String, DepylerValue>, Box<dyn std::error::Error>>{
    let flat1 = flatten_config(& config1, "");
    let flat2 = flatten_config(& config2, "");
    let _cse_temp_0 = flat1.keys().cloned().collect::<Vec<_>>().into_iter().collect::<std::collections::HashSet<_>>().union(& flat2.keys().cloned().collect::<Vec<_>>().into_iter().collect::<std::collections::HashSet<_>>()).cloned().collect::<std::collections::HashSet<_>>();
    let all_keys = _cse_temp_0;
    let diff = {
    let mut map = HashMap::new();
    map.insert("added".to_string(), DepylerValue::Str(format!("{:?}", {
    let mut map = HashMap::new();
    map })));
    map.insert("removed".to_string(), DepylerValue::Str(format!("{:?}", {
    let mut map = HashMap::new();
    map })));
    map.insert("changed".to_string(), DepylerValue::Str(format!("{:?}", {
    let mut map = HashMap::new();
    map })));
    map };
    for key in all_keys.iter().cloned() {
    if flat1.get(& key).is_none() {
    diff.get_mut(& "added").unwrap().insert(key.to_string().clone(), DepylerValue::Str(format!("{:?}", flat2.get(& key).cloned().unwrap_or_default())));
   
}
else {
    if flat2.get(& key).is_none() {
    diff.get_mut(& "removed").unwrap().insert(key.to_string().clone(), DepylerValue::Str(format!("{:?}", flat1.get(& key).cloned().unwrap_or_default())));
   
}
else {
    if flat1.get(& key).cloned().unwrap_or_default()!= flat2.get(& key).cloned().unwrap_or_default() {
    diff.get_mut(& "changed").unwrap().insert(key.to_string().clone(), DepylerValue::Str(format!("{:?}", format!("{:?}", {
    let mut map = HashMap::new();
    map.insert("from".to_string(), DepylerValue::Str(format!("{:?}", flat1.get(& key).cloned().unwrap_or_default())));
    map.insert("to".to_string(), DepylerValue::Str(format!("{:?}", flat2.get(& key).cloned().unwrap_or_default())));
    map }))));
...
#[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), std::io::Error>{
    let mut flat: DepylerValue = Default::default();
   ();
   ();
   ();
   ();
  ...
  ..}));
    ...
    ref config, ref schema,..}) =>{
    let mut f = std::fs::File::open(& config) ?;
    let mut config = std::collections::HashMap::<String, DepylerValue>::new();
    let mut f = std::fs::File::open(& schema) ?;
    let schema_dict = std::collections::HashMap::<String, DepylerValue>::new();
...
} let _cse_temp_1 = matches!(args.command, Some(Commands::Merge {
  ..}));
    match & args.command {
    Some(Commands::Merge {
    ref base, ref r#override,..}) =>{
    let r#override = r#override.to_string();
    let mut f = std::fs::File::open(& base) ?;
    let base = std::collections::HashMap::<String, DepylerValue>::new();
    let mut f = std::fs::File::open(& r#override) ?;
    let r#override = std::collections::HashMap::<String, DepylerValue>::new();
    let result = merge_configs(& base, & r#override) ?;
...
} let _cse_temp_2 = matches!(args.command, Some(Commands::Flatten {
  ..}));
    match & args.command {
...
} let _cse_temp_3 = matches!(args.command, Some(Commands::Unflatten {
  ..}));
    match & args.command {
    Some(Commands::Unflatten {
    ref config,..}) =>{
    let mut f = std::fs::File::open(& config) ?;
    flat = std::collections::HashMap::<String, DepylerValue>::new();
    let mut config = unflatten_config(& flat) ?;
...
} let _cse_temp_4 = matches!(args.command, Some(Commands::Diff {
  ..}));
    match & args.command {
    Some(Commands::Diff {
    ref config1, ref config2,..}) =>{
    let mut f = std::fs::File::open(& config1) ?;
    let config1 = std::collections::HashMap::<String, DepylerValue>::new();
    let mut f = std::fs::File::open(& config2) ?;
    let config2 = std::collections::HashMap::<String, DepylerValue>::new();
