...
pub fn match_pattern<'a, 'b>(pattern: &'a str, text: &'b str, ignore_case: bool) -> bool {
    let flags = if ignore_case { 2i32 } else { 0 };
    let r#match = DepylerRegexMatch::match_start(pattern, text);
    if let Some(ref match_val) = r#match {
        println!(
            "{}",
            format!("Match found: {}", match_val.group(0 as usize))
        );
        if !Vec::<String>::new().is_empty() {
            println!("{}", format!("Groups: {:?}", Vec::<String>::new()));
        }
        true
    } else {
        println!("{}", "No match");
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn search_pattern<'a, 'b>(pattern: &'a str, text: &'b str, ignore_case: bool) -> bool {
    let flags = if ignore_case { 2i32 } else { 0 };
    let r#match = DepylerRegexMatch::search(pattern, text);
    if let Some(ref match_val) = r#match {
        println!(
            "{}",
            format!(
                "Found at position {}: {}",
                match_val.as_ref().map(|m| m.start()).unwrap_or(0),
                match_val.group(0 as usize)
            )
        );
        true
    } else {
...
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn find_all<'b, 'a>(pattern: &'a str, text: &'b str, ignore_case: bool) -> Vec<String> {
    let flags = if ignore_case { 2i32 } else { 0 };
    let matches = DepylerRegexMatch::findall(pattern, text);
    println!("{}", format!("Found {} matches:", matches.len() as i32));
    for (i, r#match) in matches
        .iter()
        .cloned()
        .enumerate()
...
pub fn validate_email(email: &str) -> bool {
    ...
    let compiled = pattern.to_string();
    let r#match = compiled.find(email);
    if let Some(ref match_val) = r#match {
        println!("{}", format!("Valid email: {}", email));
        true
    } else {
        println!("{}", format!("Invalid email: {}", email));
        false
    }
}
...
pub fn extract_numbers(text: &str) -> Vec<String> {
    let pattern = "-?\\d+\\.?\\d*";
    let numbers = DepylerRegexMatch::findall(pattern, text);
    println!(
        "{}",
        format!("Found {} numbers: {:?}", numbers.len() as i32, numbers)
    );
    numbers
}
...
pub fn main() {
    ...
    match &args.command {
        Some(Commands::Match {
            ...
            match_pattern(&pattern, &text, args.ignore_case);
        }
        Some(Commands::Search {
            ...
            search_pattern(&pattern, &text, args.ignore_case);
        }
        Some(Commands::Findall {
            ...
            find_all(&pattern, &text, args.ignore_case);
        }
        Some(Commands::Sub {
            ref pattern,
            ref replacement,
            ...
            substitute(&pattern, &replacement, &text, args.ignore_case);
        }
        Some(Commands::Email { ref address, .. }) => {
            if !validate_email(&address) {
                std::process::exit(1);
            }
        }
        Some(Commands::Numbers { ref text, .. }) => {
