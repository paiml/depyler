...
enum Commands {
    ...
    Info { path: String },
    Parts { path: String },
    Glob { directory: String, pattern: String },
    Join { parts: Vec<String> },
}
#[derive(Default)]
...
#[doc = "Show path info. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn cmd_info(path: &str) {
    ...
    println!("{}", format!("Exists: {}", p.exists()));
}
#[doc = "Glob pattern match. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_glob(directory: &str, pattern: &str) {
    let p = std::path::PathBuf::from(&directory);
    for r#match in vec![std::path::PathBuf::from(&format!("{}/{}", p.display(), pattern))
        .unwrap()
        .filter_map(|e| e.ok())
        .collect::<Vec<_>>()
    {
        println!("{}", r#match.display());
    }
}
#[doc = "Show path parts. Depyler: proven to terminate"]
#[doc = " Depyler: verified panic-free"]
pub fn cmd_parts(path: &str) {
    let p = std::path::PathBuf::from(&path);
    for part in p
        .components()
...
#[doc = "Join path components. Depyler: proven to terminate"]
pub fn cmd_join(parts: &[String]) -> Result<(), Box<dyn std::error::Error>> {
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match &args.command {
        ...
        Some(Commands::Glob {
            ...
            cmd_glob(directory, pattern);
        }
        Some(Commands::Parts { ref path, .. }) => {
            cmd_parts(path);
        }
