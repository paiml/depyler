...
#![allow(dead_code)]
#[doc = "// NOTE: Map Python module 'collections.abc'(tracked in DEPYLER-0424)"] #[derive(Debug, Clone)] pub struct ZeroDivisionError {
    message: String ,
}
...
#[doc = r" Simple string split(NASA mode alternative to regex split)"] pub fn split(pattern: & str, text: & str) -> Vec<String>{
    ...
    #[doc = "Target value to search"] target: i32, #[doc = "Sorted values to search in"] values: Vec<i32>, #[doc = "Search mode"] mode: String, #[doc = "Search rotated array"] rotated: bool, #[doc = "Find peak element"] peak: bool
}
#[doc = r" Stub for local import from module: #module_name"] #[doc = r" DEPYLER-0615: Generated to allow standalone compilation"] #[allow(dead_code, unused_variables)] pub fn Callable<T: Default>(_args: impl std::any::Any) -> T {
    Default::default()
}
#[doc = "Basic binary search. Returns index or -1 if not found."] pub fn binary_search<'b, 'c, 'a>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue, key: & 'c Option<impl Fn()>) -> Result<i32, Box<dyn std::error::Error>>{
    let(mut left, mut right)  = (0 ,(arr.len() as i32).saturating_sub(1));
    while left <= right {
    let mid = {
    let a = left + right;
    let b = 2;
    ...
    let val = if key.is_some() {
    DepylerValue::Str(format!("{:?}", key(arr.get(mid as usize).cloned().expect("IndexError: list index out of range"))))
}
else {
    arr.get(mid as usize).cloned().expect("IndexError: list index out of range") };
    let target_val = if key.is_some() {
    DepylerValue::Str(format!("{:?}", key(target)))
}
else {
    target };
    if val == target_val {
...
#[doc = "Find leftmost position where target could be inserted."] pub fn binary_search_left<'c, 'a, 'b>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue, key: & 'c Option<impl Fn()>) -> Result<i32, Box<dyn std::error::Error>>{
...
#[doc = "Find rightmost position where target could be inserted."] pub fn binary_search_right<'b, 'a, 'c>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue, key: & 'c Option<impl Fn()>) -> Result<i32, Box<dyn std::error::Error>>{
    let mut left: i32 = Default::default();
    let(mut left, mut right)  = (0, arr.len() as i32);
    while left<right {
    let mid = {
...
#[doc = "Count occurrences of target in sorted array."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn count_occurrences<'c, 'b, 'a>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue, key: & 'c Option<impl Fn()>) -> Result<i32, Box<dyn std::error::Error>>{
    let left_idx = binary_search_left(& arr, target, key) ?;
    let right_idx = binary_search_right(& arr, target, key) ?;
    Ok(right_idx - left_idx)
}
#[doc = "Find the range [start, end) of target in sorted array."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn find_range<'b, 'c, 'a>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue, key: & 'c Option<impl Fn()>) -> Result <(i32, i32), Box<dyn std::error::Error>>{
    let left_idx = binary_search_left(& arr, target, key) ?;
    let right_idx = binary_search_right(& arr, target, key) ?;
    let _cse_temp_0 = left_idx == right_idx;
    if _cse_temp_0 {
    return Ok((- 1, - 1));
   
}
Ok((left_idx, right_idx - 1))
}
#[doc = "Search in a rotated sorted array."] pub fn search_rotated<'b, 'a>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue) -> Result<i32, Box<dyn std::error::Error>>{
    if arr.is_empty() {
    return Ok(- 1);
...
if arr.get(left as usize).cloned().expect("IndexError: list index out of range") <= arr.get(mid as usize).cloned().expect("IndexError: list index out of range") {
    if(arr.get(left as usize).cloned().expect("IndexError: list index out of range")<= (* target)) &&((* target)<arr.get(mid as usize).cloned().expect("IndexError: list index out of range")) {
    right = mid - 1;
    ...
    if(arr.get(mid as usize).cloned().expect("IndexError: list index out of range") <(* target)) &&((* target) <= arr.get(right as usize).cloned().expect("IndexError: list index out of range")) {
    left = mid + 1;
...
#[doc = "Find a peak element index(element greater than neighbors)."] pub fn find_peak(arr: & Vec<DepylerValue>) -> Result<i32, Box<dyn std::error::Error>>{
    let mut left: i32 = Default::default();
    if arr.is_empty() {
    return Ok(- 1);
...
let(mut left, mut right)  = (0 ,(arr.len() as i32).saturating_sub(1));
    ...
    if arr.get(mid as usize).cloned().expect("IndexError: list index out of range")<{
    let base = & arr;
    let idx: i32 = mid + 1;
...
#[doc = "Find index of element closest to target."] pub fn search_closest<'a, 'b>(arr: & 'a Vec<DepylerValue>, target: i32) -> Result<i32, Box<dyn std::error::Error>>{
    let mut left: i32 = Default::default();
    if arr.is_empty() {
    return Ok(- 1);
...
} let _cse_temp_0 = left == 0;
    if _cse_temp_0 {
    return Ok(0);
   
}
let _cse_temp_1 = arr.len() as i32;
    let _cse_temp_2 = left == _cse_temp_1;
    if _cse_temp_2 {
    return Ok((arr.len() as i32).saturating_sub(1) as i32);
   
}
let _cse_temp_3 = arr.get(left as usize).cloned().expect("IndexError: list index out of range") - * target;
    let _cse_temp_4  = (_cse_temp_3).abs();
...
#[doc = "Find largest element <= target."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn search_floor<'b, 'a>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue) -> Result<i32, Box<dyn std::error::Error>>{
    if arr.is_empty() {
    return Ok(- 1);
   
}
let idx = binary_search_right(& arr, target, & None) ?;
    let _cse_temp_0 = idx == 0;
    if _cse_temp_0 {
    return Ok(if arr.get(0usize).cloned().expect("IndexError: list index out of range")>(* target) {
    - 1
}
else {
    0 });
...
#[doc = "Find smallest element>= target."] #[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn search_ceil<'a, 'b>(arr: & 'a Vec<DepylerValue>, target: & 'b DepylerValue) -> Result<i32, Box<dyn std::error::Error>>{
    if arr.is_empty() {
    return Ok(- 1);
   
}
let idx = binary_search_left(& arr, target, & None) ?;
    let _cse_temp_0 = arr.len() as i32;
...
#[doc = " Depyler: verified panic-free"] #[doc = " Depyler: proven to terminate"] pub fn main () -> Result <(), Box<dyn std::error::Error>>{
    ...
    let mut idx: i32 = Default::default();
...
. split_whitespace().map(| s | s.to_string()).collect::<Vec<String>>().into_iter().map(| x |(x) as i32).collect::<Vec<_>>();
...
if args.peak {
    ...
    let _cse_temp_0 = idx>= 0;
    if _cse_temp_0 {
    println!("{}", format!("Peak at index {}: {}", idx, values.get(idx as usize).cloned().expect("IndexError: list index out of range")));
...
if args.rotated {
    ...
    let _cse_temp_1 = args.mode == "find";
    if _cse_temp_1 {
    idx = binary_search(& values, & args.target, & None) ?;
   
}
else {
    let _cse_temp_2 = args.mode == "left";
    if _cse_temp_2 {
    idx = binary_search_left(& values, & args.target, & None) ?;
   
}
else {
    let _cse_temp_3 = args.mode == "right";
    if _cse_temp_3 {
    idx = binary_search_right(& values, & args.target, & None) ?;
   
}
else {
    let _cse_temp_4 = args.mode == "count";
    if _cse_temp_4 {
    let count = count_occurrences(& values, & args.target, & None) ?;
    println!("{}", format!("Count: {:?}", count));
    return Ok(());
   
}
else {
    let _cse_temp_5 = args.mode == "range";
    if _cse_temp_5 {
    let(start, end) = find_range(& values, & args.target, & None) ?;
    let _cse_temp_6 = start == - 1;
    if _cse_temp_6 {
    println!("{}", "Not found");
   
}
else {
    println!("{}", format!("Range: [{:?}, {:?}]", start, end));
...
return Ok(());
    ...
    let _cse_temp_7 = args.mode == "closest";
    if _cse_temp_7 {
    idx = search_closest(& values, & args.target) ?;
   
}
else {
    let _cse_temp_8 = args.mode == "floor";
    if _cse_temp_8 {
    idx = search_floor(& values, & args.target) ?;
   
}
else {
    let _cse_temp_9 = args.mode == "ceil";
    if _cse_temp_9 {
...
let _cse_temp_10 = idx>= 0;
    if _cse_temp_10 {
    println!("{}", format!("Index: {}, Value: {}", idx, values.get(idx as usize).cloned().expect("IndexError: list index out of range")));
...
#[cfg(test)] mod tests {
    ...
    #[test] fn test_find_peak_examples() {
    assert_eq!(find_peak(& vec! []), 0);
    assert_eq!(find_peak(& vec! [1]), 1);
    assert_eq!(find_peak(& vec! [1, 2, 3]), 3);
