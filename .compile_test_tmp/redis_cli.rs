...
pub struct RESPType {}
impl RESPType {
    pub const SIMPLE_STRING: i32 = "+".to_string();
    pub const ERROR: i32 = "-".to_string();
    pub const INTEGER: i32 = ":".to_string();
    pub const BULK_STRING: i32 = "$".to_string();
    pub const ARRAY: i32 = "*".to_string();
    pub const NULL: i32 = "_".to_string();
    pub const BOOLEAN: i32 = "#".to_string();
    pub const DOUBLE: i32 = ",".to_string();
    pub const BIG_NUMBER: i32 = "(".to_string();
    pub const BULK_ERROR: i32 = "!".to_string();
    pub const VERBATIM_STRING: i32 = "=".to_string();
    pub const MAP: i32 = "%".to_string();
    pub const SET: i32 = "~".to_string();
    pub const PUSH: i32 = ">".to_string();
    pub fn new() -> Self {
        Self {}
    }
}
...
impl RESPValue {
    pub fn new(type_: RESPType, value: DepylerValue) -> Self {
        Self { type_, value }
    }
}
#[derive(Debug, Clone)]
...
impl RedisCommandBuilder {
    ...
    pub fn get(key: String) -> Vec<u8> {
        return encode_command(&["GET".to_string(), key]);
    }
    pub fn set(key: String, value: String, ex: Option<i32>, px: Option<i32>) -> Vec<u8> {
        let mut args = vec![key, value];
        if ex.is_some() {
            args.extend(vec!["EX".to_string(), ex.to_string()]);
        } else {
            if px.is_some() {
                args.extend(vec!["PX".to_string(), px.to_string()]);
            };
        };
        return encode_command(&["SET".to_string(), args]);
    }
    pub fn del_key(keys: Vec<String>) -> Vec<u8> {
        return encode_command(&["DEL".to_string(), keys]);
    }
    pub fn mget(keys: Vec<String>) -> Vec<u8> {
        return encode_command(&["MGET".to_string(), keys]);
    }
    pub fn mset() -> Vec<u8> {
        let mut args = vec![];
        ...
        return encode_command(&["MSET".to_string(), args]);
    }
    pub fn incr(key: String) -> Vec<u8> {
        return encode_command(&["INCR".to_string(), key]);
    }
    pub fn decr(key: String) -> Vec<u8> {
        return encode_command(&["DECR".to_string(), key]);
    }
    pub fn lpush(key: String, values: Vec<String>) -> Vec<u8> {
        return encode_command(&["LPUSH".to_string(), key, values]);
    }
    pub fn rpush(key: String, values: Vec<String>) -> Vec<u8> {
        return encode_command(&["RPUSH".to_string(), key, values]);
    }
    pub fn lrange(key: String, start: i32, stop: i32) -> Vec<u8> {
    ...
    pub fn hset(key: String, field: String, value: String) -> Vec<u8> {
        return encode_command(&["HSET".to_string(), key, field, value]);
    }
    pub fn hget(key: String, field: String) -> Vec<u8> {
        return encode_command(&["HGET".to_string(), key, field]);
    }
    pub fn hgetall(key: String) -> Vec<u8> {
        return encode_command(&["HGETALL".to_string(), key]);
    }
    pub fn sadd(key: String, members: Vec<String>) -> Vec<u8> {
        return encode_command(&["SADD".to_string(), key, members]);
    }
    pub fn smembers(key: String) -> Vec<u8> {
        return encode_command(&["SMEMBERS".to_string(), key]);
    }
    pub fn zadd(key: String, score: f64, member: String) -> Vec<u8> {
        return encode_command(&["ZADD".to_string(), key, score.to_string(), member]);
    }
    pub fn zrange(key: String, start: i32, stop: i32, withscores: bool) -> Vec<u8> {
        let mut args = vec![key, start.to_string(), stop.to_string()];
        if withscores {
            args.push("WITHSCORES".to_string());
        };
        return encode_command(&["ZRANGE".to_string(), args]);
    }
    pub fn ping(message: Option<String>) -> Vec<u8> {
        if message {
            return encode_command(&["PING".to_string(), message]);
        };
        return encode_command(&["PING".to_string()]);
    }
    pub fn info(section: Option<String>) -> Vec<u8> {
        if section {
            return encode_command(&["INFO".to_string(), section]);
        };
        return encode_command(&["INFO".to_string()]);
    }
}
#[derive(Default)]
...
struct Args {
    ...
    #[doc = "Hex-encoded RESP data to parse"]
    hex: Option<String>,
}
...
#[doc = "Encode simple string(+OK\r\n)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_simple_string(s: String) -> Vec<u8> {
    format!("+{}\r\n", s).as_bytes().to_vec()
}
#[doc = "Encode error message(-ERR message\r\n)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_error(s: String) -> Vec<u8> {
    format!("-{}\r\n", s).as_bytes().to_vec()
}
#[doc = "Encode integer(:<number>\r\n)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_integer(n: i32) -> Vec<u8> {
    format!(":{}\r\n", n).as_bytes().to_vec()
}
#[doc = "Encode bulk string($<len>\r\n<data>\r\n or $-1\r\n for null)."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_bulk_string(s: &Option<String>) -> Vec<u8> {
    if s.is_none() {
        return b"$-1\r\n";
    }
    ...
    format!(
        ...
        format!(
            "{}{}",
            format!("${}\r\n", data.len() as i32).as_bytes().to_vec(),
            data
        ),
        b"\r\n"
    )
}
#[doc = "Encode array(*<count>\r\n<items>or *-1\r\n for null)."]
#[doc = " Depyler: verified panic-free"]
pub fn encode_array(items: &Option<Vec<DepylerValue>>) -> Vec<u8> {
    let mut result: DepylerValue = Default::default();
    if items.is_none() {
        return b"*-1\r\n";
    }
    result = format!("*{}\r\n", items.len() as i32).as_bytes().to_vec();
    for item in items.iter().cloned() {
        result = format!("{}{}", result, encode_value(item));
    }
    result
}
#[doc = "Encode any value to RESP format."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_value(value: &DepylerValue) -> Vec<u8> {
    let mut result: DepylerValue = Default::default();
    if value.is_none() {
    ...
    if true {
        let _cse_temp_0 = value.contains("\r");
    ...
    if true {
        return encode_array(&Some(value.into_iter().collect::<Vec<_>>()));
    }
    if true {
        result = format!("%{}\r\n", value.len() as i32).as_bytes().to_vec();
        for (k, v) in value
            .iter()
            .map(|(k, v)| (k.clone(), v.clone()))
            .collect::<Vec<_>>()
        {
            result = format!("{}{}", result, encode_value(&k));
            result = format!("{}{}", result, encode_value(&v));
        }
        return result;
    }
    if true {
    ...
    encode_bulk_string(&Some((value).to_string()))
}
#[doc = "Parse simple string."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_simple_string(
    data: &Vec<u8>,
    pos: i32,
) -> Result<(String, i32), Box<dyn std::error::Error>> {
    let end = data[pos as usize..]
        .find(&b"\r\n")
        .map(|i| (i + pos as usize) as i32)
        .unwrap_or(-1);
    let _cse_temp_0 = end.unwrap_or_default() == -1;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new(
            "Incomplete simple string".to_string(),
        )));
    }
    Ok((
...
#[doc = "Parse error message."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_error(data: &Vec<u8>, pos: i32) -> Result<(String, i32), Box<dyn std::error::Error>> {
    let end = data[pos as usize..]
        .find(&b"\r\n")
        .map(|i| (i + pos as usize) as i32)
        .unwrap_or(-1);
    let _cse_temp_0 = end.unwrap_or_default() == -1;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new("Incomplete error".to_string())));
    }
    Ok((
        String::from_utf8_lossy(&{
            let base = &data;
...
#[doc = "Parse integer."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_integer(data: &Vec<u8>, pos: i32) -> Result<(i32, i32), Box<dyn std::error::Error>> {
    let end = data[pos as usize..]
        .find(&b"\r\n")
        .map(|i| (i + pos as usize) as i32)
        .unwrap_or(-1);
    let _cse_temp_0 = end.unwrap_or_default() == -1;
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new("Incomplete integer".to_string())));
    }
    Ok((
        (String::from_utf8_lossy(&{
            let base = &data;
...
pub fn parse_bulk_string(
    ...
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new(
            "Incomplete bulk string length".to_string(),
        )));
    }
    let _cse_temp_1 = (String::from_utf8_lossy(&{
        let base = &data;
        let start_idx = (pos) as isize;
        let stop_idx = (end) as isize;
        let start = if start_idx < 0 {
    ...
    .to_string()) as i32;
    let length = _cse_temp_1;
    let _cse_temp_2 = length == -1;
    if _cse_temp_2 {
        return Ok((None, end + 2));
    }
    let start = end + 2;
    let string_end = start + length;
    let _cse_temp_3 = data.len() as i32;
    let _cse_temp_4 = _cse_temp_3 < string_end + 2;
    if _cse_temp_4 {
        return Err(Box::new(ValueError::new(
            "Incomplete bulk string data".to_string(),
        )));
    }
    Ok((
        String::from_utf8_lossy(&{
            ...
            let stop_idx = (string_end) as isize;
            let start = if start_idx < 0 {
                (base.len() as isize + start_idx).max(0) as usize
            } else {
                start_idx as usize
...
pub fn parse_array(
    data: &Vec<u8>,
    pos: i32,
) -> Result<(Option<Vec<DepylerValue>>, i32), Box<dyn std::error::Error>> {
    let mut current_pos: i32 = Default::default();
    let end = data[pos as usize..]
        .find(&b"\r\n")
        .map(|i| (i + pos as usize) as i32)
        .unwrap_or(-1);
    ...
    for __sanitized in 0..(count) {
        let (value, mut current_pos) = parse_value(data, current_pos)?;
        items.push(DepylerValue::Str(format!("{:?}", value)));
    }
    Ok((items, current_pos))
}
#[doc = "Parse any RESP value."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_value(
    data: &Vec<u8>,
    mut pos: i32,
) -> Result<(DepylerValue, i32), Box<dyn std::error::Error>> {
    let mut end: i32 = Default::default();
    let mut value: DepylerValue = Default::default();
    let _cse_temp_0 = data.len() as i32;
    let _cse_temp_1 = pos >= _cse_temp_0;
    if _cse_temp_1 {
        return Err(Box::new(ValueError::new(
            "Unexpected end of data".to_string(),
        )));
    }
    let type_byte = char::from_u32(
        (data
            .get(pos as usize)
            .cloned()
            .expect("IndexError: list index out of range")) as u32,
    )
    .unwrap()
    .to_string();
    pos = pos + 1;
    let _cse_temp_2 = type_byte == "+";
    if _cse_temp_2 {
        return Ok(parse_simple_string(&data, pos));
    }
    let _cse_temp_3 = type_byte == "-";
    if _cse_temp_3 {
        let (error, new_pos) = parse_error(&data, pos)?;
        return Ok((RESPValue::new(RESPType::ERROR, error), new_pos));
    }
    let _cse_temp_4 = type_byte == ":";
    if _cse_temp_4 {
        return Ok(parse_integer(&data, pos));
    }
    let _cse_temp_5 = type_byte == "$";
    if _cse_temp_5 {
        return Ok(parse_bulk_string(&data, pos));
    }
    let _cse_temp_6 = type_byte == "*";
    if _cse_temp_6 {
        return Ok(parse_array(&data, pos));
    }
    let _cse_temp_7 = type_byte == "#";
    if _cse_temp_7 {
        end = Some(
            data[pos as usize..]
                .find(&b"\r\n")
        ...
        let _cse_temp_8 = String::from_utf8_lossy(&{
            let base = &data;
            let start_idx = (pos) as isize;
            let stop_idx = (end) as isize;
            let start = if start_idx < 0 {
    ...
    if _cse_temp_9 {
        ...
        let _cse_temp_10 = (String::from_utf8_lossy(&{
            let base = &data;
            let start_idx = (pos) as isize;
            let stop_idx = (end) as isize;
            let start = if start_idx < 0 {
...
#[doc = "Encode Redis command to RESP array format."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn encode_command<'a, 'b>(name: &'a str, args: &[String]) -> Vec<u8> {
    let _cse_temp_0 = vec![name]
        .iter()
        .chain(args.into_iter().collect::<Vec<_>>().iter())
        .cloned()
        .collect::<Vec<_>>();
    let items = _cse_temp_0;
    encode_array(&items)
}
#[doc = "Parse Redis command from RESP format."]
#[doc = " Depyler: proven to terminate"]
pub fn parse_command(data: &Vec<u8>) -> Result<(String, Vec<String>), Box<dyn std::error::Error>> {
    let (value, __sanitized) = parse_value(&data, 0i32)?;
    let _cse_temp_0 = (!true) || (!value);
    if _cse_temp_0 {
        return Err(Box::new(ValueError::new(
...
#[doc = "Format RESP value for display."]
#[doc = " Depyler: verified panic-free"]
#[doc = " Depyler: proven to terminate"]
pub fn format_value(value: &DepylerValue, indent: i32) -> String {
    let _cse_temp_0 = "  ".repeat(indent as usize);
    let prefix = _cse_temp_0;
    if value.is_none() {
        return format!("{}(nil)", prefix);
    }
    if true {
        let _cse_temp_1 = value.r#type == RESPType::ERROR;
        if _cse_temp_1 {
            return format!("{}(error) {}", prefix, value.value);
        }
    ...
    if true {
        return format!("{}(integer) {}", prefix, value);
    }
    if true {
        return format!("{}(double) {}", prefix, value);
    }
    if true {
    ...
    if true {
        ...
        let mut lines = vec![format!("{}(array)", prefix)];
        for (i, item) in value
            .iter()
            .cloned()
            .enumerate()
            .map(|(i, x)| (i as i32, x))
        {
            ...
            lines.push(format!(
                ...
                prefix,
                i + 1,
                format_value(&item, indent + 1).trim().to_string()
            ));
        }
        return lines.join("\n");
    }
...
pub fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _cse_temp_0 = args.mode == "encode";
    let mut encoded;
    let mut value: i32;
    if _cse_temp_0 {
        value = if args.args {
        ...
        let _cse_temp_1 = args.mode == "parse";
        if _cse_temp_1 {
            if has_hex {
                let data = (args.hex.replace(" ", ""))
                    .as_bytes()
                    .chunks(2)
                    .map(|c| u8::from_str_radix(std::str::from_utf8(c).unwrap(), 16).unwrap())
                    .collect::<Vec<u8>>();
                let (mut value, __sanitized) = parse_value(&data, 0i32)?;
                println!("{}", format_value(value, 0i32));
            }
        } else {
            let _cse_temp_2 = args.mode == "command";
            if _cse_temp_2 {
                encoded = encode_command(&[&args.cmd, &args.args]);
                println!(
                    "{}",
                    format!("Command: {} {}", args.cmd, args.args.join(" ").display())
                );
                println!("{}", format!("RESP: {:?}", encoded));
                println!(
                    "{}",
                    format!(
                        "Hex: {}",
                ...
                let _cse_temp_3 = args.mode == "builder";
                if _cse_temp_3 {
                    let builder = RedisCommandBuilder::new();
                    let examples = vec![
                        ("GET".to_string(), builder.get("mykey".to_string())),
                        (
                            "SET".to_string(),
                            builder.set("mykey".to_string(), "myvalue".to_string()),
                        ),
                        ("PING".to_string(), builder.ping()),
                        (
                            "LPUSH".to_string(),
                            builder.lpush(
                                "mylist".to_string(),
                    ...
                    for (name, encoded) in examples.iter().cloned() {
                        println!("{}", format!("\n{:?}:", name));
                        println!("{}", format!("  RESP: {:?}", encoded));
                        println!(
                            "{}",
